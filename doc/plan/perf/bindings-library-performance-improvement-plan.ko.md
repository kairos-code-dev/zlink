...
| `ws` | `MULTI_SPOT` | `보류(50.6%)` | `보류(50.4%)` | `통과(82.5%)` | `통과(115.5%)` | `보류(67.2%)` | `보류(57.2%)` | 64/256/1024B는 기존 파일 기준이다. 65536B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_022730.txt` 대비 Rust `perf_rust_multi_linux_20260525_025506.txt` 기준이다. 131072/262144B는 C `perf_c_multi_linux_20260525_063244.txt` 대비 Rust `perf_rust_multi_linux_20260525_063534.txt` 기준이다. SPOT server active publish를 `Message::with_size()` + `data_mut()` 직접 stamp로 바꾼 뒤 65536B가 통과권에 올랐다. 131072/262144B는 ws large 기본 recv worker를 6개로 좁혀 기존 worker 4 기본값 대비 처리량을 올렸지만 아직 SPOT 기준 아래다. SPOT client 복사 제거 + worker drain 위에, 131072B/262144B에서만 public `Poller` `POLLIN` wait를 사용한다. |
...
- **MULTI_SPOT ws large recv worker 6개 선택 적용**: ws 131072/262144B는 worker 4 기본값에서 각각 421.674/220.784Kmsg/s(`perf_rust_multi_linux_20260525_063450.txt`)였고, `PERF_MULTI_SPOT_RECV_WORKERS=6` 후보는 605.424/332.445Kmsg/s(`perf_rust_multi_linux_20260525_063416.txt`)로 올랐다. 이를 ws 131072B 이상 기본값으로 좁혀 반영한 공식 runner `perf_rust_multi_linux_20260525_063534.txt`는 C `perf_c_multi_linux_20260525_063244.txt` 대비 67.2/57.2%다. 아직 SPOT 기준에는 못 미치지만 절대 처리량 개선이 커서 반영하고 보류 상태를 유지한다.
...
- **남은 보류(Rust)**: SPOT tcp 64B 및 tcp/ws large(copy 영역), PUBSUB small/65536B(단일 thread drain).
