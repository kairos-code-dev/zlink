...
GUIDE_PATH="${SCRIPT_DIR}/discovery-service-execution-guide.ko.md"
MASTER_PLAN_PATH="${SCRIPT_DIR}/raw-socket-service-discovery-spec.ko.md"
LOGS_DIR="${SCRIPT_DIR}/logs"
MAX_ITERATIONS=100
POLL_SECONDS=30
STRESS_COUNT=1
GATE_LABEL="discovery_service_gate"
MODEL_ARG=()
LOCK_ACQUIRED=0
CAMPAIGN_LOCK_DIR=""
CAMPAIGN_LOCK_OWNER=""
...
Run the discovery/registry service execution campaign from the discovery doc directory.
The execution guide authority is:
  ${GUIDE_PATH}

The implementation/spec authority is:
  ${MASTER_PLAN_PATH}
...
Examples:
  ./run_discovery_service_execution.sh
  ./run_discovery_service_execution.sh --model gpt-5.4 --stress-count 10
EOF
}
...
CAMPAIGN_LOCK_DIR="${LOGS_DIR}/discovery_service_execution.lock"
...
if mkdir "${CAMPAIGN_LOCK_DIR}" 2>/dev/null; then
  ...
  if [[ -n "${existing_pid}" ]] && kill -0 "${existing_pid}" 2>/dev/null; then
    echo "Another discovery service execution campaign is already running (pid=${existing_pid})." >&2
    exit 16
  fi
  rm -rf "${CAMPAIGN_LOCK_DIR}"
...
echo "=== Discovery service execution start ==="
echo "Root: ${ROOT_DIR}"
