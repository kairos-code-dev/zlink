...
inline bool wait_for_socket_monitor_event(void *monitor_,
                                          ...
                                          int timeout_ms_)
{
    return wait_for_socket_monitor_event_with_activity (
      monitor_, NULL, success_event_, timeout_ms_);
}
  ...
  int timeout_ms_)
{
    ...
    if (activity_socket_
        && zlink_poller_add(poller, activity_socket_, activity_socket_, ZLINK_POLLIN)
             != 0) {
        (void) zlink_poller_destroy(&poller);
        return false;
    }
