...
bool parse_routing_id_hex_local (const std::string &hex_, std::string *route_id_out_)
{
    if (!route_id_out_ || hex_.empty () || (hex_.size () % 2) != 0 || hex_.size () / 2 > 255)
        return false;
                         ...
                         std::string *endpoint_out_)
{
    ...
    const std::string hex = arg_.substr (first_sep + 1, second_sep - first_sep - 1);
    std::string route_id;
    ...
    if (route_id_out_)
        route_id_out_->swap (route_id);
    if (endpoint_out_)
        endpoint_out_->assign (arg_.data () + second_sep + 1, arg_.size () - second_sep - 1);
    return true;
}
...
int apply_client_tls (spot_node_t *node_, spot_runtime_t *runtime_)
{
    ...
    if (runtime_->external_router
        && spot_node_access_t::apply_tls_client (node_, runtime_->external_router, ca, host, trust)
             != 0) {
        return -1;
    }
    return 0;
                  ...
                  const std::string *route_id_)
{
    ...
    const std::string route_id = route_id_ && !route_id_->empty () ? *route_id_ : endpoint_;

    if (runtime_->external_router->setsockopt (ZLINK_INTERNAL_OPT_ROUTING_ID, node_rid.data,
                                               node_rid.size)
        != 0)
        return -1;
