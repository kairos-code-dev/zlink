...
|----------|----------------|-------------------|------|------|
| YD-A1 | `Client/Scenarios/YdA1BasicTerminatorScenario.cs` | `Client/Scenarios/yd-a1-basic-terminator-scenario.ts`, `Server/Play/Handlers/basic-spot-handlers.ts`, `Server/Session/Handlers/yield-session.ts` | done | stream connector와 Session gateway 경로를 구현했다. `logs/20260702-051530-20410`에서 marker order 검증 통과 |
| YD-A2 | `Client/Scenarios/YdA2YieldTerminatorScenario.cs` | `Client/Scenarios/yd-a2-yield-terminator-scenario.ts`, `Server/Play/Handlers/basic-spot-handlers.ts`, `Server/Session/Handlers/yield-session.ts` | done | `ZLinkRequestCall.yield<TReply>()`를 쓰는 Spot handler를 추가했다. `logs/20260702-051530-20410`에서 yield/probe marker order 검증 통과 |
...
| YD-D1 | `run_e2e.sh`, `Client/Scenarios/YdA1BasicTerminatorScenario.cs` | `run_e2e.sh`, YD-A1/YD-C3/YD-E1 범위 | done | `play-a`와 `delay-a` local topology에서 A/B/C/E1 marker를 검증했다. `logs/20260702-051530-20410`에서 통과 |
