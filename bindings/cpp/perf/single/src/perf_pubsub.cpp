                ...
                perf::single::latency_stats_t *latency_out)
{
    ...
    auto do_one = [&] () -> bool {
        ...
        bool header_ok = false;
        const int recv_rc = recv_header_single_part (
          subscriber, payload_size, zlink::recv_flag::none, &header, &header_ok);
        if (recv_rc <= 0)
            return false;
        if (queue_probe)
                         ...
                         const std::string &lib_name)
{
    ...
    perf::single::socket_guard_t publisher (ctx, zlink::socket_type::pub);
    perf::single::socket_guard_t subscriber (ctx, zlink::socket_type::sub);
    if (!publisher.valid () || !subscriber.valid ()) {
        perf::single::print_fail_result (lib_name, "PUBSUB", transport, msg_size);
        return;
    }

    (void) publisher.sock ().set (zlink::socket_options::xpub_nodrop, 1);
    (void) subscriber.sock ().set (zlink::socket_options::subscribe, std::string ());

    if (!perf::single::setup_connected_pair (publisher.sock (),
                                             subscriber.sock (),
                                             transport,
                                             lib_name + "_pubsub")) {
        perf::single::print_fail_result (lib_name, "PUBSUB", transport, msg_size);
    ...
    perf::single::queue_probe_t queue_probe (&publisher.sock (),
                                             &subscriber.sock ());
