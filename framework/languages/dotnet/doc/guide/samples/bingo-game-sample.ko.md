...
actor 준비 응답은 logical actor handle 을 돌려주고, Session 서버는 현재 STREAM session 을
그 actor handle 에 bind 한다. actor-session binding 은 framework / core runtime 내부 상태로
관리된다.
...
4. `SessionServer` 는 `PlayServer` 로 player actor 준비를 요청하고, 응답으로
   actor id/type 을 받는다.
5. `SessionServer` 는 logical actor handle 을 받는 `BindActorHandleAsync(...)` 호출로
   현재 stream session 과 actor 를 bind 한다. 실제 actor 위치 해석은 core ActorGateway 가
   맡으므로 session handler 는 actor route 주소를 저장하지 않는다.
