...
>
> 현재 구현 기준은
> [Session Actor Dispatch 사용성 초안](./session-gateway-usability.ko.md)이다.
...
| 이전 이름 | 현재 대체 | 상태 |
|-----------|-----------|------|
| `EnableSessionGateway(...)` | `AddStreamNode(...).AddHeaderSession<TSession>()`, `CreateRemoteActorAsync(...)` | 제거 |
| `IZLinkSessionGateway` | `IZLinkSessionContext`, `IZLinkActorClient`, `IZLinkSessionProxy` | 제거 |
| `AddSessionProxyHandler(...)` | `IZLinkActorSendHandler<T>`, `IZLinkActorRequestHandler<TReq,TReply>` | 제거 |
| `OpenActorRelay(...)` | `CreateActorAsync(...)`, `CreateRemoteActorAsync(...)`, `DispatchToActorAsync(...)` | 제거 |
...
- session 위치는 `IZLinkActorSessionLocationWriter`와
  `IZLinkActorSessionRouteResolver`가 관리한다. registry metadata를 쓰는 구현은
  sample adapter이며 framework 기본 저장소가 아니다.
...
- [Session Actor Dispatch 사용성 초안](./session-gateway-usability.ko.md)
