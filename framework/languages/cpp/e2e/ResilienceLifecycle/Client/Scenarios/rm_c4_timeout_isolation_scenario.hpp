...
inline void run_rm_c4_timeout_isolation_scenario (zlink::framework::channel_client_t &channels)
{
    ...
    auto after = channels.request (api_channel, profile_req_t{.value = "rm-c4-after-timeout"})
                   .timeout (std::chrono::milliseconds (2000))
                   .async<profile_res_t> ();
    ensure (after.result ().has_value (), "RM-C4 post-timeout request failed");
    std::this_thread::sleep_for (std::chrono::milliseconds (1100));
    auto later = channels.request (api_channel, profile_req_t{.value = "rm-c4-later"})
                   .timeout (std::chrono::milliseconds (2000))
                   .async<profile_res_t> ();
