...
test('service objects expose aligned monitor and query surface', () => {
    ...
    assert.throws(() => registry.setSockOpt(zlink.RegistrySocketRole.PUB, zlink.SocketOption.LINGER, Buffer.alloc(4)), /not available on the aligned public API/);
    assert.throws(() => node.setDiscovery(discovery), /Use attachDiscovery/);
    spotMonitor.close();
    nodeMonitor.close();
