...
View in another language — [C#/.NET](../../../dotnet/guide/server/05-channel-messaging.en.md) · [C++](../../../cpp/guide/server/05-channel-messaging.en.md) · [Java](../../../java/guide/server/05-channel-messaging.en.md) · **Kotlin** · [Node/TypeScript](../../../node/guide/server/05-channel-messaging.en.md)
<!-- language-switch:end -->
...
class UserHandlers(private val publisher: ZLinkFanoutClient) {
    ...
    // A send handler -- returns nothing (no response).
    @ZLinkSend
...
class PriceService(private val client: ZLinkRouteClient) {
    ...
    suspend fun get(symbol: String): BigDecimal {
        val reply = client
            // The target is just one ChannelName. The reply type is specified in submit(T::class.java).
