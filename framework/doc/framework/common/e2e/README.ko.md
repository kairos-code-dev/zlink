...
| [Config 11 — 관측·운영 배포](config-11-observability-ops.ko.md) | location store + relocation store + Session + Play 2 + OrderWorkflow 2 | flow correlation 로그(STREAM→actor→spot 관통, error 라인, create-if-absent·off 전파, fan-out/timer origin), 런타임 메트릭(CCU·request in-flight·actor 이동·lease 계기, publish 전용 계기 부재, 카디널리티 규약, 비활성 최소 비용), `Relocate`·`Shutdown`, 무중단 patch, 동일 version 점검, eligible target blocker, Spot closing과 bounded teardown |
