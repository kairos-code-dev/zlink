...
Options:
  --guide PATH          Execution guide path
                        (default: ${GUIDE_PATH})
  --master-plan PATH    Optional main plan document.
                        When provided and different from --guide, use it as
                        the product/priority context while --guide remains the
                        execution authority.
  ...
  --check-existing-supervisor
                        Enable existing-supervisor cleanup/lock enforcement
                        before starting the loop (default: disabled)
...
- ${MASTER_PLAN_PATH} 는 이 실행의 main 문서다.
- main 문서는 무엇을 만들고 있는지, 현재 우선순위, 다음 단계 전제를 설명한다.
- ${GUIDE_PATH} 는 실행 authority 문서다.
- 따라서 product 목표/우선순위는 main 문서를 따르고,
  반복 순서/검증/종료 판정은 실행 가이드를 따른다.
EOF
)
  fi
...
- 실행 가이드:
  ${GUIDE_PATH}
- 위 두 문서 기준으로 남은 작업과 실제 구현 내용을 중단 없이 끝까지 진행한다.

작업 규칙:
- 현재 실행은 main 문서 1개와 실행 가이드 1개 체계로 운영한다.
- main 문서는 제품 목표/우선순위 설명용, 실행 가이드는 반복 실행용이다.
- 명시 요청이 없으면 main/guide 외에 별도 master/gap/spec/residual/보조 계획 문서를 새로 만들지 않는다.
- 문서가 더 필요해 보이면 먼저 main 또는 guide 안으로 흡수할 수 있는지 검토한다.
${legacy_plan_rules}
- 각 작업 묶음을 끝낼 때마다 main 문서와 실행 가이드 전체를 다시 훑고,
  아직 코드에 반영되지 않은 구현 항목이 남아 있는지 확인한다.
- 실행 가이드 하단의 historical 상태표/백로그/증거가 상단 규칙이나 main 문서와 충돌하면,
  하단 historical 메모는 참고만 하고 main 문서와 가이드 상단 규칙을 따른다.
- 실행 가이드 체크리스트가 green이어도 실제 구현 내용이 아직 덜 반영됐으면 완료로 처리하지 않는다.
- main 문서의 현재 우선순위와 어긋나면 그 iteration은 잘못 진행된 것이다.
- main 문서/실행 가이드와 현재 코드가 어긋나면 어느 문서가 틀렸는지 먼저 정리한 뒤 코드를 진행한다.
- 성능 회귀가 관측되면 새 실험 후보를 넓게 추가하기 전에
  최근 변경 검토, 원인 축소, 원복 가능한 최소 수정 또는 slice 축소를 먼저 시도한다.
- 기준선 복귀에 직접 기여한 원복/축소와 hot path 식별 결과는 관련 문서에 바로 반영한다.
- 실행 가이드는 반복 순서와 종료 판정의 source of truth다.
- main 문서는 제품 목표와 현재 우선순위의 source of truth다.
- 명시적으로 요청받지 않은 한 main/guide 자체의 git diff나 삭제된 옛 문서 diff를 근거로 판단하지 않는다.
- git status 에서 보이는 삭제된 옛 계획 문서는 현재 실행 범위의 근거가 아니다. 현재 main/guide와 현재 코드만 본다.
- 문서의 첫 미완료 항목부터 순서대로 진행한다.
- 단, `첫 미완료 항목` 판단은 historical note가 아니라
  main 문서의 현재 우선순위와 가이드 상단의 현재 선행 조건/판정 규칙을 기준으로 한다.
