...
{
public:
  void on_joined (const player_joined_notify_t &notify)
  {
    joined.push_back (notify);
  }

  void on_drawn (const number_drawn_notify_t &notify)
  {
    drawn.push_back (notify);
  }

  void on_started (const game_started_notify_t &notify)
  {
    started.push_back (notify);
  }

  void on_state (const state_notify_t &notify)
  {
    states.push_back (notify);
  }

  void on_ended (const game_ended_notify_t &notify)
  {
