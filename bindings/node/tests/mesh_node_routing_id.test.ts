...
test('MeshNode without channel membership uses node, peer, and shutdown surfaces', () => {
  ...
  try {
    ...
    try {
      assert.equal(
        node.sendToNode(zlink.RoutingId.from('missing-node'), payload),
        zlink.SubmitResult.NotConnected
      );
    } finally {
      payload.close();
    }
...
test('MeshNode bound-session send requires an explicit nonzero binding generation', () => {
  ...
  try {
    ...
    assert.equal(
      node.sendActorBoundSession(actor, 0n, Buffer.from('must-not-route')),
