...
| `RM-A3` | 구현 | 공통 connection policy를 production automatic planner와 manual handshake에 연결했다. 두 Object Client 모두 RouteMesh Channel Server membership이 없을 때만 `NotRequired`이며, weight `0`을 포함해 어느 한쪽에 Server membership이 있으면 연결이 필요하다. Actual `logs/20260729-014018-1256449`에서 Automatic pair 80회와 Manual pair 79회 상태 조회가 20초 동안 `NotRequired`, ready peer 0과 failure 0을 유지했다. Manual peer endpoint 앞의 TCP proxy는 양방향 connection open을 각각 1회만 기록했다. Object Client RID의 Node direct 결과는 `request_target_not_found`인 typed `NotFound`였다. Weight `0` Server descriptor가 ready인 상태에서 process를 정지하자 다른 Object Client가 `NotConnected`와 `no_ready_peer`를 표시했고, process 재개 뒤 양쪽 public status가 ready connection 하나를 표시했다. Runner 종료 뒤 process와 test Redis container가 남지 않았다. |
