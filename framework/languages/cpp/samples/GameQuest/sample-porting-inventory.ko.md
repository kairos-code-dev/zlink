...
| Java `Server/GameApi/Program.java` | `Server/GameApi/main.cpp` | done | stream session이 client request를 받고 owner QuestMission route로 요청한다. |
| Java `Server/QuestMission/Program.java` | `Server/QuestMission/main.cpp` | done | mission-a/mission-b가 같은 route mesh channel에 다른 routing id로 server를 연다. |
