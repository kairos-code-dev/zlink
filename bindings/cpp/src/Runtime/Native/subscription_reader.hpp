...
{
    static topic_message_t make_single (std::optional<routing_id_t> source_, std::string topic_, message_t part_)
    {
...
inline std::optional<routing_id_t> optional_native_routing_id (const zlink_routing_id_t *routing_id_)
{
    return routing_id_ && routing_id_->size > 0 ? std::optional<routing_id_t> (native_routing_id (*routing_id_))
                                                : std::nullopt;
}

inline routing_id_t routing_id_or_empty (const zlink_routing_id_t *routing_id_)
{
    return routing_id_ && routing_id_->size > 0 ? native_routing_id (*routing_id_) : unchecked_empty_routing_id ();
}
...
[[nodiscard]] int read_subscription_message (topic_message_t &message_out_, ReceivePart &&receive_part_)
{
    ...
    const int first_rc =
      receive_part_ (&source_rid, topic_buffer, sizeof (topic_buffer), &topic_length, native_part.get (), &has_more);
    ...
    if (has_more == ZLINK_PART_FINAL) {
        message_out_ = topic_message_access_t::make_single (source, std::move (topic), std::move (first_part));
        return 0;
    }
    ...
    for (;;) {
        ...
        const int rc = receive_part_ (&source_rid, nullptr, 0u, &ignored_topic_length, next_part.get (), &has_more);
        if (rc != ZLINK_RECV_OK)
            return rc;
        ...
        if (has_more == ZLINK_PART_FINAL) {
            message_out_ = topic_message_t (source, std::move (topic), std::move (parts));
            return 0;
        }
    }
}
