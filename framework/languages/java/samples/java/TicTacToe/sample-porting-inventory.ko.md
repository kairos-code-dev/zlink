...
| `.NET: Client/TicTacToeClientScenario.cs` | `Client/src/main/java/.../client/TicTacToeClientScenario.java` | validation | done | room 생성, host/guest/observer stream 연결, 인증, join, move, win, milestone push를 검증한다. |
...
| 기준 | Java 대응 | 분류 | 상태 | 비고 |
|------|-----------|------|------|------|
| `.NET: Server/Program.cs` | `Server/src/main/java/.../server/Program.java` | server-entry | done | `api`와 `play` role을 같은 Server project에서 선택해 실행한다. |
