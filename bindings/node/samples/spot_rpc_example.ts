...
// --8<-- [start:doc]
  ...
  try {
    ...
    const operationId = client.requestToSpot(
      serverNodeRid, serverSpotRid, serverSpotGeneration,
      Buffer.from('ping'), { timeoutMs: 3000 });

    // 두 노드를 번갈아 pump한다: 서버는 요청을 받아 응답하고, 클라이언트는 완료를 받는다.
    let completion = null;
    await waitUntil(() => {
      serverPump.drain((record) => {
        if (isRequestRecord(record)) record.reply(Buffer.from('pong'));
      });
    ...
    if (serverPump) serverPump.close();
    if (clientPump) clientPump.close();
