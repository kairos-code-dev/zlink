...
void discard_spot_parts (
  const zlink_routing_id_t *, const char *, size_t, zlink_msg_t *parts, size_t part_count, void *)
{
    if (!parts)
        return;
...
bool apply_spot_server_options (void *pub,
                                ...
                                size_t max_msg_size)
{
    const int sndtimeo_ms = bench_timeout_ms_from_env ("PERF_MULTI_SNDTIMEO_MS", 200);
    const int nodrop = resolve_multi_int_env ("PERF_MULTI_SPOT_XPUB_NODROP", 1, 0);
    apply_benchmark_socket_options (pub, settings.hwm, transport, ZLINK_SOCKET_DEALER,
                                    max_msg_size);

    if (zlink_set_option (pub, ZLINK_OPT_SNDTIMEO, &sndtimeo_ms, sizeof (sndtimeo_ms)) != 0
        || zlink_set_pub_option (pub, ZLINK_PUB_OPT_NODROP, &nodrop, sizeof (nodrop)) != 0) {
        return false;
    }
...
{
    return state
           && perf_multi_spot_control::ensure_connected_peers (state->control_node,
                                                               state->control_peers);
}

bool wait_for_ready_slots (spot_server_state_t *state, size_t msg_size, int timeout_ms)
{
    return state
           && perf_multi_spot_control::wait_for_ready_units (
             state->control_sub, k_topic, &state->ready_state, msg_size,
             state->expected_ready_count, timeout_ms, &state->fatal_errno);
}

bool publish_control_start (spot_server_state_t *state, size_t msg_size)
{
    return state && perf_multi_spot_control::publish_start (state->control_pub, k_topic, msg_size);
}

bool wait_for_size_start (spot_server_state_t *state, size_t msg_size, int timeout_ms)
{
    if (!state)
        return false;
...
{
    return resolve_multi_int_env ("PERF_MULTI_SPOT_LATENCY_PROBE_INTERVAL_US", 1000, 1);
}
...
{
    const int default_ms = std::max (1000, std::max (1, settings.duration_seconds) * 1000);
    return resolve_multi_int_env ("PERF_MULTI_SPOT_LATENCY_PROBE_SETTLE_MS", default_ms, 0);
}
                                  ...
                                  unsigned long long *publish_blocked_count)
{
    ...
    const size_t payload_size = std::max (state->msg_size, perf_multi_metric::header_size ());
    const auto publish_once = [&] (zlink_send_flags_t flags, int *saved_errno_out) -> int {
        if (!saved_errno_out) {
            errno = EFAULT;
            return -1;
        }
        ...
        if (!perf_multi_metric::stamp_payload (zlink_msg_data (&part), payload_size, state->run_id,
                                               state->phase, state->msg_size, state->next_seq,
                ...
                bool send_enabled)
{
    ...
    if (bench_transition_debug_enabled ()) {
        std::cerr << "[multi-spot-server] phase start ts_ns=" << perf_multi_metric::now_ns ()
                  << " size=" << msg_size << " phase=" << static_cast<int> (phase)
    ...
    const bool latency_only =
      resolve_spot_latency_only_mode () && phase == perf_multi_metric::phase_active && send_enabled;
    const bool probe_phase = phase == perf_multi_metric::phase_cooldown && send_enabled;
    const auto probe_interval =
      std::chrono::microseconds (latency_only ? resolve_spot_latency_only_interval_us ()
        ...
        if (state->send_enabled.load (std::memory_order_acquire)) {
            if (probe_phase || latency_only) {
                ...
                if (now < next_probe_at) {
                    const auto wait_ms =
                      std::chrono::duration_cast<std::chrono::milliseconds> (next_probe_at - now);
                    (void) perf_socket_poll (
                      NULL, 0, std::max<long> (1, static_cast<long> (wait_ms.count ())));
                    continue;
                }
            }
            const send_status_t rc =
              try_publish_locked (state, &publish_ok_count, &publish_blocked_count);
            if (rc == send_status_fatal) {
                state->fatal_errno.store (errno != 0 ? errno : EIO, std::memory_order_release);
                return false;
            }
    ...
    state->send_pending.store (false, std::memory_order_release);
    if (bench_transition_debug_enabled ()) {
        std::cerr << "[multi-spot-server] phase done ts_ns=" << perf_multi_metric::now_ns ()
                  << " size=" << msg_size << " phase=" << static_cast<int> (phase)
                  << " ok=" << publish_ok_count << " blocked=" << publish_blocked_count
                  << " wait=" << publish_wait_count
                  << " sent=" << (state->next_seq > 0 ? state->next_seq - 1 : 0)
                  << " stop=" << (perf_stop_requested ().load (std::memory_order_acquire) ? 1 : 0)
                  << " fatal_errno=" << state->fatal_errno.load (std::memory_order_acquire)
                  << std::endl;
    }
    if (state->fatal_errno.load (std::memory_order_acquire) != 0)
        return false;
                      ...
                      const std::vector<size_t> &msg_sizes)
{
    ...
    const int probe_settle_ms = resolve_spot_latency_probe_settle_ms (settings);
    ...
    for (size_t i = 0; i < msg_sizes.size (); ++i) {
        ...
        if (perf_stop_requested ().load (std::memory_order_acquire)) {
            if (bench_transition_debug_enabled ()) {
                std::cerr << "[multi-spot-server] loop stop before size=" << msg_sizes[i]
                          << std::endl;
            }
        ...
        if (bench_transition_debug_enabled ()) {
            std::cerr << "[multi-spot-server] ready wait begin ts_ns="
                      << perf_multi_metric::now_ns () << " size=" << msg_sizes[i]
        ...
        if (!wait_for_size_start (state, msg_sizes[i], start_timeout_ms)) {
            if (bench_transition_debug_enabled ()) {
                std::cerr << "[multi-spot-server] runner start timeout ts_ns="
                          << perf_multi_metric::now_ns () << " size=" << msg_sizes[i] << std::endl;
            }
            return false;
        }
        if (!ensure_control_peers_connected (state)) {
            if (bench_debug_enabled ()) {
                std::cerr << "[multi-spot-server] ensure control peers failed"
                          << " size=" << msg_sizes[i] << " err=" << zlink_errno () << std::endl;
            }
            return false;
        }
        if (!wait_for_ready_slots (state, msg_sizes[i], start_timeout_ms)) {
            if (bench_debug_enabled ()) {
                std::cerr << "[multi-spot-server] ready count timeout" << " size=" << msg_sizes[i]
                          << " expected=" << state->expected_ready_count << " got="
                          << perf_multi_spot_handshake::ready_units (&state->ready_state,
                                                                     msg_sizes[i])
        ...
        if (!run_phase (state, lib_name, transport, msg_sizes[i], static_cast<uint32_t> (i + 1),
                        perf_multi_metric::phase_active, active_seconds, true)) {
            if (bench_transition_debug_enabled ()) {
                std::cerr << "[multi-spot-server] loop abort size=" << msg_sizes[i] << " stop="
        ...
        if (probe_seconds > 0.0 && !wait_for_spot_probe_ready (state->pub, probe_settle_ms)) {
            if (bench_transition_debug_enabled ()) {
                std::cerr << "[multi-spot-server] probe ready wait failed size=" << msg_sizes[i]
                          << " settle_ms=" << probe_settle_ms << " err=" << zlink_errno ()
                          << std::endl;
            }
            return false;
        }
        if (probe_seconds > 0.0
            && !run_phase (state, lib_name, transport, msg_sizes[i], static_cast<uint32_t> (i + 1),
                           perf_multi_metric::phase_cooldown, probe_seconds, true)) {
            if (bench_transition_debug_enabled ()) {
                std::cerr << "[multi-spot-server] probe abort size=" << msg_sizes[i] << " stop="
                          << (perf_stop_requested ().load (std::memory_order_acquire) ? 1 : 0)
                          << " fatal_errno=" << state->fatal_errno.load (std::memory_order_acquire)
                          << std::endl;
            }
...
int run_server_benchmark (const std::string &lib_name, const std::string &transport)
{
    ...
    if (!perf_multi_spot_control::initialize_server_session (
          ctx, transport, lib_name + std::string ("_spot_server"), k_topic, settings,
          apply_spot_server_options, &session)) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-server] server session init failed err=" << zlink_errno ()
                      << std::endl;
        return 1;
    }
    ...
    if (bench_transition_debug_enabled ()) {
        std::cerr << "[multi-spot-server] phase gate open ts_ns=" << perf_multi_metric::now_ns ()
                  << std::endl;
    }

    const bool ok = run_server_loop (&state, ctx.get (), settings, lib_name, transport, msg_sizes);
    if (bench_transition_debug_enabled ()) {
