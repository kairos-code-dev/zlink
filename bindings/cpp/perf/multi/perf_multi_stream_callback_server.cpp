                       ...
                       size_t payload_size)
{
    ...
    return zlink_stream_send (g_server_socket, rid, payload, payload_size, 0)
...
int run_stream_callback_server (const std::string &pattern,
                                const std::string &transport,
                                size_t size)
{
    ...
    const std::string endpoint =
      perf_multi::make_multi_endpoint (transport, "multi-stream-callback-server");
    if (server.bind (endpoint) != 0)
        return 2;

    g_stop_requested.store (false, std::memory_order_release);
    g_callback_failed.store (false, std::memory_order_release);
    g_server_socket = server.handle ();

    if (zlink_stream_attach (g_server_socket, &on_stream_packets, 0)
        != 0) {
    ...
    if (!perf_multi::wait_monitor_ready (monitor, connect_ready_timeout_ms, true)) {
        (void) zlink_stream_detach (g_server_socket);
        g_server_socket = NULL;
        return 2;
    }
...
{
    return run_stream_callback_server ("MULTI_STREAM_CALLBACK", transport, size);
}
