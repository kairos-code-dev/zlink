...
const formalFixtures = [
  {
    ...
    required: [
      ...
      'CAS winner만 factory와 initialize를 실행하고 envelope message를 durable activation inbox의 첫 record로 확정한다.',
  ...
  },
  {
    ...
    required: [
      ...
      'Spot direct send/request의 시작 method는 global Spot ID만 대상으로 받는다.',
  ...
  },
  {
    ...
    required: [
      ...
      'Target의 CAS winner만 owner claim·factory를 만들고 durable inbox first record를 Ready 전에 확정한다.',
...
const projections = {
  dotnet: [
    ...
    'Local exact instance가 없을 때만 자신을 owner로 `Creating` row와 Spot 전체·exact Instance Spot type reserved capacity를 Reserve하며',
  ],
  cpp: [
    ...
    'Local exact instance가 없을 때만 자신을 owner로 generic Store Reserve를 수행하며',
  ],
  java: [
    ...
    'Instance가 없을 때만 자신을 owner로 `CREATING` authority와 Spot 전체·해당 Instance Spot stable type reserved slot을 하나의 typed bundle로 Reserve하며',
  ],
  kotlin: [
    'SpotId는 UTF-8 encoded 크기 1..255 bytes의 `String`이며 Location Store transaction domain 전체에서 유일한 logical ID다.',
    ...
    '`getOrCreate(spotId, spotType)`은 caller가 정한 User Spot ID를 사용한다.',
    'Manager는 Instance Spot create/get-or-create를 제공하지 않는다.',
    'Spot send/request는 global SpotId만 address로 받고 Java의 `ZLinkSpotSendCall` 또는 `ZLinkSpotRequestCall`을 반환한다.',
    ...
    'Source는 placement reservation을 만들지 않는다.',
    'Local exact instance가 없을 때만 자신을 owner로 Reserve하며',
  ],
  node: [
    ...
    'Instance가 없을 때만 자신을 owner로 Creating row와 Spot 전체·exact Instance Spot type reserved capacity를 Reserve하며',
  ],
};
