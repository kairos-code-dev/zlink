  ...
  if (!header) {
    return result_t<std::optional<route_dispatch_reply_t>>::failure (
      ...
      header.error () ? header.error ()->what ()
                      : "route envelope header decode failed");
  }
  ...
  const runtime::messaging::envelope_header_t &header) const
{
  ...
  framework::route_handler_context_t context { _router_channel_id,
                                               received.source_node_rid,
                                               header.message_name,
                                               header.content_type };
  auto dispatched = _invoker.invoke_send (*_handlers,
                                          _router_channel_id,
                                          header.message_name,
                                          *_services,
                                          *_serializers,
...
result_t<std::optional<route_dispatch_reply_t>>
route_packet_dispatcher_t::dispatch_request (
  const route_received_packet_t &received,
  const runtime::messaging::envelope_header_t &header) const
{
  if (_internal_packets != nullptr &&
  ...
  framework::route_handler_context_t context { _router_channel_id,
                                               ...
                                               header.message_name,
                                               header.content_type };
  auto reply = _invoker.invoke_request (*_handlers,
                                        _router_channel_id,
                                        header.message_name,
                                        *_services,
                                        *_serializers,
...
result_t<std::optional<route_dispatch_reply_t>>
route_packet_dispatcher_t::reply_error (
  const route_received_packet_t &received,
  ...
  const framework_exception_t &error) const
{
  ...
  auto reply = writer.reply_raw_envelope (
    writer.create_error_header (_router_channel_id, header, error),
    zlink::message_t::from (""));
  return result_t<std::optional<route_dispatch_reply_t>>::success (
    route_dispatch_reply_t { received.source_node_rid,
                             received.request_seq,
                             std::move (reply) });
}
