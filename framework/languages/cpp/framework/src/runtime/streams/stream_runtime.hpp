...
{
public:
  explicit stream_builder_state_t (std::string name)
...
public:
  ...
  result_t<std::vector<std::uint8_t>> encode_header (
    const stream_header_t &header) const;
  result_t<stream_header_t> decode_header (
  ...
  result_t<void> dispatch_connected (packet_stream_session_t &session,
                                     stream_t &stream) const;
  result_t<void> dispatch_packet (packet_stream_session_t &session,
                                  stream_t &stream,
                                  const stream_header_t &header,
                                  const zlink::message_t &payload) const;
  result_t<void> dispatch_disconnected (packet_stream_session_t &session,
                                        stream_t &stream) const;
