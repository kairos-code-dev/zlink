...
| `Client/` | PS-A1, PS-A2, PS-A3, PS-A4, PS-B1, PS-B2, PS-C1 scenario를 실행한다. |
...
`run_e2e.sh`는 Gradle `installDist`를 실행한 뒤 registry, publisher, subscriber, client binary를
각각 띄운다. 실패하면 `logs/<run-id>/` 아래 role별 stdout, stderr, message flow log를 출력한다.
