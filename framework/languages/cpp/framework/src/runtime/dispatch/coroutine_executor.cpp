...
std::unique_ptr<coroutine_executor_t> &
executor_instance ()
{
...
std::atomic<coroutine_executor_t *> &
executor_fast_path ()
{
...
handler_coroutine_executor ()
{
  ...
  if (!executor) {
    const auto workers =
      configured_worker_count () == 0 ? default_worker_count ()
                                      : configured_worker_count ();
...
void
configure_handler_coroutine_executor (std::size_t worker_count)
{
  if (worker_count == 0) {
