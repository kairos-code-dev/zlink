...
inline bool perf_auto_hwm_send_side_visible(uint32_t socket_type_,
                                            uint32_t role_)
{
...
inline bool perf_auto_hwm_recv_side_visible(uint32_t socket_type_,
                                            uint32_t role_)
{
    const char *role_name = perf_auto_hwm_role_name(role_);
    if ((socket_type_ == ZLINK_SOCKET_PUB || socket_type_ == ZLINK_SOCKET_XPUB)
        && (std::strcmp(role_name, "spot_data") == 0
            || std::strcmp(role_name, "control") == 0))
        return false;
    return true;
}
  ...
  size_t msg_size_)
{
    ...
    const char *owner =
      row_.owner == ZLINK_SPOT_NODE_SOCKET_OWNER_NODE ? "node" : "spot";
    const char *scope =
      row_.owner == ZLINK_SPOT_NODE_SOCKET_OWNER_NODE ? "shared" : "per-spot";

    std::cout << "AUTO_HWM_DETAIL"
              << ",pattern=" << pattern
              << ",transport=" << transport_
              << ",component=" << component
              << ",label=" << row_.socket_name
              << ",owner=" << owner
              << ",owner_id=" << row_.owner_id
              << ",socket=" << row_.socket_name
              ...
              << snapshot.auto_hwm_effective_message_bytes
              << ",effective_sndbuf="
              << perf_auto_hwm_sndbuf_display(snapshot, row_.socket_type)
              << ",effective_rcvbuf="
              << perf_auto_hwm_rcvbuf_display(snapshot, row_.socket_type)
              << ",last_recalc_reason="
                                                   ...
                                                   const char *label_ = NULL)
{
    ...
    const std::string key = std::to_string(
                              reinterpret_cast<unsigned long long>(node_))
                            + "|" + transport_ + "|"
    ...
    std::sort(node_rows.begin(), node_rows.end(),
              &perf_auto_hwm_snapshot_row_less);
    std::sort(spot_rows.begin(), spot_rows.end(),
              &perf_auto_hwm_snapshot_row_less);

    perf_print_auto_hwm_snapshot_table("Auto-HWM spotnode", node_rows,
                                       transport_, msg_size_);
    perf_print_auto_hwm_snapshot_table("Auto-HWM spot handles", spot_rows,
                                       transport_, msg_size_);
    return true;
}
                                           ...
                                           ZLINK_SOCKET_ANY)
{
    ...
    if (service_handle_ && std::strncmp(label, "spotnode", 8) == 0
        && perf_print_spot_node_auto_hwm_snapshot(handle_, transport,
                                                  msg_size_, label)) {
        return;
    }
    ...
    std::cout << "AUTO_HWM_DETAIL"
              ...
              << snapshot.auto_hwm_effective_message_bytes
              << ",effective_sndbuf="
              << perf_auto_hwm_sndbuf_display(snapshot, socket_type_)
              << ",effective_rcvbuf="
              << perf_auto_hwm_rcvbuf_display(snapshot, socket_type_)
...
inline bool apply_benchmark_context_auto_hwm_msg_unit(void *ctx_,
                                                      size_t msg_size_)
{
    if (!ctx_ || msg_size_ == 0)
        return true;
    const int msg_unit = perf_auto_hwm_msg_unit_for_size(msg_size_);
    if (!set_ctx_opt_int(ctx_,
                                           ...
                                           bool print_auto_hwm_snapshot = true)
{
    ...
    if (print_auto_hwm_snapshot)
        perf_print_auto_hwm_snapshot(
          socket_, false, "endpoint", transport, false, msg_size, socket_type);
}
