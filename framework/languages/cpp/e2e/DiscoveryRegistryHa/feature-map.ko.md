...
| 시나리오 | 상태 | 근거 |
|----------|------|------|
| SF-A1 | 구현 | Redis location store가 정상일 때 provider 2개가 live peer row로 보이고, consumer request가 provider에 도달한다. consumer/provider runtime status는 store와 owner lease가 healthy이고 실제 owner lease 갱신 시각과 last refresh 시각이 있음을 보여 준다. |
...
- 2026-07-15: `./run_e2e.sh SF-D3`, `./run_e2e.sh SF-A1`
  ...
  - 의미: runtime query가 만든 즉석 store probe가 아니라 owner lease heartbeat의 실패와 복구를 관측한다. 복구 뒤 두 시각 값이 장애 구간에 보존한 값보다 증가한다.
