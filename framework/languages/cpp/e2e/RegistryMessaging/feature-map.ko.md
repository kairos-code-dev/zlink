...
| Scenario | 상태 | 근거 |
|----------|------|------|
| `RM-A1` | 구현 | Redis location store로 `registry.messaging.api` peer row를 공유하고 endpoint 없는 client-server client가 자동 연결로 request를 보낸다. provider HTTP endpoint의 `/locations/peers`가 store peer row 두 개를 검증한다. `E2E_START_ORDER`의 정방향·역방향·고정 seed shuffle 변형은 두 provider의 실제 시작 순서를 바꾼다. route settle sleep이나 adapter request retry 없이 첫 결과를 그대로 검증한다. |
| `RM-A2` | 구현 | 같은 client-server channel에 `enable_client(endpoint)`로 지정한 `api-a`와 location store 기반 auto-connect를 함께 구성한다. `api-b` row가 추가되는 동안 진행 중인 manual request가 `api-a`에서 완료되고, 이후 두 endpoint가 모두 routing 대상에 유지되는지 검증한다. |
...
| `RM-C2` | 구현 | route mesh에서 target rid `api-b`로 request하고, 없는 rid는 `RouteNotConnected`로 분류되는지 검증한다. |
