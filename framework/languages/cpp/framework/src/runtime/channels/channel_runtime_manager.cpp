...
bool
enabled_for (const channel_snapshot_t &channel,
...
channel_runtime_manager_t
channel_runtime_manager_t::from (const message_bus_t &bus)
{
  return channel_runtime_manager_t (bus._state);
}

channel_runtime_manager_t
channel_runtime_manager_t::from (const zlink_builder_t &builder)
{
  return channel_runtime_manager_t (builder._state->runtime);
}

channel_runtime_bundle_t &
channel_runtime_manager_t::get_or_create_client_bundle (
...
channel_runtime_bundle_t &
channel_runtime_manager_t::get_or_create_publisher_bundle (
  const std::string &channel_name)
{
  ...
  const auto &channel =
    require_channel (channel_name, capability_t::publisher);
...
channel_runtime_manager_t::get_route_channel (
...
void
channel_runtime_manager_t::initialize_inbound_channels ()
{
  for (const auto &[channel_name, channel] : _state->channels) {
    if (channel.server.enabled &&
        _state->server_bundles.find (channel_name) ==
          _state->server_bundles.end ()) {
...
void
channel_runtime_manager_t::initialize_publisher_channels ()
{
  for (const auto &[channel_name, channel] : _state->channels) {
...
void
channel_runtime_manager_t::initialize_client_channels ()
{
  for (const auto &[channel_name, channel] : _state->channels) {
...
void
channel_runtime_manager_t::initialize_route_channels (
  const std::vector<std::string> &route_ids)
{
  for (const auto &route_id : route_ids) {
    if (_state->route_channels.find (route_id) !=
        _state->route_channels.end ()) {
      continue;
    }
    auto runtime = std::make_shared<route_channel_runtime_t> (route_id);
    runtime->start ();
    if (auto found = _state->channels.find (route_id);
        found != _state->channels.end ()) {
...
channel_runtime_manager_t::initialize_route_channels (
  const std::map<std::string,
                 std::shared_ptr<route_channel_builder_state_t>>
    &registrations)
{
...
std::string
channel_runtime_manager_t::monitoring_source (const std::string &source_name)
{
...
channel_runtime_manager_t::parse_source (const std::string &source_name)
{
  ...
  if (separator == std::string::npos || separator == 0 ||
      separator + 1 == source_name.size ()) {
    throw framework_exception_t (
      framework_error_kind_t::request_protocol_error,
      "socket monitoring source must use '<channel>.<capability>'");
  }
  return { source_name.substr (0, separator),
           source_name.substr (separator + 1) };
}
