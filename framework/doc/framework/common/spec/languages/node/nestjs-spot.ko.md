...
[Node.js 묶음](../../../../node/README.ko.md) | [인터페이스](handler-interfaces.ko.md) | [Stage wrapper](stage-wrapper-on-spot.ko.md) | [channel](nestjs-channel-messaging.ko.md) | [STREAM](nestjs-stream.ko.md)
...
새 transport 또는 wire 의미를 만들지 않으므로, 같은 channel로 연결하면 Spot
노드와도 그대로 통신한다.

등록 코드부터 handler, channel send / request, topic publish 까지 한 흐름으로
보는 샘플은 [정본 샘플](../../../../node/README.ko.md) 에 모아 두었다. 또한
...
테스트는 공통 SPOT 계약의 같은 동작을 검증하되 Node.js 테스트 이름과 구조를
따른다. 정식 매트릭스는
