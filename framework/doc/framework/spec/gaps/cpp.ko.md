...
- [ ] **SMP-CP-52** (**버그**) — **Bingo가 draw 진행 중 카드 재제출을 받아** 이미 뽑힌 번호를 새 카드에 소급 적용한다. **승리를 조작할 수 있다**
...
- [ ] **SMP-CP-63** (결함) — **TicTacToe에 도달 불가능한 status를 검사하는 dead code**가 있다(`"playing"`·`"ended"`는 존재하지 않는 값)
...
| **SMP-CP-63** | 상태값은 계약이 고정한다 | `Spots/TicTacToeGameSpot/tictactoe_game_models.hpp:13-14` — `is_playing()`이 `status == "playing"`을, `is_finished()`가 `status == "ended"`를 본다. 실제 상수는 `"WaitingForPlayers"`/`"InProgress"`/`"Won"`/`"Draw"`다(`Shared/Contracts/messages.hpp:45-51`) ⇒ **두 술어가 무조건 false**다. 이 타입 자체도 참조되지 않으며, `tictactoe_game_contract_mapper.hpp:9`(항등 `to_contract`)·`Handlers/tictactoe_game_spot_created_handler.hpp:9`(등록 0건)·`Spots/EntrySpot/tictactoe_entry_spot.hpp:77`의 `tictactoe_match_t room{"entry-match"}`(읽는 곳 0건)도 전부 dead code다 |
