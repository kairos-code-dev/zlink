                            ...
                            long *lat_count)
{
    ...
    for (size_t i = 0; i < poll_items.size () && received < recv_batch; ++i) {
        ...
        while (received < recv_batch) {
            const int rc = recv_one_message (sockets[i], scratch, ZLINK_DONTWAIT);
            if (rc < 0)
                return false;
            if (rc == 0)
                break;
...
inline bool drain_echo_pending (
  const std::vector<void *> &sockets,
  const multi_bench_settings_t &settings,
  std::vector<int> &pending,
  std::vector<zlink_pollitem_t> &poll_items,
  ...
  std::vector<char> &scratch)
{
    ...
    for (int i = 0; i < lat_count; ++i) {
        ...
        const int recv_rc = recv_one_message (socket, scratch, 0);
        if (recv_rc <= 0)
            break;
        ++completed;
    }
  ...
  bench_multi_resource_metrics_t *metrics_out)
{
    ...
    *latency_out = 0.0;
    ...
    auto recv_loop = [&] (double seconds,
        ...
        while (std::chrono::steady_clock::now () < deadline) {
            std::vector<int> pending (subs.size (), 0);
            if (!recv_with_poll (
                  subs,
                  pending,
                  poll_items,
                  ...
                  1,
                  recv_count,
                  collect ? lat_sum : NULL,
                  collect ? lat_count : NULL)) {
                return false;
            }
        }
        return true;
