                                ...
                                zlink::http_client::client_t &caller)
{
    ...
    const auto evidence_a = actor_a.get ("/evidence").fetch<std::vector<e2e::actor_evidence_t>> ();
    const auto evidence_b = actor_b.get ("/evidence").fetch<std::vector<e2e::actor_evidence_t>> ();
