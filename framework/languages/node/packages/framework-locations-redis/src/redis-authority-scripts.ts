...
local function descriptorAdmission(target, descriptorKey, admissionKey, leaseKey)
    ...
    if nodeActive + delta > tonumber(metadata[7])
        or nodePending + delta > tonumber(metadata[8])
        or (matched.activeLimit and typeActive + delta > tonumber(matched.activeLimit))
        or (matched.pendingLimit and typePending + delta > tonumber(matched.pendingLimit)) then
