...
{
    if (message.topic () != k_topic || message.parts ().size () != 1 || message.parts ()[0].size () != payload_size) {
        return true;
    }

    perf_single_metric::header_t header;
    if (!perf_single_metric::decode_payload_header (message.parts ()[0].data (), message.parts ()[0].size (),
                                                    &header)) {
        return true;
    }
...
bool run_pattern_pubsub (const std::string &transport, size_t msg_size, const std::string &lib_name)
{
    ...
    if (!perf::single::apply_single_auto_hwm_msg_unit (ctx, msg_size)
        ...
        if (perf_debug_enabled ())
            std::cerr << "pubsub: auto-hwm msg unit setup failed errno=" << errno << std::endl;
        return false;
    }

    if (!perf::single::setup_connected_pair (publisher, subscriber, transport, lib_name + "_pubsub")) {
        if (perf_debug_enabled ())
    ...
    std::this_thread::sleep_for (std::chrono::milliseconds (perf::single::resolve_single_pubsub_ready_settle_ms ()));
    ...
    std::thread sender_thread ([&] () {
        ...
        while (std::chrono::steady_clock::now () < active_deadline) {
            if (!perf_single_metric::stamp_payload (payload.data (), payload.size (), run_id,
                ...
                || !perf::single::publish_payload_blocking (publisher, k_topic, payload.data (), payload.size ())) {
    ...
    auto record_if_active = [&] (const zlink::topic_message_t &message_) {
        if (std::chrono::steady_clock::now () >= active_deadline)
            return true;
        return record_subscribed_payload (message_, run_id, msg_size, payload_size, received_count, latency_builder);
    };

    auto is_stop_message = [] (const zlink::topic_message_t &message_) {
        return message_.parts ().size () == 1 && perf::single::is_stop_token_message (message_.parts ()[0]);
    };
    ...
    perf::single::emit_single_socket_hwm_detail (publisher, "PUBSUB", transport, "publisher", "pub", msg_size);
    perf::single::emit_single_socket_hwm_detail (subscriber, "PUBSUB", transport, "subscriber", "sub", msg_size);

    const double throughput = static_cast<double> (received) / static_cast<double> (duration_s);
    perf::single::print_result (lib_name, "PUBSUB", transport, msg_size, throughput, latency.mean_ns, latency.p95_ns,
                                latency.p99_ns);
    return true;
}
