...
- `run_sample.sh`, `run_sample.ps1`: Play, API, Session 서버 process 를 시작하고 Redis location
  store readiness 를 확인한 뒤 client self-check 를 실행한다.
...
- `Server/Api/`: API channel 과 matching handler 를 제공한다. Play channel client 는 Redis
  location store 로 peer 를 찾는다.
- `Server/Session/`: stream endpoint 를 열고 인증 후 현재 session 을 Play actor 에 bind한다.
  API/Play/notification channel client 는 Redis location store 로 peer 를 찾고, client gameplay
  packet 은 bound actor 로 relay한다.
...
- runner 가 TypeScript Play, API, Session process 를 시작한다.
- Play 와 API 는 각 channel server endpoint 를 bind하고, Session 과 API 는 Redis location
  store 로 필요한 peer endpoint 를 찾는다.
