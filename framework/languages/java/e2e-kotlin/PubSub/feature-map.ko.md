...
| PS-A4 | 구현 | subscriber 중단 중 publish된 event는 재시작 후 replay되지 않고 이후 event는 수신되는지 subscriber evidence로 확인한다. |
| PS-B1 | 구현 | 느린 subscriber가 있어도 다른 subscriber가 마지막 sequence까지 받는지 bounded subscriber evidence wait로 확인한다. |
| PS-B2 | 구현 | publisher 재시작 뒤 subscriber들이 새 event를 받는지 bounded subscriber evidence wait로 확인한다. |
...
- 이 결과는 role/project 분리 구현의 동작 기준선이다. 최신 runner를 다시 실행하면 subscriber
  `/evidence/wait` 기반 검증 경로도 함께 확인한다.
