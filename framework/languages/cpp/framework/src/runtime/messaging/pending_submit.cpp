...
pending_submit_t
pending_submit_t::create_command (try_submit_fn try_submit,
                                  std::optional<clock_t::time_point> deadline,
                                  wake_fn wake)
{
  return pending_submit_t (
    std::move (try_submit), deadline, std::move (wake), true);
}

pending_submit_t
pending_submit_t::create_request (try_submit_fn try_submit,
                                  std::optional<clock_t::time_point> deadline,
                                  wake_fn wake)
{
  return pending_submit_t (
    std::move (try_submit), deadline, std::move (wake), false);
}
...
bool
pending_submit_t::expired (clock_t::time_point now) const noexcept
{
  return _deadline.has_value () && now >= *_deadline;
}

bool
pending_submit_t::try_submit ()
{
  if (completed ()) {
    return false;
  }
...
void
pending_submit_t::fail (std::exception_ptr error)
{
  if (detail::fail_pending_operation (_operation, std::move (error)) &&
      _wake) {
    _wake ();
  }
}
