...
if [[ -n "${GAMEQUEST_CPP_BASE_PORT:-}" ]]; then
  GAMEQUEST_REDIS_PORT="$((GAMEQUEST_CPP_BASE_PORT + 1))"
...
PY
  ...
  read -r GAMEQUEST_REDIS_PORT GAMEQUEST_API_A_STREAM_PORT GAMEQUEST_API_B_STREAM_PORT GAMEQUEST_API_A_HTTP_PORT GAMEQUEST_API_B_HTTP_PORT GAMEQUEST_MISSION_A_ROUTE_PORT GAMEQUEST_MISSION_B_ROUTE_PORT <<<"$PORT_ALLOCATION_OUTPUT"
fi
if [[ -z "$GAMEQUEST_REDIS_PORT" || -z "$GAMEQUEST_MISSION_B_ROUTE_PORT" ]]; then
...
if [[ -z "${GAMEQUEST_REDIS_ENDPOINT:-}" ]]; then
  REDIS_CONTAINER_NAME="zlink-cpp-gamequest-sample-redis-$$"
  docker run -d --rm --name "$REDIS_CONTAINER_NAME" -p "127.0.0.1:${GAMEQUEST_REDIS_PORT}:6379" redis:7-alpine >/dev/null
  export GAMEQUEST_REDIS_ENDPOINT="tcp://127.0.0.1:${GAMEQUEST_REDIS_PORT}"
fi
