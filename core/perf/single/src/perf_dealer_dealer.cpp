...
struct dealer_recv_state_t
{
    dealer_recv_state_t () : run_id (0), msg_size (0), payload_size (0), active_received (0) {}
...
int recv_single_part_header_flags (void *socket_,
                                   ...
                                   bool *header_ok_out_)
{
    ...
    if (!parts || part_count != 1) {
        if (bench_debug_enabled ()) {
            std::cerr << "[perf-dealer-dealer] unexpected multipart count="
                      << part_count << std::endl;
        }
        if (parts)
    ...
    if (!size_ok) {
        if (bench_debug_enabled ()) {
            std::cerr << "[perf-dealer-dealer] unexpected payload size="
                      << actual_size << " expected=" << expected_size_
                      << std::endl;
        }
        return -1;
    }
                       ...
                       latency_stats_t *latency_out_)
{
    ...
    if (!sender_ok.load (std::memory_order_acquire)) {
        if (bench_debug_enabled ()) {
            std::cerr << "[perf-dealer-dealer] active phase failed sent="
