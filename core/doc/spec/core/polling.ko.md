[English](polling.md) | 한국어

[스펙 목차](../README.ko.md) · [코어 목차](README.ko.md) · [Dispatch](service/dispatch.ko.md) · [errno map](errno-map.ko.md)
...
각 record에서 domain별 claim을 가져간다.
...
`ZLINK_CLOSE_BUSY`/`EBUSY`다. result와 errno의 전체 대응은 [errno map](errno-map.ko.md)을 따른다.
