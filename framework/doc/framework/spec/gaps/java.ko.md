...
- [x] **IMP-JV-01** (결함) — Spot peer row가 실제 actor factory마다 정렬된 `actor:<type>` capability를 게시하고, drain은 actor type별 소유 mesh에서 정확히 일치하는 capability를 가진 원격 노드만 선택하도록 고쳤다. exact-match 집중 테스트, core 전체 테스트와 Kotlin module 전체 테스트가 통과했다. 구현 커밋 `23f066b2e`(2026-07-15).
- [ ] **IMP-JV-02** (결함) — 24 §3·§5
...
| ID | 종류 | 계약 | 구현이 하는 일 |
|----|------|------|----------------|
| **IMP-JV-01** | 결함 | [40 §2.1](../server/40-location-runtime.ko.md): actor type마다 `actor:<type>` **capability**를 기록하고, handoff는 **정확히 일치하는** capability를 가진 노드만 고른다. **application metadata로 대신 기록하지 않는다** | **해결:** Spot peer row는 actor factory key를 정렬·중복 제거한 capability로 게시한다. drain은 활성 actor type별로 그 factory를 소유한 mesh만 조회하고 exact capability를 가진 비-draining 원격 노드로 같은 type의 actor만 넘긴다. prefix·다른 type·local node 거부 집중 테스트, core와 Kotlin 전체 테스트 통과. 구현 커밋 `23f066b2e`(2026-07-15). |
...
- [x] **SMP-JV-01** (**절대 규칙 위반**) — TicTacToe 밖 샘플이 **수동 연결을 쓴다**(live 재검증 15곳 → 4곳)
  - 증거: GameQuest runner에 endpoint 인자를 받는 `enableClient(...)` 금지 gate를 먼저 추가해 남은 4곳을 검출했다. 네 channel client를 location store 자동 발견으로 바꾸고, 비동기 projection은 standalone HTTP client가 완료 상태를 확인한 뒤 조회하도록 경합을 제거했다. 최초 시나리오·scale-out·mission 재기동·Redis replay/rehydrate를 포함한 전체 runner가 exit 0으로 통과했다. 구현 커밋 `311e8304c`(2026-07-15).
...
- [x] ~~**E2E-JV-04** (결함) — 앱 코드의 **환경변수 읽기 535곳**인데 feature-map에 기록 **0**~~
  - 재검증 취소: 현재 Java E2E 애플리케이션 29,519줄에서 `System.getenv`·`System.getProperty`와 `ZLINK_JAVA_E2E_*` key는 모두 0건이다. 29개 Spring bootstrap은 role별 `--config` 파일, typed `@ConfigurationProperties`, 환경 변수·JVM property source 제거를 사용한다. 11개 runner는 임시 설정을 mode 0600으로 만들고 애플리케이션 설정 환경 변수를 전달하지 않는다. 남은 `ZLINK_LIBRARY_PATH` export 10곳은 정책 §5가 허용한 native library loader 입력이다. 설정 파일 전환 커밋 묶음 `faf2aa599`~`2e6ca5da2`를 live 재검증했다(2026-07-15).
...
| ID | 계약 | 구현이 하는 일 |
|----|------|----------------|
| **SMP-JV-01** | [샘플 규약](../../common/sample/README.ko.md)의 **절대 규칙**: TicTacToe만 수동 연결을 쓸 수 있다. *"위반이 하나라도 있으면 해당 샘플 변경은 완료된 것으로 판단하지 않는다"* | **해결:** Bingo 4곳, DeliveryDispatch 5곳, ShoppingMall 2곳과 GameQuest 4곳을 인자 없는 `enableClient()`와 location store 자동 발견으로 바꿨다. GameQuest는 비동기 projection을 standalone HTTP client에서 확인하므로 업무 요청 retry나 runtime 내부 접근 없이 재기동 직후 경합을 피한다. 각 샘플 전체 runner가 통과했고 SupportChat은 재검증 시 이미 0곳이었다. |
...
| ~~**E2E-JV-04** (결함)~~ | [E2E README §2.6:343-344](../../common/e2e/README.ko.md): "timeout, 로그와 evidence 경로를 환경 변수나 JVM system property로 전달하지 않으며, server와 client 애플리케이션 코드에서 직접 사용할 수 있는 **환경 변수는 0개다**". §2.8은 미충족 항목을 feature-map에 gap으로 남기라고 한다 | **재검증 취소:** 이전 수치는 설정 파일 migration 전 상태다. 현재 앱 코드의 직접 환경/JVM property 조회와 legacy key는 0건이며, 모든 Spring host가 role별 설정 파일과 typed binding을 사용하고 외부 property source를 제거한다. Runner의 `ZLINK_LIBRARY_PATH`만 도구 실행 입력으로 남아 정책 §5 범위에 해당한다. |
