...
- [ ] **IMP-JV-32** (결함) — `listPageSize` 직접 조회 적용 완료, 복합 topology cursor 남음. spot·actor·route의 암시적 첫 page는 설정값(기본 1000)을 사용한다. 설정값 2 집중 테스트와 core unit test 통과. `kind=null` topology가 kind별 store continuation을 보존하지 못하므로 아직 완료 처리하지 않는다. 부분 구현 커밋 `29a20cf8f`(2026-07-15).
...
| **IMP-JV-32** | [40 §3·§8.2](../server/40-location-runtime.ko.md): 목록 조회는 `list page size`(기본 **1000**)를 따른다 | **부분 해결:** query service가 page size 0인 암시적 첫 page를 `options.listPageSize()`로 바꾸므로 spot·actor·route store 조회는 더 이상 무한 page를 요청하지 않는다. 설정값 2에서 세 spot 중 두 개와 continuation을 반환하는 집중 테스트와 core unit test 통과. `kind=null` 복합 topology 조회는 kind별 continuation을 하나의 token으로 보존하는 cursor 설계가 남아 있어 open이다. 부분 구현 커밋 `29a20cf8f`(2026-07-15). |
