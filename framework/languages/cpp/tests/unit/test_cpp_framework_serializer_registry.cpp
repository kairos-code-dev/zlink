...
void
to_json (nlohmann::json &json, const json_payload_t &payload)
{
  json = nlohmann::json { { "value", payload.value } };
}

void
from_json (const nlohmann::json &json, json_payload_t &payload)
{
...
main ()
{
  ...
  serializers.add<payload_t> (
    ...
    [](const zlink::message_t &message) {
      return payload_t { std::stoi (message.to_string ()) };
    });
  ...
  const auto json_encoded =
    serializers.get<json_payload_t> ().serialize ({ 77 });
  const auto json_decoded =
    serializers.get<json_payload_t> ().deserialize (json_encoded);
  ...
  try {
    serializers.add<payload_t> (
      [](const payload_t &) { return zlink::message_t {}; },
