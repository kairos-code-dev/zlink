...
- [x] **E2E-CP-08** (결함) — **§2.6 설정 정책을 `ToActorMessaging` 빼고 전 config가 위반**하고, feature-map에 기록한 곳이 0개다
  ...
  - POSD/DDD: handler와 shell에 퍼진 설정 지식은 role별 typed 설정으로 모으고, 여섯 sample runner의 shell→Python 환경 전달은 명시적 인자로 바꿨다. 재검증에서는 네 runner의 환경 fallback 분기와 startup sleep을 제거해 실행 설정 경로를 role별 JSON 하나로 줄였다. F 시나리오의 location 준비 판단도 `ensure_f_spot()` 한 곳으로 합쳐 호출부의 lifecycle 지식을 줄였으며, 새 public API나 domain 경계 변경은 없다.
