...
public:
  ...
  {
    monitoring.on_trace ([this](const runtime_event_base_t &event) {
      traces.push_back (event.correlation_id);
    });
    return *this;
  }
...
{
public:
  kafka_bridge_extension_t &map_topic (std::string topic,
...
{
public:
  grpc_bridge_extension_t &map_service (std::string service_name,
                                        std::string channel_name)
  {
    mappings.push_back (
      { std::move (service_name), std::move (channel_name) });
    return *this;
  }
...
{
public:
  http_gateway_extension_t &map_route (std::string route,
                                       std::string channel_name)
  {
...
struct retry_policy_t
{
  ...
  std::string ordering_policy =
    "preserve ordering within a channel unless explicitly disabled";
};
...
{
public:
  dead_letter_storage_extension_t &store (
    std::function<void (const channel_reliability_event_t &)> sink)
  {
...
public:
  template<typename T>
  flatbuffers_codec_extension_t &register_type ()
  {
...
inline std::vector<extension_descriptor_t>
known_extensions ()
{
  return {
    { "metrics", "zlink::framework_extension_metrics" },
