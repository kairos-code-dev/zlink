...
{
    private static int RunMultiDealerRouterServer(string transport, int size)
    {
        ...
        string endpoint = MultiEndpointFor(transport, "multi-MULTI_DEALER_ROUTER");
        ...
        while (true)
        {
            ...
            SendRetry(server, routingId.AsSpan(0, ridLen), SendFlags.SendMore);
    ...
    private static int RunMultiDealerRouterClient(string transport, int size,
        string endpoint)
    {
        size = Math.Max(1, size);
        ...
        int drainMs = ResolveMultiDrainMs("MULTI_DEALER_ROUTER");
        ...
        try
        {
            ...
            PrintResult("MULTI_DEALER_ROUTER", transport, size, throughput,
                latencyUs);
            return 0;
        }
        finally
