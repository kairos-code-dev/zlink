...
socket의 native-ready와 같은 socket의 첫 valid application record 또는 liveness beacon 수신을 모두
...
소유한다. Instance Spot event의 `InstanceSpotType`은 startup에 등록한 type이고 `ActivationState`는
