...
channel_packet_dispatcher_t::channel_packet_dispatcher_t (channel_runtime_t runtime) : _runtime (std::move (runtime))
{
}

result_t<runtime::messaging::message_parts_t>
channel_packet_dispatcher_t::dispatch_server_message (std::string channel_name,
                                                      ...
                                                      const handler_registry_t &handlers) const
{
    ...
    if (!header) {
        return result_t<runtime::messaging::message_parts_t>::failure (
          header.error_kind (), header.error () ? header.error ()->what () : "channel envelope header decode failed");
    }
    ...
    if (!body) {
        if (header.value ().kind == runtime::messaging::message_kind_t::request) {
            ...
            return result_t<runtime::messaging::message_parts_t>::success (
              writer.reply_raw_envelope (writer.create_error_header (std::move (channel_name), header.value (), error),
                                         zlink::message_t::from ("")));
        }
        return result_t<runtime::messaging::message_parts_t>::failure (
          body.error_kind (), body.error () ? body.error ()->what () : "channel body decode failed");
    }

    if (header.value ().kind == runtime::messaging::message_kind_t::request) {
        auto reply =
          _runtime.dispatch_request (channel_name, header.value ().topic.value_or ("Channel"),
        ...
        return result_t<runtime::messaging::message_parts_t>::success (
          writer.reply_raw_envelope (writer.create_reply_header (runtime::messaging::message_kind_t::response,
    ...
    if (header.value ().kind == runtime::messaging::message_kind_t::command) {
        auto result =
          _runtime.dispatch_send (std::move (channel_name), header.value ().topic.value_or ("Channel"),
                                  header.value ().message_name, services, serializers, handlers, body.value ());
        if (!result) {
            return result_t<runtime::messaging::message_parts_t>::failure (
              result.error_kind (), result.error () ? result.error ()->what () : "channel command failed");
        }
        return result_t<runtime::messaging::message_parts_t>::success (runtime::messaging::message_parts_t{});
    }
