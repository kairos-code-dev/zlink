...
stream_error_t::stream_error_t (stream_session_error_t error,
                                ...
                                std::string message)
  : _error (error),
    _native_code (native_code),
    _message (std::move (message))
{
}

stream_session_error_t
stream_error_t::error () const noexcept
{
  return _error;
}

int
stream_error_t::native_code () const noexcept
{
  return _native_code;
}

std::string_view
stream_error_t::message () const noexcept
{
  return _message;
}
...
stream_metadata_t::find (std::string_view key) const
{
...
bool
stream_metadata_t::empty () const noexcept
{
  return _values.empty ();
}

const std::map<std::string, std::string> &
stream_metadata_t::values () const noexcept
{
  return _values;
}
...
stream_header_t::kind () const noexcept
{
  return _kind;
}

stream_codec_t
stream_header_t::codec () const noexcept
{
  return _codec;
}

stream_header_flags_t
stream_header_t::flags () const noexcept
{
  return _flags;
}

std::optional<std::uint64_t>
stream_header_t::request_seq () const noexcept
{
  return _request_seq;
}

std::string_view
stream_header_t::packet_name () const noexcept
{
  return _packet_name;
}

std::optional<std::string_view>
stream_header_t::metadata (std::string_view key) const
{
  return _metadata.find (key);
}

const stream_metadata_t &
stream_header_t::metadata () const noexcept
{
  return _metadata;
}

std::optional<std::string_view>
stream_header_t::correlation_id () const
{
  return metadata ("correlation_id");
}

std::optional<std::string_view>
stream_header_t::content_type () const
{
...
stream_t::stream_t (std::shared_ptr<detail::stream_state_t> state)
                        ...
                        const zlink::message_t &payload)
{
  ...
  stream_header_t reply_header (
    stream_message_kind_t::response,
    request_header.codec (),
...
stream_builder_t &stream_builder_t::operator= (stream_builder_t &&) noexcept =
  default;
...
stream_snapshot_t
stream_builder_t::snapshot () const
{
...
{
  auto state = std::make_shared<detail::stream_builder_state_t> (
    std::move (stream_name));
  stream_builder_t builder (state);
  if (configure) {
    configure (builder);
  }
  if (state->snapshot.bind_endpoint.empty () ||
      state->snapshot.packet_session_name.empty ()) {
    throw framework_exception_t (
      framework_error_kind_t::request_protocol_error,
...
std::vector<stream_snapshot_t>
zlink_builder_t::streams () const
{
...
stream_runtime_t::stream_runtime_t (
  std::shared_ptr<stream_runtime_state_t> state)
  : _state (std::move (state))
{
}

stream_runtime_t
stream_runtime_t::from (const zlink_builder_t &builder)
{
...
bool
has_flag (stream_header_flags_t flags, stream_header_flags_t flag)
{
...
bool
known_kind (stream_message_kind_t kind)
{
  switch (kind) {
...
bool
known_codec (stream_codec_t codec)
{
  switch (codec) {
...
result_t<void>
validate_name (std::string_view name, bool allow_reserved)
{
...
result_t<void>
stream_runtime_t::validate_header (const stream_header_t &header) const
{
  ...
  if (header.kind () == stream_message_kind_t::error &&
      header.codec () != stream_codec_t::json) {
    return result_t<void>::failure (
      framework_error_kind_t::request_protocol_error,
      "STREAM error packet must use JSON codec");
  }
  if (header.kind () == stream_message_kind_t::control) {
    if (header.flags () != stream_header_flags_t::none ||
        header.codec () != stream_codec_t::raw || has_request_seq ||
        has_metadata) {
      return result_t<void>::failure (
        framework_error_kind_t::request_protocol_error,
...
stream_runtime_t::encode_header (const stream_header_t &header) const
{
  if (auto valid = validate_header (header); !valid) {
    return result_t<std::vector<std::uint8_t>>::failure (
      valid.error_kind (), valid.error ()->what ());
  }
  ...
  bytes.insert (bytes.end (), header.packet_name ().begin (),
                header.packet_name ().end ());
  if (!header.metadata ().empty ()) {
    if (header.metadata ().values ().size () >
...
stream_runtime_t::decode_header (const std::vector<std::uint8_t> &bytes) const
{
  ...
  if (auto valid = validate_header (header); !valid) {
    return result_t<stream_header_t>::failure (
      framework_error_kind_t::payload_decode_failed, valid.error ()->what ());
  }
  return result_t<stream_header_t>::success (std::move (header));
}

stream_t
stream_runtime_t::open_session (std::string stream_name) const
{
  ...
  state->session_id =
    std::move (stream_name) + ":" + std::to_string (_state->next_session_id++);
  return stream_t (state);
}

result_t<void>
stream_runtime_t::dispatch_serial (
  stream_t &stream,
...
result_t<void>
stream_runtime_t::dispatch_connected (packet_stream_session_t &session,
                                      stream_t &stream) const
{
  return dispatch_serial (stream, "connected", [&] {
    return session.on_connected (stream);
  });
}

result_t<void>
stream_runtime_t::dispatch_packet (packet_stream_session_t &session,
                                   stream_t &stream,
...
result_t<void>
stream_runtime_t::dispatch_disconnected (packet_stream_session_t &session,
                                         stream_t &stream) const
{
...
result_t<void>
stream_runtime_t::dispatch_error (packet_stream_session_t &session,
                                  stream_t &stream,
                                  const stream_error_t &error) const
{
  return dispatch_serial (stream, "error", [&] {
    return session.on_error (stream, error);
  });
}

std::vector<std::string>
stream_runtime_t::serial_log (const stream_t &stream) const
{
  return { stream._state->serial_log.begin (), stream._state->serial_log.end () };
}

std::vector<stream_header_t>
stream_runtime_t::written_headers (const stream_t &stream) const
{
