...
class bingo_room_actor_joined_handler_t
{
  public:
    ...
    explicit bingo_room_actor_joined_handler_t (bingo_notification_publisher_t &publisher) : _publisher (publisher) {}

    void
    handle (const bingo_room_t &spot, const player_actor_t &actor, const zlink::framework::spot_actor_change_result_t &)
    {
        const auto &state = spot.snapshot ();
        _publisher.publish_joined ({state.room_id, actor.actor.actor_id, actor.actor.actor_id, 0,
