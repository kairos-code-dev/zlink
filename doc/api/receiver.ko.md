...
Receiver가 보유한 Discovery 기반 control runtime을 Registry의 ROUTER
엔드포인트에 연결합니다. 등록, 등록 해제, 가중치 업데이트 메시지는 이
Discovery runtime을 통해 전송됩니다. Receiver topology entry는 control path에서
직접 파생되지 않고, Discovery가 소유하는 summary uplink를 통해 보고됩니다.
