...
// Session server packet session: owns authentication and stream binding, and
// relays all conversation packets to the bound Support actor. It does not
// interpret conversation rules.
class support_chat_session_t final : public zlink::framework::packet_stream_session_t
{
  public:
  ...
  private:
    ...
    update_bound_actor_ref (const remote_actor_packet_res_t &reply)
    {
        ...
        return _actors.bind (zlink::framework::actor_ref_t (
          zlink::framework::node_rid_t::from_string (reply.actor_node_rid), reply.actor_type,
          ...
          co_await _client
            .request (sample_names_t::support_channel,
                      remote_actor_packet_req_t{
                        ...
                        std::string (actor.ref ().actor_id ()), actor.ref ().generation (),
                        static_cast<int> (header.kind ()), static_cast<int> (header.codec ()),
                        static_cast<int> (header.flags ()), request_seq.has_value (),
                        request_seq.value_or (0), std::string (header.packet_name ()),
                        header.metadata ().values (), payload.to_bytes ()})
