...
public final class RmC5MissingPacketScenario {
    ...
    public static void run(
        ...
        Contracts.RequestFailureResult missing = discoveryConsumer.post("/profile/missing-request")
            .body(new Contracts.ProfileRequest("missing"))
        ...
        discoveryConsumer.post("/profile/missing-command")
            .body(new Contracts.ProfileCommand("missing-send"))
            .fetch(Object.class);
        String[] requestEvidence = ScenarioAssert.waitAnyEvidence(providerA, providerB, "MissingProfileRequest");
        ScenarioAssert.that(java.util.Arrays.stream(requestEvidence)
                .anyMatch(line -> line.contains("dispatch-error")
                    ...
                    && line.contains("MissingProfileRequest")),
            "RM-C5 missing request dispatch-error evidence missing");
        String[] sendEvidence = ScenarioAssert.waitAnyEvidence(providerA, providerB, "MissingProfileCommand");
        ScenarioAssert.that(java.util.Arrays.stream(sendEvidence)
                .anyMatch(line -> line.contains("dispatch-error")
                    && line.contains("HANDLER_MISSING")
                    && line.contains("DROP")
                    && line.contains("MissingProfileCommand")),
            "RM-C5 missing send dispatch-error evidence missing");
        Contracts.ProfileReply normal = discoveryConsumer.post("/profile/request")
            .body(new Contracts.ProfileRequest("normal"))
