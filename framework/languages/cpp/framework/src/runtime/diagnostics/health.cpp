...
zlink::framework::health_status_t
combine (zlink::framework::health_status_t current,
...
bool
participates_in_readiness (
  zlink::framework::health_check_scope_t scope) noexcept
{
  return scope == zlink::framework::health_check_scope_t::readiness ||
         scope ==
           zlink::framework::health_check_scope_t::readiness_and_liveness;
}

bool
participates_in_liveness (
  zlink::framework::health_check_scope_t scope) noexcept
{
...
health_builder_t::health_builder_t ()
  : _state (std::make_shared<detail::health_state_t> ())
{
}

health_builder_t::health_builder_t (
  std::shared_ptr<detail::health_state_t> state)
...
health_builder_t &health_builder_t::operator= (
  health_builder_t &&) noexcept = default;

health_builder_t &
health_builder_t::add_zlink_runtime_check (std::string name)
{
  return add_check ("zlink", std::move (name),
...
{
  return add_check ("channel", std::move (name),
                    health_check_scope_t::readiness);
}

health_builder_t &
health_builder_t::add_registry_check (std::string name)
{
  return add_check ("registry", std::move (name),
                    health_check_scope_t::readiness);
}
...
{
  return add_check ("stream", std::move (name),
                    health_check_scope_t::readiness);
}
...
health_builder_t &
health_builder_t::set_status (std::string name,
                              ...
                              std::string message)
{
  auto found = std::find_if (
    _state->checks.begin (),
    _state->checks.end (),
    [&](const health_check_result_t &check) {
...
health_builder_t &
health_builder_t::add_check (std::string component,
                             std::string name,
                             health_check_scope_t scope)
{
  auto found = std::find_if (
    _state->checks.begin (),
    _state->checks.end (),
  ...
  if (found == _state->checks.end ()) {
    _state->checks.push_back (
      health_check_result_t { std::move (name), std::move (component),
                              health_status_t::healthy, scope, {} });
  }
  return *this;
}
