...
public final class ClientScenario {
    ...
    public void run() {
        String scenario = Env.get("ZLINK_KOTLIN_E2E_SCENARIO");
        if (!"DR-A4".equals(scenario)) {
            waitForExpectedMembers();
        }
        Set<String> providers = requestUntilProviders(Env.csv("ZLINK_KOTLIN_E2E_EXPECTED_RIDS"));
        if ("DR-C1".equals(scenario)) {
            expectHttpFailure(Env.get("ZLINK_KOTLIN_E2E_DEAD_HTTP_ENDPOINT"));
        }
    ...
    private void waitForExpectedMembers() {
        Set<String> expected = new HashSet<>(Env.csv("ZLINK_KOTLIN_E2E_EXPECTED_RIDS"));
        for (String probe : Env.csv("ZLINK_KOTLIN_E2E_PROBE_HTTP_ENDPOINTS")) {
    ...
    private void expectRemoteTopologyMatchesProbe() {
        ...
        Set<String> probeRids = topologyRids(Env.get("ZLINK_KOTLIN_E2E_TOPOLOGY_HTTP_ENDPOINT"));
        try {
