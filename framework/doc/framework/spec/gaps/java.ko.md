...
| ID | 계약 | 구현이 하는 일 |
|----|------|----------------|
| **SMP-JV-03** (미구현) | [gamequest:280](../../common/sample/event/gamequest.ko.md): "노드를 재시작해도 owner spot이 **`QuestEventStore` replay로 aggregate를 rehydrate**한다". §14(`:603`)도 rehydrate를 "노드 재시작 → replay로 aggregate 복원"으로 못박는다 | SMP-JV-19에서 projection rebuild를 domain event의 delta fold로 고쳤다. 그러나 event stream 본체는 여전히 `QuestStore`의 프로세스 메모리이고, owner는 Redis stream을 시작할 때 읽지 않는다. rehydrate 게이트도 실제 재시작이 아니라 `markRehydrated` 호출 횟수를 센다. ⇒ 재시작·owner 재활성 복원과 owner spot 책임은 아직 구현되지 않았다. 심층은 SMP-JV-11·12·13 |
