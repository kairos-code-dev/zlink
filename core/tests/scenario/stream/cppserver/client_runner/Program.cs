...
internal static class Program
{
    ...
    public static async Task<int> Main(string[] args)
    {
        ...
        if (cfg.Transport != "tcp")
        {
            ...
            Console.Error.WriteLine($"[stream-cppserver-client] transport '{cfg.Transport}' skipped (tcp only)");
            return 0;
        }
    ...
    {
        Console.WriteLine("Usage: StreamSocketScenarioCppServerClient --scenario s0|s1|s2 [options]");
        Console.WriteLine("Options:");
        Console.WriteLine("  --transport tcp                  (tcp only)");
        Console.WriteLine("  --port N                         (default 27410)");
