...
public:
      ...
      const framework::route_handler_context_t &))
  {
    _send_handlers.push_back (
      [packet = std::move (packet_name),
       method](route_handler_registry_t &handlers,
               const std::string &router_channel_id) {
        handlers.on_send<TOwner, TMessage> (
          router_channel_id,
          packet,
          method);
      });
                              ...
                              const framework::route_handler_context_t &))
  {
    _request_handlers.push_back (
      [packet = std::move (packet_name),
       method](route_handler_registry_t &handlers,
               const std::string &router_channel_id) {
        handlers.on_request<TOwner, TRequest, TReply> (
          router_channel_id,
          packet,
          method);
      });
...
{
public:
  initialized_route_channel_t initialize (
