...
| 5 | Go | `bindings/go/perf` | `tcp/tls single 통과, ws/wss single latency 보류, wss SPOT 262144B 보류` | `tcp/ws 측정 완료, wss partial 측정 완료` | Go multi `wss` no-result 및 미달 항목 개선 |
...
| Go | 284 | 71.4% | 65.2% | 32.6% | 18.3% | 90.8% | 51.4% |
...
| `wss` | `MULTI_SPOT_REQREP` | `통과(58.8%)` | `통과(62.2%)` | `통과(57.3%)` | `통과(74.7%)` | `통과(72.4%)` | `미측정` | C full 파일은 위 행과 같다. Go 64B/256B/1024B `perf_go_multi_linux_20260522_064814_codex_go_wss_multi_current_20260522.txt`, 65536B `perf_go_multi_linux_20260522_065845_codex_go_wss_multi_spot_reqrep65536_recheck_20260522.txt`, 131072B `perf_go_multi_linux_20260522_065807_codex_go_wss_multi_spot_reqrep_large_recheck_20260522.txt`. 262144B는 full-run과 단독 제한 재측정 `perf_go_multi_linux_20260522_065858_codex_go_wss_multi_spot_reqrep262144_recheck_20260522.txt` 모두 no-result라 개선 대상이다. request completion은 public `POLLCOMPLETION` poller를 사용한다. auto-HWM 활성과 size별 SpotNode `MsgUnit(B)` 일치를 확인했다. |
| `wss` | `MULTI_SPOT_SENDSEND` | `미달(29.6%)` | `미달(30.7%)` | `미달(34.4%)` | `미측정` | `미달(47.9%)` | `미달(0.0%)` | C/Go full 파일은 위 행과 같다. 65536B는 full-run과 단독 제한 재측정 `perf_go_multi_linux_20260522_065912_codex_go_wss_multi_spot_sendsend65536_recheck_20260522.txt` 모두 no-result라 개선 대상이다. C와 같은 size별 active slot 제한을 유지하고, client active loop는 단일 poller 중심이다. auto-HWM 활성과 size별 SpotNode `MsgUnit(B)` 일치를 확인했다. |
