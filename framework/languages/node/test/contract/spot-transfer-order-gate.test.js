...
test('ST-F1 and ST-F3 compare packet values and require source cleanup evidence', () => {
  const client = fs.readFileSync(path.join(
    nodeRoot,
    'e2e/SpotActorTransfer/Client/main.ts'
  ), 'utf8');
  const runtime = fs.readFileSync(path.join(
    nodeRoot,
    'packages/framework/src/runtime/host/actor-transfer-runtime.ts'
  ), 'utf8');
  const f1 = client.slice(client.indexOf('async function runF1'), client.indexOf('async function runF2'));
  const f3 = client.slice(client.indexOf('async function runF3'), client.indexOf('async function runF4'));
