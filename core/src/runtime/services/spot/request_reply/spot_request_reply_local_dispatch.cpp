...
int dispatch_spot_request_to_router (const std::string &router_rid_,
...
int recv_first_router_payload_frame (zlink::socket_base_t *socket_, zlink_msg_t *payload_out_)
{
    while (true) {
        zlink_msg_t routing_id;
        zlink_msg_init (&routing_id);
        if (zlink::recv_msg_socket (socket_, ZLINK_CORE_SOCKET_ROUTER, &routing_id, ZLINK_DONTWAIT)
            != 0) {
            ...
            const int wait_rc = zlink::wait_socket_events_internal (socket_, ZLINK_POLLIN, 1);
            if (wait_rc <= 0
                || zlink::recv_msg_socket (socket_, ZLINK_CORE_SOCKET_ROUTER, &routing_id,
                                           ZLINK_DONTWAIT)
                     != 0) {
        ...
        const bool routing_id_has_more = zlink::msg_frame_has_more (routing_id);
        zlink_msg_close (&routing_id);
        if (!routing_id_has_more)
            continue;

        zlink_msg_init (payload_out_);
        if (zlink::internal_pair_queue::recv_followup_with_retry (socket_, payload_out_,
                                                                  ZLINK_DONTWAIT)
            != 0) {
            const int saved_errno = errno;
...
int zlink::spot_reqrep_internal::recv_combined_router_message (zlink::socket_base_t *socket_,
