...
> 상태: 구현 완료.
> 이 문서는 현재 코드베이스의 pub/sub public naming 재편 결과를 요약하는
> source of truth다.
...
`subscription event` 축으로 재편한 결과를 정리한다.
...
### 2.2 pub/sub family로 재편된 대상

다음 subject/type은 pub/sub family로 재편됐다.
...
1. `Msg-Only Data Plane Realignment` 기준 구현/회귀를 마무리했다.
2. public header/테스트/guide에서 `spot` / `xpub` 예외 family 사용처를 정리했다.
3. pub/sub naming을 `publish` / `subscribe` / `subscription event` 축으로 정리했다.
...
다음 API는 새 pub/sub family로 대체했고 public header에서 제거했다.
...
- pub/sub 계열은 `send/recv`가 아니라 `publish/subscribe` naming으로 재편됐다.
- 일반 transport family의 canonical `send/recv`는 유지한다.
- `publish`는 단일 함수 `zlink_publish()`로 정리됐다.
- `subscribe`는 단일 함수 `zlink_subscribe()`로 정리됐다.
- `unsubscribe`는 단일 함수 `zlink_unsubscribe()`로 정리됐다.
- pattern 구분은 enum 없이 `*` suffix 유무로 판별한다.
- `XPUB`는 pub/sub umbrella 아래 포함되더라도 data-plane이 아니라
  subscription event-plane으로 유지된다.
