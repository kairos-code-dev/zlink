...
stream.requireOpen();
const decision = await moderation.check(userId, text);
if (!decision.allowed) {
  await gateway.send(sessionId, new MessageRejected(decision.reason));
  return;
}
const message = await recent.append(streamId, userId, text);
await pubsub.publish(`stream:${streamId}`, message);
```
...
@zlinkSpotActorSendHandler({
  spot: () => StreamSpot,
  actor: () => ViewerActor,
  packetName: 'SendLiveChat',
})
export class SendLiveMessageHandler
  implements ZLinkSpotActorSendHandler<StreamSpot, ViewerActor, SendLiveChat> {
  constructor(private readonly moderation: Moderation, private readonly recent: RecentMessageStore) {}
...
@zlinkSpotActorSendHandler({
  ...
  packetName: 'PinMessage',
})
export class PinMessageHandler
  implements ZLinkSpotActorSendHandler<StreamSpot, ModeratorActor, PinMessage> {
  async handle(
    stream: StreamSpot,
...
주의할 점은 fan-out 이다. `for (const watching of stream.activeViewers())` 는 작은 방송과
