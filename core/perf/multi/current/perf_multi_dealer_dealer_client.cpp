...
static const bool k_one_way_latency = true;
  ...
  std::vector<char> &scratch)
{
    ...
    if (k_one_way_latency) {
        static const uint32_t k_latency_magic = 0xB17ECAFEu;
        static const size_t k_latency_magic_size = sizeof (k_latency_magic);
        static const size_t k_latency_ts_size = sizeof (unsigned long long);
        static const size_t k_latency_header_size =
          k_latency_magic_size + k_latency_ts_size;
        std::vector<char> send_buf (std::max<size_t> (payload_size, 64), 'c');
        if (!payload.empty ()) {
            const size_t copy_len = std::min (payload.size (), send_buf.size ());
            std::memcpy (send_buf.data (), payload.data (), copy_len);
        }

        std::vector<zlink_pollitem_t> items (sockets.size ());
        ...
        for (int i = 0; i < lat_count; ++i) {
            const unsigned long long sent_us = static_cast<unsigned long long> (
              std::chrono::duration_cast<std::chrono::microseconds> (
                std::chrono::system_clock::now ().time_since_epoch ())
                .count ());
            if (send_buf.size () >= k_latency_header_size) {
                std::memcpy (
                  send_buf.data (), &k_latency_magic, k_latency_magic_size);
                std::memcpy (
                  send_buf.data () + k_latency_magic_size,
                  &sent_us,
                  k_latency_ts_size);
            }
            ...
            while (std::chrono::steady_clock::now () < deadline && !got_reply) {
                ...
                for (size_t si = 0; si < items.size (); ++si) {
                    if ((items[si].revents & ZLINK_POLLIN) == 0)
                        continue;

                    const int rc =
                      recv_one_message (sockets[si], scratch, ZLINK_DONTWAIT);
                    if (rc < 0)
                        return empty;
                    if (rc <= 0)
                        continue;

                    if (scratch.size () >= k_latency_header_size) {
                        uint32_t marker = 0;
                        std::memcpy (
                          &marker, scratch.data (), k_latency_magic_size);
                        if (marker != k_latency_magic)
                            continue;

                        unsigned long long echoed_sent_us = 0;
                        std::memcpy (
                          &echoed_sent_us,
                          scratch.data () + k_latency_magic_size,
                          k_latency_ts_size);
                        const unsigned long long now_us =
                          static_cast<unsigned long long> (
                            std::chrono::duration_cast<std::chrono::microseconds> (
                              std::chrono::system_clock::now ().time_since_epoch ())
                              .count ());
                        if (now_us >= echoed_sent_us) {
                            lat_samples.add (
                              static_cast<double> (now_us - echoed_sent_us));
                            got_reply = true;
                            break;
                        }
                    }
