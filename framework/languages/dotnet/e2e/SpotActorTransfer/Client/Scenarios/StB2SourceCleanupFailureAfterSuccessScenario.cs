...
internal static class StB2SourceCleanupFailureAfterSuccessScenario
{
    public static async Task RunAsync(SpotActorTransferScenarioContext context)
    {
        ...
        SpotActorTransferScenarioContext.Require(join.Accepted, "ST-B2 join was rejected.");
        var beforeShutdown = await context.ProbeAsync(context.NodeB, actorId, new ProbeReq("ST-B2", "before-source-cleanup-loss"));
        SpotActorTransferScenarioContext.Require(beforeShutdown.NodeRid == "actor-b", $"ST-B2 probe expected actor-b, got {beforeShutdown.NodeRid}.");
        ...
        SpotActorTransferScenarioContext.Require(afterShutdown.NodeRid == "actor-b", $"ST-B2 target ownership was lost after source shutdown: {afterShutdown.NodeRid}.");
        SpotActorTransferScenarioContext.Require(afterShutdown.StateVersion == 22, $"ST-B2 state changed after source cleanup loss: {afterShutdown.StateVersion}.");
