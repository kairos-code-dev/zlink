...
void gateway_monitor_parent_self_close_handler (const zlink_service_event_t *, void *)
{
...
{
    if (!probe_) {
        discard_gateway_message (NULL, parts_, part_count_, NULL);
        return;
    }
    if (part_count_ > 0) {
...
void gateway_handler_a (const zlink_routing_id_t *source_rid_,
                        ...
                        size_t part_count_,
                          void *)
{
    record_gateway_message (g_probe_a, source_rid_, parts_, part_count_, NULL);
}
...
void gateway_handler_b (const zlink_routing_id_t *source_rid_,
                        ...
                        size_t part_count_,
                          void *)
{
    record_gateway_message (g_probe_b, source_rid_, parts_, part_count_, NULL);
}
  ...
  std::vector<zlink_registry_gateway_peer_entry_t> *entries_out_)
{
    ...
    if (zlink_get_routing_id (gateway_, &gateway_rid) != 0)
        return false;
                                         ...
                                         int timeout_ms_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zlink_get_routing_id (gateway_, &rid));
                                             ...
                                             int timeout_ms_)
{
    zlink_routing_id_t rid;
    memset (&rid, 0, sizeof (rid));
    if (zlink_get_routing_id (gateway_, &rid) != 0)
        return false;
...
bool wait_gateway_recv_payload (void *gateway_,
                                ...
                                int timeout_ms_)
{
    ...
    const int step_ms = 10;
    const int attempts = timeout_ms_ / step_ms;
    int last_errno = EAGAIN;
    for (int i = 0; i < attempts; ++i) {
        zlink_msg_t *parts = NULL;
        size_t part_count = 0;
        zlink_routing_id_t source_rid;
        memset (&source_rid, 0, sizeof (source_rid));
        if (zlink_gateway_recv (gateway_, &source_rid, &parts, &part_count,
                                ZLINK_DONTWAIT)
            == 0) {
            const bool matched =
              part_count >= 1
              && zlink_msg_size (&parts[part_count - 1]) == expected_size
              && memcmp (zlink_msg_data (&parts[part_count - 1]), expected_payload_,
                         expected_size)
                   == 0;
            ...
            if (getenv ("ZLINK_TEST_DEBUG")) {
                char buffer[128];
                snprintf (buffer, sizeof (buffer),
                          "gateway recv mismatch parts=%zu last_size=%zu",
                          part_count,
                          part_count > 0 ? zlink_msg_size (&parts[part_count - 1])
                                         : 0u);
                step_log (buffer);
            }
            errno = EPROTO;
        ...
        if (errno != EAGAIN) {
            if (getenv ("ZLINK_TEST_DEBUG")) {
                char buffer[128];
                snprintf (buffer, sizeof (buffer),
                          "gateway recv failed errno=%d", errno);
                step_log (buffer);
            }
            return false;
        }
    ...
    if (getenv ("ZLINK_TEST_DEBUG")) {
        char buffer[128];
        snprintf (buffer, sizeof (buffer),
                  "gateway recv wait timed out errno=%d", last_errno);
        step_log (buffer);
    }
    errno = last_errno;
    return false;
...
static void test_gateway_can_be_polled_via_service_instance ()
{
    ...
    void *gateway = create_gateway_attached (
      ctx, client_discovery, "ready-svc", "gw-ready", &discard_gateway_message);
    TEST_ASSERT_NOT_NULL (gateway);
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_send_ready_handler (gateway, &gateway_ready_handler, NULL));
    TEST_ASSERT_EQUAL_INT (-1, zlink_send_ready_handler (gateway, NULL, NULL));
    TEST_ASSERT_EQUAL_INT (EINVAL, errno);
    step_log ("gateway ready handler installed");

    gateway_server_t server;
    create_server_gateway (&server, ctx, registry_router, "ready-svc",
                           "gw-ready-server", &discard_gateway_message);
    char endpoint[MAX_SOCKET_STRING];
    int bind_seed = 22400;
    bind_gateway_with_port_seed (server.gateway, "tcp", &bind_seed, endpoint,
                                 sizeof (endpoint), 2000);
    step_log ("gateway route ready");
...
static void test_gateway_send_ready_handler_reentrant_replace_returns_edeadlk ()
{
    ...
    void *gateway = create_gateway_attached (
      ctx, discovery, "ready-reentrant", "gw-reentrant",
      &discard_gateway_message);
    TEST_ASSERT_NOT_NULL (gateway);
    ...
    gateway_server_t server;
    create_server_gateway (&server, ctx, registry_router, "ready-reentrant",
                           "gw-reentrant-server", &discard_gateway_message);
    char endpoint[MAX_SOCKET_STRING];
    int bind_seed = 22401;
    bind_gateway_with_port_seed (server.gateway, "tcp", &bind_seed, endpoint,
                                 sizeof (endpoint), 2000);
...
static void test_gateway_send_ready_handler_self_close_is_safe ()
{
    ...
    void *gateway = create_gateway_attached (
      ctx, discovery, "ready-self-close", "gw-self-close",
      &discard_gateway_message);
    TEST_ASSERT_NOT_NULL (gateway);
    TEST_ASSERT_SUCCESS_ERRNO (zlink_send_ready_handler (
    ...
    gateway_server_t server;
    create_server_gateway (&server, ctx, registry_router, "ready-self-close",
                           "gw-self-close-server", &discard_gateway_message);
    char endpoint[MAX_SOCKET_STRING];
    int bind_seed = 22411;
    bind_gateway_with_port_seed (server.gateway, "tcp", &bind_seed, endpoint,
                                 sizeof (endpoint), 2000);
    {
        ...
        bool observed = false;
        for (int i = 0; i < attempts; ++i) {
            if (g_gateway_self_close_errno == EBUSY) {
                observed = true;
                break;
            }
            msleep (step_ms);
...
static void test_gateway_monitor_callback_parent_destroy_returns_ebusy ()
{
    ...
    void *gateway = create_gateway (
      ctx, "svc-monitor-self-close", "gw-monitor-self-close",
      &discard_gateway_message);
    TEST_ASSERT_NOT_NULL (gateway);
...
static void test_gateway_send_rid_tcp ()
{
    ...
    zlink_routing_id_t rid;
    memset (&rid, 0, sizeof (rid));
    TEST_ASSERT_SUCCESS_ERRNO (zlink_get_routing_id (server2.gateway, &rid));
...
static void test_gateway_protocol_tls ()
{
    ...
    TEST_ASSERT_NOT_NULL (client);
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_set_tls_client (client, files.ca_cert.c_str (), "localhost", 0));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_set_tls_server (server.gateway, files.server_cert.c_str (),
...
static void test_gateway_weighted_load_balancing ()
{
    ...
    if (getenv ("ZLINK_TEST_DEBUG")) {
        ...
        TEST_ASSERT_SUCCESS_ERRNO (zlink_get_routing_id (server1.gateway, &rid1));
        TEST_ASSERT_SUCCESS_ERRNO (zlink_get_routing_id (server2.gateway, &rid2));
        char buffer[160];
...
static void test_gateway_recv_model_receive_regression ()
{
    step_log ("gateway recv regression: create ctx");
    void *ctx = zlink_ctx_new ();
    TEST_ASSERT_NOT_NULL (ctx);
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zlink_set_option (
      client, ZLINK_OPT_LINGER, &linger, sizeof (linger)));
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_set_routing_id (server, "gw-recv-server", 14));
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_set_routing_id (client, "gw-recv-client", 14));

    step_log ("gateway recv regression: bind/connect");
    char endpoint[MAX_SOCKET_STRING];
    int bind_seed = 22562;
    bind_gateway_with_port_seed (server, "tcp", &bind_seed, endpoint,
                                 sizeof (endpoint), 3000);
    ...
    step_log ("gateway recv regression: send");
    send_gateway_with_timeout (client, "recv-path", 2000);

    step_log ("gateway recv regression: direct recv");
...
static void test_gateway_runtime_reads_are_safe_during_concurrent_send ()
{
    ...
    std::thread reader ([&] () {
        while (stop_reads.load () == 0) {
            zlink_routing_id_t rid;
            memset (&rid, 0, sizeof (rid));
            if (zlink_get_routing_id (client, &rid) != 0 || rid.size == 0) {
...
static void test_gateway_attach_and_monitor_queries_are_safe_same_handle ()
{
    ...
    for (int round = 0; round < 32; ++round) {
        void *gateway =
          create_gateway (ctx, "svc-attach-query", NULL,
                          &discard_gateway_message);
        TEST_ASSERT_NOT_NULL (gateway);
        ...
        std::thread reader ([&] () {
            ...
            while (attach_done.load () == 0) {
                zlink_routing_id_t rid;
                memset (&rid, 0, sizeof (rid));
                if (zlink_get_routing_id (gateway, &rid) != 0) {
...
#define RUN_GATEWAY_TEST(name)                                                 \
    ...
    RUN_GATEWAY_TEST (test_gateway_callback_model_receive_regression);
