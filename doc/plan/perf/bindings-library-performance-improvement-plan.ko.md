...
| `ws` | `MULTI_SPOT` | `보류(53.7%)` | `보류(52.9%)` | `통과(82.5%)` | `통과(76.3%)` | `보류(46.6%)` | `보류(42.8%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Rust `perf_rust_multi_linux_20260525_003928.txt` median 기준이다. SPOT client 복사 제거 + 4-worker drain 위에, 131072B/262144B에서만 public `Poller` `POLLIN` wait를 사용한다. 1024B와 65536B는 같은 최종 측정에서 통과권으로 올라갔고, 131072B/262144B는 35.7/35.3%(`perf_rust_multi_linux_20260524_201915.txt`를 같은 fresh C 기준으로 재계산)에서 46.6/42.8%로 개선됐지만 아직 SPOT 기준 아래다. 64B/256B는 보류로 남는다. |
