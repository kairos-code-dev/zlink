...
{
    const int rc =
      zlink_bind (detail::native_handle (*this), endpoint_.c_str ());
    if (rc != 0)
        throw bind_error_t (detail::bind_result_from_errno (zlink_errno ()),
                            zlink_errno ());
}
...
{
    const int rc =
      zlink_connect (detail::native_handle (*this), endpoint_.c_str ());
    if (rc != 0)
...
{
    const int rc =
      zlink_unbind (detail::native_handle (*this), endpoint_.c_str ());
    if (rc != 0)
        throw connect_error_t (
          detail::connect_result_from_errno (zlink_errno ()), zlink_errno ());
}
...
{
    const int rc =
      zlink_disconnect (detail::native_handle (*this), endpoint_.c_str ());
    if (rc != 0)
        throw connect_error_t (
          detail::connect_result_from_errno (zlink_errno ()), zlink_errno ());
}
...
void socket_t::disconnect_rid (const routing_id_t &peer_rid_)
{
    ...
    const int rc =
      zlink_disconnect_rid (detail::native_handle (*this), &native);
    if (rc != 0)
        throw connect_error_t (static_cast<connect_result_t> (rc),
                               zlink_errno ());
}
...
{
    const int rc =
      zlink_set_tls_server (detail::native_handle (*this), cert_.c_str (),
    ...
    return detail::submit_one_message_part (
      part_, [&] (zlink_msg_t *part_out_, zlink_part_flag_t part_flag_) {
          return zlink_send_part (
            detail::native_handle (*this), part_out_,
...
int socket_t::receive (received_t &received_, recv_flags_t flags_)
{
    ...
    if (envelope.single_part.has_value ()) {
        ...
        received_ = received_t (
          source_rid, source_spot_rid, request_seq, std::move (envelope.parts),
          std::function<void (std::vector<message_t> &, send_flags_t)> (),
          std::move (send_fn));
    }
                                      ...
                                      message_t &part_)
{
    ...
    return detail::send_single_no_wait_result (
      result_, part_,
      [&] (zlink_msg_t *part_out_, zlink_part_flag_t part_flag_) {
          return zlink_publish_part (detail::native_handle (*this),
                                     topic_id_.c_str (), part_out_,
                                     ZLINK_DONTWAIT, part_flag_);
      });
}
                              ...
                              recv_flags_t flags_)
{
    ...
    source_rid_out_ = source_rid && source_rid->size > 0
                        ? std::optional<routing_id_t> (
                            zlink::detail::native_routing_id (*source_rid))
                        : std::nullopt;
...
int socket_t::subscription_event (routing_id_t &source_rid_out_,
...
int socket_t::set_subscription (const std::string &filter_)
{
    ...
    return zlink_set_subscription (detail::native_handle (*this),
                                   filter_.c_str ());
}
...
{
    detail::validate_no_embedded_null (filter_, "filter");
    return zlink_unset_subscription (detail::native_handle (*this),
                                     filter_.c_str ());
}
                                  ...
                                  recv_flags_t flags_)
{
    ...
    const int rc =
      subscription_event (source_rid, event_.subscribed, event_.topic, flags_);
    if (rc != 0)
        return rc;
...
{
    return zlink_set_routing_id (detail::native_handle (*this), data_.data (),
                                 data_.size ());
}
...
int socket_t::get_routing_id_raw (routing_id_t &routing_id_) const
{
    ...
    const int rc = zlink_get_routing_id (
      const_cast<void *> (detail::native_handle (*this)), &native);
    if (rc == 0)
