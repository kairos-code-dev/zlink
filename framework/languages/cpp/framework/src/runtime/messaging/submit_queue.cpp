...
submit_queue_t::submit_queue_t (std::size_t capacity)
  : _capacity (capacity)
{
  if (capacity == 0) {
...
std::size_t
submit_queue_t::pending_count () const
{
...
void
submit_queue_t::enqueue (pending_submit_t pending)
{
  std::lock_guard<std::mutex> lock (_gate);
  throw_if_disposed ();
...
bool
submit_queue_t::try_peek (pending_submit_t &pending) const
{
  std::lock_guard<std::mutex> lock (_gate);
  if (_pending.empty ()) {
    return false;
...
bool
submit_queue_t::try_dequeue_expected (const pending_operation_t &expected,
                                      pending_submit_t &pending)
{
  std::lock_guard<std::mutex> lock (_gate);
  if (_pending.empty () ||
      !_pending.front ().operation ().valid () ||
      !detail::same_pending_operation (
        _pending.front ().operation (), expected)) {
    return false;
  }
...
std::vector<pending_submit_t>
submit_queue_t::dispose_all ()
{
  std::lock_guard<std::mutex> lock (_gate);
  if (_disposed) {
    return {};
...
void
submit_queue_t::throw_if_disposed () const
{
  if (_disposed) {
