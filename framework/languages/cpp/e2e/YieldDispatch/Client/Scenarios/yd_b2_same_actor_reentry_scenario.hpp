  ...
  const yield_actor_scenario_context_t &actors)
{
    ...
    auto actor_fast =
      connector.request (actor_fast_req_t{.request_id = request_id, .marker = "b2-fast"})
        ...
        .timeout (std::chrono::milliseconds (30000))
        .template submit<actor_yield_reply_t> ();
    ensure (static_cast<bool> (actor_fast), "YD-B2 ActorFastReq failed");
    auto actor_yield_reply = actor_yield.get ();
    ensure (static_cast<bool> (actor_yield_reply), "YD-B2 ActorYieldReq failed");
    auto evidence =
    ...
    ensure (static_cast<bool> (evidence), "YD-B2 evidence request failed");
