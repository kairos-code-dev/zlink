...
class spot_node_t
{
  ...
  public:
    ...
    spot_node_t (context_t &ctx_, spot_node_mode_t mode_) : spot_node_t (ctx_, mode_, mode_ctor_tag_t ()) {}
    ...
    void connect_router_channel_peer (const std::string &channel_name_, const std::string &endpoint_);

    void disconnect_router_channel_peer (const std::string &channel_name_, const std::string &endpoint_);

    void disconnect_router_channel_peer_rid (const std::string &channel_name_, const routing_id_t &peer_rid_);

    void attach_discovery (discovery_t &discovery_);

    void attach_spot_route_channel_discovery (const std::string &channel_name_, discovery_t &discovery_);

    template <typename DealerT> void attach_channel_dealer (discovery_t &discovery_, DealerT &dealer_)
    {
        attach_channel_dealer_impl (discovery_, static_cast<zlink::socket_t &> (dealer_));
    }

    template <typename DealerT> void attach_channel_dealer_manual (const std::string &channel_name_, DealerT &dealer_)
    {
    ...
    std::optional<actor_part_t> recv_actor_part (const actor_ref_t &actor_, recv_flags_t flags_ = recv_flags_t::none);
    ...
    std::vector<spot_node_subject_entry_t> subjects (const spot_node_subject_filter_t *filter_ = nullptr) const;

    std::vector<spot_node_subject_entry_t> subjects (const spot_node_subject_filter_t &filter_) const
    {
        return subjects (&filter_);
    }

    std::vector<spot_node_socket_entry_t> internal_sockets (const spot_node_socket_filter_t *filter_ = nullptr) const;

    std::vector<spot_node_socket_entry_t> internal_sockets (const spot_node_socket_filter_t &filter_) const
    {
    ...
    static actor_ref_t remote_actor_ref (const routing_id_t &target_node_rid_, const std::string &actor_id_);

    actor_destroy_operation_t destroy_actor (const actor_ref_t &actor_);

    actor_join_operation_t
    join_actor (const actor_ref_t &actor_, const routing_id_t &dest_node_rid_, const routing_id_t &dest_spot_rid_);

    actor_join_entry_spot_operation_t join_actor_entry_spot (const actor_ref_t &actor_,
                                                             const routing_id_t &dest_node_rid_);

    actor_leave_operation_t leave_actor (const actor_ref_t &actor_, const routing_id_t &current_spot_rid_);

    actor_lookup_operation_t remote_actor_get_ref (const routing_id_t &target_node_rid_, const std::string &actor_id_);
  ...
  private:
    ...
    void attach_channel_dealer_manual_impl (const std::string &channel_name_, zlink::socket_t &dealer_);
