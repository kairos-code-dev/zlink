...
public final class PerfPubSub {
    ...
    public static int run(String transport, int msgSize) {
        try (Context context = new Context();
            ...
            publisher.setOption(SocketOptions.XPUB_NODROP, 1);
            ...
            try (Arena arena = Arena.ofConfined()) {
                ...
                if (!waitForSubscriber(publisher, subscriber, payload, payloadSize)) {
                    return 2;
                }

                int warmupCount = PerfCommon.resolveWarmupCount(PATTERN, msgSize);
                PhaseResult warmup = runPhase(publisher, subscriber, payload,
                ...
                PhaseResult active = runPhase(publisher, subscriber, payload,
    ...
    private static boolean waitForSubscriber(Socket publisher, Socket subscriber,
                                             MemorySegment payload,
                                             int payloadSize) {
        for (int i = 0; i < 2000; i++) {
            PerfSingleMetricHeader.stampPayload(payload, payloadSize, 0,
                PerfSingleMetricHeader.PHASE_WARMUP, payloadSize, i + 1L,
                PerfSingleMetricHeader.nowUs());
            sendBlocking(publisher, payload, payloadSize, SendFlag.NONE);
            try (Message msg = receiveMessage(subscriber, ReceiveFlag.DONTWAIT)) {
                return !msg.more() && msg.size() == payloadSize;
        ...
        Thread receiverThread = new Thread(() -> {
            ...
            try {
                while (true) {
                    ...
                    try {
                        Message msg = receiveMessage(receiver,
                            done ? ReceiveFlag.DONTWAIT : ReceiveFlag.NONE);
                        lastRecvNs = System.nanoTime();
                        try (msg) {
                            accountMessage(msg, payloadSize, active, header,
                                received, latency);
                        }

                        while (true) {
                            Message burst = receiveMessage(receiver,
                                ReceiveFlag.DONTWAIT);
                            lastRecvNs = System.nanoTime();
                            try (burst) {
                                accountMessage(burst, payloadSize, active,
    ...
    private static void accountMessage(Message msg, int payloadSize,
                                       boolean active,
                                       PerfSingleMetricHeader.Header header,
                                       LongAdder received,
