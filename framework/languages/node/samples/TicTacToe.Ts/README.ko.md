...
- `Server/Play/Domain/TicTacToe/`: board cell 검증, turn 검증, winner/draw 판정을 맡는
  순수 게임 규칙을 둔다.
...
- 첫 actor join 때 self-join notify 를 보내지 않는다. 두 번째 join 때 기존 member 에게
  `PlayerJoinedNotify` 와 running `GameStateNotify` 를 보낸다.
- `p1`, `p2` 가 `0,3,1,4,2` 순서로 `PlaceMarkReq` 를 보내고 최종 board 는
  `XXXOO....` 가 된다.
- 최종 status 는 `Finished` 이고 winner 는 `p1` 이다.
