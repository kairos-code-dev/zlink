...
internal static class StC2SourceDownAfterTargetCommitScenario
{
    public static async Task RunAsync(SpotActorTransferScenarioContext context)
    {
        ...
        var beforeTransferReply = await bound.Request(new BoundPushReq("ST-C2", "bound-before-transfer"))
            ...
            .Async<BoundPushRes>();
        SpotActorTransferScenarioContext.Require(beforeTransferReply.NodeRid == "actor-a", $"ST-C2 pre-transfer bound push expected actor-a, got {beforeTransferReply.NodeRid}.");
        await beforeTransferPush;

        var join = await context.JoinAsync(context.NodeA, actorId, new JoinTargetReq("ST-C2", spotRid));
        SpotActorTransferScenarioContext.Require(join.Accepted, "ST-C2 join was rejected.");
        ...
        SpotActorTransferScenarioContext.Require(beforeShutdown.NodeRid == "actor-b", $"ST-C2 target ref expected actor-b, got {beforeShutdown.NodeRid}.");

        await context.ShutdownAsync(context.NodeA);
        await Task.Delay(TimeSpan.FromSeconds(2));

        var afterShutdown = await context.GetActorRefAsync(context.NodeB, actorId);
        SpotActorTransferScenarioContext.Require(afterShutdown.NodeRid == "actor-b", $"ST-C2 target ref changed after source shutdown: {afterShutdown.NodeRid}.");
        ...
        SpotActorTransferScenarioContext.Require(probe.NodeRid == "actor-b", $"ST-C2 probe expected actor-b, got {probe.NodeRid}.");
        SpotActorTransferScenarioContext.Require(probe.SpotRid == spotRid, "ST-C2 probe did not reach target spot after source shutdown.");
        ...
        SpotActorTransferScenarioContext.Require(pushReply.NodeRid == "actor-b", $"ST-C2 bound push reply expected actor-b, got {pushReply.NodeRid}.");
        SpotActorTransferScenarioContext.Require(notify.Payload.NodeRid == "actor-b", $"ST-C2 bound push notify expected actor-b, got {notify.Payload.NodeRid}.");
        SpotActorTransferScenarioContext.Require(notify.Payload.Marker == "bound-after-source-down", "ST-C2 bound push notify marker mismatch.");
        await context.WaitEvidenceAsync(context.NodeB, [
