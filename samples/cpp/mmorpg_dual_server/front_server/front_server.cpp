...
int FrontServer::run (int argc, char *argv[])
{
    ...
    zone_svc.set_outgame_handler ([&outgame_gw] (const std::string &sk, const std::string &op,
    ...
    spot_sync.set_event_handler (
      [&zone_svc] (const std::string &topic, const std::string &payload) {
          zone_svc.on_spot_event (topic, payload);
      });

    std::printf ("[front-%d-%d] zone(%d,%d) on %s, spot on port %s\n", zone_x, zone_y, zone_x,
                 zone_y, bind_endpoint.c_str (), spot_port.c_str ());
    std::fflush (stdout);
