...
{
    session_binding_key_t () : stream (NULL) { memset (&session_rid, 0, sizeof (session_rid)); }

    session_binding_key_t (void *stream_, const zlink_routing_id_t &session_rid_) :
        stream (stream_), session_rid (session_rid_)
    {
    }
    ...
    bool operator< (const session_binding_key_t &other_) const
    {
        ...
        return memcmp (session_rid.data, other_.session_rid.data, session_rid.size) < 0;
    }
...
{
    actor_bound_session_transfer_t () : valid (false), session_node (NULL), stream (NULL)
    {
...
struct actor_session_state_t
{
    ...
    binding_map_t::iterator find_binding (void *stream_, const zlink_routing_id_t *session_rid_);
    binding_map_t::const_iterator find_binding (const void *stream_,
                                                const zlink_routing_id_t *session_rid_) const;
    binding_map_t::iterator find_remote_binding (const zlink_routing_id_t &session_rid_,
                                                 const char *actor_id_,
                                                 uint64_t generation_);
    binding_map_t::iterator bindings_end ();
    binding_map_t::const_iterator bindings_end () const;
    session_binding_t &ensure_binding (void *stream_, const zlink_routing_id_t &session_rid_);
    ...
    actor_bound_session_transfer_t capture_bound_session (const actor_handle_t *source_) const;
    bool transfer_bound_session (const actor_bound_session_transfer_t &transfer_,
                                 actor_handle_t *target_,
                                 uint64_t changed_ms_);
    bool detach_actor (actor_handle_t *actor_, bool erase_entry_, bool erase_owner_if_unused_);
