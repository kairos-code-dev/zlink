...
현재 C++ `ResilienceLifecycle`은 대부분의 public recovery 흐름을 전용 Registry, Provider, Consumer, Client target으로 실행한다. Client target은 HTTP-only dispatcher이고, framework channel client는 Consumer/Provider/Registry role 안에서 실행한다. client support는 ResilienceLifecycle 전용 option/assert/evidence/topology
...
| `RL-C4` | 구현 | HTTP-only client가 registry 종료 전 Consumer `/profile/request/manual`을 호출하고, runner가 registry를 중지한 상태에서도 Consumer role의 established manual channel request가 성공하고 provider evidence가 남는지 검증한다. 이후 runner가 `registry_host_factory.hpp` 경로로 구성된 registry와 provider A를 재기동하고, Consumer `/profile/request/new-client`가 `rl-c4-after-restart` request와 provider evidence를 성공시키는지 확인한다. |
