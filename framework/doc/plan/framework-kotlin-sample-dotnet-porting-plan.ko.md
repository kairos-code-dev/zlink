...
7. 서버 구동에는 Gradle build, JVM 시작, coroutine/framework 초기화에 필요한 충분한 시간을 준다. 다만
...
10. 실패하면 같은 샘플 안에서 원인을 고치고 다시 실행한다. Java internal bridge나 runner-only helper로
    ...
    서버 준비 대기는 build와 JVM/coroutine/framework 초기화에 필요한 범위로 제한하며, 로컬 테스트에서
    납득하기 어려운 긴 대기가 필요하면 readiness나 서버 시작 경로의 버그로 보고 원인을 추적한다.
