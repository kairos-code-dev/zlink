...
struct FLoopbackServer
{
  ...
  bool Start ()
  {
    ...
    Address->SetPort (0);
    if (!bValidAddress || !ListenSocket->Bind (*Address) ||
    ...
    Endpoint =
      FString::Printf (TEXT ("tcp://127.0.0.1:%d"), BoundAddress->GetPort ());
    return true;
  }
  ...
  bool Accept ()
  {
    ...
    ClientSocket = ListenSocket->Accept (
      TEXT ("ZLinkStreamConnectorAutomationClient"));
  ...
  static bool DecodeLz4Payload (const TArray<uint8> &Payload,
                                TArray<uint8> &Output)
  {
    Output.Empty ();
    if (Payload.Num () < 4) {
    ...
    while (Offset < Payload.Num ()) {
      ...
      const int32 MatchOffset =
        static_cast<int32> (Payload[Offset]) |
  ...
  bool ReadExact (int32 Count, TArray<uint8> &Output)
  {
    ...
    while (ClientSocket && Offset < Count) {
      ...
      if (!ClientSocket->Recv (
            Output.GetData () + Offset, Count - Offset, BytesRead) ||
          BytesRead <= 0) {
        return false;
      }
      Offset += BytesRead;
    ...
    if ((Flags & 0x02) != 0) {
      ...
      for (uint8 Index = 0; Index < MetadataCount; ++Index) {
        ...
        for (uint16 ValueIndex = 0; ValueIndex < ValueSize; ++ValueIndex) {
          Value.AppendChar (
            static_cast<TCHAR> (Header[Offset + ValueIndex]));
        }
        Offset += ValueSize;
                  ...
                  bool bCompressed = false)
  {
    ...
    Header.Append (
      reinterpret_cast<const uint8 *> (NameBytes.Get ()),
      NameBytes.Length ());
    if (!Metadata.IsEmpty ()) {
      ...
      for (const auto &Entry : Metadata) {
        ...
        MetadataBytes.Append (
          reinterpret_cast<const uint8 *> (KeyBytes.Get ()),
          KeyBytes.Length ());
        AppendU16 (MetadataBytes, static_cast<uint16> (ValueBytes.Length ()));
        MetadataBytes.Append (
          reinterpret_cast<const uint8 *> (ValueBytes.Get ()),
          ValueBytes.Length ());
      }
    ...
    TArray<uint8> PayloadBytes;
    PayloadBytes.Append (
      reinterpret_cast<const uint8 *> (PayloadText.Get ()),
      PayloadText.Length ());
    if (bCompressed) {
  ...
  {
    return SendFrame (
      3, RequestSeq, PacketName, Payload, Metadata, bCompressed);
  }
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST (
  FZLinkStreamConnectorLoopbackTest,
  "ZLink.StreamConnector.Loopback",
  EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool
FZLinkStreamConnectorLoopbackTest::RunTest (const FString &Parameters)
{
  (void) Parameters;
  ...
  Connector->OnPacketReceivedNative.AddLambda (
    [&bPacketReceived,
     ...
     &ReceivedTraceId,
     &bReceivedCompressed](
      const FZLinkStreamPacket &Packet) {
  ...
  TestEqual (TEXT ("send packet name"),
             SendPacketName,
             FName (TEXT ("chat.send")));
  TestEqual (TEXT ("send payload"),
             SendPayload,
  ...
  Connector->SendJsonWithOptions (
    TEXT ("chat.send.metadata"), TEXT ("{\"text\":\"hello\"}"), SendOptions);
  ...
  TestEqual (TEXT ("metadata send trace id"),
             SendMetadata[TEXT ("traceId")],
             FString (TEXT ("send-1")));
  TestEqual (TEXT ("metadata send compressed payload"),
             SendPayload,
             FString (TEXT ("{\"text\":\"hello\"}")));

  Connector->RequestJson (
    TEXT ("chat.request"), TEXT ("{\"text\":\"hello\"}"), 1.0f);
  uint8 RequestKind = 0;
  ...
  TestEqual (TEXT ("request packet name"),
             RequestPacketName,
             FName (TEXT ("chat.request")));

  TMap<FString, FString> ResponseMetadata;
  ResponseMetadata.Add (TEXT ("traceId"), TEXT ("reply-1"));
  TestTrue (TEXT ("server sends response"),
            Server.SendResponseFrame (
              RequestSeq,
              TEXT ("reply"),
              TEXT ("{\"ok\":true}"),
              ResponseMetadata,
              true));
  TestEqual (TEXT ("pending response count before dispatch"),
             Connector->PendingDispatchCount (),
             1);
  ...
  TestEqual (TEXT ("pending response count after dispatch"),
             Connector->PendingDispatchCount (),
             0);
  TestEqual (TEXT ("completed packet name"),
             CompletedPacketName,
             FName (TEXT ("reply")));
  TestEqual (TEXT ("completed trace id"),
             CompletedTraceId,
             FString (TEXT ("reply-1")));
  ...
  TestTrue (TEXT ("server receives metadata request packet"),
            Server.ReadFrame (RequestKind,
                              ...
                              RequestMetadata,
                              RequestPayload));
  TestEqual (TEXT ("metadata request flags"),
             RequestFlags,
             static_cast<uint8> (7));
  TestEqual (TEXT ("metadata request trace id"),
             RequestMetadata[TEXT ("traceId")],
             FString (TEXT ("request-1")));
  TestEqual (TEXT ("metadata request compressed payload"),
             RequestPayload,
             FString (TEXT ("{\"text\":\"hello\"}")));
  ...
  TestTrue (TEXT ("server sends push"),
            Server.SendPushFrame (
              TEXT ("ServerNotify"), TEXT ("{}"), PushMetadata, true));
  TestEqual (TEXT ("pending push count before dispatch"),
             Connector->PendingDispatchCount (),
             1);
  TestFalse (TEXT ("push waits for dispatch"), bPacketReceived);
  Connector->Dispatch ();
  TestTrue (TEXT ("native callback receives push"), bPacketReceived);
  TestEqual (TEXT ("pending push count after dispatch"),
             Connector->PendingDispatchCount (),
             0);
  TestEqual (TEXT ("received packet name"),
             ReceivedPacketName,
             FName (TEXT ("ServerNotify")));
  TestEqual (TEXT ("received trace id"),
             ReceivedTraceId,
             FString (TEXT ("push-1")));
