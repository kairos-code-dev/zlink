...
같은 timestamp 경계와 평균 계산을 사용해도 C의 평균 latency가 1ms보다 낮은 secure
PUBSUB 대형 셀에서는 managed subscriber가 형성하는 queue 깊이가 비율을 크게 만든다.
...
| 2 | .NET | `PAIR` 완료, `PUBSUB` tcp, ws, wss, tls 완료 | 미측정 | `PUBSUB / inproc` 전체 크기를 측정한다. |
