...
export async function runSmC2(options: ClientOptions): Promise<void> {
  ...
  const outbound = await postJson<SpotOutboundRouteReply>(options.playAUrl, '/spot/outbound', {
    spotRid,
    marker: 'sm-c2'
  } satisfies SpotOutboundRouteReq);
  ensure(outbound.accepted, 'SM-C2 outbound route was not accepted.');

  const negative = await postJson<SpotOutboundRouteReply>(options.playAUrl, '/spot/outbound-negative', {
    spotRid,
  ...
  const expectedEvidence = [
    ...
    'dispatch-error|surface=channel|kind=send|reason=handlerMissing|action=drop|packet=MissingChannelSend'
  ];
