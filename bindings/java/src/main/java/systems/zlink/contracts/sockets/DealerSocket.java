        ...
        private CompletableFuture<Received> requestInternal(List<Message> parts,
            ...
            try (Arena arena = Arena.ofConfined()) {
                ...
                RequestReplySupport.startSocketRequestProgress(future,
                    InternalAccess.socketHandle(socket), "zlink-dealer-request-progress");
            ...
            for (int i = 0; i < payload.size(); i++) {
                ...
                try (Arena arena = Arena.ofConfined()) {
                    MemorySegment nativeMsg = arena.allocate(
                        systems.zlink.runtime.nativeapi.NativeLayouts.MSG_LAYOUT);
                    InternalAccess.messageCopyTo(payload.get(i), nativeMsg);
                    int rc = Native.dealerRequestPart(InternalAccess.socketHandle(socket), nativeMsg,
