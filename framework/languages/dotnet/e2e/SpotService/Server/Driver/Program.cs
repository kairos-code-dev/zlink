...
app.MapPost("/run/{scenarioSet}", async (string scenarioSet) =>
{
    await RunAsync(options with { ScenarioSet = scenarioSet });
    return Results.Ok(new { status = "passed", scenarioSet });
});
