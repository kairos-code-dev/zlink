...
main ()
{
  ...
  if (kafka.mappings.empty () || grpc.mappings.empty () ||
      http.mappings.empty ()) {
    return 3;
  }
  ...
  if (!retry.current_policy.preserve_ordering ||
      !retry.current_policy.require_idempotency_key ||
  ...
  dead_letter.store ([&](const zlink::framework::channel_reliability_event_t &) {
    ++dead_letter_count;
  });
  ...
  if (dead_letter.duplicate_handling.find ("idempotency") ==
        std::string::npos ||
  ...
  if (flatbuffers.registered_types.empty () || yaml.paths.empty () ||
      custom_codec.codecs.empty () || custom_transport.schemes.empty ()) {
    return 6;
  }
