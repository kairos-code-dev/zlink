...
task_t<void> route_handler_invoker_t::invoke_send (const route_handler_registry_t &handlers,
                                                   ...
                                                   const framework::route_handler_context_t &context) const
{
    ...
    auto handler_task = runtime::handler_coroutine_executor ().submit<zlink::message_t> (
      [&handlers, &services, &serializers, owned_router_channel_id = std::move (owned_router_channel_id),
       owned_packet_name = std::move (owned_packet_name), owned_message = std::move (owned_message),
       owned_context = std::move (owned_context)] () -> boost::asio::awaitable<result_t<zlink::message_t>> {
          try {
              auto route_task =
                handlers.invoke_async (owned_router_channel_id, runtime::messaging::message_kind_t::command,
              ...
              co_return result_t<zlink::message_t>::failure (error.kind (), error.what (), error.is_retriable ());
          }
          catch (...) {
    ...
    detail::observe_task_completion (
      handler_task, [completion = std::move (completion)] (const result_t<zlink::message_t> &result) mutable {
          if (!result) {
              completion.complete (result_t<void>::failure (
                result.error_kind (), result.error () ? result.error ()->what () : "routed send handler failed"));
              return;
          }
    ...
    return runtime::handler_coroutine_executor ().submit<zlink::message_t> (
      [&handlers, &services, &serializers, owned_router_channel_id = std::move (owned_router_channel_id),
       ...
       owned_context = std::move (owned_context)] () -> boost::asio::awaitable<result_t<zlink::message_t>> {
          try {
              auto handler_task =
                handlers.invoke_async (owned_router_channel_id, runtime::messaging::message_kind_t::request,
                                       owned_packet_name, services, serializers, owned_message, owned_context);
              co_return result_t<zlink::message_t>::success (
