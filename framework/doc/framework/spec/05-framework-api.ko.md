...
[스펙 목차](README.ko.md) | [이전: 비동기 실행과 coroutine 정책](04-async-execution-policy.ko.md) | [다음: ZLink Framework Channel Topology](server/10-channel-topology.ko.md)
<!-- framework-adapter-nav:end -->


[문서 묶음](../common/README.ko.md) | [개요](01-overview.ko.md) | [상호작용 모델](02-interaction-model.ko.md) | [메시지 모델](03-message-model.ko.md) | [channel topology](server/10-channel-topology.ko.md) | [공통 sample](../common/sample/README.ko.md) | [공통 E2E](../common/e2e/README.ko.md) | [.NET](../dotnet/README.ko.md) | [Java](../java/README.ko.md) | [Node.js](../node/README.ko.md) | [C++](../cpp/README.ko.md)
...
- 운영 점검이나 관리 API에서는 location runtime query
  (`IZLinkLocationRuntimeQuery` — [location runtime](server/40-location-runtime.ko.md) §7)의
...
- session server와 play server를 분리하는 구조에서는 `actorId`를 client-facing
  ...
  과 표면은 [22-actor-model.ko.md](server/22-actor-model.ko.md)에서, gateway 패턴의 사용성
  결정은 [31-session-actor-dispatch.ko.md](server/31-session-actor-dispatch.ko.md)에서
  본다.
...
[.NET SPOT 문서](server/languages/dotnet/01-system-structure.ko.md) 등 별도
문서에서 따로 다룬다.
...
- location runtime/spot source는 raw monitor를 가장한 표면보다, 일정 주기로 상태를 읽고 직전 상태와
  비교해 바뀐 때만 event를 만드는 방식으로 설명하는 편이 맞다([location runtime](server/40-location-runtime.ko.md) §9).
...
[channel 메시징 §3](server/11-channel-messaging.ko.md), [SPOT 메시징 §5](server/20-spot-messaging.ko.md),
[STREAM 서버 세션](server/30-stream-session.ko.md)이 소유하며, 결과는 세 가지다.
...
| `action` | `ReplyError`, `FailCaller`, `Drop` — [§dispatch 실패 정책](server/11-channel-messaging.ko.md) 참조 |
...
([52 메시지 흐름 추적 §2](server/52-message-flow-tracing.ko.md)). observer callback 실패는
...
[.NET 언어별 스펙](server/languages/dotnet/README.ko.md)을 참고한다.
...
자세한 contract와 샘플은
[.NET SPOT 문서](server/languages/dotnet/01-system-structure.ko.md)
...
([location runtime §4](server/40-location-runtime.ko.md#4-ownergeneration-규칙)).
