...
internal static class AtdE1TimeoutScenario
{
    public static async Task RunAsync(IZlinkStreamConnector client)
    {
        ...
        var spot = await client.Request(new EnsureSpotReq(spotRid))
            ...
            .Async<EnsureSpotRes>();
        ScenarioAssert.That(spot.SpotRid == spotRid, "ATD-E1 spot creation mismatch.");

        var requestId = $"ATD-E1-{Guid.NewGuid():N}";
