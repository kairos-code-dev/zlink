...
public final class PayloadRequestHandler
    implements ZLinkRequestHandler<Contracts.PayloadRequest, Contracts.PayloadReply> {
    private final ScenarioState state;

    public PayloadRequestHandler(ScenarioState state) {
        this.state = state;
    }

    @Override
    public Contracts.PayloadReply handle(
        Contracts.PayloadRequest request,
        ZLinkRequestContext context) {
        state.record("payload-request", request.marker());
        return new Contracts.PayloadReply(
            request.marker(),
