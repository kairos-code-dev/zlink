    ...
    : BackgroundService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var second = await RequestChannelWithRetryAsync<OfferDelivery, OfferDeliveryResult>(
            SampleNames.CourierChannel("courier-b"),
            new OfferDelivery(request.DeliveryId, request.PickupAddress, request.DropoffAddress),
            cancellationToken);
        if (!second.Accepted)
        {
            await PublishStatusAsync(
