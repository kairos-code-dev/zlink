...
class discovery_service_state_t
{
  public:
    ...
    void begin_observer_notification (const std::string &channel_name_,
    ...
    void take_shutdown_observers (std::vector<discovery_observer_t *> *observers_out_);

    void snapshot_providers (std::vector<provider_info_t> *out_) const;
    void snapshot_member_peers (zlink_auto_connect_type_t auto_connect_type_,
...
class discovery_bootstrap_socket_config_t
{
  public:
    ...
    int set_option (discovery_t *owner_, int option_, const void *optval_, size_t optvallen_);
  ...
  private:
    ...
    void upsert_socket_option (int option_, const void *optval_, size_t optvallen_);
...
class discovery_bootstrap_runtime_t
{
  public:
    ...
    void apply_socket_option_to_bootstrap_dealers (discovery_t *owner_,
                                                   int option_,
                                                   const void *optval_,
                                                   size_t optvallen_) const;
    void remember_bootstrap_endpoint (discovery_t *owner_, const char *registry_endpoint_);
    bool bootstrap_completed (discovery_t *owner_, const char *registry_endpoint_) const;
    void begin_bootstrap_request (discovery_t *owner_, const std::string &registry_endpoint_);
    bool reset_bootstrap_request_if_timed_out (discovery_t *owner_,
                                               const std::string &registry_endpoint_,
                                               uint64_t now_ms_);
    ...
    void collect_pending_bootstrap_endpoints (discovery_t *owner_,
                                              std::vector<std::string> *out_) const;
    void collect_registry_pub_endpoints (discovery_t *owner_, std::set<std::string> *out_) const;
    void
    take_shutdown_state (discovery_t *owner_,
    ...
    uint32_t heartbeat_interval_ms () const { return _request_state.heartbeat_interval_ms; }
...
class discovery_uplink_runtime_t
{
  public:
    ...
    void refresh_registered_service_heartbeats (discovery_t *owner_, uint64_t now_ms_);
    void remember_registry_uplink (discovery_t *owner_, const std::string &uplink_endpoint_);
    void adopt_report_dealer (discovery_t *owner_,
                              const std::string &uplink_endpoint_,
                              socket_base_t *bootstrap_dealer_);
    void collect_uplink_endpoints (discovery_t *owner_, std::vector<std::string> *out_) const;
