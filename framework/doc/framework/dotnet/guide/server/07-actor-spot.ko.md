...
// 이 type을 지정해야 아래 membership callback들이 생긴다.
public sealed class PlayEntrySpot(IZLinkEntrySpotContext context)
    : IZLinkEntrySpot<PlayerActor>
{
    // Framework가 생성자로 넣어 준 context를 그대로 노출한다. 이 property로
    // handler 등록, Actor 종료, outbound 호출에 접근한다.
...
- 이 챕터 계약의 실행 검증 예문: [13-interface-catalog](13-interface-catalog.ko.md) §4 — 검증 클래스 `ActorContracts`
...
[문서 목록](../../../../README.ko.md) | [이전: Spot](06-spot.ko.md) | [다음: Session과 Actor binding](08-actor-session.ko.md)
