...
generation named by the record. This control has no
`zlink_mesh_operation_kind_t` representation and produces no `COMPLETION`
record. Its operation ID is zero; operation kind, terminal result, and failure
errno do not apply. It never enters the application domain.
...
does not repeat a MeshNode, Spot, Actor handle, or domain. Core returns only
record kinds valid for that claim owner and domain. Application and
infrastructure domains drain through separate claims.
