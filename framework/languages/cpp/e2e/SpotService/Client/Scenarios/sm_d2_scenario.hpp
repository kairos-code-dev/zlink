                                ...
                                const std::string &session_stream_endpoint)
{
    if (play_b_http_endpoint.empty ()) {
        throw std::runtime_error ("ZLINK_CPP_E2E_PLAY_B_HTTP_ENDPOINT is required for SM-D2");
    }
    if (session_stream_endpoint.empty ()) {
        throw std::runtime_error ("ZLINK_CPP_E2E_STREAM_ENDPOINT is required for SM-D2");
    }
