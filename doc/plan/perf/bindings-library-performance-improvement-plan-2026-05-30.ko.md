...
| 4 | Node | `bindings/node/perf` | `미달 1/144 (0.7%)` | `미달 12/156 (7.7%)` | Single은 current C 제한 재측정으로 routed tcp 대용량 5개가 통과로 회복했고, `DEALER_ROUTER tcp 131072B`만 남았다. Multi는 10% gate 아래로 내려왔다. fastpath 변경, single routed 단일 payload native 수신, single routed tcp 대용량 sender native submit과 HWM floor 64, `sendToSpot` DontWait result-code 경로, `MULTI_PUBSUB` 단일 payload 내부 수신 경로, SPOT large active slot 16 기본값, stream echo direct result send 경로를 적용했다. |
| 5 | Go | `bindings/go/perf` | `미달 6/144 (4.2%)` | `미달 19/192 (9.9%)` | `MULTI_SPOT_REQREP tcp 4096B`는 request message 누수 수정으로 통과했고, 같은 complete 검증에서 `ws 131072B`도 통과로 회복했다. 2026-06-04 current C/Go 제한 재측정에서 `MULTI_ROUTER_ROUTER tcp 256B`도 통과로 회복해 Go multi는 10% gate 아래로 내려왔다. 잔여 routed echo, one-way 64B, builder hot path 후보를 추가로 시험했지만 코드 변경으로 새 통과를 만들지는 못했다. |
| 6 | Rust | `bindings/rust/perf` | `미달 20/144 (13.9%)` | `미달 11/192 (5.7%)` | Single 공통 송신 loop를 public `Message::with_size(...).data_mut()` 직접 작성으로 바꿔 `PUBSUB wss 64B`를 통과로 올렸다. 2026-06-04 current C/Rust 제한 재측정에서 `PUBSUB ws 64B`, `PUBSUB tls 256B`, `SPOT tcp/ws/tls 1024B`도 통과로 회복했다. routed 대용량과 `PUBSUB tcp/tls 64B`는 아직 미달한다. |
| 7 | Python | `bindings/python/perf` | `tcp/64 제한 통과` | `tcp/64 smoke partial` | 2026-06-03에 public socket contract와 perf 의미를 복구하면서 perf script의 private native active-loop 직접 호출을 제거했다. public Python API 경로의 single tcp/64 제한 재측정에서 6개 패턴이 모두 기준을 넘겼다. 2026-06-04 current Python multi tcp/64 smoke는 `MULTI_SPOT` 제어선 누락과 `MULTI_STREAM` RESULT 누락으로 partial이며, `MULTI_PUBSUB`, `MULTI_SPOT`, `MULTI_SPOT_REQREP`도 current C 기준에 못 닿았다. 아래 Python full 표의 이전 통과 수치는 현재 코드 기준 판정으로 쓰지 않고, full matrix는 public Python API 경로로 다시 측정해야 한다. |
...
잔여 미달은 multi routed echo 일부, `MULTI_PUBSUB`의 tcp large, `MULTI_SPOT_SENDSEND`,
...
통과권에 들어왔다. 이후 stream echo server가 기존 runtime native result-send 경로를 직접 사용해
public stream handler와 packet framing 의미를 유지하면서 send operation builder 생성을 줄였고,
`MULTI_STREAM tcp 64B`가 current C 제한 재측정 기준으로 통과권에 들어왔다. 남은 차이는 이전 1차 개선 문서에서
...
| `tcp` | `MULTI_PUBSUB` | `통과(31.1%)` | `통과(33.8%)` | `통과(71.0%)` | `해당 없음` | `미달(22.1%)` | `통과(32.5%)` | 65536/131072B는 제한 재측정 Node `perf_node_multi_linux_20260604_193459_node_multi_pubsub_tcp_large_hwm2000_cli_probe_20260604.txt`, C `perf_c_multi_linux_20260604_193537_node_pubsub_tcp_large_c_recheck_20260604.txt` 기준. 나머지는 단일 payload 내부 수신 경로 추가 후 complete 측정 `perf_node_multi_linux_20260602_110744_node_multi_pubsub_single_payload_native_probe_20260602.txt` 기준. public contract와 perf 기준/HWM/profile은 바꾸지 않았다. Node multi report에 4096B RESULT가 없어 4096B는 해당 없음으로 둔다. |
...
| `tcp` | `MULTI_SPOT_SENDSEND` | `통과(34.1%)` | `통과(31.7%)` | `통과(29.0%)` | `해당 없음` | `통과(33.2%)` | `통과(41.4%)` | 65536B는 제한 재측정 Node `perf_node_multi_linux_20260604_194459_node_multi_spot_sendsend_residual_recheck_20260604.txt`, C `perf_c_multi_linux_20260604_194510_node_spot_sendsend_residual_c_recheck_20260604.txt` 기준. 131072B는 SPOT large active slot 16 기본값 검증 Node `perf_node_multi_linux_20260604_195408_node_multi_spot_sendsend_active16_default_verify_20260604.txt`와 같은 C 재측정 기준. 64/256/1024B는 `sendToSpot` DontWait result-code 경로 추가 후 complete 측정 `perf_node_multi_linux_20260602_104143_node_multi_spot_sendsend_nowait_result_small_recheck_20260602.txt` 기준. Node multi report에 4096B RESULT가 없어 4096B는 해당 없음으로 둔다. |
| `tcp` | `MULTI_STREAM` | `통과(30.2%)` | `미달(29.7%)` | `미달(25.8%)` | `해당 없음` | `통과(62.2%)` | `해당 없음` | 64/256/1024B는 stream echo direct result send 적용 후 Node `perf_node_multi_linux_20260604_204847_node_multi_stream_direct_native_send_probe_20260604.txt`, C `perf_c_multi_linux_20260604_204854_node_multi_stream_small_c_recheck_20260604.txt` 기준. 64B는 통과권에 들어왔고, 256B는 29.7%로 아직 기준에 못 닿는다. 65536B는 pending reply queue를 head-index로 바꾼 뒤 complete 측정 `perf_node_multi_linux_20260601_121847_node_multi_stream_pending_queue_probe_20260601.txt` 기준이다. MULTI_STREAM은 runner 정책상 4096B와 131072B 결과를 내지 않는다. |
...
| `wss` | `MULTI_SPOT_SENDSEND` | `통과(29.1%)` | `미달(25.1%)` | `통과(29.8%)` | `해당 없음` | `통과(64.1%)` | `통과(58.3%)` | 256B는 제한 재측정 Node `perf_node_multi_linux_20260604_194459_node_multi_spot_sendsend_residual_recheck_20260604.txt`, C `perf_c_multi_linux_20260604_194510_node_spot_sendsend_residual_c_recheck_20260604.txt` 기준으로 올랐지만 SPOT 기준에는 아직 못 닿는다. 64/1024B는 `sendToSpot` DontWait result-code 경로 추가 후 complete 측정 `perf_node_multi_linux_20260602_104143_node_multi_spot_sendsend_nowait_result_small_recheck_20260602.txt` 기준. large는 client active loop를 C-style send sweep으로 정렬한 뒤 `perf_node_multi_linux_20260601_100833_node_multi_spot_sendsend_cstyle_large_probe_20260601.txt` 기준. Node multi report에 4096B RESULT가 없어 4096B는 해당 없음으로 둔다. |
| `wss` | `MULTI_STREAM` | `미달(28.9%)` | `통과(30.5%)` | `미달(28.9%)` | `해당 없음` | `통과(99.8%)` | `해당 없음` | 64/256/1024B는 제한 재측정 Node `perf_node_multi_linux_20260604_195155_node_multi_stream_small_recheck_20260604.txt`, C `perf_c_multi_linux_20260604_195416_node_multi_stream_small_c_recheck_20260604.txt` 기준. 256B만 단순/stream 기준 30%에 닿았고, 64/1024B는 아직 미달한다. 65536B는 pending reply queue를 head-index로 바꾼 뒤 complete 측정 `perf_node_multi_linux_20260601_121847_node_multi_stream_pending_queue_probe_20260601.txt` 기준이다. MULTI_STREAM은 runner 정책상 4096B와 131072B 결과를 내지 않는다. |
...
| `tcp` | `MULTI_ROUTER_ROUTER` | `미달(36.5%)` | `통과(42.3%)` | `미달(37.7%)` | `통과(40.3%)` | `미달(38.3%)` | `통과(46.0%)` | 256B는 current C/Go 제한 재측정 C `perf_c_multi_linux_20260604_213420_go_multi_rr_tcp256_c_recheck_20260604.txt`, Go `perf_go_multi_linux_20260604_213420_go_multi_rr_tcp256_recheck_20260604.txt` 기준으로 통과했다. 64/1024/65536B는 제한 재측정 `perf_go_multi_linux_20260601_200558_go_multi_routed_tcp_tls_failset_recheck_20260601.txt` 기준. 나머지는 C full/Go multi full. |
...
| `tcp` | `PUBSUB` | `미달(77.7%)` | `통과(92.4%)` | `통과(107.2%)` | `통과(96.7%)` | `통과(97.0%)` | `통과(94.2%)` | 64/256B는 current C/Rust 제한 재측정 C `perf_c_single_linux_20260604_205634_rust_single_pubsub_small_c_recheck_20260604.txt`, Rust `perf_rust_single_linux_20260604_205723_rust_single_pubsub_small_recheck_20260604.txt` 기준. 64B는 아직 기준에 못 닿고, 256B는 통과를 유지한다. 나머지는 C full/Rust single full. |
...
| `tls` | `PUBSUB` | `미달(79.7%)` | `통과(91.1%)` | `통과(124.1%)` | `통과(97.9%)` | `통과(98.0%)` | `통과(94.6%)` | 64/256B는 current C/Rust 제한 재측정 C `perf_c_single_linux_20260604_205634_rust_single_pubsub_small_c_recheck_20260604.txt`, Rust `perf_rust_single_linux_20260604_205723_rust_single_pubsub_small_recheck_20260604.txt` 기준. 64B는 기준에 근접했지만 아직 미달이고, 256B는 통과로 회복했다. 나머지는 C full/Rust single full. |
