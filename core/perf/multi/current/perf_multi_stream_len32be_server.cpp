...
static const char *k_pattern = "MULTI_STREAM_LEN32BE";
static const int k_dispatch_flags = ZLINK_STREAM_DISPATCH_LEN32BE;
...
int main (int argc, char **argv)
{
    ...
    const std::string endpoint =
      bind_server_endpoint (server, transport, lib_name + "_multi_stream_len32be_server");
    if (endpoint.empty ()) {
        zlink_close (server);
        return 1;
    }
