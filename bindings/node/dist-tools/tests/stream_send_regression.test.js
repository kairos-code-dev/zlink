...
test('stream routed send delivers consecutive frames to the same raw client', async () => {
    ...
    try {
        ...
        await waitFor(() => stream.recv(received, zlink.RecvFlags.DontWait), 'stream request receive');
        try {
...
test('stream routed send from spot-to-spot dispatch delivers repeated frames to raw client', async () => {
    ...
    try {
        ...
        await waitFor(() => stream.recv(streamReceived, zlink.RecvFlags.DontWait), 'stream session receive');
