...
이 문서는 Config 7 Runtime Monitoring 공통 시나리오 중 Node framework E2E 상태를 정리한다. 현재
추적된 Node Monitoring E2E runner/source가 없으므로 Node 전용 stdout marker로 구현 완료를 주장하지
않는다.
...
- `MON-A1`: socket 이벤트 관찰 Node runner와 marker가 아직 없다.
...
- `MON-B1`: event kind filter Node runner와 marker가 아직 없다.
- `MON-B2`: monitoring 등록 검증 Node runner와 marker가 아직 없다.
