...
void assert_recv_parts (void *socket_,
                        const char *part0_,
                        const char *part1_,
                        const char *part2_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_recv (socket_, NULL, &parts, &part_count,
...
void assert_recv_single (void *socket_, const char *payload_)
{
    ...
    TEST_ASSERT_EQUAL_MEMORY (
      payload_, zlink_msg_data (&parts[0]), strlen (payload_));
    zlink_multipart_close (parts, part_count);
}
}
...
void test_send_part_matches_aggregate_wire_shape ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_bind (receiver, "inproc://helper-send-part-basic"));
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_connect (sender, "inproc://helper-send-part-basic"));
    msleep (SETTLE_TIME);
    ...
    TEST_ASSERT_EQUAL_INT (
      ZLINK_SUBMIT_OK,
      zlink_send_part (sender, &helper0, static_cast<zlink_send_flags_t> (0),
                       ZLINK_PART_MORE));
    TEST_ASSERT_EQUAL_INT (
      ZLINK_SUBMIT_OK,
      zlink_send_part (sender, &helper1, static_cast<zlink_send_flags_t> (0),
                       ZLINK_PART_MORE));
    TEST_ASSERT_EQUAL_INT (
      ZLINK_SUBMIT_OK,
      zlink_send_part (sender, &helper2, static_cast<zlink_send_flags_t> (0),
                       ZLINK_PART_FINAL));
...
void test_send_part_single_final_keeps_next_multipart_valid ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_bind (receiver, "inproc://helper-send-part-single-final"));
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_connect (sender, "inproc://helper-send-part-single-final"));
    msleep (SETTLE_TIME);
    ...
    TEST_ASSERT_EQUAL_INT (
      ZLINK_SUBMIT_OK,
      zlink_send_part (sender, &single, static_cast<zlink_send_flags_t> (0),
                       ZLINK_PART_FINAL));
