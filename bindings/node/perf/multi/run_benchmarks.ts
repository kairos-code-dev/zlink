...
async function main() {
  ...
  for (let patternIndex = 0; patternIndex < runnablePatterns.length; patternIndex += 1) {
    ...
    const transports = options.transports.filter(
      (transport) => POLICY_TRANSPORTS[patternName].includes(transport)
    );
    ...
    for (let transportIdx = 0; transportIdx < transports.length; transportIdx += 1) {
      ...
      for (let run = 0; run < options.runs && !transportUnsupported; run += 1) {
        ...
        for (const msgSize of msgSizes) {
          ...
          try {
            const { lines, metrics: activeMetrics } =
              await spawnMeasuredPair(runner, caseOptions);
            for (const line of lines) {
              autoHwm.addLine(line);
            }
            const hasMetrics =
              hasPrimaryMetricsFromResultLines(patternName, msgSize, lines);
            ...
            const metrics = await applySpotCleanLatency(
              patternName, runner, caseOptions, activeMetrics
            );
      ...
      for (const msgSize of msgSizes) {
        ...
        const skipped = allSkips.some(
          ([label]) => label === `${patternName} ${transport} ${msgSize}B`
        );
        if (skipped) {
          statusCounts.skip += 1;
          continue;
        ...
        const [, p95, p99] = require('../common/perf_c_emitter')
          .resolveLatencyTriplet(
            metrics.latency, metrics.latency_p95, metrics.latency_p99
          );
        successRecords.push({
          pattern: patternName,
          transport,
  ...
  const resultFile = path.join(
    reportDir,
    `perf_node_multi_${platformTag()}_${stamp}${tag}.txt`
  );
  emit('');
