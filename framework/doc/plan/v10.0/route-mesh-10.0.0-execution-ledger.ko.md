...
| S8-CPP lane (C ABI+C++ bindings→framework) | bindings 리뷰 진행 | 0 | 0 | 0 | bindings 라이브러리+15 samples 컴파일·링크 green(`2f34aacf2`). 전환 중 라이브러리 결함 2건 수정(received_t::send/reply 정의 복원·mesh_node set_routing_id). **bindings 리뷰 campaign iteration-1 착수**(snapshot 129파일 `e1adbf34`, Codex·Sonnet 3축). 게이트=`BINDINGS REVIEW CLEAN` |
