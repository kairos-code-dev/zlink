...
public static class ZlinkStreamJsonConnectorExtensions
{
    ...
    public static ZlinkStreamJsonWaitBuilder<TPayload> WaitFor<TPayload>(
        this IZlinkStreamConnector connector,
        string name)
    {
        ArgumentNullException.ThrowIfNull(connector);
        return new ZlinkStreamJsonWaitBuilder<TPayload>(connector.WaitFor(name));
    }
...
public sealed class ZlinkStreamJsonWaitBuilder<TPayload>
{
    private readonly IZlinkStreamWaitCall _inner;

    internal ZlinkStreamJsonWaitBuilder(IZlinkStreamWaitCall inner)
    {
        _inner = inner;
    }

    public ZlinkStreamJsonWaitBuilder<TPayload> Timeout(TimeSpan timeout)
    {
        _inner.Timeout(timeout);
        return this;
    }

    public ZlinkStreamJsonWaitBuilder<TPayload> Where(Func<ZlinkStreamMessage<TPayload>, bool> predicate)
    {
        ArgumentNullException.ThrowIfNull(predicate);
        _inner.Where(message => predicate(new ZlinkStreamMessage<TPayload>(
            message.Name,
            message.Metadata,
            message.Payload.FromJson<TPayload>())));
        return this;
    }
