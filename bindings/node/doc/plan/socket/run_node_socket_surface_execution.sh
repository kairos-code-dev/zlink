...
GUIDE_PATH="${SCRIPT_DIR}/node-socket-surface-execution-guide.ko.md"
...
MAX_ITERATIONS=100
POLL_SECONDS=30
GATE_LABEL="node_socket_surface_execution"
STRESS_COUNT=1
MODEL_ARG=()
...
Run the Node socket surface split execution campaign.
...
Examples:
  ./bindings/node/plan/socket/run_node_socket_surface_execution.sh
  ./bindings/node/plan/socket/run_node_socket_surface_execution.sh --max-iterations 0
  ./bindings/node/plan/socket/run_node_socket_surface_execution.sh --model gpt-5.4
EOF
}
...
echo "=== Node socket surface execution start ==="
echo "Root: ${ROOT_DIR}"
