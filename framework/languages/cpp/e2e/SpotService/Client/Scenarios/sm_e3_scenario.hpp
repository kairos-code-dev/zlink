...
inline void run_sm_e3_scenario (const std::string &play_http_endpoint)
{
    if (play_http_endpoint.empty ()) {
        throw std::runtime_error ("ZLINK_CPP_E2E_PLAY_HTTP_ENDPOINT is required for SM-E3");
    }

    constexpr auto spot_rid = "user:play-a:sm-e3-idle";
    ...
    if (!idle_reply.closed || idle_reply.spot_rid != spot_rid || idle_reply.name != timer_name) {
    ...
    if (closed_spot_reply.spot_rid != spot_rid || !closed_spot_reply.failed) {
