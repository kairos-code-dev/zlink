...
void set_zero_linger (zlink::socket_base_t *socket_)
{
    ...
    const int zero = 0;
    TEST_ASSERT_SUCCESS_ERRNO (socket_->setsockopt (ZLINK_INTERNAL_OPT_LINGER,
                                                    &zero, sizeof (zero)));
}
...
void test_ctx_close_socket_and_wait_updates_socket_registry ()
{
    ...
    zlink::socket_base_t *socket =
      ctx->create_socket (ZLINK_CORE_SOCKET_PAIR);

    TEST_ASSERT_NOT_NULL (socket);
    set_zero_linger (socket);
    TEST_ASSERT_EQUAL_UINT64 (1u,
                              static_cast<uint64_t> (ctx->socket_count ()));
    TEST_ASSERT_FAILURE_ERRNO (ETIMEDOUT,
                               ctx->wait_for_socket_count_at_most (0, 0));
...
void test_ctx_inproc_endpoint_registry_tracks_owner ()
{
    ...
    zlink::socket_base_t *socket =
      ctx->create_socket (ZLINK_CORE_SOCKET_PAIR);
    zlink::socket_base_t *other_socket =
      ctx->create_socket (ZLINK_CORE_SOCKET_PAIR);
    ...
    TEST_ASSERT_FAILURE_ERRNO (
      ENOENT,
      ctx->unregister_endpoint ("inproc://ctx-runtime-registry", other_socket));

    ctx->unregister_endpoints (socket);
    TEST_ASSERT_SUCCESS_ERRNO (
      ctx->register_endpoint ("inproc://ctx-runtime-registry", endpoint));
    TEST_ASSERT_SUCCESS_ERRNO (
      ctx->unregister_endpoint ("inproc://ctx-runtime-registry", socket));
...
void test_ctx_io_thread_selection_respects_affinity ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      ctx->set (ZLINK_IO_THREADS, &io_threads, sizeof (io_threads)));
    ...
    zlink::io_thread_t *stream_first =
      ctx->choose_io_thread_stream (uint64_t (1) << 0);
    zlink::io_thread_t *stream_second =
      ctx->choose_io_thread_stream (uint64_t (1) << 1);
...
void test_ctx_spot_worker_runtime_respects_configured_count ()
{
    zlink::ctx_t *ctx = new zlink::ctx_t;
    TEST_ASSERT_NOT_NULL (ctx);

    const int worker_threads = 2;
    TEST_ASSERT_SUCCESS_ERRNO (ctx->set (ZLINK_SPOT_WORKER_THREADS,
                                         &worker_threads,
                                         sizeof (worker_threads)));

    zlink::service_control_runtime_t *first =
      ctx->spot_worker_runtime_for_key (0);
    zlink::service_control_runtime_t *second =
      ctx->spot_worker_runtime_for_key (1);
    zlink::service_control_runtime_t *third =
      ctx->spot_worker_runtime_for_key (2);

    TEST_ASSERT_NOT_NULL (first);
    TEST_ASSERT_NOT_NULL (second);
    TEST_ASSERT_NOT_EQUAL (first, second);
    TEST_ASSERT_EQUAL_PTR (first, third);
