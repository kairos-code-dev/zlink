...
1. **기본 원칙** — 대부분의 프로젝트에 그대로 적용한다. POSD의 복잡성 관리 원칙과
   DDD의 경계·언어 원칙이 함께 들어 있다.
2. **예외를 판단하는 기준** — 기본 원칙에서 벗어나도 되는 조건.
3. **아키텍처 레이아웃 선택** — 1부에서 찾은 경계를 헥사고날·레이어드 같은 구체적인
...
**클래시티스(Classitis):** "클래스가 많을수록 좋은 설계"라는 잘못된 믿음이 클래시티스를 만든다 —
작고 얕은 클래스가 끝없이 늘어나는 병이다. 클래스 하나하나는 단순해 보이지만, 인터페이스
복잡성을 다 합치면 거대해진다. **작은 것이 미덕이 아니다. 깊은 것이 미덕이다.**
...
**정보 누출 — 가장 흔한 설계 오류.** 하나의 설계 결정이 여러 모듈에 퍼져 있으면 그 결정이
"누출"된 것이다. 증상: 하나를 바꾸면 여러 곳을 같이 고쳐야 한다.
...
**시간적 분해 — 가장 흔한 함정.** 정보의 소유권이 아니라 실행 순서를 기준으로 코드를
쪼개는 것. "읽기 → 파싱 → 쓰기"처럼 **언제 일어나는가**를 기준으로 클래스를 나누면,
각 클래스가 서로 몰라야 할 것까지 각자 다시 알아야 한다.
...
**의미가 달라지는 경계 = bounded context.** 앞서 정보 누출을 판단하는 기준은
"두 모듈이 같은 설계 지식을 공유하는가"였다 — 공유하면 합치고, 아니면 나눈다.
DDD의 bounded context는 정확히 같은 기준을 지식 대신 **의미**에 적용한다:
같은 단어(`timeout`, `status`, `owner`)가 두 코드 영역에서 같은 모델(같은 필드,
같은 규칙, 같은 상태 전이)을 가리키면 하나의 bounded context이고, 모델이 실제로
달라지면 그 지점이 경계다.
...
반대로 의미가 같은데 이름만 다르다면, 이건 그냥 정보 누출이다: 합친다.
...
**Ubiquitous language.** "같은 개념 → 같은 이름" 규칙은 한 파일 안에서만 지키면
부족하다. DDD의 ubiquitous language는 이 규칙을 도메인 전체로 넓힌다 — 코드,
문서, 테스트, 리뷰, 회의에서 같은 개념은 항상 같은 단어를 쓴다. 다만 이 규칙은
하나의 bounded context 안에서만 성립한다. 경계를 넘으면 같은 단어라도 뜻이
...
**불변 조건을 함께 지키는 경계 = aggregate.** 여러 필드나 여러 객체가 하나의 불변
조건을 함께 지켜야 한다면, 그 묶음이 DDD의 aggregate다. aggregate는 정보 은닉의
극단적인 형태다 — 불변 조건을 지키는 유일한 통로(aggregate root)만 공개하고,
내부 구성원은 그 통로를 거치지 않고는 바꿀 수 없게 막는다. 경계를 잘못 그리면
두 가지로 망가진다: 너무 크게 잡으면 서로 무관한 변경들이 하나의 락·트랜잭션에
묶여 변경 증폭이 생기고, 너무 작게 잡으면 불변 조건이 여러 aggregate에 걸쳐
있어 아무도 끝까지 지키는 쪽이 없는 back-door 누출이 된다. 판단 기준은 "이 상태들이
항상 함께 참이어야 하는가"다 — 함께 둘까 나눌까를 판단하는 기준과 같다.
...
| 헥사고날/레이어드 같은 구체적 코드 레이아웃을 쓴다 | 3부의 배치 방식은 선택지이지 의무가 아니다. 1부의 경계·불변 조건 원칙은 항상 적용되지만, 그걸 별도 디렉터리·계층으로 나눌지는 별개다. 도메인 복잡성이 낮고 수명이 짧은 코드에는 과한 계층이 얕은 래퍼만 늘릴 수 있다 |
...
language, 이벤트 스토밍 — 은 경계와 이름을 어떻게 잡을지 알려준다. 이 부는 그
경계를 실제 코드 구조로 어떻게 배치할지를 다룬다. **여기부터가 진짜 선택지다.**
...
| 17 | **application 내부 adapter 구현** (3부 레이아웃 채택 시) | application 계층이 외부 기술 구현이나 framework callback 타입을 직접 아는가? |
...
14. **경계는 도메인 의미로도 잡는다.** bounded context·ubiquitous language·aggregate는
    ...
    같은 원칙의 다른 고도다.
15. 원칙에서 벗어나려면 2부의 조건을 먼저 확인한다 — "이번은 특별해서"로 예외를 만들지 않는다.
