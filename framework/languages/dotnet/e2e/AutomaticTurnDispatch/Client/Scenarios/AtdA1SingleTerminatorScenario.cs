...
internal static class AtdA1SingleTerminatorScenario
{
    public static async Task<(string SpotRid, string RequestId)> RunAsync(IZlinkStreamConnector client)
    {
        ...
        var spot = await client.Request(new EnsureSpotReq(spotRid))
            ...
            .Async<EnsureSpotRes>();
        ScenarioAssert.That(spot.SpotRid == spotRid, "ATD-A spot creation mismatch.");

        var requestId = $"ATD-A1-{Guid.NewGuid():N}";
