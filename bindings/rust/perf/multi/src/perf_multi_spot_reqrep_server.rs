...
fn main() {
            ...
            loop {
                let received = {
                    let spot = dispatch_spot.lock().expect("spot lock");
                    {
                        ...
                        spot.recv_routed(&mut received, RecvFlags::DONT_WAIT)
                            .map(|ok| ok.then_some(received))
                    }
                };
                match received {
                    Ok(Some(received)) => {
    ...
    let Some(data_router_bind) =
        common::benchmark_endpoint(PATTERN, &args.transport, "multi-spot-reqrep-router-server")
    else {
        return;
    };
    data_node
