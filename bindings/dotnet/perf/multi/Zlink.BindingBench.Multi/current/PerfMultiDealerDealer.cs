...
{
    private static int RunMultiDealerDealerServer(string transport, int size)
    {
        ...
        string endpoint = MultiEndpointFor(transport, "multi-MULTI_DEALER_DEALER");
    ...
    private static int RunMultiDealerDealerClient(string transport, int size,
        string endpoint)
    {
        size = Math.Max(1, size);
        ...
        int drainMs = ResolveMultiDrainMs("MULTI_DEALER_DEALER");
        ...
        try
        {
            ...
            PrintResult("MULTI_DEALER_DEALER", transport, size, throughput,
                latencyUs);
            return 0;
        }
        finally
