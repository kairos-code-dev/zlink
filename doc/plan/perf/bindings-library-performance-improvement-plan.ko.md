...
| 5 | Go | `bindings/go/perf` | `tcp/ws/wss/tls single 통과 (6.0.3 재검증, SPOT large 회귀 수정, latency 발산은 측정 아티팩트로 판정)` | `tcp/ws/wss/tls 측정 완료, SPOT_SENDSEND/SPOT/PUBSUB 및 일부 routed/stream 보류 남음` | core 안정 후 `GOMAXPROCS=4` single `tcp` 재검증, `ws/wss/tls` latency 보류 제한 재측정, multi `SPOT_SENDSEND`/`SPOT`/`PUBSUB` 순서 재검토 |
...
| `tls` | `MULTI_ROUTER_ROUTER` | `보류(22.0%)` | `통과(42.8%)` | `통과(42.3%)` | `통과(46.6%)` | `통과(52.1%)` | `통과(51.0%)` | C/Go full 파일은 위 행과 같다. 절대 기준을 통과하는 256B 이상은 상대 기준을 진단 보조로 본다. 64B는 multi routed echo 최소 기준보다 낮다. routed client는 C와 같은 단일 poll loop와 public `RecvPart` 경로를 유지한다. server `RecvPart` 전체 확대와 client send scan round-robin 후보는 tcp/ws 계열에서 small 또는 large를 악화시켜 반영하지 않았다. 64B는 public 계약을 바꾸지 않는 추가 후보가 없어 보류한다. auto-HWM 활성과 size별 `MsgUnit(B)` 일치를 확인했다. |
...
| `tcp` | `MULTI_ROUTER_ROUTER` | `보류(18.4%)` | `보류(21.2%)` | `보류(20.6%)` | `통과(89.8%)` | `통과(863.6%)` | `보류(52.9%)` | C `perf_c_multi_linux_20260522_143606_codex_c_multi_tcp_no_stream_for_rust_20260522.txt`, Rust `perf_rust_multi_linux_20260522_145044_codex_rust_multi_tcp_router_router_20260522.txt` 기준이다. 64B~1024B와 262144B는 기준보다 낮다. 131072B는 C 기준 run이 낮게 나온 outlier 가능성이 커서 C/Rust 재측정 대상이다. |
...
| `tcp` | `MULTI_SPOT` | `보류(36.1%)` | `보류(37.7%)` | `보류(32.3%)` | `보류(11.0%)` | `보류(1.5%)` | `보류(1.0%)` | C `perf_c_multi_linux_20260522_143606_codex_c_multi_tcp_no_stream_for_rust_20260522.txt` 기준이다. 256B/1024B/65536B는 Rust `perf_rust_multi_linux_20260522_145753_codex_rust_multi_tcp_spot_20260522.txt`, 64B는 `perf_rust_multi_linux_20260522_205603_codex_rust_multi_tcp_spot64_waitpid_retry_20260522.txt`, 131072B/262144B는 `perf_rust_multi_linux_20260522_205452_codex_rust_multi_tcp_spot_waitpid_deadline_fix_20260522.txt` 기준으로 갱신했다. client inner drain loop에 active deadline check가 없어 backlog가 계속 들어오는 size에서 runner timeout이 났고, deadline check 추가 뒤 64B/131072B/262144B complete를 확보했다. runner의 fast-exit child 대기도 zombie를 종료로 처리하도록 보강했다. 전 size는 SPOT 기준보다 낮다. |
...
| `ws` | `MULTI_ROUTER_ROUTER` | `보류(19.6%)` | `보류(21.1%)` | `보류(20.8%)` | `보류(70.3%)` | `보류(79.2%)` | `통과(92.1%)` | C 파일은 위 행과 같고, Rust `perf_rust_multi_linux_20260522_151114_codex_rust_multi_ws_router_router_20260522.txt` 기준이다. 262144B 외에는 기준보다 낮아 보류한다. |
...
| `wss` | `MULTI_ROUTER_ROUTER` | `보류(19.5%)` | `보류(19.8%)` | `보류(20.8%)` | `통과(91.3%)` | `통과(91.0%)` | `통과(91.3%)` | C 파일은 위 행과 같고, Rust `perf_rust_multi_linux_20260522_153441_codex_rust_multi_wss_router_router_20260522.txt` 기준이다. small size는 기준보다 낮아 보류한다. |
...
| `tls` | `MULTI_ROUTER_ROUTER` | `보류(19.6%)` | `보류(21.6%)` | `보류(20.7%)` | `보류(68.4%)` | `통과(93.0%)` | `통과(101.9%)` | C 파일은 위 행과 같고, Rust `perf_rust_multi_linux_20260522_155322_codex_rust_multi_tls_router_router_20260522.txt` 기준이다. 실행 중 server `nlwp=8`, client `nlwp=7` 수준이었다. small/mid size는 기준보다 낮아 보류한다. |
...
- **(A) 바인딩 hot-path 비효율 — 최적화 대상(보류, 영구 아님)**:
  ...
  - 이 항목들은 `보류(비율%)`로 두고(영구 아님), 위처럼 client-scaling/프로파일링으로 hot-path를 특정한 기록을 남긴다.
