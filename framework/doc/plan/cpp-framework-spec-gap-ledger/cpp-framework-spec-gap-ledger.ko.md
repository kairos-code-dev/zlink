...
- 기준 시점: 2026-08-03 09:29 KST. canonical `framework/languages/cpp/build`를 기준으로 build, focused CTest, non-sample CTest, package consumer, sample serial smoke와 bounded common E2E를 다시 확인했다.
- 판정: 미완료. 현재 C++ Framework는 common Framework spec과 common E2E spec을 전체 충족한다고 판정할 수 없다.
- 변경 범위: 이 문서를 C++ Framework spec과 sample을 함께 관리하는 유일한 ledger로 유지한다. 이번 갱신에서는 runtime error mapping과 unittest를 먼저 보강하고, 그 결과와 남은 contract·E2E·package 조건을 이 문서와 진행 log에 기록한다. 기존의 별도 sample ledger는 중복 문서가 되므로 통합 후 제거한다. 기존 사용자 변경은 수정하지 않는다.
...
- 확인한 동작과 기대 동작: exact C++ interface가 정의한 13개 `framework_error_kind_t` 값과 현재 public header의 enum 이름·numeric value는 일치한다. 현재 header의 `detail::boundary_error_t`와 `failure_origin_t`는 private detail이며 public error kind의 추가 enumerator로 계산하지 않는다. runtime에는 Core submit result를 Framework kind로 바꾸는 공통 internal mapper가 추가되어 `not_admitted`를 `rejected`, backpressure를 `capacity_exceeded`, terminated를 `shutting_down`, invalid submit을 `invalid_operation`으로 변환한다. request failure mapper는 route 단절·timeout·shutdown의 kind와 `std::error_code`를 함께 보존하고, User Spot operation mapper도 cancelled/shutdown을 각각 `invalid_operation`/`shutting_down`으로 변환한다. 기대 동작은 이 mapping이 모든 operation family와 installed package에서 같은 결과를 내는 것이다.
- 판정 근거: `test_cpp_framework_messaging` executable과 `test_cpp_framework_store_location_resolvers`가 위 mapping과 boundary `code()`를 통과했고, latest non-sample CTest 49/49에도 포함됐다. 그러나 actor/spot/stream/worker/submit의 모든 operation family에서 `kind()`, `code()`, exception과 callback terminal reason이 exact table대로 전달되는지 process evidence가 없고, clean installed package에서 numeric ABI와 full terminal matrix를 확인하는 probe도 남아 있다.
...
- [ ] `CPP-TEST-001~002`: verifier와 textual gate가 exact contract와 actual runtime/E2E gate를 서로 모순 없이 검사한다. package verifier와 latest non-sample CTest 49/49는 통과했지만 current canonical parallel full CTest는 50/55이며 exact contract와 common E2E gate는 남아 있다.
