...
| S8-CPP lane (C ABI+C++ bindings→framework) | 진행 중 | 0 | 0 | 0 | Runtime 레이어 전환(message property 제거) 완료. Service 전환 설계·매핑 확정(`log/s8-cpp-lane/bindings-transition-design.ko.md`): 파일군 7단계로 컴파일 green 유지 재작성. mesh_node_t 개명, push→pull·reply 토큰·일괄 송신, route_bridge/subjects 등 폐기. 군1(공개계약)부터 진행 |
