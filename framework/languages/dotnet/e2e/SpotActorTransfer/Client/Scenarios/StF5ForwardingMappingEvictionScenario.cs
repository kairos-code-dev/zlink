...
internal static class StF5ForwardingMappingEvictionScenario
{
    public static async Task RunAsync(SpotActorTransferScenarioContext context)
    {
        ...
        SpotActorTransferScenarioContext.Require((await context.JoinAsync(context.NodeA, actorId, new JoinTargetReq("ST-F5", spotB))).Accepted,
            "ST-F5 first transfer was rejected.");
        var oldRefB = await context.GetActorRefAsync(context.NodeB, actorId);
        SpotActorTransferScenarioContext.Require((await context.JoinAsync(context.NodeB, actorId, new JoinTargetReq("ST-F5", spotC))).Accepted,
        ...
        SpotActorTransferScenarioContext.Require((await context.JoinAsync(context.NodeC, actorId, new JoinTargetReq("ST-F5", spotB))).Accepted,
        ...
        SpotActorTransferScenarioContext.Require(!staleB.Succeeded && staleB.ErrorKind == "ActorLocationStale",
