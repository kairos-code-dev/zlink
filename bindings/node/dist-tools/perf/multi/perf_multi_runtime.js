...
async function waitForConnectionReady(socket, connectFn = null, timeoutMs = integerEnvPair('PERF_MULTI_CONNECT_READY_TIMEOUT_MS', 'PERF_CONNECT_READY_TIMEOUT_MS', 1000)) {
    return waitForConnectionReadyCount(socket, 1, connectFn, timeoutMs);
}
async function waitForConnectionReadyCount(socket, expectedCount, connectFn = null, timeoutMs = integerEnvPair('PERF_MULTI_CONNECT_READY_TIMEOUT_MS', 'PERF_CONNECT_READY_TIMEOUT_MS', 1000)) {
    const monitor = socket.monitorOpen([MonitorEventType.ConnectionReady]);
    try {
...
async function publishControlUntilSent(socket, _waiter, topic, payload) {
    ...
    const deadlineMs = Math.max(1, integerEnvPair('PERF_MULTI_CONNECT_READY_TIMEOUT_MS', 'PERF_CONNECT_READY_TIMEOUT_MS', 1000));
