...
| SA-E2E-05 | 미구현 | C++ Node direct는 현재 Core admitted pipe만 조회하므로 descriptor·location이 없는 RID도 `RouteNotConnected`가 된다. Location Store resolve snapshot과 알려진 target의 route-only gate를 연결하기 전에는 `TargetNotFound`와 구분했다고 판정하지 않는다. |
...
| SA-E2E-08 | 부분 구현 | Self RID와 remote RID의 즉시 수락이 같은 public call을 사용하고 기존 application mailbox admission·drain accounting을 거쳐 handler를 한 번 실행하는지 확인한다. Pending 뒤 수락·deadline case와 transport observer가 없다. |
