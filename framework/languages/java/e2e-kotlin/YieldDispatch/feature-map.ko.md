...
- `YD-A1`: stream connector가 session gateway를 통해 `HoldCommand`와 `ProbeCommand`를 같은 Spot에
  보내고, Spot handler가 public `requestToChannel(...).await(...)`로 기다리는 동안 같은 Spot의
  다음 command가 먼저 실행되지 않는지 evidence 순서로 확인한다.
- `YD-A2`: stream connector가 session gateway를 통해 `YieldCommand`와 `ProbeCommand`를 같은 Spot에
...
- `YD-A4`: stream connector가 session gateway를 통해 `WorkerYieldCommand`와 `ProbeCommand`를 같은
...
- `YD-D3` 일부: D2 전용 topology에서 stream connector packet이 Session route mesh를 거쳐 `play-b`
  target Spot으로 전달되고, target Spot의 `YieldCommand` handler가 yield 중일 때 같은 target Spot의
  `ProbeCommand` marker가 먼저 실행되는지 검증한다.
...
- `YD-B2`는 같은 actor의 slow/fast request 순서와 대기 시간을 검증하려는 scenario file은 남아 있지만
  ...
  `ActorYieldRequest`/`ActorFastRequest`를 pending으로 둔 시도는 `logs/20260630-104321-3583163`,
  `logs/20260630-104727-3598586`, `logs/20260630-104910-3604933`에서 timeout되어 완료로 올리지
  않는다. 현재 `ProbeRequest` 기반 부분 시나리오도 full runner 안에서는 `logs/20260630-113806-3810792`,
...
- `YD-B3`는 actor handler의 public `joinSpot(...).yield(...)` 대기 중 다른 actor progress를
  ...
  `ActorJoinYieldRequest` 기반 정렬 시도는 `logs/20260630-105615-3628535`,
  `logs/20260630-110000-3641656`, `logs/20260630-110248-3653255`에서 timeout 또는 marker 순서
  불일치로 실패해 완료로 올리지 않는다. 현재 `ActorJoinRequest`/`ProbeRequest` 기반 부분 시나리오도
