...
{
public:
  envelope_header_t create_envelope (
    message_kind_t kind,
  ...
  result_t<TReply> decode_envelope_reply (
    const message_parts_t &reply,
    ...
    const std::string &operation_name) const
  {
    ...
    if (header.value ().kind == message_kind_t::error) {
      const auto error = _failure_mapper.error_header_exception (
        ...
        header.value ().error_message.value_or (error_message),
        operation_name);
      return result_t<TReply>::failure (
