...
void throw_config_result (zlink_config_result_t result_)
{
    detail::throw_if_failed<config_error_t> (
      static_cast<config_result_t> (result_));
}
    ...
    _impl (std::make_unique<impl> ()), _last_error (0)
{
    ...
    _impl->handle =
      zlink_spot_node_new (zlink::detail::native_handle (ctx_), &options);
    if (!_impl->handle)
        _last_error = errno != 0 ? errno : EFAULT;
}
...
void spot_node_t::set_router_bind (const std::string &endpoint_)
{
    ...
    throw_config_result (
      zlink_spot_node_set_router_bind (_impl->handle, endpoint_.c_str ()));
}
...
void spot_node_t::set_pub_bind (const std::string &endpoint_)
{
    validate_endpoint (endpoint_);
    throw_config_result (
      zlink_spot_node_set_pub_bind (_impl->handle, endpoint_.c_str ()));
}
...
std::string spot_node_t::last_endpoint () const
{
    ...
    detail::throw_if_failed<config_error_t> (static_cast<config_result_t> (
      zlink_spot_node_status (_impl->handle, &status)));
...
void spot_node_t::connect_peer (const std::string &endpoint_)
{
    ...
    throw_connect_result (
      zlink_spot_node_connect_peer (_impl->handle, endpoint_.c_str ()));
}
...
void spot_node_t::disconnect_peer (const std::string &endpoint_)
{
    validate_endpoint (endpoint_);
    throw_connect_result (
      zlink_spot_node_disconnect_peer (_impl->handle, endpoint_.c_str ()));
}
...
void spot_node_t::disconnect_peer_rid (const routing_id_t &target_node_rid_)
{
    ...
    detail::throw_if_failed<connect_error_t> (static_cast<connect_result_t> (
      zlink_spot_node_disconnect_peer_rid (_impl->handle, &native)));
}
...
void spot_node_t::disconnect_router_channel_peer (
...
void spot_node_t::disconnect_router_channel_peer_rid (
...
{
    detail::throw_if_failed<config_error_t> (
      static_cast<config_result_t> (zlink_spot_node_attach_discovery (
        _impl->handle, zlink::detail::native_handle (discovery_))));
}

void spot_node_t::attach_spot_route_channel_discovery (
...
void spot_node_t::attach_channel_dealer_impl (discovery_t &discovery_,
                                              zlink::socket_t &dealer_)
{
    detail::throw_if_failed<config_error_t> (
      static_cast<config_result_t> (zlink_spot_node_attach_channel_dealer (
        _impl->handle, zlink::detail::native_handle (discovery_),
        zlink::detail::native_handle (dealer_))));
}

void spot_node_t::attach_channel_dealer_manual_impl (
  const std::string &channel_name_, zlink::socket_t &dealer_)
{
    zlink::detail::validate_bounded_c_string (channel_name_, 255u,
                                              "channel_name");
    if (channel_name_.empty ())
...
{
    detail::throw_if_failed<config_error_t> (
      static_cast<config_result_t> (zlink_spot_node_attach_pub_ingress (
        _impl->handle, zlink::detail::native_handle (pub_))));
}
...
void spot_node_t::get_routing_id (routing_id_t &out_) const
{
    ...
    detail::throw_if_failed<config_error_t> (static_cast<config_result_t> (
      zlink_get_routing_id (_impl->handle, &native)));
    out_ = zlink::detail::native_routing_id (native);
}

std::optional<actor_part_t>
spot_node_t::recv_actor_part (const actor_ref_t &actor_, recv_flags_t flags_)
{
    ...
    const recv_result_t rc =
      static_cast<recv_result_t> (zlink_spot_node_actor_recv_part (
        zlink::detail::native_handle (*this),
        zlink::detail::actor_ref_native (actor_), &native_info, &native_part,
        &has_more,
...
{
    detail::throw_if_failed<config_error_t> (static_cast<config_result_t> (
      zlink_set_tls_server (_impl->handle, cert_.c_str (), key_.c_str (),
...
spot_node_status_t spot_node_t::status () const
{
    ...
    detail::throw_if_failed<config_error_t> (static_cast<config_result_t> (
      zlink_spot_node_status (_impl->handle, &native)));
...
spot_node_t::peers_query (const spot_node_peer_filter_t &filter_) const
{
    ...
    if (filter_.peer_endpoint ())
        std::snprintf (native_filter.peer_endpoint,
                       sizeof (native_filter.peer_endpoint), "%s",
...
spot_node_t::subjects (const spot_node_subject_filter_t *filter_) const
{
    ...
    if (filter_) {
        ...
        if (filter_->subject ())
            std::snprintf (native_filter.subject,
                           sizeof (native_filter.subject), "%s",
    ...
    if (count > 0) {
        detail::throw_if_failed<config_error_t> (
          static_cast<config_result_t> (zlink_spot_node_subjects (
...
spot_node_t::internal_sockets (const spot_node_socket_filter_t *filter_) const
{
    ...
    if (filter_) {
        ...
        if (filter_->socket_name ())
            std::snprintf (native_filter.socket_name,
                           sizeof (native_filter.socket_name), "%s",
...
actor_ref_t spot_node_t::actor_lookup (const std::string &actor_id_) const
{
    ...
    std::memset (&native, 0, sizeof (native));
    detail::throw_if_failed<config_error_t> (
      static_cast<config_result_t> (zlink_spot_node_actor_lookup (
        _impl->handle, actor_id_.c_str (), &native)));
...
spot_node_t::get_or_create_spot (const routing_id_t &spot_rid_)
{
    ...
    detail::throw_if_failed<config_error_t> (
      static_cast<config_result_t> (zlink_spot_node_spot_get_or_new (
        ...
        &created)));
    return std::pair<spot_t, bool> (
      zlink::detail::spot_access_t::adopt_native_handle (handle), created != 0);
}
...
std::optional<spot_t> spot_node_t::spot_lookup (const routing_id_t &spot_rid_)
{
    ...
    return std::optional<spot_t> (
      zlink::detail::spot_access_t::adopt_native_handle (handle));
}
...
int spot_node_t::get_spot_node_option_int (int option_) const
{
    ...
    detail::throw_if_failed<config_error_t> (
      static_cast<config_result_t> (zlink_get_spot_node_option (
        _impl->handle, static_cast<zlink_spot_node_option_t> (option_), &value,
        &size)));
    return value;
}
...
void spot_node_t::set_spot_node_option_int (int option_, int value_)
{
    detail::throw_if_failed<config_error_t> (
      static_cast<config_result_t> (zlink_set_spot_node_option (
        _impl->handle, static_cast<zlink_spot_node_option_t> (option_), &value_,
        sizeof (value_))));
}
