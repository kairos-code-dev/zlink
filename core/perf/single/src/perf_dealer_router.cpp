...
inline bool wait_for_receive_quiet (dealer_router_callback_state_t &state_,
...
inline bool prime_dealer_router_receiver (void *dealer,
                                          std::vector<char> *payload,
                                          dealer_router_callback_state_t *state,
                                          int recv_timeout_ms)
{
    if (!dealer || !payload || !state)
        return false;
                        ...
                        const std::string &lib_name)
{
    ...
    if (!prime_dealer_router_receiver (
          dealer.get (), &payload, &callback_state, recv_timeout_ms)) {
        stop_single_metric_worker (&metric_worker);
        print_fail_with_queue ();
        return;
    }
