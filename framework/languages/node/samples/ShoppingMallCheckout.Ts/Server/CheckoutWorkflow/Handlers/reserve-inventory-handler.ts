...
class ReserveInventoryHandler implements ZLinkRequestHandler<CheckoutStepReq, CheckoutState> {
  ...
  async handle(request: CheckoutStepReq): Promise<CheckoutState> {
    return this.checkout.advance(request.checkoutId, 'InventoryReserved');
  }
}
