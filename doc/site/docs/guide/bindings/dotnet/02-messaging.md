[← 시작하기](./01-getting-started.md) · [.NET 가이드](./index.md) · [다음: 서비스 →](./03-services.md)
...
사용합니다. 패턴별 의미는 [소켓 패턴](../../03-0-socket-patterns.md)을 참고하세요.

> **연결 대기**: 아래 예제들은 첫 메시지 전에 연결이 맺어졌는지
> `MonitorOpen(...)` + `monitor.Recv()`로 기다립니다. 자세한 내용은
> [운영 — 모니터링](./04-operations.md#모니터링-monitor) 절을 참고하세요.
...
> 반복하세요. 개념은 [PUB/SUB](../../03-2-pubsub.md) 참고.
...
레벨의 추가 모드는 코어 [STREAM §콜백](../../03-5-stream.md)이 소유합니다.
...
> [서비스 — Actor](./03-services.md#actor-액터)를 참고하세요.
...
다음: [서비스 — Registry · Discovery · SpotNode·Spot · Actor →](./03-services.md)
