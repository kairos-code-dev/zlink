...
final class PerfGateway {
    ...
    static int run(String transport, int size) {
        ...
        try {
            ...
            String providerEp = PerfUtil.endpointFor(transport, "gateway-provider");
            ...
            PerfUtil.printResult("GATEWAY", transport, size, thr, latUs);
            return 0;
        } catch (Exception e) {
            System.err.println("PerfGateway failed: transport=" + transport +
    ...
    private static void recvGatewayPayloadBlocking(Socket router,
                                                   ...
                                                   int dataCap) {
        PerfUtil.recvBlocking(router, ridBuffer, 256);
        for (int i = 0; i < 3; i++) {
            int payloadLen = PerfUtil.recvBlocking(router, payloadBuffer,
              dataCap);
            if (payloadLen > 0)
                return;
        }
    ...
    private static void recvGatewayPayloadWithTimeout(Socket router,
                                                      ...
                                                      int dataCap,
                                                      int timeoutMs) {
        PerfUtil.recvWithTimeout(router, ridBuffer, 256, timeoutMs);
        for (int i = 0; i < 3; i++) {
            int payloadLen = PerfUtil.recvWithTimeout(router, payloadBuffer,
              dataCap, timeoutMs);
            if (payloadLen > 0)
                return;
        }
