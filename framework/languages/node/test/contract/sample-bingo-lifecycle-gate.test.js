...
test('Bingo runner proves room leave and actor destroy lifecycle boundaries', () => {
  ...
  assert.match(runner, /waitLog\('play-b', 'bingo-lifecycle entry-destroy-complete actor=player-1'\)/);
  assert.match(runner, /waitLog\('play-replacement', 'bingo-lifecycle entry-destroy-complete actor=player-2'\)/);
  assert.match(runner, /assertLogCount\('play-b', 'bingo-lifecycle entry-leave actor=player-1', 1\)/);
  assert.match(runner, /assertLogCount\('play-replacement', 'bingo-lifecycle entry-leave actor=player-2', 1\)/);
  assert.match(runner, /assertLogCount\('play-replacement', 'bingo-record reported actor=observer', 0\)/);
