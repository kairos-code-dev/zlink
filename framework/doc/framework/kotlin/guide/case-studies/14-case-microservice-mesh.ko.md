...
public final class PricingService {
    ...
    public QuoteReply quote(QuoteRequest req) { return store.quote(req.userId()).toReply(); }
}
```
...
```java
public final class CorrelationInterceptor implements ClientInterceptor {
    @Override
    public <ReqT, RespT> ClientCall<ReqT, RespT> interceptCall(MethodDescriptor<ReqT, RespT> method, CallOptions callOptions, Channel next) {
        Metadata metadata = new Metadata();
        metadata.put(CORRELATION_ID, CurrentTrace.id());
...
```kotlin
@ZLinkHandlerGroup("pricing")
class QuoteHandler(private val store: PriceStore) :
    ZLinkSuspendingRequestHandler<Quote, QuoteDto> {
    override suspend fun handle(req: Quote, context: ZLinkRequestContext): QuoteDto =
        store.quote(req.userId)
}
```
...
class MeshOpsConfig {
    @Bean
    fun meshFramework(): ZLinkFrameworkConfigurer =
...
|----|-------------------|-------|
| 계약/호출 | 서비스별 생성 stub `profile.get(...)` | `client.request<ProfileDto>("profile", ...)` channel 이름 |
