...
spot_thread_metrics_t *bind_spot_thread_metrics (spot_client_state_t *state)
{
    ...
    if (!metrics->registered || metrics->owner != state) {
        ...
        if (std::find (state->thread_metrics.begin (), state->thread_metrics.end (), metrics)
                                  ...
                                  bench_latency_stats_t *probe_latency_out)
{
    ...
    unsigned long long active_received = 0;
    {
        ...
        for (size_t i = 0; i < state->thread_metrics.size (); ++i) {
            ...
            if (!metrics)
                continue;
            if (metrics->owner != state || metrics->epoch.load (std::memory_order_acquire) != epoch)
                continue;
            active_received += metrics->active_received.load (std::memory_order_acquire);
...
uint64_t resolve_spot_drain_grace_ns (uint64_t active_duration_ns, size_t msg_size)
{
...
int resolve_spot_post_phase_settle_ms (size_t msg_size)
{
    ...
    return resolve_multi_int_env ("PERF_MULTI_SPOT_POST_PHASE_SETTLE_MS", settle_ms, 0);
}
...
size_t resolve_spot_recv_worker_count (size_t slot_count)
{
    ...
    const size_t configured =
      static_cast<size_t> (resolve_multi_int_env ("PERF_MULTI_SPOT_RECV_WORKERS", 0, 0));
    if (configured > 0)
        return std::min (slot_count, configured);

    const size_t scaled = std::max<size_t> (4, std::min<size_t> (128, (slot_count + 15) / 16));
...
bool wait_for_control_activity (spot_client_state_t *state,
...
bool apply_spot_sub_options (void *sub,
                             ...
                             size_t max_msg_size)
{
    const int rcvtimeo_ms = bench_timeout_ms_from_env ("PERF_MULTI_RCVTIMEO_MS", 200);
    apply_benchmark_socket_options (sub, settings.hwm, transport, ZLINK_SOCKET_DEALER,
                                    max_msg_size);
    return zlink_set_option (sub, ZLINK_OPT_RCVTIMEO, &rcvtimeo_ms, sizeof (rcvtimeo_ms)) == 0;
}
...
bool wait_for_spot_ready_settle (spot_client_state_t *state)
{
    ...
    const auto deadline = std::chrono::steady_clock::now () + std::chrono::milliseconds (settle_ms);
...
bool publish_control_ready_count (spot_client_state_t *state, size_t msg_size, size_t ready_count)
{
    return state
           && perf_multi_spot_control::publish_ready_count (state->control_pub, k_topic, msg_size,
                                                            ready_count);
}

std::string bind_client_spot_endpoint (void *node, const std::string &transport, size_t slot_index)
{
    if (!node) {
        errno = EINVAL;
        return std::string ();
    ...
    const int base_bind_port = resolve_multi_int_env ("PERF_MULTI_CLIENT_BIND_PORT", 0, 0);
    std::string bind_endpoint;
    if (base_bind_port > 0) {
        bind_endpoint =
          make_fixed_endpoint (transport, base_bind_port + static_cast<int> (slot_index));
    } else {
...
#endif
        bind_endpoint = perf_bind_fixed_endpoint_range (node, transport, range_base_port, 64,
    ...
    if (base_bind_port > 0
        && zlink_spot_node_set_pub_bind (node, bind_endpoint.c_str ()) != ZLINK_CONFIG_OK)
        return std::string ();
...
bool parse_control_endpoint_arg (int argc, char **argv, std::string *endpoint_out)
{
    if (!endpoint_out)
        return false;
...
void destroy_spot_slots (spot_client_state_t *state, std::vector<spot_client_slot_t *> *slots)
{
    if (!slots || !state)
        return;

    if (bench_transition_debug_enabled ()) {
        std::cerr << "[multi-spot-client] destroy start ts_ns=" << perf_multi_metric::now_ns ()
    ...
    if (bench_transition_debug_enabled ()) {
        std::cerr << "[multi-spot-client] destroy done ts_ns=" << perf_multi_metric::now_ns ()
                  << std::endl;
    }
}
                               ...
                               spot_client_slot_t *slot)
{
    ...
    if (topic_len != k_topic_len || std::memcmp (topic, k_topic, k_topic_len) != 0) {
        perf_close_multipart (parts, part_count);
        return;
    }
    ...
    if (perf_multi_spot_handshake::parse_start_command (
          zlink_msg_data (&parts[0]), zlink_msg_size (&parts[0]), &started_size)) {
        const uint64_t start_recv_ns = perf_multi_metric::now_ns ();
        {
            std::lock_guard<std::mutex> lock (state->mutex);
            state->control_started_msg_size.store (started_size, std::memory_order_relaxed);
            if (start_recv_ns != 0)
                state->control_start_recv_ns.store (start_recv_ns, std::memory_order_relaxed);
            state->seen_msg_size.store (started_size, std::memory_order_relaxed);
            state->seen_phase.store (static_cast<int> (perf_multi_metric::phase_active),
                                     std::memory_order_relaxed);
        }
        if (bench_transition_debug_enabled ()) {
            std::cerr << "[multi-spot-client] start recv ts_ns=" << perf_multi_metric::now_ns ()
                      << " size=" << started_size << " slot=" << (slot ? slot->index : 0)
                      << std::endl;
        }
        perf_close_multipart (parts, part_count);
        state->cv.notify_all ();
    ...
    const size_t expected_msg_size = state->expected_msg_size.load (std::memory_order_acquire);
    ...
    {
        std::lock_guard<std::mutex> lock (state->mutex);
        previous_msg_size = state->seen_msg_size.load (std::memory_order_relaxed);
        ...
        phase_changed = !phase_regressed
                        && (previous_msg_size != header.msg_size || previous_phase != header_phase);
        if (phase_changed) {
            state->seen_msg_size.store (header.msg_size, std::memory_order_relaxed);
    ...
    if (phase_changed && bench_transition_debug_enabled ()
        ...
        std::cerr << "[multi-spot-client] transition recv ts_ns=" << perf_multi_metric::now_ns ()
                  << " size=" << header.msg_size
                  << " phase=" << static_cast<unsigned int> (header.phase)
                  << " prev_size=" << previous_msg_size << " prev_phase=" << previous_phase
                  << std::endl;
    }
    const bool collect_active = state->collect_active.load (std::memory_order_acquire);
    const bool collect_probe = state->collect_probe.load (std::memory_order_acquire);
    ...
    if (trace_phases && slot
        && header.msg_size == state->expected_msg_size.load (std::memory_order_acquire)) {
        if (header.phase == static_cast<uint8_t> (perf_multi_metric::phase_active)
            && slot->first_active_ns.load (std::memory_order_acquire) == 0) {
            uint64_t expected = 0;
            (void) slot->first_active_ns.compare_exchange_strong (expected, recv_ts_ns,
                                                                  std::memory_order_acq_rel);
        }
    }

    if (bench_debug_enabled () && previous_msg_size == 0) {
        std::cerr << "[multi-spot-client] first recv size=" << header.msg_size
                  << " phase=" << static_cast<unsigned int> (header.phase)
                  << " run=" << header.run_id << std::endl;
    }

    if (collect_active && header.magic == perf_multi_metric::k_magic
        && header.run_id == state->expected_run_id.load (std::memory_order_acquire)
        && header.phase == static_cast<uint8_t> (perf_multi_metric::phase_active)
        && header.msg_size == state->expected_msg_size.load (std::memory_order_acquire)) {
        uint64_t sender_window_end_ns =
          state->sender_window_end_ns.load (std::memory_order_acquire);
        if (sender_window_end_ns == 0) {
            const uint64_t duration_ns = state->active_duration_ns.load (std::memory_order_acquire);
            ...
            if (state->sender_window_start_ns.compare_exchange_strong (
                ...
                state->sender_window_end_ns.store (sender_window_end_ns, std::memory_order_release);
                state->cv.notify_all ();
            } else {
                sender_window_end_ns = state->sender_window_end_ns.load (std::memory_order_acquire);
            }
        }
        ...
        if (metrics) {
            ...
            if (should_sample_spot_latency (sample_index)) {
                ...
                const double latency_ns = header.sent_ts_ns > 0 && sample_ts_ns >= header.sent_ts_ns
    ...
    if (collect_probe && header.magic == perf_multi_metric::k_magic
        && header.run_id == state->expected_run_id.load (std::memory_order_acquire)
        && header.phase == static_cast<uint8_t> (perf_multi_metric::phase_cooldown)
        && header.msg_size == state->expected_msg_size.load (std::memory_order_acquire)) {
        spot_thread_metrics_t *metrics = bind_spot_thread_metrics (state);
        if (metrics) {
            const unsigned long long sample_index =
              metrics->probe_sample_index.fetch_add (1, std::memory_order_relaxed) + 1;
...
bool recv_one_spot_message (spot_client_slot_t *slot, int flags, bool *received)
{
    ...
    const int rc =
      perf_zlink_spot_subscribe_parts (slot->handle, NULL, &parts, &part_count, topic, &topic_len,
    ...
    if (rc != 0) {
        ...
        if (slot->stop.load (std::memory_order_acquire))
            return false;
        if (err == EAGAIN || err == EINTR || err == EWOULDBLOCK || err == ETIMEDOUT) {
            return true;
        }
        if (bench_debug_enabled ()) {
            std::cerr << "[multi-spot-client] recv fatal slot=" << slot->index << " err=" << err
                      << std::endl;
        }
        mark_fatal ();
        return false;
...
bool drain_spot_client_slot (spot_client_slot_t *slot, int initial_flags, bool *progressed)
{
    if (!slot)
        return false;
...
void on_spot_dispatch_event (void *spot, const zlink_spot_dispatch_info_t *info, void *userdata)
{
    (void) spot;
    spot_client_slot_t *slot = static_cast<spot_client_slot_t *> (userdata);
    if (!slot || !info || slot->stop.load (std::memory_order_acquire))
        return;
    if (info->event != ZLINK_SPOT_DISPATCH_EVENT_SUBSCRIBE_READABLE)
        return;
    ...
    for (size_t sweep = 0; progressed && sweep < 32; ++sweep) {
        progressed = false;
        if (!drain_spot_client_slot (slot, ZLINK_DONTWAIT, &progressed)) {
            mark_fatal ();
            return;
...
bool install_spot_dispatch_handlers (spot_client_state_t *state)
{
    if (!state || state->slots.empty ())
        return false;
    ...
    for (size_t i = 0; i < state->slots.size (); ++i) {
        ...
        if (!slot || !slot->handle)
            return false;
        if (zlink_spot_dispatch_event_handler (slot->handle, on_spot_dispatch_event, slot)
            != ZLINK_HANDLER_OK) {
            if (bench_debug_enabled ()) {
                std::cerr << "[multi-spot-client] dispatch handler failed slot=" << slot->index
...
bool recv_one_control_message (spot_client_state_t *state, bool *received)
{
    ...
    std::string payload;
    if (!perf_multi_spot_control::receive_control_payload (state->control_sub, k_topic, &payload,
                                                           received)) {
        return false;
    }

    size_t started_size = 0;
    if (perf_multi_spot_handshake::parse_control_connected (payload.data (), payload.size ())) {
        state->control_link_ready.store (true, std::memory_order_release);
        state->cv.notify_all ();
        return true;
    }
    if (perf_multi_spot_handshake::parse_start_command (payload.data (), payload.size (),
                                                        &started_size)) {
        const uint64_t start_recv_ns = perf_multi_metric::now_ns ();
        {
            std::lock_guard<std::mutex> lock (state->mutex);
        ...
        if (bench_transition_debug_enabled ()) {
            std::cerr << "[multi-spot-client] control start recv ts_ns="
                      << perf_multi_metric::now_ns () << " size=" << started_size << std::endl;
        }
        state->cv.notify_all ();
    }
    return true;
...
{
    return perf_multi_spot_control::wait_for_control_link_ready (state, timeout_ms,
                                                                 recv_one_control_message);
}
                          ...
                          spot_client_state_t *state)
{
    ...
    if (!state->control_node || !setup_tls_server (state->control_node, transport)
    ...
    if (!apply_benchmark_context_auto_hwm_msg_unit (ctx.get (), max_msg_size))
        return false;
    if (!apply_benchmark_spot_node_hwm (state->control_node, settings.hwm, true, true)) {
        zlink_spot_node_destroy (&state->control_node);
        return false;
    }
    ...
    if (!perf_multi_spot_control::apply_control_options (state->control_pub, state->control_sub,
                                                         settings)) {
        return false;
    }

    state->control_endpoint = bind_client_spot_endpoint (state->control_node, transport, 10000);
    state->server_control_endpoint = server_control_endpoint;
    if (state->control_endpoint.empty ()) {
        if (bench_debug_enabled ()) {
            std::cerr << "[multi-spot-client] control bind failed err=" << zlink_errno ()
                      << std::endl;
        }
        return false;
    }
    if (zlink_spot_node_connect_peer (state->control_node, server_control_endpoint.c_str ())
        != ZLINK_CONNECT_OK) {
        if (bench_debug_enabled ()) {
            std::cerr << "[multi-spot-client] control connect failed endpoint="
                      << server_control_endpoint << " err=" << zlink_errno () << std::endl;
        }
        return false;
    }
    if (zlink_set_subscription (state->control_sub, k_topic) != ZLINK_CONFIG_OK) {
        if (bench_debug_enabled ()) {
            std::cerr << "[multi-spot-client] control subscribe failed err=" << zlink_errno ()
                      << std::endl;
        }
        return false;
    }
...
bool ensure_control_connected (spot_client_state_t *state)
{
    return state
           && perf_multi_spot_control::ensure_connected (
             state->control_node, state->server_control_endpoint, &state->control_connected);
}
                        ...
                        std::vector<spot_client_slot_t *> *slots_out)
{
    ...
    if (bench_transition_debug_enabled ()) {
        std::cerr << "[multi-spot-client] create slots begin ts_ns=" << perf_multi_metric::now_ns ()
                  << " slots=" << service_clients << " mode=recv" << std::endl;
    }
    if (!state->data_node) {
        state->data_node = zlink_spot_node_new (ctx.get (), NULL);
        if (!state->data_node || !setup_tls_client (state->data_node, transport)) {
            g_last_spot_slot_failure =
              "data node create/tls failed err=" + std::to_string (zlink_errno ());
            if (state->data_node)
                zlink_spot_node_destroy (&state->data_node);
            return false;
        }
        if (!apply_benchmark_context_auto_hwm_msg_unit (ctx.get (), max_msg_size))
            return false;
        if (!apply_benchmark_spot_node_hwm (state->data_node, settings.hwm, true, true)) {
            g_last_spot_slot_failure =
              "data node hwm failed err=" + std::to_string (zlink_errno ());
            zlink_spot_node_destroy (&state->data_node);
            return false;
        }
        if (zlink_spot_node_connect_peer (state->data_node, endpoint.c_str ())
            != ZLINK_CONNECT_OK) {
            g_last_spot_slot_failure =
              "data node connect failed err=" + std::to_string (zlink_errno ());
            zlink_spot_node_destroy (&state->data_node);
            return false;
        }
    }
    ...
    for (size_t i = 0; i < service_clients; ++i) {
        ...
        if (!slot)
            return false;
        slot->index = i;
        slot->node = state->data_node;

        slot->handle = perf_create_default_spot_handle (slot->node);
        if (!slot->handle) {
            g_last_spot_slot_failure = "slot handle create failed slot=" + std::to_string (i)
                                       + " err=" + std::to_string (zlink_errno ());
            if (bench_debug_enabled ()) {
                std::cerr << "[multi-spot-client] slot handle create failed slot=" << i
                          << " err=" << zlink_errno () << std::endl;
                std::cout << "[multi-spot-client] slot handle create failed slot=" << i
                          << " err=" << zlink_errno () << std::endl;
            }
            delete slot;
            return false;
        ...
        const bool sub_options_ok =
          apply_spot_sub_options (slot->handle, settings, transport, max_msg_size);
        const zlink_config_result_t sub_rc = zlink_set_subscription (slot->handle, k_topic);
        if (!sub_options_ok || sub_rc != ZLINK_CONFIG_OK) {
            g_last_spot_slot_failure = "slot create failed slot=" + std::to_string (i)
            ...
            if (bench_debug_enabled ()) {
                std::cerr << "[multi-spot-client] slot create failed slot=" << i
                          ...
                          << " sub_rc=" << static_cast<int> (sub_rc) << " err=" << zlink_errno ()
                          << std::endl;
        ...
        if (bench_transition_debug_enabled ()
            ...
            std::cerr << "[multi-spot-client] create slots progress ts_ns="
                      << perf_multi_metric::now_ns () << " ready_slots=" << (i + 1) << "/"
                      << service_clients << std::endl;
        }
    }

    if (bench_transition_debug_enabled ()) {
        std::cerr << "[multi-spot-client] create slots done ts_ns=" << perf_multi_metric::now_ns ()
...
void print_phase_spread_summary (spot_client_state_t *state, size_t msg_size)
{
    ...
    for (size_t i = 0; i < state->slots.size (); ++i) {
        spot_client_slot_t *slot = state->slots[i];
        if (!slot || slot->phase_trace_msg_size.load (std::memory_order_acquire) != msg_size) {
            continue;
        }

        const uint64_t active_ns = slot->first_active_ns.load (std::memory_order_acquire);
    ...
    const uint64_t start_recv_ns = state->control_start_recv_ns.load (std::memory_order_acquire);
    ...
    std::cerr << "[multi-spot-client] phase spread size=" << msg_size
              << " start_recv_ns=" << start_recv_ns << " active_slots=" << active_count
              ...
              << ((start_recv_ns != 0 && active_max_ns >= start_recv_ns)
                    ...
                    : 0)
              << " active_spread_ns=" << active_spread_ns << std::endl;
}

bool publish_client_ready_count (spot_client_state_t *state, size_t msg_size);
                                               ...
                                               int timeout_ms)
{
    ...
    while (std::chrono::steady_clock::now () < deadline) {
        if (state->fatal.load (std::memory_order_acquire)
            || state->control_started_msg_size.load (std::memory_order_acquire) == msg_size) {
            return true;
        }
        ...
        if (!recv_one_control_message (state, &received))
            return false;
        if (!received)
            (void) perf_socket_poll (NULL, 0, std::min<long> (remaining_wait_ms (deadline), 25));
    }
...
bool wait_runner_start (spot_client_state_t *state, size_t msg_size, int timeout_ms)
{
    if (!state)
        return false;

    return perf_multi_handshake::wait_for_start (&state->start_gate, msg_size, timeout_ms);
}
...
bool publish_client_ready_count (spot_client_state_t *state, size_t msg_size)
{
    ...
    return publish_control_ready_count (state, msg_size, state->slots.size ());
}
...
bool wait_phase_start (spot_client_state_t *state,
                       ...
                       int timeout_ms)
{
    return perf_multi_spot_control::wait_for_phase (state, msg_size,
                                                    static_cast<int> (expected_phase), timeout_ms);
}

int resolve_spot_phase_timeout_ms (const multi_bench_settings_t &settings, size_t msg_size)
{
    int timeout_ms =
      std::max (settings.connect_ready_timeout_ms, std::max (1, settings.duration_seconds) * 5000);

    if (settings.clients >= 400) {
        timeout_ms = std::max (timeout_ms, std::max (30000, settings.connect_ready_timeout_ms * 3));
    }
    if (settings.clients >= 1000) {
        timeout_ms = std::max (timeout_ms, std::max (60000, settings.connect_ready_timeout_ms * 6));
    }
    ...
    if (msg_size >= 65536) {
        const int base_large_timeout = settings.clients >= 100 ? 60000 : 30000;
        timeout_ms = std::max (
          timeout_ms, std::max (base_large_timeout, settings.connect_ready_timeout_ms * 6));
    }
    if (msg_size >= 131072) {
        timeout_ms =
          std::max (timeout_ms, std::max (90000, settings.connect_ready_timeout_ms * 12));
    }
    if (msg_size >= 262144) {
        timeout_ms =
          std::max (timeout_ms, std::max (120000, settings.connect_ready_timeout_ms * 18));
    }

    return resolve_multi_int_env ("PERF_MULTI_SPOT_PHASE_TIMEOUT_MS", timeout_ms, 1);
}
...
bool wait_spot_sender_window_done (spot_client_state_t *state)
{
    ...
    const uint64_t active_duration_ns = state->active_duration_ns.load (std::memory_order_acquire);
    const size_t msg_size = state->expected_msg_size.load (std::memory_order_acquire);
    const uint64_t grace_ns = resolve_spot_drain_grace_ns (active_duration_ns, msg_size);
    ...
    while (!state->fatal.load (std::memory_order_acquire)) {
        ...
        if (sender_window_end_ns == 0) {
            ...
            if (control_start_recv_ns == 0) {
                state->cv.wait (lock, [state] () {
                    return state->fatal.load (std::memory_order_acquire)
                           || state->sender_window_end_ns.load (std::memory_order_acquire) != 0
                           || state->control_start_recv_ns.load (std::memory_order_acquire) != 0;
                });
                continue;
            }
            ...
            const auto wait_ns = std::chrono::nanoseconds (fallback_deadline_ns - now_ns);
        ...
        (void) state->cv.wait_for (
          lock, wait_ns, [state] () { return state->fatal.load (std::memory_order_acquire); });
    }
                           ...
                           uint32_t run_id)
{
    if (!apply_benchmark_context_auto_hwm_msg_unit (ctx, msg_size))
        return false;
    const int phase_timeout_ms = resolve_spot_phase_timeout_ms (settings, msg_size);
    ...
    if (!wait_for_control_link_ready (state, settings.connect_ready_timeout_ms)) {
        if (bench_debug_enabled ()) {
            std::cerr << "[multi-spot-client] control link ready timeout" << " size=" << msg_size
                      << std::endl;
        }
        return false;
    }
    ...
    if (!wait_for_spot_ready_settle (state)) {
        if (bench_debug_enabled ()) {
            std::cerr << "[multi-spot-client] ready settle failed size=" << msg_size
                      << " err=" << zlink_errno () << std::endl;
        }
        return false;
    }
    ...
    state->active_duration_ns.store (static_cast<uint64_t> (std::max (1, settings.duration_seconds))
                                       * 1000000000ULL,
                                     std::memory_order_release);
    state->probe_duration_ns.store (probe_duration_ns, std::memory_order_release);
    if (!ensure_control_connected (state)) {
        if (bench_debug_enabled ()) {
            std::cerr << "[multi-spot-client] ensure control connect failed" << " size=" << msg_size
                      << " err=" << zlink_errno () << std::endl;
        }
        return false;
    }
    ...
    if (!publish_client_ready_count (state, msg_size)) {
        if (bench_debug_enabled ()) {
            std::cerr << "[multi-spot-client] ready count publish failed" << " size=" << msg_size
                      << " count=" << state->slots.size () << " err=" << zlink_errno ()
                      << std::endl;
        }
        return false;
    }
    ...
    if (bench_transition_debug_enabled ()) {
        std::cerr << "[multi-spot-client] size wait start ts_ns=" << perf_multi_metric::now_ns ()
                  << " size=" << msg_size << " timeout_ms=" << phase_timeout_ms << std::endl;
    }
    ...
    if (!wait_msg_size_start_with_ready_republish (state, msg_size, phase_timeout_ms)) {
        if (bench_debug_enabled ()) {
            std::cerr << "[multi-spot-client] size start timeout size=" << msg_size << std::endl;
        }
        state->collect_active.store (false, std::memory_order_release);
        return false;
    }
    if (bench_debug_enabled ()) {
        std::cerr << "[multi-spot-client] collect start size=" << msg_size << " any-phase=1"
                  << std::endl;
    }
    if (bench_transition_debug_enabled ()) {
        std::cerr << "[multi-spot-client] collect start ts_ns=" << perf_multi_metric::now_ns ()
                  << " size=" << msg_size << std::endl;
    }

    if (!wait_phase_start (state, msg_size, perf_multi_metric::phase_active, phase_timeout_ms)) {
        if (bench_debug_enabled ()) {
            std::cerr << "[multi-spot-client] active start timeout size=" << msg_size << std::endl;
        }
        state->collect_active.store (false, std::memory_order_release);
        return false;
    }
    ...
    if (!wait_spot_sender_window_done (state)) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-client] active wait failed size=" << msg_size << std::endl;
        return false;
    }
    state->collect_active.store (false, std::memory_order_release);
    if (bench_transition_debug_enabled ()) {
        std::cerr << "[multi-spot-client] collect done ts_ns=" << perf_multi_metric::now_ns ()
                  << " size=" << msg_size << std::endl;
    }
    if (bench_transition_debug_enabled ())
    ...
    if (bench_transition_debug_enabled ()) {
        std::cerr << "[multi-spot-client] metrics merge start ts_ns="
                  << perf_multi_metric::now_ns () << " size=" << msg_size << std::endl;
    }
    collect_spot_thread_metrics (state, &active_received, &active_latency, &probe_latency);
    if (bench_transition_debug_enabled ()) {
        std::cerr << "[multi-spot-client] metrics merge done ts_ns=" << perf_multi_metric::now_ns ()
                  << " size=" << msg_size << " received=" << active_received << std::endl;
    }
    if (state->fatal.load (std::memory_order_acquire) || active_received == 0) {
        if (bench_debug_enabled ()) {
            std::cerr << "[multi-spot-client] metrics invalid fatal="
                      ...
                      << " probe_latency_mean=" << probe_latency.mean_ns << std::endl;
        }
        return false;
    }
    throughput = static_cast<double> (active_received)
                 / static_cast<double> (std::max (1, settings.duration_seconds));

    if (probe_duration_ns > 0) {
        state->metrics_epoch.fetch_add (1, std::memory_order_acq_rel);
        state->collect_probe.store (true, std::memory_order_release);
        if (!wait_phase_start (state, msg_size, perf_multi_metric::phase_cooldown,
                               phase_timeout_ms)) {
            if (bench_debug_enabled ()) {
                std::cerr << "[multi-spot-client] probe start timeout size=" << msg_size
                          << std::endl;
            }
            state->collect_probe.store (false, std::memory_order_release);
            return false;
        }
        if (!perf_multi_spot_control::wait_for_phase_duration (state, probe_seconds)) {
            state->collect_probe.store (false, std::memory_order_release);
            return false;
        }
        state->collect_probe.store (false, std::memory_order_release);
        collect_spot_thread_metrics (state, NULL, NULL, &probe_latency);
    }
    ...
    if (latency.mean_ns <= 0.0) {
        if (bench_debug_enabled ()) {
            std::cerr << "[multi-spot-client] latency missing size=" << msg_size
                      << " active_latency_mean=" << active_latency.mean_ns
                      << " probe_latency_mean=" << probe_latency.mean_ns << std::endl;
        }
        return false;
    }
    ...
    if (post_phase_settle_ms > 0
        ...
        if (bench_debug_enabled ()) {
            std::cerr << "[multi-spot-client] post phase settle failed size=" << msg_size
                      << " settle_ms=" << post_phase_settle_ms << " err=" << zlink_errno ()
                      << std::endl;
        }
        return false;
    }
                          ...
                          size_t fallback_size)
{
    ...
    if (!create_control_spot (ctx, transport, control_endpoint, settings, max_msg_size, &state)) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-client] create_control_spot failed" << std::endl;
        fast_exit_process (1);
    }
    perf_multi_spot_control::start_client_stdin_watcher (
      &state, [] (spot_client_state_t *client_state, size_t start_size) {
          {
              std::lock_guard<std::mutex> lock (client_state->mutex);
              client_state->control_started_msg_size.store (start_size, std::memory_order_relaxed);
              client_state->control_start_recv_ns.store (perf_multi_metric::now_ns (),
                                                         std::memory_order_relaxed);
              client_state->seen_msg_size.store (start_size, std::memory_order_relaxed);
              client_state->seen_phase.store (static_cast<int> (perf_multi_metric::phase_active),
                                              std::memory_order_relaxed);
          }
          client_state->cv.notify_all ();
          perf_multi_handshake::signal_start (&client_state->start_gate, start_size);
      });
    if (!create_spot_slots (ctx, transport, endpoint, settings, max_msg_size, &state,
        ...
        if (bench_debug_enabled ()) {
            std::cerr << "[multi-spot-client] create_spot_slots failed"
                      << " detail=" << g_last_spot_slot_failure << " mode=recv" << std::endl;
        }
        fast_exit_process (1);
    }

    for (size_t i = 0; i < msg_sizes.size (); ++i) {
        if (!run_single_size_case (&state, ctx.get (), settings, lib_name, transport, msg_sizes[i],
...
int main (int argc, char **argv)
{
    ...
    return run_client_benchmark (lib_name, transport, endpoint, control_endpoint, fallback_size);
}
