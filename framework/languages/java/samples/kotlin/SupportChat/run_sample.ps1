...
$RolePattern = "systems\.zlink\.samples\.kotlin\.supportchat\.(server\.(registry|api|support|session)\.Program|client\.Program|probe\.Program)"
...
try {
    ...
    $supportChannel = Split-Endpoint $endpoints[3]
    $sessionSpot = Split-Endpoint $endpoints[4]
    $sessionRouter = Split-Endpoint $endpoints[5]
    $supportSpot = Split-Endpoint $endpoints[6]
    $supportRouter = Split-Endpoint $endpoints[7]
    $sessionRoute = Split-Endpoint $endpoints[8]
    $supportRoute = Split-Endpoint $endpoints[9]
    $stream = Split-Endpoint $endpoints[10]

    $env:JAVA_TOOL_OPTIONS = "$oldJavaToolOptions -Dzlink.samples.supportchat.registryPubEndpoint=tcp://$($registryPub.Host):$($registryPub.Port) -Dzlink.samples.supportchat.registryRouterEndpoint=tcp://$($registryRouter.Host):$($registryRouter.Port) -Dzlink.samples.supportchat.apiChannelEndpoint=tcp://$($apiChannel.Host):$($apiChannel.Port) -Dzlink.samples.supportchat.supportChannelEndpoint=tcp://$($supportChannel.Host):$($supportChannel.Port) -Dzlink.samples.supportchat.sessionSpotEndpoint=tcp://$($sessionSpot.Host):$($sessionSpot.Port) -Dzlink.samples.supportchat.sessionRouterEndpoint=tcp://$($sessionRouter.Host):$($sessionRouter.Port) -Dzlink.samples.supportchat.supportSpotEndpoint=tcp://$($supportSpot.Host):$($supportSpot.Port) -Dzlink.samples.supportchat.supportSpotRouterEndpoint=tcp://$($supportRouter.Host):$($supportRouter.Port) -Dzlink.samples.supportchat.sessionRouteEndpoint=tcp://$($sessionRoute.Host):$($sessionRoute.Port) -Dzlink.samples.supportchat.supportRouteEndpoint=tcp://$($supportRoute.Host):$($supportRoute.Port) -Dzlink.samples.supportchat.streamEndpoint=tcp://$($stream.Host):$($stream.Port)"
    ...
    Start-GradleRole -Arguments @("--settings-file", "standalone.settings.gradle.kts", "--no-daemon", ":Server:Support:run", "--quiet") -LogName "support.log"
    Wait-Port $supportChannel.Host $supportChannel.Port
    Wait-Port $supportRoute.Host $supportRoute.Port
    Wait-Port $supportRouter.Host $supportRouter.Port
    Wait-Port $supportSpot.Host $supportSpot.Port
