...
async function main() {
  ...
  try {
    ...
    const result = requireNative().spotPerfSendSendLoop(
      activeSlots.map((slot) => slot.spot.nativeHandle()),
      SERVER_NODE_ROUTING_ID.toBytes(),
      SERVER_SPOT_ROUTING_ID.toBytes(),
      activeSlots.map((slot) => slot.payload),
      options.msgSize,
      options.duration,
      runId
    );
