...
bool expect_monitor_event (zlink::socket_t &monitor_,
...
bool has_no_monitor_event (zlink::socket_t &monitor_, int timeout_ms_)
{
    const auto deadline =
      std::chrono::steady_clock::now () + std::chrono::milliseconds (timeout_ms_);
    while (std::chrono::steady_clock::now () < deadline) {
...
void reconnect_default ()
{
    ...
    zlink::socket_t sub_mon = sub.monitor_open (zlink::monitor_event::all);
    const int interval = 60 * 1000;
    assert (sub.set (zlink::socket_option::reconnect_ivl, interval) == 0);
...
void reconnect_success ()
{
    ...
    zlink::socket_t pub2 (ctx, zlink::socket_type::pub);
    ...
    assert (sub.set (zlink::socket_option::linger, zero) == 0);
    assert (pub2.set (zlink::socket_option::linger, zero) == 0);
    assert (sub.close () == 0);
    assert (pub2.close () == 0);
    assert (sub_mon.close () == 0);
}
