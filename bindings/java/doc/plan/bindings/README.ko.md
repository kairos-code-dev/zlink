# Java Bindings Core Alignment 문서

이 디렉토리는 `bindings/java`를 최신 `core` public surface에 맞춰 정렬하는
실행 문서를 담는다.
...
- [`java-bindings-alignment-execution-guide.ko.md`](./java-bindings-alignment-execution-guide.ko.md)
  메인 플랜을 실제 실행 순서와 완료 판정 기준으로 고정한 실행 문서다.
- [`run_java_bindings_alignment_execution.sh`](./run_java_bindings_alignment_execution.sh)
