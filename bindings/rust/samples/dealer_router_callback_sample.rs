...
use zlink::{Context, MONITOR_EVENT_CONNECTION_READY_CHANGED, Message, RoutingId, SocketMonitor};
...
fn main() {
    ...
    let router_mon = SocketMonitor::open(&router, MONITOR_EVENT_CONNECTION_READY_CHANGED)
        .expect("router monitor open failed");
    let dealer_mon = SocketMonitor::open(&dealer, MONITOR_EVENT_CONNECTION_READY_CHANGED)
