# C++ Framework E2E 재포팅 작업자 프롬프트

## 역할

너는 `/home/hep7/project/kairos/zlink` 저장소에서 C++ framework E2E를 `.NET` 기준으로 재포팅하는 작업자다.
기준 문서는 `framework/doc/plan/framework-cpp-e2e-dotnet-porting-repair-list.ko.md`다.

## 목표

C++ E2E가 `.NET` E2E의 실행 책임, role 분리, 파일 분류, Client driver 방식을 따르도록 수정한다.
Scenario marker만 통과시키는 것이 목표가 아니다. Client가 `app_t`, `hosted_service_t`, framework channel
client, route mesh server로 직접 구동되는 구조는 HTTP client 또는 stream connector driver 구조로 고친다.
...
한 번에 하나의 config를 끝까지 닫는다. `DeliveryDispatch`는 framework E2E 기준에 남길지 sample-derived
E2E로 분리할지 먼저 명확히 문서화한다.

## 반드시 지킬 규칙

- `.NET` source-only inventory를 먼저 만들고 C++ source와 대조한다.
...
3. C++ `framework/languages/cpp/e2e/<Config>` tree와 role, Client 책임, scenario file 분류를 비교한다.
4. 누락된 config, role, extra role, 잘못된 Client 구조를 수정한다.
...
- 마지막 Codex 반복 리뷰 결과가 `NO MISSING CPP ITEMS`다.
