...
| YD-A2 | 구현 | stream connector request가 session gateway를 거쳐 play 노드에 도달한다. `YieldReq`가 `RequestToChannel(...).Yield<TRes>()`로 기다리는 동안 같은 Spot의 `ProbeReq`가 먼저 실행되는 marker를 검증한다. |
...
| YD-B1 | 구현 | stream connector request가 session gateway에서 actor relay로 이어진다. actor A가 `RequestToChannel(...).Yield<TRes>()`로 기다리는 동안 actor B의 fast request가 먼저 완료되는 marker를 검증한다. |
...
| YD-C1 | 구현 | stream connector request가 timer scenario를 시작한다. timer A가 `RequestToChannel(...).Yield<TRes>()`로 기다리는 동안 timer B tick이 먼저 완료되는 marker를 검증한다. |
...
| YD-D2 | 구현 | `play-a` Spot handler가 `RequestToSpot(...).Yield<TRes>()`로 `play-b` target Spot을 기다린다. `play-a`에는 owner continuation marker가 남고, `play-b`에는 target Spot handler marker만 남는 것을 검증한다. |
| YD-D3 | 구현 | stream connector request가 session gateway를 거쳐 `play-b` route mesh control로 relay되고, `play-b`가 target Spot route request handler에서 `RequestToChannel(...).Yield<TRes>()`를 수행한다. yield 중 같은 target Spot의 probe가 먼저 실행되는 marker도 검증한다. |
...
| YD-E1 | 구현 | stream connector request가 session gateway를 거쳐 play 노드에 도달한다. `RequestToChannel(...).Timeout(...).Yield<TRes>()` timeout 뒤 같은 Spot mailbox가 `ProbeReq`를 처리하는 marker를 검증한다. |
| YD-E2 | 구현 | server-side cancellation token이 `Yield<TRes>(token)` 대기를 취소하고, cancellation catch와 completion marker가 원래 Spot mailbox에서 이어진 뒤 같은 Spot mailbox가 post-cancel `ProbeReq`를 처리하는 marker를 검증한다. 이 scenario가 `ZLinkSerialTurn`의 yield cancellation 회귀를 잡는다. |
| YD-E3 | core 패치 대기 | `shutdown-wait` client scenario가 stream connector request로 long yield를 시작하고, runner가 play-a evidence의 `yield-released` marker를 확인한 뒤 play-a를 SIGTERM으로 정상 종료한다. client는 timeout이 아니라 closed/cancelled 계열 public error를 받아야 통과한다. 현재 native artifact에서는 stream packet handler에서 받은 rid로 `DisconnectRid(rid)`를 호출해도 raw TCP client close가 전파되지 않는 core blocker가 .NET binding 회귀 테스트에서도 재현된다. `shutdown-wait` client는 `RequestTimeout`을 성공으로 보지 않도록 좁혔고, 새 core 패치 버전과 .NET native artifact 갱신 뒤 `shutdown-wait`/`shutdown-recovery`를 다시 실행해야 한다. |
