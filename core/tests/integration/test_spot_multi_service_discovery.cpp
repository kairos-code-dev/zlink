    ...
    return zlink_test_wait_until (timeout_ms_, [=] {
        ...
        if (zlink_registry_member_peers (registry_, channel_name_, entries, &count)
            ...
            for (size_t j = 0; j < count; ++j) {
                if (entries[j].service_role == zlink::discovery_protocol::service_role_spot
...
void test_spot_service_discovery_replays_existing_filters_end_to_end ()
{
    ...
    snprintf (registry_pub, sizeof (registry_pub), "tcp://127.0.0.1:%d", test_port (6010));
    snprintf (registry_router, sizeof (registry_router), "tcp://127.0.0.1:%d", test_port (6011));
    snprintf (xpub_endpoint, sizeof (xpub_endpoint), "tcp://127.0.0.1:%d", test_port (6012));
    snprintf (sub_endpoint, sizeof (sub_endpoint), "tcp://127.0.0.1:%d", test_port (6013));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zlink::discovery_owned_service::register_endpoint (
      static_cast<zlink::discovery_t *> (provider_discovery), xpub_endpoint, &resolved_pub, NULL,
      service_role_pub));
    TEST_ASSERT_SUCCESS_ERRNO (zlink::discovery_owned_service::register_endpoint (
      static_cast<zlink::discovery_t *> (provider_discovery), sub_endpoint, &resolved_sub, NULL,
      service_role_sub));

    TEST_ASSERT_TRUE (
      wait_for_service_summary_count_local (consumer_discovery, service_role_pub, 1, 5000));
    TEST_ASSERT_TRUE (
      wait_for_service_summary_count_local (consumer_discovery, service_role_sub, 1, 5000));
    TEST_ASSERT_TRUE (
      wait_for_service_attachment_shape_local (node, "spot-auto-sub", 0, 1, 1, 5000));

    int subscribed = 0;
    TEST_ASSERT_TRUE (wait_for_subscription_event_local (xpub, "bench", &subscribed, 5000));
    TEST_ASSERT_EQUAL_INT (1, subscribed);

    zlink_msg_t part;
    TEST_ASSERT_SUCCESS_ERRNO (zlink_msg_init_size (&part, 4));
    memcpy (zlink_msg_data (&part), "pong", 4);
    TEST_ASSERT_EQUAL_INT (ZLINK_SUBMIT_OK, zlink_publish (xpub, "bench", &part, 1, 0));

    TEST_ASSERT_TRUE (
      wait_for_spot_service_message_local (spot, "spot-auto-sub", "bench", "pong", 5000));
    close_zero_linger (sub);
    close_zero_linger (xpub);
    destroy_multi_service_fixture_local (&spot, &node, &consumer_discovery, &provider_discovery,
                                         &registry, ctx);
}
...
void test_spot_node_provider_registers_default_weight ()
{
    ...
    char registry_pub[MAX_SOCKET_STRING];
    char registry_router[MAX_SOCKET_STRING];
    snprintf (registry_pub, sizeof (registry_pub), "tcp://127.0.0.1:%d", test_port (6070));
    snprintf (registry_router, sizeof (registry_router), "tcp://127.0.0.1:%d", test_port (6071));
    ...
    TEST_ASSERT_TRUE (
      connect_discovery_registry_with_retry_local (discovery, registry_router, 3000));

    void *node = zlink_spot_node_new (ctx, NULL);
    TEST_ASSERT_NOT_NULL (node);
    TEST_ASSERT_SUCCESS_ERRNO (zlink_spot_node_set_pub_bind (node, "tcp://127.0.0.1:*"));
    TEST_ASSERT_EQUAL_INT (ZLINK_CONFIG_OK, zlink_spot_node_attach_discovery (node, discovery));

    TEST_ASSERT_TRUE (
      wait_for_spot_node_provider_weight_local (registry, "spot-weight-default", 100, 5000));

    void *spot = NULL;
    void *provider_discovery = NULL;
    destroy_multi_service_fixture_local (&spot, &node, &discovery, &provider_discovery, &registry,
                                         ctx);
}
...
void test_spot_service_discovery_replays_filters_after_pubsub_churn ()
{
    ...
    char xpub_endpoint[MAX_SOCKET_STRING];
    char sub_endpoint[MAX_SOCKET_STRING];
    snprintf (registry_pub, sizeof (registry_pub), "tcp://127.0.0.1:%d", test_port (6014));
    snprintf (registry_router, sizeof (registry_router), "tcp://127.0.0.1:%d", test_port (6015));
    snprintf (xpub_endpoint, sizeof (xpub_endpoint), "tcp://127.0.0.1:%d", test_port (6016));
    snprintf (sub_endpoint, sizeof (sub_endpoint), "tcp://127.0.0.1:%d", test_port (6017));
    ...
    TEST_ASSERT_TRUE (
      wait_for_service_attachment_shape_local (node, "spot-auto-churn", 0, 1, 1, 5000));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zlink::discovery_owned_service::unregister_endpoint (
      static_cast<zlink::discovery_t *> (provider_discovery), resolved_pub.c_str (),
      service_role_pub));
    TEST_ASSERT_SUCCESS_ERRNO (zlink::discovery_owned_service::unregister_endpoint (
      static_cast<zlink::discovery_t *> (provider_discovery), resolved_sub.c_str (),
      service_role_sub));
    TEST_ASSERT_TRUE (
      wait_for_service_attachment_shape_local (node, "spot-auto-churn", 0, 0, 0, 5000));
...
void test_spot_service_discovery_multi_router_distributes_across_candidates ()
{
    ...
    snprintf (registry_pub, sizeof (registry_pub), "tcp://127.0.0.1:%d", test_port (6020));
    snprintf (registry_router, sizeof (registry_router), "tcp://127.0.0.1:%d", test_port (6021));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zlink::discovery_owned_service::register_endpoint (
      static_cast<zlink::discovery_t *> (provider_discovery), router_a_endpoint, &resolved_a, NULL,
      service_role_router));
    TEST_ASSERT_SUCCESS_ERRNO (zlink::discovery_owned_service::register_endpoint (
      static_cast<zlink::discovery_t *> (provider_discovery), router_b_endpoint, &resolved_b, NULL,
      service_role_router));

    TEST_ASSERT_TRUE (
      wait_for_service_summary_count_local (consumer_discovery, service_role_router, 2, 5000));
    TEST_ASSERT_TRUE (
      wait_for_service_attachment_shape_local (node, "spot-auto-router", 2, 0, 0, 5000));

    TEST_ASSERT_TRUE (
      wait_for_router_distribution_local (spot, "spot-auto-router", router_a, router_b, 5000));
...
void test_spot_service_discovery_many_router_candidates_attach_and_distribute ()
{
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        TEST_ASSERT_SUCCESS_ERRNO (zlink_set_routing_id (router, routing_id, strlen (routing_id)));
    ...
    TEST_ASSERT_TRUE (wait_for_service_summary_count_local (consumer_discovery, service_role_router,
                                                            routers.size (), 5000));
    TEST_ASSERT_TRUE (wait_for_service_attachment_shape_local (
      node, "spot-auto-router-n", static_cast<uint32_t> (routers.size ()), 0, 0, 5000));
    TEST_ASSERT_TRUE (
      wait_for_router_set_distribution_local (spot, "spot-auto-router-n", routers, 5000));
...
int main (void)
{
    ...
    RUN_TEST (test_spot_service_discovery_multi_router_distributes_across_candidates);
    RUN_TEST (test_spot_service_discovery_many_router_candidates_attach_and_distribute);
    return UNITY_END ();
}
