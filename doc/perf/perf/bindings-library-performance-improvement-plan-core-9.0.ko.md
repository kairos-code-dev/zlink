...
- Multi 상태: `MULTI_DEALER_DEALER, MULTI_DEALER_ROUTER, MULTI_DEALER_ROUTER_REQREP, MULTI_ROUTER_ROUTER, MULTI_ROUTER_ROUTER_REQREP, MULTI_PUBSUB, MULTI_SPOT 완료, 나머지 pattern 미측정`
- 다음 작업: Multi `MULTI_SPOT_REQREP`의 tcp transport를 C와 C++ 순서로 CPU pin 없이 paired 측정한다.
...
| 현재 pattern | Multi `MULTI_SPOT` 완료 | tcp, ws, wss, tls의 모든 size가 throughput과 평균 latency 목표를 통과했다. |
...
| 커밋과 푸시 | 완료 | `MULTI_SPOT` 완료 근거만 별도 커밋하고 원격에 푸시한 뒤 다음 pattern으로 이동한다. |

### 10.3 언어 진행 상태

| 순서 | 언어 | Single 상태 | Multi 상태 | 다음 작업 |
|------|------|-------------|------------|-----------|
| 1 | C++ | 전체 pattern 완료 | `MULTI_DEALER_DEALER`, `MULTI_DEALER_ROUTER`, `MULTI_DEALER_ROUTER_REQREP`, `MULTI_ROUTER_ROUTER`, `MULTI_ROUTER_ROUTER_REQREP`, `MULTI_PUBSUB`, `MULTI_SPOT` 완료 | `MULTI_SPOT_REQREP` tcp를 C와 C++ 순서로 CPU pin 없이 paired 측정한다. |
