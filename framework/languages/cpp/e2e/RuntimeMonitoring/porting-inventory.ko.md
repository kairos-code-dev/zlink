...
| `Client/Program.cs` | `Client/main.cpp` | client | done | `.NET`처럼 plain HTTP driver로 common MON-A/B/C slice와 MON-D1 재시작 검증 slice를 dispatch한다. client는 framework runtime을 호스트하지 않고 trigger/service/registry HTTP endpoint만 호출한다. runner가 두 client invocation을 모두 실행해 `.NET` Program의 전체 scenario 실행 의미를 맞춘다. |
| `Client/Support/ClientOptions.cs` | `Client/Support/client_options.hpp` | support | done | C++ runner의 env 기반 실행 규약에 맞춰 scenario, registry URL, service URL, trigger URL 같은 HTTP endpoint option을 한 곳에서 읽는다. |
...
| `Client/Scenarios/MonD1FailureRecoveryScenario.cs` | `Client/Scenarios/mon_d1_failure_recovery_scenario.hpp` | scenario | done | service stop/restart 뒤 trigger HTTP request, restarted service evidence, restart 이후 registry topology continuity evidence를 검증한다. |
...
| `Server/Trigger/TriggerEndpoints.cs` | `Server/Trigger/trigger_host_factory.hpp`, `Server/Shared/evidence_store.hpp` | endpoint | done | health, evidence, evidence/wait, profile request, service-a/service-b routed request, throw stderr log wait, validation endpoint, handshake failure endpoint route mapping은 trigger host factory에서 담당한다. |
| `Server/Trigger/TriggerHandlers.cs` | `Server/Trigger/trigger_handlers.hpp` | handler | done | profile request, service-a/service-b/throw routed request, throw stderr log, validation, handshake failure handler class를 role-local handler header로 분리했다. `.NET`의 socket event recorder는 현재 C++ trigger role이 public monitoring source를 등록하지 않아 대응하지 않는다. |
...
| `Server/Trigger/Support/TriggerSupport.cs` | `Server/Trigger/Support/trigger_options.hpp`, `Server/Shared/evidence_store.hpp` | support | done | C++ runner env 기반 trigger endpoint/log option parser와 shared evidence store가 있다. log reader, trigger channel request helper, validation helper는 별도 support header로 분리했다. |
...
- 2026-07-02: `cmake --build framework/languages/cpp/build --target zlink_cpp_e2e_runtime_monitoring_registry zlink_cpp_e2e_runtime_monitoring_service zlink_cpp_e2e_runtime_monitoring_filtered_service zlink_cpp_e2e_runtime_monitoring_throwing_service zlink_cpp_e2e_runtime_monitoring_trigger zlink_cpp_e2e_runtime_monitoring_client -j 4`
  - 결과: 통과
  - 의미: client를 `.NET` baseline과 같은 HTTP-only driver로 바꾼 뒤에도 RuntimeMonitoring 전체 role target이 빌드된다.
- 2026-07-02: `timeout 420s framework/languages/cpp/e2e/RuntimeMonitoring/run_e2e.sh`
  - 결과: 통과
  - 로그: `logs/20260702-073103-59798`
  - 의미: client role이 자체 framework runtime 없이 trigger/service/registry HTTP endpoint만 호출하는 상태에서
    registry, service, filtered service, throwing service, trigger, client role target이 같은 gate에서
    MON-A1, MON-A2, MON-A3, MON-A4, MON-A5, MON-B1, MON-B2, MON-C1, MON-D1을 검증한다.
