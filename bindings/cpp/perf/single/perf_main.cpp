...
{
    // Keep the registry alive until process termination because cleanup runs
    // from atexit and may execute after static destruction order kicks in.
...
{
    int env = env_int("PERF_MSG_COUNT", 0);
    if (env > 0)
        return env;
...
static bool is_echo_pattern(const char *pattern)
{
    if (!pattern)
        return false;
    return std::strcmp(pattern, "MULTI_DEALER_ROUTER") == 0
...
static double bandwidth_mbps(const char *pattern, double throughput, size_t size)
{
    const double multiplier = is_echo_pattern(pattern) ? 2.0 : 1.0;
    return (throughput * static_cast<double>(size) * multiplier) / 1000000.0;
}
...
void print_result(const char *pattern,
                  ...
                  double latency)
{
    const double bandwidth = bandwidth_mbps(pattern, throughput, size);
...
int main(int argc, char **argv)
{
    ...
    if (pattern == "STREAM_CALLBACK")
        return run_pattern_stream_callback(transport, size);
