...
class spot_reqrep_client_bench_t
{
  public:
    ...
    ~spot_reqrep_client_bench_t ()
    {
        for (size_t i = 0; i < _slots.size (); ++i) {
            if (!_slots[i])
                continue;
            _slots[i]->owner = NULL;
            _slots[i]->waiting_reply.store (false, std::memory_order_release);
        }
        for (size_t i = 0; i < _slots.size (); ++i) {
    ...
    bool run ()
    {
        ...
        const bool active_ok = run_active_phase (&active_count, &latency);
    ...
    bool prepare_data_plane () { return setup (); }

    const std::string &local_data_endpoint () const { return _local_data_endpoint; }

    zlink::service::spot_node_t &data_node () { return *_data_node; }
  ...
  private:
    bool setup ()
    {
        ...
        if (!perf::multi::apply_spot_auto_hwm_msg_unit (_ctx.ctx (), _msg_size))
            return false;
        _local_data_endpoint = perf::multi::bind_routed_spot_endpoint (*_data_node, _transport, 0);
        ...
        if (!perf::multi::recalculate_auto_hwm (_ctx))
            return false;
        perf::multi::emit_spot_node_auto_hwm_snapshot (*_data_node, _transport, _msg_size);
        ...
        try {
            for (size_t i = 0; i < _slots.size (); ++i) {
                _poller.add (*_slots[i]->spot, zlink::poll_event_flag_t::pollcompletion, i);
            }
        }
    ...
    bool submit_request (client_slot_t &slot_)
    {
        ...
        try {
            const zlink::routing_id_t server_node_rid = make_text_rid (k_server_node_rid_text);
            const zlink::routing_id_t server_spot_rid = make_text_rid (k_server_spot_rid_text);
            client_slot_t *slot_ptr = &slot_;
            const bool ok =
              slot_.spot->request_to_spot (server_node_rid, server_spot_rid)
                .message (request)
    ...
    static void on_reply (client_slot_t *slot_, zlink::request_result_t result_, std::vector<zlink::message_t> parts_)
    {
        ...
        const uint64_t deadline_ns = bench->_active_deadline_ns.load (std::memory_order_acquire);
        if (deadline_ns != 0 && now_ns >= 0 && static_cast<uint64_t> (now_ns) >= deadline_ns)
            return;
        if (header.sent_ts_ns <= 0 || now_ns < header.sent_ts_ns)
            return;

        const double sample_ns = perf_metric::elapsed_latency_ns (now_ns, header.sent_ts_ns) * 0.5;
        bench->_active_reply_count.fetch_add (1, std::memory_order_acq_rel);
        {
    ...
    bool run_active_phase (unsigned long long *count_out_, perf::multi::bench_latency_stats_t *latency_out_)
    {
        ...
        while (std::chrono::steady_clock::now () < deadline) {
            ...
            const size_t active_slots = active_spot_slot_limit (_slots.size (), _msg_size);
            for (size_t i = 0; i < active_slots; ++i) {
                if (_slots[i]->waiting_reply.load (std::memory_order_acquire)) {
                    continue;
                }
        ...
        *count_out_ = _active_reply_count.load (std::memory_order_acquire);
        {
            std::lock_guard<std::mutex> lock (_latency_mutex);
                              ...
                              const std::string &control_endpoint)
{
    ...
    spot_reqrep_client_bench_t *bench =
      new spot_reqrep_client_bench_t (transport, lib_name, msg_size, endpoint, settings);
...
int main (int argc, char **argv)
{
    ...
    const std::string control_endpoint = parse_control_endpoint_arg (argc, argv);
    if (control_endpoint.empty ()) {
        std::cerr << "missing --control-endpoint" << std::endl;
        return 1;
    }

    const bool ok = perf_spot_reqrep_client (lib_name, transport, size, endpoint, control_endpoint);
    std::cout.flush ();
    std::cerr.flush ();
    std::_Exit (ok ? 0 : 1);
}
