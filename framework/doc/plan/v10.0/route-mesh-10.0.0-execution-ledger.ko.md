...
| S8-CPP lane (C ABI+C++ bindings→framework) | bindings 리뷰 iter-3 | 0 | 0 | 0 | iter-2 결함(C2-0..C2-4) 수정·검증(라이브러리+15samples green, no-hit ZERO). **iteration-3 착수**(121파일 `dbe1085f`). 게이트=`BINDINGS REVIEW CLEAN` |
