...
unique_tcp (const char *base)
{
  ...
  const unsigned port =
    20000u + ((pid * 131u + now + salt + (++counter * 17u)) % 30000u);
  std::ostringstream stream;
  stream << "tcp://127.0.0.1:" << port;
  return stream.str ();
}
...
main ()
{
  ...
  const auto embedded_registry_router =
    unique_tcp ("embedded-registry-router");
  ...
  if (spot_topology.size () != 1 ||
      spot_topology[0].name != "play-actors") {
    return 24;
  }
  ...
  const auto missing_endpoint_result =
    disconnected_client.connect (missing_endpoint);
  ...
  options.spot_mesh ("game.spots")
    .node ("game-node")
    .enable_router (
      framework_router_endpoint,
      [](zlink::framework::spot_router_capability_builder_t &router) {
        router.routing_id (zlink::routing_id_t::from ("7300"))
          .connect ("tcp://router-peer:7302");
      })
    .enable_pub_sub (
      framework_pub_endpoint,
      [](zlink::framework::spot_pub_sub_capability_builder_t &pub_sub) {
        pub_sub.routing_id (zlink::routing_id_t::from ("7301"))
  ...
  if (framework_spots.size () != 1 ||
      ...
      *framework_spots[0].pub_bind_endpoint != framework_pub_endpoint ||
      ...
      framework_spots[0].router_manual_connections[0] !=
        "tcp://router-peer:7302" ||
      ...
      framework_spots[0].pub_sub_manual_connections[0] !=
        "tcp://pub-peer:7303" ||
      ...
      *framework_spots[0].registry_spot_route_channel != "game.route" ||
      ...
      framework_spots[0].accepted_route_channels[0].channel_name !=
        "game.route" ||
      !framework_spots[0].accepted_route_channels[0]
  ...
  const auto manual_route_router_endpoint =
    unique_tcp ("manual-accepted-router");
  manual_route_options.client_server_channel ("manual.api")
    .server (manual_route_endpoint);
  ...
  if (manual_route_spots.size () != 1 ||
      ...
      manual_route_spots[0].accepted_route_channels[0].channel_name !=
        "manual.api" ||
      manual_route_spots[0].accepted_route_channels[0]
          .manual_connections.size () != 1 ||
      manual_route_spots[0].accepted_route_channels[0]
          .manual_connections[0] != manual_route_endpoint) {
    return 22;
  }
  ...
  auto remote_connected = remote_client.connect (
    zlink::framework::registry_query_client_options_t { registry_router });
  if (!remote_connected) {
    return 28;
  }
  ...
  while (std::chrono::steady_clock::now () < deadline && !remote_found) {
    ...
    if (remote_topology) {
      for (const auto &entry : remote_topology.value ()) {
        if (entry.name == "remote.play" &&
            entry.endpoint == provider_endpoint &&
