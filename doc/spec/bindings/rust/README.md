...
let timer = SpotTimer::new(&spot);
timer.start(Duration::from_secs(1), 0)?;  // 1초, 무한 반복

// dispatch event handler -- 단일 context, 동기화 불필요
spot.on_dispatch_event(|s, event| {
    match event {
        SpotDispatchEvent::SubscribeReadable => {
            let sub = s.subscribe_recv().unwrap();
            // pub/sub 메시지 처리
        }
        SpotDispatchEvent::RoutedReadable => {
            let msg = s.spot_recv().unwrap();
            if msg.request_seq != 0 {
                s.reply_to_spot(&msg.source_rid, &msg.spot_rid,
                                msg.request_seq, Message::from(b"ok")).unwrap();
            }
        }
        SpotDispatchEvent::TimerReadable => {
            let fire_count = timer.recv().unwrap();
            s.publish("heartbeat", Message::from(b"ping")).unwrap();
        }
    }
});
