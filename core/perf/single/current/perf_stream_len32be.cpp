...
void run_stream_tcp_raw(size_t msg_size, int msg_count, const std::string &lib_name)
{
    ...
    if (!server_ctx.valid()) {
        print_result(lib_name, "STREAM_LEN32BE", "tcp", msg_size, 0.0, 0.0);
        return;
    }
    ...
    if (effective <= 0) {
        print_result(lib_name, "STREAM_LEN32BE", "tcp", msg_size, 0.0, latency);
        cleanup();
        return;
    }
    ...
    print_result(lib_name, "STREAM_LEN32BE", "tcp", msg_size, throughput, latency);
    cleanup();
}
                             ...
                             const std::string &lib_name)
{
    ...
    if (!server_ctx.valid() || !client_ctx.valid()) {
        print_result(lib_name, "STREAM_LEN32BE", transport, msg_size, 0.0, 0.0);
        return;
    }
    apply_stream_server_ctx_threads(server_ctx.get());
    ...
    const int effective = sent < recv_count ? sent : recv_count;
    if (effective <= 0) {
        print_result(lib_name, "STREAM_LEN32BE", transport, msg_size, 0.0, latency);
        stop_dispatch();
        return;
    }
    ...
    print_result(lib_name, "STREAM_LEN32BE", transport, msg_size, throughput, latency);
    if (ws_family)
        settle();
    stop_dispatch();
}
