...
[문서 목록](../../../../node/README.ko.md) | [이전: ZLink Framework NestJS Actor](nestjs-actor.ko.md) | [다음: ZLink Framework NestJS Monitoring](nestjs-monitoring.ko.md)
<!-- framework-adapter-nav:end -->

[스펙 목차](../../../../node/README.ko.md)

[Node 묶음](../../../../node/README.ko.md) | [인터페이스](handler-interfaces.ko.md) | [Actor](nestjs-actor.ko.md) | [STREAM](nestjs-stream.ko.md) | [policy/Session Actor Dispatch 사용성](../../session-actor-dispatch.ko.md)
...
> `RoutingId`=string, NestJS)는 TypeScript 모양이다. 구현과 contract test는 이
> 문서의 시그니처를 따라야 한다.
...
export class TicTacToeSession implements ZLinkSession {
  ...
  async onDispatch(
    ...
    if (dispatch.packetName === 'auth') {
      ...
      const actor = await this.context.actors.bind(request.actorId);

      this.authenticatedActors.remember(request.actorId, actor);

      this.context.client.reply(new AuthRep(true)).submit();
      return;
    }
