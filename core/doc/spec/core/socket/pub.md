...
to the wire-prefix convention. When it is non-NULL, Core prepends a topic
frame. A multipart message started with `ZLINK_PART_MORE` must continue with
this function on the same thread through `ZLINK_PART_FINAL`. Do not call a
