[TicTacToe Sample](./README.ko.md) | [일반 버전](./direct.ko.md)

# TicTacToe SessionGateway Sample

> 이 문서는 SessionGateway TicTacToe 샘플의 구현 기준을 정한다.
...
SessionGateway 버전은 client 연결 관리와 gameplay 실행을 분리한 서버 구성을 보여
준다. Session 서버는 stream 연결과 인증 상태를 소유하고, Play 서버는 actor와 game
room을 소유한다. Play 서버가 client에게 메시지를 보낼 때는 SessionGateway를 통해
현재 actor가 bind된 Session 서버로 보낸다.
...
- Play 서버는 `actorId -> session node RoutingId` 위치를 사용해 client에게 Notify를 보낸다.
...
flowchart LR
    ...
    S -->|Routed Channel backend / ActorRelay| P
    P -->|Routed Channel backend / SessionGateway| S
```

이 버전에서 client가 직접 연결하는 서버는 Session 서버뿐이다. API 서버와 Play
서버는 client-facing endpoint를 열지 않는다. Session 서버는 API 성격의 packet은
API 서버로 channel request를 보내고, gameplay packet은 actor relay로 Play 서버에
전달한다. Play 서버는 SessionGateway로 client-facing 메시지를 Session 서버에
전달한다.
...
| `TicTacToeGateway.Session` | routed channel node | Play 서버와 actor relay/session gateway 메시지를 주고받는다. |
...
| `backend` routed channel | Session, Play | Session <-> Play | actor relay와 session gateway |
...
코드에서는 `Context.OpenActorRelay(...)`와 `SessionGateway.SendToActor(...)` 같은
표면을 사용한다.

## 6. Location Store

SessionGateway 샘플에는 application 위치 저장소가 필요하다.
...
sequenceDiagram
    ...
    S->>S: BindActorAsync(actorId=playerId)
    S->>STORE: Save actorId -> session node rid
    S-->>C: AuthenticateRes
```

인증이 성공하면 Session 서버는 `playerId`를 `actorId`로 사용한다. 이 시점에
`BindActorAsync(actorId)`를 호출해서 현재 stream과 actor를 묶는다. 같은 actor가 다시
인증되면 새 stream binding이 이전 binding을 대체한다.
...
sequenceDiagram
    ...
    STORE-->>S: play-1
    S->>R: OpenActorRelay(backend, play-1, actorId)
    R->>P: Routed request ZLink.ActorRelay
    P->>A: Get or create actor(actorId)
    A->>ROOM: Join match
    ROOM-->>A: Mark + state
    A-->>P: JoinMatchRes
    P-->>R: Routed reply
    ...
    STORE-->>P: session-2
    P->>GW: SendToActor(OpponentJoinedNotify)
    GW->>OS: Routed send ZLink.SessionGateway
    OS-->>OC: Stream OpponentJoinedNotify
```

Session 서버는 `JoinMatchReq`의 내용을 해석해서 게임 규칙을 처리하지 않는다.
Session 서버가 하는 일은 인증된 `actorId`와 target Play node를 정한 뒤 원본 packet을
relay하는 것이다.
...
sequenceDiagram
    ...
    STORE-->>SA: play-1
    SA->>PA: ActorRelay request PlaceMarkReq
    PA->>ROOM: Place mark
    ...
    STORE-->>PA: session-2
    PA->>GW: SendToActor(TurnChangedNotify)
    GW->>SB: Routed send ZLink.SessionGateway
    SB-->>CB: Stream TurnChangedNotify
```

Play 서버가 client에게 push할 때는 client stream을 직접 알지 않는다.
`actorId -> session node RoutingId`를 조회하고, SessionGateway로 대상 Session 서버에
전달한다. Session 서버는 자기 binding table에서 `actorId -> stream`을 찾아 client에게
Notify를 보낸다.
...
sequenceDiagram
    ...
    S2->>S2: BindActorAsync(actor-a)
    S2->>STORE: actor-a -> session-2
    P->>STORE: Lookup actor-a session
    STORE-->>P: session-2
    P->>S2: SessionGateway Notify
    S2-->>C: Stream Notify
```

이 흐름이 SessionGateway 버전의 핵심 장점이다. client가 다른 Session 서버에
...
sequenceDiagram
    ...
    STORE-->>S: play-1
    S->>P1: ActorRelay PlaceMarkReq
    P1-->>S: PlaceMarkRes
    STORE->>STORE: Update actorId -> play-2
    C->>S: Stream PlaceMarkReq
    S->>STORE: Lookup actorId -> play node rid
    STORE-->>S: play-2
    S->>P2: ActorRelay PlaceMarkReq
    P2-->>S: PlaceMarkRes
...
| SessionGateway target binding 없음 | request는 `ActorSessionNotBound` 오류를 반환하고, one-way Notify는 runtime event와 log를 남긴다. |
...
- 인증 응답의 `playerId`를 `actorId`로 사용하고 `BindActorAsync`를 호출한다.
...
- `JoinMatchReq`와 `PlaceMarkReq`는 Session 서버에서 Play 서버로 actor relay된다.
- Play 서버는 actor와 game room을 소유한다.
- `OpponentJoinedNotify`, `TurnChangedNotify`, `GameEndedNotify`는 SessionGateway로 client에게 전달된다.
