...
| spot node | SpotNode 생성, route bridge, actor 생성, ActorGateway attach 대상 | `createSpotNode`, `SpotNode.createActor`, `attachDiscovery`, `createRouteBridge`, `createPublisher` | gap 없음 |
| stream | stream socket 생성, packet handler, ActorGateway attach, bound session operation | `createStreamSocket`, `StreamSocket.setPacketHandler`, `attachActorGateway`, `bindActor`, `unbindActor`, `sendBoundActor`, `boundActors` | gap 없음 |
