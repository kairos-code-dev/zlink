...
| 5 | Go | `bindings/go/perf` | `tcp/tls single 통과, ws/wss single latency 보류, wss SPOT 262144B 보류` | `tcp/ws/wss/tls 측정 완료, public 계약 보존상 보류 항목 남음` | Go 완료 아님. 고성능 consume/send path 계약 설계와 회귀 테스트가 먼저 필요 |
...
| `wss` | `MULTI_PUBSUB` | `보류(44.4%)` | `보류(45.2%)` | `통과(63.2%)` | `통과(53.2%)` | `통과(60.4%)` | `통과(64.7%)` | C full 파일은 위 행과 같다. Go 64B/256B `perf_go_multi_linux_20260522_085159_codex_go_wss_tls_multi_pubsub_small_selected_window_message_20260522.txt`, 1024B/262144B `perf_go_multi_linux_20260522_064814_codex_go_wss_multi_current_20260522.txt`, 65536B/131072B 제한 재측정 `perf_go_multi_linux_20260522_065747_codex_go_wss_multi_pubsub_mid_recheck_20260522.txt`. Go perf latency 기록은 C처럼 수신 시각을 한 번만 잡아 계산하도록 공통 hot path를 줄였고, server publish는 `MoveMessage`로 명시적 ownership 이전 경로를 사용한다. 64B는 `NewWindowMessage`로 native message payload에 직접 stamp하는 경로를 선택 적용했고, 256B는 같은 current-code 재측정에서 소폭 개선됐다. 64B/256B는 이전보다 올랐지만 Go 단순 one-way 최소 기준보다 낮다. `NewWindowMessage` 전체 적용 후보 `perf_go_multi_linux_20260522_085042_codex_go_wss_tls_multi_pubsub_small_window_message_20260522.txt`는 wss 256B를 낮춰 선택 적용으로 좁혔다. topic C 문자열 캐시 후보 `perf_go_multi_linux_20260522_091723_codex_go_wss_tls_pubsub_spot_topic_cstring_cache_20260522.txt`와 `MoveMessage` 내부 `zlink_msg_adopt` 후보 `perf_go_multi_linux_20260522_092410_codex_go_wss_tls_dd_pubsub_movemessage_adopt_20260522.txt`는 small 기준을 올리지 못해 반영하지 않았다. full-run의 65536B/131072B no-result는 같은 조건 제한 재측정으로 배제했다. auto-HWM 활성과 size별 `MsgUnit(B)` 일치를 확인했다. |
| `wss` | `MULTI_SPOT` | `보류(48.6%)` | `보류(26.4%)` | `보류(47.5%)` | `통과(50.4%)` | `통과(56.4%)` | `통과(93.1%)` | C/Go full 파일은 위 행과 같다. Go 64B/256B/1024B `perf_go_multi_linux_20260522_080113_codex_go_wss_multi_spot_small_single_timestamp_20260522.txt`, 나머지는 기존 Go full 파일이다. public `SubscribePart` 수신 경로와 C와 같은 worker 분배 의미를 사용한다. Go perf latency 기록은 C처럼 수신 시각을 한 번만 잡아 계산하도록 공통 hot path를 줄였지만 64B/256B/1024B는 아직 SPOT 최소 기준보다 낮다. C의 `DONTWAIT` 실패 후 blocking publish 1회 재시도, `NODROP`/timeout 분리, worker/stride 조정 후보는 이전 제한 재측정에서 small 또는 large를 해결하지 못했다. server publish를 `MoveMessage`로 바꾸는 후보 `perf_go_multi_linux_20260522_090056_codex_go_wss_tls_multi_spot_small_movemessage_20260522.txt`, 선택 적용 후보 `perf_go_multi_linux_20260522_090236_codex_go_wss_tls_multi_spot_small_selected_movemessage_20260522.txt`는 1024B를 크게 낮추거나 256B 개선이 재현되지 않아 반영하지 않았다. topic C 문자열 캐시 후보 `perf_go_multi_linux_20260522_091723_codex_go_wss_tls_pubsub_spot_topic_cstring_cache_20260522.txt`는 `wss 1024B`를 크게 낮춰 반영하지 않았다. 남은 개선은 C의 native part 소비형 publish와 Go public 원본 보존 계약 차이를 해소하는 고성능 publish path 설계가 필요해 보류한다. auto-HWM 활성과 size별 SpotNode `MsgUnit(B)` 일치를 확인했다. |
...
| `wss` | `MULTI_SPOT_SENDSEND` | `보류(29.2%)` | `보류(29.3%)` | `보류(34.6%)` | `보류(0.0%)` | `보류(46.8%)` | `통과(51.8%)` | C/Go full 파일은 위 행과 같다. Go 64B/256B/1024B/65536B/131072B `perf_go_multi_linux_20260522_083934_codex_go_wss_multi_spot_sendsend_current_move_branch_20260522.txt`, 262144B 제한 재측정 `perf_go_multi_linux_20260522_084035_codex_go_wss_multi_spot_sendsend_262_current_recheck_20260522.txt`. 이전 full-run과 단독 제한 재측정 `perf_go_multi_linux_20260522_065912_codex_go_wss_multi_spot_sendsend65536_recheck_20260522.txt`의 65536B no-result는 Go perf client가 active 송신 중 `SubmitNotConnected`를 fatal로 처리하던 차이 때문이었다. C `MULTI_SPOT_SENDSEND` client처럼 `ZLINK_SUBMIT_NOT_CONNECTED`는 fatal이 아니라 다음 poll loop에서 재시도하는 의미로 맞췄다. C와 같은 size별 active slot 제한을 유지하고, client active loop는 단일 poller 중심이다. `Received.Send()`는 `MoveMessage`가 포함될 때만 move-aware send callback을 사용하고, 일반 `Message`는 기존 실패 원본 보존 경로를 유지한다. 131072B echo reply에만 `MoveMessage`를 선택 적용해 2.9%에서 46.8%로 올렸지만 아직 SPOT 최소 기준보다 낮다. 받은 part를 detach해서 `MoveMessage`로 직접 echo하는 public API 후보 `perf_go_multi_linux_20260522_084548_codex_go_wss_multi_spot_sendsend_takefirst_move_20260522.txt`는 C consume 의미에는 가깝지만 65536B 이상을 크게 악화시켜 반영하지 않았다. `Received.Parts()[0]`를 직접 `Received.Send().MoveMessage(...)`로 echo하는 후보 `perf_go_multi_linux_20260522_094530_codex_go_spot_sendsend_direct_received_part_echo_20260522.txt`도 `wss 262144B` 실패와 large size 붕괴를 만들어 반영하지 않았다. client active 송신을 `MoveMessage`로 바꾸는 후보 `perf_go_multi_linux_20260522_090910_codex_go_wss_tls_multi_spot_sendsend_active_movemessage_probe_20260522.txt`와 small size 선택 후보 `perf_go_multi_linux_20260522_091108_codex_go_wss_tls_multi_spot_sendsend_small_active_movemessage_20260522.txt`도 65536B 또는 262144B completion을 크게 낮춰 반영하지 않았다. active payload를 native message buffer에 직접 stamp하고 `MoveMessage`로 보내는 후보 `perf_go_multi_linux_20260522_093924_codex_go_wss_tls_spot_sendsend_active_window_message_20260522.txt`와 선택 적용 후보 `perf_go_multi_linux_20260522_094156_codex_go_wss_tls_spot_sendsend_direct_active_selected_20260522.txt`는 일부 size를 올렸지만 `wss 262144B` 통과값을 깨 반영하지 않았다. auto-HWM 활성과 size별 SpotNode `MsgUnit(B)` 일치를 확인했다. |
...
| `tls` | `MULTI_PUBSUB` | `보류(44.1%)` | `보류(45.3%)` | `통과(52.4%)` | `통과(51.0%)` | `통과(56.2%)` | `통과(58.7%)` | C/Go full 파일은 위 행과 같다. Go 64B/256B `perf_go_multi_linux_20260522_085159_codex_go_wss_tls_multi_pubsub_small_selected_window_message_20260522.txt`. client는 public `SubscribePart` caller-owned 수신 경로를 사용하고, C처럼 하나의 stop/cooldown 신호로 phase를 종료한다. 64B/256B server publish는 `NewWindowMessage`로 native message payload에 직접 stamp하는 경로를 선택 적용해 이전보다 올랐지만 Go 단순 one-way 최소 기준보다 낮다. 단일 part publish move/restore 후보는 public 실패 원본 보존 계약을 깨 반영하지 않았다. topic C 문자열 캐시 후보 `perf_go_multi_linux_20260522_091723_codex_go_wss_tls_pubsub_spot_topic_cstring_cache_20260522.txt`와 `MoveMessage` 내부 `zlink_msg_adopt` 후보 `perf_go_multi_linux_20260522_092410_codex_go_wss_tls_dd_pubsub_movemessage_adopt_20260522.txt`는 small 기준을 올리지 못해 반영하지 않았다. 남은 개선은 고성능 consume publish path 설계가 필요해 보류한다. auto-HWM 활성과 size별 `MsgUnit(B)` 일치를 확인했다. |
| `tls` | `MULTI_SPOT` | `보류(46.9%)` | `통과(57.2%)` | `통과(79.1%)` | `통과(160.8%)` | `통과(139.8%)` | `통과(112.4%)` | C/Go full 파일은 위 행과 같다. Go 64B `perf_go_multi_linux_20260522_080155_codex_go_tls_multi_spot64_single_timestamp_20260522.txt`, 나머지는 기존 Go full 파일이다. public `SubscribePart` 수신 경로와 C와 같은 worker 분배 의미를 사용한다. Go perf latency 기록은 C처럼 수신 시각을 한 번만 잡아 계산하도록 공통 hot path를 줄였지만 64B는 아직 SPOT 최소 기준보다 낮다. wss/tcp에서 확인한 publish retry, worker/stride, NODROP/timeout 분리 후보가 small 기준을 해결하지 못했다. server publish를 `MoveMessage`로 바꾸는 후보 `perf_go_multi_linux_20260522_090056_codex_go_wss_tls_multi_spot_small_movemessage_20260522.txt`, 선택 적용 후보 `perf_go_multi_linux_20260522_090236_codex_go_wss_tls_multi_spot_small_selected_movemessage_20260522.txt`는 일부 small size만 소폭 올리고 1024B 또는 256B를 낮춰 반영하지 않았다. topic C 문자열 캐시 후보 `perf_go_multi_linux_20260522_091723_codex_go_wss_tls_pubsub_spot_topic_cstring_cache_20260522.txt`는 `wss 1024B`를 크게 낮춰 반영하지 않았다. 남은 개선은 고성능 consume publish path 설계가 필요해 보류한다. 65536B/131072B는 120%를 넘어 outlier 재검토 대상이다. auto-HWM 활성과 size별 SpotNode `MsgUnit(B)` 일치를 확인했다. |
...
| `tls` | `MULTI_SPOT_SENDSEND` | `보류(28.0%)` | `보류(29.2%)` | `보류(28.3%)` | `보류(0.0%)` | `보류(32.3%)` | `보류(0.0%)` | C/Go full 파일은 위 행과 같다. Go `perf_go_multi_linux_20260522_080303_codex_go_tls_multi_spot_sendsend_single_timestamp_20260522.txt`. `SubmitNotConnected`는 C처럼 fatal이 아니라 다음 poll loop에서 재시도하는 의미로 처리한다. C와 같은 size별 active slot 제한을 유지하고, client active loop는 단일 poller 중심이다. Go 서버가 받은 part를 그대로 echo하는 후보와 단일 part send move/restore 후보는 large completion 또는 public 실패 원본 보존 계약을 깨 반영하지 않았다. `Received.Parts()[0]`를 직접 `Received.Send().MoveMessage(...)`로 echo하는 후보 `perf_go_multi_linux_20260522_094530_codex_go_spot_sendsend_direct_received_part_echo_20260522.txt`도 large size를 크게 낮춰 반영하지 않았다. client active 송신을 `MoveMessage`로 바꾸는 후보 `perf_go_multi_linux_20260522_090910_codex_go_wss_tls_multi_spot_sendsend_active_movemessage_probe_20260522.txt`와 small size 선택 후보 `perf_go_multi_linux_20260522_091108_codex_go_wss_tls_multi_spot_sendsend_small_active_movemessage_20260522.txt`도 65536B 또는 262144B completion을 크게 낮춰 반영하지 않았다. active payload를 native message buffer에 직접 stamp하고 `MoveMessage`로 보내는 후보 `perf_go_multi_linux_20260522_093924_codex_go_wss_tls_spot_sendsend_active_window_message_20260522.txt`와 선택 적용 후보 `perf_go_multi_linux_20260522_094156_codex_go_wss_tls_spot_sendsend_direct_active_selected_20260522.txt`는 일부 tls size를 올렸지만 같은 코드 경로 검증에서 `wss 262144B` 통과값을 깨 반영하지 않았다. 남은 개선은 고성능 routed consume send path 설계가 필요해 보류한다. auto-HWM 활성과 size별 SpotNode `MsgUnit(B)` 일치를 확인했다. |
...
- Single `ws`/`wss` latency 보류: throughput은 기준권이어도 C 대비 latency가 커서 통과로
  확정하지 않는다.
- Single `wss SPOT 262144B`: C와 같은 `DONTWAIT` publish, `SubscribePart` 수신 의미에서
  large gap이 재현됐다.
- Multi `DEALER_DEALER`, `PUBSUB`, `SPOT`, `SPOT_SENDSEND` 보류: 단일 poll loop와
  ...
  `MoveMessage(...)`를 추가했다. `Message(...).Submit(...)`의 실패 시 원본 보존
  계약과 `MoveMessage(...)`의 실패 후 재사용 금지 계약은 회귀 테스트로 확인했다.
  다만 `SPOT`/`SPOT_SENDSEND`와 large size에서는 이 경로가 아직 목표 기준을 해결하지
  못했거나 악화돼 선택 적용만 유지한다.
- 다음 구현 단위: `Received.Send()`의 move-aware 경로는 적용됐지만 large size에서는
  아직 기준을 만족하지 못한다. public API를 임의로 넓히지 않고, C의 native part
  consume 경로와 Go의 실패 시 원본 보존 계약 사이에서 어떤 비용이 남는지
  `SPOT_SENDSEND`, `SPOT`, `PUBSUB` 순서로 더 좁힌다.
