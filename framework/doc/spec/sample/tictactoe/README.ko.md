...
> SessionGateway 버전은 client 연결을 가진 Session 서버와 actor를 가진 Play 서버를
> 분리하는 구조를 보여 준다.
...
| [session-gateway.ko.md](./session-gateway.ko.md) | SessionGateway TicTacToe 샘플. client는 Session 서버에만 연결하고, Session 서버가 Play 서버로 packet을 relay한다. |
...
| play -> client push | Play actor가 stream으로 직접 전송 | Play 서버가 SessionGateway로 Session 서버에 전송 |
...
좋다. SessionGateway 버전은 서버간 통신과 session relay가 필요한 운영 구조를
보여 주기 위한 샘플이다.
