...
bool
has_flag (header_flags_t flags, header_flags_t flag)
{
...
void
set_flag (header_flags_t &flags, header_flags_t flag)
{
  flags = flags | flag;
}

void
clear_flag (header_flags_t &flags, header_flags_t flag)
{
  flags = static_cast<header_flags_t> (
    static_cast<std::uint8_t> (flags) & ~static_cast<std::uint8_t> (flag));
}

void
write_u16 (std::vector<std::uint8_t> &bytes, std::uint16_t value)
{
...
void
write_u64 (std::vector<std::uint8_t> &bytes, std::uint64_t value)
{
...
bool
is_defined (message_kind_t kind)
{
  switch (kind) {
...
result_t<void>
validate_header (const stream_header_t &header)
{
  if (!is_defined (header.kind) || !is_defined (header.codec) ||
      (static_cast<std::uint8_t> (header.flags) & ~known_flags) != 0) {
    return result_t<void>::failure (
      error_code_t::frame_decode_failed, "Unknown stream header enum value.");
  }
  const auto has_request_seq =
    has_flag (header.flags, header_flags_t::has_request_seq);
  const auto has_metadata =
    has_flag (header.flags, header_flags_t::has_metadata);
  ...
  if ((header.kind == message_kind_t::request ||
       header.kind == message_kind_t::response) &&
      !has_request_seq) {
    return result_t<void>::failure (
      error_code_t::frame_decode_failed,
  ...
  if (header.kind == message_kind_t::control) {
    if (header.flags != header_flags_t::none || header.codec != codec_t::raw ||
  ...
  if (header.name.empty () ||
    ...
    return result_t<void>::failure (
      error_code_t::validation_failed, "Packet name length is invalid.");
  }
...
header_codec_t::encode (const stream_header_t &source) const
{
  auto header = source;
  ...
  if (auto validation = validate_header (header); !validation) {
    return result_t<std::vector<std::uint8_t>>::failure (
      validation.error_code (), validation.error ()->message);
  }
  auto metadata = metadata_codec_t::encode (header.metadata);
  if (!metadata) {
    return result_t<std::vector<std::uint8_t>>::failure (
      metadata.error_code (), metadata.error ()->message);
  }
  ...
  bytes.reserve (3 + (header.request_seq ? 8 : 0) + 1 +
                 header.name.size () + (metadata.value ().empty ()
                                           ? 0
                                           : 2 + metadata.value ().size ()));
  ...
  if (!metadata.value ().empty ()) {
    ...
    bytes.insert (bytes.end (), metadata.value ().begin (),
                  metadata.value ().end ());
  }
...
header_codec_t::decode (const std::vector<std::uint8_t> &bytes) const
{
  if (bytes.size () < 4) {
    return result_t<stream_header_t>::failure (
      error_code_t::frame_decode_failed, "Helper header is too short.");
  }
  std::size_t offset = 0;
  ...
  header.name =
    std::string (bytes.begin () + static_cast<std::ptrdiff_t> (offset),
                 bytes.begin () +
                   static_cast<std::ptrdiff_t> (offset + name_size));
  offset += name_size;
