...
async function awaitReply(requester, poller, deadlineNs) {
    while (currentEpochNs() < deadlineNs) {
        ...
        const timeoutMs = Math.max(1, Math.min(Number.MAX_SAFE_INTEGER, Number(remainingNs / 1000000n)));
        let ready = [];
        try {
...
async function runSpotReqRepBenchmark(msgSize, options) {
    ...
    try {
        ...
        await waitForProbeReady(requester, replierNode, replier, payload, runId, msgSize, seqRef, requesterPoller);
        ...
        while (currentEpochNs() < activeStopNs) {
            ...
            requester.sendToSpot(replierNode.routingId, replier.routingId, payload);
            seqRef.current += 1n;
            collector.record(await awaitReply(requester, requesterPoller, activeStopNs + 2000000000n), currentEpochNs());
        }
        stampPayload(payload, {
            phase: 2,
            runId,
        ...
        requester.sendToSpot(replierNode.routingId, replier.routingId, payload);
        await awaitReply(requester, requesterPoller, currentEpochNs() + 2000000000n);
