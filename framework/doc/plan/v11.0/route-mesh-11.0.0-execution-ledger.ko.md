...
| `V11-M6-OBJECT-CONTEXT-NODE` | Node.js object Context composition | Node.js runtime lane, `P-DEEP` | `V11-CA-OBJECT-CONTEXT-REVIEW` | 완료 | Context-only factory·identity·source fence test 통과 | `ZLinkActorFactory.create(context, signal?)`만 사용하고 `ZLinkActor.actorId`, Actor Context의 handler·leave·mutable Spot·ActorRef projection을 제거해 `actor.context.actorId`를 identity의 단일 기준으로 사용한다. User·Entry·Instance Spot Context에는 Core 또는 activation authority에서 받은 `objectGeneration`과 생성 시점의 Mesh·Spot·Node identity를 설치하고 네 identity field를 immutable로 고정했으며 Actor leave는 containing User Spot Context가 수행한다. 다른 Context를 반환한 Actor factory를 거부한다. Framework·NestJS build, exact type contract, Object Context focused 2/2, Entry dispatch 9/9와 M6B 39/39가 통과했다. |
...
| `V11-M6-MESSAGE-CONTEXT-DN` | .NET MessageContext 통일 | .NET runtime lane, `P-DEEP` | `V11-CA-MESSAGE-CONTEXT-REVIEW` | 진행 | context field·marker 제거와 dispatch test 통과 | Production handler를 `IZLinkMessageContext`, `ZLinkRouteMessageContext`, `ZLinkPublishMessageContext`와 `ZLinkHandlerInvocation.MessageContext`로 전환하고 이전 handler·request/send/route/Spot Actor context를 제거했다. Framework source build와 owned-path diff check는 통과했다. UnitTests에는 이전 handler signature가 59건 남아 compile이 실패하므로 test migration, exact contract와 evidence를 완료해야 한다. ObjectContext는 이 작업에서 변경하지 않았다. |
...
| `V11-M6-SESSION-BINDING-JVM` | JVM stored route·disconnect runtime | JVM runtime lane, `P-DEEP` | `V11-CA-SESSION-BINDING-REVIEW` | 진행 | IMP-SA-1~4 Java·Kotlin 항목과 contract test gap 0 | Relay의 `ZLinkActorDirectory.find`와 per-message native bind·hidden rebind를 제거하고 exact binding snapshot all-settled·dedupe·failure cleanup, logical notification과 same-ObjectGeneration route fence를 구현했다. 전체 `:zlink-framework-core:test`와 `:zlink-framework-kotlin:test`가 통과했다. 현재 public object role·Context-only factory·RelocationAdapter로 전환한 `SpotActorTransfer` 실제 process는 Actor 생성, remote User Spot admission, immutable relocation payload restore, target location commit, generation 유지와 relocation 이후 Actor request handler·reply까지 통과했다(`framework/languages/java/e2e/SpotActorTransfer/log/20260725-095852-3211205`). 남은 blocker는 target에서 source의 stored session route로 보내는 `BoundPushNotify`가 8초 observation budget 안에 도착하지 않는 점이다. 이 relay와 `ST-E1A`를 통과한 뒤 완료로 전환한다. |
