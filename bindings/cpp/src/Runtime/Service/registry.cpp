...
const void *registry_access_t::native_handle (const service::registry_t &registry_) noexcept
{
    return registry_._impl ? registry_._impl->handle : nullptr;
}
...
registry_t::registry_t (context_t &ctx_) : _impl (std::make_unique<impl> ()), _last_error (0)
{
...
registry_t::registry_t (registry_t &&other) noexcept : _impl (std::move (other._impl)), _last_error (other._last_error)
{
    if (!other._impl)
...
void registry_t::bind (const std::string &pub_endpoint_, const std::string &router_endpoint_)
{
    ...
    detail::throw_if_failed<bind_error_t> (static_cast<bind_result_t> (
      zlink_registry_bind (_impl->handle, pub_endpoint_.c_str (), router_endpoint_.c_str ())));
}

void registry_t::set (int option_, uint32_t value_)
{
    detail::throw_if_failed<config_error_t> (static_cast<config_result_t> (
      zlink_registry_set (_impl->handle, static_cast<zlink_registry_option_t> (option_), value_)));
}

uint32_t registry_t::get (int option_) const
{
    zlink_config_result_t err = static_cast<zlink_config_result_t> (0);
    const uint32_t value = zlink_registry_get (_impl->handle, static_cast<zlink_registry_option_t> (option_), &err);
    detail::throw_if_failed<config_error_t> (static_cast<config_result_t> (err));
    return value;
}

void registry_t::set_heartbeat (std::chrono::milliseconds interval_, std::chrono::milliseconds timeout_)
{
    set_heartbeat (zlink::detail::native_timeout_ms (interval_), zlink::detail::native_timeout_ms (timeout_));
}
...
void registry_t::add_peer (const std::string &peer_pub_endpoint_)
{
    ...
    detail::throw_if_failed<connect_error_t> (
      static_cast<connect_result_t> (zlink_registry_add_peer (_impl->handle, peer_pub_endpoint_.c_str ())));
}

void registry_t::set_tls_server (const std::string &cert_, const std::string &key_, bool require_client_cert_)
{
    detail::throw_if_failed<config_error_t> (static_cast<config_result_t> (
      zlink_set_tls_server (_impl->handle, cert_.c_str (), key_.c_str (), require_client_cert_ ? 1 : 0)));
}

void registry_t::set_tls_client (const std::string &ca_cert_, const std::string &hostname_, bool trust_system_)
{
...
registry_status_t registry_t::status () const
{
    ...
    detail::throw_if_failed<config_error_t> (
      static_cast<config_result_t> (zlink_registry_status (_impl->handle, &native)));
    return zlink::detail::service_model_access_t::from_native (native);
}

std::vector<registry_service_summary_entry_t>
registry_t::service_summary (const registry_service_summary_filter_t *filter_) const
{
    ...
    size_t count = 0;
    detail::throw_if_failed<config_error_t> (
      static_cast<config_result_t> (zlink_registry_service_summary (_impl->handle, filter_ptr, nullptr, &count)));
    std::vector<zlink_registry_service_summary_entry_t> native (count);
    if (count > 0) {
        detail::throw_if_failed<config_error_t> (static_cast<config_result_t> (
          zlink_registry_service_summary (_impl->handle, filter_ptr, native.data (), &count)));
        native.resize (count);
    }
...
std::vector<registry_topology_entry_t> registry_t::topology () const
{
    size_t count = 0;
    detail::throw_if_failed<config_error_t> (
      static_cast<config_result_t> (zlink_registry_topology (_impl->handle, nullptr, nullptr, &count)));
    std::vector<zlink_registry_topology_entry_t> native (count);
    if (count > 0) {
        detail::throw_if_failed<config_error_t> (
          static_cast<config_result_t> (zlink_registry_topology (_impl->handle, nullptr, native.data (), &count)));
        native.resize (count);
    }
...
std::vector<registry_topology_entry_t> registry_t::topology (const registry_topology_filter_t &filter_) const
{
    ...
    size_t count = 0;
    detail::throw_if_failed<config_error_t> (
      static_cast<config_result_t> (zlink_registry_topology (_impl->handle, &native_filter, nullptr, &count)));
    std::vector<zlink_registry_topology_entry_t> native (count);
    if (count > 0) {
        detail::throw_if_failed<config_error_t> (static_cast<config_result_t> (
...
std::vector<member_peer_entry_t> registry_t::member_peers (const std::string &channel_name_) const
{
    ...
    if (count > 0) {
        detail::throw_if_failed<config_error_t> (static_cast<config_result_t> (
          zlink_registry_member_peers (_impl->handle, channel_name_.c_str (), native.data (), &count)));
        native.resize (count);
    }
...
void registry_t::close ()
{
    ...
    detail::throw_if_failed<close_error_t> (static_cast<close_result_t> (zlink_registry_destroy (&tmp)));
