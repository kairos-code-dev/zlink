...
async function main(): Promise<void> {
  ...
  const handles = app.get(ZLINK_SPOT_HANDLE_RESOLVER, { strict: false }) as ZLinkSpotHandleResolver;
  ...
  const server = await startHttpServer(options.httpUrl, [
    ...
    }) },
    {
      method: 'POST', path: '/workflows', handle: async (body) => {
        ...
        const handle = await handles.resolveSpotHandle(String(created.spotRid));
