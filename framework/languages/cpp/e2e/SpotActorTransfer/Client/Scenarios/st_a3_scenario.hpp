...
inline void scenario_runner_t::run_st_a3_scenario ()
{
    ...
    auto join_task = std::async (
      std::launch::async, [&] { return join_actor (_nodes.a, actor_id, {"ST-A3", spot_rid}); });
    const auto waiting =
      wait_evidence (_nodes.a, {
                                 "ST-A3|" + actor_id + "|admission|spot=" + spot_rid,
                                 "transfer|" + actor_id + "|leave|13",
                                 "ST-A3|" + actor_id + "|joined_wait|" + spot_rid,
                               });
    ...
    require (probe.spot_rid == spot_rid, "ST-A3 blocked packet did not resume on the target spot.");
    ...
    wait_evidence (_nodes.a, {"ST-A3|" + actor_id + "|success_reply|" + spot_rid});
}
