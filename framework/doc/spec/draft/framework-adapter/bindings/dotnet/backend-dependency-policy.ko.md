...
[문서 목록](../../README.ko.md) | [이전: ZLink Framework .NET Implementation Scope And Non-Goals](implementation-scope-and-nongoals.ko.md) | [다음: ZLink Framework .NET Channel Messaging Samples](channel-messaging-samples.ko.md)
...
> 이 문서는 **구현 전 초안**이다.
> 아직 공개된 계약[^public-contract]이 아니며, `.NET` framework 가 현재
> `bindings/dotnet` 을 backend[^backend] 로 쓰면서도 나중에 다른 저수준
> 라이브러리로 갈아 끼울 수 있게 만들기 위한 기준을 정리한다.

## 1. 목적

지금 시점에서 가장 현실적인 선택은 `bindings/dotnet` 라이브러리를 backend 로
그대로 활용하는 것이다. 하지만 framework 의 public contract 가 이 backend 구현체에
지나치게 얽혀 버리면, 나중에 저수준 라이브러리를 교체할 때 public API 까지 같이
깨질 수밖에 없다.
...
- framework 의 public contract 가 우선이고, backend 라이브러리는 언제든 교체할
  수 있는 구현체로 본다.
...
가 곧 `bindings/dotnet` 의 객체 모델 그대로여야 한다" 는 주장은 같은 의미가
아니다.
...
이 타입들은 특정 runtime 객체가 아니라 transport identity[^transport-identity],
payload, submit option 처럼 의미가 분명한 기초 primitive 다. 즉 나중에 backend 가
교체되더라도 같은 의미를 유지할 수 있도록 compatibility layer[^compatibility-layer]
를 제공할 수 있는 종류다.
...
에 한 번 새어 들어가면, 다음 backend 교체가 곧바로 breaking change[^breaking-change]
가 되어 버린다.
...
- 만약 backend 가 교체될 때 동일 의미를 유지하기 어려운 값이라면, framework
  쪽에서 정의한 synthetic enum[^synthetic-enum] 과 snapshot DTO[^dto] 를 우선
  드러내고 native detail 은 줄이는 편을 기본으로 본다.

즉 monitoring public API 는 "backend 의 raw event 를 그대로 재수출하는 구조"
보다, "framework 가 한 번 재해석한 typed runtime event + 꼭 필요할 때만 추가되는
native detail" 구조가 backend 교체에 더 안전하다.
...
- registration, lifecycle[^lifecycle], monitoring, query 같은 framework 본연의
  역할은 framework service 가 맡고, 실제 backend 호출은 adapter layer 가 떠맡는다.
...
즉 backend 교체는 어디까지나 adapter layer 의 교체가 기본이며, public API 자체의
교체는 그것과는 별도의 breaking change 작업으로 분리해서 진행하는 편을 원칙으로
삼는다.

## 9. 회귀 테스트

backend 의존 정책은 framework 의 public API 와 adapter factory 두 축으로 점검한다.
구현체가 바뀌더라도 사용자는 backend 의 concrete type 을 알 필요가 없어야 하며,
