...
| `wss` | `MULTI_SPOT_SENDSEND` | `보류(27.9%)` | `보류(28.9%)` | `보류(34.3%)` | `통과(52.2%)` | `보류(48.5%)` | `통과(52.5%)` | C `perf_c_multi_linux_20260522_063910_codex_c_wss_multi_for_go_20260522.txt` 대비 Go `perf_go_multi_linux_20260522_122832_codex_go_spot_sendsend_forward_routed_selected_20260522.txt`, 131072B 단독 확인 `perf_go_multi_linux_20260522_123038_codex_go_wss_spot_sendsend_131_forward_gate_recheck_20260522.txt`. C와 같은 size별 active slot 제한과 `SubmitNotConnected` 재시도 의미를 유지한다. 서버 echo는 새 public `Spot.ForwardRouted(...)`를 65536B/262144B에만 적용해 no-result였던 65536B를 유효 통과값으로 올렸고 262144B 통과권도 유지했다. small size와 131072B는 기존 builder 경로가 더 낫거나 최신 재측정에서 기준 아래라 consume-forward를 적용하지 않는다. 이전 `Bytes(...)`/`MoveMessage(...)`/`RecvRoutedPart(...)` 후보들은 다른 size의 completion 또는 public 원본 보존 계약을 깨 반영하지 않았다. auto-HWM 활성과 size별 SpotNode `MsgUnit(B)` 일치를 확인했다. |
...
- Go multi routed echo 보류의 consume-forward 1차 구현은 완료했다.
  ...
  small size와 wss 131072B는 아직 보류다.
...
- 다음 구현 단위: `ForwardRouted(...)`가 해결하지 못한 encrypted routed echo submit 비용을
  `tls MULTI_SPOT_SENDSEND`와 `wss 131072B`에서 더 좁힌다. public API를 임의로 넓히지 않고,
...
| Transport | Pattern | 64 | 256 | 1024 | 65536 | 131072 | 262144 | 결과 파일 / 메모 |
|-----------|---------|----|-----|------|-------|--------|--------|------------------|
| `tcp` | `MULTI_DEALER_DEALER` | `보류(6.0%)` | `보류(9.2%)` | `보류(11.9%)` | `통과(45.8%)` | `통과(35.9%)` | `보류(20.7%)` | C `perf_c_multi_linux_20260522_162958_codex_c_multi_tcp_for_python_duration1_20260522.txt`, Python `perf_python_multi_linux_20260522_164901_codex_python_multi_tcp_nonspot_stream_duration1_20260522.txt` 기준이다. 실행 중 non-SPOT server/client는 각 `nlwp=8`/`nlwp=7` 수준이었고 `clients=100`이 thread 100개로 확장되지는 않았다. 65536B/131072B만 기준선을 넘고 나머지는 낮다. |
| `tcp` | `MULTI_DEALER_ROUTER` | `보류(12.8%)` | `보류(13.3%)` | `보류(13.1%)` | `통과(40.5%)` | `통과(39.1%)` | `통과(38.9%)` | C/Python 파일은 위 행과 같다. large size는 routed multi 기준선을 넘고 small size는 낮다. |
...
| `tcp` | `MULTI_SPOT_SENDSEND` | `보류(11.7%)` | `보류(11.8%)` | `통과(74.7%)` | `보류(13.9%)` | `보류(18.6%)` | `보류(37.4%)` | C `perf_c_multi_linux_20260522_162958_codex_c_multi_tcp_for_python_duration1_20260522.txt` 기준이다. 64/256/1024B는 Python `perf_python_multi_linux_20260522_204542_codex_python_multi_tcp_sendsend_small_final_recheck_20260522.txt`, 65536B는 `perf_python_multi_linux_20260522_203936_codex_python_multi_tcp_sendsend65536_smallpoll_no_largepoller_20260522.txt`, 131072B는 `perf_python_multi_linux_20260522_204134_codex_python_multi_tcp_sendsend_large_no_poller_recheck_20260522.txt`, 262144B는 `perf_python_multi_linux_20260522_204253_codex_python_multi_tcp_sendsend262144_no_large_poller_recheck_20260522.txt` 기준으로 갱신했다. server/client는 각 `nlwp=12` 수준으로 thread 폭증은 아니었다. small size는 서버/client의 고정 1ms sleep 대신 poller wake를 쓰도록 바꿔 64/256/1024B가 개선됐고, 65536B 이상은 Spot poller 등록 자체가 timeout을 유발해 기존 sleep fallback을 유지한다. 1024B만 기준선을 넘고 나머지는 낮다. |
...
- 같은 `.data` 경로를 DD server / pubsub·routed client 헤더 디코드에도 재사용할 수 있으나, 같은 per-call FFI 벽이 이득을 제한한다. Python 메시지당 FFI 호출 수 감소(배치 수신)는 별도 public API 설계 대상이다.
