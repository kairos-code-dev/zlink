...
현재 공통 시나리오는 `.NET` 샘플에서 검증된 흐름을 반영한다. 다른 언어 framework를
구현할 때도 결과 샘플은 같은 역할 분리, 같은 request/response/notify 이름, 같은
상태 필드, 같은 smoke 검증 순서를 따라야 한다. 언어별 API 모양은 달라도 사용자가
샘플을 읽었을 때 같은 framework 기능을 같은 순서로 확인할 수 있어야 한다.
...
- 게임 규칙은 작게 유지하고, session, actor, Spot, channel, timer, push 흐름이
  코드에서 잘 보이게 둔다.
- Play 서버는 `Domain`, `Application`, `Adapters` 레이어를 나누어 구현한다.
  디렉토리 이름은 언어 관용에 맞게 바꿀 수 있지만 같은 책임 분리는 유지해야 한다.
  - `Domain`은 게임 규칙, 상태 전이, 승패 판정, 도메인 event 생성을 맡는다.
...
언어별 문법과 build system은 달라도 Play 서버 소스는 아래 구조를 기준으로 맞춘다.
...
필요 없으면 `Notifications/`를 두지 않아도 된다. 반대로 Bingo처럼 bound session push와
domain event 변환이 필요하면 `Notifications/`를 둔다.
