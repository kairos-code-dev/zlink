...
> `zlink_set_sub_option(ZLINK_SUB_OPT_SUBSCRIBE, ...)` 경로로 흡수된다.
> 범위: `core/include/zlink.h` public C API에 타입별 옵션 함수/enum 추가,
> gateway/spot 기존 옵션 API 통합, subscribe/unsubscribe 경로 통합을 다룬다.
...
- spot/spot_node를 하나의 public surface로 유지하면 pub/sub 의미 모호,
  getter 계약 충돌, routing_id 정의 불가, discovery attach 불명확 등
  문제가 반복된다.

이 문서는 raw 소켓에 타입별 옵션 함수+enum을 도입하고,
gateway/discovery도 같은 함수 체계로 통합하되
**spot/spot_node public surface 자체를 pub/sub side 기준으로 분리**하는
계획을 정리한다.
...
- unified `spot` / `spot_node` handle은 제거하고,
  `spot-pub`, `spot-sub`, `spotnode-pub`, `spotnode-sub`를 1급 public
  surface로 도입한다.
...
- **현재 mainline 기준 구현은 아직 unified surface 모델이다.**
  ...
  이 문서는 그 상태에서 **major break**를 일으켜 새 surface로 교체하는 계획이다.
- **문서의 목표 상태와 현재 코드는 다르다.**
  문서에 `spot-pub`, `spot-sub`, `spotnode-pub`, `spotnode-sub`가
  등장하더라도, 현재 공개 헤더에 그 이름의 API/handle이 이미 있다고
  가정하면 안 된다. 대부분은 이번 작업에서 새로 추가/분리해야 한다.
...
- **unified spot / unified spot_node**: 현재 코드에 존재하지만 이 계획에서
  제거 대상인 기존 public surface
...
- unified `spot` / `spot_node` public handle은 최종 상태에서 제거된다.
...
- `spot-pub/sub`, `spotnode-pub/sub`는 side가 명확한 API만 허용한다.
...
| `zlink_spot_sub_set_option(spot_sub, ZLINK_SPOT_SUB_OPT_RCVHWM, ...)` | `zlink_set_sub_option(spot_sub, ZLINK_OPT_RCVHWM, ...)` |
...
- socket, gateway, discovery, spot-pub/sub, spotnode-pub/sub에서 동작하며,
  각각 내부 경로가 다르다.

전용 함수 2개로 분리하면 set/get 데이터 계약이 명확하고,
STREAM 제약도 함수 내부에서 일관되게 처리된다.

완전 분리 모델에서는 unified base ID 개념을 두지 않는다.
각 side handle은 자신의 routing_id를 직접 가진다:
- `zlink_set_routing_id(spot_pub, ...)` → 해당 pub handle의 routing_id 설정
- `zlink_set_routing_id(spot_sub, ...)` → 해당 sub handle의 routing_id 설정
- `zlink_set_routing_id(node_pub, ...)` → 해당 node-pub handle의 routing_id 설정
- `zlink_set_routing_id(node_sub, ...)` → 해당 node-sub handle의 routing_id 설정
...
gateway, discovery, spot-pub/sub, spotnode-pub/sub, registry를 포함한다.

### 2.5 spot/spot_node public surface 완전 분리 결정

spot/spot_node는 pub/sub가 서로 다른 socket 의미를 가진다. 이를 하나의
public surface로 유지하면 다음 문제가 반복된다:

- side-specific 옵션(`SNDHWM`, `RCVHWM` 등)의 getter/setter 계약이 모호해진다.
- 공통 옵션과 side-specific 옵션을 같은 getter에 억지로 태우면
  source-of-truth가 흔들린다 (lossy contract).
- routing_id를 하나의 값으로 설명하기 어렵다.
- discovery attach가 실제로 어느 쪽 socket 집합에 적용되는지 불명확해진다.

따라서 이 계획은 unified `spot` / `spot_node` surface를 유지하지 않고,
public surface 자체를 side 기준으로 분리한다:

- **spot-pub** 관점: pub socket에만 적용되는 옵션/조회
- **spot-sub** 관점: sub socket에만 적용되는 옵션/조회
- **spotnode-pub** 관점: spot_node 자체 pub socket에 직접 적용
- **spotnode-sub** 관점: spot_node 자체 sub socket에 직접 적용

즉, unified `spot` / `spot_node` public API는 더 이상 1급 표면으로 두지 않는다.
옵션/송수신/monitor/discovery 연결 모두 side가 명확한 public surface로 재구성한다.
...
| routing_id | side handle own routing_id | ✗ 비복제 — child spot은 자체 ID를 별도 설정해야 함 |
| runtime/read-only | TOPICS_COUNT, FD, EVENTS, TYPE, LAST_ENDPOINT | ✗ 비복제 (socket별 고유 runtime 값) |

이 분리로 인해:
- set/get 대상 socket이 하나로 고정된다.
- SNDHWM, LINGER 등 모든 옵션의 의미가 자연스럽다.
- getter 계약이 단순해진다.
- routing_id도 side별로 바로 설명된다.
- discovery attach도 어느 side에 적용되는지 명시적으로 다룰 수 있다.
...
| spot 통합 facade에 양면 옵션 setter/getter | getter에서 lossy contract 발생, side 모호 → 기각 (§2.5) |
| **타입별 함수+enum 분리 + spot side 분리** | 기존 패턴 확장, 직관적, 계약 명확 → **채택** |

## 3. 새 API 명세

### 3.0 spot/discovery 계약 정리

- unified `spot` / `spot_node` handle은 새 public API에서 제거한다.
- 새 public surface는 `spot-pub`, `spot-sub`, `spotnode-pub`, `spotnode-sub`
  네 종류의 side handle이다.
- `zlink_set_option(spot, ...)`/`zlink_get_option(spot, ...)` 같은 unified
  surface는 제공하지 않는다.
- 공통 옵션도 각 side handle에 대해
  `zlink_set_pub_option(spot_pub, ...)` / `zlink_set_sub_option(spot_sub, ...)`
  형태로만 설정한다.
...
/* ── Routing ID (socket/gateway/discovery/spot-pub/sub/spotnode-pub/sub,
 *               STREAM 제외) ── */
...
/* ── TLS (gateway/discovery/spot-pub/sub/spotnode-pub/sub/registry) ── */
...
/* ── Router 전용 (ROUTER socket, DEALER의 PROBE만, gateway) ── */
...
/* ── Pub 옵션 (PUB/XPUB socket, spot-pub, spotnode-pub) ──
 * option_: zlink_pub_option_t (pub 전용) 또는
 *          zlink_option_t (공통 옵션 — 해당 side에 적용) */
...
/* ── Sub 옵션 (SUB/XSUB socket, spot-sub, spotnode-sub) ──
 * option_: zlink_sub_option_t (sub 전용) 또는
 *          zlink_option_t (공통 옵션 — 해당 side에 적용) */
...
`zlink_set_pub_option` / `zlink_get_pub_option`은 `int option_`으로
pub 전용 enum(0x33xx)과 공통 enum(0x30xx) 값을 모두 수용한다.
spot-pub에서 공통 옵션(LINGER, SNDHWM 등)을 pub side에 적용할 때
별도 enum 없이 `zlink_set_pub_option(spot_pub, ZLINK_OPT_LINGER, ...)`처럼 사용한다.
...
{
    ZLINK_SUB_OPT_SUBSCRIBE            = 0x3400,
    ZLINK_SUB_OPT_UNSUBSCRIBE          = 0x3401,
    ZLINK_SUB_OPT_TOPICS_COUNT         = 0x3402   /* get-only */
} zlink_sub_option_t;
```

pub과 동일하게 `int option_`으로 공통 enum 값도 수용한다.
...
| 함수 | PAIR | PUB/XPUB | SUB/XSUB | DEALER | ROUTER | STREAM | Gateway | Discovery | SpotPub | SpotSub | SpotNodePub | SpotNodeSub |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `zlink_set_option` | set | set | set | set | set | set | set† | set⁂ | - | - | - | - |
| `zlink_get_option` | get | get | get | get | get | get | get† | - | - | - | - | - |
| `zlink_set_routing_id` | set | set | set | set | set | EINVAL§ | set | set | set | set | set | set |
| `zlink_get_routing_id` | get | get | get | get | get | EINVAL§ | get | get | get | get | get | get |
...
- SpotPub/SpotSub: `spot-pub`, `spot-sub` side handle
- SpotNodePub/SpotNodeSub: `spotnode-pub`, `spotnode-sub` side handle
...
| spot-pub | `zlink_get_pub_option` | pub side 허용 옵션 (§3.3.5) + TOPICS_COUNT | live XPUB socket |
| spot-pub | `zlink_get_routing_id` | routing_id | side handle own routing_id |
| spot-sub | `zlink_get_sub_option` | sub side 허용 옵션 (§3.3.5) + TOPICS_COUNT | live XSUB socket |
| spot-sub | `zlink_get_routing_id` | routing_id | side handle own routing_id |
| spotnode-pub | `zlink_get_pub_option` | pub side 허용 옵션 (§3.3.5) + TOPICS_COUNT | live pub socket `getsockopt()` |
| spotnode-pub | `zlink_get_routing_id` | routing_id | side handle own routing_id |
| spotnode-sub | `zlink_get_sub_option` | sub side 허용 옵션 (§3.3.5) + TOPICS_COUNT | live sub socket `getsockopt()` |
| spotnode-sub | `zlink_get_routing_id` | routing_id | side handle own routing_id |

#### 3.3.4 lifecycle 계약

| 핸들 상태 | set 동작 | get 동작 |
| --- | --- | --- |
| **spot-pub** + `zlink_set_pub_option(spot_pub, ...)` | live pub socket에 직접 적용 | live pub socket에서 읽음 |
| **spot-sub** + `zlink_set_sub_option(spot_sub, 정수옵션)` | live sub socket에 직접 적용 | live sub socket에서 읽음 |
| **spot-sub** + `zlink_set_sub_option(spot_sub, SUBSCRIBE)` | live sub socket에 직접 적용 | N/A (set-only) |
| **spotnode-pub** + `zlink_set_pub_option(node_pub, ...)` | 자체 pub socket에 직접 적용 | live pub socket에서 읽음 |
| **spotnode-sub** + `zlink_set_sub_option(node_sub, ...)` | 자체 sub socket에 직접 적용 | live sub socket에서 읽음 |
...
#### 3.3.5 spot/spot_node side별 허용 옵션 목록

spot-pub/sub, spotnode-pub/sub의 `set_pub_option`/`set_sub_option`은
`zlink_option_t` 전체가 아니라
기존 내부 구현(`spot_pub_t::set_option()`, `spot_sub_t::set_option()`,
`spot_node_t::set_pub_option()`, `spot_node_t::set_sub_option()`)이
실제로 지원하는 옵션만 허용한다. 미지원 옵션은 EINVAL을 반환한다.

**pub side 허용 공통 옵션** (spot-pub, spotnode-pub):
...
**sub side 허용 공통 옵션** (spot-sub, spotnode-sub):
...
| sub 전용 옵션 | 비고 |
| --- | --- |
| `ZLINK_SUB_OPT_SUBSCRIBE` | set-only, filter 문자열 |
| `ZLINK_SUB_OPT_UNSUBSCRIBE` | set-only, filter 문자열 |
| `ZLINK_SUB_OPT_TOPICS_COUNT` | get-only |

참조: `spot_pub_t::set_option()` (`spot_pub.cpp:269`),
`spot_sub_t::set_option()` (`spot_sub.cpp:490`),
`spot_node_t::set_pub_option()` (`spot_node.cpp:2482`),
`spot_node_t::set_sub_option()` (`spot_node.cpp:2510`).
...
/* ── Routing ID (side handle direct) ── */
zlink_set_routing_id (spot_pub, "spot-pub-01", 11);
zlink_set_routing_id (spot_sub, "spot-sub-01", 11);
zlink_routing_id_t out;
zlink_get_routing_id (spot_pub, &out);
...
/* ── spot-pub: 공통 옵션을 pub side에 적용 ── */
int spot_sndhwm = 2000;
zlink_set_pub_option (spot_pub, ZLINK_OPT_SNDHWM,
                      &spot_sndhwm, sizeof (spot_sndhwm));
int spot_linger = 500;
zlink_set_pub_option (spot_pub, ZLINK_OPT_LINGER,
                      &spot_linger, sizeof (spot_linger));

/* ── spot-sub: 공통 옵션을 sub side에 적용 ── */
int spot_rcvhwm = 3000;
zlink_set_sub_option (spot_sub, ZLINK_OPT_RCVHWM,
                      &spot_rcvhwm, sizeof (spot_rcvhwm));

/* ── spot-sub: 구독 필터 ── */
zlink_set_sub_option (spot_sub, ZLINK_SUB_OPT_SUBSCRIBE,
                      "market.AAPL", 11);

/* ── spotnode-pub: 자체 pub socket에 직접 적용 ── */
int node_sndhwm = 4000;
zlink_set_pub_option (node_pub, ZLINK_OPT_SNDHWM,
                      &node_sndhwm, sizeof (node_sndhwm));
```

### 3.5 분리로 인해 변경되는 기존 unified API

완전 분리 모델에서는 기존 unified `spot` / `spot_node` public API도 영향을 받는다.
옵션 함수만 바꾸는 것으로는 surface가 닫히지 않으므로, 아래 API를 함께 정리해야 한다.

#### 3.5.1 기존 unified API inventory

| 기존 API | 현재 의미 | 새 모델에서의 처리 |
| --- | --- | --- |
| `zlink_spot_new()` | unified spot 생성 | **삭제**. side별 생성 API로 분리 |
| `zlink_spot_destroy()` | unified spot 파괴 | **삭제**. side별 destroy로 분리 |
| `zlink_spot_publish()` | unified spot에서 pub 송신 | **split** → spot-pub 전용 |
| `zlink_spot_sub_recv()` | unified spot에서 sub 수신 | **split** → spot-sub 전용 |
| `zlink_spot_subscribe()` | unified spot에서 subscribe | **split** → spot-sub 전용 |
| `zlink_spot_subscribe_pattern()` | unified spot에서 pattern subscribe | **split** → spot-sub 전용 |
| `zlink_spot_unsubscribe()` | unified spot에서 unsubscribe | **split** → spot-sub 전용 |
| `zlink_spot_send_ready_handler()` | unified spot에서 pub send-ready | **split** → spot-pub 전용 |
| `zlink_spot_monitor_open(role)` | role 인자로 side 선택 | **split**. role 없는 side별 monitor API로 분리 |
| `zlink_spot_node_new()` | unified spot_node 생성 | **삭제**. side별 node 생성 API로 분리 |
| `zlink_spot_node_destroy()` | unified spot_node 파괴 | **삭제**. side별 destroy로 분리 |
| `zlink_spot_node_bind()` | unified node bind | **split**. node-pub / node-sub bind 계약 재정의 필요 |
| `zlink_spot_node_publish()` | unified node에서 pub 송신 | **split** → spotnode-pub 전용 |
| `zlink_spot_node_recv()` | unified node에서 sub 수신 | **split** → spotnode-sub 전용 |
| `zlink_spot_node_subscribe()` | unified node에서 subscribe | **split** → spotnode-sub 전용 |
| `zlink_spot_node_subscribe_pattern()` | unified node에서 pattern subscribe | **split** → spotnode-sub 전용 |
| `zlink_spot_node_unsubscribe()` | unified node에서 unsubscribe | **split** → spotnode-sub 전용 |
| `zlink_spot_node_send_ready_handler()` | unified node에서 pub send-ready | **split** → spotnode-pub 전용 |
| `zlink_spot_node_monitor_open(role)` | role 인자로 side 선택 | **split**. role 없는 side별 monitor API로 분리 |
| `zlink_spot_node_connect_peer_pub()` | unified node peer pub 연결 | **split** → spotnode-pub 관리 API |
| `zlink_spot_node_disconnect_peer_pub()` | unified node peer pub 해제 | **split** → spotnode-pub 관리 API |
| `zlink_spot_node_attach_discovery()` | unified node에 discovery attach | **split**. node-pub/node-sub 각각 attach 필요 |
...
#### 3.5.2 role 기반 API 정리

`zlink_spot_role_t`, `zlink_spot_monitor_open()`, `zlink_spot_node_monitor_open()`
같이 role 인자로 side를 선택하는 API는 완전 분리 모델과 맞지 않는다.
새 surface에서는 role 인자를 제거하고 side별 함수로 분리한다.

#### 3.5.3 옵션 API 삭제 대상

분리 이후 삭제되는 옵션 API는 다음과 같다:

| 삭제 대상 (옵션 API) | 대체 |
| --- | --- |
| `zlink_spot_node_set_pub_option(node, opt, ...)` | `zlink_set_pub_option(node_pub, ...)` |
| `zlink_spot_node_set_sub_option(node, opt, ...)` | `zlink_set_sub_option(node_sub, ...)` |
| `zlink_spot_set_pub_option(spot, opt, ...)` | `zlink_set_pub_option(spot_pub, ...)` |
| `zlink_spot_set_sub_option(spot, opt, ...)` | `zlink_set_sub_option(spot_sub, ...)` |

#### 3.5.4 헤더 문구 수정 대상

현재 `zlink.h`의 `zlink_spot_node_set_pub_option()` 주석은
"default SpotPub option for the node and future child pubs"라고 기술한다.
새 모델에서는 이 문구를 다음으로 교체해야 한다:

> "spotnode-pub handle의 pub socket option을 설정한다."
...
| side handle tag | 신규 | `core/src/api/zlink_option.cpp` |

```
zlink_set_option(handle, option, val, len)
  ├─ as_socket_handle(handle) → socket?
  │     → socket->setsockopt(map_common_option(option), ...)
  ├─ as_gateway_service(handle) → gateway?
  │     → 지원 옵션만 → gateway->set_socket_option(legacy_opt, ...)
  ├─ as_discovery_service(handle) → discovery?
  │     → managed socket set에 fan-out 적용
  ├─ as_spot_pub_handle(handle) / as_spot_sub_handle(handle) → EINVAL
  ├─ as_spotnode_pub_handle(handle) / as_spotnode_sub_handle(handle) → EINVAL
  └─ 그 외 → EFAULT

zlink_set_pub_option(handle, option, val, len)
  ├─ option 범위 판별: pub 전용(0x33xx) 또는 공통(0x30xx)
  ├─ as_socket_handle(handle) → PUB/XPUB 타입 검증 → setsockopt
  ├─ as_spot_pub_handle(handle) → spot-pub live socket에 직접 적용
  ├─ as_spotnode_pub_handle(handle) → 자체 pub socket에 직접 적용
  └─ 그 외 → EFAULT
...
zlink_set_routing_id(handle, data, size)
  ...
  ├─ as_spot_pub_handle(handle) → 해당 pub handle에 직접 설정
  ├─ as_spot_sub_handle(handle) → 해당 sub handle에 직접 설정
  ├─ as_spotnode_pub_handle(handle) → 해당 node-pub handle에 직접 설정
  ├─ as_spotnode_sub_handle(handle) → 해당 node-sub handle에 직접 설정
  └─ 그 외 → EFAULT
```
...
pub/sub 함수는 `int option_` 파라미터로 두 범위를 모두 받는다:
- **0x30xx** (`zlink_option_t`) — `map_common_option()`으로 변환
- **0x33xx** (`zlink_pub_option_t`) / **0x34xx** (`zlink_sub_option_t`) — `map_pub_option()` / `map_sub_option()`으로 변환

```c
static int map_pub_or_common_option (int option_)
{
    if ((option_ & 0xFF00) == 0x3300)
        return map_pub_option ((zlink_pub_option_t) option_);
    if ((option_ & 0xF000) == 0x3000)
        return map_common_option ((zlink_option_t) option_);
    errno = EINVAL;
    return -1;
}
```
...
```c
int zlink_set_pub_option (void *handle_, int option_, ...)
{
    int legacy = map_pub_or_common_option (option_);
    ...
    /* 2. spot-pub */
    spot_pub_handle_t *spot_pub = as_spot_pub_handle (handle_);
    if (spot_pub)
        return spot_pub->impl->set_option (legacy, optval_, optvallen_);

    /* 3. spotnode-pub */
    spotnode_pub_handle_t *node_pub = as_spotnode_pub_handle (handle_);
    if (node_pub)
        return node_pub->impl->set_option (legacy, optval_, optvallen_);
...
| spot-pub set/get | `spot_pub_t::set_option()` / live getter | 기존 기반, side handle 직접 연결 |
| spot-sub set/get (정수 옵션) | `spot_sub_t::set_option()` / live getter | 기존 기반, side handle 직접 연결 |
| spot-sub set (SUBSCRIBE/UNSUBSCRIBE) | `spot_sub_t::subscribe()` / `unsubscribe()` | 기존 경로 재사용 |
| spot-pub/sub routing_id | side handle own routing_id 경로 | 신규 public routing 연결 |
| spotnode-pub set/get | `spot_node_t::set_pub_option()` / live getter | 기존, 자체 socket 직접 적용 |
| spotnode-sub set/get (정수) | `spot_node_t::set_sub_option()` / live getter | 기존, 자체 socket 직접 적용 |
| spotnode-sub set (SUBSCRIBE) | `spot_node_t` 자체 sub socket에 subscribe | 기존 경로 재사용 |
| spotnode-pub/sub routing_id | side handle own routing_id 경로 | 신규 public routing 연결 |
...
- SUBSCRIBE/UNSUBSCRIBE: 문자열 filter 전용 경로
  - spot-sub: `spot_sub->subscribe(val, len)` / `unsubscribe(...)`
  - spotnode-sub: 자체 sub socket에 직접 subscribe / unsubscribe
...
| `spot-pub/sub` side handle | side-handle tag/감지 경로 | unified handle 제거 후 side 직접 분기 |
| `spotnode-pub/sub` side handle | side-handle tag/감지 경로 | unified handle 제거 후 side 직접 분기 |
...
| `zlink_subscribe(subject, filter)` | `zlink_set_sub_option(subject, ZLINK_SUB_OPT_SUBSCRIBE, filter, len)` |
| `zlink_unsubscribe(subject, filter)` | `zlink_set_sub_option(subject, ZLINK_SUB_OPT_UNSUBSCRIBE, filter, len)` |
...
| unified `spot` / `spot_node` public API 전체 (§3.5.1) | side별 public API로 분리 |
| `zlink_spot_set_pub_option(spot, opt, ...)` | `zlink_set_pub_option(spot_pub, ...)` |
| `zlink_spot_set_sub_option(spot, opt, ...)` | `zlink_set_sub_option(spot_sub, ...)` |
| `zlink_spot_node_set_pub_option(node, opt, ...)` | `zlink_set_pub_option(node_pub, ...)` |
| `zlink_spot_node_set_sub_option(node, opt, ...)` | `zlink_set_sub_option(node_sub, ...)` |
...
| --- | --- |
| `zlink_set_subscribe(subject, filter)` | `zlink_set_sub_option(subject, ZLINK_SUB_OPT_SUBSCRIBE, filter, len)` |
| `zlink_unset_subscribe(subject, filter)` | `zlink_set_sub_option(subject, ZLINK_SUB_OPT_UNSUBSCRIBE, filter, len)` |

유지되는 항목: `zlink_subscribe_recv` → `zlink_subscribe` 이름 변경,
...
- [`zlink.h`](core/include/zlink.h)
  - 새 enum 6개 추가 (§3.2)
  - 새 함수 선언 15개 추가 (§3.1)
  - unified `spot` / `spot_node` API 분리 반영
...
- side handle 감지/helper — `spot-pub`, `spot-sub`, `spotnode-pub`, `spotnode-sub`
  direct dispatch 지원 (**신규**)
- 서비스별 TLS apply helper — gateway/discovery/spot-pub/sub/spotnode-pub/sub/registry 공통 진입점 지원 (**신규/확장**)
- `spot_node_t` — side별 discovery/peer/bind 분리 helper (**신규/확장**)
...
### Phase 4. spot-pub / spot-sub surface 분리

목표: unified `spot`를 제거하고 side별 public handle로 전환한다.

작업:
- `spot-pub`, `spot-sub` side handle 도입
- `zlink_set_pub_option()` / `zlink_get_pub_option()`에 spot-pub 핸들 분기 추가
- `zlink_set_sub_option()` / `zlink_get_sub_option()`에 spot-sub 핸들 분기 추가
- SUBSCRIBE/UNSUBSCRIBE는 spot-sub live socket에 직접 연결
- `zlink_set_routing_id(spot_pub, ...)` / `zlink_set_routing_id(spot_sub, ...)`
  직접 경로 추가
- unified `zlink_spot_*` API를 삭제/분리 대상으로 이관

완료 기준:
- `zlink_set_pub_option(spot_pub, ZLINK_PUB_OPT_NODROP, ...)` 동작
- `zlink_set_pub_option(spot_pub, ZLINK_OPT_SNDHWM, ...)` 동작
- `zlink_set_sub_option(spot_sub, ZLINK_OPT_RCVHWM, ...)` 동작
- `zlink_set_sub_option(spot_sub, ZLINK_SUB_OPT_SUBSCRIBE, ...)` live sub socket에 직접 적용
- `zlink_set_routing_id(spot_pub, ...)` / `zlink_set_routing_id(spot_sub, ...)` 동작
- unified `zlink_spot_*` API 삭제/대체 계획이 문서와 코드에 반영됨
- `core/` 빌드 성공

### Phase 5. spotnode-pub / spotnode-sub surface 분리

목표: unified `spot_node`를 제거하고 side별 public handle로 전환한다.

작업:
- `spotnode-pub`, `spotnode-sub` side handle 도입
- `zlink_set_pub_option()` / `zlink_get_pub_option()`에 spotnode-pub 핸들 분기
- `zlink_set_sub_option()` / `zlink_get_sub_option()`에 spotnode-sub 핸들 분기
- SUBSCRIBE/UNSUBSCRIBE는 spotnode-sub live socket에 직접 연결
- `zlink_set_routing_id(node_pub, ...)` / `zlink_set_routing_id(node_sub, ...)`
  직접 경로 추가
- `zlink_spot_node_*` unified API를 삭제/분리 대상으로 이관
- `attach_discovery`, `connect_peer_pub`, `bind`, TLS 설정, monitor API의 side별 분리 반영
- `zlink_set_tls_server()` / `zlink_set_tls_client()`의 spotnode side handle 분기 반영

완료 기준:
- `zlink_set_pub_option(node_pub, ZLINK_OPT_SNDHWM, ...)` 자체 pub socket에 적용
- `zlink_get_pub_option(node_pub, ZLINK_OPT_SNDHWM, ...)` 자체 pub socket에서 읽음
- `zlink_set_sub_option(node_sub, ZLINK_SUB_OPT_SUBSCRIBE, ...)` 자체 sub socket에 subscribe
- child `spot` 생성 시 node option/filter가 자동 복제되지 않음
- `zlink_set_routing_id(node_pub, ...)` / `zlink_set_routing_id(node_sub, ...)` 동작
- `zlink_set_tls_server(node_pub, ...)` / `zlink_set_tls_client(node_sub, ...)` 동작
- unified `zlink_spot_node_*` API 삭제/대체 계획이 문서와 코드에 반영됨
- `core/` 빌드 성공
...
- `core/tests/unittest/unittest_typed_option.cpp` 신규 생성:
  ...
  - `test_routing_id_spot_side`: spot-pub/spot-sub side별 routing_id 검증
  - `test_router_option_type_check`: ROUTER/DEALER/gateway router 옵션
  - `test_tls_dedicated_api`: gateway/discovery/spot-pub/sub/spotnode-pub/sub/registry TLS 전용 함수 동작
  - `test_stream_option_set_get`: STREAM set/get 대칭
  - `test_raw_pub_sub_common_getter`: raw PUB/XPUB, SUB/XSUB에서 side 공통 getter 동작
  - `test_spot_pub_option`: spot-pub side 옵션 (전용 + 공통)
  - `test_spot_sub_option`: spot-sub side 옵션 (전용 + 공통)
  - `test_spot_sub_subscribe_live`: spot-sub live socket subscribe 직접 적용
  - `test_spotnode_pub_option`: spotnode-pub 자체 pub socket 옵션 set/get
  - `test_spotnode_sub_option`: spotnode-sub 자체 sub socket 옵션 set/get
  - `test_spotnode_subscribe`: spotnode-sub 자체 sub socket subscribe
  - `test_spotnode_child_independent_options`: child spot 생성 시 node option/filter 비복제 확인
  - `test_unified_spot_api_removed`: unified `zlink_spot_*` / `zlink_spot_node_*` 제거 검증
...
| side handle 감지 helper | `core/src/api/zlink_option.cpp` | spot-pub/sub, spotnode-pub/sub 핸들 감지 |
...
| `spot_node_t::set_pub_option()` | `core/src/services/spot/spot_node.cpp` | spotnode-pub 자체 pub socket 옵션 |
| `spot_node_t::set_sub_option()` | `core/src/services/spot/spot_node.cpp` | spotnode-sub 자체 sub socket 옵션 |

## 10. 수정 대상 파일 요약

| 파일 | 변경 | Phase |
| --- | --- | --- |
| `core/include/zlink.h` | enum 6개 추가, 함수 15개 추가, unified spot/spot_node API 분리 반영, legacy enum/function/alias 삭제 | 1-2, 7 |
...
| `core/src/services/spot/spot_node.cpp` | side별 public surface 연결, discovery/peer/bind 분리, TLS 전용 함수 분기 반영 | 5 |
| `core/src/services/spot/spot_node.hpp` | side별 helper 선언 추가 | 5 |
...
- unified `spot` / `spot_node` public handle이 제거되었다.
- `zlink_set_pub_option(spot_pub, ...)`이 spot-pub side에 적용된다.
- `zlink_set_sub_option(spot_sub, ...)`이 spot-sub side에 적용된다.
- `zlink_set_pub_option(node_pub, ...)`이 spotnode-pub side에 적용된다.
- `zlink_set_sub_option(node_sub, ...)`이 spotnode-sub side에 적용된다.
- §3.3.5의 side별 허용 공통 옵션이 `set_pub_option`/`set_sub_option`으로 설정된다.
- `zlink_get_option(socket/gateway, ZLINK_OPT_LAST_ENDPOINT, ...)`이 동작한다.
- `zlink_set_routing_id`가 socket/gateway/discovery/spot-pub/sub/spotnode-pub/sub에서 동작한다.
- `zlink_get_routing_id`가 각 side handle의 own routing_id를 반환한다.
- `zlink_set_tls_server` / `zlink_set_tls_client`가 gateway/discovery/spot-pub/sub/spotnode-pub/sub/registry에서 동작한다.
...
- spot-pub/sub, spotnode-pub/sub side handle dispatch 구현 완료.
- spotnode-pub/sub 자체 socket 옵션 set/get 동작 확인.
- child spot 생성 시 node option/filter 비복제 동작 확인.
- 기존 unified `zlink_spot_*` / `zlink_spot_node_*` API가 side별 API로 대체되거나 제거되었다.
