...
- [x] **E2E-ND-07** (결함) — e2e 클라이언트가 **HTTP client wrapper를 안 쓴다** — 전부 raw `fetch`
...
- [x] **E2E-ND-40** (결함) — `PS-A3`가 subscriber `ConnectionReady` 대신 HTTP health와 반복 publish에 의존하고, `PS-A4`는 transport fault 대신 subscriber process를 재시작하며, `PS-B2`는 terminal `Drained`·old row 제거·재연결 readiness 없이 고정 sleep과 반복 publish로 복구를 판정한다.
  - 근거: 실제 TCP fault proxy와 subscriber socket event evidence를 추가해 PS-A3는 ready 전/후 event를 한 번씩, PS-A4는 같은 process의 disconnect·자동 subscription 재적용·replay 부재를 검증하고, PS-B2는 terminal `drained`·publisher row 제거·같은 rid/endpoint 재등록·`ConnectionReady` 뒤 첫 event를 완료 조건으로 삼았다. 기존 PS-B2는 fanout 위치 행에 `nodeRid`가 없어 `log/20260716-103913-3528109`에서 실패했고 이를 보존하는 host 계약 테스트도 추가했으며, 수정 뒤 집중 테스트 6/6과 PubSub 전체 7/7이 `log/20260716-105224-3576369`에서 통과했다. transport fault와 관측 대기를 각각 전용 helper에 가두고 반복 publish·sleep을 제거했다. 커밋 `87d8ad7a7`.
- [x] **E2E-ND-41** (결함) — `SM-G2`가 두 SpotNode를 모두 미리 시작한 뒤 각 node HTTP API로 Spot을 하나씩 만들 뿐, 실제 node 추가·기존 owner 유지·새 Entry Spot/capability readiness를 검증하지 않는다.
  - 근거: SM-G2 runner를 node A 준비와 node B 추가의 두 단계로 나누고, A에 기존 Spot·actor를 만든 뒤에만 B를 시작한다. A의 역할 server가 Redis peer의 B actor capability와 B Entry Spot `SpotHandle` resolve를 bounded wait로 확인한 다음 기존 Spot·actor owner가 A인지 재검증하고, B의 로컬 manager와 Entry Spot handler로 새 Spot·actor를 만든다. 두 node 선기동을 금지하는 집중 게이트는 수정 전 실패에서 1/1 통과로 바뀌었고 실제 SM-G2와 공유 MultiNode 회귀 SM-F6가 각각 `log/20260716-110658-3617643`, `log/20260716-110725-3619470`에서 통과했다. 배포 순서는 runner, readiness 지식은 역할 server, 업무 검증은 시나리오에 두어 책임을 분리했다. 커밋 `88cfff5e3`.
- [x] **E2E-ND-42** (결함) — `MON-A4`가 정상 replacement와 socket weight 변경만 실행하고 provider `SIGKILL`·owner lease 만료·남은 provider follow-up을 별도 failover 단계로 검증하지 않으며 weight 변경을 drain으로 표현한다.
  - 근거: MON-A4를 terminal `Drained`와 old row 제거를 확인하는 정상 replacement, `SIGKILL` 뒤 owner lease 만료·`Disconnected`·남은 provider 후속 요청을 확인하는 crash failover, `/admin/exclude`의 weight 0과 `PeerAdmissionChanged`를 확인하는 부하 제외 단계로 분리했다. 필수 단계가 없으면 실패하는 집중 게이트가 수정 전 실패에서 2/2 통과로 바뀌었고 RuntimeMonitoring 전체 9개 시나리오가 `log/20260716-113646-3682839`에서 정상 종료까지 통과했다. 역할별 재시작 경로와 managed process 소유권을 한곳에 모으고 runner 포트 중복을 제거했다. 커밋 `737d5d2ad`.
...
- [x] **E2E-ND-36** (결함) — framework host와 복잡한 standalone client가 role 설정을 개별 CLI option과 Node 전용 경로로 전달하고, browser client가 정적 설정 파일을 읽지 않는다.
  - 근거: 모든 host를 정확히 `--config <path>`만 받는 `ConfigModule` typed provider와 `ZLinkModule.forRootFactory(...)` 주입으로 통일하고, topology·파일 경로가 필요한 client는 typed 설정 파일, browser client는 `/config.json`을 사용하게 했다. SpotService의 개별 CLI와 browser의 파일 직접 읽기 때문에 실패하던 집중 게이트가 8/8 통과로 바뀌고 Node build·typecheck, 관련 11개 대표 E2E가 통과했다. 후속 재검토에서는 SpotService 하나만 보던 host-launch 단언을 Server가 있는 모든 Node E2E runner의 실제 shell 명령으로 확장해, runner의 설정 파일 생성 인자와 host의 `--config` 인자를 구분하면서 전수 9/9를 확인했다. 임시 설정·TLS 파일은 `0600`, 디렉터리는 `0700`으로 만들고 정상·실패 종료에서 정리한다. 커밋 `06486c2af`, `22565a281`.
- [x] **SMP-ND-21** (결함) — 하나의 공통 runner가 sample 이름으로 여섯 실행 절차를 분기하고 여러 server role에 같은 전체 topology 설정을 전달한다.
  - 근거: 절차를 sample별 `Runner/sample-runner.mjs`로 옮기고 공통 runner에는 process·Redis·readiness·정리만 남겼으며, 각 host에는 필요한 항목만 담은 role 설정을 전달한다. 범용 runner 분기를 검출하도록 먼저 확장한 집중 게이트가 실패에서 통과로 바뀌고 여섯 sample runner가 통과했다. 재검증 중 드러난 SupportChat의 명시적 close와 close-grace 종료 책임 혼합도 분리해 집중 lifecycle test가 실패에서 3/3 통과로 바뀌었다. sample별 절차를 자료 테이블로 압축하는 안보다 변경 지식을 각 sample에 가두는 모듈 분리를 선택해 공통 runner의 조건부 지식과 lifecycle pass-through를 제거했다. 추가 재검토에서는 여섯 PowerShell wrapper가 여전히 sample 이름을 넘겨 집중 설정 게이트가 실패하는 것을 확인한 뒤 모두 sample별 runner 경로를 넘기도록 고쳤고, 해당 게이트와 여섯 sample 자체 검증 및 Node 전체 `npm test`가 통과했다. 커밋 `06486c2af`, `71cc0b17e`, `dc43ed761`.
- [x] **SMP-ND-22** (미구현) — Bingo API·Play·Session runtime이 역할별 allocation group을 공유하지 않고 고정 routing id 설정에 의존해 `BINGO-RID-1`부터 `BINGO-RID-5`까지 검증할 수 없다.
  - 근거: API·Play·Session을 각각 `bingo.api`·`bingo.play`·`bingo.session` allocation group으로 전환하고 Play channel과 room Spot mesh가 같은 `playN`을 공유하게 했으며, Session은 자신의 slot에서 preferred `playN`을 계산해 public Spot handle로 actor를 만든다. Play channel의 부하 분산 대상이 preferred owner와 달라도 Entry Spot에 위임해 room owner와 실제 SpotNode RID를 일치시켰다. 고정 RID 설정을 제거하고 runner가 역할별 역순 시작, replacement의 `WaitingForSlot`, 정상 release 뒤 slot 1과 generation `1 → 2`, 기존 browser·lifecycle 시나리오를 검증한다. API 부재로 0/4 실패하던 집중 게이트를 확장해 5/5, 관련 Bingo 계약 게이트 8/8, typecheck·lint와 실제 `PASS Bingo.Ts`가 통과했다. allocation 보고 형식과 room provisioning을 각각 한 모듈에 모으고 handoff 절차를 runner의 별도 함수로 분리해 역할 main과 시나리오 본문의 시간적 복잡성을 줄였다. 커밋 `5e8ac025f`.
- [ ] **SMP-ND-23** (미구현) — 공통 ZoneWorld 샘플과 `ZW-G1`부터 `ZW-G5`까지의 자동 routing id·handoff 검증에 대응하는 Node 샘플이 없다.
