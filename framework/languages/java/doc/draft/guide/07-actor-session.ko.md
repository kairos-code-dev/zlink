...
# Draft -- Java Actor/Session Guide

> 이 문서는 **구현 전 초안**이다.
> 자세한 계약은 [spring-boot-actor-session](../spring-boot-actor-session.ko.md)이
> 소유한다. 이 챕터는 [06-spot](./06-spot.ko.md)을 먼저 읽었다고 가정한다.
...
[spring-boot-actor-session](../spring-boot-actor-session.ko.md)과
[stream 샘플](../stream-samples.ko.md)이 소유한다.
...
- 전체 계약: [spring-boot-actor-session](../spring-boot-actor-session.ko.md)
