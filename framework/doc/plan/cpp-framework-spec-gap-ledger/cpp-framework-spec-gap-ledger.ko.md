...
- 기준 시점: 2026-08-03 20:27 KST. Core `core/build`와 canonical `framework/languages/cpp/build`를 다시 빌드하고, Core duplicate-routing regression, C++ runtime unit, sample contract, full serial CTest, 6개 sample process와 bounded common E2E를 trace/file log로 재검증했다. PubSub fanout missing-handler runtime gap과 Core STREAM listener stop gap은 owner-layer unittest·process file trace로 추가 확인했다. 현재 full serial CTest는 56/56, shell·PowerShell sample aggregate는 각각 6/6, packaged clean consumer는 PASS다. RuntimeMonitoring source porting은 current exact C++ interface 기준으로 A1/A2 process가 통과했지만 A3 fixture/contract 조건과 A4 physical peer removal은 아직 닫히지 않았다. 20:25~20:27 KST에 A4 physical disconnect를 owner layer에서 재현했으나 monitor event의 peer RID·value 의미를 잘못 해석하는 추가 blocker가 확인되어 임시 patch와 regression은 제거하고 이 상태를 pause checkpoint로 기록했다.
- 판정: 미완료. 현재 C++ Framework는 common Framework spec과 common E2E spec을 전체 충족한다고 판정할 수 없다.
- 변경 범위: 이 문서를 C++ Framework spec과 sample을 함께 관리하는 유일한 ledger로 유지한다. 이번 갱신에서는 bilateral admission race와 Core standby RID 정규화를 owner layer에서 보강하고, SupportChat/GameQuest sample contract와 nullable field를 공통 문서·contract test에 맞추고, 여섯 runner의 bounded cleanup을 정렬했다. 추가로 fanout pump가 missing-handler publish 실패를 기존 dispatch error/message-flow reporter로 기록하도록 보강하고, Core STREAM listener의 cross-thread poll stop 관찰을 bounded interval로 고정했으며, stream connector lifecycle callback lifetime을 close 경계에서 owner runtime이 관리하도록 정렬했다. TicTacToe와 Bingo runner는 application stdout과 diagnostic trace stderr를 별도 file log로 보존하고 asynchronous cleanup marker를 bounded하게 기다린다. PubSub settle/readiness와 location option fixture, E2E build-directory override도 정렬했다. ClientServer topology를 `location_runtime_query_t`로 읽던 stale E2E 경로는 current `client_server_runtime_t` 기준으로 이관을 시작했지만 automatic RID·endpoint contract와 일부 config의 retired API는 blocker로 남겼다. RuntimeMonitoring은 current `route_mesh_runtime_t`, `spot_manager_t`, `route_message_context_t`와 current builder로 A1/A2 source porting을 진행했으며, A3/A4의 runtime/fixture 조건은 실패 evidence를 유지한다. unittest·sample·aggregate·package 결과와 남은 contract·package 조건을 이 문서와 진행 log에 기록한다. process 검증은 `LD_LIBRARY_PATH=core/build/lib`로 최신 Core를 강제한 source-runtime evidence이며 formal local package provenance와 분리한다. 기존 사용자 변경은 수정하지 않는다.
...
이 문서의 C++ 작업은 `.NET`, Java/Kotlin, Node.js 작업과 동시에 진행한다. 기준 마감
`2026-08-03 10:00 KST (+09:00)`은 지났고, 현재 시각 `2026-08-03 19:49 KST (+09:00)`에도
미완료 항목이 남아 있다. 완료하지 못한 항목은 완료로 표시하지 않고, 현재 조건과 blocker를
기록한 뒤 다음 작업으로 이관한다.
...
현재 runtime 작업으로 HTTP buffer access, Boost include order, mesh admission, submit retry, worker scheduling·shutdown, worker options, owner-managed deadline, blocking request offload, ClientServer hello readiness와 sample process 경로를 수정했다. M6A liveness regression의 logical-clock 사용도 정렬했고, HWM exact-boundary와 application mailbox의 한 dispatch turn 한 건 제한, STREAM reply-token terminal-consumption 회귀를 추가했다. 이번 단계에서는 수신 측 bounded admission queue와 Core ROUTER standby RID 정규화를 owner layer에 추가하고, SupportChat의 public extra field와 GameQuest의 spec 밖 response/action message를 제거했으며, Delivery null timestamp와 ShoppingMall nullable `OrderState`를 fixture로 고정했다. Core STREAM listener stop의 bounded poll regression과 stream connector lifecycle callback lifetime regression도 owner-layer unittest/ASan으로 고정했다. Core regression 3/3, focused C++ runtime/unit, full serial CTest 56/56, shell·PowerShell sample aggregate 6/6과 RM-C2 10/10을 확인했다. exact public contract, common E2E 전체 분모, formal package ownership/ABI, CI와 S1 exact inventory는 아직 닫히지 않았으므로 이 ledger 전체를 완료로 판정하지 않는다.
...
- Core router source를 수정한 뒤 `core/build/lib/libzlink.so`를 formal install prefix에 다시 배포하지 않았으므로, 이번 2026-08-03 sample와 common process evidence는 `LD_LIBRARY_PATH=core/build/lib`로 최신 source runtime을 강제했다. 기존 `.artifacts/wsl/install/zlink-core/11.1.0`과 `zlink-cpp/11.1.0`의 clean consumer 결과는 이전 package evidence이며 이번 Core fix의 package closure가 아니다. Candidate manifest와 독립 review evidence가 준비되기 전에는 이를 새 package PASS로 기록하지 않는다.
...
| `cmake --build core/build -j2` + Core routed regression CTest | build exit 0, 3/3 passed | `test_router_handover`, `test_zmp_request_reply_router_recv_surface`, `test_router_concurrent_routed_recv`에서 Core duplicate ROUTER의 standby pipe가 `zlink_router_recv`에 internal 5-byte RID 대신 stable node RID를 반환하고, promoted standby request reply와 기존 routed/concurrent surface가 유지되는지 확인했다. |
| `ctest --test-dir framework/languages/cpp/build -R 'm6a|m6b|m6c|vertical|client_server_runtime' -j1` | 5/5 passed, exit 0 | receiver-side bounded admission queue와 Core stable RID 정규화 이후 bilateral request race, relocation/vertical direct-pump, ClientServer runtime regression을 확인했다. |
| `ctest --test-dir framework/languages/cpp/build --output-on-failure --timeout 120 -j1` | 56/56 passed, exit 0 | 최신 Core runtime을 `LD_LIBRARY_PATH=core/build/lib`로 사용해 framework unit, contract, package consumer, connector와 6개 sample smoke를 포함한 serial gate를 확인했다. 총 103.51초이며 log는 `/tmp/zlink-cpp-full-ctest-runtime-gap-final-20260803.log`다. |
| Core STREAM stop unittest + stream connector lifecycle regression | 1/1 + 1/1 passed, exit 0 | `test_cpp_framework_stream_framework`가 실제 Core STREAM listener stop을 2초 이내로 확인했고, `test_cpp_stream_connector` focused CTest와 bounded ASan run이 stale lifecycle callback을 재현하지 않았다. logs는 `/tmp/zlink-cpp-stream-stop-unit-final-20260803.log`, `/tmp/zlink-cpp-stream-ctest-lifetime-20260803.log`, `/tmp/zlink-cpp-stream-asan-lifetime-20260803.log`다. |
...
| runtime terminal mapping regression: `test_cpp_framework_messaging` + `test_cpp_framework_store_location_resolvers` | executable exit 0, CTest 1/1 passed, exit 0 | `not_admitted → rejected`, `backpressured → capacity_exceeded`, `terminated/shutdown → shutting_down`, invalid submit → `invalid_operation`을 공통 internal mapper와 User Spot mapper에서 확인했다. `not_connected`, timeout, shutdown은 `framework_exception_t::code()`의 `std::errc`와 함께 확인했다. |
| Actor identity boundary regression: `test_cpp_framework_actor_gateway` | executable exit 0, 관련 CTest 3/3 passed | `actor_id_t`의 UTF-8·1..255 byte·malformed sequence 경계와 `actor_ref_t`의 `ObjectGeneration=1..INT64_MAX` 경계를 생성 시점에 확인했다. 유효한 Unicode byte sequence는 변경하지 않고 보존한다. |
| 최신 6개 sample individual/aggregate 실행 | 6/6 passed, exit 0 | `TicTacToe`, `Bingo`, `DeliveryDispatch`, `SupportChat`, `GameQuest`, `ShoppingMall`의 public client result, role evidence, message trace/file log와 bounded cleanup을 확인했다. TicTacToe/Bingo는 application stdout과 diagnostic trace stderr를 분리하고 asynchronous cleanup marker를 bounded하게 기다린다. runner aggregate log는 `/tmp/zlink-cpp-official-sample-aggregate-runtime-gap-final3-20260803.log`다. |
| PowerShell sample aggregate | 6/6 passed, exit 0 | `/usr/bin/pwsh`에서 동일한 6개 selector와 결과 marker를 실행했다. log는 `/tmp/zlink-cpp-powershell-sample-aggregate-runtime-gap-final-20260803.log`이며 Windows host/package provenance와 구분한다. |
| `framework/languages/cpp/scripts/verify_packaged_contract.sh framework/languages/cpp/build` | `PASS`, exit 0 | installed library 3개 hash, 111 headers, required/forbidden manifest와 clean consumer를 확인했다. log는 `/tmp/zlink-cpp-package-contract-runtime-gap-final-20260803.log`이며 HTTP ownership과 full ABI matrix는 여전히 gap이다. |
| `LD_LIBRARY_PATH=core/build/lib timeout -k 10s 1800s ./run_e2e_all.sh --scenario-timeout-seconds=180` | exit 2, 128초 | 현재 runner가 선택하는 12-config 집합에서 RegistrationCodec과 RegistryMessaging RM-A1~C7, RM-C8 payload round-trip이 통과했다. RM-C8 max-size subflow는 exact C++ public interface에 없는 ClientServer listener `MaxMessageSize` 요구로 terminal contract blocker가 됐다. 로그는 `/tmp/zlink-cpp-common-aggregate-contract-20260803.log`, C8 run은 `logs/20260803-182134-3354686`이다. |
| `PubSub/run_e2e.sh all` + `test_cpp_framework_message_flow` | PubSub 6/6, unittest 1/1 passed | fanout pump가 `dispatch_send()`의 missing-handler 결과를 버리던 runtime gap을 owner layer의 `dispatch_error_reporter_t` 호출로 보완했다. `PS-C1` subscriber flow file에 `surface=channel kind=publish reason=handler_missing action=drop`가 기록됐다. log는 `framework/languages/cpp/e2e/PubSub/logs/20260803-184845-3467452`, `/tmp/zlink-cpp-message-flow-unit-20260803.log`다. |
| DiscoveryRegistryHa·ResilienceLifecycle·RuntimeMonitoring current-surface recheck | 미완료 | DiscoveryRegistryHa는 Redis `client-server` descriptor와 lease는 존재하지만 `location_runtime_query_t::list_topology()`가 MeshNode만 반환해 stale E2E query가 빈 row를 받는다. ResilienceLifecycle은 `client_server_runtime_t::snapshot()` 이관 뒤에도 automatic server RID가 `api-a/api-b`와 일치하지 않고 endpoint가 exact snapshot에 없어 stable topology assertion이 닫히지 않는다. RuntimeMonitoring은 current interface로 service/client target build와 A1/A2 process porting까지 진행했지만 A3는 여러 Server peer를 포함한 fixture에서 weight 0 target count가 수렴하지 않고, A4는 service stop 뒤 peer readiness가 false로 수렴하지 않는다. A4 재현 trace에서 `DISCONNECTED value=4`는 disconnect reason이고 뒤의 `CONNECTION_READY value=0`은 current ready count로 확인됐지만, 현재 owner는 이를 physical connection id·stable peer removal로 사용할 수 없다. 임시 owner patch와 regression은 실패 후 제거했다. A5/C1/D1은 retired snapshot field를 더 port해야 한다. 상세 log와 판단은 진행 log 15·17·18절에 있다. |
| `ZLINK_CPP_MESH_TRACE=1 timeout -k 5s 120s RegistryMessaging/run_e2e.sh RM-A3 --start-order=forward` | exit 0, log `20260803-101440-667730` | automatic/manual `NotRequired` 80회 안정성, node direct `NotFound`, manual logical intent 1개당 Core Application/Completion transport lane 2개, weight-zero Server required 전환과 cleanup을 확인했다. trace stderr file log는 `hello`·`admit` control record를 기록했으며, 이 targeted result는 14-config aggregate 완료를 대신하지 않는다. |
| `LD_LIBRARY_PATH=core/build/lib` RM-C2 반복 | 10/10 passed, exit 0 | 각 실행은 dedicated Redis와 role process를 사용했고 log directory를 보존했다. 최신 trace에서 `kind=17` request가 `source=api-a`로 dispatch되며 `peer-not-admitted` drop 없이 completion된다. logs는 `framework/languages/cpp/e2e/RegistryMessaging/logs/20260803-174825-3194383`부터 `20260803-174906-3199704` 범위다. |
| `LD_LIBRARY_PATH=core/build/lib` 6개 sample aggregate | 6/6 passed, exit 0 | TicTacToe, Bingo, DeliveryDispatch, SupportChat, GameQuest, ShoppingMall의 client self-check, role evidence, message trace/file log와 cleanup이 모두 통과했다. 결과 log는 `/tmp/zlink-cpp-official-sample-aggregate-runtime-gap-final3-20260803.log`다. |

현재 canonical evidence에서는 최신 Core owner regression, C++ focused runtime 5/5, full serial CTest
56/56와 6개 sample process 6/6이 통과했다. 다만 이 process 결과는 `LD_LIBRARY_PATH=core/build/lib`로
현재 source runtime을 사용한 결과이며, formal Core/bindings local package provenance를 닫은 결과가
아니다. common aggregate는 14개 config/374개 ID 전체가 아니고, 현재 12-config runner도 RM-C8 contract
blocker에서 terminal failure가 발생했다. PubSub의 fanout negative-dispatch runtime gap은 별도 unittest와
실제 subscriber file trace에서 닫혔지만, 다른 E2E source/contract blocker와 package 조건은 남아 있다.
따라서 runtime과 sample process의 현재 green 결과를 S0/S1
closure로 승격하지 않는다.
...
- 확인한 동작과 기대 동작: 현재 `actor_client_t`의 global `actor_id_t` 기반 `send`·`request`, `actor_request_call_t`의 metadata·`submit`·`yield`, `actor_create_call_t`와 `spot_create_call_t`의 move-only call·`submit`·`yield`, `session_message_context_t`와 STREAM의 routing/address accessor는 runtime source와 header에 연결되어 있다. 이번 단계에서 `actor_id_t`는 exact가 요구하는 UTF-8 `1..255` byte 검사를 수행하고, `actor_ref_t`는 `ObjectGeneration=1..INT64_MAX`을 생성 시점에 검증하도록 보강했다. 그러나 `actor_id_t`의 default/implicit compatibility conversion, `actor_ref_t`의 legacy constructor·`actor_ref_snapshot_t`, Spot/Actor manager의 concrete value surface와 exact interface의 virtual operation 차이, `spot_context_t::publish`와 `spot_publisher_client_t`의 owner 차이는 남아 있다. `actor_context_t::actor_id()`는 현재 `const actor_id_t &`이고 STREAM callback은 `session_message_context_t`를 사용하므로 이 두 항목은 현재 gap으로 계산하지 않는다.
- 판정 근거: identity construction 경계는 `test_cpp_framework_actor_gateway`와 관련 CTest 3/3에서 확인했지만, source에 관련 class와 method가 있다는 사실만으로 parameter, const/reference/value, move/copy, virtual/lifetime 계약의 전체 일치를 증명하지 못한다. 현재 public header를 exact interface 충족으로 세지 않는다.
...
- C++ 경로: `framework/languages/cpp/framework/include/zlink/framework/contracts/errors/error.hpp:13-177`, `framework/languages/cpp/framework/src/runtime/messaging/request_failure_mapper.cpp`, `framework/languages/cpp/framework/src/runtime/messaging/submit_result_mapper.hpp`, `framework/languages/cpp/framework/src/runtime/mesh/user_spot_terminal_mapping.hpp`, `framework/languages/cpp/framework/src/runtime/messaging/async_submit_runtime.cpp:139-140,298-299`.
- 확인한 동작과 기대 동작: exact C++ interface가 정의한 13개 `framework_error_kind_t` 값과 현재 public header의 enum 이름·numeric value는 일치한다. 현재 header의 `detail::boundary_error_t`와 `failure_origin_t`는 private detail이며 public error kind의 추가 enumerator로 계산하지 않는다. runtime에는 Core submit result를 Framework kind로 바꾸는 공통 internal mapper가 추가되어 `not_admitted`를 `rejected`, backpressure를 `capacity_exceeded`, terminated를 `shutting_down`, invalid submit을 `invalid_operation`으로 변환한다. C++ request enum에는 `backpressured`가 없으므로 wire terminal `113`은 reply mapper에서 `capacity_exceeded`로 처리한다. request failure mapper는 route 단절·timeout·shutdown의 kind와 `std::error_code`를 함께 보존하고, User Spot operation mapper도 cancelled/shutdown을 각각 `invalid_operation`/`shutting_down`으로 변환한다. 기대 동작은 이 mapping이 모든 operation family와 installed package에서 같은 결과를 내는 것이다.
- 판정 근거: `test_cpp_framework_messaging` executable과 `test_cpp_framework_store_location_resolvers`가 위 mapping과 boundary `code()`를 통과했고, 최신 canonical full serial CTest 56/56에도 포함됐다. 그러나 actor/spot/stream/worker/submit의 모든 operation family에서 `kind()`, `code()`, exception과 callback terminal reason이 exact table대로 전달되는지 process evidence가 없고, clean installed package에서 numeric ABI와 full terminal matrix를 확인하는 probe도 남아 있다.
- 수정 목록: 내부 세부 원인은 private mapping에 두고 public `kind()`와 optional diagnostic `code()`를 분리한다. 공통 submit mapper를 유지하면서 stale generation을 exact contract가 정한 `invalid_operation` 또는 해당 route reason으로 일관되게 매핑한다. 남은 operation-family process matrix와 installed ABI probe를 추가하고 public error enum에 private detail name을 추가하지 않는다.
...
- 확인한 동작과 기대 동작: current request call은 serial turn plan이 있을 때 Framework offload executor에 `_submit(...).result()`를 예약하고 결과를 completion source에 넣는다. channel request도 bounded offload executor에서 `blocking_submit()`을 실행한다. one-way runtime은 첫 attempt 뒤 backpressure를 pending queue와 owner reservation에 넣고 deadline 또는 shutdown에서 completion한다. application receive는 HWM을 넘는 batch dispatch를 만들지 않고 한 dispatch turn에서 application record 하나만 시작한다. RouteMesh manual peer의 한 logical `connect()`는 Core가 Application/Completion transport lane 두 개로 표현하므로, runtime은 physical candidate와 logical intent를 혼동하지 않고 실제 `remote_addr`를 기준으로 disconnect해야 한다. 기대 동작은 source-local queue admission, bounded deadline, no automatic resubmit after terminal, owner/generation fencing, payload lifetime, retry/replay order와 callback exactly-once가 operation family별로 동일하게 지켜지는 것이다.
- 판정 근거: request/channel call surface에서 unmanaged `std::thread(...).detach()`를 제거하고 runtime offload executor rejection을 typed request failure로 처리했다. owner epoch와 retry admission 뒤 shutdown을 다시 검사해 late success를 `runtime_shutdown`으로 종결하는 회귀를 추가했다. HWM은 exact-boundary pause/resume와 mailbox batch 제한, STREAM reply token은 transport failure 뒤 one-shot 소비를 focused test에서 확인했다. Core submit result와 User Spot terminal mapping도 공통 error kind로 정렬했고, `test_cpp_framework_messaging`와 `test_cpp_framework_store_location_resolvers`에서 회귀를 확인했다. `offload_executor_t::drain_until()`과 cooperative/non-cooperative worker 회귀는 `test_cpp_framework_execution`에서 통과했다. `app_t::run_shared_shutdown()`의 active-run teardown barrier는 `wait_until(deadline_at)`으로 바뀌었고, blocking hosted service 회귀가 `force_stopped/deadline_exceeded`와 run-thread cleanup을 확인한다. 또한 raw mesh connection candidate가 monitor의 실제 `remote_addr`를 보존해 `not_required` disconnect가 descriptor advertised endpoint와 proxy endpoint를 혼동하지 않도록 보강했고, `test_cpp_framework_m6a_runtime`에서 endpoint 보존을 확인했다. `RM-A3` trace process는 이 path와 Core의 두 transport lane을 확인했지만 runner aggregate 전체는 남아 있다. 따라서 이 bounded barrier와 focused mapping runtime gap은 현재 충족 범위로 이동했지만, normal finalizer의 unbounded `drain()`은 cooperative teardown 뒤에 사용하는 내부 cleanup이고 caller cancellation, relocation backlog와 common process matrix는 아직 남아 있다. initial canonical full CTest 50/55와 historical non-sample 49/49를 합산하지 않으며, 최신 canonical full serial CTest 56/56도 narrow green test를 full semantic compliance로 판정하게 하지 않는다.
...
- 확인한 동작과 기대 동작: current verifier는 installed library hash 3개, 111 headers와 clean packaged consumer를 통과한다. 2026-08-02 historical sample runner는 Release `zlink_cpp=11.1.0`, Core `11.0.0`의 `build-v11-samples`를 사용했고, 2026-08-03 canonical build는 `11.1.0/11.1.0`이다. exact C++ interface 문서는 Framework `11.0.0`을 표기하고, current public `framework/version.hpp`는 `0.1.0`을 표기하므로 문서·public version·package pin도 하나의 provenance가 아니다. C++ HTTP package policy와 CMake export의 ownership 차이는 남아 있다. 기대 동작은 exact C++ contract에 맞는 header/target만 install하고 Framework, sample과 E2E가 같은 version·package·ABI provenance를 사용하는 것이다.
- 판정 근거: stale verifier manifest는 current tree에 맞춰 수정되어 통과했지만, HTTP ownership, Core exact version/ABI, clean consumer의 source-tree negative boundary와 compiler/configuration matrix는 전체 확인하지 않았다. 이번 Core router fix의 process 검증도 `LD_LIBRARY_PATH=core/build/lib` source-runtime evidence이므로 formal package evidence로 승격하지 않는다. target alias와 library hash만으로 install tree, E2E runtime과 ABI가 완전히 일치한다고 판정하지 않는다.
...
- 확인한 동작과 기대 동작: target contract test는 public header와 E2E wiring text를 filesystem scan하고 current CTest에서 PASS했다. `test_cpp_framework_install_consumer`와 `test_cpp_http_client`도 PASS했고, 2026-08-03 focused CTest 12/12와 package verifier가 통과했다. 이후 Core STREAM stop regression을 포함한 최신 canonical full serial CTest도 56/56으로 통과했다. initial parallel full CTest는 sample process 5개 failure를 포함해 50/55였으며, common 완료는 built declaration, actual runtime, package consumer와 process E2E evidence까지 요구하므로 aggregate process gate와 exact contract는 아직 남아 있다.
...
아래 9개 ID는 이 ledger의 regression 추적 ID다. runtime phase에서 `CPP-REG-002`, `CPP-REG-003`과 `CPP-REG-005`의 핵심 focused test를 추가했으며, 나머지는 exact contract·process·package 범위가 남아 있다.
...
| `CPP-REG-005` | error mapping | focused 구현·검증 완료 | Core submit result의 `not_admitted → rejected`, backpressure, terminated와 invalid submit mapping, request route/timeout/shutdown boundary `code()`, User Spot cancelled/shutdown mapping을 unit에서 확인했다. operation-family 전체 process matrix와 installed ABI probe는 남아 있다. |
...
- [ ] `CPP-TEST-001~002`: verifier와 textual gate가 exact contract와 actual runtime/E2E gate를 서로 모순 없이 검사한다. current source runtime 기준 full serial CTest 56/56과 sample process 6/6은 통과했지만 formal package provenance, exact inventory와 common E2E contract gate는 남아 있다.
...
- C++ Framework production/runtime: Core `test_router_handover` 1/1, focused m6a/m6b/m6c/vertical/client-server 5/5, full serial CTest 56/56이 최신 Core source runtime에서 통과했다. 이 결과는 formal package consumer provenance와 분리한다.
- Regression stability: receiver-side unadmitted application queue와 Core standby RID normalization regression이 trace/file log에서 확인됐다. RM-C2는 dedicated Redis/process run 10/10, 6개 sample aggregate는 6/6 통과했지만 full common inventory와 package gate는 남아 있다.
- Public contract/runtime boundary: worker, actor/spot declaration gap과 error terminal mapping의 operation-family/process evidence gap.
...
- E2E: Config 12/14 부재, Config 1~11/13 scenario 누락 또는 partial aggregate, Config 10 migration host, role evidence와 private include boundary가 남아 있다. 최신 bounded 12-config aggregate는 RM-A1~C7과 RM-C2를 통과했지만 RM-C8 뒤 exact C++ public interface에 없는 ClientServer listener `MaxMessageSize` 요구로 exit 2가 됐다. 추가 current recheck에서는 PubSub all이 6/6 통과했고, DiscoveryRegistryHa는 `list_topology()`와 ClientServer descriptor의 semantic mismatch, ResilienceLifecycle은 automatic RID/endpoint projection gap, RuntimeMonitoring은 retired C++ API compile gap이 확인됐다. 이 blocker들은 E2E만으로 public API를 추가하지 않는다.
...
- Sample 후속: 최신 Core source runtime에서 6개 sample individual/aggregate와 6개 sample smoke가 모두 통과했다. S1 exact message/field/transport inventory, formal package provenance, Windows host parity와 common E2E cross-reference는 남아 있다.
...
runtime phase에서 C++ sample runner, pinned build helper와 TicTacToe/Bingo readiness 및 file-log path를 수정했다.
...
| `LD_LIBRARY_PATH=core/build/lib timeout -k 10s 1800s env ZLINK_CPP_CLIENT_SERVER_TRACE=1 ZLINK_CPP_MESH_TRACE=1 ZLINK_CPP_STREAM_TRACE=1 ZLINK_CPP_HOST_STOP_TRACE=1 bash framework/languages/cpp/samples/run_samples.sh` | 6/6 passed, exit 0 | TicTacToe, Bingo, DeliveryDispatch, SupportChat, GameQuest, ShoppingMall의 public client result, role evidence, message trace/file log와 bounded cleanup을 같은 current source runtime에서 확인했다. file log는 `/tmp/zlink-cpp-official-sample-aggregate-runtime-gap-final3-20260803.log`다. |
| `framework/languages/cpp/samples/run_samples.sh`와 `run_samples.ps1` 실행 | shell·PowerShell 모두 6/6, exit 0 | bind retry를 제거하고 두 runner의 sample 목록을 6개로 맞춘 current source를 확인했다. `/usr/bin/pwsh` explicit run도 여섯 sample marker와 `sample all result=passed`를 trace/file log 조건에서 통과했다. PowerShell log는 `/tmp/zlink-cpp-powershell-sample-aggregate-runtime-gap-final-20260803.log`이며 Windows host/package provenance는 별도 evidence다. |
| `framework/languages/cpp/scripts/verify_packaged_contract.sh framework/languages/cpp/build` | passed | 2026-08-03 canonical installed manifest, library hash 3개, 111 headers와 clean packaged consumer가 통과했다. log는 `/tmp/zlink-cpp-package-contract-runtime-gap-final-20260803.log`다. |
| RuntimeMonitoring current-interface porting | service/client target build passed; MON-A1·MON-A2 process passed; MON-A3·MON-A4 failed | `RuntimeMonitoring` service host와 A1/A2 scenario를 `route_mesh_runtime_t`, `spot_manager_t`, `route_message_context_t`와 current builder/snapshot field로 이관했다. A3는 current snapshot field mismatch가 아니라 Config 7 fixture에 여러 positive Server peer가 남아 있어 filtered service의 zero-weight target count assertion과 맞지 않는다. A4는 Location Store discovery로 전환한 뒤에도 service stop 후 svc-b peer state가 false로 수렴하지 않아 physical disconnect/runtime owner 경계를 blocker로 남겼다. logs는 `/tmp/zlink-cpp-runtime-monitoring-build-port1-20260803.log`, `/tmp/zlink-cpp-runtime-monitoring-mon-a1-port5-20260803.log`, `/tmp/zlink-cpp-runtime-monitoring-mon-a2-port1-20260803.log`, `/tmp/zlink-cpp-runtime-monitoring-mon-a3-port1-20260803.log`, `/tmp/zlink-cpp-runtime-monitoring-mon-a4-port1-20260803.log`다. |
| S0 full CTest와 C++ common E2E aggregate | full serial CTest 56/56 / common aggregate exit 2 | full serial CTest log는 `/tmp/zlink-cpp-full-ctest-runtime-gap-final-20260803.log`다. 6개 sample process와 current runtime regression은 green이다. PubSub 6/6은 추가로 통과했지만 common aggregate는 RM-C8 contract, DiscoveryRegistryHa·ResilienceLifecycle·RuntimeMonitoring source/contract/runtime blocker로 14-config/374-ID 전체 완료가 아니다. RuntimeMonitoring A1/A2는 current source porting 후 별도 process log에서 통과했고 A3/A4는 실패 evidence를 유지한다. |

최신 source runtime에서 6개 sample aggregate는 실제 role server 호출, client self-check와 server
evidence를 모두 통과했다. 이 결과는 Windows host/package provenance, common E2E 전체와 exact
message inventory fixture를 대신하지 않는다.
...
| 항목 | 현재 판정 | 제한 |
|---|---|---|
| C++ sample 범위 | `Bingo`, `TicTacToe`, `SupportChat`, `DeliveryDispatch`, `ShoppingMall`, `GameQuest`의 6개 목록이 source와 shell·PowerShell aggregate에 존재한다. | exact common inventory와 Windows host/package provenance는 미완료다. |
...
| public client boundary | client source에서 public HTTP/stream 호출 경계를 확인했고 최신 6개 process에서 role endpoint self-check가 통과했다. | full source negative scan과 package-mode process evidence가 남아 있다. |
| static parity/layout/target gate | 4/4 narrow CTest가 통과했다. | static green은 전체 계약 완료가 아니다. |
| role executable 분리와 Redis cleanup | 최신 6개 sample aggregate에서 readiness, role evidence와 cleanup이 통과했다. | Windows host와 formal package provenance는 별도 확인이 필요하다. |
| exact message/field/transport parity | 부분 충족 | known mismatch는 current source에서 정렬됐지만 `CPP-SAMPLE-IMP-001`의 전체 inventory/fixture/extra 분류가 남았다. |
| 6개 process와 aggregate | 부분 충족 | 최신 Core source runtime에서 6개 sample individual/aggregate가 6/6 통과했다. formal package provenance와 S1 exact inventory/PowerShell host evidence, S0 common 14-config/374-ID aggregate가 남아 있어 완료로 판정하지 않는다. |

## 15. `CPP-SAMPLE-IMP-*` production implementation gap

### CPP-SAMPLE-IMP-001 — 공통 inventory 정렬은 진행됐지만 전체 증명과 extra 분류가 남음

- 상태: `부분 충족`, 전체 inventory와 contract review는 남음.
...
- 확인한 실제 동작과 기대 동작: `SetTypingMsg`, `LeaveGameMsg`, SupportChat `createdAtUnixMs`, TicTacToe optional game name와 failed notification, Delivery `occurredAtUnixMs`·nullable courier, GameQuest `ClosePlayerQuestMsg`·typed JSON payload·one-way `CollectItemReq/EnterAreaReq`, ShoppingMall `OrderState`/`sourceCommandId` 차이는 current shared contract에서 정렬됐다. SupportChat `JoinConversationFailedNotify.isRetriable`와 GameQuest의 `CompleteMission*`, `UnlockFeature*`, `CollectItemRes`, `EnterAreaRes`는 공통 계약 밖 public surface라 제거했다. contract parity test는 SupportChat field 수, GameQuest action response 부재, Delivery null/timestamp, ShoppingMall nullable `OrderState`를 검사하며, 최신 6개 sample process의 trace/file log도 통과했다. 공통 문서와 C++ 선언의 전체 field·optional/null·transport·reply semantics를 machine-readable diff로 비교하는 작업과 모든 extra 분류는 아직 남아 있다.
- gap 판정 근거: current static parity와 process PASS는 확인한 mismatch가 현재 실행 경로에서 사라졌다는 근거지만, 전체 inventory one-to-one와 모든 extra public surface 부재를 직접 증명하지 않는다. 공통 문서에 없는 extra를 다른 언어 구현만으로 public C++ API로 승격하지 않는다.
...
### CPP-SAMPLE-IMP-004 — shell·PowerShell runner inventory는 정렬됐지만 host/process evidence가 남음

- 상태: `부분 충족`; source runner parity와 Linux process aggregate는 통과했지만 Windows host와 formal package provenance는 남음.
...
- 확인한 실제 동작과 기대 동작: current shell·PowerShell manifest는 같은 6개 sample을 호출하고 bind failure retry를 사용하지 않는다. 최신 source runtime에서 shell aggregate와 `/usr/bin/pwsh` explicit aggregate가 각각 6/6, CTest sample smoke도 6/6 통과했으며 sample별 message trace/file log·role evidence·bounded cleanup이 보존됐다. 기존 shell individual runner가 Framework host의 30초 shutdown deadline보다 짧게 기다리던 false cleanup failure를 여섯 runner의 bounded wait로 정렬했다. TicTacToe와 Bingo는 application stdout과 diagnostic trace stderr를 별도 file log로 보존하고 asynchronous actor cleanup marker를 bounded하게 기다린다. PowerShell log는 `/tmp/zlink-cpp-powershell-sample-aggregate-runtime-gap-final-20260803.log`다. Windows host에서의 package/build provenance와 full result schema 검증은 남아 있다. 기대 동작은 양쪽 runner가 같은 6개 inventory, selector, retry policy, bounded timeout, result schema와 cleanup을 사용하고 모든 선택 sample의 결과를 수집하는 것이다.
- gap 판정 근거: Linux source-runtime 결과는 process success를 증명하지만 Windows host/package parity와 formal package provenance를 보장하지 않는다. sample aggregate green은 common E2E 14-config completion을 의미하지 않는다.
- 구체적인 수정 목록: shell·PowerShell individual runner를 6개로 맞추고 aggregate manifest에서 중복·누락을 machine-check한다. bind failure retry를 제거하고, sample별 exit status와 evidence schema를 수집한 뒤 하나라도 실패하면 aggregate를 실패시킨다. readiness, Redis, child process와 log cleanup을 bounded timeout으로 판정한다. 정상 shutdown을 기다린 뒤에도 process가 남으면 강제 종료하고 aggregate를 실패시킨다.
...
### CPP-SAMPLE-E2E-IMP-001 — Linux source-runtime sample process는 통과했지만 package/host 범위가 남음

- 상태: `부분 충족`, formal package/Windows host와 exact inventory cross-reference가 남음.
...
- 확인한 실제 동작과 기대 동작: `LD_LIBRARY_PATH=core/build/lib`와 `ZLINK_CPP_CLIENT_SERVER_TRACE=1`, `ZLINK_CPP_MESH_TRACE=1`, `ZLINK_CPP_STREAM_TRACE=1`, `ZLINK_CPP_HOST_STOP_TRACE=1`을 사용한 최신 6개 sample aggregate가 6/6 통과했다. 각 sample의 public client result와 role server self-check/evidence, readiness, message trace/file log와 bounded cleanup이 같은 run에서 확인됐다. full serial CTest도 56/56으로 통과했고, Core STREAM stop unittest와 stream connector lifecycle ASan evidence가 별도로 남아 있다. 기대 동작은 client가 public HTTP/STREAM API로 role server를 호출하고, 같은 run에서 server-side evidence가 상태 전이, owner, generation, callback exactly-once, cleanup과 terminal reason을 검증하는 것이다.
- gap 판정 근거: 이번 결과는 현재 source Core runtime을 강제한 Linux process evidence이며 formal local package의 current Core candidate provenance나 Windows host 실행을 증명하지 않는다. common E2E aggregate는 12-config에서 RegistrationCodec과 RegistryMessaging RM-A1~C7, RM-C8 payload round-trip까지 통과했지만 RM-C8 max-size subflow가 ClientServer listener `MaxMessageSize` exact contract blocker로 종료됐다. common E2E ID cross-reference와 exact sample inventory fixture도 별도 gate다.
...
| C++ runtime test | Core STREAM listener bounded-stop unittest, stream connector lifecycle ASan/focused test와 latest full serial CTest 56/56이 통과했다. | current sample smoke와 common E2E가 runtime gate를 대신하지 않도록 별도 evidence를 유지한다. |
...
- 확인한 실제 동작과 기대 동작: CTest sample smoke target은 6개 shell script를 등록한다. 최신 Core source runtime 기준 full serial CTest는 56/56, sample smoke는 6/6, shell aggregate와 `/usr/bin/pwsh` explicit aggregate는 각각 6/6 통과했다. PowerShell manifest도 6개로 정렬됐고 Linux explicit run은 통과했지만 Windows host와 formal package provenance는 별도 확인이 필요하다. static parity 결과만으로 role server evidence, callback count, terminal reason, owner/generation, cleanup과 aggregate completeness를 판정하지 않는다. 기대 동작은 sample별 process 결과와 aggregate completeness를 실제 실행으로 확인하는 것이다.
