...
| S8-CPP lane (C ABI+C++ bindings→framework) | bindings 라이브러리 green | 0 | 0 | 0 | **bindings Service 재작성 통합·main 컴파일 green**(`212ba9a08`, `zlink_cpp` 0 err/warn, 폐기 개념 0 hits). `mesh_node.hpp`/`dispatch.hpp`/`stream_session.hpp` 신설, route_bridge/spot_send/spot_receive/actor_operations 삭제, raw-layer drift 수정. **패러다임 발견(전 lane 공통)**: 10.0.0 C API가 콜백·fluent builder 불가(요청=operation_id 반환, 완료=pull dispatch 도착)→service=direct-method. **컴파일 green은 전제조건일 뿐, stage 게이트=`BINDINGS REVIEW CLEAN`**(Codex·Sonnet 2리뷰어 3축, S5와 동일 규격). 잔여: samples 변환→전체 스냅샷 프리즈→bindings 리뷰 campaign→framework |
| **S8-DN lane (.NET bindings→framework) [참조 lane]** | bindings 라이브러리 green | 0 | 0 | 0 | **dotnet parity 정본 = framework 참조**. bindings Service 재작성 통합·main 컴파일 green(Build succeeded 0/0, 폐기 개념 0 hits). 참조 C# 표면 확정: pull dispatch(SetReadyHandler/DrainReady/TakeClaim/Receive/RetainMessage/Reply-token)·SubmitResult+MeshOperationId direct·IStreamSessionService. 게이트=`BINDINGS REVIEW CLEAN`. 잔여: samples 12 변환→스냅샷 프리즈→bindings 리뷰 campaign→framework(참조 확정) |
