...
| config | 현재 판단 | 작업 원칙 |
|--------|-----------|-----------|
| `RegistryMessaging` | RM-A/B/C 대부분 완료, RM-C9 P2 gap | `.NET`의 `Client/Scenarios`, `Server/<Role>`, `Shared` 분류에 맞춘 C++ role/source와 inventory가 있다. `timeout 420s framework/languages/cpp/e2e/RegistryMessaging/run_e2e.sh all` 최신 통과 증거는 `logs/20260630-161051-366893`이며 RM-C9 child log는 `logs/20260630-161317-377857`이다. RM-C9 send pressure/recovery는 검증하지만 C++ public send submit이 `.NET`처럼 bounded failure oracle을 노출하지 않아 config 완료 판정은 보류한다. |
| `SpotService` | 구현 범위 통과 | `.NET` 기준 inventory와 C++ scenario header/role source가 있다. `Client/Support/client_support.hpp`, `Client/Support/client_options.hpp`, `Client/Support/spot_lifecycle_order_context.hpp`를 추가해 client 공통 env/assert/stream helper, option 값, channel handler state, `.NET`식 lifecycle context를 `Client/main.cpp`에서 분리했고, `Shared/spot_service_contracts.hpp`에 generic JSON stream payload hook을 추가해 stream connector DTO가 기본 JSON codec 경로를 쓰게 했다. `SM-A5`는 `.NET`의 app-level `ScenarioStage` 의미를 C++ user spot handler와 public timer API 위에 구현했다. `SM-Q9`는 MultiNode A/B role과 외부 route client target spot request로 focused runtime proof를 추가했다. `.NET` SM-D2처럼 `session-a`에서 `play-b`로 가는 stream relay readiness를 확인하고, crash setup route endpoint를 독립시킨 뒤 `timeout 900s framework/languages/cpp/e2e/SpotService/run_e2e.sh all` 최신 통과 증거는 `logs/20260630-205514-1176161`이다. |
...
| `DeliveryDispatch` | courier gateway/actor-node route split 구현, customer/session role split partial | 기존 `DiscoveryRegistryHa` harness 대신 `.NET DeliveryDispatch` 샘플 기준 C++ E2E가 있다. registry, dispatch API, dispatch center, courier gateway, courier actor node 1/2, tracking, session, probe, client target을 실행하고, customer stream 1개와 courier stream 2개로 `BindCourierSessionReq`, `BindCourierReq`, `EnsureCourierActorReq`, `OfferDeliveryNotify`, `CourierDecisionMsg` 흐름을 검증한다. `timeout 420s framework/languages/cpp/e2e/DeliveryDispatch/run_e2e.sh` 최신 통과 증거가 있으며 successful delivery, reassignment, server evidence self-check, courier gateway/actor-node message-flow evidence가 통과한다. 남은 차이는 `.NET`의 CustomerGateway/CourierSession 전용 role과 actor-bound session 배치다. |
| `ResilienceLifecycle` | 구현 범위 통과 | 전용 Registry, Provider, Workflow, Client target과 `.NET` 기준 inventory가 있다. `timeout 420s framework/languages/cpp/e2e/ResilienceLifecycle/run_e2e.sh` 최신 통과 증거는 `logs/20260630-184559-770532`이며 RL-A1, RL-A2, RL-A3, RL-A4, RL-A5, RL-B1, RL-B2, RL-B3, RL-B4, RL-B5, RL-B6, RL-C1, RL-C2, RL-C3, RL-C4 registry outage 및 registry/provider 재기동 뒤 새 discovery client 복구, RL-D1, RL-D2, RL-D3, RL-D4, RL-D5를 검증한다. |
| `RuntimeMonitoring` | 구현 범위 통과 | `.NET` 기준 이름인 `RuntimeMonitoring`으로 전환했고 기존 C++ `Monitoring` PubSub 보조 wrapper는 삭제했다. C++ 전용 registry, service, filtered service, throwing service, trigger, client target과 `.NET` 기준 inventory가 있다. `timeout 420s framework/languages/cpp/e2e/RuntimeMonitoring/run_e2e.sh` 최신 통과 증거는 `logs/20260630-182556-730831`이며 MON-A1, MON-A2, MON-A3, MON-A4, MON-A5, MON-B1, MON-B2, MON-C1, MON-D1을 검증한다. |
| `YieldDispatch` | Track A/B/C/D/E1/E3/E4/E5 진행 중 | C++ e2e config와 runner가 생겼고 YD-A1~YD-A4, YD-B1~YD-B3, YD-C1~YD-C3, YD-D1~YD-D4, YD-E1, YD-E3, YD-E4 static gate와 YD-E5 report 생성은 runner 통과 증거가 있다. 최신 `timeout 420s framework/languages/cpp/e2e/YieldDispatch/run_e2e.sh` 통과 증거는 `logs/20260630-211459-1245708`이며 `scenario YD-E5 passed`와 `yield-dispatch-report.json`을 확인한다. 구현된 client scenario header는 YD-A/B/C/D2~D4/E1/E3까지 완료했고, Registry host factory/support, Delay host factory/handler/support, Play host factory/support/control/basic/timer/actor/remote/failure handler, spot type, `YieldProbeSpot` runtime, Session host factory/support, `yield_session_t` support header도 분리했다. 남은 완료 보류 사유는 YD-C3 actor-yield/timer-fast half가 observer connector를 쓰는 partial과 YD-E2 public yield cancellation token gap이다. |
