...
```csharp
var conversation = await conversations.LoadAsync(conversationId, ct);
conversation.RequireParticipant(senderId);

var message = await messages.AppendAsync(conversationId, senderId, text, ct);
await moderation.EnqueueAsync(message, ct);

foreach (var userId in conversation.ParticipantIds)
{
    var session = await connectionRegistry.FindAsync(userId, ct);
    if (session is not null)
        await gatewayDeliver.SendAsync(session.NodeId, session.SessionId, message, ct);
    else
        await notifications.EnqueueAsync(userId, message, ct);
}
```
...
```csharp
public sealed class SendMarketplaceMessageHandler(
    IMessageStore messages,
    INotificationQueue notifications)
    : IZLinkSpotActorRequestHandler<ConversationSpot, UserActor, SendChatReq, SendChatRes>
{
        ...
        CancellationToken ct)
    {
        ...
        return new SendChatRes(saved.MessageId);
    }
}
```
