...
- `core/`: 컨텍스트, 컨텍스트 옵션, routing id, 버전/capability 헬퍼, 유틸리티
  계약.
...
- `core/`: `context.ts`, `context_options.ts`, 버전/capability wrapper와 같은
  런타임 헬퍼 함수.
...
- `createPoller()`, `createTimer()`, `createTimer(spot)`은 eventing 리소스를
  생성한다.
- 버전, capability, strerror, proxy, sleep, multipart cleanup 헬퍼 같은 패키지
...
- Core: 컨텍스트, 버전/capability 헬퍼, 옵션, 유틸리티 함수.
...
- 컨텍스트 생명주기, 옵션, shutdown, auto-HWM 재계산, 버전, capability,
  strerror.
