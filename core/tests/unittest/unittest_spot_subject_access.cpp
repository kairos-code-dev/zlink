...
namespace
{
void noop_spot_dispatch_handler (void *, const zlink_spot_dispatch_info_t *, void *)
{
}

void noop_reply_handler (zlink_request_result_t, zlink_msg_t *, size_t, void *)
{
}

struct send_ready_probe_t
{
    send_ready_probe_t () : calls (0), reinstall_result (ZLINK_HANDLER_OK), reinstall_errno (0) {}
...
void reentrant_send_ready_handler (void *subject_, void *userdata_)
{
    ...
    probe->reinstall_result =
      zlink_send_ready_handler (subject_, &reentrant_send_ready_handler, userdata_);
    probe->reinstall_errno = zlink_errno ();
    probe->calls.fetch_add (1, std::memory_order_release);
}

std::vector<zlink_spot_node_socket_entry_t> read_socket_snapshot (void *node_)
{
    size_t count = 0;
...
{
    std::vector<zlink_spot_node_socket_entry_t> rows = read_socket_snapshot (node_);
                                ...
                                const char *socket_name_)
{
    std::vector<zlink_spot_node_socket_entry_t> rows = read_socket_snapshot (node_);
    for (size_t i = 0; i < rows.size (); ++i) {
        if (rows[i].owner == owner_ && strcmp (rows[i].socket_name, socket_name_) == 0) {
            return true;
        }
    }
    return false;
...
size_t count_owner_rows (const std::vector<zlink_spot_node_socket_entry_t> &rows_,
                         zlink_spot_node_socket_owner_t owner_)
{
    size_t count = 0;
...
size_t count_socket_rows (const std::vector<zlink_spot_node_socket_entry_t> &rows_,
...
size_t count_socket_name_rows (const std::vector<zlink_spot_node_socket_entry_t> &rows_,
                               const char *socket_name_)
{
    size_t count = 0;
    for (size_t i = 0; i < rows_.size (); ++i) {
...
const zlink_spot_node_socket_entry_t *
find_socket_row (const std::vector<zlink_spot_node_socket_entry_t> &rows_,
                 zlink_spot_node_socket_owner_t owner_,
                 const char *socket_name_)
{
    for (size_t i = 0; i < rows_.size (); ++i) {
        if (rows_[i].owner == owner_ && strcmp (rows_[i].socket_name, socket_name_) == 0) {
            return &rows_[i];
        }
    }
    return NULL;
...
void test_spot_node_options_default_zero_and_invalid_mode ()
{
    ...
    TEST_ASSERT_TRUE (snapshot_has_socket (default_node, "routed-router"));
    ...
    TEST_ASSERT_TRUE (snapshot_has_socket (zero_node, "routed-router"));
...
void test_spot_node_pubsub_mode_disables_routed_sockets_without_lazy_create ()
{
    ...
    TEST_ASSERT_FALSE (snapshot_has_socket (node, "routed-router"));

    void *spot = zlink_spot_new (node);
    TEST_ASSERT_NOT_NULL (spot);
    const size_t count_before = read_socket_snapshot (node).size ();
    TEST_ASSERT_FALSE (snapshot_has_owner_socket (node, ZLINK_SPOT_NODE_SOCKET_OWNER_SPOT, "pub"));
    TEST_ASSERT_FALSE (snapshot_has_owner_socket (node, ZLINK_SPOT_NODE_SOCKET_OWNER_SPOT, "sub"));
    errno = 0;
    TEST_ASSERT_EQUAL_INT (ZLINK_HANDLER_NOT_SUPPORTED, zlink_spot_dispatch_event_handler (
    ...
    zlink_msg_init (&msg);
    errno = 0;
    TEST_ASSERT_EQUAL_INT (ZLINK_SUBMIT_NOT_SUPPORTED,
...
void test_spot_node_routed_mode_disables_pubsub_sockets_without_lazy_create ()
{
    ...
    TEST_ASSERT_NOT_NULL (node);
    TEST_ASSERT_FALSE (snapshot_has_socket (node, "internal-router"));
    TEST_ASSERT_TRUE (snapshot_has_socket (node, "routed-router"));
    ...
    void *spot = zlink_spot_new (node);
    TEST_ASSERT_NOT_NULL (spot);
    TEST_ASSERT_FAILURE_RAW_ERRNO (ENOTSUP, spot_subject_set_subscription (spot, "alpha"));
    TEST_ASSERT_FALSE (snapshot_has_owner_socket (node, ZLINK_SPOT_NODE_SOCKET_OWNER_SPOT, "pub"));
    TEST_ASSERT_FALSE (snapshot_has_owner_socket (node, ZLINK_SPOT_NODE_SOCKET_OWNER_SPOT, "sub"));
    TEST_ASSERT_EQUAL_UINT (count_before, read_socket_snapshot (node).size ());
...
void test_spot_node_internal_socket_snapshot_contract ()
{
    ...
    size_t count = 0;
    TEST_ASSERT_SUCCESS_ERRNO (zlink_spot_node_internal_sockets (node, NULL, NULL, &count));
    ...
    TEST_ASSERT_EQUAL_INT (ZLINK_CONFIG_INTERNAL_ERROR,
                           zlink_spot_node_internal_sockets (node, NULL, &one[0], &small_count));
    ...
    const std::vector<zlink_spot_node_socket_entry_t> all_rows = read_socket_snapshot (node);
    TEST_ASSERT_TRUE (count_owner_rows (all_rows, ZLINK_SPOT_NODE_SOCKET_OWNER_NODE) > 0);
    TEST_ASSERT_EQUAL_UINT (0u, count_owner_rows (all_rows, ZLINK_SPOT_NODE_SOCKET_OWNER_SPOT));

    for (size_t i = 0; i < all_rows.size (); ++i) {
        TEST_ASSERT_TRUE (all_rows[i].socket_type == ZLINK_SOCKET_PUB
        ...
        TEST_ASSERT_TRUE (all_rows[i].monitor_status.source_kind == ZLINK_MONITOR_SOURCE_SOCKET
                          || all_rows[i].monitor_status.source_kind == ZLINK_MONITOR_SOURCE_SPOT_PUB
    ...
    count = 0;
    TEST_ASSERT_SUCCESS_ERRNO (zlink_spot_node_internal_sockets (node, &filter, NULL, &count));
    TEST_ASSERT_EQUAL_UINT (count_owner_rows (all_rows, ZLINK_SPOT_NODE_SOCKET_OWNER_NODE), count);

    memset (&filter, 0, sizeof (filter));
    filter.socket_type = ZLINK_SOCKET_ROUTER;
    count = 0;
    TEST_ASSERT_SUCCESS_ERRNO (zlink_spot_node_internal_sockets (node, &filter, NULL, &count));
    TEST_ASSERT_EQUAL_UINT (count_socket_rows (all_rows, ZLINK_SOCKET_ROUTER), count);

    memset (&filter, 0, sizeof (filter));
    snprintf (filter.socket_name, sizeof (filter.socket_name), "%s", "internal-router");
    count = 0;
    TEST_ASSERT_SUCCESS_ERRNO (zlink_spot_node_internal_sockets (node, &filter, NULL, &count));
    TEST_ASSERT_EQUAL_UINT (count_socket_name_rows (all_rows, "internal-router"), count);
...
void test_spot_node_admission_hwm_socket_snapshot_contract ()
{
    ...
    const std::vector<zlink_spot_node_socket_entry_t> rows = read_socket_snapshot (node);
    ...
    const zlink_spot_node_socket_entry_t *routed_router =
      find_socket_row (rows, ZLINK_SPOT_NODE_SOCKET_OWNER_NODE, "routed-router");

    TEST_ASSERT_NULL (find_socket_row (rows, ZLINK_SPOT_NODE_SOCKET_OWNER_NODE, "pub-ingress-tx"));
    TEST_ASSERT_NULL (find_socket_row (rows, ZLINK_SPOT_NODE_SOCKET_OWNER_NODE, "ingress-sub"));
    TEST_ASSERT_NULL (find_socket_row (rows, ZLINK_SPOT_NODE_SOCKET_OWNER_NODE, "internal-router"));
    TEST_ASSERT_NULL (
      find_socket_row (rows, ZLINK_SPOT_NODE_SOCKET_OWNER_NODE, "internal-router-tx"));
    TEST_ASSERT_NOT_NULL (fanout);
    TEST_ASSERT_NOT_NULL (mesh_pub);
    TEST_ASSERT_NOT_NULL (mesh_xsub);
    TEST_ASSERT_NOT_NULL (routed_router);

    TEST_ASSERT_EQUAL_INT (0, fanout->monitor_status.auto_hwm_applied_sndhwm);
    TEST_ASSERT_EQUAL_INT (pubsub_hwm, mesh_pub->monitor_status.auto_hwm_applied_sndhwm);
    TEST_ASSERT_EQUAL_INT (pubsub_hwm, mesh_xsub->monitor_status.auto_hwm_applied_rcvhwm);

    TEST_ASSERT_EQUAL_INT (router_hwm, routed_router->monitor_status.auto_hwm_applied_rcvhwm);
    TEST_ASSERT_EQUAL_INT (router_hwm, routed_router->monitor_status.auto_hwm_applied_sndhwm);
...
void test_spot_node_router_bind_endpoint_is_explicit ()
{
    ...
    const char *derived_endpoint = "inproc://explicit-router-mesh.zlink-spot-route";

    TEST_ASSERT_EQUAL (ZLINK_CONFIG_OK, zlink_spot_node_set_router_bind (node, router_endpoint));
    TEST_ASSERT_EQUAL (ZLINK_CONFIG_OK, zlink_spot_node_set_pub_bind (node, mesh_endpoint));
    TEST_ASSERT_EQUAL (ZLINK_CONFIG_INVALID_STATE,
                       zlink_spot_node_set_router_bind (node, "inproc://late-router-ingress"));

    void *conflict = zlink_socket (ctx, ZLINK_SOCKET_ROUTER);
    TEST_ASSERT_NOT_NULL (conflict);
    TEST_ASSERT_NOT_EQUAL (ZLINK_BIND_OK, zlink_bind (conflict, router_endpoint));
    TEST_ASSERT_EQUAL_INT (EADDRINUSE, zlink_errno ());
    TEST_ASSERT_EQUAL (ZLINK_BIND_OK, zlink_bind (conflict, derived_endpoint));
...
void test_spot_subject_access_resolves_composite_and_node_poller_sockets ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (resolve_spot_sub_subject_poller_fd (spot, &spot_sub_fd));
    TEST_ASSERT_NOT_EQUAL (zlink::retired_fd, spot_sub_fd);

    zlink::socket_base_t *node_pub_socket = resolve_spot_pub_subject_poller_socket (handle->node);
    TEST_ASSERT_NULL (node_pub_socket);
    TEST_ASSERT_EQUAL_INT (ENOTSUP, zlink_errno ());

    zlink::socket_base_t *node_sub_socket = resolve_spot_sub_subject_poller_socket (handle->node);
...
void test_spot_subject_access_routes_subscription_and_routing_state ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (spot_subject_set_routing_id (spot, rid_bytes, sizeof (rid_bytes)));

    zlink_routing_id_t rid;
    TEST_ASSERT_SUCCESS_ERRNO (spot_subject_get_routing_id (spot, &rid));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (spot_subject_set_subscription (spot, "beta*"));

    int topics_count = 0;
    size_t topics_size = sizeof (topics_count);
    TEST_ASSERT_SUCCESS_ERRNO (
      spot_subject_get_sub_option (spot, ZLINK_SUB_OPT_TOPICS_COUNT, &topics_count, &topics_size));
    ...
    int is_pattern = 0;
    TEST_ASSERT_SUCCESS_ERRNO (
      spot_subject_subscription_at (spot, 0, filter, &filter_len, &is_pattern));
    ...
    is_pattern = 0;
    TEST_ASSERT_SUCCESS_ERRNO (
      spot_subject_subscription_at (spot, 1, filter, &filter_len, &is_pattern));
    TEST_ASSERT_EQUAL_UINT (5, filter_len);
    ...
    TEST_ASSERT_SUCCESS_ERRNO (spot_subject_unset_subscription (spot, "beta*"));
...
void test_spot_subject_access_routes_composite_and_node_options ()
{
    ...
    const int linger = 27;
    TEST_ASSERT_SUCCESS_ERRNO (
      spot_subject_set_common_option (spot, ZLINK_OPT_LINGER, &linger, sizeof (linger)));
    ...
    TEST_ASSERT_EQUAL_INT (
      -1, spot_subject_get_common_option (spot, ZLINK_OPT_LINGER, &actual, &actual_size));
    TEST_ASSERT_EQUAL_INT (ENOTSUP, errno);

    actual = 0;
    actual_size = sizeof (actual);
    TEST_ASSERT_EQUAL_INT (
      -1, spot_subject_get_common_option (spot, ZLINK_OPT_RCVTIMEO, &actual, &actual_size));
    TEST_ASSERT_EQUAL_INT (ENOTSUP, errno);

    const int sndtimeo = 44;
    TEST_ASSERT_SUCCESS_ERRNO (
      spot_subject_set_common_option (spot, ZLINK_OPT_SNDTIMEO, &sndtimeo, sizeof (sndtimeo)));

    actual = 0;
    actual_size = sizeof (actual);
    TEST_ASSERT_EQUAL_INT (
      -1, spot_subject_get_common_option (spot, ZLINK_OPT_SNDTIMEO, &actual, &actual_size));
    TEST_ASSERT_EQUAL_INT (ENOTSUP, errno);
...
void test_spot_subject_access_applies_pending_pub_defaults_on_lazy_create ()
{
    ...
    const int nodrop = 1;
    TEST_ASSERT_EQUAL_INT (
      -1, spot_subject_set_common_option (spot, ZLINK_OPT_SNDHWM, &sndhwm, sizeof (sndhwm)));
    TEST_ASSERT_EQUAL_INT (EINVAL, errno);
    TEST_ASSERT_SUCCESS_ERRNO (
      spot_subject_set_common_option (spot, ZLINK_OPT_SNDTIMEO, &sndtimeo, sizeof (sndtimeo)));
    TEST_ASSERT_SUCCESS_ERRNO (
      spot_subject_set_pub_option (spot, ZLINK_PUB_OPT_NODROP, &nodrop, sizeof (nodrop)));
    TEST_ASSERT_NULL (resolve_spot_pub_subject_poller_socket (spot));
    TEST_ASSERT_EQUAL_INT (ENOTSUP, zlink_errno ());
    ...
    size_t actual_size = sizeof (actual);
    TEST_ASSERT_EQUAL_INT (
      -1, spot_subject_get_common_option (spot, ZLINK_OPT_SNDHWM, &actual, &actual_size));
    TEST_ASSERT_EQUAL_INT (EINVAL, errno);
    ...
    actual_size = sizeof (actual);
    TEST_ASSERT_SUCCESS_ERRNO (
      spot_subject_get_pub_option (spot, ZLINK_PUB_OPT_NODROP, &actual, &actual_size));
...
void test_spot_send_ready_callback_runs_on_worker_and_rejects_reentrant_install ()
{
    ...
    TEST_ASSERT_TRUE (zlink_test_wait_until (
      1000, [&probe] () { return probe.calls.load (std::memory_order_acquire) == 1; }));
...
int main ()
{
    ...
    RUN_TEST (test_spot_node_pubsub_mode_disables_routed_sockets_without_lazy_create);
    RUN_TEST (test_spot_node_routed_mode_disables_pubsub_sockets_without_lazy_create);
    ...
    RUN_TEST (test_spot_send_ready_callback_runs_on_worker_and_rejects_reentrant_install);
    return UNITY_END ();
}
