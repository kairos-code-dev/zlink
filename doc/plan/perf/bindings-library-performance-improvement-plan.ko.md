...
| `ws` | `MULTI_SPOT` | `보류(39.6%)` | `보류(38.7%)` | `보류(30.9%)` | `보류(14.3%)` | `보류(10.2%)` | `보류(1.4%)` | C 파일은 위 행과 같고, 64B~131072B는 Rust `perf_rust_multi_linux_20260522_152121_codex_rust_multi_ws_spot_20260522.txt`, 262144B는 inner drain deadline check 뒤 `perf_rust_multi_linux_20260522_205636_codex_rust_multi_ws_spot262_deadline_inner_loop_20260522.txt` 기준이다. 실행 중 server/client는 각 `nlwp=14` 수준으로 spot dispatch/data thread가 추가되는 구조다. 262144B timeout은 해소됐지만 전 size가 기준보다 낮다. |
...
| `wss` | `MULTI_SPOT` | `보류(40.0%)` | `보류(37.9%)` | `보류(31.7%)` | `보류(20.7%)` | `보류(15.8%)` | `보류(14.3%)` | C 파일은 위 행과 같고, Rust `perf_rust_multi_linux_20260522_154237_codex_rust_multi_wss_spot_tls_path_20260522.txt` 기준이다. TLS PEM 내용을 넘기던 경로 오류를 파일 경로 전달로 수정해 server_ready_timeout은 해소했지만, 전 size가 기준보다 낮다. |
...
| `tls` | `MULTI_SPOT` | `보류(37.4%)` | `보류(38.0%)` | `보류(38.2%)` | `보류(17.5%)` | `보류(10.5%)` | `보류(1.9%)` | C 파일은 위 행과 같고, Rust `perf_rust_multi_linux_20260522_155408_codex_rust_multi_tls_spot_tls_path_20260522.txt` 기준이다. TLS PEM 경로 수정 후 complete이며, 실행 중 client `nlwp=14`로 `spot-dispatch`/`spot-data` thread가 추가되는 구조다. 전 size가 기준보다 낮다. |
...
- **SPOT client per-message 복사 제거 + multi-worker drain**: (1) `MULTI_SPOT` client 수신 hot path가 매 메시지 payload를 `message_payload(...).to_vec()`로 전체 복사했다(262144B면 256KB/msg) → borrowed `&[u8]` 디코드로 `.to_vec()` 제거(large 10x). (2) 그 위에, 단일 thread가 100 spot을 순차 drain하던 것을 Go recv worker처럼 scoped thread 4개로 분할했다(`Spot: Send`, `chunks_mut` disjoint, `LatencyStats::merge`로 합산). 최종 tcp 결과(fresh C baseline 대비): small 64/256/1024 = **84.7/90.5/85.0% 통과**(원래 36/37/32%), large 65536/131072/262144 = 53.0/36.5/42.1%(원래 11/1.5/1.0%). small은 SPOT 목표 75% 통과, large는 routed/copy 영역 보류. 결과 `perf_rust_multi_linux_20260524_*` SPOT tcp. (러너 stale-guard symlink 버그도 `readlink -f`로 수정.)
