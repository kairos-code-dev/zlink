...
#ifndef __ZMQ_DISCOVERY_PROTOCOL_HPP_INCLUDED__
#define __ZMQ_DISCOVERY_PROTOCOL_HPP_INCLUDED__
...
{
    zmq_msg_t msg;
    if (zmq_msg_init_size (&msg, size_) != 0)
        return -1;
    if (size_ > 0 && data_)
        memcpy (zmq_msg_data (&msg), data_, size_);
    const int rc = zmq_msg_send (&msg, socket_, flags_);
    if (rc == -1)
        zmq_msg_close (&msg);
    return rc;
}
...
inline int send_routing_id (void *socket_, const zmq_routing_id_t &rid_,
                            int flags_)
{
    return send_frame (socket_, rid_.size ? rid_.data : NULL, rid_.size, flags_);
}

inline bool read_u16 (const zmq_msg_t &msg_, uint16_t *out_)
{
    if (!out_)
        return false;
    if (zmq_msg_size (&msg_) != sizeof (uint16_t))
        return false;
    memcpy (out_, zmq_msg_data (const_cast<zmq_msg_t *> (&msg_)),
            sizeof (uint16_t));
    return true;
}

inline bool read_u32 (const zmq_msg_t &msg_, uint32_t *out_)
{
    if (!out_)
        return false;
    if (zmq_msg_size (&msg_) != sizeof (uint32_t))
        return false;
    memcpy (out_, zmq_msg_data (const_cast<zmq_msg_t *> (&msg_)),
            sizeof (uint32_t));
    return true;
...
inline bool read_u64 (const zmq_msg_t &msg_, uint64_t *out_)
{
    if (!out_)
        return false;
    if (zmq_msg_size (&msg_) != sizeof (uint64_t))
        return false;
    memcpy (out_, zmq_msg_data (const_cast<zmq_msg_t *> (&msg_)),
            sizeof (uint64_t));
    return true;
...
inline std::string read_string (const zmq_msg_t &msg_)
{
    const size_t size = zmq_msg_size (&msg_);
    if (size == 0)
        return std::string ();
    const char *data =
      static_cast<const char *> (zmq_msg_data (const_cast<zmq_msg_t *> (&msg_)));
    return std::string (data, data + size);
}

inline bool read_routing_id (const zmq_msg_t &msg_, zmq_routing_id_t *out_)
{
    if (!out_)
        return false;
    const size_t size = zmq_msg_size (&msg_);
    ...
    if (size > 0)
        memcpy (out_->data,
                zmq_msg_data (const_cast<zmq_msg_t *> (&msg_)), size);
    return true;
}
}
}
