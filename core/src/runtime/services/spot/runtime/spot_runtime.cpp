...
size_t fill_runtime_socket_slot_refs_impl (runtime_t *runtime_, slot_ref_t *out_)
{
    ...
    size_t count = 0;
    out_[count++] = slot_ref_t{&runtime_->data_ctrl_front, &runtime_->data_ctrl_endpoint, false};
    out_[count++] = slot_ref_t{&runtime_->data_ctrl_back, &runtime_->data_ctrl_endpoint, false};
...
void spot_runtime_t::set_external_route_id (const std::string &peer_endpoint_,
...
int spot_runtime_t::start ()
{
    ...
    data_ctrl_front = spot_node_access_t::create_socket (owner, ZLINK_CORE_SOCKET_PAIR);
    ...
    int worker_errno = 0;
    if (spot_data_plane_t::initialize_runtime (owner, this, &execution.data_plane_state) != 0
        || !spot_node_access_t::recv_ctrl_reply (data_ctrl_front, &worker_errno)) {
...
int spot_runtime_t::start_dispatch_workers ()
{
    ...
    spot_dispatch_worker_pool_t *pool = new (std::nothrow) spot_dispatch_worker_pool_t ();
    if (!pool) {
        errno = ENOMEM;
        return -1;
    }
...
int spot_runtime_t::post_dispatch_task (spot_dispatch_worker_pool_t::task_fn_t fn_, void *arg_)
{
    if (!dispatch_workers) {
        errno = ETERM;
        return -1;
