...
- [ ] **E2E-JV-03** (결함) — Client 주 경로·보조 launcher raw 0. 서버 위임 경로 raw 1
...
- [ ] **E2E-JV-06** (결함) — StoreFailure·RuntimeMonitoring·ResilienceLifecycle 해결. SpotService의 서버 위임만 남아 있다
...
| **E2E-JV-03** (결함) | [E2E README:43-44](../../common/e2e/README.ko.md): client는 **언어별 HTTP client wrapper**를 사용하고 raw `HttpClient`로 app endpoint를 호출하지 않는다 | **부분 해결:** Client 주 경로와 보조 launcher는 wrapper 11 : raw 0이고 RuntimeMonitoring·ResilienceLifecycle의 server driver도 제거했다. 서버 위임 코드에는 raw 1곳이 남아 있다. 아래 `E2E-JV-03 진행 기록`에서 검증과 잔여 경로를 추적한다. |
...
| **E2E-JV-06** (결함) | [E2E README:236](../../common/e2e/README.ko.md): `Client/Scenarios/<ScenarioId><Name>Scenario.*`는 시나리오 ID마다 파일 하나를 두고, [§2.5:324-329](../../common/e2e/README.ko.md)는 server의 `/run`·`/scenario` endpoint에 검증 전체를 위임하지 못하게 한다 | **부분 해결:** StoreFailure·RuntimeMonitoring·ResilienceLifecycle은 각 Client 시나리오 파일이 순서와 단언을 직접 소유한다. SpotService의 server driver 위임만 남아 있다. 아래 `E2E-JV-06 진행 기록`에서 검증과 잔여 구조를 추적한다. |
...
- 전체 app을 다시 검색하면 서버 위임 raw 경로는
  `SpotService/Shared/.../ClientScenario.java` 1곳만 남는다. 이 경로는 서버가 Client 역할을 대신하는
...
- POSD/DDD 재리뷰에서는 JDK transport 타입 누출, Shared의 server/client 책임 혼합, endpoint·상태·수명
  ...
  Shared server helper에서 Client로 옮겼다. 의미 있는 잔여 위험 신호는 위 raw 1곳과 E2E-JV-06에서
  계속 추적한다.
...
- 남은 server 위임은 `SpotService/Shared/.../ClientScenario.java` 1곳이다. 이 경로는 E2E-JV-03의
  마지막 raw HTTP 책임 누출과 함께 제거한다.
