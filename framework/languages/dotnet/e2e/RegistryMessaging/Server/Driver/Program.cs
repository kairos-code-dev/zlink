...
builder.WebHost.UseUrls(options.DriverUrl);
...
static async Task RunAsync(ClientOptions options)
{
    ...
    if (!string.IsNullOrWhiteSpace(options.RegistryProject)
        && !string.IsNullOrWhiteSpace(options.ProviderProject))
    {
    ...
    string? ProviderProject)
{
    public static ClientOptions Parse(string[] args)
    {
        ...
        return new ClientOptions(
            values.GetValueOrDefault("driver-url", "http://127.0.0.1:0"),
            ...
            values.GetValueOrDefault("registry-project"),
            values.GetValueOrDefault("provider-project"));
    }
}
...
internal sealed class DynamicCluster : IAsyncDisposable
{
    ...
    public static async Task<DynamicCluster> StartAsync(ClientOptions options, string scenarioName)
    {
        ...
        var registry = cluster.StartServer(
            ...
            null,
            null,
            cluster._options.RegistryProject);
    ...
    private DynamicProcess StartServer(
        ...
        string? projectPath)
    {
        var serverProject = projectPath
            ?? throw new InvalidOperationException("Dynamic scenarios require a server project path.");
