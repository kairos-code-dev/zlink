...
| TA-B2 | implemented | 정상 actor snapshot에서 generation만 오래된 값으로 바꿔 `actorLocationStale`를 검증한다. | client runner가 stale actor ref의 send/request 실패 kind를 확인한다. |
| TA-B3 | implemented | actor row는 만들고 연결되지 않은 node rid를 가진 actor snapshot으로 `routeNotConnected`를 검증한다. | client runner가 route 미연결 send/request 실패 kind를 확인한다. |
