...
| **E2E-ND-06** (결함) | [§2.6:336-344](../../common/e2e/README.ko.md): runner가 실행별 role 설정 파일을 만들고 framework host에는 **설정 파일 경로만** 넘긴다. "Endpoint, Redis, routing id, timeout, 로그와 evidence 경로를 환경 변수…로 전달하지 않으며, **server와 client 애플리케이션 코드에서 직접 사용할 수 있는 환경 변수는 0개다.**" 어긋나면 **feature-map에 configuration migration gap을 기록**한다(`:352-354`) | 앱 코드가 환경변수를 **읽을 뿐 아니라 쓴다.** `SpotService/Server/Play/Configuration/play-options.ts:28`·`Session/Configuration/session-options.ts:29`·`MultiNode/Configuration/multi-node-options.ts:28`·`Gateway/gateway-host-factory.ts:160`이 role 설정 파일에서 rid를 읽은 뒤 **`process.env.ZLINK_E2E_RID = rid`로 프로세스 전역에 되쓴다** — 공유 `e2e/evidence-store.js:7,10`이 그 전역을 읽어 marker의 rid를 정하기 때문이다. **설정 전달을 전역 변수로 하는 뒷문**이고, 설정 파일 계약을 코드가 스스로 무효화한다. `SpotService/evidence-store.ts:7`은 `process.env.ZLINK_NODE_E2E_ROOT`로 **모듈 경로까지** 환경변수에서 받고, `RuntimeMonitoring/Client/Scenarios/mon-d1-failure-recovery-scenario.ts:67`은 client가 `env: { ...process.env, ZLINK_E2E_RID: 'svc-b' }`로 서버를 띄운다. 그리고 **11개 feature-map 중 configuration migration gap을 기록한 곳이 0개다** |
...
| **E2E-ND-09** (결함) | [§2.2:218-238](../../common/e2e/README.ko.md): config는 `.NET`과 같은 **독립 실행 배포 묶음**으로 옮기고 역할마다 `Server/<Role>/` 하나를 둔다. [§6.1:577-582](../../common/e2e/README.ko.md): 로그는 실행별 **`log/` 폴더**에 남기고 VCS에서 제외한다 | **소스가 한 줄도 없는 `Server/<Role>/`가 둘 남아 있다** — `DiscoveryRegistryHa/Server/Embedded/`와 `DiscoveryRegistryHa/Server/Probe/`에 `.ts` 파일이 **0개**인데, `dist/`엔 컴파일된 `main.js`·`embedded-host-factory.js`·`probe-host-factory.js` 등 **16개 `.js`가 남아 있다.** `package.json`도 없고 `run_e2e.sh`가 빌드하지도 실행하지도 않는다(현행 probe 역할은 `LocationProbe`다 — `run_e2e.sh:80,96`). **지워진 역할의 빌드 산출물만 화석으로 남은 것**이며, 실행 경로에서 죽어 있으므로 `rm -rf dist` 재빌드(`runner-common.sh:20-24`)로도 사라지지 않는다. 이름도 갈렸다 — Config 1이 `RegistryMessaging`(문서 제목과 `.NET` 프로젝트는 `LocationMessaging`), Config 6이 `DiscoveryRegistryHa`(`.NET`·Java는 `StoreFailure`), 그리고 **폐기된 config-8 하네스 `AutomaticTurnDispatch`가 `run_e2e_all.sh:15`의 기본 스윕에 그대로 있다.** 로그 폴더도 10개 config가 `logs/`, `SpotActorTransfer`만 §6.1이 정한 `log/`다 |
...
| **E2E-ND-09** | [e2e README §2.2:220-221,244-245](../../common/e2e/README.ko.md): config를 `.NET`과 같은 독립 실행 배포 묶음으로 옮기고 역할 경계와 파일 분류를 유지한다. [§3:468,473](../../common/e2e/README.ko.md): config 1 = Location messaging, config 6 = Store 장애·복구 | (a) `e2e/DiscoveryRegistryHa/Server/Probe/`에는 **`dist/`만 있고 source가 없다** — 삭제된 역할 서버의 컴파일 산출물이 그대로 남아 있다. (b) `e2e/ToActorMessaging/Client/dist/`에는 `Client/`·`Shared/`·`ToActorMessaging/` **세 세대의 출력이 겹쳐** 있다. (c) 디렉토리 이름이 config 문서·`.NET`과 어긋난다 — `RegistryMessaging`(config-1, `.NET`은 `LocationMessaging`), `DiscoveryRegistryHa`(config-6, `.NET`·Java는 `StoreFailure`). **주의: [§7:622-626](../../common/e2e/README.ko.md)이 명시적으로 유지한다고 밝힌 것은 시나리오 ID 접두사 `RM`이지 디렉토리 이름이 아니다** — 디렉토리 이름을 규정한 조항은 없으므로 이 항목은 `.NET` 기준선과의 drift로 읽는다. (d) 로그 디렉토리도 `SpotActorTransfer/run_e2e.sh:18`만 `log/`, 나머지 10개는 `logs/`다 |
