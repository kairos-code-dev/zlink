...
inline sample_topology_t load_sample_topology (int argc, char **argv)
{
    ...
    if (const char *endpoint = std::getenv ("ZLINK_CPP_CLIENT_STREAM_ENDPOINT")) {
        ...
        topology.session_a_stream_endpoint = endpoint;
        topology.session_b_stream_endpoint = endpoint;
    }
    if (const char *endpoint = std::getenv ("ZLINK_CPP_CLIENT_SESSION_A_STREAM_ENDPOINT")) {
        topology.session_a_stream_endpoint = endpoint;
    }
    if (const char *endpoint = std::getenv ("ZLINK_CPP_CLIENT_SESSION_B_STREAM_ENDPOINT")) {
        topology.session_b_stream_endpoint = endpoint;
    }
    return topology;
}
