...
int main (void)
{
    ...
    assert (zlink_spot_node_actor_new (node, "single-player", &actor)
            == ZLINK_CONFIG_OK);
    ...
    assert (zlink_stream_send_bound_actor_part (
              stream, &session_rid, "single-player", &before, ZLINK_DONTWAIT,
              ZLINK_PART_FINAL)
            == ZLINK_SUBMIT_OK);
    ...
    assert (zlink_stream_send_bound_actor_part (
              stream, &session_rid, "single-player", &between, ZLINK_DONTWAIT,
              ZLINK_PART_FINAL)
            == ZLINK_SUBMIT_OK);
