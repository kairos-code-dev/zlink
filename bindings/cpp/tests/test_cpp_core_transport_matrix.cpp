...
std::string bind_endpoint (zlink::socket_t &socket_,
...
void run_router_dealer (const std::string &transport_)
{
    ...
    assert (dealer.set (zlink::socket_option::routing_id, "DEALER1", 7) == 0);
    ...
    assert (router.send ("DEALER1", 7, zlink::send_flag::sndmore) == 7);
...
void run_router_router (const std::string &transport_)
{
    ...
    assert (server.set (zlink::socket_option::routing_id, "SERVER", 6) == 0);
    assert (client.set (zlink::socket_option::routing_id, "CLIENT", 6) == 0);

    const std::string endpoint = bind_endpoint (server, transport_, "rr");
    assert (client.connect (endpoint) == 0);
    sleep_ms (300);

    assert (client.send ("SERVER", 6, zlink::send_flag::sndmore) == 6);
    ...
    assert (server.send ("CLIENT", 6, zlink::send_flag::sndmore) == 6);
