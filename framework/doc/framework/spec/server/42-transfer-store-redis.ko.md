...
이 문서는 Framework 11.0 Redis Transfer Store가 cross-node Actor·Spot 이동에 필요한 immutable payload를
저장하는 규칙을 정의한다. Application state, accepted journal, participant별 state·journal payload, terminal
completion과 replay·recovery payload가 이 Store에 속한다. Provider는 transfer envelope, application state와
journal record를 해석하지 않는다.

Transfer Store는 Location Store와 별도 public interface, 별도 등록과 별도 Redis implementation을 사용한다.
두 implementation은 같은 Redis deployment 또는 cluster를 서로 다른 key prefix로 사용할 수 있고 물리적으로
분리된 Redis를 사용할 수도 있다. Location Store transaction은 Transfer Store key를 포함하지 않으며 두 Store
사이에 distributed transaction이나 2PC를 요구하지 않는다.

## 2. Immutable root와 manifest

Transfer logical stream은 최대 256 GiB다. Provider는 최대 64 MiB data를 가진 immutable chunk와 최대 4096개
...
Transfer manifest는 participant별 state·journal payload를 찾기 위한 projection과 같은 canonical inventory digest를
가질 수 있지만 owner·membership commit의 권한 근거가 아니다. Runtime은 Location participant set의 digest와
Transfer manifest digest가 정확히 같은 경우에만 restore와 replay를 시작한다.
...
Transfer Store는 opaque payload를 저장하고 읽고, retention을 갱신하고, reference를 삭제하는 generic operation만
제공한다. Actor·Spot별 Store interface, transfer phase별 method와 Redis 전용 root 등록 API는 제공하지 않는다.
...
1. Transfer Store에 모든 immutable chunk를 저장한다.
...
Target restore와 recovery는 current Location authority가 연결한 exact reference만 사용한다. Transfer manifest만으로
...
CAS가 성공하기 전에는 Transfer root를 삭제하지 않는다. Release 뒤에는 즉시 idempotent delete하거나 recovery
...
`TransferDataLost`로 seal한다. Commit된 owner와 membership을 source로 rollback하거나 이전 root를 추측해 복원하지
않는다.

## 6. 공식 Redis extension

공식 Redis extension package는 Location Store와 Transfer Store를 서로 다른 class로 제공한다. 각 instance는
...
authority의 availability와 Transfer payload의 availability는 독립적이며 한쪽 Redis script가 다른 Store의 key를
읽거나 변경하지 않는다.

## 7. 검증 요구

- Chunk와 root가 immutable하고 completion append가 새 root를 만든다.
- Transfer manifest의 inventory digest가 Location Store의 authoritative canonical participant set과 일치한다.
- Put과 root 검증이 authority CAS보다 먼저 수행된다.
- CAS conflict로 연결되지 않은 root가 orphan retention 또는 idempotent cleanup으로 제거된다.
- Location authority reference release가 Transfer root delete보다 먼저 수행된다.
- `Recreate`가 application state 없이도 accepted journal과 recovery payload를 Transfer Store에 기록한다.
- Published root의 permanent missing·checksum mismatch·inventory digest mismatch가 `TransferDataLost`이며 rollback하지
  않는다.
- Location과 Transfer Redis가 같은 deployment와 분리 deployment에서 모두 동작하고 cross-store transaction을
  요구하지 않는다.
