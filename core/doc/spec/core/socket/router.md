...
| Record | `source_node_rid_out_` | `source_spot_rid_out_` | `request_seq_out_` |
|---|---|---|---:|
| Ordinary raw multipart | Sending peer's routing ID | A zero-length routing ID | `0` |
| Received request | Sending peer's routing ID | A zero-length routing ID | A nonzero reply sequence |
...
The two routing-ID outputs are thread-local views owned by Core. The caller
does not release them and copies their values if they must be retained beyond
the next raw receive call. Starting the next receive call on the same thread
invalidates the preceding views. Every part of one multipart record returns
the same routing IDs and request sequence.
