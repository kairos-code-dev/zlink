...
final class PerfMultiSpotSendSend {
    private static final String CHANNEL = "bench-sendsend";
    ...
    static PerfUtil.Result runServer(PerfUtil.Config config) {
        ...
        try (Context ctx = PerfUtil.newContext(config);
            ...
            node.start();
            try (Spot server = node.getOrCreateSpot(SERVER_SPOT_RID).spot();
                ...
                awaitDirectControlStart(control, node, config,
                    "mesh sendsend server", spotServerReadyTimeoutMs(config));
                PerfUtil.recalculateAutoHwm(ctx);
                long activeEnd = System.nanoTime()
                    + Duration.ofSeconds(config.durationSeconds()).toNanos();
                while (System.nanoTime() < activeEnd) {
                    int count = dispatch.drain((owner, record, parts) -> {
                        if (record.kind() != RecordKind.SPOT_SEND
                            || record.sourceNodeRid() == null
                            || record.sourceSpotRid() == null
                            || parts.isEmpty()) {
                            return;
                        }
        ...
        try (Context ctx = PerfUtil.newContext(config);
            ...
            try (PerfMeshDispatch dispatch =
                ...
                PerfControl.awaitControlConnected(clientControlEndpoint,
                    "mesh sendsend client");
                waitForConnectedPeers(node, 1, config.connectReadyTimeoutMs(),
                    "mesh sendsend client data link");
                control.publishDataEndpoint(clientDataEndpoint);
                control.publishConnected();
                control.publishReadyCount(config.size(), clients.size());
                PerfControl.emitClientReady(config.size());
                PerfControl.awaitStart(config.size(), "mesh sendsend client");
                control.waitStart(config.size(), config.connectReadyTimeoutMs());
                runTraffic(dispatch, clients, config, metrics);
                return metrics.finishMulti(config);
            } finally {
                clients.forEach(Spot::close);
            }
        }
    }
    ...
    private static void runTraffic(
        ...
        int cursor = 0;
        while (System.nanoTime() < activeEnd) {
            Spot client = clients.get(cursor++ % clients.size());
    ...
    private static void sleepQuietly(int millis) {
        try {
            ...
            throw new IllegalStateException("mesh sendsend sleep interrupted", error);
        }
    }
