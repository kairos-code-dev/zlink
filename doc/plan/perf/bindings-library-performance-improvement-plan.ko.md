...
| `tls` | `MULTI_DEALER_DEALER` | `보류(38.0%)` | `통과(57.7%)` | `통과(68.1%)` | `보류(42.9%)` | `통과(53.8%)` | `통과(53.2%)` | 64B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_055005.txt` 대비 Go `perf_go_multi_linux_20260525_055047.txt` 기준이다. 256/1024B는 Go `perf_go_multi_linux_20260524_222134.txt`, 65536B는 Go `perf_go_multi_linux_20260525_001654.txt` median 기준이다. 131072B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_071516.txt` 대비 Go `perf_go_multi_linux_20260525_071539.txt` 기준이고, 262144B는 C `perf_c_multi_linux_20260525_071013.txt` 대비 Go `perf_go_multi_linux_20260525_071028.txt` 기준이다. C처럼 단일 poll loop와 pending socket `POLLOUT` 의미를 유지했다. active send는 1024B 이하에서 `MoveMessage(...)`를 사용하고, tls 131072/262144B는 public `Bytes(...)` client send를 사용한다. 131072B server 수신은 public `RecvPart` caller-owned 수신 경로로 확장해 no-code 재측정 17.177Kops/s(`perf_go_multi_linux_20260525_015918.txt`)에서 24.207Kops/s로 올렸고, 이후 client send를 `Bytes(...)`로 좁혀 바꿔 27.490Kmsg/s까지 올렸다. 262144B까지 넓힌 이전 broad 후보는 tls 262144B를 낮춰 반영하지 않았지만(`perf_go_multi_linux_20260525_015620.txt`), tls 262144B 단독 `Bytes(...)` 적용은 같은 조건 제한 재측정에서 14.154Kmsg/s, C 대비 53.2%로 통과했다. 64B/65536B는 최신 재측정에서도 단순 one-way 기준보다 낮다. |
...
- **2026-05-23 근본 원인 규명 + 수정 (strace 프로파일링)**: 위 large many-client 병목의 진짜 원인을 strace로 특정했다.
  ...
  - **남은 보류(수정 후에도 목표 미달, 후속 개선 대상)**: `MULTI_SPOT_SENDSEND` large(tcp/ws 일부와 tls 65536B 이상, routed-echo FFI 및 large payload 비용).
    `MULTI_DEALER_DEALER`의 tcp 262144B, wss 64/131072B, tls 64/65536B copy/FFI 비용.
