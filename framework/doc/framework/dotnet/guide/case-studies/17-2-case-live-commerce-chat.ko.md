...
```csharp
var stream = await streams.LoadAsync(streamId, ct);
stream.RequireOpen();
rateLimiter.RequireAllowed(userId, streamId);

var decision = await moderation.CheckAsync(userId, text, ct);
if (!decision.Allowed)
{
    await gateway.SendAsync(sessionId, new MessageRejected(decision.Reason), ct);
    return;
}

var message = await recent.AppendAsync(streamId, userId, text, ct);
await pubsub.PublishAsync($"stream:{streamId}", message, ct);
```
    ...
    : IZLinkSession
{
        ...
        CancellationToken ct)
    {
        if (dispatch.PacketName == "auth")
        {
            ...
            ActorRef actor = await actors.GetOrCreateAsync(req.ViewerId, "viewer", ct);
            _viewer = await context.Actors.BindAsync(actor, ct);
            context.Client.Reply(new AuthViewerOk()).Submit();
            return;
        }

        await _viewer!.RelayAsync(payload, ct);
    }
}
```

```csharp
public sealed class SendLiveMessageHandler(
    IRecentMessageStore recent,
    ILiveModeration moderation)
    : IZLinkSpotActorSendHandler<StreamSpot, ViewerActor, SendLiveChat>
{
...
public sealed class PinMessageHandler
    : IZLinkSpotActorSendHandler<StreamSpot, ModeratorActor, PinMessage>
{
    public async ValueTask HandleAsync(
        StreamSpot stream,
...
주의할 점은 fan-out 이다. `foreach (var watching in stream.ActiveViewers)` 는 작은 방송과
