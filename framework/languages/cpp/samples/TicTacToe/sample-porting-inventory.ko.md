...
| `.NET: Server/Play/Infrastructure/ZLink/Spots/EntrySpot/Handlers/PlayActorJoinGameHandler.cs` | `Server/Play/Infrastructure/ZLink/Spots/EntrySpot/tictactoe_entry_spot.hpp` | spot-handler | done | actor room join request를 room Spot join으로 연결한다. |
| `.NET: Server/Play/Infrastructure/ZLink/Spots/EntrySpot/Handlers/PlayActorObserveMilestoneHandler.cs` | `Server/Play/Infrastructure/ZLink/Spots/EntrySpot/tictactoe_entry_spot.hpp` | spot-handler | done | observer actor를 local Entry Spot milestone observer로 등록한다. |
| `.NET: Server/Play/Infrastructure/ZLink/Spots/EntrySpot/Handlers/PlayerWinMilestoneEventHandler.cs` | `Server/Play/Infrastructure/ZLink/Spots/EntrySpot/tictactoe_entry_spot.hpp` | pubsub-handler | done | milestone pub/sub event를 observer bound session push로 바꾼다. |
...
| `.NET: Server/Play/Infrastructure/ZLink/Spots/TicTacToeGameSpot/Handlers/PlayActorLeaveGameHandler.cs` | `Server/Play/Infrastructure/ZLink/Spots/TicTacToeGameSpot/tictactoe_game_spot.hpp` | spot-handler | done | leave request와 Entry Spot 복귀 흐름이 대응한다. |
| `.NET: Server/Play/Infrastructure/ZLink/Spots/TicTacToeGameSpot/Handlers/PlayActorPlaceMarkHandler.cs` | `Server/Play/Infrastructure/ZLink/Spots/TicTacToeGameSpot/tictactoe_game_spot.hpp` | spot-handler | done | mark request를 domain state 변경으로 연결한다. |
...
| `common: stream-inbound marker와 message-flow evidence` | `Client/main.cpp`; `run_sample.sh`; `run_sample.ps1`; role `main.cpp` trace option | validation | done | runner가 client marker와 sample log directory message-flow 기록을 검사한다. |
