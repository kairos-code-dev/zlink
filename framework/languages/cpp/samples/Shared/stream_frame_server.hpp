...
{
  zlink::stream_connector::message_kind_t kind =
...
inline bool
has_request_seq (zlink::stream_connector::header_flags_t flags)
{
  return (static_cast<std::uint8_t> (flags) &
          static_cast<std::uint8_t> (
            zlink::stream_connector::header_flags_t::has_request_seq)) != 0;
}
...
inline void
write_u32 (std::vector<std::uint8_t> &bytes, std::uint32_t value)
{
...
inline std::uint64_t
read_u64 (const std::string &bytes, std::size_t &offset)
{
...
inline std::optional<stream_server_frame_t>
try_read_stream_frame (std::string &buffer)
{
  if (buffer.size () < 6) {
    return std::nullopt;
  }
  const auto header_size =
    (static_cast<std::size_t> (static_cast<std::uint8_t> (buffer[0])) << 8) |
...
make_stream_frame (zlink::stream_connector::message_kind_t kind,
                   std::optional<std::uint64_t> request_seq,
                   std::string name,
                   std::string payload)
{
  ...
  header.push_back (
    static_cast<std::uint8_t> (zlink::stream_connector::codec_t::raw));
  header.push_back (
    request_seq ? static_cast<std::uint8_t> (
...
void
send_stream_push (zlink::received_t &inbound,
                  std::string packet_name,
                  const TNotify &notify)
{
  send_stream_frames (
