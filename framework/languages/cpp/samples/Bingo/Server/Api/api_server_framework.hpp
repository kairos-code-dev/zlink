...
inline zlink::framework::app_t &add_bingo_api_server (zlink::framework::app_t &app, const sample_topology_t &topology)
{
    ...
    app.add_zlink_framework ([&] (zlink::framework::zlink_framework_options_t &options) {
        options.handlers ().add<authenticate_player_handler_t> ("api").add<match_bingo_api_handler_t> ("api");
        ...
        options.client_server_channel (sample_names_t::play_channel).client ();
    });
    return app;
}
