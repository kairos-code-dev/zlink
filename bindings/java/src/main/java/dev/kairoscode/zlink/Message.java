...
public final class Message implements AutoCloseable {
    ...
    /** Message type constants for request-reply. */
    ...
    public static final int METADATA_KEY_USER_MIN = 0x0100;
    /** Maximum metadata value size in bytes. */
    public static final int METADATA_VALUE_MAX = 65535;

    /**
     * Mark this message as REQUEST with the given correlation ID.
     */
    public void setRequest(long correlationId) {
        if (!valid) throw new IllegalStateException("message is closed");
        int rc = NativeMsg.msgSetRequest(msg, correlationId);
        if (rc != 0)
            throw ZlinkException.fromLastError("zlink_msg_set_request");
    }

    /**
     * Mark this message as REPLY with the given correlation ID.
     */
    public void setReply(long correlationId) {
        if (!valid) throw new IllegalStateException("message is closed");
        int rc = NativeMsg.msgSetReply(msg, correlationId);
        if (rc != 0)
            throw ZlinkException.fromLastError("zlink_msg_set_reply");
    }

    /**
     * Retrieve request-reply information from the message.
     * @return long array: [msgType, correlationId]. msgType: 0=DATA, 1=REQUEST, 2=REPLY.
     */
    public long[] getRequestInfo() {
        if (!valid) throw new IllegalStateException("message is closed");
        try (Arena arena = Arena.ofConfined()) {
            MemorySegment typeOut = arena.allocate(ValueLayout.JAVA_BYTE);
            MemorySegment idOut = arena.allocate(ValueLayout.JAVA_LONG);
            int rc = NativeMsg.msgGetRequestInfo(msg, typeOut, idOut);
            if (rc != 0)
                throw ZlinkException.fromLastError("zlink_msg_get_request_info");
            return new long[] { Byte.toUnsignedLong(typeOut.get(ValueLayout.JAVA_BYTE, 0)),
                                idOut.get(LONG_LE, 0) };
        }
    }
     ...
     * Keys below 0x0100 are reserved and will throw.
     */
    public void setMetadata(int key, byte[] value) {
        if (!valid) throw new IllegalStateException("message is closed");
        if (key < METADATA_KEY_USER_MIN)
            throw new IllegalArgumentException("metadata key must be >= 0x0100");
        try (Arena arena = Arena.ofConfined()) {
            MemorySegment valueSeg = (value != null && value.length > 0)
                ? arena.allocateFrom(ValueLayout.JAVA_BYTE, value)
                : MemorySegment.NULL;
            long valueSize = (value != null) ? value.length : 0;
            int rc = NativeMsg.msgSetMetadata(msg, (short) key, valueSeg, valueSize);
            if (rc != 0)
                throw ZlinkException.fromLastError("zlink_msg_set_metadata");
        }
    }

    /**
     * Retrieve a metadata value by key. Returns null if absent.
     */
    public byte[] getMetadata(int key) {
        if (!valid) throw new IllegalStateException("message is closed");
        try (Arena arena = Arena.ofConfined()) {
            MemorySegment sizeOut = arena.allocate(ValueLayout.JAVA_LONG);
            MemorySegment ptr = NativeMsg.msgGetMetadata(msg, (short) key, sizeOut);
