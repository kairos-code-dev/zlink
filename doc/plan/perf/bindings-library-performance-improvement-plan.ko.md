...
| 5 | Go | `bindings/go/perf` | `tcp/ws/wss/tls single 통과 (6.0.3 재검증, SPOT large 회귀 수정, latency 발산은 측정 아티팩트로 판정)` | `2026-05-25 추가 수정으로 ROUTER_ROUTER/DEALER_ROUTER/PUBSUB 전 transport 통과, DD large 통과, SPOT/SPOT_REQREP/SPOT_SENDSEND 통과. 잔여 보류: MULTI_DEALER_DEALER ws/wss/tls 64B FFI` | 잔여 보류 추적: DD 64B small FFI 경계 비용 분리 |
| 6 | Rust | `bindings/rust/perf` | `tcp/ws/wss/tls 측정 완료, routed large와 SPOT/outlier 보류 남음` | `tcp/ws/wss/tls 측정 완료, SPOT small과 일부 PUBSUB small 보류 남음` | Rust 완료 아님. single outlier와 multi SPOT/PUBSUB hot path를 후속 재검토 |
...
| `ws` | `MULTI_DEALER_DEALER` | `보류(37.8%)` | `통과(55.2%)` | `통과(67.7%)` | `통과(59.2%)` | `통과(58.7%)` | `통과(64.4%)` | 64B는 fresh C `perf_c_multi_linux_20260525_115320.txt` 대비 current Go `perf_go_multi_linux_20260525_115319.txt` 기준이다. 256B/1024B는 Go `perf_go_multi_linux_20260524_222134.txt`, 65536/131072/262144B는 Go `perf_go_multi_linux_20260525_001654.txt` median 기준이다. C처럼 단일 poll loop와 pending socket만 `POLLOUT`으로 두는 poll set 의미를 유지했다. client active send는 65536B/131072B에서 public `Bytes(...)` 경로를 사용해 submit 전 native `Message` 생성을 줄였다. 262144B는 기존 경로로도 current HEAD에서 통과권이 유지된다. 64B는 최신 같은 조건 재측정에서도 Go 단순 one-way 최소 기준보다 낮다. |
...
- **2026-05-23 근본 원인 규명 + 수정 (strace 프로파일링)**: 위 large many-client 병목의 진짜 원인을 strace로 특정했다.
  ...
  - **남은 보류(수정 후에도 목표 미달, 후속 개선 대상)**: `MULTI_DEALER_DEALER`의 ws/wss/tls 64B copy/FFI 비용.
    일부 64B per-message FFI cost는 LockOSThread와 hot-path 축소로 부분 개선됐으나 목표 미만이라 계속 추적한다.
  ...
  - **`ws/wss/tls MULTI_DEALER_DEALER 64B` fresh 재확인 및 `GOMAXPROCS=8` 후보 기각**: 같은 조건 제한 재측정
...
| `tcp` | `MULTI_PUBSUB` | `보류(28.3%)` | `보류(14.2%)` | `보류(13.8%)` | `통과(50.2%)` | `통과(92.0%)` | `통과(101.8%)` | 64/256/1024/65536B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_105401.txt` 대비 Rust `perf_rust_multi_linux_20260525_105401.txt` median 기준이다. 131072/262144B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_022108.txt` 대비 Rust `perf_rust_multi_linux_20260525_022246.txt` 기준이다. server active publish는 payload `Vec`에 stamp한 뒤 `Message::copy_from`으로 다시 복사하던 경로를 `Message::with_size()` + `data_mut()` 직접 stamp로 바꿨다. client drain은 `TopicMessage::empty()` placeholder를 메시지마다 새로 만들지 않고 drain 호출 안에서 재사용해 caller-provided output 의미를 유지하면서 할당을 줄였다. 2026-05-25 추가로 latency 계산/샘플 저장을 기본 32개당 1개로 줄여 throughput count는 전 메시지를 유지하면서 hot path의 timestamp/vector push 비용을 낮췄고, tcp 65536B는 단순 one-way 기준을 넘었다. 64/256/1024B는 여전히 낮다. |
...
| `ws` | `MULTI_PUBSUB` | `통과(94.6%)` | `통과(94.0%)` | `보류(10.9%)` | `통과(83.9%)` | `통과(103.9%)` | `통과(125.3%)` | 64/256/1024/65536B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_105449.txt` 대비 Rust `perf_rust_multi_linux_20260525_105449.txt` 기준이다. 131072/262144B는 기존 Rust `perf_rust_multi_linux_20260522_152055_codex_rust_multi_ws_pubsub_ready_fix_full_20260522.txt` 기준이다. PUBSUB client가 `CLIENT_READY`를 너무 일찍 출력하던 문제는 `SocketMonitor` connection-ready 대기를 추가해 131072B/262144B timeout을 해소했다. latency sampling 적용 뒤에도 64/256/65536B는 통과권을 유지하고, 1024B는 낮아 보류한다. 262144B는 C보다 높아 outlier 재검토 대상으로 남긴다. |
| `ws` | `MULTI_SPOT` | `보류(66.6%)` | `보류(67.9%)` | `보류(64.4%)` | `통과(115.5%)` | `통과(84.2%)` | `통과(76.8%)` | 64/256/1024B는 fresh C 제한 재측정 `perf_c_multi_linux_20260525_102719.txt` 대비 current Rust `perf_rust_multi_linux_20260525_103041.txt` 기준이다. 65536B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_022730.txt` 대비 Rust `perf_rust_multi_linux_20260525_025506.txt` 기준이다. 131072B는 C `perf_c_multi_linux_20260525_072541.txt` 대비 Rust `perf_rust_multi_linux_20260525_072933.txt` 기준이다. 262144B는 C `perf_c_multi_linux_20260525_093343.txt` 대비 Rust `perf_rust_multi_linux_20260525_093530.txt` 기준이다. SPOT server active publish를 `Message::with_size()` + `data_mut()` 직접 stamp로 바꾼 뒤 65536B가 통과권에 올랐다. 64/256B는 기본 recv worker를 2개로 좁혀 기존 50%대에서 60%대로 올렸지만 아직 SPOT 기준 아래다. 1024B도 fresh C 기준에서는 기준 아래로 확인되어 보류로 되돌린다. 131072B는 public `Poller` `POLLIN` wait 위에서 기본 recv worker를 8개로 좁혀 729.462Kmsg/s, C 대비 84.2%로 통과했다. 262144B도 current HEAD에서 8-worker를 단독 재확인해 405.988Kmsg/s, C 대비 76.8%로 올라 통과했다. SPOT client 복사 제거 + worker drain 위에, 131072B/262144B에서만 public `Poller` `POLLIN` wait를 사용한다. |
...
| `tls` | `MULTI_PUBSUB` | `통과(84.0%)` | `통과(81.2%)` | `보류(8.4%)` | `통과(77.0%)` | `통과(101.8%)` | `통과(91.3%)` | 64/256/1024/65536B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_105449.txt` 대비 Rust `perf_rust_multi_linux_20260525_105449.txt` 기준이다. 131072/262144B는 C `perf_c_multi_linux_20260522_154822_codex_c_multi_tls_no_stream_for_rust_20260522.txt` 대비 Rust `perf_rust_multi_linux_20260522_155342_codex_rust_multi_tls_pubsub_20260522.txt` 기준이다. client drain의 `TopicMessage` placeholder 재사용 뒤 65536B 이상은 기준선을 넘었다. latency sampling 적용 뒤 256B도 통과권으로 올라왔고, 1024B는 최신 재측정에서 기준보다 낮다. |
| `tls` | `MULTI_SPOT` | `보류(61.0%)` | `보류(59.1%)` | `보류(55.2%)` | `통과(151.4%)` | `통과(109.3%)` | `통과(103.1%)` | 64/256/1024B는 fresh C 제한 재측정 `perf_c_multi_linux_20260525_102719.txt` 대비 current Rust `perf_rust_multi_linux_20260525_103041.txt` 기준이다. 65536/131072B는 C `perf_c_multi_linux_20260522_154822_codex_c_multi_tls_no_stream_for_rust_20260522.txt` 대비 Rust `perf_rust_multi_linux_20260524_201915.txt` 기준이다. 262144B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_014732.txt` 373.2K 대비 Rust `perf_rust_multi_linux_20260525_014728.txt` 384.928K median 기준이다. SPOT client 복사 제거 + 4-worker drain 재측정으로 65536B/131072B는 통과권에 올랐고, 262144B도 최신 같은 조건 C 기준으로 통과한다. tls 64/256/1024B는 기본 recv worker를 2개로 좁혀 절대 처리량을 올렸지만 fresh C 기준에는 아직 낮다. |
...
- **남은 보류(Rust)**: `MULTI_SPOT` ws/wss/tls 64/256/1024B, `MULTI_PUBSUB` small, single routed large.
...
| `tcp` | `MULTI_PUBSUB` | `보류(5.4%)` | `보류(5.8%)` | `보류(12.5%)` | `통과(40.8%)` | `통과(90.9%)` | `통과(95.0%)` | 64/256/1024/65536B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_110110.txt` 대비 Python `perf_python_multi_linux_20260525_111359.txt` median 기준이다. 131072/262144B는 기존 `perf_python_multi_linux_20260524_191738.txt` 기준이다. subscriber의 metric header decode를 `to_bytes_list()` 대신 `TopicMessage.data` view로 바꾼 뒤 large size는 더 올라갔다. 2026-05-25 추가로 throughput count는 전 메시지를 유지하고 latency 계산/리스트 저장만 기본 32개당 1개로 줄였다. 기존 동작에 해당하는 `PERF_MULTI_PUBSUB_LATENCY_SAMPLE_STRIDE=1` 재측정 `perf_python_multi_linux_20260525_111617.txt`보다 tcp 64/256/1024/65536B 절대 처리량이 모두 높아 기본값으로 유지한다. small size는 아직 기준보다 낮다. |
...
| `ws` | `MULTI_PUBSUB` | `보류(6.3%)` | `보류(6.0%)` | `보류(7.2%)` | `통과(76.3%)` | `통과(77.0%)` | `통과(116.1%)` | 64/256/1024/65536B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_110110.txt` 대비 Python `perf_python_multi_linux_20260525_111359.txt` median 기준이다. 131072/262144B는 기존 full 기준이다. 1024B client timeout은 active 수집 실패가 아니라 일부 subscriber가 wire-level stop token을 놓치며 무한 대기하던 종료 조건 문제였고, bounded stop wait로 complete를 유지한다. latency sampling 적용 뒤 65536B는 통과권을 유지하지만 64/256/1024B는 아직 낮다. |
...
| `wss` | `MULTI_PUBSUB` | `보류(5.4%)` | `보류(5.3%)` | `보류(8.9%)` | `통과(111.4%)` | `통과(81.2%)` | `통과(99.6%)` | 64/256/1024/65536B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_110110.txt` 대비 Python `perf_python_multi_linux_20260525_111359.txt` median 기준이다. 131072/262144B는 기존 full 기준이다. TLS 설정 누락과 stop token 유실 시 무한 대기하던 종료 조건을 보강한 상태에서 complete를 유지한다. latency sampling 적용 뒤 65536B는 C보다 높게 재측정됐지만 small size는 낮다. |
...
| `tls` | `MULTI_PUBSUB` | `보류(5.5%)` | `보류(5.2%)` | `보류(7.8%)` | `통과(100.8%)` | `통과(99.7%)` | `통과(88.9%)` | 64/256/1024/65536B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_110110.txt` 대비 Python `perf_python_multi_linux_20260525_111359.txt` median 기준이다. 131072/262144B는 기존 full 기준이다. TLS 설정 누락과 stop token 유실 시 무한 대기하던 종료 조건을 보강한 상태에서 complete를 확보했고, latency sampling 적용 뒤 65536B는 C보다 높게 재측정됐다. small size는 여전히 낮다. |
