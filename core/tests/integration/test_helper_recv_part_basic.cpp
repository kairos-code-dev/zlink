...
zlink_recv_result_t recv_router_part_with_retry (
  void *router_,
...
zlink_recv_result_t recv_subscribe_part_with_retry (
  void *sub_,
  char *topic_id_buf_,
  ...
  zlink_part_flag_t *has_more_out_)
{
    ...
    while (std::chrono::steady_clock::now () < deadline) {
        const zlink_recv_result_t rc =
          zlink_subscribe_part (sub_, NULL, topic_id_buf_, topic_id_capacity_,
                                topic_id_len_out_, part_out_, has_more_out_,
                                static_cast<zlink_recv_flags_t> (0));
                          ...
                          reply_probe_t *probe_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_dealer_request (dealer_, parts, 2, &capture_reply, probe_,
                            static_cast<zlink_send_flags_t> (0), 3000));
}

void send_dealer_request_single (void *dealer_,
                                 const char *part0_,
                                 reply_probe_t *probe_)
{
    zlink_msg_t part;
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_dealer_request (dealer_, &part, 1, &capture_reply, probe_,
                            static_cast<zlink_send_flags_t> (0), 3000));
}
                        ...
                        const char *payload_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_router_reply (router_, peer_rid_, request_seq_, &part, 1));
}
...
void test_recv_part_reads_each_part_and_tracks_has_more ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_set_routing_id (receiver, "D1", 2));
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_bind (router, "inproc://helper-recv-part-basic"));
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_connect (receiver, "inproc://helper-recv-part-basic"));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_send_rid (router, source_rid, parts, 2,
                      static_cast<zlink_send_flags_t> (0)));
    msleep (SETTLE_TIME);
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        TEST_ASSERT_EQUAL_MEMORY (expected[i], zlink_msg_data (&part),
                                  strlen (expected[i]));
...
void test_router_recv_part_metadata_view_invalidates_on_next_recv_like_call ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_bind (router, "inproc://helper-router-recv-part-basic"));
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_connect (dealer1, "inproc://helper-router-recv-part-basic"));
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_connect (dealer2, "inproc://helper-router-recv-part-basic"));
    ...
    TEST_ASSERT_EQUAL_INT (
      ZLINK_RECV_NO_DATA,
      zlink_router_recv (router, &primed_source_rid, &primed_source_spot_rid,
    ...
    TEST_ASSERT_FALSE (
      ...
      && memcmp (first_source_copy.data, first_source_rid->data,
                 first_source_copy.size)
           == 0);
...
void test_subscribe_part_reports_needed_topic_size_and_recovers_after_emsgsize ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_bind (pub, "inproc://helper-subscribe-part-basic"));
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_connect (sub, "inproc://helper-subscribe-part-basic"));
    TEST_ASSERT_SUCCESS_ERRNO (
...
int main (void)
{
    ...
    RUN_TEST (
      test_subscribe_part_reports_needed_topic_size_and_recovers_after_emsgsize);
