...
|---|---|---|
| **SpotNode 수신** | `zlink_subscribe_recv()` | `zlink_subscribe_handler()` 콜백 |
| **Spot 수신** | `zlink_subscribe_recv()` | `zlink_subscribe_handler()` 콜백 |
...
- callback 모드 전환 후 `zlink_subscribe_recv()`는
  `EBUSY`로 실패합니다.
