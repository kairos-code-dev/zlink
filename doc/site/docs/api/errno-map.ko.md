...
| Actor join completion (전용 typedef) | `zlink_spot_node_actor_join_spot` (`zlink_actor_join_handler_fn`) |
...
| 분류 | 함수 |
|---|---|
| Bind | `zlink_bind`, `zlink_spot_node_bind`, `zlink_registry_bind` |
...
| `zlink_config_result_t` | `zlink_ctx_set`, `zlink_ctx_auto_hwm_recalculate`, 메시지 lifecycle 함수(`zlink_msg_init` 계열 + `zlink_msg_adopt`), socket 옵션/routing/subscription 설정 함수 전체, attach 함수 전체, SpotNode lifecycle/lookup 함수 전체, registry/discovery 설정 함수 전체, snapshot/query 함수 전체, poller 변경 함수 전체, `zlink_proxy`, `zlink_proxy_steerable`, `zlink_timer_start`, `zlink_timer_stop`, `zlink_monitor_snapshot` |
