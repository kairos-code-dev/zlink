// ROUTER-ROUTER-POLL benchmark: router->router with poller-driven receive.
                ...
                perf::single::latency_stats_t *latency_out)
{
    ...
    (void) poller.add (receiver, zlink::poll_event::pollin);
    ...
    auto do_one = [&] () -> bool {
        ...
        const int poll_rc = poller.wait (events, recv_timeout_ms);
        if (poll_rc == 0)
            return true;
        if (poll_rc < 0)
            return false;
                                     ...
                                     const std::string &lib_name)
{
    if (!perf::single::transport_available (transport)) {
        std::cout << "UNSUPPORTED,ROUTER_ROUTER_POLL," << transport << std::endl;
        return;
    }
    ...
    const std::string bind_id = "RRP_BIND";
    const std::string conn_id = "RRP_CONN";
