...
=== "C++"
    ...
    `ZLink Framework`는 **실시간 메시징 프레임워크**다. 다른 언어에서는 `ASP.NET Core`나
    Spring Boot 위에 계층으로 올라가지만, **C++에는 그런 표준 애플리케이션 프레임워크가
    없다.** 그래서 C++ framework는 DI·설정·HTTP hosting을 함께 제공하고 프로세스 자체를
    구성한다. 쓰던 모델 안에 들어가는 대신 그 모델을 같이 준다.
...
=== "C++"
    ...
    node.listen ("tcp://0.0.0.0:9001");
    node.channel_name ("game.room").server ();  // mesh는 최소 1개 logical membership을 갖는다
    ...
    {
        _board.mark (request.number);           // lock 없음
...
=== "C++"
    ...
    {
        co_return co_await start_workflow (request);   // spot 상태에 lock 없이 접근
    }
    ```
...
=== "Node/TypeScript"
    ...
    export class StartOrderWorkflowHandler
      ...
      async handle(
        ...
        return spot.start(request);                    // spot 상태에 lock 없이 접근
      }
    }
    ```
...
=== "C++"
    ...
    | 동시 요청의 상태 보호 | SPOT의 직렬 실행으로 lock 없이 상태 관리 |
    | 서비스 생성·의존성 관리 | framework DI 컨테이너에서 handler, client, filter를 생성 |
    | 서버 주소 관리·연결 결정 | location store를 통해 현재 활성 endpoint 추적 |
    | 설정·로그·모니터링 | framework 내장 config·logging·hosted service |
...
=== "Node/TypeScript"
    ...
    | 로깅·검증·권한 확인 같은 공통 처리 반복 | HTTP route는 middleware, ZLink handler는 `ZLinkHandlerFilter`로 분리 |
    | 동시 요청의 상태 보호 | SPOT의 직렬 실행으로 lock 없이 상태 관리 |
    | 서비스 생성·의존성 관리 | NestJS DI에서 handler, client, filter를 생성 |
    | 서버 주소 관리·연결 결정 | location store를 통해 현재 활성 endpoint 추적 |
...
=== "C++"
    ...
    class get_price_handler_t
    {
      public:
        ...
        {
            return price_reply_t{request.symbol, 187.42};   // 187.42는 데모용 고정값(실제론 조회 결과)
        }
    };

    // 등록 — MeshNode endpoint와 price membership의 handler를 함께 선언한다.
    app.add_zlink_framework ([] (zlink_framework_options_t &options) {
        options.add_route_mesh ("services")                 // MeshName으로 통신 범위를 구분한다.
          .listen ("tcp://0.0.0.0:7301")                    // 이 MeshNode의 endpoint를 연다.
          .set_routing_id (routing_id_t::from ("price-1"))
          .channel_name ("price")                           // price 처리 membership을 등록한다.
          .server ()
...
=== "Node/TypeScript"
    ...
    export class GetPriceHandler implements ZLinkRequestHandler<PriceRequest, PriceReply> {
      ...
      async handle(request: PriceRequest): Promise<PriceReply> {
        return { symbol: request.symbol, price: 187.42 };   // 187.42는 데모용 고정값(실제론 조회 결과)
      }
    }
    ...
    const reply = await client
      .requestToChannel(
        'price',                                            // process-local로 찾을 ChannelName
        priceRequest('AAPL'))
      .submit<PriceReply>();                                // 송신한 뒤 reply를 비동기로 기다린다.
    ```
...
=== "C++"
    ...
    ```cpp
    app.add_zlink_framework ([] (zlink_framework_options_t &options) {
        options.add_location_store (std::make_shared<redis_location_store_t> (...));  // node·actor·spot 위치정보 제공 — 이 정보를 기반으로 node 간 연결은 자동

        options.add_route_mesh ("services")                     // 서버 간 request/send용 MeshNode
          .listen ("tcp://0.0.0.0:7301")
          .set_routing_id (routing_id_t::from ("service-a"))
          .channel_name ("orders").server ();                   // 처리할 논리 membership
        options.add_fanout_channel ("events")
          .enable_publisher ("tcp://0.0.0.0:7302");             // classic event fan-out
        options.add_route_mesh ("game.room")                    // SPOT·actor도 MeshNode가 소유
          .listen ("tcp://0.0.0.0:7304")
...
=== "Node/TypeScript"
    ...
    ZLinkModule.forRootFactory({
      useFactory: () => {
        ...
        builder.addLocationStore(new ZLinkRedisLocationStore(...));  // node·actor·spot 위치정보 제공 — 이 정보를 기반으로 node 간 연결은 자동
...
=== "C++"
    ...
    ### DI 컨테이너 — framework가 제공한다

    C++에는 얹을 애플리케이션 프레임워크가 없다. `ASP.NET Core` 스타일의 DI 컨테이너를
    framework가 직접 제공한다. `service_collection_t`에 등록하면 handler 생성자에
    자동으로 주입된다. 수명은 singleton / scoped / transient 셋이다.

    [18장 →](18-di-container.ko.md)

    ### Configuration — framework가 제공한다

    CLI 인자, 환경 변수, JSON 파일을 우선순위 순서로 합성한다. `bind<T>()` 한 번으로
    설정 섹션을 struct에 매핑한다.

    [19장 →](19-configuration.ko.md)

    ### HTTP Hosting — 프로세스 안에 REST API 내장

    별도 웹 서버 없이 같은 프로세스 안에 HTTP endpoint를 올린다. 경로 파라미터, TLS,
    readiness/liveness endpoint를 fluent builder로 선언한다. HTTP handler는 채널 handler와
    같은 `request_type`/`reply_type`/`handle()` 계약을 쓴다.

    [20장 →](20-http-hosting.ko.md)

=== "Java"

    ### DI 컨테이너 — Spring 서비스 의존성 사용

    `ZLinkFrameworkConfigurer` 안에서 등록한 handler, client, filter는 Spring DI와
    같은 컨테이너에서 만들어진다. application 코드는 handler 생성자에 필요한 서비스를 선언하고,
    framework는 dispatch 시점에 필요한 객체를 DI 컨테이너에서 가져온다.

    [3장 →](03-concepts.ko.md)

    ### Configuration — Spring 설정과 함께 사용

    Framework 설정은 `AddZLinkFramework(options => ...)`에서 channel, location store,
    codec, SPOT, STREAM 역할을 선언하는 방식으로 묶는다. 주소나 환경별 값은 일반
    Spring configuration에서 읽어 options에 넘긴다.
...
=== "Node/TypeScript"
    ...
    ### DI 컨테이너 — NestJS 서비스 의존성 사용

    `ZLinkModule.forRootFactory(...)` 안에서 등록한 handler, client, filter는 NestJS DI와
    같은 컨테이너에서 만들어진다. application 코드는 handler 생성자에 필요한 서비스를 선언하고,
    framework는 dispatch 시점에 필요한 객체를 DI 컨테이너에서 가져온다.

    [3장 →](03-concepts.ko.md)

    ### Configuration — NestJS 설정과 함께 사용

    Framework 설정은 `AddZLinkFramework(options => ...)`에서 channel, location store,
    codec, SPOT, STREAM 역할을 선언하는 방식으로 묶는다. 주소나 환경별 값은 일반
    NestJS configuration에서 읽어 options에 넘긴다.
...
=== "C++"
    ...
    | fanout | `AddFanoutChannel`, fanout handler | [05-channel-messaging](05-channel-messaging.ko.md) |
    | SPOT | typed spot factory, Spot context outbound, timer | [06-spot](06-spot.ko.md) |
...
=== "C#/.NET"
    ...
    | 항목 | 값 |
    | --- | --- |
    | 필수 패키지 | `Systems.Zlink` · `Zlink.Framework` · `Zlink.Framework.AspNetCore` |
    | 선택 패키지 | `Zlink.Framework.Locations.Redis` · `Zlink.Framework.Codecs.Protobuf`·`.MessagePack` · `Zlink.HttpClient` |
    | 버전 | framework 계열은 0.9부터 배포한다. core binding과 HTTP client는 각자 버전 계열을 따른다 |
    | namespace | `Zlink.Framework`, `Zlink.Framework.Contracts.*` |
    | 등록 진입점 | `builder.Services.AddZLinkFramework(...)` — `Zlink.Framework.AspNetCore`가 제공한다 |
    | public 계약 | `Zlink.Framework.Contracts.*`와 `spec/interfaces/`(언어별 exact interface) |

    설치 절차와 최소 예제는 [02-getting-started §1](02-getting-started.ko.md#1-설치)이 다룬다.
...
=== "Java"
    ...
    설치 절차와 최소 예제는 [02-getting-started](02-getting-started.ko.md)가 다룬다.
...
=== "Kotlin"
    ...
    | `systems.zlink:zlink-http-client` | HTTP 요청을 보내는 쪽 |
    | `systems.zlink:zlink-framework-testkit` | E2E 테스트 |
    | `systems.zlink:zlink-framework-kotlin` | coroutine idiom 레이어. 선택이다 |

    설치 절차와 최소 예제는 [02-getting-started](02-getting-started.ko.md)가 다룬다.

    **런타임은 `zlink-framework-core` 하나다.** `zlink-framework-kotlin`은 `suspend`·`Flow`
    표면을 얹는 얇은 레이어이지 별도 구현이 아니다.
...
=== "C++"
    ...
    - 타입 이름은 `_t` 접미사를 쓴다. 예: `spot_context_t`, `message_context_t`.
...
=== "Java"
    ...
    - Java framework public 타입은 `ZLink` prefix(대문자 `L`)를 쓴다. 예:
    ...
    - client 측 Stream Connector는 서버 framework 아티팩트에 의존하지 않는 독립 라이브러리다.
    - 하부 zlink core C API는 `zlink_*` snake_case다.
...
=== "Node/TypeScript"
    ...
    - 계약 타입은 `ZLink` prefix(대문자 `L`)를 쓴다. 예: `ZLinkRouteClient`.
...
=== "Java"
    ...
    이 가이드가 설명하는 public API는 [spec/interfaces 목차](../../../common/spec/server/languages/java/interfaces/README.ko.md)의 계약
    카탈로그를 따른다. 구현이 진행되는 동안에도 인터페이스의 모양과 동사(`requestToChannel`,
...
=== "Kotlin"
    ...
    이 가이드가 설명하는 public API는 [spec/interfaces 목차](../../../common/spec/server/languages/java/interfaces/README.ko.md)의 계약
    카탈로그를 따른다. 구현이 진행되는 동안에도 인터페이스의 모양과 동사(`requestToChannel`,
    `submit`, `await`, `addRequestHandler` 등)는 spec 문서를 기준으로 확인한다. 세부
    필드까지 정확한 정식 정의가 필요하면 항상 spec 문서를 교차 참조한다.
...
=== "Java"
    ...
    13. [17-alternative](17-alternative.ko.md) — **ZLink를 어디에 쓰나**(사용처·문제 신호·기술 선택 경계)
    14. [공통 샘플](../../../common/sample/README.ko.md) — 대표 업무 시나리오와 검증 기준
    15. [Java exact interface 목차](../../../common/spec/server/languages/java/interfaces/README.ko.md) — 정식 계약
