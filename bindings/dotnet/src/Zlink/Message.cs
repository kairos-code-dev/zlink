...
public sealed class Message : IDisposable, IAsyncDisposable
{
    ...
    /// </summary>
    public void SetRequest(ulong correlationId)
    {
        EnsureValid();
        int rc = NativeMethods.zlink_msg_set_request(ref _msg, correlationId);
        if (rc != 0)
            throw ZlinkException.FromLastError();
    }
    ...
    /// </summary>
    public void SetReply(ulong correlationId)
    {
        EnsureValid();
        int rc = NativeMethods.zlink_msg_set_reply(ref _msg, correlationId);
        if (rc != 0)
            throw ZlinkException.FromLastError();
    }
    ...
    /// </summary>
    /// <returns>Tuple of (msgType, correlationId). msgType: 0=DATA, 1=REQUEST, 2=REPLY.</returns>
    public (byte MsgType, ulong CorrelationId) GetRequestInfo()
    {
        EnsureValid();
        int rc = NativeMethods.zlink_msg_get_request_info(ref _msg,
            out byte msgType, out ulong correlationId);
        if (rc != 0)
            throw ZlinkException.FromLastError();
        return (msgType, correlationId);
    }
    ...
    /// Keys below 0x0100 are reserved for zlink internal use.
    /// </summary>
    public unsafe void SetMetadata(ushort key, ReadOnlySpan<byte> value)
    {
        EnsureValid();
        fixed (byte* ptr = value)
        {
            int rc = NativeMethods.zlink_msg_set_metadata(ref _msg, key,
                value.Length > 0 ? (IntPtr)ptr : IntPtr.Zero,
                (nuint)value.Length);
            if (rc != 0)
                throw ZlinkException.FromLastError();
        }
    ...
    /// </summary>
    public unsafe byte[]? GetMetadata(ushort key)
    {
        EnsureValid();
        IntPtr ptr = NativeMethods.zlink_msg_get_metadata(ref _msg, key,
            out nuint size);
        if (ptr == IntPtr.Zero)
            return null;
        byte[] result = new byte[(int)size];
        new ReadOnlySpan<byte>((void*)ptr, (int)size).CopyTo(result);
        return result;
    }
