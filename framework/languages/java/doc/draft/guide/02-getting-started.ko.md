...
# Draft -- Java Getting Started

> 이 문서는 **구현 전 초안**이다.
> 첫 channel request를 붙이는 흐름을 Spring Boot 기준으로 보여 준다.
...
public class PriceServerConfig implements ZLinkFrameworkOptionsCustomizer {
    ...
    public void customize(ZLinkFrameworkOptions options) {
        ...
        options.useDiscovery(registry -> registry.add("tcp://127.0.0.1:5551"));
    }
}
```
...
[channel 샘플 §2](../channel-messaging-samples.ko.md)에 있다.
...
- pub/sub는 [channel messaging](../spring-boot-channel-messaging.ko.md)을 본다.
