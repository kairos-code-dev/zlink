...
int main(void)
{
    ...
    CHECK(zlink_recv_part(receiver, &rid, &empty_part, &has_more,
                          ZLINK_RECV_FLAGS_DONTWAIT) == ZLINK_RECV_NO_DATA);

    zlink_msg_t outbound;
    CHECK(make_part(&outbound, "hello-c") == 0);
    CHECK(zlink_send_part(sender, &outbound, ZLINK_SEND_FLAGS_NONE,
                          ZLINK_PART_FINAL) == ZLINK_SUBMIT_OK);

    zlink_msg_t inbound;
    CHECK(zlink_recv_part(receiver, &rid, &inbound, &has_more,
    ...
    CHECK(zlink_spot_node_spot_get_or_new(node, &room_rid, &room_a,
                                          &created_a) == ZLINK_CONFIG_OK);
    CHECK(zlink_spot_node_spot_get_or_new(node, &room_rid, &room_b,
                                          &created_b) == ZLINK_CONFIG_OK);
    CHECK(room_a != NULL);
    CHECK(room_b != NULL);
    CHECK(created_a == 1);
    CHECK(created_b == 0);
