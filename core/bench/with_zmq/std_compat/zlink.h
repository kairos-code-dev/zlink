...
{
    const char *v = std::getenv ("BENCH_DEBUG");
...
#define ZLINK_TLS_CERT ZLINK_OPT_TLS_CERT
#endif
#ifndef ZLINK_TLS_KEY
#define ZLINK_TLS_KEY ZLINK_OPT_TLS_KEY
#endif
#ifndef ZLINK_TLS_CA
#define ZLINK_TLS_CA ZLINK_OPT_TLS_CA
#endif
#ifndef ZLINK_TLS_HOSTNAME
#define ZLINK_TLS_HOSTNAME ZLINK_OPT_TLS_HOSTNAME
#endif
#ifndef ZLINK_TLS_TRUST_SYSTEM
#define ZLINK_TLS_TRUST_SYSTEM ZLINK_OPT_TLS_TRUST_SYSTEM
#endif
...
#endif
#ifndef ZLINK_STREAM
#define ZLINK_STREAM ZMQ_STREAM
#endif
#ifndef ZLINK_SNDBUF
#define ZLINK_SNDBUF ZMQ_SNDBUF
#endif
#ifndef ZLINK_RCVBUF
#define ZLINK_RCVBUF ZMQ_RCVBUF
#endif
#ifndef ZLINK_BACKLOG
#define ZLINK_BACKLOG ZMQ_BACKLOG
#endif
#ifndef ZLINK_RCVMORE
#define ZLINK_RCVMORE ZMQ_RCVMORE
#endif
#ifndef ZLINK_EVENTS
#define ZLINK_EVENTS ZMQ_EVENTS
#endif
#ifndef ZLINK_TYPE
#define ZLINK_TYPE ZMQ_TYPE
#endif
#ifndef ZLINK_LINGER
#define ZLINK_LINGER ZMQ_LINGER
#endif
#ifndef ZLINK_SNDHWM
...
#define ZLINK_UNSUBSCRIBE ZMQ_UNSUBSCRIBE
#endif
#ifndef ZLINK_SNDMORE
#define ZLINK_SNDMORE ZMQ_SNDMORE
#endif
#ifndef ZLINK_BLOCKY
#define ZLINK_BLOCKY ZLINK_CTX_OPT_BLOCKY
#endif
...
#define ZLINK_TCP_NODELAY ZMQ_TCP_NODELAY
#endif
#endif
...
#ifndef ZLINK_EVENT_CONNECTION_READY
#define ZLINK_EVENT_CONNECTION_READY ZLINK_EVENT_CONNECTION_READY_CHANGED
#endif
...
struct zlink_monitor_handle_t
{
    ...
    std::atomic<uint32_t> synthetic_ready_pending;
    int socket_type;
};
...
inline void zlink_mark_synthetic_monitor_ready (void *socket_)
{
    if (!socket_)
        return;

    std::lock_guard<std::mutex> lock (g_zlink_monitor_registry_sync);
    for (std::set<void *>::const_iterator it = g_zlink_monitor_registry.begin ();
         it != g_zlink_monitor_registry.end ();
         ++it) {
        zlink_monitor_handle_t *handle =
          static_cast<zlink_monitor_handle_t *> (*it);
        if (handle && handle->owner == socket_) {
            handle->synthetic_ready_pending.fetch_add (
              1, std::memory_order_acq_rel);
            if (zlink_std_compat::bench_debug_enabled ()) {
                std::fprintf (stderr,
                              "zlink-compat: synthetic ready queued owner=%p pending=%u\n",
                              socket_,
                              static_cast<unsigned> (
                                handle->synthetic_ready_pending.load (
...
{
    const int rc = zmq_connect (socket_, endpoint_);
    if (rc == 0)
        zlink_mark_synthetic_monitor_ready (socket_);
    return rc;
}
...
inline int zlink_socket_peers (void *socket_,
                               ...
                               size_t *count_inout_)
{
    ...
    const size_t count = *count_inout_;
    for (size_t i = 0; i < count; ++i)
        std::memset (&peers_[i], 0, sizeof (peers_[i]));
    *count_inout_ = 0;
    return 0;
}
...
inline int zlink_set_option (void *socket_,
                             ...
                             size_t optvallen_)
{
    return zmq_setsockopt (
      zlink_resolve_socket_handle (socket_), option_, optval_, optvallen_);
}
...
inline int zlink_setsockopt (void *socket_,
                             zlink_option_t option_,
                             const void *optval_,
                             size_t optvallen_)
{
    return zlink_set_option (socket_, option_, optval_, optvallen_);
}

inline int zlink_getsockopt (void *socket_,
                             zlink_option_t option_,
                             void *optval_,
                             size_t *optvallen_)
{
    return zlink_get_option (socket_, option_, optval_, optvallen_);
}
...
inline int zlink_subscription_at (void *handle_,
                                  size_t index_,
                                  char *filter_out_,
...
inline int zlink_set_router_option (void *socket_,
                                    int option_,
                                    const void *optval_,
                                    size_t optvallen_)
{
...
inline int zlink_msg_init_data (zlink_msg_t *msg_,
                                void *data_,
                                size_t size_,
                                zmq_free_fn *ffn_,
                                void *hint_)
...
inline int zlink_poll (zlink_pollitem_t *items_, int nitems_, long timeout_)
{
    ...
    for (int i = 0; i < nitems_; ++i) {
        ...
        zlink_monitor_handle_t *monitor = zlink_monitor_from_handle (items_[i].socket);
        if (monitor
            && monitor->synthetic_ready_pending.load (std::memory_order_acquire)
                 > 0) {
            raw_items[i].socket = NULL;
            raw_items[i].fd = 0;
            raw_items[i].events = 0;
            if ((items_[i].events & ZLINK_POLLIN) != 0) {
                items_[i].revents = ZLINK_POLLIN;
                ++synthetic_ready;
                if (zlink_std_compat::bench_debug_enabled ()) {
                    std::fprintf (stderr,
                                  "zlink-compat: synthetic poll ready socket=%p pending=%u\n",
                                  items_[i].socket,
                                  static_cast<unsigned> (
                                    monitor->synthetic_ready_pending.load (
                                      std::memory_order_acquire)));
                }
            }
        } else if (monitor && zlink_std_compat::bench_debug_enabled ()) {
            std::fprintf (stderr,
                          "zlink-compat: monitor without synthetic ready socket=%p pending=%u\n",
                          items_[i].socket,
                          static_cast<unsigned> (
                            monitor->synthetic_ready_pending.load (
                              std::memory_order_acquire)));
    ...
    const long poll_timeout = synthetic_ready > 0 ? 0 : timeout_;
    const int rc = zmq_poll (&raw_items[0], nitems_, poll_timeout);
    if (rc < 0)
        return rc;

    int ready = synthetic_ready;
    for (int i = 0; i < nitems_; ++i) {
        if (raw_items[i].revents != 0) {
                       ...
                       zlink_send_flags_t flags_)
{
    ...
    for (size_t i = 0; i < part_count_; ++i) {
        const int flags = static_cast<int> (
          flags_ | ((i + 1 < part_count_) ? ZMQ_SNDMORE : 0));
...
inline int zlink_send (void *s_,
                       ...
                       int flags_)
{
    return zmq_send (zlink_resolve_socket_handle (s_), buf_, size_, flags_);
}
                           ...
                           zlink_send_flags_t flags_)
{
    ...
    if (target_rid_->size > 0) {
        std::memcpy (
          zmq_msg_data (&rid_msg), target_rid_->data, target_rid_->size);
    }
    ...
    if (frame_count > 0
        ...
        if (source_rid_out_) {
            const size_t rid_size =
              std::min (static_cast<size_t> (255), zmq_msg_size (&frames[0]));
...
{
    return zmq_recv (zlink_resolve_socket_handle (s_), buf_, len_, flags_);
}
                              ...
                              int flags_)
{
    ...
    if (zmq_msg_send (&rid_msg, s_, flags_ | ZMQ_SNDMORE) < 0) {
        zmq_msg_close (&rid_msg);
        return -1;
    }

    return zmq_send (
      zlink_resolve_socket_handle (s_), payload_, payload_size_, flags_);
}
                          ...
                          zlink_send_flags_t flags_)
{
    ...
    if (zmq_msg_send (&topic_msg, subject_, static_cast<int> (flags_ | ZMQ_SNDMORE))
        < 0) {
                                         ...
                                         zlink_socket_monitor_event_t *out_)
{
    ...
    if (handle_->synthetic_ready_pending.load (std::memory_order_acquire) > 0) {
        handle_->synthetic_ready_pending.fetch_sub (1, std::memory_order_acq_rel);
        std::memset (out_, 0, sizeof (*out_));
        out_->event = ZLINK_EVENT_CONNECTION_READY_CHANGED;
        out_->value = handle_->ready_count.fetch_add (1, std::memory_order_acq_rel) + 1;
        if (zlink_std_compat::bench_debug_enabled ()) {
            std::fprintf (stderr,
                          "zlink-compat: synthetic monitor recv owner=%p value=%llu pending=%u\n",
                          handle_->owner,
                          static_cast<unsigned long long> (out_->value),
                          static_cast<unsigned> (
                            handle_->synthetic_ready_pending.load (
                              std::memory_order_acquire)));
        }
        return 0;
    }
    ...
    const size_t remote_size =
      std::min (static_cast<size_t> (255), zmq_msg_size (&part2));
    if (remote_size > 0) {
  ...
  void *s_, const zlink_socket_monitor_open_options_t *options_)
{
    ...
    const unsigned long seq =
      g_zlink_monitor_seq.fetch_add (1, std::memory_order_relaxed);
    std::snprintf (endpoint, sizeof (endpoint), "inproc://zlink-compat-mon-%lu",
                   seq);
...
inline int zlink_socket_monitor (void *s_,
                                 const char *endpoint_,
                                 int events_)
{
    return zmq_socket_monitor (s_, endpoint_, events_);
}

inline void *zlink_socket_monitor_open (void *s_, int /*events_*/)
{
    zlink_socket_monitor_open_options_t options;
    std::memset (&options, 0, sizeof (options));
    return zlink_socket_monitor_open (s_, &options);
}

inline int zlink_socket_monitor_handler (
  void *monitor_,
...
inline int zlink_monitor_recv (void *monitor_,
                               zlink_monitor_event_t *out_,
                               int /*flags_*/)
{
    return zlink_socket_monitor_recv (monitor_, out_);
}
...
inline int zlink_monitor_close (void **monitor_p_)
{
    if (!monitor_p_ || !*monitor_p_)
        return 0;
    zlink_monitor_handle_t *handle =
      static_cast<zlink_monitor_handle_t *> (*monitor_p_);
    *monitor_p_ = NULL;
    {
        std::lock_guard<std::mutex> lock (g_zlink_monitor_registry_sync);
...
inline int zlink_poller_size (void *poller_)
{
    ...
    return static_cast<int> (
      static_cast<zlink_poller_handle_t *> (poller_)->items.size ());
}

inline int zlink_poller_add (void *poller_,
                             void *socket_,
                             void *user_data_,
                             short events_)
{
                                  ...
                                  long timeout_)
{
    ...
    const int rc =
      zlink_poll (&items[0], static_cast<int> (items.size ()), timeout_);
    if (rc <= 0)
        return rc;
...
inline int zlink_send_ready_handler (void *socket_,
