[English](actor.md) | 한국어

[스펙 목차](../../README.ko.md) · [코어 목차](../README.ko.md) · [서비스 공통](README.ko.md) · [MeshNode](mesh-node.ko.md) · [Spot](spot.ko.md) · [STREAM session](stream-session.ko.md) · [Dispatch](dispatch.ko.md)
...
Actor ID의 생성 수명을 구분한다. membership epoch는 같은 Actor generation이 참여하는 Spot 위치의
변경을 구분하며 ActorRef generation과 독립적이다.
...
join은 target Spot infrastructure/application control claim에 `SPOT_CONTROL` record를 넣는다. `kind_data`는
...
target token의 commit은 framework authority가 확정한 새 membership epoch를 설치하고 target forwarding과
reserved staging 수신을 허용하지만 Actor application dispatch는 시작하지 않는다. committed target token의
activate는 participant flush, session binding과 terminal high-water가 모두 확인된 뒤 Actor ready를 공개하고
...
성공한다. terminal token에 서로 다른 종결 operation을 호출하거나 같은 token의 commit에 다른 epoch를 전달하면
...
[STREAM session service](stream-session.ko.md)가 소유한다. Actor service는 binding transport나 raw
