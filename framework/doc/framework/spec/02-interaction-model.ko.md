...
[10-channel-topology.ko.md](server/10-channel-topology.ko.md)의 section 3을 참고한다.
...
- `SPOT` 쪽 public request는 resolver가 만든 `SpotHandle`을 받는다.
  ...
  ([spot 주소 메시징](server/24-spot-address-messaging.ko.md)).
...
leave, destroy)과 application 로직 분담은 [22-actor-model.ko.md](server/22-actor-model.ko.md)
