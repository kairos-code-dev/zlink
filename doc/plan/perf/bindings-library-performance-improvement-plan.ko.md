...
| Go | 192 | 78.1% | 76.0% | 28.1% | 17.9% | 90.8% | 39.9% |
...
| Transport | Pattern | 64 | 256 | 1024 | 65536 | 131072 | 262144 | 결과 파일 / 메모 |
|-----------|---------|----|-----|------|-------|--------|--------|------------------|
| `tcp` | `MULTI_DEALER_DEALER` | `미달(22.6%)` | `통과(50.7%)` | `통과(66.8%)` | `미달(5.7%)` | `미달(19.8%)` | `미달(11.1%)` | C `perf_c_multi_linux_20260522_020803_codex_c_tcp_multi_for_go_20260522.txt` 대비 Go `perf_go_multi_linux_20260522_034617_codex_go_tcp_multi_dd_submitmessage_20260522.txt`. C처럼 단일 poll loop와 pending socket만 `POLLOUT`으로 두는 poll set 의미를 유지했다. perf helper가 backpressure 실패 메시지를 닫고 native message에 직접 timestamp를 찍도록 해 256B/1024B는 통과로 올랐다. 64B와 large size는 Go 단순 one-way 최소 기준보다 낮아 미달이다. auto-HWM 활성과 size별 `MsgUnit(B)` 일치를 확인했다. |
| `tcp` | `MULTI_DEALER_ROUTER` | `미달(30.3%)` | `미달(29.0%)` | `미달(29.1%)` | `미달(21.1%)` | `미달(27.4%)` | `미달(38.4%)` | C 파일은 위 행과 같다. Go `perf_go_multi_linux_20260522_035216_codex_go_tcp_multi_submitmessage_followup_20260522.txt`. per-socket goroutine/poller 구조에서 발생하던 `Bad address`/`I/O error`를 C와 같은 단일 poll loop로 정렬해 전체 size complete를 확보했다. 새 perf helper 적용 뒤에도 전체 size가 multi routed echo 최소 기준보다 낮아 미달이다. auto-HWM 활성과 size별 `MsgUnit(B)` 일치를 확인했다. |
| `tcp` | `MULTI_ROUTER_ROUTER` | `미달(21.8%)` | `미달(22.6%)` | `미달(21.4%)` | `미달(21.6%)` | `미달(28.1%)` | `미달(37.1%)` | C 파일은 위 행과 같다. Go `perf_go_multi_linux_20260522_035216_codex_go_tcp_multi_submitmessage_followup_20260522.txt`. routed echo server는 수신 `RoutingID` 값을 명시적으로 복사해 `SendTo`하고, client는 C와 같은 단일 poll loop로 정렬했다. 실패 없이 유효 수치를 얻었지만 최소 기준보다 낮아 미달이다. auto-HWM 활성과 size별 `MsgUnit(B)` 일치를 확인했다. |
