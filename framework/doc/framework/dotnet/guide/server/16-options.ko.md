...
| `MailboxMessageBudget` · `MailboxByteBudget` | Spot·Actor 같은 실행 단위 하나가 쌓아 둘 수 있는 message 수와 byte. 넘으면 대기가 아니라 **drop**이다([04-backpressure](04-backpressure.ko.md#4-영향을-주는-옵션)) | 느린 실행 단위가 burst를 더 버틴다 | 밀리는 실행 단위를 일찍 드러낸다 |
