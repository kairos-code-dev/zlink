...
| ATD-A4 | `Client/Scenarios/YdA4WorkerAwaitScenario.cs` | `Client/Scenarios/atd-a4-worker-await-scenario.ts`, `Server/Play/Handlers/basic-spot-handlers.ts`, `Server/Session/Handlers/await-session.ts` | done | `runWorker(...).submit()`와 probe interleaving marker order가 runner에서 통과했다. |
