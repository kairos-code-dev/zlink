...
RAW 프로토콜은 외부 클라이언트가 ZMP(zlink Message Protocol) 프레이밍 없이 연결할 때 사용된다. STREAM 소켓은 이 프로토콜을 사용하여 지원되는 모든 트랜스포트(tcp, tls, ws, wss)에서 임의의 연결을 수락한다.
...
- Length: payload 순수 길이 (4B Big Endian)
- Payload: 애플리케이션 데이터
