...
| 5 | Go | `bindings/go/perf` | `tcp/ws/wss/tls single 통과 (6.0.3 재검증, SPOT large 회귀 수정, latency 발산은 측정 아티팩트로 판정)` | `2026-05-25 추가 수정으로 ROUTER_ROUTER/DEALER_ROUTER/PUBSUB 전 transport 통과, DD large 통과, SPOT large 대폭 개선, SPOT_SENDSEND large 통과(6.6.3). 잔여 보류: SPOT_REQREP tcp 262144B, DD/SPOT 64~1024B FFI` | 잔여 보류 추적: SPOT_REQREP tcp 262144B server reply hot path 재검토, DD/SPOT small FFI 경계 비용 분리 |
...
- **2026-05-23 근본 원인 규명 + 수정 (strace 프로파일링)**: 위 large many-client 병목의 진짜 원인을 strace로 특정했다.
  ...
  - **남은 보류(수정 후에도 목표 미달, 후속 개선 대상)**: `MULTI_DEALER_DEALER`의 wss 64B, tls 64B copy/FFI 비용.
    `MULTI_SPOT_REQREP` tcp 262144B와 `wss MULTI_SPOT 1024B`, 일부 64B per-message FFI cost는
    LockOSThread와 hot-path 축소로 부분 개선됐으나 목표 미만이라 계속 추적한다.
