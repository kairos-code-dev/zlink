...
TEST (ZLinkFrameworkLocationLifecycle, ReleasesActorWithOwnerGenerationToken)
{
    ...
    const auto released =
      lifecycle.release_actor (actor_location_key_t{.actor_type = "player", .actor_id = "actor-1"});
...
TEST (ZLinkFrameworkLocationLifecycle, IgnoresActorOperationsWhenClaimIsNotTracked)
{
    ...
    moved.actor_ref = "node-a:2:actor-missing";
    ...
    const auto renewed =
      lifecycle.renew_actor (
        actor_location_key_t{.actor_type = "player", .actor_id = "actor-missing"});
...
TEST (ZLinkFrameworkLocationLifecycle, UpdatesTrackedActorLocationWithoutChangingOwnerToken)
{
    ...
    ASSERT_EQ (location_write_status_t::stored, claim.status);
    EXPECT_TRUE (
      lifecycle.owns_actor (actor_location_key_t{.actor_type = "player", .actor_id = "actor-1"}));

    auto moved = make_actor ("actor-1");
    moved.actor_ref = "node-a:7:actor-1";
    ...
    const auto stored =
      store.resolve_actor (actor_location_key_t{.actor_type = "player", .actor_id = "actor-1"})
        ...
        .value ();
    ASSERT_TRUE (stored.has_value ());
    EXPECT_EQ ("node-a:7:actor-1", stored->actor_ref);
...
TEST (ZLinkFrameworkLocationLifecycle, RejectsConflictingSpotClaimAndIgnoresUntrackedRelease)
{
    ...
    location_lifecycle_t lifecycle_a (owner_a);
    location_lifecycle_t lifecycle_b (owner_b);
    const auto key = spot_location_key_t{.mesh_name = "node-a",
...
TEST (ZLinkFrameworkLocationLifecycle, DeactivatesTrackedActorWhenOwnershipIsStale)
{
    ...
    const auto stale =
      lifecycle_a.renew_actor (actor_location_key_t{.actor_type = "player", .actor_id = "actor-1"});
