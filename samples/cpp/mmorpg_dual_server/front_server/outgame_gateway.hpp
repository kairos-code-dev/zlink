...
using GatewayResponseHandler = std::function<void (
  const std::string &session_key, const std::string &req_id, const std::string &result)>;
...
class OutgameGateway
{
  public:
    ...
    void send_request (const std::string &session_key,
                       const std::string &op,
