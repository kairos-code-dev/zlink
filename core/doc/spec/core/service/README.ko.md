...
- direct SPOT 주소 지정:
  ...
  저수준 ROUTER socket(`zlink_router_send_spot()` / `zlink_router_request_spot()`)
  양쪽에 모두 있습니다.

일반적인 서비스 호출은 channel 방식으로 처리하고, concrete destination 직접
지정이 정말 필요할 때만 direct `rid` 경로를 사용합니다.
