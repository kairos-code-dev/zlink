...
static void configure_tls_service (void *server_,
                                   void *client_,
                                   const tls_test_files_t &files_)
{
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_set_tls_server (server_, files_.server_cert.c_str (),
                            files_.server_key.c_str (), 0));
    TEST_ASSERT_SUCCESS_ERRNO (
...
{
    const bool debug = getenv ("ZLINK_TEST_DEBUG") != NULL;
    for (size_t transport_index = 0;
         transport_index < sizeof (kTransports) / sizeof (kTransports[0]);
         ++transport_index) {
        const char *transport = kTransports[transport_index];
        ...
        for (size_t i = 0; i < sizeof (kHwmBuckets) / sizeof (kHwmBuckets[0]);
             ++i) {
            if (debug) {
                fprintf (stderr, "[spot] transport=%s sndhwm=%d\n", transport,
                         kHwmBuckets[i]);
            }
            spot_case_t spot;
...
int main (int, char **)
{
    ...
    if (should_run_case ("test_multi_spot_backpressure_matrix"))
        RUN_TEST (test_multi_spot_backpressure_matrix);
    return UNITY_END ();
}
