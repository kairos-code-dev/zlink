...
class route_spot_state_handler_t
{
  public:
    ...
    zlink::framework::http_response_t handle (const zlink::framework::http_request_t &http)
    {
        ...
        const auto owner = e2e::owner_node_rid_for_key (request.key);
        const auto spot_rid = e2e::user_spot_rid_for_key (request.key);
        auto reply =
          _routes
            .request (e2e::route_channel, zlink::routing_id_t::from (owner),
