...
[문서 목록](../../../../node/README.ko.md) | [이전: ZLink Framework NestJS SPOT](nestjs-spot.ko.md) | [다음: ZLink Framework Session Actor Dispatch](session-actor-dispatch.ko.md)
<!-- framework-adapter-nav:end -->

[스펙 목차](../../../../node/README.ko.md)

[node 묶음](../../../../node/README.ko.md) | [channel](nestjs-channel-messaging.ko.md) | [SPOT](nestjs-spot.ko.md) | [STREAM](nestjs-stream.ko.md) | [Registry](nestjs-registry.ko.md)
...
None
  +--(factory.create)-> Created (Entry Spot, unbound)
        ...
        |                 +--(destroyActor: Entry Spot)-> None
        +--(disconnect / unbind: framework)-> Entry Spot, unbound
```
...
- Entry Spot destroy 는 `onLeaveActor(actor)` 또는 다른 lifecycle callback 을 호출하지 않고
  actor 상태만 정리한다. 같은 actor instance 를 다시 destroy 해도 성공으로 끝난다.
...
다른 위치에 존재하는 actor(예: session-attached actor)가 어떤 Spot에 합류하려면
...
[공통 actor 모델](../../actor-model.ko.md) §4 lifecycle 표를
참고한다.
...
`submit()`은 bound session send의 one-way terminator다. 완료 객체를 반환하지 않으며
비동기 실패는 message-flow observer와 runtime 진단으로 보고한다.
...
것이다 (정식 정의는
[regression-test-matrix](../../../../node/internals/regression-test-matrix.ko.md) 가 소유).
