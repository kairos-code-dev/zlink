...
| `ws` | `MULTI_DEALER_DEALER` | `보류(38.2%)` | `통과(55.2%)` | `통과(67.7%)` | `통과(59.2%)` | `통과(58.7%)` | `통과(64.4%)` | 64B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_055005.txt` 대비 Go `perf_go_multi_linux_20260525_055047.txt` 기준이다. 256B/1024B는 Go `perf_go_multi_linux_20260524_222134.txt`, 65536/131072/262144B는 Go `perf_go_multi_linux_20260525_001654.txt` median 기준이다. C처럼 단일 poll loop와 pending socket만 `POLLOUT`으로 두는 poll set 의미를 유지했다. client active send는 65536B/131072B에서 public `Bytes(...)` 경로를 사용해 submit 전 native `Message` 생성을 줄였다. 262144B는 기존 경로로도 current HEAD에서 통과권이 유지된다. 64B는 최신 같은 조건 재측정에서도 Go 단순 one-way 최소 기준보다 낮다. |
...
| `tcp` | `MULTI_PUBSUB` | `보류(24.5%)` | `보류(14.2%)` | `보류(11.6%)` | `보류(44.9%)` | `통과(92.0%)` | `통과(101.8%)` | 64/256/1024/65536B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_054224.txt` 대비 Rust `perf_rust_multi_linux_20260525_054248.txt` median 기준이다. 131072/262144B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_022108.txt` 대비 Rust `perf_rust_multi_linux_20260525_022246.txt` 기준이다. server active publish는 payload `Vec`에 stamp한 뒤 `Message::copy_from`으로 다시 복사하던 경로를 `Message::with_size()` + `data_mut()` 직접 stamp로 바꿨다. 같은 조건 원래 copy 경로 재측정 `perf_rust_multi_linux_20260524_215352.txt`보다 64/256B가 높다. client는 SUB connection-ready를 기다리도록 맞춰 262144B timeout까지 해소했다. 64/256/1024/65536B는 현재 core 기준 재측정에서도 단순 one-way 기준보다 낮고, 131072/262144B는 최신 같은 조건 재측정에서 통과권이다. |
...
| `tcp` | `MULTI_SPOT_REQREP` | `통과(80.0%)` | `통과(88.9%)` | `통과(85.9%)` | `통과(89.4%)` | `통과(77.6%)` | `보류(69.6%)` | 64/256/1024B는 fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Rust `perf_rust_multi_linux_20260522_150056_codex_rust_multi_tcp_spot_reqrep_20260522.txt` 기준이다. 65536/131072B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_022108.txt` 대비 Rust `perf_rust_multi_linux_20260525_022246.txt` 기준이다. 262144B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_054540.txt` 대비 Rust `perf_rust_multi_linux_20260525_054601.txt` median 기준이다. server reply가 받은 single-part `Message`를 다시 복사하지 않고 그대로 reply로 넘기고, client active request를 C처럼 `DONT_WAIT`로 맞췄다. 최신 단독 재측정에서 262144B는 9.887Kops/s까지 올랐지만 SPOT 기준보다 낮아 보류를 유지한다. Rust public binding 정책상 C++의 external buffer attach 방식은 이 셀의 perf 전용 fast path로 넣지 않는다. |
