...
node_rid_t
node_rid_t::from_string (std::string value)
{
  return node_rid_t (std::move (value));
}

std::string_view
node_rid_t::value () const noexcept
{
  return _value;
}

bool
node_rid_t::empty () const noexcept
{
  return _value.empty ();
}
...
spot_rid_t
spot_rid_t::from_string (std::string value)
{
  return spot_rid_t (std::move (value));
}

std::string_view
spot_rid_t::value () const noexcept
{
  return _value;
}

bool
spot_rid_t::empty () const noexcept
{
  return _value.empty ();
}
...
spot_context_t &spot_context_t::operator= (spot_context_t &&) noexcept =
  default;

node_rid_t
spot_context_t::node_rid () const
{
  return _state->node_rid;
}

spot_rid_t
spot_context_t::spot_rid () const
{
  return _state->spot_rid;
}

std::string
spot_context_t::spot_name () const
{
...
send_call_t
spot_context_t::send_to_erased (node_rid_t node_rid, spot_rid_t spot_rid)
{
...
spot_context_t::erased_request_call_t
spot_context_t::request_to_erased (node_rid_t node_rid, spot_rid_t spot_rid)
{
  if (node_rid.empty () || spot_rid.empty ()) {
  ...
  _state->ordering_log.push_back (
    "request_to:" + std::string (spot_rid.value ()));
  return erased_request_call_t (framework_exception_t (
                                        ...
                                        std::type_index payload_type)
{
  ...
  _state->packets.push_back (
    spot_packet_descriptor_t { std::move (packet_name), payload_type });
  return *this;
}

std::vector<spot_packet_descriptor_t>
...
spot_node_builder_t::spot_node_builder_t (spot_node_builder_t &&) noexcept =
  default;
spot_node_builder_t &spot_node_builder_t::operator= (
  spot_node_builder_t &&) noexcept = default;
  ...
  std::string route_channel_name)
{
  ...
  _state->snapshot.registry_spot_remote_addresses_enabled = true;
  _state->snapshot.registry_spot_route_channel =
                                       ...
                                       bool entry_spot)
{
  const auto [_, inserted] =
    _state->spot_factories.emplace (spot_name, spot_type);
  if (!inserted) {
    throw framework_exception_t (
      framework_error_kind_t::request_protocol_error,
...
{
  const auto [_, inserted] =
    _state->actor_factories.emplace (actor_type, factory_type);
  if (!inserted) {
    throw framework_exception_t (
...
spot_node_snapshot_t
spot_node_builder_t::snapshot () const
{
  return _state->snapshot;
}

spot_context_t
spot_node_builder_t::create_spot (std::string spot_name)
{
  return detail::spot_node_runtime_t (_state).create_spot (std::move (spot_name));
}

std::optional<std::string>
spot_node_builder_t::spot_name_for (spot_rid_t spot_rid) const
{
  return detail::spot_node_runtime_t (_state).spot_name_for (std::move (spot_rid));
}

std::optional<spot_route_t>
spot_node_builder_t::resolve_spot (spot_rid_t spot_rid) const
{
...
{
  auto state = std::make_shared<detail::spot_node_builder_state_t> (
    std::move (spot_node_name));
...
std::vector<spot_node_snapshot_t>
zlink_builder_t::spot_nodes () const
{
...
spot_node_runtime_t::spot_node_runtime_t (
...
spot_node_runtime_t
spot_node_runtime_t::from (const spot_node_builder_t &builder)
{
  return spot_node_runtime_t (builder._state);
}

spot_context_t
spot_node_runtime_t::create_spot (std::string spot_name)
{
  ...
  auto rid = spot_rid_t::from_string (
    _state->snapshot.name + ":" + spot_name + ":" +
...
std::optional<std::string>
spot_node_runtime_t::spot_name_for (spot_rid_t spot_rid) const
{
  const auto found = _state->spot_names_by_rid.find (
...
std::optional<spot_route_t>
spot_node_runtime_t::resolve_spot (spot_rid_t spot_rid) const
{
  if (const auto name = spot_name_for (spot_rid)) {
    return spot_route_t {
      node_rid_t::from_string (_state->snapshot.name),
      std::move (spot_rid),
      *name };
  }
