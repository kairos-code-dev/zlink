...
**event loop 스레드는 막히지 않는다.** C++ 산출물이 execute scheduler(Asio worker)에
HTTP 작업을 올리던 자리를 Node에서는 런타임의 비동기 I/O가 무료로 제공한다.
...
## resume scheduler — Node에서의 차이

C++ client는 `.coroutines(resume_scheduler)`로 continuation 재개 위치를 주입할 수 있었다.
**Node는 단일 event loop라 이 주입 개념이 사라진다.** `.coroutines()` 빌더 항목은
존재하지 않으며, 표준 `Promise`/`await`만 제공한다. 이는 정당한 언어 차이다.

## blocking 경로 없음

Node에는 동기 blocking HTTP 접근이 없다. C++의 `fetch<T>`/`.result()`, dotnet의
`Fetch<T>()`에 대응하는 blocking 경로를 제공하지 않는다. 모든 제출은 `await`한다.
