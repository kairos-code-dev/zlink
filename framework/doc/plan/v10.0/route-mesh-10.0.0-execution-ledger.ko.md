...
| S5-13 | framework core-blocked 결함 수정과 POSD 재검 | BUG-1~5를 결정적 회귀로 확인하고 실제 책임 계층을 수정한 뒤 Core 전체 I1·I2·I3 재리뷰와 종료 검증 | 진행 | BUG-1 stale transport 식별, BUG-2 connector 은퇴, BUG-4 session FIFO mutex 경계, BUG-5 pre-admission monitor 승격의 red→green 완료. focused Core 15/15·11/11·7/7. BUG-3·framework E2E, 전체 Core 검증과 두 reviewer 재검 대기 |
...
| S8-08 | 기존 topology API와 runtime 제거 | v10 plan·review record만 제외하고 builder, registration, production `UseInMemoryLocationStores()`, bridge, Spot·Actor–STREAM service part와 Actor join/lifecycle 전용 wrapper, test와 현재 docs no-hit | 진행 | 구 spot-node builder·SpotRouteBridge wrapper 제거, production in-memory location store 선택 표면 제거 + 미등록 store startup fail-fast(S8-04B). Internal test helper도 `UseTestLocationStore()`로 바꿔 금지 이름의 .NET source·test scoped no-hit를 만들었고 non-documentation UnitTests 640/640을 확인했다. `MapSpotMeshToRouteChannel`과 내부 `ZLinkSpotRouterChannelMap`은 제거하고 location row의 `MeshName`을 Spot·Actor handle이 직접 보존하도록 정리했다. API snapshot을 재생성했고 ContractTests 46/46, LocationResolver·NodesAndServices focused UnitTests 67/67을 확인했다. DocFixtures와 TestHost의 channel·route mode를 `AddRouteMesh`·`ChannelName`·`PeerConnections` 및 `IZLinkRouteClient`로 이행했고 두 testapp build가 warning/error 0으로 통과했다. TicTacToe의 API·Play request channel도 `AddRouteMesh`·`ChannelName`·manual peer 및 `IZLinkRouteClient`로 이행하고, MeshNode membership explicit handler가 DI에 등록되지 않던 registrar 누락을 회귀 테스트와 함께 수정했다. 관련 5개 프로젝트 build 0/0, non-documentation UnitTests 640/640, TicTacToe scoped sample regression 13/14이며 기존 samples README 문구 단언 1건만 실패했다. Requester-only membership은 weight 0으로 고정해 local node가 select-one 후보가 되지 않도록 보완했다. 실제 runner 재실행에서도 새 channel 경로의 게임 생성·API 인증까지 통과한 뒤 `framework.internal.actor-session-bind` 반복 제출 구간에서 timeout되어 Core 작업 완료 뒤 재검한다. `AddClientServerChannel`·`AddRouteMeshChannel`·두 builder는 source·snapshot·sample·E2E·guide의 남은 호출을 계속 제거한다 |
