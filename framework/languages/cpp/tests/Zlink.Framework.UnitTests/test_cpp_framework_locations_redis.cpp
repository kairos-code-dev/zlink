...
std::optional<redis_location_options_t> find_redis_options ()
{
    ...
    for (const auto &endpoint : redis_test_endpoints ()) {
        ...
        if (probe.result ().has_value ()
            && probe.result ().value ().status == location_write_status_t::stored) {
...
TEST (ZLinkFrameworkLocationsRedis, StoreWithoutRedisClientReportsUnavailable)
{
    ...
    auto claim = store.update_actor (
      actor_location_t{.actor_type = "player",
                       .actor_id = "alice",
                       .actor_ref = "node-a:1:alice",
                       .node_rid = zlink::routing_id_t::from ("node-a"),
                       .owner_id = "owner-a"},
      zlink::framework::location_write_intent_t::new_claim);
    ASSERT_TRUE (claim.result ().has_value ());
    EXPECT_EQ (location_write_status_t::store_unavailable, claim.result ().value ().status);

    auto lease = store.renew_owner_lease ("owner-a", zlink::routing_id_t::from ("node-a"),
                                          std::chrono::milliseconds (500));
    ASSERT_TRUE (lease.result ().has_value ());
    EXPECT_EQ (location_write_status_t::store_unavailable, lease.result ().value ().status);
    ...
    auto peer = store.update_peer (
      peer_location_t{.auto_connect_type = location_auto_connect_type_t::client_server,
                      ...
                      .owner_id = "owner-a"},
      zlink::framework::location_write_intent_t::new_claim);
    ASSERT_TRUE (peer.result ().has_value ());
    EXPECT_EQ (location_write_status_t::store_unavailable, peer.result ().value ().status);
    ...
    auto spot = store.update_spot (
      spot_location_t{.mesh_name = "play",
                      ...
                      .owner_id = "owner-a"},
      zlink::framework::location_write_intent_t::new_claim);
    ASSERT_TRUE (spot.result ().has_value ());
    EXPECT_EQ (location_write_status_t::store_unavailable, spot.result ().value ().status);
    ...
    auto route = store.update_route (
      route_location_t{.route_kind = route_kind_t::spot_name,
                       ...
                       .value = {1, 2, 3}},
      zlink::framework::location_write_intent_t::new_claim);
    ASSERT_TRUE (route.result ().has_value ());
    EXPECT_EQ (location_write_status_t::store_unavailable, route.result ().value ().status);

    auto read = store.resolve_actor (actor_location_key_t{.actor_type = "player",
                                                          .actor_id = "alice"});
    ...
    ASSERT_TRUE (removed_peer.result ().has_value ());
    EXPECT_EQ (location_write_status_t::store_unavailable,
               removed_peer.result ().value ().status);

    auto removed_spot = store.remove_spot (
      spot_location_key_t{.mesh_name = "play", .spot_rid = zlink::routing_id_t::from ("spot-a")},
      location_owner_token_t{"owner-a", 0});
    ASSERT_TRUE (removed_spot.result ().has_value ());
    EXPECT_EQ (location_write_status_t::store_unavailable,
               removed_spot.result ().value ().status);

    auto removed_route = store.remove_route (
      route_location_key_t{.route_kind = route_kind_t::spot_name, .route_key = "room-a"},
      location_owner_token_t{"owner-a", 0});
    ASSERT_TRUE (removed_route.result ().has_value ());
    EXPECT_EQ (location_write_status_t::store_unavailable,
               removed_route.result ().value ().status);

    auto removed_actor = store.remove_actor (
      actor_location_key_t{.actor_type = "player", .actor_id = "alice"},
      location_owner_token_t{"owner-a", 0});
    ASSERT_TRUE (removed_actor.result ().has_value ());
    EXPECT_EQ (location_write_status_t::store_unavailable,
               removed_actor.result ().value ().status);

    auto removed = store.remove_actors_by_owner ("owner-a");
    EXPECT_FALSE (removed.result ().has_value ());
    ASSERT_NE (nullptr, removed.result ().error ());
    EXPECT_TRUE (removed.result ().error ()->is_retriable ());

    auto removed_peers = store.remove_peers_by_owner ("owner-a");
    EXPECT_FALSE (removed_peers.result ().has_value ());
    ASSERT_NE (nullptr, removed_peers.result ().error ());
    EXPECT_TRUE (removed_peers.result ().error ()->is_retriable ());

    auto removed_spots = store.remove_spots_by_owner ("owner-a");
    EXPECT_FALSE (removed_spots.result ().has_value ());
    ASSERT_NE (nullptr, removed_spots.result ().error ());
    EXPECT_TRUE (removed_spots.result ().error ()->is_retriable ());

    auto removed_routes = store.remove_routes_by_owner ("owner-a");
    EXPECT_FALSE (removed_routes.result ().has_value ());
    ASSERT_NE (nullptr, removed_routes.result ().error ());
    EXPECT_TRUE (removed_routes.result ().error ()->is_retriable ());

    auto removed_lease = store.remove_owner_lease ("owner-a");
    ASSERT_TRUE (removed_lease.result ().has_value ());
    EXPECT_EQ (location_write_status_t::store_unavailable,
               removed_lease.result ().value ().status);
...
TEST (ZLinkFrameworkLocationsRedis, RedisServerRoundTripUsesStoreSchema)
{
    ...
    ASSERT_EQ (location_write_status_t::stored,
               store.renew_owner_lease ("owner-a", zlink::routing_id_t::from ("node-a"),
    ...
    auto resolved = store.resolve_actor (actor_location_key_t{.actor_type = "player",
                                                              .actor_id = "alice"});
    ...
    ASSERT_EQ (location_write_status_t::stored,
               store.renew_owner_lease ("publisher-owner", zlink::routing_id_t::from ("pub-a"),
                                        std::chrono::seconds (5))
                 .result ()
                 .value ()
                 .status);
    ...
    auto missing = store.resolve_actor (actor_location_key_t{.actor_type = "player",
                                                             .actor_id = "alice"});
    ASSERT_TRUE (missing.result ().has_value ());
    EXPECT_FALSE (missing.result ().value ().has_value ());
    (void) store.remove_peers_by_owner ("publisher-owner");
...
TEST (ZLinkFrameworkLocationsRedis, CrossLanguageWritesRowsForDotnetToRead)
{
    ...
    redis_location_store_t store (*options);
    ASSERT_EQ (location_write_status_t::stored,
               store.renew_owner_lease ("cpp-owner", zlink::routing_id_t::from ("cpp-node"),
...
TEST (ZLinkFrameworkLocationsRedis, CrossLanguageReadsDotnetRows)
{
    ...
    redis_location_store_t store (*options);
    auto actor = store.resolve_actor (actor_location_key_t{.actor_type = "player",
                                                          .actor_id = "dotnet-actor"})
                   .result ();
...
TEST (ZLinkFrameworkLocationsRedis, LuaScriptsPreserveDotnetAtomicStoreContract)
{
    ...
    EXPECT_NE (std::string::npos, remove_by_owner.find ("redis.call('SMEMBERS', KEYS[1])"));
    EXPECT_NE (std::string::npos, remove_by_owner.find ("local mesh = redis.call('HGET', rowHash, 'mesh')"));
    EXPECT_NE (std::string::npos, remove_by_owner.find ("redis.call('INCR', ARGV[2] .. ':' .. mesh)"));
    EXPECT_NE (std::string::npos, remove_by_owner.find ("redis.call('DEL', KEYS[1])"));
...
TEST (ZLinkFrameworkLocationsRedis, ActorRowJsonUsesDotnetFieldSchema)
{
    ...
    EXPECT_EQ ("node-a:7:alice", json.at ("ActorRef").get<std::string> ());
    EXPECT_EQ (zlink::routing_id_t::from ("node-a").to_hex (),
               json.at ("NodeRid").get<std::string> ());
