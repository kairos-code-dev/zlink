[한국어](03-spot.ko.md) | English

[Spec Index](../../README.md) · [Core Index](../README.md) · [Service Common](README.md) · [MeshNode](01-mesh-node.md) · [Dispatch](02-dispatch.md)
...
A Spot is a logical destination owned by one MeshNode. It owns no network socket,
...
validates the explicit `target_spot_generation`. The caller supplies the exact
target node RID, Spot RID, and lifecycle generation in one public API call.
Core neither resolves nor completes a logical location. Generation zero returns `ZLINK_SUBMIT_INVALID_ARGUMENT` with
...
[dispatch contract](02-dispatch.md), without reconstructing a source route or
request sequence.
...
After a successful publish, each Spot lifecycle generation in the target
ChannelName whose subscription matches the topic receives exactly one
`SPOT_MULTICAST` record. A nonmatching Spot receives none. The publish is not
relayed to another node or replayed.
...
Any publish that starts after a successful set or unset operation completes
observes the changed subscription. A publish concurrent with set or unset observes
either the state before or the state after the change; it never observes a
partial state or delivers twice for one subscription.
...
ChannelName, topic, and application metadata. Direct and multicast records
expose application metadata as a separate immutable view. Actor application
payload is not part of this table.
...
This function creates an eventing timer handle that belongs to the current Spot
lifecycle generation. A tick from an earlier generation is not delivered after
Spot close or recreation at the same RID. No new tick is delivered after a
successful stop or destroy returns. A timer handler and an application-claim
handler for the same Spot generation do not execute concurrently. Timer start,
