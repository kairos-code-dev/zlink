...
std::shared_ptr<connector_state_t>
state_from (const std::shared_ptr<void> &state)
{
...
{
  return connector_runtime_t (
    state_from (connector_internal_handle (connector)));
}
...
std::shared_ptr<void>
connector_internal_handle (const connector_t &connector)
{
...
codec_registry_t::codec_registry_t (std::shared_ptr<void> state)
  : _state (std::move (state))
{
}
...
send_call_t::submit ()
{
  ...
  return task_t<void> (
    detail::submit_send (detail::state_from (_state), std::move (_packet)));
}
...
connector_t::connect ()
{
  ...
  const auto websocket_endpoint =
    state->options.transport == transport_t::websocket
  ...
  const auto max_attempts =
    state->options.reconnect.enabled
  ...
  for (int attempt = 1; attempt <= max_attempts; ++attempt) {
    ...
    try {
      ...
      } else if (state->options.transport ==
...
#ifdef ZLINK_STREAM_CONNECTOR_WITH_OPENSSL
        state->connection = detail::connect_tls (
          state->io_context,
          *tls_endpoint,
          state->options.skip_server_certificate_validation);
#else
      ...
      if (attempt < max_attempts) {
        ...
        retry_delay = std::min (
          state->options.reconnect.max_delay,
...
{
  return task_t<packet_t> (
    detail::receive_next (detail::state_from (_state), timeout));
}
...
{
  detail::state_from (_state)->state_handlers.push_back (
    std::move (handler));
  return *this;
}
...
{
  detail::state_from (_state)->error_handlers.push_back (
    std::move (handler));
  return *this;
}
...
{
  detail::state_from (_state)->disconnected_handlers.push_back (
    std::move (handler));
  return *this;
}
...
{
  detail::state_from (_state)
    ->packet_handlers[std::move (packet_name)]
