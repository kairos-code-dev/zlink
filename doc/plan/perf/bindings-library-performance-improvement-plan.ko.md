...
| 5 | Go | `bindings/go/perf` | `tcp/ws/wss/tls single 통과 (6.0.3 재검증, SPOT large 회귀 수정, latency 발산은 측정 아티팩트로 판정)` | `2026-05-25 추가 수정으로 ROUTER_ROUTER/DEALER_ROUTER/PUBSUB 전 transport 통과, DD·SPOT_REQREP 일부 large 통과, SPOT large 대폭 개선, SPOT_SENDSEND large 통과(6.6.3). 잔여 보류: DD 일부 large, SPOT_REQREP tcp 262144B, 64B FFI` | 잔여 보류 추적: 암호화 DD large copy 축소, SPOT_REQREP tcp 262144B server reply hot path 재검토, 64B FFI 경계 비용 분리 |
