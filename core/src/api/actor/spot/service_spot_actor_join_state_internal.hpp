...
{
    typedef std::map<spot_logical_state_t *, std::deque<queued_join_request_t *>> queue_map_t;

    std::deque<queued_join_request_t *> &queue_for (spot_logical_state_t *key_);
    ...
    bool peek_for_spot (spot_handle_t *spot_, queued_join_request_t **request_out_);
    void remove_queued (queued_join_request_t *request_);
    void take_queue (spot_logical_state_t *key_, std::deque<queued_join_request_t *> *pending_);
    void replace_live_spot (spot_handle_t *from_, spot_handle_t *to_);
    void collect_live_for_state (const std::shared_ptr<spot_logical_state_t> &state_,
                                 std::deque<queued_join_request_t *> *pending_) const;
    void drain_queued_for_stream (void *stream_, std::deque<queued_join_request_t *> *aborted_);
    void collect_live_for_stream (void *stream_,
    ...
    bool has_pending_remote_actor (zlink::spot_node_t *node_, const char *actor_id_) const;
    ...
    std::set<std::pair<zlink::spot_node_t *, std::string>> pending_remote_actor_keys;
