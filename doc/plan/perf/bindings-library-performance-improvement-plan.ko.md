...
| 5 | Go | `bindings/go/perf` | `tcp/ws/wss/tls single 통과 (6.0.3 재검증, SPOT large 회귀 수정, latency 발산은 측정 아티팩트로 판정)` | `2026-05-24 재측정/추가 수정으로 ROUTER_ROUTER/DEALER_ROUTER/PUBSUB 전 transport 통과, DD·SPOT large 대폭 개선, SPOT_REQREP 일부 stale 보류 제거(6.6.3). 잔여 보류: SPOT_SENDSEND large, DD 일부 64B/large, SPOT_REQREP tcp large/ws 131072B, 64B FFI` | 잔여 보류 추적: `SPOT_SENDSEND` request-reply echo(size별 active-slot+1ms cap) 재설계, 암호화 DD large copy 축소 |
...
| Transport | Pattern | 64 | 256 | 1024 | 65536 | 131072 | 262144 | 결과 파일 / 메모 |
|-----------|---------|----|-----|------|-------|--------|--------|------------------|
| `tcp` | `MULTI_DEALER_DEALER` | `보류(38.8%)` | `통과(54.7%)` | `통과(71.2%)` | `보류(48.7%)` | `보류(46.2%)` | `보류(46.3%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 64/65536/131072/262144B는 Go current HEAD 재측정 `perf_go_multi_linux_20260524_231656.txt` median 기준이고, 256/1024B는 Go `perf_go_multi_linux_20260524_222134.txt` median 기준이다. C처럼 단일 poll loop와 pending socket만 `POLLOUT`으로 두는 poll set 의미를 유지했다. server는 C의 single-part `zlink_recv_part` 의미와 맞춰 64B/65536B에서 public `RecvPart` caller-owned 수신 경로를 사용한다. 현재 코드 재측정으로 256B/1024B는 통과권이고, 이전 262144B 7.0% 보류는 current HEAD에서는 재현되지 않았다. 64B/65536B/131072B/262144B는 아직 Go 단순 one-way 최소 기준보다 낮다. `RecvPart`를 1024B 이상 또는 large에 넓힌 이전 후보들은 large size를 악화시켜 반영하지 않는다. auto-HWM 활성과 size별 `MsgUnit(B)` 일치를 확인했다. |
...
| `tcp` | `MULTI_SPOT_REQREP` | `통과(55.8%)` | `통과(55.4%)` | `통과(57.4%)` | `보류(30.8%)` | `보류(32.0%)` | `보류(37.0%)` | 64B/256B/1024B는 기존 Go `perf_go_multi_linux_20260522_023554_codex_go_tcp_multi_current_after_runner_cleanup_20260522.txt` 기준이다. 65536B 이상은 fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go `perf_go_multi_linux_20260524_223541.txt` median 기준이다. request completion은 public `POLLCOMPLETION` poller를 사용한다. 2026-05-24에 client hot path의 payload `append` 복사를 제거해 `NewMessage(...)` 복사 한 번만 남겼고, 65536B/131072B/262144B가 27.7/28.2/32.6%에서 30.8/32.0/37.0%로 올랐다. tcp large는 아직 SPOT 최소 기준보다 낮아 보류다. 과거 같은 후보가 실패했던 것은 당시 runner/core 조건의 완료성 문제였으므로 현재 core 6.0.3 기준으로 재검증해 반영했다. binding 내부 단일 part request fast path 후보 `perf_go_multi_linux_20260522_052815_codex_go_tcp_multi_spot_reqrep_request_fastpath_only_failed_sizes_20260522.txt`는 262144B 또는 large size 실패를 만들어 반영하지 않았다. auto-HWM 활성과 size별 SpotNode `MsgUnit(B)` 일치를 확인했다. |
| `tcp` | `MULTI_SPOT_SENDSEND` | `보류(44.7%)` | `보류(44.9%)` | `보류(48.3%)` | `보류(18.7%)` | `보류(18.8%)` | `보류(24.7%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go `perf_go_multi_linux_20260524_220932.txt` median 기준이다. C++처럼 client active loop에서 waiting reply를 먼저 `RecvRouted(...DONTWAIT)`로 drain한 뒤 같은 slot에서 즉시 다음 send를 시도하도록 맞췄다. client send는 `ws`가 아니고 65536B 이하일 때만 public `MoveMessage(...)`를 사용해 submit 전 복사를 줄인다. `ws`와 131072B 이상은 후보 측정에서 일부 size가 낮아져 기존 `Message(...)` 경로를 유지한다. 전 size complete를 유지하지만, SPOT 최소 기준에는 아직 못 미친다. |
...
| `ws` | `MULTI_DEALER_DEALER` | `보류(38.4%)` | `통과(55.2%)` | `통과(67.7%)` | `보류(45.4%)` | `보류(46.3%)` | `통과(53.3%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go `perf_go_multi_linux_20260524_222134.txt` median 기준이다. C처럼 단일 poll loop와 pending socket만 `POLLOUT`으로 두는 poll set 의미를 유지했다. 262144B는 current HEAD 재측정에서 통과권으로 올라 이전 4.8% 보류가 stale이었음을 확인했다. server `RecvPart`를 ws 65536B 이상으로 넓힌 이전 후보는 large를 낮춰 반영하지 않는다. 64B/65536B/131072B는 아직 Go 단순 one-way 최소 기준보다 낮다. auto-HWM 활성과 size별 `MsgUnit(B)` 일치를 확인했다. |
...
| `ws` | `MULTI_SPOT_REQREP` | `통과(61.1%)` | `통과(60.8%)` | `통과(63.8%)` | `통과(51.1%)` | `보류(37.8%)` | `통과(51.7%)` | 64B/256B/1024B는 기존 Go `perf_go_multi_linux_20260522_061530_codex_go_ws_multi_spot_reqrep_msgunit_report_fix_20260522.txt` 기준이다. 65536B/131072B/262144B는 fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go `perf_go_multi_linux_20260524_223541.txt` median 기준이다. request completion은 public `POLLCOMPLETION` poller를 사용한다. client payload 중복 복사 제거 뒤 65536B와 262144B는 SPOT 최소 기준을 넘겼고, 131072B만 보류로 남았다. binding 내부 단일 part request fast path 후보는 실패 size를 해결하지 못하거나 completion을 깨 반영하지 않았다. auto-HWM 활성과 size별 SpotNode `MsgUnit(B)` 일치를 확인했다. |
| `ws` | `MULTI_SPOT_SENDSEND` | `보류(46.6%)` | `보류(41.3%)` | `보류(37.6%)` | `보류(0.2%)` | `보류(23.4%)` | `보류(35.2%)` | C `perf_c_multi_linux_20260522_055144_codex_c_ws_multi_for_go_20260522.txt` 대비 Go `perf_go_multi_linux_20260524_220932.txt` median 기준이다. drain-then-submit loop로 complete는 유지하지만 large collapse는 남는다. `MoveMessage(...)` client send 후보는 `ws` small과 일부 large를 낮춰 적용하지 않는다. C와 같은 size별 active slot 제한을 유지하고, 서버 echo는 ws large에서 public builder 경로를 유지한다. |
...
| `wss` | `MULTI_DEALER_DEALER` | `보류(37.4%)` | `통과(58.3%)` | `통과(71.2%)` | `보류(37.7%)` | `보류(41.4%)` | `보류(35.6%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 64/65536B는 Go current HEAD 재측정 `perf_go_multi_linux_20260524_231656.txt` median 기준이고, 131072/262144B는 5회 제한 재측정 `perf_go_multi_linux_20260524_232008.txt` median 기준이다. 256/1024B는 Go `perf_go_multi_linux_20260524_222134.txt` median 기준이다. C처럼 단일 poll loop와 pending socket `POLLOUT` 의미를 유지했다. Go send builder에 명시적 ownership 이전 단계 `MoveMessage`를 추가했고, active send는 1024B 이하와 wss 131072B/262144B, tls 131072B에서만 이 경로를 사용한다. 262144B를 `MoveMessage(...)`로 넓힌 후보는 이전 제한 측정에서 48.0%까지 보였지만, current HEAD 5회 반복에서는 35.6%가 median이라 암호화 large 보류로 유지한다. `Message`의 실패 시 원본 보존 계약은 유지하며 `go test ./...`로 회귀를 확인했다. tls 262144B까지 넓힌 후보 `perf_go_multi_linux_20260524_222040.txt`, `perf_go_multi_linux_20260524_222134.txt`는 full run에서 tls 262144B를 낮춰 wss에만 반영한다. auto-HWM 활성과 size별 `MsgUnit(B)` 일치를 확인했다. |
...
| `wss` | `MULTI_SPOT_SENDSEND` | `보류(39.5%)` | `보류(41.6%)` | `보류(37.5%)` | `통과(53.3%)` | `보류(49.4%)` | `통과(53.6%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go `perf_go_multi_linux_20260524_220932.txt` median 기준이다. client active loop를 C++처럼 drain-then-submit으로 바꿨고, 65536B 이하 client send는 public `MoveMessage(...)`를 사용한다. 서버 echo는 public `Spot.ForwardRouted(...)`를 65536B 이상에 적용한다. 131072B는 49.4%로 통과선 직전이지만, 새 loop 기준 active slot 16 후보와 131072B `MoveMessage(...)` 후보 모두 통과선을 넘기지 못해 8-slot 제한과 기존 send 경로를 유지한다. |
...
| `tls` | `MULTI_DEALER_DEALER` | `보류(38.1%)` | `통과(57.7%)` | `통과(68.1%)` | `보류(43.8%)` | `보류(44.0%)` | `보류(43.4%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 64/65536/131072/262144B는 Go current HEAD 재측정 `perf_go_multi_linux_20260524_231656.txt` median 기준이고, 256/1024B는 Go `perf_go_multi_linux_20260524_222134.txt` median 기준이다. C처럼 단일 poll loop와 pending socket `POLLOUT` 의미를 유지했다. active send는 1024B 이하와 tls 131072B에서만 `MoveMessage(...)`를 사용한다. 256B/1024B는 current HEAD 재측정에서 통과권으로 올라 이전 보류가 stale이었음을 확인했다. tls 262144B까지 `MoveMessage(...)`를 넓힌 후보는 full run에서 낮아져 기존 `Message(...)` 경로를 유지한다. 64B/65536B/131072B/262144B는 아직 단순 one-way 기준보다 낮다. auto-HWM 활성과 size별 `MsgUnit(B)` 일치를 확인했다. |
...
| `tls` | `MULTI_SPOT_SENDSEND` | `보류(42.4%)` | `보류(45.4%)` | `보류(44.6%)` | `보류(38.1%)` | `보류(37.3%)` | `보류(42.4%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go `perf_go_multi_linux_20260524_220932.txt` median 기준이다. `SubmitNotConnected`는 C처럼 fatal이 아니라 다음 poll loop에서 재시도하는 의미로 처리하고, 서버 echo는 public `Spot.ForwardRouted(...)`를 65536B 이상에 적용한다. client send는 65536B 이하에서만 public `MoveMessage(...)`를 사용한다. 131072B까지 넓힌 후보는 일부 transport를 낮춰 반영하지 않았다. drain-then-submit loop와 작은 frame move send 뒤에도 아직 Go SPOT 최소 기준에는 못 미친다. active slot 16 후보 `perf_go_multi_linux_20260524_205629.txt`는 131072B를 낮춰 반영하지 않는다. |
...
| `tcp` | `MULTI_PUBSUB` | `보류(26.3%)` | `보류(16.9%)` | `보류(15.5%)` | `보류(49.5%)` | `통과(98.4%)` | `통과(120.4%)` | 64/256/1024B는 fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Rust 반복 측정 `perf_rust_multi_linux_20260524_215258.txt` median 기준이다. 65536/131072B는 Rust `perf_rust_multi_linux_20260522_145345_codex_rust_multi_tcp_pubsub_stop_retry_fix_20260522.txt`, 262144B는 `perf_rust_multi_linux_20260522_152752_codex_rust_multi_tcp_pubsub262_monitor_ready_20260522.txt` 기준이다. server active publish는 payload `Vec`에 stamp한 뒤 `Message::copy_from`으로 다시 복사하던 경로를 `Message::with_size()` + `data_mut()` 직접 stamp로 바꿨다. 같은 조건 원래 copy 경로 재측정 `perf_rust_multi_linux_20260524_215352.txt`보다 64/256B가 높다. client는 SUB connection-ready를 기다리도록 맞춰 262144B timeout까지 해소했다. 64/256/1024B는 아직 단순 one-way 기준보다 낮고, 262144B는 C보다 높아 outlier 재검토 대상으로 남긴다. |
...
| `tcp` | `MULTI_SPOT_REQREP` | `통과(80.0%)` | `통과(88.9%)` | `통과(85.9%)` | `통과(85.1%)` | `보류(70.6%)` | `보류(71.1%)` | 64/256/1024B는 fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Rust `perf_rust_multi_linux_20260522_150056_codex_rust_multi_tcp_spot_reqrep_20260522.txt` 기준이다. 65536/131072/262144B는 fresh C 대비 Rust current HEAD 재측정 `perf_rust_multi_linux_20260524_224759.txt` median 기준이다. server reply가 받은 single-part `Message`를 다시 복사하지 않고 그대로 reply로 넘기고, client active request를 C처럼 `DONT_WAIT`로 맞춰 65536B는 통과 상태다. 131072/262144B는 재측정에서 70%대까지 올라왔지만 아직 기준보다 낮다. |
...
- **남은 보류(Rust)**: SPOT large(copy 영역), SPOT_REQREP tcp 131072/262144B, SPOT_SENDSEND tcp 262144B, PUBSUB small(단일 thread drain).
...
| `tcp` | `MULTI_DEALER_ROUTER` | `보류(13.1%)` | `보류(14.3%)` | `보류(13.3%)` | `통과(43.2%)` | `통과(57.0%)` | `통과(63.2%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Python `perf_python_multi_linux_20260524_235408.txt` median 기준이다. reply header decode를 `to_bytes_list()` 대신 `ReceivedMessage.data` view로 바꾼 뒤, routed `recv_into` single-part fast path를 추가해 수신 시 리스트 구성과 C 배열 재복사를 피했다. large 수치는 모두 기준선 위로 올라갔지만 small size는 여전히 낮다. |
| `tcp` | `MULTI_ROUTER_ROUTER` | `보류(9.6%)` | `보류(10.3%)` | `보류(10.4%)` | `통과(33.1%)` | `통과(44.1%)` | `통과(57.3%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Python `perf_python_multi_linux_20260524_235408.txt` median 기준이고, 65536B와 262144B는 complete 단독 재측정 `perf_python_multi_linux_20260524_235704.txt`, `perf_python_multi_linux_20260524_235511.txt`로 보강했다. client reply header decode `.data` 경로와 routed `recv_into` single-part fast path를 함께 적용한 상태다. 65536B 이상은 기준선을 넘었지만, small size는 여전히 낮다. |
...
| `ws` | `MULTI_ROUTER_ROUTER` | `보류(9.8%)` | `보류(9.6%)` | `보류(9.6%)` | `보류(28.8%)` | `통과(34.8%)` | `통과(51.7%)` | C `perf_c_multi_linux_20260522_150505_codex_c_multi_ws_no_stream_for_rust_20260522.txt` 대비 Python 단독 complete 재측정 `perf_python_multi_linux_20260525_000503.txt` median 기준이다. 65536B는 20.0%→28.8%로 올랐지만 아직 기준선 아래다. 131072B/262144B만 기준선을 넘었다. |
...
| `wss` | `MULTI_ROUTER_ROUTER` | `보류(10.4%)` | `보류(8.9%)` | `보류(10.2%)` | `통과(56.1%)` | `통과(54.1%)` | `통과(70.7%)` | C `perf_c_multi_linux_20260522_152936_codex_c_multi_wss_no_stream_for_rust_20260522.txt` 대비 Python `perf_python_multi_linux_20260525_000301.txt` median 기준이다. TLS 설정 보강 뒤 complete를 확보했고, fast path 뒤 65536B 통과 여유가 커졌다. small size는 여전히 낮다. |
