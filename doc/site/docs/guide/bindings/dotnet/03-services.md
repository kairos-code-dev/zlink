[← 메시징](./02-messaging.md) · [.NET 가이드](./index.md) · [다음: 운영 →](./04-operations.md)
...
| 상황 | 권장 |
|------|------|
| 주소가 고정된 소수 노드 (예: 클라이언트↔단일 서버) | raw 소켓([02 메시징](./02-messaging.md))으로 충분. 서비스 레이어 불필요 |
...
패턴(임베디드 vs 독립)은 [Registry](../../07-4-registry.md).
...
만들지 않으므로 숨은 비용이 없습니다(상세: [07-3 §SpotNode 모드](../../07-3-spot.md)).
...
> ([PUB/SUB](../../03-2-pubsub.md) 참고). `TopicMessage`는 `Received`처럼
...
Actor 절, 개념: [07-3](../../07-3-spot.md)). 기존 Spot을 routing id로 다시 얻으려면
...
전달됩니다. 개념·상태 전이: [Actor](../../07-4-actor.md).
...
직통 API를 두지 않습니다 — [07-4](../../07-4-actor.md)), Actor는 "그 Spot에
...
유용합니다(콜백 규칙은 [운영 — 폴러/타이머](./04-operations.md#폴러--타이머)와
[STREAM 콜백](./02-messaging.md#stream) 참고).
...
> [Actor](../../07-4-actor.md) 챕터를 참고하세요.

---

다음: [운영 — 옵션 · TLS · 모니터링 · 폴러/타이머 · 스레딩 →](./04-operations.md)
