...
framework_exception_t request_failure_mapper_t::completion_exception (request_result_t result,
                                                                      const std::string &operation_name) const
{
    switch (result) {
        case request_result_t::timed_out:
            return framework_exception_t (framework_error_kind_t::timeout, operation_name + " timed out.");
        ...
        case request_result_t::rejected:
            return framework_exception_t (framework_error_kind_t::request_rejected, operation_name + " was rejected.");
        ...
        case request_result_t::internal_error:
            return framework_exception_t (framework_error_kind_t::request_failed, operation_name + " failed.");
    }
    return framework_exception_t (framework_error_kind_t::request_failed, operation_name + " failed.");
}

framework_exception_t request_failure_mapper_t::error_header_exception (const std::string &error_code,
                                                                        ...
                                                                        const std::string &operation_name) const
{
    ...
    if (error_code == "request_target_not_found") {
        return framework_exception_t (
          ...
          error_message.empty () ? operation_name + " failed because the target was not found." : error_message);
    }
    ...
    if (error_code == "handler_not_found") {
        return framework_exception_t (
          ...
          error_message.empty () ? operation_name + " failed because the handler was not found." : error_message);
    }
