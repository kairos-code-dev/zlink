...
namespace zlink
{
int registry_t::ensure_channel_contract_locked (const std::string &channel_name_,
                                                uint16_t auto_connect_type_,
                                                uint64_t now_ms_,
                                                uint32_t owner_registry_id_)
{
    if (channel_name_.empty ()
        || !discovery_protocol::is_valid_auto_connect_type (auto_connect_type_)) {
        errno = EINVAL;
        return -1;
    }
    ...
    if (it == _projection_state.channel_contracts.end ()) {
        ...
        contract.owner_registry_id = owner_registry_id_ == 0 ? 1 : owner_registry_id_;
    ...
    if (it->second.auto_connect_type != auto_connect_type_) {
        ...
        const uint32_t incoming_owner = owner_registry_id_ == 0 ? 1 : owner_registry_id_;
...
void registry_t::remove_channel_providers_locked (const std::string &channel_name_)
{
    service_key_t key;
...
{
    if (!msg_id_out_ || !peer_registry_id_out_ || !list_seq_out_ || frames_.size () < 4)
        return false;

    uint16_t msg_id = 0;
    if (zlink_msg_size (&frames_[0]) == sizeof (discovery_protocol::bootstrap_req_t)) {
...
bool registry_t::accept_peer_message_locked (uint16_t msg_id_,
                                             ...
                                             uint32_t *local_registry_id_out_)
{
    ...
    std::map<uint32_t, uint64_t>::iterator it = _projection_state.peer_seq.find (peer_registry_id_);
    std::map<uint32_t, uint64_t>::iterator route_it =
      _projection_state.peer_route_seq.find (peer_registry_id_);
    if (msg_id_ == discovery_protocol::msg_service_list && it != _projection_state.peer_seq.end ()
        && list_seq_ <= it->second)
        return false;
    if (msg_id_ == discovery_protocol::msg_registry_sync
        && route_it != _projection_state.peer_route_seq.end () && list_seq_ <= route_it->second)
        return false;
    return true;
}

void registry_t::handle_peer_route_sync (const scoped_msg_frames_t &frames_,
                                         ...
                                         uint32_t local_registry_id)
{
    ...
    for (size_t i = 0; i < route_list.routes.size (); ++i) {
        const discovery_protocol::route_record_t &record = route_list.routes[i];
        ...
        if (route_key.channel_name.empty () || route_key.kind == ZLINK_ROUTE_KIND_INVALID
            || record.key.empty () || record.key.size () > ZLINK_ROUTE_KEY_MAX) {
            return;
        }
        route_key.key.assign (reinterpret_cast<const char *> (&record.key[0]), record.key.size ());
        ...
        if (!record.owner_routing_id_key.empty ()) {
            route.owner.routing_id_key.assign (
              reinterpret_cast<const char *> (&record.owner_routing_id_key[0]),
...

    {
        ...
        for (std::vector<route_entry_t>::const_iterator it = incoming_routes.begin ();
        ...
        if (route_list.chunk_index + 1 == route_list.chunk_count) {
            ...
            for (route_observation_map_t::const_iterator it = staging.observations.begin ();
                ...
                upsert_route_observation_locked (it->second, &dirty_routes);
            }
...
void registry_t::handle_peer_service_list (const scoped_msg_frames_t &frames_,
                                           uint32_t peer_registry_id,
                                           uint64_t list_seq,
                                           uint64_t now,
                                           uint32_t local_registry_id)
{
    ...
    for (size_t i = 0; i < service_list.services.size (); ++i) {
        const discovery_protocol::service_record_t &record = service_list.services[i];
        ...
        contract.owner_registry_id = peer_registry_id == 0 ? 1 : peer_registry_id;
        ...
        for (size_t p = 0; p < record.providers.size (); ++p) {
            const discovery_protocol::service_provider_record_t &provider = record.providers[p];
            ...
            entry.source_registry =
              provider.source_registry == 0 ? peer_registry_id : provider.source_registry;
...

    {
        ...
        for (std::map<std::string, channel_contract_t>::const_iterator cit =
            ...
            if (ensure_channel_contract_locked (cit->first, contract.auto_connect_type,
                                                contract.created_at, contract.owner_registry_id)
                != 0) {
        ...
        if (!peer_service_snapshot_changed_locked (incoming, peer_registry_id)) {
...
bool registry_t::peer_provider_matches_locked (const provider_entry_t &current_,
                                               const provider_entry_t &incoming_,
                                               uint32_t peer_registry_id_) const
{
    if (current_.source_registry != peer_registry_id_)
        return true;
    return current_.service_role == incoming_.service_role && current_.weight == incoming_.weight
           && current_.value == incoming_.value && current_.metadata == incoming_.metadata
...
bool registry_t::peer_service_snapshot_changed_locked (const service_map_t &incoming_,
                                                       uint32_t peer_registry_id_) const
{
    for (service_map_t::const_iterator sit = incoming_.begin (); sit != incoming_.end (); ++sit) {
        ...
        for (provider_map_t::const_iterator pit = providers.begin (); pit != providers.end ();
            ...
            if (existing_provider == existing_service->second.providers.end ())
                return true;
            if (!peer_provider_matches_locked (existing_provider->second, pit->second,
                                               peer_registry_id_))
                return true;
        }
    }
    ...
    for (service_map_t::const_iterator sit = _projection_state.services.begin ();
        ...
        for (provider_map_t::const_iterator pit = providers.begin (); pit != providers.end ();
            ...
            if (pit->second.source_registry != peer_registry_id_)
                continue;
            service_map_t::const_iterator incoming_service = incoming_.find (sit->first);
...
void registry_t::remove_peer_service_providers_locked (uint32_t peer_registry_id_, uint64_t now_ms_)
{
...
void registry_t::apply_peer_service_snapshot_locked (const service_map_t &incoming_,
                                                     uint32_t peer_registry_id_)
{
    for (service_map_t::const_iterator sit = incoming_.begin (); sit != incoming_.end (); ++sit) {
        ...
        for (provider_map_t::const_iterator pit = providers.begin (); pit != providers.end ();
             ++pit) {
            provider_map_t::iterator existing = service.providers.find (pit->first);
...
void registry_t::handle_peer (void *sub_)
{
    ...
    {
        scoped_lock_t lock (_sync);
        if (!accept_peer_message_locked (msg_id, peer_registry_id, list_seq, now,
                                         &local_registry_id))
            return;
    }

    if (msg_id == discovery_protocol::msg_registry_sync) {
        handle_peer_route_sync (frames, peer_registry_id, list_seq, now, local_registry_id);
        return;
    }

    handle_peer_service_list (frames, peer_registry_id, list_seq, now, local_registry_id);
}
...
{
    if (frame_count_ < 5) {
        send_register_ack (router_, sender_id_, discovery_protocol::status_invalid, std::string (),
    ...
    if (!discovery_protocol::read_u16 (frames_[1], &auto_connect_type)
        || !discovery_protocol::is_valid_auto_connect_type (auto_connect_type)) {
        send_register_ack (router_, sender_id_, discovery_protocol::status_invalid, std::string (),
                           0, 0, "invalid type");
        return;
    }
    ...
    if (!discovery_protocol::read_u16 (frames_[2], &service_role)
        || !discovery_protocol::auto_connect_type_allows_role (auto_connect_type, service_role)) {
        send_register_ack (router_, sender_id_, discovery_protocol::status_invalid, std::string (),
                           0, 0, "invalid role");
        return;
    }
    const std::string channel_name = discovery_protocol::read_string (frames_[3]);
    const std::string endpoint = discovery_protocol::read_string (frames_[4]);
    ...
    if (frame_count_ >= 6) {
        ...
        if (discovery_protocol::read_u16 (frames_[5], &raw_weight) && raw_weight <= 100) {
    ...
    std::string error;
    {
        ...
        if (ensure_channel_contract_locked (channel_name, auto_connect_type, now, registry_id)
            != 0) {
            status = errno == EEXIST ? discovery_protocol::status_conflict
                                     : discovery_protocol::status_invalid;
    ...
    send_register_ack (router_, sender_id_, status, endpoint, source_registry, registration_id,
                       error);
}
...
{
    if (frame_count_ < 5) {
        send_unregister_ack (router_, sender_id_, discovery_protocol::status_invalid,
                             "invalid unregister");
        return;
    }
    ...
    if (sit == _projection_state.services.end ()) {
        send_unregister_ack (router_, sender_id_, discovery_protocol::status_not_found,
                             "service not found");
        return;
    }
...
void registry_t::handle_heartbeat (const zlink_msg_t *frames_, size_t frame_count_)
{
    if (frame_count_ < 5)
        return;
                                           ...
                                           const zlink_routing_id_t &sender_id_)
{
    ...
    service_map_t::iterator sit = _projection_state.services.find (service_key);
    if (sit == _projection_state.services.end ()) {
        send_register_ack (router_, sender_id_, discovery_protocol::status_not_found, endpoint, 0,
                           0, "service not found");
        return;
    }
    ...
    const uint32_t local_registry_id =
      _coordination_state.registry_id == 0 ? 1 : _coordination_state.registry_id;
    if (pit->second.source_registry != local_registry_id) {
        send_register_ack (router_, sender_id_, discovery_protocol::status_not_found, endpoint, 0,
                           0, "provider not local");
        return;
    }
    ...
    send_register_ack (router_, sender_id_, discovery_protocol::status_ok, endpoint,
                       pit->second.source_registry, pit->second.registration_id, std::string ());
}
}
