...
[문서 목록](../../../../../README.ko.md) | [이전: ZLink Framework NestJS SPOT Integration](nestjs-spot.ko.md) | [다음: ZLink Framework NestJS STREAM Integration](nestjs-stream.ko.md)
<!-- framework-adapter-nav:end -->

[스펙 목차](../../../../node/README.ko.md)

[Node.js 묶음](../../../../node/README.ko.md) | [SPOT](nestjs-spot.ko.md) | [인터페이스](handler-interfaces.ko.md)
...
이 절은 `SPOT` 계약에 더해 `Stage` 모델이 요구하는 내용을 정의한다.

`SPOT` 계약은 다음 기능을 중심으로 한다.
...
## 2. 기존 SPOT 계약으로 충족하는 부분

이 절은 `SPOT` 계약 중 `Stage wrapper`가 그대로 사용할 수 있는 부분을
정리한다.
...
- `ZLinkSpotManager`로 spot 생성 lifecycle을 따로 분리해 둔 점
...
- `requestToChannel(...)` 의 continuation 도 같은 spot execution context 에서
...
7. 그 실행기 안에서만 `ZLinkSpotContext.addHandler(...)`로 등록한 actor handler가
   호출된다.
...
이 모델에서 핵심은 다음과 같다. actor packet 처리를 session callback 위에서
...
내부 구현은 mailbox, queue, executor 등 여러 방식으로 만들 수 있다. 하지만
wrapper 문서에서 못 박아야 하는 최소 의미는 다음 정도다.

- join 된 actor 의 packet 은 `Spot` 실행 문맥 바깥에서 직접 처리하지 않는다.
- framework 내부의 `submit(...)` 는 `Spot` 이 소유한 직렬 실행 규칙 안에서만
  수행된다. 그 안에서 최종적으로 actor packet handler 가 호출된다.
...
따라서 timer를 두 갈래로 나누기보다는, spot lifecycle 안에서 등록하는
하나의 모델로 정리하는 편이 맞다.

필요한 최소 표면은 다음과 같다. `TimeSpan period`는 node에서
`periodMs: number` (밀리초)로 옮긴다.
...
하부 Node 바인딩 자체는 이미 다음과 같은 timer 표면을 제공한다. (`Timer.fromSpot`,
`start/stop/recv/onFire/close`). 시간 단위는 ms number 다.

```ts
export declare class Timer implements ZlinkTimer {
  static fromSpot(spot: Spot): Timer;
  start(intervalMs: number, repeatCount: number): void;
  stop(): void;
  recv(flags?: RecvFlags): number | undefined;
  onFire(handler: (timer: ZlinkTimer, fireCount: number) => void): void;
  close(): void;
}
```

framework 의 `context.addTimer<THandler>(...)` 는 native timer handle 을
그대로 드러내는 표면이 아니다. framework runtime 이 만든 managed timer 를 spot
lifecycle / DI 모델에 붙여 주는 wrapper 로 읽는 편이 맞다.
...
않는다. 그 뒤 `ZLinkSpotTimerHandler<TSpot>.handle(...)` 를 호출한다.
`ZLinkTimer.cancel()` 는 이 managed timer loop 를 멈추고 정리하는 고수준 timer
handle 이다.
...
또한 이 표면에 NestJS DI 컨테이너를 매번 끼워 넣을 필요는 없다. 즉 굳이 public
...
영역이다. `Stage wrapper` 계약에서는 이를 별도 축으로 명시한다.
...
이 계약은 위치 해석 책임을 명확히 고정한다.

## 5. 정리

이 절은 지금까지 정리한 책임 경계와 부족한 축을 짧게 요약한다.

`SPOT` 계약은 `Stage wrapper`의 하부 transport로 사용한다. 다만
`Stage`처럼 그 위의 상위 모델을 설명하려면 다음 네 축이 더
필요하다.
...
1. `ZLinkSpotManager` metadata 확장을 wrapper 후보로 따로 정리
...
wrapper 전용 API 가 생기면, 이 표에는 실제로 실행되는 테스트만 추가한다. node
회귀 테스트 이름은 dotnet 테스트와 동일한 시나리오를 가리키며, 실제 등재는
...
[^heartbeat]: heartbeat는 연결이 유효한지를 확인하기 위해 일정 주기로 보내는 짧은 신호 메시지다.
[^per-spot-scope]: per-spot scope는 Spot 하나의 lifecycle 동안에만 유지되는 DI scope다. Spot이 제거되면 그 scope에서 만든 객체도 함께 정리된다.
