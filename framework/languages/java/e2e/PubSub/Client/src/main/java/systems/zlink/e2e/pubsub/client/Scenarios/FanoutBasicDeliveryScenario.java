...
public final class FanoutBasicDeliveryScenario {
    ...
    public static void run(ScenarioContext context) {
        for (int index = 0; index < 20; index++) {
            context.publisher().publish("all", new Contracts.EventNotify("warmup", index, "warmup-" + index));
        }
        ...
        for (int sequence = 0; sequence < 12; sequence++) {
            context.publisher().publish("all", new Contracts.EventNotify("ps-a1", sequence, "fanout-" + sequence));
        }
