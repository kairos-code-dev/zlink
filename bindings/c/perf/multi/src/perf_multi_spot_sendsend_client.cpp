...
static const char *k_server_node_rid_text = "SPOT-SENDSEND-SERVER-NODE";
static const char *k_server_spot_rid_text = "SPOT-SENDSEND-SERVER-SPOT";
...
std::string bind_client_spot_endpoint (void *node, const std::string &transport, size_t slot_index)
{
    ...
    if (perf_set_spot_node_router_endpoint_raw (
          node, make_endpoint (transport, "perf-multi-spot-sendsend-client-router-"
                        ...
                        spot_reqrep_client_state_t *state)
{
    ...
    for (size_t i = 0; i < state->slots.size (); ++i) {
        ...
        apply_benchmark_socket_options (slot.socket, settings.hwm, transport, ZLINK_SOCKET_DEALER,
                                        max_msg_size);
        const std::string routing_id = std::string ("SPOT-SENDSEND-") + std::to_string (i);
...
bool handle_recv_parts (spot_reqrep_client_slot_t *slot, zlink_msg_t *parts, size_t part_count)
{
    if (!slot || !slot->owner || !parts || part_count == 0) {
        errno = EINVAL;
        return false;
    }
    ...
    bool recorded_sample = false;
    {
        ...
        if (perf_multi_metric::is_expected (header,
            ...
            const double sample_ns =
              static_cast<double> (now_ns - static_cast<uint64_t> (header.sent_ts_ns)) / 2.0;
    ...
    if (recorded_sample) {
        if (bench_debug_enabled ()
            ...
            std::cerr << "[multi-spot-sendsend-client] recv reply slot=" << slot->index
...
bool drain_slot_recv (spot_reqrep_client_slot_t *slot)
{
    if (!slot || !slot->socket) {
        errno = EINVAL;
        return false;
    ...
    for (;;) {
        ...
        size_t part_count = 0;
        const zlink_recv_result_t rc = perf_zlink_spot_recv_parts (
          slot->socket, &source_rid, &source_spot_rid, &request_seq, &parts, &part_count,
          static_cast<zlink_recv_flags_t> (ZLINK_DONTWAIT));
        const int saved_errno = zlink_errno ();
        if (rc == ZLINK_RECV_NO_DATA && (saved_errno == EAGAIN || saved_errno == EWOULDBLOCK)) {
            return true;
        }
        if (rc != ZLINK_RECV_OK) {
            errno = saved_errno;
                            ...
                            size_t msg_size)
{
    ...
    const zlink_submit_result_t rc = perf_zlink_spot_send_spot_parts (
      slot->socket, server_node_rid, server_spot_rid, &part, 1, ZLINK_DONTWAIT);
    ...
    switch (rc) {
        case ZLINK_SUBMIT_OK:
            if (bench_debug_enabled ()
                ...
                std::cerr << "[multi-spot-sendsend-client] send ok slot=" << slot->index
        ...
        case ZLINK_SUBMIT_NOT_CONNECTED:
            if (bench_debug_enabled ()
                && g_client_debug_send_logs.fetch_add (1, std::memory_order_acq_rel) < 8) {
                std::cerr << "[multi-spot-sendsend-client] send not connected slot=" << slot->index
                          << std::endl;
            }
            return send_result_not_connected;
        default:
            if (bench_debug_enabled ()) {
                std::cerr << "[multi-spot-sendsend-client] send failed slot=" << slot->index
...
bool handle_sendsend_ready_events (spot_reqrep_client_state_t *state, int event_count);
...
bool reset_sendsend_poller (spot_reqrep_client_state_t *state)
{
    ...
    for (size_t i = 0; i < state->slots.size (); ++i) {
        if (zlink_poller_modify (state->poller, state->slots[i].socket, ZLINK_POLLIN) != 0) {
            if (bench_debug_enabled ()) {
                std::cerr << "[multi-spot-sendsend-client] poller reset failed slot=" << i
                          << " err=" << zlink_errno () << std::endl;
            }
            return false;
        }
...
bool drain_pending_replies (spot_reqrep_client_state_t *state, size_t active_slots)
{
    if (!state || !state->poller) {
        errno = EINVAL;
        return false;
    ...
    while (any_waiting_reply (state, active_slots)
        ...
        if (event_count < 0) {
            if (zlink_errno () == EINTR)
                continue;
            if (bench_debug_enabled ()) {
                std::cerr << "[multi-spot-sendsend-client] drain poller wait failed err="
                          << zlink_errno () << std::endl;
            }
            return false;
        }
    ...
    for (size_t i = 0; i < active_slots; ++i) {
        ...
        if (send_rc == send_result_ok) {
            *send_progress_out = true;
            ...
            if (bench_debug_enabled ()
                && g_client_debug_send_logs.fetch_add (1, std::memory_order_acq_rel) < 8) {
                std::cerr << "[multi-spot-sendsend-client] send blocked slot=" << slot.index
                          << std::endl;
            }
        } else if (send_rc != send_result_not_connected) {
            if (bench_debug_enabled ()) {
                std::cerr << "[multi-spot-sendsend-client] send request fatal slot=" << slot.index
                          << " err=" << zlink_errno () << std::endl;
            }
            return false;
        }
    ...
    for (int i = 0; i < event_count; ++i) {
        ...
        if (!drain_slot_recv (slot)) {
            if (zlink_errno () != EAGAIN && zlink_errno () != EWOULDBLOCK) {
                if (bench_debug_enabled ()) {
                    std::cerr << "[multi-spot-sendsend-client] recv after poll failed slot="
...
{
    if (!state || !reply_count_out || !latency_out) {
        errno = EINVAL;
        return false;
    }
    ...
    *latency_out = bench_latency_stats_t ();
    ...
    while (std::chrono::steady_clock::now () < deadline) {
        ...
        if (!try_submit_ready_sends (state, &server_node_rid, &server_spot_rid, run_id, msg_size,
                                     &send_progress, &has_waiting_reply))
            return false;
        if (send_progress)
            continue;
