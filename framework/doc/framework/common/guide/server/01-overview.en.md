...
=== "C#/.NET"
    ...
    > This document is the entry point of the `.NET` guide. The guide explains the concepts
...
=== "Java"
    ...
    > This document is the entry point of the `Java` guide. The guide explains the concepts
    > and usage of ZLink Framework directly so a Java developer can **read it and start
    > writing code right away.** The **language-neutral formal definition** of a concept is
    > owned by the [common spec
    > overview](../../../common/spec/02-overview.ko.md), and the **formal contract** of the
    > `Java` public API is owned by the
    > [Java exact interface index](../../../common/spec/server/languages/java/interfaces/README.ko.md)
    > document. If the two disagree, the spec wins.

=== "Kotlin"

    > This document is the entry point of the `Kotlin` guide. The guide explains the
...
=== "Node/TypeScript"
    ...
    > [Node.js exact interface index](../../../common/spec/server/languages/node/interfaces/README.ko.md)
    > document. If the two disagree, the spec wins.
...
| `options.AddRouteMesh(...)` / `AddFanoutChannel(...)` | Declare RouteMesh/fanout | [Chapter 5](05-channel-messaging.ko.md) |
...
=== "C#/.NET"
    ...
    | Operations | Runtime metrics (one `AddMeter` line), graceful drain, readiness probe | [12-operations](12-operations.ko.md) |
...
=== "Java"
    ...
    ```mermaid
    flowchart LR
      App[Spring Boot app] --> FW[ZLink Framework]
      FW --> CM[channel messaging<br/>request · send]
      FW --> PS[PUB / SUB<br/>event fan-out]
      FW --> SP[SPOT<br/>room·stage·zone·actor]
      FW --> ST[STREAM<br/>external client connector]
      CM & PS & SP & ST --> ZB[zlink Spring Boot binding]
    ```
...
=== "Node/TypeScript"
    ...
    ```mermaid
    flowchart LR
      App[NestJS app] --> FW[ZLink Framework]
      FW --> CM[channel messaging<br/>request · send]
      FW --> PS[PUB / SUB<br/>event fan-out]
      FW --> SP[SPOT<br/>room·stage·zone·actor]
      FW --> ST[STREAM<br/>external client connector]
      CM & PS & SP & ST --> ZB[zlink NestJS binding]
    ```
...
=== "Node/TypeScript"
    ...
    service, handler, and location store model. Anyone reviewing the formal public API
    contract should also read the
    [spec/interfaces index](../../../common/spec/server/languages/node/interfaces/README.ko.md);
    anyone changing the runtime's internal structure should also read
    [internals/](../../internals/backend-dependency-policy.ko.md).
...
=== "C#/.NET"
    ...
    - **Every public type the framework adapter exposes** (interface, record, enum,
      attribute, exception, DI extension method) uses the `ZLink` prefix (capital `L`). e.g.
...
=== "Java"
    ...
    - Java framework public types use the `ZLink` prefix (capital `L`). e.g.
    ...
    - The client-side Stream Connector is an independent library with no dependency on the
      server framework artifact.
    - The underlying zlink core C API is `zlink_*` snake_case.

=== "Kotlin"

    Uses the Java notation as-is. Any surface the Kotlin layer adds follows the same rule.

    - Public types use the `ZLink` prefix (capital `L`).
    - A coroutine extension is a `suspend` function, or returns a `Flow`.
    - The Maven coordinates and package are `systems.zlink.*`.
    - The underlying zlink core C API is `zlink_*` snake_case.
...
=== "Node/TypeScript"
    ...
    - Contract types use the `ZLink` prefix (capital `L`). e.g. `ZLinkRouteClient`.
    - A decorator starts lowercase. e.g. `zlinkRequestHandler`.
    - An injection token is uppercase SNAKE. e.g. `ZLINK_ROUTE_CLIENT`.
    - The npm package is `@zlink-systems/*`.
    - The underlying zlink core C API is `zlink_*` snake_case.
...
=== "C#/.NET"
    ...
    - [02-getting-started](02-getting-started.ko.md) — From the package to your first working check
