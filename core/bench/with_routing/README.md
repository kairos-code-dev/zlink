...
./core/bench/benchwithroutercompare/setup_grpc_dist.sh

# 또는 특정 경로 지정
GRPC_PREFIX=/opt/grpc ./core/bench/benchwithroutercompare/setup_grpc_dist.sh
```
...
# 빠른 테스트 (1회 반복)
./core/bench/benchwithroutercompare/run_benchmarks.sh --runs 1

# 클라이언트 수 지정
./core/bench/benchwithroutercompare/run_benchmarks.sh --runs 3 --clients 50

# zlink만 실행 (zmq/grpc는 캐시 사용)
./core/bench/benchwithroutercompare/run_benchmarks.sh --zlink-only

# 캐시 갱신
./core/bench/benchwithroutercompare/run_benchmarks.sh --refresh-cache

# 빌드 디렉토리 지정
./core/bench/benchwithroutercompare/run_benchmarks.sh --build-dir core/build
```
...
# libzmq
LIBZMQ_LIB=core/bench/benchwithroutercompare/libzmq/libzmq_dist/linux-x64/lib
