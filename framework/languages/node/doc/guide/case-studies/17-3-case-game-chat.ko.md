...
```ts
const guild = await guilds.load(guildId);
guild.requireMember(playerId);
const decision = await moderation.check(playerId, text);
if (!decision.allowed) {
  return;
}
const message = await chatHistory.appendGuild(guildId, playerId, text);
...
export class GameChatSession implements ZLinkSession {
  ...
  async onDispatch(header: ZlinkStreamHeader, payload: Message): Promise<void> {
    ...
    await this.player.relay(header, payload);
  }
}
```
...
@zlinkSpotActorSendHandler({
  spot: () => PartyRoomSpot,
  actor: () => PartyActor,
  packetName: 'SendPartyChat',
})
export class SendPartyChatHandler
  implements ZLinkSpotActorSendHandler<PartyRoomSpot, PartyActor, SendPartyChat> {
  async handle(
    spot: PartyRoomSpot,
    party: PartyActor,
