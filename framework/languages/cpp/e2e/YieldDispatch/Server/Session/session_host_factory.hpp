...
inline zlink::framework::app_t create_session_host ()
{
    ...
    app.add_zlink_framework ([=] (zlink::framework::zlink_framework_options_t &framework_options) {
        framework_options.configure_dispatch ()
          ...
          .trace_label ("cpp-yd-" + session_options.node_rid);
        server::configure_codecs (framework_options.codecs ());
        framework_options.use_discovery ().add_registry_endpoint (session_options.registry_router);
        framework_options.add_route_mesh (yd::control_channel)
