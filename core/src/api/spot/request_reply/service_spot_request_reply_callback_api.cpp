                                    ...
                                    zlink_recv_flags_t flags_)
{
    ...
    if (zlink::spot_reqrep_internal::ensure_spot_recv_ready (state) != 0)
        return zlink::recv_result_internal::from_errno (errno);
    const zlink_recv_flags_t try_flags = static_cast<zlink_recv_flags_t> (flags_ | ZLINK_DONTWAIT);
    const bool blocking = (flags_ & ZLINK_DONTWAIT) == 0;
    while (true) {
        (void) zlink::spot_reqrep_internal::drain_spot_reply_completions (state, spot_);
        const int recv_rc = zlink::spot_reqrep_internal::recv_internal_spot_queue (
          state.get (), source_rid_out_, spot_rid_out_, request_seq_out_, parts_out_,
          part_count_out_, try_flags);
        if (recv_rc == 0)
...
zlink_handler_result_t zlink_spot_dispatch_event_handler (
  void *spot_, zlink_spot_dispatch_event_handler_fn handler_, void *userdata_)
{
    if (!handler_) {
        errno = EINVAL;
...
zlink_recv_result_t zlink_spot_recv_part (void *spot_,
                                          ...
                                          zlink_recv_flags_t flags_)
{
    if (!spot_ || !source_node_rid_out_ || !source_spot_rid_out_ || !request_seq_out_ || !part_out_
        || !has_more_out_) {
        errno = EFAULT;
        return zlink::recv_result_internal::from_errno (errno);
    }
    ...
    if (zlink::part_helper_internal::prepare_recv_step (
          spot_, zlink::part_helper_internal::recv_family_spot, source_socket, &helper_state,
    ...
    if (first_part) {
        ...
        size_t part_count = 0;
        if (spot_recv_impl (spot_, &source_node_rid, &source_spot_rid, &request_seq, &parts,
                            &part_count, flags_)
            != ZLINK_RECV_OK) {
        ...
        bool move_failed = false;
        {
            ...
            helper_state->recv.return_source_spot_rid_as_null = source_spot_rid == NULL;
            zlink::part_helper_internal::copy_routing_id (source_node_rid,
                                                          &helper_state->recv.source_node_rid);
            zlink::part_helper_internal::copy_routing_id (source_spot_rid,
...

    {
        ...
        *source_node_rid_out_ =
          helper_state->recv.return_source_rid_as_null ? NULL : &helper_state->recv.source_node_rid;
        *source_spot_rid_out_ = helper_state->recv.return_source_spot_rid_as_null
                                  ? NULL
                                  : &helper_state->recv.source_spot_rid;
...
extern "C" void zlink_spot_request_reply_cleanup_spot (void *spot_)
{
    ...
    zlink::service_control_runtime_t *dispatch_runtime = NULL;
    uint64_t dispatch_task_id = 0;
    std::shared_ptr<spot_request_reply_state_t> state = try_find_spot_state (spot);
    const bool last_facade = zlink::spot_node_access_t::is_last_spot_facade_for_logical_state (
    ...
    zlink::spot_reqrep_internal::set_spot_completion_phase (
      state, zlink::spot_reqrep_internal::spot_request_reply_completion_closing);
    if (state)
        (void) zlink::spot_reqrep_internal::drain_close_spot_request_reply_state (spot_);
    if (state) {
        {
            std::lock_guard<std::mutex> dispatch_lock (state->dispatch.mutex);
            state->dispatch.subscribe_pending.clear ();
    ...
    if (state) {
        zlink::spot_reqrep_internal::set_spot_completion_phase (
          state, zlink::spot_reqrep_internal::spot_request_reply_completion_closed);
        zlink::spot_reqrep_internal::unregister_spot_channel_reply_observers (state);
        std::vector<std::shared_ptr<zlink::spot_reqrep_internal::spot_channel_reply_source_t>>
          sources;
        zlink::spot_reqrep_internal::clear_spot_channel_reply_sources (state, &sources);
    ...
    for (spot_state_identity_index_t::iterator it = spot_state_identity_index ().begin ();
...
extern "C" void zlink_spot_request_reply_cleanup_router (void *router_)
{
    ...
    if (state)
        (void) zlink::spot_reqrep_internal::drain_close_router_spot_request_reply_state (router_);
    if (state) {
        std::lock_guard<std::mutex> state_lock (state->mutex);
    ...
    for (router_state_identity_index_t::iterator it = router_state_identity_index ().begin ();
        ...
        std::shared_ptr<router_spot_request_reply_state_t> indexed = it->second.lock ();
        if (!indexed || indexed == state)
