...
int main(void)
{
    ...
    CHECK(zlink_set_routing_id(pair, rid255.data, 256)
    ...
    channel256[256] = '\0';
    CHECK(zlink_socket_set_channel_name(pair, channel256)
          == ZLINK_CONFIG_INVALID_ARGUMENT);
    ...
    CHECK(zlink_send_part_rid(router, &unknown, &failed, ZLINK_SEND_FLAGS_DONTWAIT,
    ...
    CHECK(zlink_publish_part(pair, "topic", &pub_failed, ZLINK_SEND_FLAGS_DONTWAIT,
                             ZLINK_PART_FINAL) == ZLINK_SUBMIT_NOT_SUPPORTED);
