...
from ..enums.enums import CloseResult, ConfigResult, HandlerResult, RecvResult
from ..errors.errors import CloseError, ConfigError, HandlerError, RecvError
from ..._native.ffi import lib
from ..._runtime.core.core import _raise_result_error
from ..core.utilities import _as_handle, _report_unhandled_exception
...
class Timer:
    ...
    def recv(self) -> int | None:
        """Drain the next timer fire count.

        Returns ``None`` when no fire is pending (EAGAIN). Value-return form
        is allowed for monitor/timer control-plane APIs by
        doc/spec/bindings/python/README.md §Receive And Subscribe Shape.
        """
        if not self._handle:
