...
| `wss` | `MULTI_SPOT_SENDSEND` | `보류(27.9%)` | `보류(28.9%)` | `보류(34.3%)` | `통과(52.2%)` | `보류(49.8%)` | `통과(52.5%)` | C `perf_c_multi_linux_20260522_063910_codex_c_wss_multi_for_go_20260522.txt` 대비 Go `perf_go_multi_linux_20260522_122832_codex_go_spot_sendsend_forward_routed_selected_20260522.txt`, 131072B는 fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go 반복 측정 `perf_go_multi_linux_20260524_200004.txt` 기준이다. C와 같은 size별 active slot 제한과 `SubmitNotConnected` 재시도 의미를 유지한다. 서버 echo는 새 public `Spot.ForwardRouted(...)`를 65536B 이상에 적용해 no-result였던 65536B를 유효 통과값으로 올렸고 262144B 통과권도 유지했다. 131072B는 단독 1회 측정 `perf_go_multi_linux_20260524_195651.txt`에서 50.1%였지만 반복 측정 median이 49.8%라 통과로 확정하지 않는다. small size는 기존 builder 경로를 유지한다. 이전 `Bytes(...)`/`MoveMessage(...)`/`RecvRoutedPart(...)` 후보들은 다른 size의 completion 또는 public 원본 보존 계약을 깨 반영하지 않았다. auto-HWM 활성과 size별 SpotNode `MsgUnit(B)` 일치를 확인했다. |
...
| `tls` | `MULTI_SPOT_SENDSEND` | `보류(28.4%)` | `보류(29.1%)` | `보류(30.8%)` | `보류(36.2%)` | `보류(38.3%)` | `보류(43.2%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go `perf_go_multi_linux_20260524_200213.txt` 기준이다. `SubmitNotConnected`는 C처럼 fatal이 아니라 다음 poll loop에서 재시도하는 의미로 처리한다. 서버 echo는 public `Spot.ForwardRouted(...)`를 65536B 이상에 적용해 기존 no-result였던 65536B/262144B를 유효 수치로 바꿨고, 최신 재측정에서도 전 size complete를 유지했다. 다만 fresh C 기준으로도 Go SPOT 최소 기준에는 아직 못 미친다. small size는 기존 builder 경로를 유지한다. 남은 개선은 consume-forward primitive만으로 해결되지 않는 encrypted routed echo submit 비용을 더 좁히는 쪽이다. auto-HWM 활성과 size별 SpotNode `MsgUnit(B)` 일치를 확인했다. |
