...
route_handler_registry_t::add_handler (
  ...
  invoker_t invoker)
{
  key_t key { descriptor.router_channel_id,
              descriptor.kind,
              descriptor.packet_name };
  const auto [_, inserted] = _handlers.emplace (
    std::move (key),
    entry_t { std::move (descriptor), std::move (invoker) });
  if (!inserted) {
...
route_handler_registry_t::invoke (
  std::string_view router_channel_id,
  runtime::messaging::message_kind_t kind,
...
{
  return invoke_async (
           router_channel_id, kind, packet_name, services, serializers, message, context)
    .result ();
}

task_t<zlink::message_t>
route_handler_registry_t::invoke_async (
  std::string_view router_channel_id,
  runtime::messaging::message_kind_t kind,
  std::string_view packet_name,
  service_provider_t &services,
  ...
  if (found == _handlers.end ()) {
    return task_t<zlink::message_t> (
      result_t<zlink::message_t>::failure (
        ...
        "routed handler is not registered"));
  }
