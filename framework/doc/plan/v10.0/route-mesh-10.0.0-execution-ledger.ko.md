...
| S1-15A | remote `NODROP` ingress 계약 보강 | 수신 snapshot, peer별 bounded staging, infrastructure 우선 처리와 overflow 시 peer 한정 protocol failure를 정식 spec에 명시 | 진행 중 | 2026-07-18 원격 mailbox 포화 시 silent drop 구현 gap을 확인해 `core/doc/spec/core/service/01-mesh-node.*`, `03-spot.*`, `core/doc/spec/core/07-monitoring.*` 수정 및 S3 재리뷰 재개방 |
...
| S3-11 | S1-15A 변경분 독립 재리뷰 | 한영 정식 spec이 기존 NODROP 설계와 일치하고 구현 가능한지 Codex와 Claude Fable이 독립 검토 | 미착수 | S1-15A 문서 수정 뒤 구현·test와 함께 고정 snapshot을 만들어 검토 |
...
| S4-12A | remote multicast ingress staging 구현 | 공유 ROUTER read를 막지 않는 peer별 bounded staging, fixed snapshot, 공정한 재시도, overflow peer 종료와 shutdown 회귀 통과 | 수정 중 | 2026-07-18 기존 remote ingress가 mailbox 포화 target을 silent drop하는 gap과 전체 ingress를 기다리게 한 1차 수정의 설계 위반을 확인. S1-15A 정식 계약에 맞춰 재구현 중 |
