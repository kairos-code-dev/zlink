...
> 하위 Codex 에이전트들에게 작업을 분배하고 그 결과를 직접 리뷰한 뒤,
...
- 해당 대상에 더 이상 스펙 미구현 내용이 없고, 더 이상 의미 있는
...
- 그다음 PERF 정책의 리팩토링 원칙에 따라 추가 구조 개선이 필요한 부분을
  계속 찾아 수정한다.
...
- 공통화 이후에도 변경 증폭이 줄지 않고 여러 패턴이 한 helper에 과결합되어 있음
...
- 즉 single과 multi를 각각 실행하며 `--pattern ALL --msg-sizes 64`를 사용하고,
  transport는 기본값 전체를 쓴다.
