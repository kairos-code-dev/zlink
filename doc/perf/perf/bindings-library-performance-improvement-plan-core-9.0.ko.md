...
| 현재 pattern | Single `DEALER_ROUTER_REQREP` 완료 | tcp, ws, wss, tls, inproc, ipc의 모든 크기 판정을 완료했다. 다음 pattern은 `ROUTER_ROUTER`다. |
| paired C | .NET `DEALER_ROUTER_REQREP / ipc` 완료 | 전체 크기를 CPU pin 없이 C와 .NET 순서로 각각 5회 측정했다. |
| 개선 반복 | .NET `DEALER_ROUTER_REQREP` 완료 | ipc 최소 72.2%, 중앙값 84.2%, 평균 latency 최대 약 1.36배를 확인했다. |
| 커밋과 푸시 | .NET `DEALER_ROUTER_REQREP / ipc` 문서 반영 중 | source 추가 변경은 없으며 측정 근거만 별도 커밋한다. |
...
| 2 | .NET | `PAIR`, `PUBSUB`, `DEALER_DEALER`, `DEALER_ROUTER`, `DEALER_ROUTER_REQREP` 완료 | 미측정 | `ROUTER_ROUTER / tcp` 전체 크기를 측정한다. |
