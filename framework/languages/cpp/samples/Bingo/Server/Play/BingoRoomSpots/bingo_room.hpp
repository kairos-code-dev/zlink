...
public:
  ...
  player_joined_notify_t join (std::string actor_id, std::string display_name)
  {
    ...
    _state.players.push_back ({ actor_id,
                                display_name,
                                seat,
                                host,
                                make_card (seat),
    ...
    return { _state.room_id, std::move (actor_id), std::move (display_name),
             seat, host, _state };
  }
  ...
  number_drawn_notify_t draw (int number)
  {
    ...
    for (auto &player : _state.players) {
      ...
      if (player.completed_lines > 0 &&
          std::find (_state.winners.begin (), _state.winners.end (),
                     player.actor_id) == _state.winners.end ()) {
