...
struct actor_node_registry_t
{
    ...
    spot_handle_t *find_spot_for_state (zlink::spot_node_t *node_,
                                        const std::shared_ptr<spot_logical_state_t> &state_) const;
    spot_handle_t *
    find_replacement_spot (spot_handle_t *spot_,
                           const std::shared_ptr<spot_logical_state_t> &state_) const;
    bool has_peer_spot_facade (spot_handle_t *spot_) const;
    bool collect_spots_for_node (zlink::spot_node_t *node_,
                                 const std::shared_ptr<spot_logical_state_t> &entry_state_,
                                 std::vector<actor_spot_snapshot_t> *out_) const;
    void register_node (zlink::spot_node_t *node_, const zlink_routing_id_t &node_rid_);
