...
| 5 | Go | `bindings/go/perf` | `미달 6/144 (4.2%)` | `미달 18/192 (9.4%)` | `MULTI_SPOT_REQREP tcp 4096B`는 request message 누수 수정으로 통과했고, 같은 complete 검증에서 `ws 131072B`도 통과로 회복했다. 2026-06-04 current C/Go 제한 재측정에서 `MULTI_ROUTER_ROUTER tcp 256B`도 통과로 회복해 Go multi는 10% gate 아래로 내려왔다. `MULTI_DEALER_DEALER tcp 4096B`는 Go client/server active poll wait를 deadline으로 제한해 partial에서 complete로 회복했고 통과권에 들어왔다. |
...
제한한 뒤 complete 재측정에서 통과권으로 회복했다. 나머지 잔류 미달은 18개다. Multi 잔여 항목은
