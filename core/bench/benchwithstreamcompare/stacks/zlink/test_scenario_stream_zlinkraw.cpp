...
void apply_socket_tuning (void *socket, const server_options_t &opt)
{
    ...
    // zlinkraw stack is fixed to raw stream mode in this dedicated binary.
    const int packet_mode = ZLINK_STREAM_PACKET_MODE_RAW;
    (void)zlink_setsockopt (socket, ZLINK_STREAM_PACKET_MODE, &packet_mode,
                            sizeof (packet_mode));
}

class zlink_raw_echo_server_t
{
  public:
    explicit zlink_raw_echo_server_t (const server_options_t &opt_) :
        opt (opt_),
        ctx (NULL),
        server (NULL),
        recv_msgs (0),
    ...
    ~zlink_raw_echo_server_t () { cleanup (); }

    int run ()
    {
        ctx = zlink_ctx_new ();
        if (!ctx) {
            std::fprintf (stderr, "zlinkraw stream: zlink_ctx_new failed: %s\n",
        ...
        if (!server) {
            std::fprintf (stderr, "zlinkraw stream: zlink_socket failed: %s\n",
                          zlink_strerror (zlink_errno ()));
            return 2;
        }
        ...
        if (zlink_bind (server, endpoint.c_str ()) != 0) {
            std::fprintf (
              stderr, "zlinkraw stream: bind failed: %s endpoint=%s\n",
        ...
        while (!stop.load (std::memory_order_acquire)) {
            ...
            if (prc < 0) {
                ...
                if (err == EINTR || err == ETERM)
                    break;
                std::fprintf (stderr, "zlinkraw stream: poll failed: %s\n",
                              zlink_strerror (err));
                break;
            }
            if (prc == 0)
        ...
        std::printf ("%s\n",
                     stream_echo::make_metric_line ("zlinkraw", opt.size,
                                                    recv_msgs, parse_error,
                                                    protocol_error, send_error, 0)
                       .c_str ());
...
bool parse_options (int argc, char **argv, server_options_t &opt)
{
    ...
    if (opt.size > k_max_payload_size) {
        std::fprintf (stderr, "zlinkraw stream: size too large %zu\n", opt.size);
        return false;
    }
...
{
    if (argc <= 1) {
        std::printf ("test_scenario_stream_zlinkraw: no args -> skip\n");
        return 0;
    }
    ...
    zlink_raw_echo_server_t server (opt);
    return server.run ();
}
