...
- [ ] **E2E-JV-03** (결함) — RuntimeMonitoring·ATD·OBS·StoreFailure·SpotActorTransfer·PubSub·SpotService·ToActor 해결. HTTP 호출 경로 wrapper 10 : raw 1
...
| **E2E-JV-03** (결함) | [E2E README:43-44](../../common/e2e/README.ko.md): "client는 **언어별 HTTP client wrapper**를 사용한다 … raw `HttpClient`로 e2e app endpoint를 직접 호출하지 않는다" | **부분 해결:** `RuntimeMonitoring`, `AutomaticTurnDispatch`, `ObservabilityOps`, `StoreFailure`, `SpotActorTransfer`, `PubSub`, `SpotService`, `ToActorMessaging`의 HTTP 호출 코드에 raw client가 남으면 실행 전에 실패하는 gate를 추가해 각각 기존 `TriggerScenarioClient`, evidence GET helper, drain 요청, Shared의 혼합 `HttpSupport`, Client `Program`, PubSub의 네 Client 보조 클래스, SpotService의 `Program`과 `GatewayScenarioClient`, ToActorMessaging Shared의 client 정적 메서드에서 red를 확인했다. RuntimeMonitoring은 역할 endpoint별 `ZLinkHttpClient` 하나를 소유하고 종료 시 닫으며, 기존 5분 request timeout과 raw status/body 단언을 유지한다. MON-A1 대표 실행과 MON-A1~D1 전체 실행이 통과했다. ATD는 evidence URL이 여러 역할을 가리키므로 기존 GET helper 안에서 URL을 wrapper의 base URL과 path로 나누고 one-shot wrapper가 요청 뒤 자동으로 자원을 정리하게 했다. ATD-A1 대표 실행과 ATD-A1~E5 전체 실행이 통과했다. ObservabilityOps Trigger의 유일한 app HTTP 호출인 drain 요청도 같은 public wrapper를 사용하고 2xx를 단언하며 OBS-C4가 통과했다. 직전 OBS-A1~C5 전체 실행도 같은 runner에서 통과했지만 독립 Client 부재는 E2E-JV-01에 남는다. StoreFailure는 Shared `HttpSupport`에서 client GET/POST를 제거하고 Client 전용 `StoreFailureHttpClient`가 wrapper의 수명·5초 GET·10초 POST·2xx 검증을 소유한다. SF-A1 대표 실행이 통과했고 전체 실행은 SF-A1~D1까지 통과한 뒤 기존 SF-D2 traffic stall 경계가 6.068초로 한 번 실패했다. SF-D2 단독과 뒤의 SF-D3·E1은 통과했으며 sleep이나 retry를 추가하지 않았다. SpotActorTransfer는 node A·B·C별 wrapper를 Client가 소유하고 URI의 authority에 맞는 client와 path를 내부에서 선택한다. 기존 14초 비동기 POST와 5초 GET 상한을 보존했으며 `:Client:compileJava`와 ST-A1~F6 20개 selector가 통과했다. 실행 중 두 번의 node readiness 실패는 로그에서 `ZlinkBindException`을 확인한 뒤 같은 selector만 재실행했고, 기능 재시도나 대기 확대는 추가하지 않았다. PubSub는 문서 실측에서 빠졌던 `ServerProcessLauncher`를 포함해 `ScenarioContext`·`PublisherClient`·`Evidence`의 raw transport를 제거했다. Client 전용 `PubSubHttpClient`가 endpoint별 wrapper 재사용, URL 분해, 2xx 검사와 종료를 소유하며 `ScenarioContext`는 이를 외부에 노출하지 않고 프로그램 종료 때 닫는다. 서버 evidence wait의 기존 15초 가능성을 5초로 줄이던 첫 수정은 재리뷰에서 폐기하고, 기존에 300ms였던 process health probe만 같은 상한을 유지했다. `:Client:compileJava`와 PS-A1~C1 7개 selector가 통과했다. SpotService는 같은 2분 POST를 중복 구현하던 `Program`과 `GatewayScenarioClient` 중 기존 Client 역할 객체 하나만 남기고, 요청마다 닫히는 wrapper를 그 안에서 사용한다. `SM-A1`, `default-batch`, `SM-F6`, `SM-G1~G4`를 포함한 `all` runner가 통과했다. 서버가 `/scenario/<mode>`에서 검증을 대신하는 구조는 HTTP 전환으로 완료 처리하지 않고 E2E-JV-06에 계속 남겼다. ToActorMessaging은 Shared `JsonHttp`에서 client GET/POST를 제거해 서버 endpoint 책임만 남기고 Client 전용 `ToActorHttpClient`가 wrapper의 typed JSON 경로와 2xx 검사를 소유한다. wrapper 기본 timeout이 30초 route-failure 결과보다 먼저 끝나 TA-B3가 red였고, 기존 내부 경계를 수용하는 35초 HTTP 상한을 명시한 뒤 TA-B3 단독과 TA-A1~B3 전체 실행이 통과했다. POSD/DDD 재리뷰에서는 JDK transport 타입 누출, 여러 PubSub 보조 클래스의 endpoint/상태/수명 정책 중복, SpotService의 같은 POST 구현 중복, Shared의 server/client 책임 혼합을 위험 신호로 분류했다. StoreFailure와 ToActorMessaging의 Shared에 wrapper 의존성을 더하는 안 대신 Client 역할 client를 분리해 server 보조 코드는 Shared에 유지하고 client transport 정책은 Client에 가뒀다. RuntimeMonitoring과 SpotActorTransfer는 기존 역할·node client가 wrapper 수명과 endpoint 선택을 함께 소유하는 안을, ATD와 OBS는 이미 한 곳뿐인 호출 경계에서 one-shot wrapper를 사용하는 안을 선택해 변경 지점을 늘리지 않았다. SpotActorTransfer에서 요청마다 wrapper를 만드는 대안은 연결 수명과 endpoint 선택 정책이 반복되므로 채택하지 않았다. PubSub 각 보조 클래스가 wrapper를 따로 소유하는 대안 대신 Client 내부 HTTP 모듈 하나가 가변 endpoint의 wrapper를 관리하게 해 생성·상태·종료 지식을 한곳에 가뒀다. SpotService 두 호출부에 wrapper를 각각 두는 대안 대신 기존 `GatewayScenarioClient`를 재사용해 transport 정책 중복을 없앴다. 여덟 config 모두 domain model은 변경하지 않았고 ToActorMessaging은 transport 의존성을 Shared server helper에서 Client로 옮겨 경계를 좁혔다. 현재 HTTP 호출 경로 실측은 wrapper 10(`RuntimeMonitoring`·`AutomaticTurnDispatch`·`ObservabilityOps`·`StoreFailure`·`SpotActorTransfer`·`PubSub`·`SpotService`·`ToActorMessaging`·`RegistryMessaging`·`RegistrationCodec`) 대 raw 1이다. 남은 raw 경로는 `ResilienceLifecycle/Client/Support/ConsumerScenarioClient.java`다. `RegistrationCodec/Client/Support/Evidence.java`와 `RegistryMessaging/Client/Support/DynamicClusterLauncher.java`에도 wrapper 밖 raw 보조 경로가 남으므로 이 gap은 open이다. |
