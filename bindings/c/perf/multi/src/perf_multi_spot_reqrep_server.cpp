...
bool accept_ready_barrier_payload (spot_reqrep_server_state_t *state, const std::string &payload)
{
    ...
    if (perf_multi_spot_handshake::parse_data_endpoint_command (payload.data (), payload.size (),
                                                                &data_endpoint)) {
        return perf_multi_spot_handshake::register_peer (&state->data_peers, data_endpoint)
               && perf_multi_spot_control::ensure_connected_peers (state->node, state->data_peers);
    }

    if (perf_multi_spot_handshake::parse_ready_count_command (payload.data (), payload.size (),
                                                              &ready_size, &ready_count)) {
        perf_multi_spot_handshake::record_ready_count (&state->ready_state, ready_size,
                                                       ready_count);
        return true;
    }

    if (perf_multi_spot_handshake::parse_ready_slot_command (payload.data (), payload.size (),
                                                             &ready_size, &slot_index)) {
        perf_multi_spot_handshake::record_ready_slot (&state->ready_state, ready_size, slot_index);
    }

    return true;
}

bool ready_barrier_satisfied (spot_reqrep_server_state_t *state, size_t msg_size)
{
    return state && state->control_connected.load (std::memory_order_acquire)
           && perf_multi_spot_handshake::ready_units (&state->ready_state, msg_size)
                >= state->expected_ready_count;
}

bool wait_for_data_peers_ready (spot_reqrep_server_state_t *state, int timeout_ms)
{
    if (!state || !state->node) {
        errno = EINVAL;
        return false;
    ...
    const size_t expected_connected = std::max<size_t> (1, state->data_peers.endpoints.size ());
    return perf_multi_spot_control::wait_for_connected_peer_count (state->node, expected_connected,
                                                                   timeout_ms, &state->fatal_errno);
}
...
{
    return handle && text && *text && zlink_set_routing_id (handle, text, std::strlen (text)) == 0;
}

bool apply_replier_options (void *spot, const multi_bench_settings_t &settings)
{
    if (!spot)
        return false;

    apply_benchmark_socket_options (spot, settings.hwm, "tcp", ZLINK_SOCKET_DEALER,
                                    perf_current_benchmark_max_msg_size (64));
    return true;
}

bool initialize_reqrep_server_session (ctx_guard_t &ctx,
                                       ...
                                       perf_multi_spot_control::server_session_t *session)
{
    ...
    if (!session->pub || !session->control_pub || !session->control_sub
        ...
        || !perf_multi_spot_control::apply_control_options (session->control_pub,
                                                            session->control_sub, settings)
        || zlink_set_subscription (session->control_sub, k_topic) != ZLINK_CONFIG_OK) {
        perf_multi_spot_control::destroy_server_session (session);
        return false;
    }

    if (perf_set_spot_node_router_endpoint_raw (session->node,
                                                make_endpoint (transport, token + "-router"))
        != 0) {
        perf_multi_spot_control::destroy_server_session (session);
        return false;
    }
    ...
    session->control_endpoint =
      perf_multi_spot_control::bind_control_endpoint (session->control_node, transport);
...
void on_spot_dispatch (void *spot, const zlink_spot_dispatch_info_t *info, void *userdata)
{
    spot_reqrep_server_state_t *state = static_cast<spot_reqrep_server_state_t *> (userdata);
    if (!state || !info || info->event != ZLINK_SPOT_DISPATCH_EVENT_ROUTED_READABLE) {
        return;
    }
    ...
    for (;;) {
        ...
        if (rc != ZLINK_RECV_OK) {
            ...
            if (bench_debug_enabled ()) {
                std::cerr << "[multi-spot-reqrep-server] recv failed err=" << saved_errno
                          << std::endl;
            }
        ...
        if (!source_rid || source_rid->size == 0 || !source_spot_rid || source_spot_rid->size == 0
            ...
            if (bench_debug_enabled ()) {
                std::cerr << "[multi-spot-reqrep-server] invalid routed recv"
                          ...
                          << " spot=" << (source_spot_rid ? source_spot_rid->size : 0)
                          << " seq=" << request_seq << " parts=" << part_count << std::endl;
            }
        ...
        if (bench_debug_enabled ()
            ...
            std::cerr << "[multi-spot-reqrep-server] recv request seq=" << request_seq
                      << " parts=" << part_count << std::endl;
        }
        if (spot_trace_enabled ()
            && g_server_trace_recv_logs.fetch_add (1, std::memory_order_acq_rel) < 8) {
            ...
            std::cerr << "[multi-spot-reqrep-trace] server recv seq=" << request_seq
                      << " payload_seq=" << (decoded ? header.seq : 0) << " recv_delay_ms="
        ...
        const uint64_t reply_begin_ns = spot_trace_enabled () ? perf_multi_metric::now_ns () : 0;
        const zlink_submit_result_t reply_rc = perf_zlink_spot_reply_spot_parts (
          spot, source_rid, source_spot_rid, request_seq, parts, part_count);
        if (reply_rc != ZLINK_SUBMIT_OK) {
            const int reply_errno = zlink_errno ();
            if (is_peer_disconnect_submit (reply_rc) || is_transient_submit_errno (reply_errno)) {
                zlink_multipart_close (parts, part_count);
                continue;
            }
            if (bench_debug_enabled ()) {
                std::cerr << "[multi-spot-reqrep-server] reply failed err=" << reply_errno
                          << std::endl;
            }
            zlink_multipart_close (parts, part_count);
        ...
        if (spot_trace_enabled ()
            && g_server_trace_reply_logs.fetch_add (1, std::memory_order_acq_rel) < 8) {
            const uint64_t reply_end_ns = perf_multi_metric::now_ns ();
            std::cerr << "[multi-spot-reqrep-trace] server reply seq=" << request_seq
                      << " submit_ms=" << ((reply_end_ns - reply_begin_ns) / 1000000.0)
                      << std::endl;
        }
    }
}

bool wait_for_ready_barrier (spot_reqrep_server_state_t *state, size_t msg_size, int timeout_ms)
{
    ...
    while (std::chrono::steady_clock::now () < deadline) {
        bool received = false;
        std::string payload;
        if (!perf_multi_spot_control::receive_control_payload (state->control_sub, k_topic,
                                                               &payload, &received)) {
            return false;
        }

        if (received && !payload.empty () && !accept_ready_barrier_payload (state, payload)) {
            return false;
        }
        ...
        if (!perf_multi_spot_control::wait_for_control_readable (state->control_sub, deadline)) {
            return false;
        }
    }
                             ...
                             const spot_reqrep_server_state_t *state)
{
    while (std::chrono::steady_clock::now () < deadline) {
        if (perf_stop_requested ().load (std::memory_order_acquire)
            || (state && state->fatal_errno.load (std::memory_order_acquire) != 0)) {
            errno = EIO;
            return false;
        }
                      ...
                      const std::vector<size_t> &msg_sizes)
{
    ...
    for (size_t i = 0; i < msg_sizes.size (); ++i) {
        ...
        if (!apply_benchmark_context_auto_hwm_msg_unit (ctx, msg_size))
            return false;
        if (!perf_multi_handshake::wait_for_start (&state->start_gate, msg_size,
                                                   start_timeout_ms)) {
            if (bench_debug_enabled ())
                std::cerr << "[multi-spot-reqrep-server] runner start timeout size=" << msg_size
                          << std::endl;
            return false;
        }
        if (!perf_multi_spot_control::ensure_connected_peers (state->control_node,
                                                              state->control_peers)) {
            if (bench_debug_enabled ())
                std::cerr << "[multi-spot-reqrep-server] ensure control peers failed size="
                          << msg_size << " err=" << zlink_errno () << std::endl;
            return false;
        }
        if (!wait_for_ready_barrier (state, msg_size, start_timeout_ms)) {
            if (bench_debug_enabled ())
                std::cerr << "[multi-spot-reqrep-server] ready barrier failed size=" << msg_size
                          << " err=" << zlink_errno () << std::endl;
            return false;
        }
        if (!wait_for_data_peers_ready (state, start_timeout_ms)) {
            if (bench_debug_enabled ())
                std::cerr << "[multi-spot-reqrep-server] data peer ready timeout size=" << msg_size
                          << " expected="
                          << std::max<size_t> (1, state->data_peers.endpoints.size ())
                          << " err=" << zlink_errno () << std::endl;
            return false;
        }
        if (!perf_multi_spot_control::publish_start (state->control_pub, k_topic, msg_size)) {
            if (bench_debug_enabled ())
                std::cerr << "[multi-spot-reqrep-server] start publish failed size=" << msg_size
                          << " err=" << zlink_errno () << std::endl;
            return false;
        }
        ...
        if (!idle_until_server_stop (deadline, state)) {
            if (bench_debug_enabled ())
                std::cerr << "[multi-spot-reqrep-server] active wait failed size=" << msg_size
                          << " err=" << zlink_errno () << std::endl;
            return false;
        }
    }
...
int run_server_benchmark (const std::string &lib_name, const std::string &transport)
{
    ...
    if (!initialize_reqrep_server_session (
          ctx, transport, lib_name + std::string ("_spot_reqrep_server"), settings, &session)) {
        return 1;
    }
    ...
    perf_multi_spot_control::prepare_server_runtime (
      &state, session, state.expected_ready_count, settings.connect_ready_timeout_ms,
      [] (spot_reqrep_server_state_t *server_state, size_t start_size) {
          perf_multi_handshake::signal_start (&server_state->start_gate, start_size);
      });
    if (zlink_spot_dispatch_event_handler (state.pub, on_spot_dispatch, &state) != 0) {
        return 1;
    }
