    ...
    int LocationGraceMs)
{
    public static ServerOptions Parse(string[] args, string defaultRole)
    {
        ...
        return new ServerOptions(
            ...
            int.TryParse(Get("--location-heartbeat-ms"), out var heartbeat) ? heartbeat : 1000,
            int.TryParse(Get("--location-lease-ttl-ms"), out var leaseTtl) ? leaseTtl : 3000,
            int.TryParse(Get("--location-polling-ms"), out var polling) ? polling : 500,
            int.TryParse(Get("--location-grace-ms"), out var grace) ? grace : 6000);
    }
}