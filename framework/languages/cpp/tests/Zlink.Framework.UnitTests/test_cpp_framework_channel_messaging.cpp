...
int main ()
{
    ...
    auto request_result = request_call.submit_async ().result ();
    if (request_result
    ...
    auto send_result = send_call.submit_async ().result ();
    if (!send_result) {
        return 3;
    }
    ...
    auto disconnected_result = bus.send ("missing", request_t{4}).submit_async ().result ();
    ...
    auto queue_full_result =
      full_queue.message_bus ().request<request_t, reply_t> ("profile", {5}).submit_async ().result ();
