...
| `ZLINK_POLLOUT` | 2 | send recovery readiness. 해당 핸들이 backpressure 상태에서 벗어나 송신을 다시 시도할 가치가 있음을 뜻합니다. 단순히 transport가 쓰기 가능한 상태라는 뜻이 아니며, 재시도가 반드시 성공한다는 보장도 아닙니다. `zlink_send_ready_handler()` 콜백과 동일한 readiness 축을 공유합니다. `ZLINK_SUBMIT_BACKPRESSURED` 이후 이 신호가 관찰되면 재시도할 수 있지만, 재시도가 다시 `BACKPRESSURED`로 실패할 수도 있습니다. |
