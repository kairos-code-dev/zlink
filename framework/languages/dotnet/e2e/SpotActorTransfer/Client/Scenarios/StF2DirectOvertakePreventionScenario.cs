...
internal static class StF2DirectOvertakePreventionScenario
{
    public static async Task RunAsync(SpotActorTransferScenarioContext context)
    {
        ...
        var joinTask = context.JoinAsync(context.NodeA, actorId, new JoinTargetReq("ST-F2", spotRid));
        await context.WaitEvidenceAsync(context.NodeB, [$"ST-F2|{actorId}|joined_wait|{spotRid}"]);
