...
template <typename SocketT> class has_routed_send_t
{
  private:
    template <typename T>
    static auto test (int) -> decltype (std::declval<T &> ().send (std::declval<const zlink::routing_id_t &> (),
...
{
  private:
    template <typename T> static auto test (int) -> decltype (std::declval<T &> ().send (), std::true_type ());
...
template <typename SocketT> class has_receive_t
{
  private:
    template <typename T>
    static auto test (int) -> decltype (std::declval<T &> ().recv (std::declval<zlink::received_t &> ()),
                                        std::true_type ());
...
template <typename SocketT> class has_single_part_recv_t
{
  private:
    template <typename T>
    static auto test (int) -> decltype (std::declval<T &> ().recv (std::declval<zlink::message_t &> ()),
                                        std::true_type ());
...
template <typename SocketT> class has_send_no_wait_t
{
  private:
    template <typename T>
    static auto test (int) -> decltype (std::declval<T &> ().send_no_wait (std::declval<zlink::send_result_t &> (),
                                                                           std::declval<zlink::message_t &> ()),
                                        std::true_type ());
...
template <typename SocketT> class has_publish_no_wait_t
{
  private:
    template <typename T>
    static auto test (int) -> decltype (std::declval<T &> ().publish_no_wait (std::declval<zlink::send_result_t &> (),
...
template <typename SocketT> class has_subscribe_part_t
{
  private:
    template <typename T>
    static auto
    test (int) -> decltype (std::declval<T &> ().subscribe_part (std::declval<std::optional<zlink::routing_id_t> &> (),
...
template <typename SocketT> class has_routed_single_part_recv_t
{
  private:
    template <typename T>
    static auto test (int) -> decltype (std::declval<T &> ().recv (std::declval<zlink::routing_id_t &> (),
                                                                   std::declval<zlink::message_t &> ()),
                                        std::true_type ());
...
template <typename SocketT> class has_raw_common_option_get_t
{
  private:
    template <typename T>
    static auto test (int) -> decltype (std::declval<T &> ().get_option (0, static_cast<int *> (nullptr)),
                                        std::true_type ());
...
template <typename SocketT> class has_attach_discovery_t
{
  private:
    template <typename T>
    static auto
    test (int) -> decltype (std::declval<T &> ().attach_discovery (std::declval<zlink::service::discovery_t &> ()),
                            std::true_type ());
...
template <typename SocketT> class has_connect_t
{
  private:
    template <typename T>
    static auto test (int) -> decltype (std::declval<T &> ().connect (std::declval<const std::string &> ()),
                                        std::true_type ());
...
template <typename SocketT> class has_disconnect_t
{
  private:
    template <typename T>
    static auto test (int) -> decltype (std::declval<T &> ().disconnect (std::declval<const std::string &> ()),
                                        std::true_type ());
...
template <typename SocketT> class has_disconnect_rid_t
{
  private:
    template <typename T>
    static auto
    test (int) -> decltype (std::declval<T &> ().disconnect_rid (std::declval<const zlink::routing_id_t &> ()),
                            std::true_type ());
...
{
  private:
    template <typename T> static auto test (int) -> decltype (std::declval<T &> ().recv_spot (), std::true_type ());
...
static_assert (!has_routed_send_t<zlink::pair_socket_t>::value, "pair_socket_t must not expose routed send");
static_assert (has_send_builder_t<zlink::pair_socket_t>::value, "pair_socket_t must expose send builder");
static_assert (has_receive_t<zlink::pair_socket_t>::value, "pair_socket_t must expose recv");
static_assert (has_single_part_recv_t<zlink::pair_socket_t>::value, "pair_socket_t must expose single-part recv");
static_assert (!has_attach_discovery_t<zlink::pair_socket_t>::value, "pair_socket_t must not expose attach_discovery");
...
static_assert (!has_routed_send_t<zlink::dealer_socket_t>::value, "dealer_socket_t must not expose routed send");
static_assert (has_send_builder_t<zlink::dealer_socket_t>::value, "dealer_socket_t must expose send builder");
static_assert (has_receive_t<zlink::dealer_socket_t>::value, "dealer_socket_t must expose recv");
static_assert (has_single_part_recv_t<zlink::dealer_socket_t>::value, "dealer_socket_t must expose single-part recv");
static_assert (has_send_no_wait_t<zlink::dealer_socket_t>::value,
               "dealer_socket_t must expose nonblocking direct send result");
static_assert (has_attach_discovery_t<zlink::dealer_socket_t>::value, "dealer_socket_t must expose attach_discovery");
static_assert (!has_routed_send_t<zlink::router_socket_t>::value, "router_socket_t must not expose direct routed send");
...
static_assert (!has_recv_spot_t<zlink::router_socket_t>::value, "router_socket_t must not expose recv_spot");
static_assert (has_attach_discovery_t<zlink::router_socket_t>::value, "router_socket_t must expose attach_discovery");
static_assert (has_attach_discovery_t<zlink::pub_socket_t>::value, "pub_socket_t must expose attach_discovery");
static_assert (has_publish_builder_t<zlink::pub_socket_t>::value, "pub_socket_t must expose publish builder");
static_assert (has_publish_no_wait_t<zlink::pub_socket_t>::value,
               "pub_socket_t must expose nonblocking direct publish result");
static_assert (has_attach_discovery_t<zlink::sub_socket_t>::value, "sub_socket_t must expose attach_discovery");
static_assert (has_subscribe_part_t<zlink::sub_socket_t>::value, "sub_socket_t must expose single-part subscribe");
static_assert (!has_attach_discovery_t<zlink::xpub_socket_t>::value, "xpub_socket_t must not expose attach_discovery");
static_assert (has_publish_builder_t<zlink::xpub_socket_t>::value, "xpub_socket_t must expose publish builder");
static_assert (has_publish_no_wait_t<zlink::xpub_socket_t>::value,
               "xpub_socket_t must expose nonblocking direct publish result");
static_assert (!has_attach_discovery_t<zlink::xsub_socket_t>::value, "xsub_socket_t must not expose attach_discovery");
static_assert (has_subscribe_part_t<zlink::xsub_socket_t>::value, "xsub_socket_t must expose single-part subscribe");
static_assert (!has_routed_send_t<zlink::stream_socket_t>::value, "stream_socket_t must not expose direct routed send");
...
static_assert (!has_connect_t<zlink::stream_socket_t>::value, "stream_socket_t must not expose connect");
static_assert (!has_disconnect_t<zlink::stream_socket_t>::value, "stream_socket_t must not expose disconnect");
static_assert (!has_disconnect_rid_t<zlink::stream_socket_t>::value, "stream_socket_t must not expose disconnect_rid");
...
void test_dealer_send_no_wait_direct ()
{
    ...
    zlink::routing_id_t source = zlink::routing_id_t::from (reinterpret_cast<const uint8_t *> ("placeholder"), 11);
    zlink::message_t inbound;
...
void test_router_recv_single_part_direct ()
{
    ...
    const zlink::routing_id_t dealer_id = zlink::routing_id_t::from (reinterpret_cast<const uint8_t *> ("dealer-a"), 8);
    dealer.set_routing_id (dealer_id);
...
void test_router_direct_recv_multipart_failure_preserves_output ()
{
    ...
    const zlink::routing_id_t dealer_id = zlink::routing_id_t::from (reinterpret_cast<const uint8_t *> ("dealer-b"), 8);
    dealer.set_routing_id (dealer_id);
