...
Typed request의 reified entry method는 wrapper를 만들 때 `KClass<TReply>.java`를 내부 Java `Class<TReply>`로
고정한다. Application은 terminal에서 reply type을 다시 넘기거나 `.submit().await()`를 작성하지 않는다.
