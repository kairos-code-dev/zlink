...
- **2026-05-23 근본 원인 규명 + 수정 (strace 프로파일링)**: 위 large many-client 병목의 진짜 원인을 strace로 특정했다.
  ...
  - **남은 보류(수정 후에도 목표 미달, 후속 개선 대상)**: `MULTI_DEALER_DEALER`의 wss 64B, tls 64B copy/FFI 비용.
    `wss MULTI_SPOT 1024B`와 일부 64B per-message FFI cost는 LockOSThread와 hot-path 축소로 부분 개선됐으나
    목표 미만이라 계속 추적한다. `MULTI_SPOT_REQREP` tcp 262144B는 fresh C/current Go 제한 재측정에서
    50.1%로 기준을 넘어 남은 보류 목록에서 제거했다.
