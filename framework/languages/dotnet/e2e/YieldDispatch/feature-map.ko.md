...
| YD-A3 | 부분 구현 | request id와 spot rid 보존은 검증한다. Spot request handler public surface가 request metadata를 직접 전달하지 않아 metadata 보존 검증은 후속 public contract 검토가 필요하다. |
...
| YD-C3 | 미구현 | actor yield와 timer yield를 서로 교차시키는 조합 검증은 아직 별도 scenario로 분리하지 않았다. |
...
| YD-E3 | 미구현 | yield 중 play 노드 정상 종료와 routing id 재사용 검증은 아직 별도 process-control scenario로 분리하지 않았다. |
| YD-E4 | 구현 | runner가 HTTP trigger/client 사용과 Play Spot/Entry Spot handler 밖의 `.Yield` 사용을 정적으로 검사한다. |
| YD-E5 | 미구현 | 다른 framework 언어의 Config 8 report를 같은 scenario id로 집계하는 단계는 아직 추가하지 않았다. |
