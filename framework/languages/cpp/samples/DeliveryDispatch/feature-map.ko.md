...
| 항목 | 상태 | 근거 |
|------|------|------|
| `DD-A1` registry readiness | 구현 | `Probe` 실행 파일이 registry discovery로 Tracking route readiness를 확인한다. 최신 통과: `timeout 420s framework/languages/cpp/samples/DeliveryDispatch/run_sample.sh`, 출력: `deliverydispatch sample result=passed`. |
