                              ...
                              latency_stats_t *out_latency)
{
    ...
    std::thread receiver_thread ([&] () {
        ...
        if (!recv_poller.valid ()
            || !recv_poller.add (router, router, ZLINK_POLLIN)) {
        ...
        while (true) {
            ...
            const int recv_rc = recv_router_header_flags (
              router, payload_size, ZLINK_DONTWAIT, &header, &header_ok);
            if (recv_rc > 0) {
                ...
                for (;;) {
                    ...
                    const int burst_rc = recv_router_header_flags (
                      router, payload_size, ZLINK_DONTWAIT, &burst_header,
                      &burst_header_ok);
                    if (burst_rc > 0) {
                        last_recv_at = std::chrono::steady_clock::now ();
    ...
    if (!send_poller.valid ()
        || !send_poller.add (dealer, dealer, ZLINK_POLLOUT)) {
    ...
    while (std::chrono::steady_clock::now () < deadline) {
        ...
        const int send_rc =
          send_single_part_with_backpressure (
            dealer, &part, &send_poller, deadline);
        if (send_rc < 0) {
