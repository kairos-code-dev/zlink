...
# ShoppingMallCheckout Sample (Java/Kotlin)

> 언어 중립 시나리오 정본은 [공통 샘플 — ShoppingMallCheckout](../../../../../doc/spec/sample/event/shoppingmall-checkout.ko.md)다.
> 실행 코드는 `samples/java/ShoppingMallCheckout`(Java), `samples/kotlin/ShoppingMallCheckout`(Kotlin)에 있다.
...
dotnet 행위 정본은 `samples/dotnet/.../ShoppingMall`이다(ShoppingMallCheckout dir에는 stale
