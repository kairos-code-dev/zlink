...
| `PS-A3` | `Client/Scenarios/LateSubscriberScenario.cs` | `Client/Scenarios/ps-a3-late-subscriber-scenario.ts` | done | 늦게 시작한 subscriber가 이전 publish를 replay하지 않는 marker를 실제 subscriber 역할 server evidence로 확인 |
| `PS-A4` | `Client/Scenarios/SubscriberReconnectScenario.cs` | `Client/Scenarios/ps-a4-subscriber-reconnect-scenario.ts` | done | subscriber 재시작 후 disconnect gap을 replay하지 않는 marker를 실제 subscriber 역할 server evidence로 확인 |
...
| `PS-B2` | `Client/Scenarios/PublisherRestartScenario.cs` | `Client/Scenarios/ps-b2-publisher-restart-scenario.ts` | done | publisher 재시작 뒤 새 publish가 다시 전달되는 marker를 실제 subscriber 역할 server evidence로 확인 |
