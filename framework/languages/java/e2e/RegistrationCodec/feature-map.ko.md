...
이 앱은 `framework/doc/framework/common/e2e/config-4-registration-codec.ko.md`의 Java 지원 범위를 검증한다.

| 시나리오 | 상태 | 근거 |
|----------|------|------|
| RC-A1 자동 등록 | 구현 | `addHandlersFromPackageOf`와 handler group으로 interface handler를 스캔해 request/send를 처리한다. |
| RC-A2 annotation 등록 | 구현 | Java `@ZLinkHandlerGroup`, `@ZLinkRequest`, `@ZLinkSend`로 request/send를 처리한다. |
...
| RC-B1 JSON codec | 구현 | JSON DTO request/send가 왕복하고 evidence에 남는다. 현재 Java channel context는 content-type을 공개하지 않는다. |
