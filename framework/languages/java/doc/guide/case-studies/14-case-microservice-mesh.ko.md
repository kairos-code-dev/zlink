...
```java
@Component
public final class QuoteHandler implements ZLinkRequestHandler<Quote, QuoteDto> {
    private final PriceStore store;
    @Override
    public QuoteDto handle(Quote req, ZLinkRequestContext context) { return store.quote(req.userId()); }
}
```
...
public class MeshOpsConfig implements ZLinkFrameworkConfigurer {
    @Override
