...
function activeSpotSlotLimit(totalSlots, msgSize) {
  const override = Number(process.env.PERF_MULTI_SPOT_REQREP_ACTIVE_SLOTS || 0);
...
function pollCompletionUntil(poller, pollBuffer, stopNs) {
  ...
  const remainingMs = Number((BigInt(remainingNs) + 999_999n) / 1_000_000n);
...
async function main() {
  ...
  try {
    ...
    try {
      ...
      const drainStopNs =
        currentEpochNs() + BigInt(Math.max(1000, requestTimeoutMs * 4)) * 1_000_000n;
