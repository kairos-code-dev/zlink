...
| **S8-DN lane (.NET bindings→framework) [참조 lane]** | framework core-correct | 0 | 0 | 0 | **bindings CLEAN**(iter-3 두 리뷰어 CLEAN, 197 P/Invoke⊆ABI). **framework compile-green + 런타임 정확성**(`387fcb62c`): framework-owned dispatch record+DrainReady pump(Option B, bindings 무변경)·lifecycle/channel sequencing·stream session completion·straggler relay·native transfer 배선. 잔여=S8-02A live-weight DI·MeshName selection·S8-04A Redis transfer authority(분산)·samples/E2E·`DOTNET REVIEW CLEAN` |
