...
| 현재 pattern | Single `DEALER_ROUTER_REQREP` 진행 중 | tcp를 완료했고 다음 transport는 ws다. |
| paired C | .NET `DEALER_ROUTER_REQREP / tcp` 완료 | C 전체 크기 5회 뒤 perf 의미를 바로잡고 .NET 전체 크기를 5회 측정했다. |
| 개선 반복 | .NET `DEALER_ROUTER_REQREP / tcp` 완료 | 최소 66.4%, 크기 중앙값 83.1%, 평균 latency 최대 약 1.47배를 확인했다. |
| 커밋과 푸시 | .NET request/reply pipeline budget 반영 중 | perf 수정, 기능 test, 측정 근거를 함께 커밋하고 푸시한다. |
...
| 2 | .NET | `PAIR`, `PUBSUB`, `DEALER_DEALER`, `DEALER_ROUTER` 완료, `DEALER_ROUTER_REQREP` tcp 완료 | 미측정 | `DEALER_ROUTER_REQREP / ws` 전체 크기를 측정한다. |
