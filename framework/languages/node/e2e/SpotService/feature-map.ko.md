...
| SM-A5 | 구현 | app-level `ScenarioStage` wrapper, `StageProbeReq`, `StageTimerStartCommand`, stage timer handler가 routed readiness request 뒤 같은 spot serial 흐름에서 동작하는지 검증한다. 선택 PASS: `logs/20260630-081839-3241830` |
...
| SM-A8 | 구현 | public `executeOnSpot(...)` 안에서 `spot.context.runWorker(...)`를 시작하고, worker completion 전 다른 `executeOnSpot(...)` state turn이 처리되는 evidence 순서를 검증한다. 로그: `logs/20260630-074201-3148526` |
...
| SM-C4 | 구현 | local spot factory가 없는 Gateway role이 public `ZLinkSpotPublisherClient.publishSpot(...)`로 publish하고, Play-A subscribed spot이 같은 marker의 `SpotEvent`를 받으며 미구독 alternate spot은 받지 않는지 검증한다. Play-A가 Gateway publish endpoint에 `connectPeerPub(...)`로 붙는 `.NET` 기준 topology를 사용한다. PASS: `logs/20260630-083734-3303700` |
...
| SM-D3 | 구현 | entry actor stream bind 뒤 `ActorPushReq` reply와 bound session push를 검증한다. user spot bind는 `UserSpotAuthReq`로 spot/actor join marker를 남기고 `UserActorPingReq`/`UserActorPushReq` relay reply, user spot rid, push payload, `actor-ping` evidence를 검증한다. 선택 PASS: `logs/20260629-212739-1577626`; `all` PASS: `logs/20260630-074201-3148526` |
...
| SM-D11 | 구현 | 같은 client flow에서 stream `ActorPingReq`와 Session HTTP channel control-ping을 차례로 호출해 stream reply와 channel reply가 서로 간섭 없이 각 경로로 돌아오는지 검증한다. 선택 PASS: `logs/20260629-220146-1678135`; `all` PASS: `logs/20260630-074201-3148526` |
...
- 선택 scenario: `timeout 360s framework/languages/node/e2e/SpotService/run_e2e.sh SM-B1`
  - PASS: `logs/20260630-070738-3054201` (`entry-created`, `entry-joined`, actor ping evidence 확인)
...
- 선택 scenario: `timeout 360s framework/languages/node/e2e/SpotService/run_e2e.sh SM-B2`
  - PASS: `logs/20260630-070751-3054866` (remote `play-b` `entry-created`, `entry-joined`, actor ping evidence 확인)
