...
[문서 목록](../../../../node/README.ko.md) | [handler-interfaces](handler-interfaces.ko.md)

[Runtime Lifecycle](../../../../node/internals/runtime-lifecycle.ko.md) | [Backend 의존 정책](../../../../node/internals/backend-dependency-policy.ko.md)

> 이 문서는 공통 framework 계약을 NestJS와 TypeScript 표면으로 구체화한다. 호스트
> 표면은 NestJS의 `DynamicModule`과 provider lifecycle hook, 언어 표면은
> TypeScript의 `Promise`, `interface`와 decorator를 사용한다. 구현과 contract test는
> 이 문서의 public contract를 따라야 한다.
...
노출할 뿐이다. Node.js host 모델은 NestJS module, provider와 lifecycle hook을
사용한다.
...
2. **backend 스왑 지점** — framework의 유일한 backend 의존은 §5의 backend port
   interface다. Node adapter는 이 port를 `@zlink-systems/zlink`에 연결하며, 다른
   framework 영역은 binding package를 직접 import하지 않는다.
...
`forRootFactory`는 설정값이 `ConfigService` 같은 다른 provider에서 와야 할 때
사용하며 NestJS의 표준 async module 패턴을 따른다.
...
> [runtime lifecycle](../../../../node/internals/runtime-lifecycle.ko.md)이 설명한다. registry의
...
| provider token | 주입 타입 | 등록 조건 | 미등록 시 |
|------|------|------|------|
...
[runtime lifecycle](../../../../node/internals/runtime-lifecycle.ko.md)에서 설명한다.
...
- TypeScript interface는 `I` prefix 없이 `ZLink` prefix를 유지한다.
- 비동기 완료는 `Promise<void>` 또는 `Promise<T>`로 표현한다.
- 호출자가 중단할 수 있는 request 대기, 연결, 위치 조회 같은 장기 작업만
  `signal?: AbortSignal`을 사용한다. 일반 handler와 lifecycle callback에는 signal을
  추가하지 않는다.
...
- `.NET` / C++ 의 coroutine 계열 이름에 붙는 `Async` suffix 는 Node public API 로
  ...
  [framework 공통 정책](../../async-execution-policy.ko.md)을 따르고,
...
> 노출하지 않는다([backend-dependency-policy](../../../../node/internals/backend-dependency-policy.ko.md)).
...
부트스트랩은 `ZLinkModule.forRoot/forRootFactory`, provider와 lifecycle wiring을 제공한다.
backend adapter는 §5의 포트를 `@zlink-systems/zlink`에 연결하는 유일한 backend 의존
지점이다. application handler, decorator, context와 options는
[handler interface 계약](handler-interfaces.ko.md)을 따른다.

channel messaging, Spot, actor, stream, registry와 monitoring의 세부 계약은 각각의
언어별 spec에서 고정한다. [회귀 테스트 기준](../../../../node/internals/regression-test-matrix.ko.md)은
이 public surface와 공통 스펙의 관찰 가능한 결과를 검증해야 한다.
...
[문서 목록](../../../../node/README.ko.md) | [Runtime Lifecycle](../../../../node/internals/runtime-lifecycle.ko.md) | [Backend Policy](../../../../node/internals/backend-dependency-policy.ko.md)
