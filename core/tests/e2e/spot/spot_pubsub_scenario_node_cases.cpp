...
void test_spot_node_direct_local_and_child_interop ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_set_subscription (sub_node, "mix:direct"));
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_set_subscription (sub_node, "mix:visibility"));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (publish_text (
      &zlink_publish, pub_node, "mix:direct", "direct-msg", 0));
    TEST_ASSERT_TRUE (wait_for_node_message (
      sub_node, "mix:direct", "direct-msg", 10, 3000));

    TEST_ASSERT_SUCCESS_ERRNO (publish_text (
      &zlink_publish, pub_node, "mix:visibility", "before-unsub", 0));
    TEST_ASSERT_TRUE (wait_for_node_message (
      sub_node, "mix:visibility", "before-unsub", 12, 3000));

    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_unset_subscription (sub_node, "mix:visibility"));
    TEST_ASSERT_SUCCESS_ERRNO (publish_text (
      &zlink_publish, pub_node, "mix:visibility", "after-unsub", 0));
    TEST_ASSERT_FALSE (wait_for_node_message (
      sub_node, "mix:visibility", "after-unsub", 11, 200));

    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_set_subscription (sub_node, "mix:visibility"));
    TEST_ASSERT_SUCCESS_ERRNO (publish_text (
      &zlink_publish, pub_node, "mix:visibility", "after-resub", 0));
    TEST_ASSERT_TRUE (wait_for_node_message (
      sub_node, "mix:visibility", "after-resub", 11, 3000));

    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_unset_subscription (sub_node, "mix:direct"));
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_unset_subscription (sub_node, "mix:visibility"));
...
void test_spot_node_direct_remote_peer_mesh ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_set_subscription (node_b, "mesh:direct"));
    msleep (100);
    ...
    for (int i = 0; i < 15 && !got_direct; ++i) {
        ...
        got_direct =
          wait_for_node_message (node_b, "mesh:direct", "from-node", 9, 200);
    }
...
void test_spot_node_direct_sub_option_inheritance_and_handler_conflict ()
{
    ...
    void *node = zlink_spot_node_new (ctx, "spot-node-options");
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_subscribe_handler (node, &ignore_spot_handler, NULL));
    TEST_ASSERT_EQUAL_INT (
      -1, zlink_subscribe_handler (node, &ignore_spot_handler, NULL));
...
void test_spot_node_direct_first_publish_race ()
{
    ...
    TEST_ASSERT_TRUE (
      wait_for_node_message (sub_node, "race:topic", "a", 1, 3000)
      || wait_for_node_message (sub_node, "race:topic", "b", 1, 3000));
