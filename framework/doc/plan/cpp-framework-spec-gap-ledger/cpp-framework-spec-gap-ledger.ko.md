...
| `ctest --test-dir framework/languages/cpp/build --output-on-failure --timeout 180 -j2` | 50/55 passed, exit 8 | non-sample gate와 TicTacToe는 통과했다. 병렬 sample 실행에서 DeliveryDispatch/SupportChat은 FetchContent/ExternalProject shared build race로 실패했고 Bingo/GameQuest/ShoppingMall은 process failure가 확인됐다. |
...
- 판정 근거: request/channel call surface에서 unmanaged `std::thread(...).detach()`를 제거하고 runtime offload executor rejection을 typed request failure로 처리했다. owner epoch와 retry admission 뒤 shutdown을 다시 검사해 late success를 `runtime_shutdown`으로 종결하는 회귀를 추가했다. HWM은 exact-boundary pause/resume와 mailbox batch 제한, STREAM reply token은 transport failure 뒤 one-shot 소비를 focused test에서 확인했고, 2026-08-03 canonical focused CTest 12/12도 통과했다. 반면 `offload_executor_t::drain()`은 queue와 active work가 비워질 때까지 자체 deadline 없이 기다리고(`framework/languages/cpp/framework/src/runtime/dispatch/offload_executor.cpp:111-139`), `app_t::run_shared_shutdown()`도 deadline 결과를 만든 뒤 active run의 `teardown_complete`를 기다리는 경로가 있다(`framework/languages/cpp/framework/src/runtime/host/app.cpp:3577-3587`). 별도 hang 재현은 하지 않았지만 exact host shutdown의 bounded `stopped`/`force_stopped` 계약은 이 코드 경로와 process evidence로 아직 닫히지 않았다. 이전 `build-v11-tests` full CTest 49/49는 historical snapshot이며, 현재 sample process를 포함한 canonical full CTest는 50/55로 종료했다. relocation backlog, caller cancellation과 role evidence도 common process matrix 전체에서 확인하지 않았다. narrow green test를 full semantic compliance로 판정하지 않는다.
...
- 확인한 동작과 기대 동작: target contract test는 public header와 E2E wiring text를 filesystem scan하고 current CTest에서 PASS했다. `test_cpp_framework_install_consumer`와 `test_cpp_http_client`도 PASS했고, 2026-08-03 focused CTest 12/12와 package verifier가 통과했다. 2026-08-02 full CTest 49/49 5회는 historical snapshot이며, 현재 canonical full CTest는 sample process 5개 failure를 포함해 50/55다. common 완료는 built declaration, actual runtime, package consumer와 process E2E evidence까지 요구하며, aggregate process gate와 exact contract는 아직 남아 있다.
...
- C++ Framework production/runtime: canonical build와 focused CTest 12/12는 통과했다. full CTest `-j2`는 50/55이며, sample process 3건은 serial에서도 실패했다. 이전 failure와 현재 sample 결과는 progress log에 분리해 기록했다.
- Regression stability: worker/admission 100회와 2026-08-03 focused CTest 12/12가 통과했다. 2026-08-02 full CTest 49/49 `-j2` 5회는 historical snapshot이다. DeliveryDispatch와 SupportChat의 병렬 실패는 FetchContent/ExternalProject shared build race로 분리했으며, Bingo/GameQuest/ShoppingMall process failure는 남아 있다.
...
| S0 full CTest와 C++ common E2E aggregate | canonical full CTest 50/55 / bounded aggregate 미완료 | focused CTest 12/12와 package gate는 통과했지만 sample process 3건과 common 14-config process aggregate는 별도 blocker다. |
...
| C++ runtime test | focused CTest 12/12와 worker/admission 100회가 통과했다. 2026-08-02 full CTest 49/49 5회는 historical snapshot이다. | current sample smoke와 common E2E가 runtime gate를 대신하지 않도록 별도 evidence를 유지한다. |
...
- 확인한 실제 동작과 기대 동작: CTest sample smoke target은 6개 shell script를 등록한다. 2026-08-03 canonical full CTest는 50/55였고, 실패 sample 5개를 serial로 재실행하면 3/5가 통과한다. PowerShell aggregate는 2개만 실행한다. static 4/4 결과는 role server evidence, callback count, terminal reason, owner/generation, cleanup과 6개 aggregate를 직접 판정하지 않는다. 기대 동작은 sample별 process 결과와 aggregate completeness를 실제 실행으로 확인하는 것이다.
