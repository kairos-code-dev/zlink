...
parsed_url_t
parse_base_url (const std::string &url)
{
  std::string scheme;
  std::string rest;
...
std::string
make_target (const parsed_url_t &url, const std::string &path)
{
...
http::verb
to_beast_method (http_method_t method)
{
  switch (method) {
...
raw_http_response_t
to_raw_response (const http::response<http::string_body> &response)
{
  raw_http_response_t raw;
  raw.status = static_cast<int> (response.result_int ());
  raw.body = response.body ();
  for (const auto &field : response) {
    raw.headers.emplace (std::string (field.name_string ()),
