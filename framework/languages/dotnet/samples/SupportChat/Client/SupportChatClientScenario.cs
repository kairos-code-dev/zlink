...
internal sealed class SupportChatClientScenario
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var available = await agent.Request(new SetAgentAvailableReq(true)).Async<SetAgentAvailableRes>(cancellationToken);
        Ensure(available.IsAvailable);

        await customer.Connect.Async(cancellationToken);
        var customerAuth = await customer.Request(new AuthenticateReq("customer-1")).Async<AuthenticateRes>(cancellationToken);
        ...
        var opened = await customer.Request(new OpenConversationReq("checkout payment failed")).Async<OpenConversationRes>(cancellationToken);
        ...
        var typing = await agent.Request(new SetTypingReq(opened.ConversationId, true)).Async<SetTypingRes>(cancellationToken);
        ...
        var sentByAgent = await agent.Request(new SendChatMessageReq(opened.ConversationId, "I can help with a new payment link.")).Async<SendChatMessageRes>(cancellationToken);
        ...
        var reconnected = await reconnectingCustomer.Request(new AuthenticateReq("customer-1")).Async<AuthenticateRes>(cancellationToken);
        ...
        var sentByCustomer = await reconnectingCustomer.Request(new SendChatMessageReq(opened.ConversationId, "I cannot complete payment.")).Async<SendChatMessageRes>(cancellationToken);
        ...
        var followUpByAgent = await agent.Request(new SendChatMessageReq(opened.ConversationId, "Please try this replacement link.")).Async<SendChatMessageRes>(cancellationToken);
        ...
        var closed = await reconnectingCustomer.Request(new CloseConversationReq(opened.ConversationId, "resolved")).Async<CloseConversationRes>(cancellationToken);
        ...
        await ExpectFailureAsync(
            reconnectingCustomer.Request(new SendChatMessageReq(opened.ConversationId, "after close")).Async<SendChatMessageRes>(cancellationToken).AsTask(),
            "Closed conversation must reject follow-up messages.");

        await waitingCustomer.Connect.Async(cancellationToken);
        var waitingCustomerAuth = await waitingCustomer.Request(new AuthenticateReq("customer-2")).Async<AuthenticateRes>(cancellationToken);
        Ensure(waitingCustomerAuth.ActorId == "customer-2");
        var noAgentConversation = await waitingCustomer.Request(new OpenConversationReq("agent unavailable")).Async<OpenConversationRes>(cancellationToken);
        ...
        var closingAgentAuth = await closingAgent.Request(new AuthenticateReq("agent-2")).Async<AuthenticateRes>(cancellationToken);
        Ensure(closingAgentAuth.ActorId == "agent-2");
        var secondAgentAvailable = await closingAgent.Request(new SetAgentAvailableReq(true)).Async<SetAgentAvailableRes>(cancellationToken);
        Ensure(secondAgentAvailable.IsAvailable);

        await closingCustomer.Connect.Async(cancellationToken);
        var closingCustomerAuth = await closingCustomer.Request(new AuthenticateReq("customer-3")).Async<AuthenticateRes>(cancellationToken);
        ...
        var explicitOpened = await closingCustomer.Request(new OpenConversationReq("explicit close")).Async<OpenConversationRes>(cancellationToken);
        ...
        var explicitClosed = await closingCustomer.Request(new CloseConversationReq(explicitOpened.ConversationId, "done")).Async<CloseConversationRes>(cancellationToken);
        ...
        await ExpectFailureAsync(
            closingCustomer.Request(new CloseConversationReq(explicitOpened.ConversationId, "again")).Async<CloseConversationRes>(cancellationToken).AsTask(),
