...
export async function runRmB2(options: ClientOptions): Promise<void> {
  ...
  try {
    ...
    const repliesBefore: ProfileReply[] = [];
    for (let i = 0; i < 40; i += 1) {
      repliesBefore.push(await postJson<ProfileReply>(providerA.httpUrl, '/profile/request', { value: `${markerBefore}-${i}` }));
    }
    ...
    const repliesAfter: ProfileReply[] = [];
    for (let i = 0; i < 20; i += 1) {
      repliesAfter.push(await postJson<ProfileReply>(providerA.httpUrl, '/profile/request', { value: `${markerAfter}-${i}` }));
    }
