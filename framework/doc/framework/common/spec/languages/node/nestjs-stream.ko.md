...
[Node.js 묶음](../../../../node/README.ko.md) | [인터페이스](handler-interfaces.ko.md) | [channel](nestjs-channel-messaging.ko.md) | [SPOT](nestjs-spot.ko.md)
...
> 정식 stream 계약은 raw session과 `writePacket`을 application public surface로
> 제공하지 않는다. session 진입점은 header와 payload를 받는 dispatch callback
> 하나이며 raw write는 `write(payload, flags)` 하나다. 따라서 이 스펙은 packet
> session 단일 축으로 고정하고,
...
회귀 테스트 매핑은
[regression-test-matrix.ko.md](../../../../node/internals/regression-test-matrix.ko.md) 와
