  ...
  bool has_route_id_)
{
    ...
    const bool parsed =
      has_route_id_
        ? spot_data_plane_router_channel_control::parse_peer_rid_arg (
            arg_, &channel_name, &route_id, &endpoint)
        : spot_data_plane_router_channel_control::parse_peer_arg (
            arg_, &channel_name, &endpoint);
    if (!parsed)
        return send_ctrl_errno_reply (ctx_, EINVAL);

    if (spot_data_plane_router_channel_control::apply_client_tls (
          ctx_.node, ctx_.runtime)
        != 0)
        return send_ctrl_errno_reply (ctx_, errno);
  ...
  const ctrl_command_context_t &ctx_, const std::string &arg_)
{
    std::string channel_name;
    std::string endpoint;
    if (!spot_data_plane_router_channel_control::parse_peer_arg (
          arg_, &channel_name, &endpoint))
        return send_ctrl_errno_reply (ctx_, EINVAL);
    if (ctx_.runtime->external_router
