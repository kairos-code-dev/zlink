...
namespace Systems.Zlink.Stream.Connector.Headers;
...
public sealed class ZlinkStreamHeaderCodec : IZlinkStreamHeaderCodec
{
    ...
    public ReadOnlyMemory<byte> Encode(ZlinkStreamHeader header)
    {
        ...
        var hasRid = header.RequestId is not null;
        var hasMetadata = header.Metadata.Count > 0;
        var flags = header.Flags;
        ValidateHeaderSemantics(header.Kind, header.Codec, hasRid);

        flags = hasRid ? flags | ZlinkStreamHeaderFlags.HasRid : flags & ~ZlinkStreamHeaderFlags.HasRid;
        ...
        var size = 3 + (hasRid ? 8 : 0) + 1 + nameBytes.Length + (hasMetadata ? 2 + metadataBytes.Length : 0);
        ...
        if (hasRid)
        {
            if (header.RequestId!.Value.Value == 0)
            {
                throw ZlinkStreamConnector.Error(ZlinkStreamErrorCode.ValidationFailed, "Request id must not be zero.");
            }

            BinaryPrimitives.WriteUInt64BigEndian(buffer.AsSpan(offset, 8), header.RequestId.Value.Value);
            offset += 8;
        }
    ...
    public ZlinkStreamHeader Decode(ReadOnlyMemory<byte> header)
    {
        ...
        var offset = 3;
        ZlinkStreamRequestId? rid = null;
        if (flags.HasFlag(ZlinkStreamHeaderFlags.HasRid))
        {
            if (span.Length - offset < 8)
            {
                throw ZlinkStreamConnector.Error(ZlinkStreamErrorCode.FrameDecodeFailed, "Helper header request id is incomplete.");
            }

            var ridValue = BinaryPrimitives.ReadUInt64BigEndian(span.Slice(offset, 8));
            if (ridValue == 0)
            {
                throw ZlinkStreamConnector.Error(ZlinkStreamErrorCode.FrameDecodeFailed, "Request id must not be zero.");
            }

            rid = new ZlinkStreamRequestId(ridValue);
            offset += 8;
        }
        ...
        ValidateHeaderSemantics(kind, codec, rid is not null);
        return new ZlinkStreamHeader(kind, codec, flags, rid, name, metadata);
    }
    ...
    private static void ValidateHeaderSemantics(
        ...
        bool hasRid)
    {
        if (kind == ZlinkStreamMessageKind.Send && hasRid)
        {
            throw ZlinkStreamConnector.Error(ZlinkStreamErrorCode.FrameDecodeFailed, "Send packet must not contain a request id.");
        }

        if (kind is ZlinkStreamMessageKind.Request or ZlinkStreamMessageKind.Response && !hasRid)
        {
            throw ZlinkStreamConnector.Error(ZlinkStreamErrorCode.FrameDecodeFailed, "Request and response packets must contain a request id.");
        }
