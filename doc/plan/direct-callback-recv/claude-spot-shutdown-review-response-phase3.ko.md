...
실제 원인은 더 깊은 곳에 있다. 바로 **I/O 스레드와 reaper 스레드 사이의 data race**다.
...
`spot_sub_t::set_direct_handler()`가 `socket->sub_dispatch_start()`를 호출하면:
...
**모든 명령**을 처리하고, 그 결과로 `_pipes`, `_term_acks` 등
소켓 내부 상태를 바꾼다.
...
**`stop_async_mailbox_processing()`은 I/O 스레드가 끝나기를 기다리지 않는다:**
...
**`start_reaping()`은 이전 I/O context가 끝났는지 확인하지 않는다:**
...
`_pipes`와 `_term_acks`를 두 스레드가 **동기화 없이**
동시에 건드리는 **data race**다. 그래서:

- `_pipes.size()`가 부정확해져 `register_term_acks()`에서 오류가 난다
- `pipe_terminated()`에서 이미 정리된 pipe에 접근해 UB가 된다
- `_term_acks`가 0인데 `unregister_term_ack()`을 호출해 assertion에 걸린다

### 2.5 왜 stress 테스트에서만 재현되는가

- 단일 실행: I/O 스레드가 `_async_mailbox_active = false`를 빠르게 확인하고 종료한다
- stress 반복: I/O 스레드가 pending 명령을 잔뜩 처리 중이거나
  스케줄링이 밀려 flag 확인이 늦어진다
- 그 결과 `close_socket()`으로 reaper가 시작될 때 I/O 스레드가 아직 살아 있다

---

## 3. 왜 이것이 구조적 문제인가

하나를 고치면 다른 문제가 튀어나오는 이유는 이렇다.

1. **1차 수정** (wait_drained tracking): lifecycle tracker 문제를 해결했다
   → 소켓이 더 자주 제대로 추적되고
   → reaper가 더 빠르게 소켓을 처리하기 시작했으며
   → 그만큼 I/O 스레드와의 경합 window가 더 자주 열렸다

2. **2차 시도** (term_endpoint 제거): pipe 상태 경합 경로 하나를 없앴다
   → 하지만 근본 원인인 I/O 스레드 vs reaper 동시 접근은 그대로 남았다

3. **구조적 결함**: 소켓의 내부 상태(`_pipes`, `_term_acks`, `_terminating`)는
   **단일 스레드에서만 접근한다는 암묵적 가정**에 기대고 있는데,
   async mailbox가 바로 이 가정을 깬다.
...
`close_socket()`을 호출하기 **전에** I/O 스레드의 async handler가
완전히 끝났음을 보장한다.
...
`socket_base_t::close()` 자체가, async mailbox가 활성일 때
자동으로 quiesce를 수행한다:
...
이 방향이 더 안전한 이유는 두 가지다.
- spot 코드뿐 아니라 **모든 async dispatch 사용자**가 자동으로 보호된다
- 호출자가 `wait_async_quiesced()`를 잊어도 `close()`가 알아서 처리한다

### 방향 C: start_reaping()에서 async 완료 보장 (방어적)

방향 B가 적용되지 않을 때를 위한 safety net이다:
...
I/O 스레드 quiesce와 별개로, spot node의 teardown 순서도 손봐야 한다:
...
data plane을 먼저 정지하면 이렇게 된다.
- ingress/fanout 소켓이 이미 close된 상태다
- attachment pipe의 peer가 이미 reaper에 있다
- pipe 명령이 순차적으로 처리된다

단, `stop_and_join()` 전에 "terminate" ctrl 명령을 보내는 경로를 확인해야 한다.
지금 `stop_and_join()`은 `stop.set(1)` → `stop_sockets()` → join → `close_control_sockets()`
순서인데, data plane의 poll loop가 정상적으로 빠져나오려면
ctrl에 "terminate"을 보내거나 socket stop으로 poll을 끊어줘야 한다.
...
- 모든 async dispatch 사용자에게 자동 적용된다
- 호출 순서 실수를 방어한다
- 변경이 최소 침습적이다
