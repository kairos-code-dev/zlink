...
| `tcp` | `MULTI_SPOT_SENDSEND` | `통과(55.3%)` | `통과(54.4%)` | `통과(58.3%)` | `보류(18.7%)` | `보류(18.9%)` | `보류(24.7%)` | 64/256/1024B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_064305.txt` 대비 Go `perf_go_multi_linux_20260525_064343.txt` 기준이다. 131072/262144B는 C `perf_c_multi_linux_20260525_021556.txt` 대비 Go `perf_go_multi_linux_20260525_021722.txt` 기준이고, 65536B는 fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go `perf_go_multi_linux_20260524_220932.txt` median 기준이다. C++처럼 client active loop에서 waiting reply를 먼저 `RecvRouted(...DONTWAIT)`로 drain한 뒤 같은 slot에서 즉시 다음 send를 시도하도록 맞췄다. 1024B 이하 server idle 대기는 고정 `sleep(1ms)` 대신 public `Poller` `POLLIN` wake를 사용한다. client send는 `ws`가 아니고 65536B 이하일 때만 public `MoveMessage(...)`를 사용해 submit 전 복사를 줄인다. `ws`와 131072B 이상은 후보 측정에서 일부 size가 낮아져 기존 `Message(...)` 경로를 유지한다. large size는 최신 재측정에서도 SPOT 최소 기준에는 아직 못 미친다. |
...
| `tls` | `MULTI_SPOT_SENDSEND` | `통과(54.2%)` | `통과(56.2%)` | `통과(62.1%)` | `보류(38.1%)` | `보류(37.3%)` | `보류(42.4%)` | 64/256/1024B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_064438.txt` 대비 Go `perf_go_multi_linux_20260525_064632.txt` 기준이다. 65536/131072/262144B는 fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go `perf_go_multi_linux_20260524_220932.txt` median 기준이다. `SubmitNotConnected`는 C처럼 fatal이 아니라 다음 poll loop에서 재시도하는 의미로 처리하고, 서버 echo는 public `Spot.ForwardRouted(...)`를 65536B 이상에 적용한다. client send는 65536B 이하에서만 public `MoveMessage(...)`를 사용한다. 1024B 이하 server idle 대기는 고정 `sleep(1ms)` 대신 public `Poller` `POLLIN` wake를 사용해 small 보류를 해소했다. 131072B까지 넓힌 후보는 일부 transport를 낮춰 반영하지 않았다. large size는 아직 Go SPOT 최소 기준에는 못 미친다. active slot 16 후보 `perf_go_multi_linux_20260524_205629.txt`는 131072B를 낮춰 반영하지 않는다. |
...
- 2026-05-25 추가 수정: `MULTI_SPOT_SENDSEND` server active loop가 drain 뒤 항상
  ...
  large size는 이번 라운드에서 다시 재지 않았으므로 기존 sleep fallback을 유지한다.
...
- **2026-05-23 근본 원인 규명 + 수정 (strace 프로파일링)**: 위 large many-client 병목의 진짜 원인을 strace로 특정했다.
  ...
  - **2026-05-25 `wss/tls MULTI_DEALER_DEALER 64B` `Bytes(...)` 후보 기각**: small 64B도 public `Bytes(...)` client send로 바꿔 시험했지만, C `perf_c_multi_linux_20260525_071841.txt` 대비 Go `perf_go_multi_linux_20260525_071913.txt`가 tls 34.2%, wss 37.0%에 머물렀다. tls는 기존 대표값보다 낮고 wss도 보류를 해소하지 못하므로 small 64B는 기존 `MoveMessage(...)` 경로를 유지한다.
  ...
  - **남은 보류(수정 후에도 목표 미달, 후속 개선 대상)**: `MULTI_SPOT_SENDSEND` large(tcp/ws 일부와 tls 65536B 이상, routed-echo FFI 및 large payload 비용).
    `MULTI_DEALER_DEALER`의 tcp 262144B, wss 64B, tls 64B copy/FFI 비용.
...
| `ws` | `MULTI_SPOT` | `보류(64.7%)` | `보류(65.9%)` | `통과(82.5%)` | `통과(115.5%)` | `통과(84.2%)` | `보류(69.8%)` | 64/256B는 C `perf_c_multi_linux_20260525_074050.txt` 대비 Rust `perf_rust_multi_linux_20260525_074242.txt` 기준이다. 1024B는 기존 파일 기준이다. 65536B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_022730.txt` 대비 Rust `perf_rust_multi_linux_20260525_025506.txt` 기준이다. 131072B는 C `perf_c_multi_linux_20260525_072541.txt` 대비 Rust `perf_rust_multi_linux_20260525_072933.txt` 기준이다. 262144B는 C `perf_c_multi_linux_20260525_072826.txt` 대비 Rust `perf_rust_multi_linux_20260525_073248.txt` 기준이다. SPOT server active publish를 `Message::with_size()` + `data_mut()` 직접 stamp로 바꾼 뒤 65536B가 통과권에 올랐다. 64/256B는 기본 recv worker를 2개로 좁혀 기존 50%대에서 64.7/65.9%까지 올렸지만 아직 SPOT 기준 아래다. 131072B는 public `Poller` `POLLIN` wait 위에서 기본 recv worker를 8개로 좁혀 729.462Kmsg/s, C 대비 84.2%로 통과했다. 262144B는 7-worker로 좁혀 362.457Kmsg/s, C 대비 69.8%까지 올렸지만 아직 SPOT 기준 아래다. 8-worker 후보는 server ready 안정성을 깨서 반영하지 않는다. SPOT client 복사 제거 + worker drain 위에, 131072B/262144B에서만 public `Poller` `POLLIN` wait를 사용한다. |
...
| `wss` | `MULTI_PUBSUB` | `통과(87.5%)` | `보류(7.4%)` | `보류(6.1%)` | `통과(87.9%)` | `통과(115.5%)` | `통과(111.4%)` | 64/256/1024/65536B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_073540.txt` 대비 Rust `perf_rust_multi_linux_20260525_073701.txt` median 기준이다. 131072B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_052938.txt` 대비 Rust `perf_rust_multi_linux_20260525_052955.txt` 기준이고, 262144B는 C `perf_c_multi_linux_20260522_152936_codex_c_multi_wss_no_stream_for_rust_20260522.txt` 대비 Rust `perf_rust_multi_linux_20260522_153511_codex_rust_multi_wss_pubsub_ready_fix_20260522.txt` 기준이다. client drain의 `TopicMessage` placeholder 재사용 뒤 65536B는 단순 one-way 기준을 넘었다. 256B/1024B는 여전히 낮다. |
...
| `tls` | `MULTI_PUBSUB` | `통과(83.3%)` | `통과(74.3%)` | `보류(5.3%)` | `통과(86.4%)` | `통과(101.8%)` | `통과(91.3%)` | 64/256/1024/65536B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_073540.txt` 대비 Rust `perf_rust_multi_linux_20260525_073701.txt` median 기준이다. 131072/262144B는 C `perf_c_multi_linux_20260522_154822_codex_c_multi_tls_no_stream_for_rust_20260522.txt` 대비 Rust `perf_rust_multi_linux_20260522_155342_codex_rust_multi_tls_pubsub_20260522.txt` 기준이다. client drain의 `TopicMessage` placeholder 재사용 뒤 1024B만 보류로 남고, 나머지 size는 기준선을 넘었다. |
...
- **MULTI_SPOT ws 262144B recv worker 7개 선택 적용**: ws 262144B도 worker 6 기본값이 보류권이라 7/8-worker를 분리 확인했다. 8-worker 후보 `perf_rust_multi_linux_20260525_072826.txt`는 `server_ready_timeout` partial로 불안정했다. 7-worker 후보 `perf_rust_multi_linux_20260525_073213.txt`는 complete였고 C `perf_c_multi_linux_20260525_072826.txt` 대비 71.8%였다. 기본값을 ws 262144B에만 7-worker로 좁혀 반영한 공식 runner `perf_rust_multi_linux_20260525_073248.txt`는 362.457Kmsg/s, 69.8%로 기존 57.2%보다 올랐지만 아직 SPOT 기준 아래라 보류를 유지한다.
...
- **MULTI_PUBSUB client `TopicMessage` placeholder 재사용 적용**: PUBSUB client가 수신 메시지마다 `TopicMessage::empty()`를 새로 만들던 것을 drain 호출 안의 caller-provided placeholder 재사용으로 바꿨다. public `SubSocket::subscribe(&mut TopicMessage, ...)` 의미는 그대로이며, 새 API를 추가하지 않는다. 공식 제한 재측정은 complete였고 C `perf_c_multi_linux_20260525_073540.txt` 대비 Rust `perf_rust_multi_linux_20260525_073701.txt`에서 `wss 65536B`가 61.033Kmsg/s, 87.9%로 올라 통과권에 들어왔다. tcp 64/256/1024/65536B는 26.4/15.0/13.4/48.4%, wss 256/1024B는 7.4/6.1%, tls 1024B는 5.3%라 PUBSUB small 보류는 계속 유지한다.
...
- **남은 보류(Rust)**: SPOT tcp 64B 및 ws 64/256/262144B, PUBSUB small과 tcp 65536B(단일 thread drain).
