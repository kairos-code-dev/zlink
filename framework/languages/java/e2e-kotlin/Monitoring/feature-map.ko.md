...
- `MON-B1`: service host의 socket source를 `CONNECTION_READY` kind로 필터링하고, evidence에 필터에 포함한 kind만 기록되는지 확인한다.
- `MON-B2`: monitoring 등록 검증에서 비양수 polling interval은 구성 시점에 실패하고, 미존재 socket/spot source는 host 시작 시점에 명확한 오류로 실패하는지 확인한다.
- `MON-C1`: monitoring event handler가 예외를 던져도 dispatcher가 예외를 격리하고, 그 뒤 channel messaging이 계속 성공하는지 확인한다.
