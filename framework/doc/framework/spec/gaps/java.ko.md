...
- [ ] **E2E-JV-03** (결함) — RuntimeMonitoring·ATD·OBS·StoreFailure·SpotActorTransfer 해결. HTTP 호출 경로 wrapper 7 : raw 4
...
| **E2E-JV-03** (결함) | [E2E README:43-44](../../common/e2e/README.ko.md): "client는 **언어별 HTTP client wrapper**를 사용한다 … raw `HttpClient`로 e2e app endpoint를 직접 호출하지 않는다" | **부분 해결:** `RuntimeMonitoring`, `AutomaticTurnDispatch`, `ObservabilityOps`, `StoreFailure`, `SpotActorTransfer`의 HTTP 호출 코드에 raw client가 남으면 실행 전에 실패하는 gate를 추가해 각각 기존 `TriggerScenarioClient`, evidence GET helper, drain 요청, Shared의 혼합 `HttpSupport`, Client `Program`에서 red를 확인했다. RuntimeMonitoring은 역할 endpoint별 `ZLinkHttpClient` 하나를 소유하고 종료 시 닫으며, 기존 5분 request timeout과 raw status/body 단언을 유지한다. MON-A1 대표 실행과 MON-A1~D1 전체 실행이 통과했다. ATD는 evidence URL이 여러 역할을 가리키므로 기존 GET helper 안에서 URL을 wrapper의 base URL과 path로 나누고 one-shot wrapper가 요청 뒤 자동으로 자원을 정리하게 했다. ATD-A1 대표 실행과 ATD-A1~E5 전체 실행이 통과했다. ObservabilityOps Trigger의 유일한 app HTTP 호출인 drain 요청도 같은 public wrapper를 사용하고 2xx를 단언하며 OBS-C4가 통과했다. 직전 OBS-A1~C5 전체 실행도 같은 runner에서 통과했지만 독립 Client 부재는 E2E-JV-01에 남는다. StoreFailure는 Shared `HttpSupport`에서 client GET/POST를 제거하고 Client 전용 `StoreFailureHttpClient`가 wrapper의 수명·5초 GET·10초 POST·2xx 검증을 소유한다. SF-A1 대표 실행이 통과했고 전체 실행은 SF-A1~D1까지 통과한 뒤 기존 SF-D2 traffic stall 경계가 6.068초로 한 번 실패했다. SF-D2 단독과 뒤의 SF-D3·E1은 통과했으며 sleep이나 retry를 추가하지 않았다. SpotActorTransfer는 node A·B·C별 wrapper를 Client가 소유하고 URI의 authority에 맞는 client와 path를 내부에서 선택한다. 기존 14초 비동기 POST와 5초 GET 상한을 보존했으며 `:Client:compileJava`와 ST-A1~F6 20개 selector가 통과했다. 실행 중 두 번의 node readiness 실패는 로그에서 `ZlinkBindException`을 확인한 뒤 같은 selector만 재실행했고, 기능 재시도나 대기 확대는 추가하지 않았다. POSD/DDD 재리뷰에서는 JDK transport 타입 누출과 StoreFailure의 server/client 책임 혼합을 위험 신호로 분류했다. StoreFailure Shared에 wrapper 의존성을 더하는 안 대신 Client 역할 client를 분리해 server 보조 코드는 Shared에 유지하고 client transport 정책은 Client에 가뒀다. RuntimeMonitoring과 SpotActorTransfer는 기존 역할·node client가 wrapper 수명과 endpoint 선택을 함께 소유하는 안을, ATD와 OBS는 이미 한 곳뿐인 호출 경계에서 one-shot wrapper를 사용하는 안을 선택해 변경 지점을 늘리지 않았다. SpotActorTransfer에서 요청마다 wrapper를 만드는 대안은 연결 수명과 endpoint 선택 정책이 반복되므로 채택하지 않았다. 다섯 config 모두 domain 코드는 없어 DDD 경계 변화는 없다. 현재 HTTP 호출 경로 실측은 wrapper 7(`RuntimeMonitoring`·`AutomaticTurnDispatch`·`ObservabilityOps`·`StoreFailure`·`SpotActorTransfer`·`RegistryMessaging`·`RegistrationCodec`) 대 raw 4다. 남은 raw 경로는 `PubSub/Client/Support/{ScenarioContext,PublisherClient,Evidence}.java`, `SpotService/Client/Program.java`, `ResilienceLifecycle/Client/Support/ConsumerScenarioClient.java`, `ToActorMessaging/Shared/JsonHttp.java`다. `RegistrationCodec/Client/Support/Evidence.java`와 `RegistryMessaging/Client/Support/DynamicClusterLauncher.java`에도 wrapper 밖 raw 보조 경로가 남으므로 이 gap은 open이다. |
