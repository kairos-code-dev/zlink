...
- [ ] **E2E-JV-03** (결함) — `RuntimeMonitoring` 해결. config 주 경로 실측 wrapper 3 : raw 8
...
| **E2E-JV-03** (결함) | [E2E README:43-44](../../common/e2e/README.ko.md): "client는 **언어별 HTTP client wrapper**를 사용한다 … raw `HttpClient`로 e2e app endpoint를 직접 호출하지 않는다" | **부분 해결:** `RuntimeMonitoring` client에 raw HTTP가 남으면 실행 전에 실패하는 gate를 추가해 기존 `TriggerScenarioClient`에서 red를 확인했다. 이 client는 역할 endpoint별 `ZLinkHttpClient` 하나를 소유하고 종료 시 닫으며, 기존 5분 request timeout과 raw status/body 단언을 유지한다. MON-A1 대표 실행과 MON-A1~D1 전체 실행이 통과했다. POSD/DDD 재리뷰에서는 JDK transport 타입이 client support의 정책과 예외 처리를 노출한 것을 정보 누출로 분류했다. 호출마다 one-shot wrapper를 만드는 안보다 기존 역할 client가 wrapper 수명과 endpoint를 함께 소유하는 안이 연결 정책과 정리 책임을 한 곳에 두므로 이를 선택했으며 domain 코드는 없어 DDD 경계 변화는 없다. 현재 config 주 경로 실측은 wrapper 3(`RuntimeMonitoring`·`RegistryMessaging`·`RegistrationCodec`) 대 raw 8이다. 남은 raw 경로는 `PubSub/Client/Support/{ScenarioContext,PublisherClient,Evidence}.java`, `SpotService/Client/Program.java`, `ResilienceLifecycle/Client/Support/ConsumerScenarioClient.java`, `AutomaticTurnDispatch/Client/Support/AutomaticTurnDispatchScenarioSupport.java`, `SpotActorTransfer/Client/Program.java`, `ToActorMessaging/Shared/JsonHttp.java`, `StoreFailure/Shared/HttpSupport.java`, `ObservabilityOps/Trigger/Program.java`다. `RegistrationCodec/Client/Support/Evidence.java`와 `RegistryMessaging/Client/Support/DynamicClusterLauncher.java`에도 wrapper 밖 raw 보조 경로가 남으므로 이 gap은 open이다. |
