...
void spot_data_plane_poller_interest_t::refresh_fixed (spot_data_plane_runtime_state_t *state_)
{
    ...
    const bool pause_mesh =
      state_->pending.remote_mesh.pending_bytes >= state_->pending.remote_mesh.pending_pause_threshold
      || !state_->pending.staged.mesh_messages.empty () || !state_->pending.staged.ingress_messages.empty ();
    const bool resume_mesh =
      state_->pending.remote_mesh.pending_bytes <= state_->pending.remote_mesh.pending_resume_threshold;
    ...
    const bool mesh_pub_need_pollout = !state_->pending.remote_mesh.broadcast_pending_message_ids.empty ();
    if (state_->mesh_pub && mesh_pub_need_pollout != state_->pending.remote_mesh.pollout_armed) {
        (void) state_->poller->modify (state_->mesh_pub, mesh_pub_need_pollout ? ZLINK_POLLOUT : 0);
        state_->pending.remote_mesh.pollout_armed = mesh_pub_need_pollout;
    }
}
