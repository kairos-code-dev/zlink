                            ...
                            const std::string &lib_name)
{
    ...
    zmq_pollitem_t poll_r1[] = {{router1.get(), 0, ZMQ_POLLIN, 0}};
    zmq_pollitem_t poll_r2[] = {{router2.get(), 0, ZMQ_POLLIN, 0}};
    ...
    while (!connected) {
        ...
        if (rc == 4 && wait_for_input(poll_r1, 0)) {
            int len = zmq_recv(router1.get(), buf, 16, ZMQ_DONTWAIT);
            if (len > 0) {
                zmq_recv(router1.get(), buf, 16, ZMQ_DONTWAIT);
                connected = true;
            }
        }
