...
{
    private static int RunMultiGatewayServer(string transport, int size)
    {
        ...
        string endpoint = MultiEndpointFor(transport, "multi-MULTI_GATEWAY");
    ...
    private static int RunMultiGatewayClient(string transport, int size,
        string endpoint)
    {
        size = Math.Max(1, size);
        ...
        int drainMs = ResolveMultiDrainMs("MULTI_GATEWAY");
        ...
        try
        {
            ...
            PrintResult("MULTI_GATEWAY", transport, size, throughput, latencyUs);
            return 0;
        }
        finally
        {
