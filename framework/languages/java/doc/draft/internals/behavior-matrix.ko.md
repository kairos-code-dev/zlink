...
# Draft -- Java Behavior Matrix

> 이 문서는 **구현 전 초안**이다.
> Java/Kotlin 구현이 `.NET`과 같은 판정을 내려야 하는 validation/runtime matrix다.
...
| Entry Spot actor packet | actor mailbox 순서, Entry Spot 전역 queue에 묶지 않음 |
