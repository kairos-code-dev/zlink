...
| `data()`/`bytes()` | mutable·`const` overload |
...
| `parts()` | `const std::vector<message_t>&`(mutable overload도) | — |
| `is_single_part()` | `bool` | — |
...
| `request_operation_t`/`request_submit_operation_t` | `send`와 동일 + `.timeout(std::chrono::milliseconds)` | — |
