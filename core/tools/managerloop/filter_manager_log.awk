...
function is_control_line(line) {
  return line ~ /^(OpenAI Codex|--------|user|codex|exec|warning:|=== manager iteration|spec: |guide: |log: |guide status:|guide remaining_tasks:|guide completion_verified:|guide blocked_reason:|Manager workflow|Max iterations reached|codex exec failed|Manager requested user input|[0-9]{4}-[0-9]{2}-[0-9]{2}T)/
}
