...
- 다만 바인딩이 서비스 계층 컴포넌트를 구현하면 해당 서비스 계층 샘플은
  canonical 세트에 포함된다.
...
- 각 바인딩은 다음 canonical sample 세트를 기준으로 맞춘다.
  - `pair_recv_sample`
  ...
  - `monitor_recv_sample`
- service/spot 계열이 없는 바인딩은 `spot_*` 샘플을 제외할 수 있다.
- 서비스 계층 컴포넌트를 구현하는 바인딩은 다음 샘플을 추가로 포함한다.
...
- callback 완료 검증을 위한 동기화 규칙:
  - 허용 패턴: `CountDownLatch`, `CompletableFuture`, `Event`,
    `condition variable`, `channel`, `Promise`, `semaphore` 등
    언어별 일회성 signal 메커니즘
  - 금지 패턴: `sleep`, `Thread.sleep`, `time.sleep`, busy-wait loop,
  ...
  - 이 패턴이면 "callback이 정말 불렸는지"를 결정적으로 검증할 수 있다
...
- TCP bind/connect 후 메시지를 주고받기 전에 monitor API로 connection readiness를
  확인해야 한다.
- connection handshake는 monitor event 기반으로 작성한다.
- handshake 호출은 샘플 본문에서 보이게 두고, monitor polling 같은 반복 로직은
  helper로 감쌀 수 있다.
- 목표는 샘플 본문만 보고도 bind/connect 이후 왜 바로 send/publish 하지 않는지
  이해할 수 있게 만드는 것이다.
...
- 모든 바인딩의 동일 패턴 샘플은 동일한 메시지 내용을 사용한다.
...
- 각 바인딩은 canonical sample 세트를 공식 샘플 표면으로 유지한다.
- direct recv 계열:
...
- service/spot 계열이 있는 바인딩은 spot recv/callback 샘플을 canonical에 포함한다.
...
- 상위 디렉토리의 `run_samples.*` wrapper는 두지 않는다.
