...
| `.NET: Server/Play/Infrastructure/Redis/RedisBingoMatchQueue.cs` | `Server/Play/Infrastructure/Redis/redis_bingo_match_queue.hpp` | external-adapter | done | Redis waiting room record와 actor reservation을 adapter 안에 둔다. |
...
| `.NET: Server/Play/Infrastructure/ZLink/Handlers/EnsurePlayerActorHandler.cs` | `Server/Play/Infrastructure/ZLink/Spots/EntrySpot/Handlers/ensure_player_actor_handler.hpp` | spot-handler | done | Entry Spot request로 인증된 player actor ref를 만들거나 찾는다. |
...
| `common: Redis-backed match queue` | `Server/Play/Infrastructure/Redis/redis_bingo_match_queue.hpp` | external-adapter | done | Redis client dependency는 match queue adapter 안에 있다. |
