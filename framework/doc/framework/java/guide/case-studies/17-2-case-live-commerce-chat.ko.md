...
```java
@Component
public final class LiveChatSession implements ZLinkSession {
    private final ZLinkSessionContext context;
    private final ZLinkActorManager actors;
    private ZLinkSessionActor viewer;

    public LiveChatSession(ZLinkSessionContext context, ZLinkActorManager actors) {
...
public final class SendLiveMessageHandler implements ZLinkSpotActorSendHandler<StreamSpot, ViewerActor, SendLiveChat> {
    ...
    public void handle(StreamSpot stream, ViewerActor viewer, ZLinkSpotActorSendContext context, SendLiveChat req, CancellationToken cancellationToken) {
        ...
        ModerationDecision decision = moderation.check(stream.streamId(), viewer.actorId(), req.text());
        if (!decision.allowed()) { viewer.push(new ChatRejected(decision.reason())); return; }
        ChatMessage message = recent.append(stream.streamId(), viewer.actorId(), req.text());
        for (ViewerActor watching : stream.activeViewers()) { watching.push(message); }
    }
}
```
