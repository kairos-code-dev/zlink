...
async function main() {
  ...
  const transportTransitionMs = Math.max(
    0, Number(process.env.PERF_TRANSPORT_TRANSITION_MS || 3000)
  );
  ...
  for (const name of names) {
    ...
    const transports = options.transports.filter(
      (transport) => policyTransports(name).includes(transport)
    );
      ...
      if (transportUnsupported) {
        ...
        for (const size of sizes) {
          ...
          if (failedSizes.has(size) || !list || list.length === 0) {
            comboResults.set(
              comboKey(name, transport, size),
              failedRecords.get(size) || { status: 'fail' }
            );
  ...
  const expectedResultLines = Math.max(
    0,
    (requestedComboCount - unsupportedComboCount - skipComboCount) * 5
  );
  ...
  const resultFile = path.join(
    reportDir,
    `perf_node_single_${platformTag()}_${stamp}${tag}.txt`
  );
  emit('');
