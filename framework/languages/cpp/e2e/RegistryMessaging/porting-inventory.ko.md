...
| `Client/Scenarios/RmA1DiscoveryRequestScenario.cs` | `Client/Scenarios/rm_a1_discovery_request_scenario.hpp` | scenario | done | RM-A1 registry discovery request를 provider HTTP endpoint를 통해 검증한다. |
...
| `Client/Scenarios/RmC7WeightedProviderScenario.cs` | `Client/Scenarios/rm_c7_weighted_provider_scenario.hpp` | scenario | done | RM-C7 weighted provider distribution을 provider HTTP endpoint로 실행한다. |
...
| `Server/Provider/Endpoints/ProviderEndpoints.cs` | `Server/Provider/Endpoints/provider_endpoints.hpp` | endpoint | done | health/evidence와 profile request/manual/send, route request/missing HTTP endpoint를 제공한다. 이 endpoint들이 public framework client를 호출하고 C++ E2E client는 HTTP로만 운전한다. |
