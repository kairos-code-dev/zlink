...
| Transport | Pattern | 64 | 256 | 1024 | 65536 | 131072 | 262144 | 결과 파일 / 메모 |
|-----------|---------|----|-----|------|-------|--------|--------|------------------|
| `tcp` | `MULTI_DEALER_DEALER` | `보류(38.8%)` | `통과(54.7%)` | `통과(71.2%)` | `통과(81.4%)` | `보류(45.9%)` | `보류(46.4%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 64B는 Go `perf_go_multi_linux_20260524_231656.txt`, 256/1024B는 Go `perf_go_multi_linux_20260524_222134.txt`, 65536/131072/262144B는 Go `perf_go_multi_linux_20260525_001654.txt` median 기준이다. C처럼 단일 poll loop와 pending socket만 `POLLOUT`으로 두는 poll set 의미를 유지했다. server는 C의 single-part `zlink_recv_part` 의미와 맞춰 64B/65536B에서 public `RecvPart` caller-owned 수신 경로를 사용한다. client active send는 65536B에서 public `Bytes(...)` 경로를 사용해 submit 전 native `Message` 생성을 줄였고, 131072B/262144B는 후보 측정에서 불안정하거나 낮아 기존 경로를 유지한다. |
...
| `ws` | `MULTI_DEALER_DEALER` | `보류(38.4%)` | `통과(55.2%)` | `통과(67.7%)` | `통과(59.2%)` | `통과(58.7%)` | `통과(64.4%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 64B/256B/1024B는 Go `perf_go_multi_linux_20260524_222134.txt`, 65536/131072/262144B는 Go `perf_go_multi_linux_20260525_001654.txt` median 기준이다. C처럼 단일 poll loop와 pending socket만 `POLLOUT`으로 두는 poll set 의미를 유지했다. client active send는 65536B/131072B에서 public `Bytes(...)` 경로를 사용해 submit 전 native `Message` 생성을 줄였다. 262144B는 기존 경로로도 current HEAD에서 통과권이 유지된다. 64B는 아직 Go 단순 one-way 최소 기준보다 낮다. |
...
| `wss` | `MULTI_DEALER_DEALER` | `보류(37.4%)` | `통과(58.3%)` | `통과(71.2%)` | `보류(42.5%)` | `보류(45.2%)` | `보류(46.6%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 64B는 Go `perf_go_multi_linux_20260524_231656.txt`, 256/1024B는 Go `perf_go_multi_linux_20260524_222134.txt`, 65536/131072/262144B는 Go `perf_go_multi_linux_20260525_001654.txt` median 기준이다. C처럼 단일 poll loop와 pending socket `POLLOUT` 의미를 유지했다. Go send builder에 명시적 ownership 이전 단계 `MoveMessage`를 추가했고, active send는 1024B 이하와 wss 131072B/262144B, tls 131072B에서만 이 경로를 사용한다. `Bytes(...)` large 후보는 wss/tls에서 낮아져 적용하지 않는다. 암호화 large는 아직 기준선 아래다. |
...
| `tls` | `MULTI_DEALER_DEALER` | `보류(38.1%)` | `통과(57.7%)` | `통과(68.1%)` | `보류(42.9%)` | `보류(43.6%)` | `보류(43.2%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 64B는 Go `perf_go_multi_linux_20260524_231656.txt`, 256/1024B는 Go `perf_go_multi_linux_20260524_222134.txt`, 65536/131072/262144B는 Go `perf_go_multi_linux_20260525_001654.txt` median 기준이다. C처럼 단일 poll loop와 pending socket `POLLOUT` 의미를 유지했다. active send는 1024B 이하와 tls 131072B에서만 `MoveMessage(...)`를 사용한다. `Bytes(...)` large 후보와 262144B `MoveMessage(...)` 확대는 낮아져 기존 경로를 유지한다. 64B/65536B/131072B/262144B는 아직 단순 one-way 기준보다 낮다. |
