...
internal static class ObsC3SpotDrainPoliciesScenario
{
    public static async Task RunAsync(ScenarioContext context)
    {
        ...
        await context.PlayA.Post("/rooms")
            .Body(new CreateRoomReq(roomRid, "auto-close")).SubmitRawAsync();
        await context.PlayA.Post("/drain?deadlineMs=10000").SubmitRawAsync();
        var duringNatural = (await context.PlayA.Get("/evidence").SubmitAsync<EvidenceSnapshot>()).Body;
        ...
        var playMetrics = (await context.PlayA.Get("/evidence").SubmitAsync<EvidenceSnapshot>()).Body.Metrics;
        ...
        var released = (await context.WorkflowB.Get("/evidence").SubmitAsync<EvidenceSnapshot>()).Body;
        ...
        var replayed = (await context.WorkflowB.Get($"/workflows/{workflowRid}/state")
            .SubmitAsync<ReadWorkflowRes>()).Body;
        ...
        var workflowMetrics = (await context.WorkflowA.Get("/evidence")
            .SubmitAsync<EvidenceSnapshot>()).Body.Metrics;
