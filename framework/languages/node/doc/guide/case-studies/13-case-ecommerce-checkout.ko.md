...
> 사용법 정식은 [04-channel-messaging](../04-channel-messaging.ko.md)이 다룬다.
...
```ts
// order-service: 주문 생성 + outbox + 동기 결제 호출 (NestJS + @grpc/grpc-js)
@Injectable()
export class OrderGrpcService {
  constructor(
    private readonly payments: PaymentGrpcClient,
    private readonly orders: OrderRepository,
  ) {}

  async placeOrder(req: PlaceOrderRequest): Promise<PlaceOrderReply> {
    const existing = await this.orders.find(req.orderId);
    if (existing) {
  ...
  private async createOrder(req: PlaceOrderRequest): Promise<PlaceOrderReply> {
    const charge = await this.payments.charge(
      { orderId: req.orderId, accountId: req.accountId, amountMinor: req.amountMinor, idempotencyKey: req.orderId },
      { deadlineMs: 2000 },
    );
    await this.orders.withTransaction(async (tx) => {
      await this.orders.insertOrder(req.orderId, charge.receiptId, tx);
      await this.orders.insertOutbox('OrderPlaced', req.orderId, tx);
    });
    return { orderId: req.orderId, receiptId: charge.receiptId };
  }
}
```
...
```ts
await outbox.drain((row) => kafkaProducer.send('order.events', row.toMessage()));

await kafkaConsumer.consume('order.events', async (event) => {
  const reserved = await inventory.tryReserve(event.orderId, event.sku, event.qty);
  if (!reserved) {
    await kafkaProducer.send('order.events', new StockReserveFailed(event.orderId));
  }
});
```
...
ZLinkModule.forRoot(
  zlinkFramework()
    ...
    .addClientServerChannel('orders')
      .enableServer('tcp://0.0.0.0:7401')
      .addRequestHandler('PlaceOrder', PlaceOrderHandler)
    .addClientServerChannel('payments')
      .enableClient()
    .addFanoutChannel('order.events')
      .enablePublisher('tcp://0.0.0.0:7402')
    .build()
);
```

```ts
@zlinkRequestHandler('orders', 'PlaceOrder')
export class PlaceOrderHandler
  implements ZLinkRequestHandler<PlaceOrder, OrderPlaced> {
  constructor(
    @Inject(ZLINK_CHANNEL_CLIENT) private readonly services: ZLinkChannelClient,
    private readonly orders: OrderRepository,
  ) {}

  async handle(req: PlaceOrder, context: ZLinkRequestContext): Promise<OrderPlaced> {
    const existing = await this.orders.find(req.orderId);
    if (existing) {
      return existing.toReply();
    }
...
> 보장한다(remote 수신 보장 아님, [03-concepts](../03-concepts.ko.md)). 즉
