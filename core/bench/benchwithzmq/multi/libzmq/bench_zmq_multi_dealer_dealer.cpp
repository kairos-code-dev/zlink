                            ...
                            const std::string& lib_name)
{
    ...
    for (size_t i = 0; i < clients.size(); ++i) {
        clients[i] = zmq_socket(ctx.get(), ZMQ_DEALER);
        if (!clients[i]) {
    ...
    auto recv_any_client = [&](int recv_timeout_ms) -> bool {
        ...
        while (std::chrono::steady_clock::now() < deadline) {
            for (size_t i = 0; i < clients.size(); ++i) {
                if (zmq_recv(clients[i], recv_buf.data(), recv_buf.size(),
                            ZMQ_DONTWAIT)
                    >= 0) {
                    return true;
                }
    ...
    const int received = run_multi_timed_benchmark(
      ...
      settings,
      [&](size_t idx) {
          return zmq_send(clients[idx], buffer.data(), msg_size, ZMQ_DONTWAIT) >=
                 0;
      },
      [&]() {
