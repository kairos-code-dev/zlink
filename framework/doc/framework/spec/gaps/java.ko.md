...
- [ ] **E2E-JV-03** (결함) — RuntimeMonitoring·ATD·ObservabilityOps 해결. HTTP 호출 경로 wrapper 5 : raw 6
...
| **E2E-JV-03** (결함) | [E2E README:43-44](../../common/e2e/README.ko.md): "client는 **언어별 HTTP client wrapper**를 사용한다 … raw `HttpClient`로 e2e app endpoint를 직접 호출하지 않는다" | **부분 해결:** `RuntimeMonitoring`, `AutomaticTurnDispatch`, `ObservabilityOps`의 HTTP 호출 코드에 raw client가 남으면 실행 전에 실패하는 gate를 추가해 각각 기존 `TriggerScenarioClient`, evidence GET helper, drain 요청에서 red를 확인했다. RuntimeMonitoring은 역할 endpoint별 `ZLinkHttpClient` 하나를 소유하고 종료 시 닫으며, 기존 5분 request timeout과 raw status/body 단언을 유지한다. MON-A1 대표 실행과 MON-A1~D1 전체 실행이 통과했다. ATD는 evidence URL이 여러 역할을 가리키므로 기존 GET helper 안에서 URL을 wrapper의 base URL과 path로 나누고 one-shot wrapper가 요청 뒤 자동으로 자원을 정리하게 했다. ATD-A1 대표 실행과 ATD-A1~E5 전체 실행이 통과했다. ObservabilityOps Trigger의 유일한 app HTTP 호출인 drain 요청도 같은 public wrapper를 사용하고 2xx를 단언하며 OBS-C4가 통과했다. 직전 OBS-A1~C5 전체 실행도 같은 runner에서 통과했지만 독립 Client 부재는 E2E-JV-01에 남는다. POSD/DDD 재리뷰에서는 JDK transport 타입이 client support의 정책과 예외 처리를 노출한 것을 정보 누출로 분류했다. RuntimeMonitoring은 호출마다 client를 만드는 안보다 기존 역할 client가 wrapper 수명과 endpoint를 함께 소유하는 안을, ATD와 OBS는 endpoint별 client 필드를 늘리는 안보다 이미 한 곳뿐인 호출 경계에서 one-shot wrapper를 사용하는 안을 선택해 변경 지점을 늘리지 않았다. 세 config 모두 domain 코드는 없어 DDD 경계 변화는 없다. 현재 HTTP 호출 경로 실측은 wrapper 5(`RuntimeMonitoring`·`AutomaticTurnDispatch`·`ObservabilityOps`·`RegistryMessaging`·`RegistrationCodec`) 대 raw 6이다. 남은 raw 경로는 `PubSub/Client/Support/{ScenarioContext,PublisherClient,Evidence}.java`, `SpotService/Client/Program.java`, `ResilienceLifecycle/Client/Support/ConsumerScenarioClient.java`, `SpotActorTransfer/Client/Program.java`, `ToActorMessaging/Shared/JsonHttp.java`, `StoreFailure/Shared/HttpSupport.java`다. `RegistrationCodec/Client/Support/Evidence.java`와 `RegistryMessaging/Client/Support/DynamicClusterLauncher.java`에도 wrapper 밖 raw 보조 경로가 남으므로 이 gap은 open이다. |
