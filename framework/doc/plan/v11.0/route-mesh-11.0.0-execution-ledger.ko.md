...
| `V11-M6B-NODE` | Node stateful object runtime | Node lane, `P-DEEP` | `V11-R5A` | 진행 | turn·membership·session·Instance Promise 경쟁 contract 통과 | public raw binding 위에 Spot·Actor exact-generation dispatch, owner별 serial turn, membership epoch, logical multicast local fan-out·remote node당 1회 전송, STREAM binding generation·delivery와 terminal-once Promise registry를 연결했다. Instance Spot stable type·attempt reservation, command 39 exact route·source·authority fence와 registered intent cold activation generation도 연결했다. R5B review에서 확인한 무제한 request operation table은 기본 65,536개 capacity를 ID·timer 할당 전에 검사하고 stable `OperationCapacityExceededError`를 반환하도록 수정했다. M6B internal contract 10/10, M6A 7/7, M5 5/5와 Framework TypeScript compile이 통과했다. Location authority에서 durable Instance intent를 자동 등록하는 연결과 전체 runtime regression 증거 확정을 계속한다. Sample·E2E 변경·실행은 0이다. |
