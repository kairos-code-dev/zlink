...
snapshot과 이미 수락한 target을 rollback하거나 전체 publish를 다시 실행하지 않는다. Target별 send timeout
...
준비되지 않은 경우에는 `Submitted`와 unreachable detail을 반환할 수 있다. Local Spot drop은 top-level
...
status를 만들지 않는다. Pre-cancellation은 runtime admission을 시작하지 않는다. Admission·timeout·shutdown과
