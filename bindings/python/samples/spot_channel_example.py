...
import zlink
from sample_support import tcp_endpoint, wait_connected, wait_until
...
# --8<-- [start:doc]
    with zlink.create_context() as ctx, \
        ...
        with api_router.monitor_open(zlink.MonitorEventMask.CONNECTION_READY) as api_monitor, \
             room_router.monitor_open(zlink.MonitorEventMask.CONNECTION_READY) as room_monitor:
            api_router.bind(endpoint)
            room_router.connect(endpoint)
            wait_connected(api_monitor, room_monitor)
