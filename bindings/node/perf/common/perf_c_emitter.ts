...
function singleTableRowLine(size, status, record, _pattern) {
  ...
  if (status === 'success' && record) {
    const [, p95, p99] = resolveLatencyTriplet(
      record.latency, record.latency_p95, record.latency_p99
    );
  ...
  return (
    `| ${padEnd(`${size}B`, 8)} | ${padStart(tp, 16)} | ${padStart(bw, 12)} | `
...
function multiTableHeaderLine() {
  ...
  return (
    `| ${padEnd('Size', 8)} | ${padStart('Throughput', 18)} | `
...
function multiTableRowLine(pattern, size, status, record) {
  ...
  let lat99;
  if (status === 'success' && record) {
    const [mean, p95, p99] = resolveLatencyTriplet(
      record.latency, record.latency_p95, record.latency_p99
    );
...
function buildMultiOptionItems(opts) {
  ...
  return [
    ...
    ['connect_ready_timeout_ms', optionValue(opts.connectReadyTimeoutMs, 'PERF_MULTI_CONNECT_READY_TIMEOUT_MS', parseEnvInt('PERF_CONNECT_READY_TIMEOUT_MS', 1000))],
...
function createAutoHwmCollector() {
  ...
  function cellWidths(tableRows, columns) {
    return columns.map(([header, key]) => tableRows.reduce(
      (w, f) => Math.max(w, String(f[key] !== undefined ? f[key] : '?').length),
      header.length
    ));
  }
  ...
  function emitMarkdownTable(out, indent, columns, tableRows) {
    ...
    const headerCells = columns.map(
      ([header], i) => ` ${String(header).padEnd(widths[i])} `
    );
    ...
    for (const f of tableRows) {
      const cells = columns.map(
        ([, key], i) => ` ${String(f[key] !== undefined ? f[key] : '?').padEnd(widths[i])} `
      );
  ...
  function emitSpotSnapshotSocketTable(out, title, tableRows) {
    ...
    const sorted = [...tableRows].sort((a, b) => (
      parseInt10(a.msg_size || '0', 0) - parseInt10(b.msg_size || '0', 0)
  ...
  function emitSpotTables(out, tableRows) {
    const snapshotRows = tableRows.filter(
      (f) => f.source === 'spotnode_snapshot' && f.socket
    );
    ...
    const spotRows = tableRows.filter(
      (f) => f.source !== 'option_fallback'
  ...
  function emitDetailTable(out, patternName) {
    ...
    patternRows = patternRows.filter(
      (f) => f.msg_size && f.msg_size !== '0'
    );
    if (patternRows.length === 0) {
      return false;
    }
    patternRows.sort((a, b) => (
      parseInt10(a.msg_size || '', 0) - parseInt10(b.msg_size || '', 0)
