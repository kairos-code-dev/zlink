English | [한국어](mesh-node.ko.md)

[Spec Index](../../README.md) · [Core Index](../README.md) · [Service Common](README.md) · [Dispatch](dispatch.md)
...
messages, or other memberships. A change increments descriptor generation.
...
submits one routed message per remote node; each receiving node evaluates its
local subscriptions. There is no relay or replay.

With `NODROP=1`, all remote pipes and local Spot queues are reserved and
committed atomically. Any blocked target returns
`ZLINK_SUBMIT_BACKPRESSURED`/`EAGAIN` with `DONTWAIT`, or `ETIMEDOUT` after
`SNDTIMEO` for a blocking call, with delivery to no target. With `NODROP=0`,
only blocked targets are dropped.
...
correlation are not exposed as payload or application metadata. Local dispatch
reuses the retained multipart block without re-encoding the envelope.
