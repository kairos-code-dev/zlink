...
| QuestMission 역할 | 부분 구현 | player별 quest projection, dedupe, progress/completion/reward event를 기록한다. startup과 projection rebuild는 Redis stream의 progress delta와 후속 domain event를 fold한다. owner Spot은 아직 남아 있다. |
...
GameplayStateStore 기반 reset 보정, event delta fold, Redis startup replay와 실제 process 재시작 gate는 닫혔다. owner Spot, owner lease/re-home, session binding 같은 남은
