...
- 복잡한 외부 객체 생성이나 조건부 생성이 필요한 경우에만
  `add_factory<T, Dep1, Dep2>()`를 사용한다. Factory는 선언한 dependency의 typed reference만 받으며
...
MeshNode는 ROUTER listen endpoint와 0개 이상의 ChannelName membership을 가질 수 있다. 호출 또는
Node direct 전용 MeshNode는 membership 없이 시작할 수 있고, Channel handler를 제공하는 MeshNode는
...
자동 peer는 Redis descriptor로, 수동 peer는 `peer_connections()`로 구성한다.
