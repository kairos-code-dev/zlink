...
{
    private static int RunMultiPubSubServer(string transport, int size)
    {
        ...
        string endpoint = MultiEndpointFor(transport, "multi-MULTI_PUBSUB");

        using var ctx = new Context();
        ApplyMultiServerContextOptions(ctx);
        using var server = new Zlink.Socket(ctx, Zlink.SocketType.Sub);
        ApplyMultiSocketOptions(server);
        ConfigureTlsServerIfNeeded(server, transport);
        server.SetOption(SocketOptions.Subscribe, string.Empty);
    ...
    private static int RunMultiPubSubClient(string transport, int size,
        string endpoint)
    {
        size = Math.Max(1, size);
        ...
        int drainMs = ResolveMultiDrainMs("MULTI_PUBSUB");
        ...
        try
        {
            ...
            {
                var client = new Zlink.Socket(ctx, Zlink.SocketType.Pub);
            ...
            PrintResult("MULTI_PUBSUB", transport, size, throughput, latencyUs);
            return 0;
        }
        finally
        {
