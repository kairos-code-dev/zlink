...
- `gateway` family public/internal/protocol/test/perf/bindings surface는 제거됐고,
  broad search 기준 남은 `gateway` 언급은 plan/migration 문서 설명뿐이다.
...
- 삭제 이후 POSD 정리에서는 discovery lifecycle helper, `spot_peer_state_t`,
  topology/provider 경계 정리로 hidden coupling과 shallow wrapper를 줄였다.
...
- 삭제 작업은 public API, internal runtime, discovery/registry protocol,
  테스트, `core/perf`, 문서, bindings까지 함께 정리한다는 뜻이다.
...
현재 `gateway`는 사용자에게 아래 두 가지 가치를 준다.
...
- weighted routing의 본질이 `gateway` 자체가 아니라
  registry/discovery로 모은 service peer `value` / `metadata`
  distribution이라면,
...
- `gateway`의 편의성 차이는 있지만, raw `dealer/router` 조합으로 대체할 때
  사용자가 감수하는 비용은 "몇 줄 더 작성" 수준일 가능성이 크다.
- `gateway`의 더 본질적인 차이는 metadata-aware routing과 metadata distribution이다.
- 이 기능을 registry/discovery의 일반 메커니즘으로 승격할 수 있다면,
  `gateway`는 장기적으로 별도 family가 아니라 raw socket service profile로 흡수된다.

즉 후속 검토의 핵심은 "gateway를 유지할지"가 아니라
"`gateway`가 품고 있던 요구 중 무엇을 generic contract로 되살릴지"다.
...
- raw socket profile로 대체 가능한 부분과 아닌 부분을 갈라야 한다.
- metadata-aware routing의 실질 요구가 metadata distribution인지,
  `gateway` 전용 runtime contract인지 구분해야 한다.
- `gateway` 삭제 이유와 후속 대체 방향을 2-3문장 안에 설명할 수 있어야 한다.
...
"registry/discovery로 각 service peer의 `value` / `metadata`를 모으고
send path가 그 정보로 routing policy를 수행한다"라면,
`gateway`를 평가할 때 핵심은 topology가 아니라 metadata-aware routing contract다.

현재 `gateway` public API는 `peer weight` 용어를 쓰지만,
후속 공통 모델은 generic `value + metadata`를 기준으로 정렬한다.
즉 `gateway`는 현재 `weight`를 소비하는 family이고,
후속 metadata layer는 이를 더 일반적인 numeric/blob attribute 모델로 끌어올린다.

### 5.2 metadata distribution은 별도 공통 모듈로 분리할 수 있다

metadata-aware routing에 필요한 정보가
`service_type + service_name + role + endpoint`에 부가된
`value + metadata`라면,
그 distribution은 registry/discovery 공통 인프라로 승격할 수 있다.
...
이번 계획은 `gateway` 존치 근거를 더 모으지 않는다.
...
- 삭제를 먼저 해야 public/internal/protocol 전반의 hidden coupling을 드러내고
  어떤 contract가 진짜 필요한지 가를 수 있다.
- 공통 기능이 실제로 필요하면 제거 이후 더 작은 surface로 다시 도입한다.
...
- topology convenience와 metadata-aware routing 요구를 갈라서 기록
...
- protocol/store/query path의 `gateway` 특수 처리 제거
...
아래 public C API도 같이 제거하거나 대체 경로를 정리해야 한다.
...
아래 public type/constant도 함께 정리한다.
...
- routing attribute query는 member peer query로, 운영/관측은 topology/introspection query로
  분리 이전됐는지 같이 기록한다
- 제거 후 남은 `gateway` 전용 internal branch, helper, adapter가 정리됐는지 기록한다

## 11. 제거 대상 상세 목록

`gateway` 제거는 public API 삭제만 뜻하지 않는다.
...
build 산출물이나 generated artifact는 제거 작업의 판정 기준에 넣지 않는다.
...
- `gateway` 제거 이후에도 남은 helper/adapter/branch가
  deep module을 만들지 못하는 얕은 wrapper라면 함께 제거하거나 통합
...
- `gateway` 제거를 서두르면 현재 사용자-facing 편의와 관측 surface를 잃는다
- raw socket profile로 흡수하는 과정에서 오히려 사용자 개념 수가 늘어난다
- 삭제 후 실제 공백을 잘못 판단하면 불필요한 metadata layer를 다시 만들 위험이 있다
- public symbol만 제거하고 internal dead code를 남기면 구조 단순화 효과가 약해진다
