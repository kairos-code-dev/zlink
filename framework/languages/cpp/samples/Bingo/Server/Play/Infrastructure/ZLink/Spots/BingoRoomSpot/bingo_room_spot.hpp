...
class bingo_room_spot_t : public spot_t
{
                                              ...
                                              const message_t &request_message)
    {
        ...
        if (request.observe_only) {
            ...
            std::cerr << "bingo observer room: actor joined. observedRoom=" << _observed_room_id
                      << ", observer=" << actor_id << ", nodeRid=" << _context.node_rid ().value ()
                      << '\n';
        ...
        std::cerr << "bingo room: actor joined. room=" << snapshot ().room_id
                  << ", actor=" << actor_id << ", requestActor=" << request.actor_id
                  << ", count=" << snapshot ().players.size () << ", status=" << snapshot ().status
  ...
  private:
    void publish_reward (const number_drawn_notify_t &drawn)
    {
        ...
        auto publish_task = _context.publish (sample_names_t::reward_topic, reward_event).async ();
        std::cerr << "bingo reward: published. room=" << drawn.state.room_id
                  ...
                  << ", nodeRid=" << _context.node_rid ().value () << '\n';
        observe_task_completion (publish_task, [room_id = drawn.state.room_id] (
                                                 const result_t<void> &published) {
            if (!published) {
                std::cerr << "bingo reward: publish failed. room=" << room_id << ", error="
    ...
    void send_to_players (const TNotify &notify, const std::string &excluded_actor_id = {})
    {
        for (auto &[actor_id, actor] : actors) {
            ...
            observe_task_completion (send_task, [actor_id] (const result_t<void> &sent) {
                if (!sent) {
                    std::cerr << "bingo notify: send failed. actor=" << actor_id
                              << ", packet=" << TNotify::packet_name
                              << ", error=" << (sent.error () ? sent.error ()->what () : "unknown")
                              << '\n';
                }
            });
        }
    ...
    void on_reward_acquired (const bingo_reward_acquired_event_t &event)
    {
        ...
        if (!_is_observer || event.room_id != _observed_room_id) {
            std::cerr << "bingo reward: ignored. room=" << event.room_id
                      << ", actor=" << event.actor_id << ", item=" << event.item_id
                      << ", observer=" << (_is_observer ? "true" : "false")
        ...
        for (auto &[_, actor] : observers) {
            ...
            observe_task_completion (
              send_task, [room_id = event.room_id,
                          observer = actor->actor.actor_id] (const result_t<void> &sent) {
                  if (!sent) {
    ...
    void leave_finished_actors ()
    {
        ...
        std::cerr << "bingo cleanup: leaving players. room=" << snapshot ().room_id
                  << ", count=" << leaving.size () << ", nodeRid=" << _context.node_rid ().value ()
                  << '\n';
