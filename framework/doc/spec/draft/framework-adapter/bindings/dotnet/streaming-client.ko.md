...
내려 풀어 쓴 버전이다. 핵심 목표는 서버 framework의 STREAM packet[^packet] 모델과 같은
메시지를 `.NET` client에서도 똑같이 주고받을 수 있도록 하는 것이다. 서버 framework
callback이 받는 값은 `IZLinkSessionPacket`이고, connector의 typed API와 fluent API는
client 쪽에서 wire header/body를 만들어 주는 helper 계층이다.

이 client는 게임 도메인 자체를 포함하지 않는다. 사용자가 그 위에 채팅, 게임, 장비 제어,
알림 같은 자신의 protocol을 얹어 사용한다.
...
Unity 패키지는 일반 C# 패키지 위에 얇게 얹는 형태로만 둔다. 별도의 wire protocol을
만들어서는 안 된다.

`.NET` Stream Connector는 NuGet[^nuget]을 통해 별도로 배포할 수 있어야 한다. NuGet의
package id와 `.NET` namespace는 `Systems.Zlink.Stream.Connector` 계열을 사용한다. 이
패키지가 ASP.NET Core adapter, SPOT[^spot], Stage wrapper 같은 서버 framework 측 package에
의존하면 안 된다. 의존성은 transport, codec, compression처럼 connector를 실행하는 데
필요한 client-side runtime dependency로 한정한다.

서버 framework 측 package가 Stream Connector를 굳이 참조해야 하는 것도 아니다. 서버는
framework session packet 계약을 유지하고, connector package는 외부 client가 그 계약에
맞는 wire packet을 만들고 해석할 수 있도록 돕는 역할만 맡는다.

## 3. Transport

`.NET` core 패키지는 다음 네 가지 transport를 모두 지원해야 한다.
...
URI scheme만 보고도 transport를 추론할 수 있어야 한다.
...
`Transport`가 명시되어 있는데 그 값이 `Endpoint`의 scheme과 어긋난다면, configuration
error로 처리한다.
...
`SkipServerCertificateValidation`은 테스트용 자체 서명 인증서를 검증할 때만 쓰는 옵션이다.
운영 환경에서는 기본값인 `false`를 반드시 유지해야 한다.

`MaxSendFrameSize`와 `MaxSendMetadataSize`는 connector가 내보내는 packet에 대한 기본
보호 장치다. 수신 payload에 도메인별로 거는 크기 제한은 connector의 기본 계약 안에는
넣지 않는다. 수신 크기 제한이 필요한 애플리케이션이라면 handler 또는 상위 protocol에서
별도로 검사한다.

`HeartbeatTimeout`은 마지막으로 받은 heartbeat 응답을 기준으로, 이 시간이 지나도록 새
응답이 더 들어오지 않으면 해당 연결을 죽은 것으로 본다는 임계값이다. 기본값은 30초다.

`IdleTimeout`은 어느 방향으로든 트래픽이 전혀 없는 상태가 이 시간을 넘기면 connector가
연결을 닫는다는 임계값이다. 기본값은 60초다.
...
사용자 API는 raw header bytes를 직접 받지 않는다. body 객체의 CLR 타입 이름을 기본
packet 이름으로 쓰고, 호출자가 명시한 이름이 있다면 그쪽이 우선이다. 추가 metadata가
필요하면 작은 key-value 쌍을 metadata로 덧붙인다.
...
`ZlinkStreamMessage`는 어디까지나 helper 모델이다. 실제 transport framing은 항상
`ReadOnlyMemory<byte> Header`와 `ReadOnlyMemory<byte> Body`를 기준으로 이루어진다.
helper는 `Name`, `Metadata`, codec 정보, request correlation 정보를 모두 byte header로
인코딩한다. 서버 framework는 이렇게 만들어진 결과를 `IZLinkSessionPacket`으로 감싸 session
callback에 흘려보낸다.

STREAM frame의 앞쪽 `2B`는 connector helper header가 아니라 `header_size`다.
따라서 `.NET` helper가 만들어 내는 packet도 wire에서는 다음 순서를 그대로 따른다.
...
helper header는 binary header다. `kind`와 `codec`은 문자열이 아니라 1바이트 enum 값으로
인코딩한다. packet name은 `u8 name_len + UTF-8 bytes` 형식이며, 최대 길이는 255 bytes다.
...
`.NET` enum 값은 공통 helper header 값과 일치시킨다.
...
sequence다. 같은 connector instance 안에서 동시에 pending 상태에 있는 request 사이에는
connector가 만들어 내는 `request_seq` 값이 결코 중복되면 안 된다. 값 `0`은 사용하지
않는다. metadata는 `u16 meta_len + metadata bytes` 형태로 붙는다. metadata는 header 크기
제한 안에 모두 들어가야 하므로, trace id, locale, tenant id처럼 작은 값만 넣는다.
`MaxSendMetadataSize`의 기본값은 1024 bytes다. `meta_len` wire 필드 자체가 표현할 수 있는
최댓값은 65535 bytes지만, `.NET` connector는 `MaxSendMetadataSize`를 넘어가는 metadata를
보내려 하면 error로 처리해야 한다.

helper header에 들어가는 모든 multi-byte integer는 network byte order로 표기한다.
알 수 없는 kind, codec, flag bit가 보이면 decode error다. 또한 `HasRequestSeq`,
`HasMetadata` flag와 실제 필드의 존재 여부가 어긋나도 decode error로 처리한다.

metadata bytes는 다음과 같은 순서로 늘어선 binary key-value 목록이다.
...
metadata의 key와 value는 모두 UTF-8 문자열이다. `key_len`은 1 이상이어야 한다. 같은
key가 두 번 등장하면 decode error다. `count` 값은 뒤따라오는 entry 개수와 반드시
일치해야 한다.

helper header의 `flags` 값은 공통 스펙과 동일하게 맞춘다.
...
압축 알고리즘은 packet마다 header에 따로 적지 않는다. `.NET` connector의 `Compression`
option으로 한 번 정한다. `body compressed` flag는 단지 "이 packet의 body가 그 알고리즘으로
압축되어 있다"는 표시일 뿐이다. 이 옵션이 client-to-server 자동 압축을 켜는 것은 아니다.
client에서 server로 보낼 때는 send/request builder에서 `.Compress()`를 명시한 경우에만
압축이 적용된다.

## 6. Metadata API 초안

사용자 API는 임의의 header schema를 그대로 받지 않는다. connector는 정해진 helper
header를 사용하고, 사용자가 추가로 실어 보낼 수 있는 값은 작은 metadata key-value 쌍으로
제한한다. 임의 header bytes를 직접 다루는 API는 connector의 공개 표면에 두지 않는다.
...
metadata의 key와 value는 UTF-8 문자열로 인코딩한다. 구현은 key 중복, 빈 key, 지나치게 큰
metadata를 validation error로 잡아내야 한다.

## 7. Packet Name API 초안

packet 이름은 helper header에 그대로 실려 나가기 때문에 짧아야 한다. 기본 이름은 CLR
타입 이름을 사용하지만, 사용자는 attribute나 resolver를 통해 이름을 다르게 지정할 수
있어야 한다.
...
resolver가 돌려준 이름은 UTF-8 기준 255 bytes를 넘으면 안 된다.
...
connector는 이 문서에서 정의한 helper header만 decode한다. 수신한 packet은 `On(...)`에
등록해 둔 이름별 handler로 전달한다. helper header decode가 실패하면 `ErrorReceived`를
통해 오류만 전달하고, packet body를 raw 상태 그대로 사용자에게 넘겨주지는 않는다.

`Send(...).Submit(...)`는 응답을 기다리지 않는 submit API다. 실제 transport write는 connector
내부에서 비동기로 수행된다. 호출 시점에는 frame size, packet name, metadata size, 연결
상태처럼 즉시 판단할 수 있는 validation 실패만 예외로 던진다. write 도중 발생한 오류는
`ErrorReceived`로 전달한다.
stream connector의 public option에는 `SendTimeout`을 두지 않는다. connector send는
request/reply 대기와는 의미가 다른 fire-and-forget submit이라서, timeout 옵션을 노출하면
request timeout과 의미가 뒤섞이게 된다. connector에서 timeout이 필요한 공개 옵션은
request의 reply 대기용인 `RequestTimeout`이다.

## 9. Send / Request Builder API 초안

fluent builder는 timeout, metadata, cancellation을 호출 단위로 조절할 수 있게 해 준다.
codec은 `ZlinkStreamEncodedBody` 안에 들어 있는 값을 그대로 사용한다. 호출자는 `ToJson`,
`ToMsgPack`, `ToProto` 같은 명시형 helper를 통해 body bytes와 codec 값을 함께 만든다.
각 codec package는 일반 CLR 객체를 곧장 넘길 수 있는 typed convenience builder도 함께
제공한다. 예를 들어 JSON package의 namespace를 사용하면 `Request(request)`는 JSON으로
...
builder instance는 한 번만 실행할 수 있다. 같은 builder에서 `Submit`을 두 번 호출하면
...
packet 이름을 따로 명시하지 않으면, 기본 이름은 namespace를 뺀 body CLR 타입 이름이
된다. 다른 이름이 필요하다면 `ZlinkStreamPacketNameAttribute` 또는
...
client에서 server로의 압축은 명시적으로 호출했을 때만 적용한다.
...
actor adapter는 packet의 metadata를 보고 필요한 경우 body의 압축을 해제한다.
...
async API는 실패 시 exception을 던진다. callback 기반 API는 `ErrorReceived`를 호출한다.
두 경로의 error code가 가지는 의미는 서로 같아야 한다.

## 11. Codec API 초안

core 패키지는 codec을 강제로 정해 두지 않는다. codec package는 명시적인 extension
method를 통해 `ZlinkStreamEncodedBody`를 만들고 읽는다. 또한 같은 package 안에 typed
convenience builder도 함께 제공해서, 일반 CLR 객체를 `Send`, `Request`, `On`에 그대로
넘길 수 있게 한다.
...
connector core는 타입만 보고서 JSON, MessagePack, Protobuf 가운데 하나를 임의로 고르지
않는다. 어떤 codec을 사용할지는 호출자가 import한 codec package의 namespace로 명시한다.
`ToJson`, `ToMsgPack`, `ToProto`를 직접 호출해도 되고, 해당 package의 `Request<T>()`,
`Send<T>()`, `On<T>()` convenience API를 활용해도 된다.

`Systems.Zlink.Stream.Connector.Codecs` package는 다음 순서로 codec을 자동 선택하는
편의 API를 제공한다.

1. `Google.Protobuf.IMessage`를 구현한 타입이면 Protobuf를 사용한다.
2. `[MessagePackObject]`가 붙은 타입이면 MessagePack을 사용한다.
3. 그 외 일반 CLR 객체는 JSON을 사용한다.

## 12. Compression Codec API 초안

compression package는 아래 인터페이스로 body의 압축과 해제를 제공한다.
...
사용하도록 설정을 맞추는 책임은 사용자에게 있다. `ZlinkStreamCompression.None`은 body를
그대로 둔다. `BodyCompressed` flag가 켜져 있는데도 `Compression`이 `None`이거나
`CompressionCodec`이 없다면 decode error로 처리한다.

## 13. Request Helper

request helper는 `header + body` 전송 위에 얹는 선택적인 기능이다. 다만 일반 client에서도
...
request sequence는 header helper 영역에서 다룬다. 다만 사용자가 직접 byte header를 구성하는
경로를 막지는 않는다.
...
`Error`의 body는 codec과 무관하게 UTF-8 JSON object로 인코딩한다.

```json
{"code":"error_code","message":"message"}
```

애플리케이션 도메인의 error를 정상 reply body로 다루고 싶다면, `Response` kind와 사용자
정의 body schema를 사용한다.

## 14. Codec Extension

core 패키지는 codec을 강제하지 않는다. 다음 확장 패키지는 packet 이름, optional metadata,
body payload를 만들고 parse하는 일만 돕는다.
...
codec extension은 body bytes와 함께 `ZlinkStreamCodec` 값을 만들어 주고, connector는 그
codec 값을 helper header에 그대로 적는다. core API는 reply나 handler에 `ZlinkStreamEncodedBody`를
그대로 흘려보내고, codec package의 typed convenience API가 이를 다시 지정된 reply/body
타입으로 풀어 준다. codec extension이 transport, timeout, request map, callback dispatch의
규칙을 바꿔서는 안 된다.
...
- 서버가 helper header에 `body compressed` flag를 켜고 body를 보내면, `.NET` connector가
  typed 사용자 callback이 호출되기 전에 body를 미리 압축 해제해 둔다.
- typed message handler와 request reply decode는 모두 압축이 해제된 body를 받는다.
- typed message handler와 request reply decode는 모두 압축이 해제된 body를 받는다.
- 압축 해제에 실패하면 `DecompressionFailed` error로 사용자에게 전달한다.

client-to-server 방향은 명시적으로 호출했을 때만 압축한다.

- 기본 `Send`와 `Request`는 압축하지 않는다.
- `.Compress()`를 호출한 send/request만 body를 압축한다.
- 압축이 적용된 packet에는 helper header에 `body compressed` flag를 켠다.

압축은 오직 body에만 적용한다. helper header는 압축하지 않는다.

## 16. Unity Adapter

`Systems.Zlink.Stream.Connector.Unity`는 별도의 Unity package로 배포한다. 이 Unity package는
`Systems.Zlink.Stream.Connector` core를 참조하고, 그 위에 Unity main thread callback dispatch,
`MonoBehaviour` wrapper, Unity lifecycle만 얇게 얹는다.

Unity 측 상세 계약은 [unity-stream-connector.ko.md](./unity-stream-connector.ko.md)를
기준으로 한다. Unity adapter는 `ZlinkStreamConnector`의 packet 의미나 helper header 의미를
임의로 바꾸지 않는다.
...
구현은 다음 순서로 진행한다. 단계마다 공개 계약을 따로따로 끊어 내기 위한 순서는
아니다. 최종 공개 package에는 이 문서의 계약을 한 번에 적용한다.
...
Stream Connector 항목은 transport frame, typed request/reply, metadata, packet name, codec,
compression, error handling을 각각 분리해서 검증한다. Connector API를 수정하는 경우에는
아래 테스트 이름과 문서 설명을 반드시 함께 맞춰 둔다.
