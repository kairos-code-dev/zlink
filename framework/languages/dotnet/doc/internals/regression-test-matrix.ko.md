...
| channel handler에서 fanout publish | `integration-single-process` | 일반 request handler가 같은 DI 컨테이너의 `IZLinkFanoutPublisher`로 fanout event 를 publish 하고 subscriber handler가 수신한다 |
