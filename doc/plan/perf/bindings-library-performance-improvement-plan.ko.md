...
| `tcp` | `MULTI_SPOT` | `보류(실패)` | `보류(37.7%)` | `보류(32.3%)` | `보류(11.0%)` | `보류(실패)` | `보류(실패)` | C `perf_c_multi_linux_20260522_143606_codex_c_multi_tcp_no_stream_for_rust_20260522.txt`, Rust `perf_rust_multi_linux_20260522_145753_codex_rust_multi_tcp_spot_20260522.txt` 기준이다. 256B/1024B/65536B는 complete지만 기준보다 낮다. 64B/131072B/262144B는 timeout이고, SPOT 실행 중 server/client는 각 `nlwp=14` 수준이라 thread 폭증은 아니며 spot dispatch/data thread가 추가되는 구조다. |
...
| `ws` | `MULTI_SPOT` | `보류(39.6%)` | `보류(38.7%)` | `보류(30.9%)` | `보류(14.3%)` | `보류(10.2%)` | `보류(실패)` | C 파일은 위 행과 같고, Rust `perf_rust_multi_linux_20260522_152121_codex_rust_multi_ws_spot_20260522.txt` 기준이다. 실행 중 server/client는 각 `nlwp=14` 수준으로 spot dispatch/data thread가 추가되는 구조다. 262144B는 timeout이다. |
