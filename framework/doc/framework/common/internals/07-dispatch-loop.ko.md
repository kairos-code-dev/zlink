...
```mermaid
flowchart TB
    W["깨어난다"] --> C["회전 cursor 위치에서 시작"]
    C --> R["이 연결에서 읽는다"]
    R --> L{"건수 · byte · 경과 시간<br/>중 하나라도 닿았나"}
    L -- "아니오, 더 있다" --> R
    L -- "아니오, 이 연결은 비었다" --> N["다음 연결로"]
    L -- "예" --> S["cursor를 여기에 남긴다"]
    N --> E{"한 바퀴 돌았나"}
    E -- "아니오" --> R
    E -- "예" --> S
    S --> Z["잠든다"]
    Z -. "남은 것은 다음 회전에서<br/>cursor 자리부터 이어 읽는다" .-> W
```

`L`에서 나가는 화살표 셋 중 **"예"가 없으면** 한 연결이 수신 단계를 독점한다. `S`가
없으면 항상 앞쪽 연결부터 다시 시작해, 상한을 두어도 뒤쪽 연결이 밀린다.
