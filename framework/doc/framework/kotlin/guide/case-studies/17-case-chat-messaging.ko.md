...
```java
String userId = authenticate(ws);
redis.hashSet("presence:" + userId, "node", thisNodeId);
Set<String> members = redis.setMembers("room:" + roomId + ":members");
for (String member : members) {
    String node = redis.hashGet("presence:" + member, "node");
    redis.publish("deliver:" + node, encode(roomId, msg));
}
messageDb.append(roomId, msg);
```
...
```kotlin
@Component
class SayHandler(private val db: MessageDb) :
    ZLinkSuspendingSpotActorSendHandler<ChatRoomSpot, UserActor, Say> {
    override suspend fun handle(
        ...
        val chat = ChatMessage(spot.roomId(), actor.actorId(), msg.text)
        db.append(chat)
        spot.members().forEach { member -> member.pushChat(chat) }
    }
}

class UserActor(private val context: ZLinkActorContext) : ZLinkActor {
    fun pushChat(chat: ChatMessage): CompletionStage<Void> =
        context.boundSession().send(chat).submit()
}
```
...
- 실행 코드: [Kotlin SupportChat 샘플](../../../../../languages/java/samples/kotlin/SupportChat)
