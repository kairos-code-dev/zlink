...
| config | 현재 판단 | 작업 원칙 |
|--------|-----------|-----------|
| `RegistryMessaging` | RM-A/B/C 구현 범위 통과 | `.NET`의 `Client/Scenarios`, `Server/<Role>`, `Shared` 분류에 맞춘 C++ role/source와 inventory가 있다. `timeout 420s framework/languages/cpp/e2e/RegistryMessaging/run_e2e.sh all` 최신 통과 증거는 `logs/20260630-161051-366893`이다. RM-C9는 public one-way send 계약에 맞춰 send pressure/recovery를 검증하고, 수정 뒤 focused 통과 증거는 `logs/20260701-041908-22985`이다. bounded send failure oracle은 공통 E2E 기대에서 제거했다. |
| `SpotService` | 구현 범위 통과 | `.NET` 기준 inventory와 C++ scenario header/role source가 있다. `Client/Support/client_support.hpp`, `Client/Support/client_options.hpp`, `Client/Support/spot_lifecycle_order_context.hpp`를 추가해 client 공통 env/assert/stream helper, option 값, channel handler state, `.NET`식 lifecycle context를 `Client/main.cpp`에서 분리했고, `Shared/spot_service_contracts.hpp`에 generic JSON stream payload hook을 추가해 stream connector DTO가 기본 JSON codec 경로를 쓰게 했다. `SM-A5`는 `.NET`의 app-level `ScenarioStage` 의미를 C++ user spot handler와 public timer API 위에 구현했다. `SM-F3`/`SM-F5`는 `.NET`에 별도 scenario 파일은 없지만 공통 E2E와 feature-map에 있는 scenario ID라 `Client/Scenarios/sm_f3_scenario.hpp`, `Client/Scenarios/sm_f5_scenario.hpp`로 분리하고 focused runner로 검증했다. `SM-Q9`는 MultiNode A/B role과 외부 route client target spot request로 focused runtime proof를 추가했다. `.NET` SM-D2처럼 `session-a`에서 `play-b`로 가는 stream relay readiness를 확인하고, crash setup/recover route endpoint를 독립시켰다. `SM-G3`는 `.NET`처럼 두 stream client를 먼저 순차 auth/bind한 뒤 ping/leave만 동시에 실행하도록 맞췄다. Full `all` runner는 낡은 monolith client 대신 `.NET`처럼 focused scenario child sweep로 실행하며, `timeout 1200s framework/languages/cpp/e2e/SpotService/run_e2e.sh` 최신 full 통과 증거는 `logs/20260701-024713-1805184`이다. 수정 후 SM-G3 focused 통과 증거는 `logs/20260701-024519-1802586`, `logs/20260701-024547-1803383`이고, SM-F3/SM-F5 focused 통과 증거는 `logs/20260701-014358-1695850`, `logs/20260701-014425-1696662`이다. |
| `PubSub` | 완료 | `.NET`의 publisher, registry, subscriber 역할에 대응하는 C++ 전용 executable과 Client/Scenarios, Client/Support, Shared contract가 있다. subscriber role server의 `/evidence/wait` endpoint로 PS-A1, PS-A2, PS-A3, PS-A4, PS-B1, PS-B2, PS-C1을 검증하며, `timeout 420s framework/languages/cpp/e2e/PubSub/run_e2e.sh all` 최신 통과 증거는 `logs/20260630-162014-391042`이다. |
| `RegistrationCodec` | 구현 범위 통과, RC-A2 public gap | `.NET` 기준 inventory와 C++ client scenario/support, server configuration/handler/endpoint/support, invalid role, JSON-only peer role 분리가 있다. `timeout 420s framework/languages/cpp/e2e/RegistrationCodec/run_e2e.sh` 최신 통과 증거는 `logs/20260630-182337-725659`이며 RC-A1, RC-A3, RC-A4, RC-A5, RC-A6, RC-B1, RC-B2, RC-B3, RC-B4, RC-B5를 검증한다. RC-A2 attribute 등록은 `.NET` 전용 public 표면이라 C++에 새 public API를 만들지 않고 feature-map gap으로 유지한다. |
| `DeliveryDispatch` | 구현 범위 통과 | 기존 `DiscoveryRegistryHa` harness 대신 `.NET DeliveryDispatch` 샘플 기준 C++ E2E가 있다. registry, dispatch API, dispatch center, customer gateway, courier session, courier gateway, courier actor node 1/2, tracking, probe, client target을 실행하고, customer stream 1개와 courier stream 2개로 `SubscribeDelivery`, actor-bound `DeliveryStatusNotify`, `BindCourierSessionReq`, `BindCourierReq`, `EnsureCourierActorReq`, actor-bound `OfferDeliveryNotify`, `CourierDecisionMsg` 흐름을 검증한다. `timeout 420s framework/languages/cpp/e2e/DeliveryDispatch/run_e2e.sh` 최신 통과 증거가 있으며 successful delivery, reassignment, server evidence self-check, customer/courier gateway/session/actor-node message-flow evidence가 통과한다. CustomerGateway와 CourierSession은 public actor/session API로 stream을 actor에 bind하고 bound session으로 status/offer를 push한다. |
| `ResilienceLifecycle` | 구현 범위 통과, 이름 정리 partial | 전용 Registry, Provider, Workflow, Consumer, Client target과 `.NET` 기준 inventory가 있다. Client support를 `client_options.hpp`, `scenario_assert.hpp`, `lifecycle_api_result.hpp`, `topology_entry_result.hpp`로 분리했고 profile request/reply/send/failure/status DTO는 `.NET`식 marker 필드를 지원한다. Registry host wiring은 `registry_host_factory.hpp`로 분리했고 Registry evidence store와 fault state를 `Infrastructure/evidence_store.hpp`, `Infrastructure/fault_state.hpp`로 뒀으며, 선택적 registry channel endpoint가 설정되면 `Handlers/registry_handlers.hpp`의 profile handler와 dispatch error observer를 설치한다. Provider host wiring은 `provider_host_factory.hpp`로 분리했다. Consumer role main은 ResilienceLifecycle 전용 `Configuration/consumer_options.hpp`, `Endpoints/consumer_endpoints.hpp` wrapper를 사용하며, Consumer endpoint handler는 marker를 보존하는 ResilienceLifecycle contract로 분리했다. Provider evidence store는 `Infrastructure/evidence_store.hpp`로, Provider dispatch error observer는 `Handlers/evidence_dispatch_error_observer.hpp`로, observer fault mode state는 `Infrastructure/fault_state.hpp`로 분리했다. RL-A1, RL-A2, RL-A3, RL-A4, RL-A5, RL-B3는 ResilienceLifecycle 전용 scenario wrapper와 runner scenario 이름으로 실행한다. RL-B1은 Consumer HTTP `/profile/request/timeout/100`과 후속 `/profile/request`로 검증하고, RL-C1은 Consumer HTTP `/profile/request/new-client` 반복 request와 cleanup marker evidence로 검증한다. RL-C2는 provider crash 뒤 Consumer HTTP new-client request가 `api-a`로 수렴하고 provider B 재기동 뒤 restored marker가 `api-b` evidence에 남는지 검증한다. RL-C3는 provider B down 중 Consumer HTTP request가 `api-a`로 수렴하고 recovery 뒤 `api-b` evidence가 남는지 검증한다. RL-D1은 Consumer HTTP request burst와 provider evidence marker로, RL-D3/RL-D4는 Consumer HTTP missing request와 provider flow marker로, RL-D5는 Consumer HTTP request/send mixed burst와 provider evidence marker로 검증한다. `timeout 1200s framework/languages/cpp/e2e/ResilienceLifecycle/run_e2e.sh` 최신 통과 증거는 `logs/20260701-031111-1848503`이며 Consumer smoke, RL-C1 consumer new-client, RL-A1, RL-A2, RL-A3, RL-A4, RL-A5, RL-B1, RL-B2, RL-B3, RL-B4, RL-B5, RL-B6, RL-C1, RL-C2, RL-C3, RL-C4 registry outage 및 registry/provider 재기동 뒤 새 discovery client 복구, RL-D1, RL-D2, RL-D3, RL-D4, RL-D5를 검증한다. 남은 partial은 public 동작 추가가 아니라 RegistryMessaging 기반에서 시작한 일부 helper와 파일 이름 정리다. |
| `RuntimeMonitoring` | 구현 범위 통과 | `.NET` 기준 이름인 `RuntimeMonitoring`으로 전환했고 기존 C++ `Monitoring` PubSub 보조 wrapper는 삭제했다. C++ 전용 registry, service, filtered service, throwing service, trigger, client target과 `.NET` 기준 inventory가 있다. `.NET ClientOptions`에 대응하는 C++ `client_options.hpp`를 추가해 client endpoint option을 한 곳에서 읽고 scenario에 전달한다. Registry role host wiring은 `Server/Registry/registry_host_factory.hpp`로 분리했고 health/evidence/wait/shutdown endpoint mapping도 factory에서 담당한다. Registry event evidence 기록은 `Server/Registry/Handlers/registry_event_recorders.hpp`로 분리했다. Service/FilteredService/ThrowingService는 role-local factory wrapper가 all/socket-filter/throwing profile을 선택하고 공통 service host 구성을 재사용한다. Service event evidence 기록은 `Server/Service/Handlers/service_event_recorders.hpp`로 분리했다. Trigger role host wiring은 `Server/Trigger/trigger_host_factory.hpp`로 분리했고 trigger endpoint route mapping은 factory에서 담당한다. Trigger HTTP handler class는 `Server/Trigger/trigger_handlers.hpp`로 분리했다. `timeout 420s framework/languages/cpp/e2e/RuntimeMonitoring/run_e2e.sh` 최신 통과 증거는 `logs/20260701-020431-1722060`이며 MON-A1, MON-A2, MON-A3, MON-A4, MON-A5, MON-B1, MON-B2, MON-C1, MON-D1을 검증한다. |
| `YieldDispatch` | Track A/B/C/D/E1/E3/E4/E5 진행 중 | C++ e2e config와 runner가 생겼고 YD-A1~YD-A4, YD-B1~YD-B3, YD-C1~YD-C3, YD-D1~YD-D4, YD-E1, YD-E3, YD-E4 static gate와 YD-E5 report 생성은 runner 통과 증거가 있다. 최신 `timeout 420s framework/languages/cpp/e2e/YieldDispatch/run_e2e.sh` 통과 증거는 `logs/20260701-013458-1684685`이며 `scenario YD-E5 passed`와 `yield-dispatch-report.json`을 확인한다. public send `submit()` 반환값 의존을 제거한 뒤에도 Session relay와 client scenario target이 빌드되고 full runner가 통과한다. 구현된 client scenario header는 YD-A/B/C/D2~D4/E1/E3까지 완료했고, Registry host factory/support, Delay host factory/handler/support, Play host factory/support/control/basic/timer/actor/remote/failure handler, spot type, `YieldProbeSpot` runtime, Session host factory/support, `yield_session_t` support header도 분리했다. 남은 완료 보류 사유는 YD-C3 actor-yield/timer-fast half가 observer connector를 쓰는 partial과 YD-E2 public yield cancellation token contract gap이다. YD-C3 같은 connector 재시도는 `logs/20260701-003005-1569332`에서 ordering 실패했고, session actor relay를 fire-and-forget으로 바꾸는 우회도 `logs/20260701-013205-1681063`에서 YD-B1 request reply 실패로 폐기했다. YD-E2는 공통 E2E가 cancellation cleanup을 요구하지만 C++ `cpp-framework-interfaces` spec이 public cancellation token/registration을 제공하지 않는다고 명시하고 현재 public `yield()` 표면도 token 인자를 받지 않으므로, 내부 pending cancel이나 handler-local timeout으로 대체하지 않는다. |
