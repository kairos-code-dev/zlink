...
void ConsoleDisplay::print_send (const std::string &client_name,
...
void ConsoleDisplay::print_recv (const std::string &client_name,
                                 const std::string &server_label,
                                 const std::string &message,
                                 const std::string &annotation)
{
    ...
    std::string padded = pad_name (client_name);
    if (annotation.empty ()) {
        std::printf ("  %s[%s<-%s]%s <<< %s\n", color, padded.c_str (), server_label.c_str (),
                     RESET, message.c_str ());
    } else {
        std::printf ("  %s[%s<-%s]%s <<< %s  %s%s%s\n", color, padded.c_str (),
                     server_label.c_str (), RESET, message.c_str (), DIM, annotation.c_str (),
                     RESET);
    }
    std::fflush (stdout);
}

void ConsoleDisplay::print_no_event (const std::string &client_name,
                                     const std::string &server_label,
                                     const std::string &reason)
{
    std::string padded = pad_name (client_name);
    std::printf ("  %s[%s<-%s]     (no event -- %s)%s\n", DIM, padded.c_str (),
                 server_label.c_str (), reason.c_str (), RESET);
    std::fflush (stdout);
}
