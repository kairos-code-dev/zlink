...
struct pair_recv_state_t
{
    pair_recv_state_t () : run_id (0), msg_size (0), payload_size (0), active_received (0) {}
                                   ...
                                   bool *header_ok_out_)
{
    ...
    if (source_rid || has_more != ZLINK_PART_FINAL) {
        if (bench_debug_enabled ()) {
            std::cerr << "[perf-pair] unexpected recv metadata has_more="
    ...
    if (!size_ok) {
        if (bench_debug_enabled ()) {
            std::cerr << "[perf-pair] unexpected payload size=" << actual_size
...
bool send_pair_samples (void *sender_,
                        std::vector<char> *payload_,
                        pair_recv_state_t *state_,
                        int duration_s_,
                       ...
                       latency_stats_t *latency_out_)
{
    ...
    if (!sender_ok.load (std::memory_order_acquire)) {
        if (bench_debug_enabled ()) {
            std::cerr << "[perf-pair] active phase failed sent="
               ...
               const std::string &lib_name)
{
    ...
    if (!run_active_phase (connect_socket.get (),
                           bind_socket.get (),
                           &payload,
                           &state,
                           duration_s,
