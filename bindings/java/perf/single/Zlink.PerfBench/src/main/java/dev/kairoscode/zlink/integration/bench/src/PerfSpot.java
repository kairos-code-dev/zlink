        ...
        try (Context context = new Context();
            ...
            try (Spot publisher = new Spot(pubNode);
                ...
                for (int i = 0; i < warmupCount; i++) {
                    ...
                    try (Message msg = Message.fromBytes(payload, 0,
                        payloadSize)) {
                        publisher.publish(TOPIC, msg, SendFlag.NONE);
                    }
                    receiveSpotBlocking(subscriber, recv);
                    while (receiveSpotNonBlocking(subscriber, recv) > 0) {
                    }
                }
                ...
                while (System.nanoTime() < endNs) {
                    ...
                    int n = receiveSpotBlocking(subscriber, recv);
                    if (n > 0) {
                        received += collectActiveSample(recv, n, runId, header,
                            latency);
                        while (true) {
                            int drained = receiveSpotNonBlocking(subscriber, recv);
                            if (drained <= 0) {
                                break;
                            }
