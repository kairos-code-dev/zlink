# Node.js YieldDispatch E2E 포팅 인벤토리
...
|----------|----------------|-------------------|------|------|
| YD-A1 | `Client/Scenarios/YdA1BasicTerminatorScenario.cs` | `Client/Scenarios/yd-a1-basic-terminator-scenario.ts`, `Server/Play/Handlers/basic-spot-handlers.ts`, `Server/Session/Handlers/yield-session.ts` | done | stream connector와 Session gateway 경로를 구현했다. `logs/20260629-235727-1952125`에서 marker order 검증 통과 |
| YD-A2 | `Client/Scenarios/YdA2YieldTerminatorScenario.cs` | `Client/Scenarios/yd-a2-yield-terminator-scenario.ts`, `Server/Play/Handlers/basic-spot-handlers.ts`, `Server/Session/Handlers/yield-session.ts` | done | `ZLinkRequestCall.yield<TReply>()`를 쓰는 Spot handler를 추가했다. `logs/20260629-235727-1952125`에서 yield/probe marker order 검증 통과 |
| YD-A3 | `Client/Scenarios/YdA3ContinuationContextScenario.cs` | `Client/Scenarios/yd-a3-continuation-context-scenario.ts` | done | `.NET`과 같은 request id, spot rid, correlation id, yield continuation marker order 검증은 통과했다. stream metadata 직접 노출은 Spot request handler public surface가 아니므로 이 시나리오의 완료 조건에 넣지 않는다. |
| YD-A4 | `Client/Scenarios/YdA4WorkerYieldScenario.cs` | `Client/Scenarios/yd-a4-worker-yield-scenario.ts`, `Server/Play/Handlers/basic-spot-handlers.ts`, `Server/Session/Handlers/yield-session.ts` | done | `runWorker(...).yield()`와 probe interleaving marker order가 runner에서 통과했다. |
| YD-B1 | `Client/Scenarios/YdB1OtherActorProgressScenario.cs` | `Client/Scenarios/yd-b1-other-actor-progress-scenario.ts`, `Server/Play/Spots/yield-actors.ts`, `Server/Play/Handlers/control-handlers.ts`, `Server/Session/Handlers/yield-session.ts` | done | actor A yield 중 actor B fast request가 먼저 완료되는 marker order와 actor mailbox id를 검증했다. `logs/20260629-235727-1952125`에서 통과 |
| YD-B2 | `Client/Scenarios/YdB2SameActorReentryScenario.cs` | `Client/Scenarios/yd-b2-same-actor-reentry-scenario.ts`, `Server/Play/Spots/yield-actors.ts`, `Server/Session/Handlers/yield-session.ts` | done | 같은 actor A의 fast packet이 yield continuation과 completion 뒤에 실행되는 marker order를 검증했다. `logs/20260629-235727-1952125`에서 통과 |
| YD-B3 | `Client/Scenarios/YdB3ActorJoinYieldScenario.cs` | `Client/Scenarios/yd-b3-actor-join-yield-scenario.ts`, `Server/Play/Spots/yield-actors.ts`, `Server/Play/Spots/yield-probe-spot.ts`, `Server/Session/Handlers/yield-session.ts` | done | `actor.context.joinSpot(...).yield()` 중 actor B fast request가 먼저 완료되는 marker order를 검증했다. `logs/20260629-235727-1952125`에서 통과 |
| YD-C1 | `Client/Scenarios/YdC1TimerIsolationScenario.cs` | `Client/Scenarios/yd-c1-timer-isolation-scenario.ts`, `Server/Play/Handlers/timer-spot-handlers.ts`, `Server/Play/Spots/yield-probe-spot.ts`, `Server/Play/Spots/yield-timer-state.ts`, `Server/Session/Handlers/yield-session.ts` | done | timer A yield 중 timer B fast tick이 먼저 완료되는 marker order를 검증했다. `logs/20260629-235727-1952125`에서 통과 |
| YD-C2 | `Client/Scenarios/YdC2TimerReentryScenario.cs` | `Client/Scenarios/yd-c2-timer-reentry-scenario.ts`, `Server/Play/Handlers/timer-spot-handlers.ts`, `Server/Play/Spots/yield-probe-spot.ts`, `Server/Play/Spots/yield-timer-state.ts`, `Server/Session/Handlers/yield-session.ts` | done | 같은 timer의 다음 tick이 yield continuation 뒤에 실행되는 marker order를 검증했다. `logs/20260629-235727-1952125`에서 통과 |
...
| YD-E3 | `Client/Scenarios/ShutdownYieldScenario.cs` | `Client/Scenarios/shutdown-yield-scenario.ts`, `Server/Session/Handlers/yield-session.ts`, `run_e2e.sh` | done | pending yield 중 `play-a`를 종료한 뒤 stream request가 public closed/cancelled error로 끝나고, 같은 endpoint로 재시작한 `play-a`가 새 Spot probe를 처리하는지 검증했다. 선택 실행 `logs/20260630-015152-2270680`, full 실행 `logs/20260630-102243-3490701`에서 통과 |
| YD-E4 | `run_e2e.sh` | `run_e2e.sh` | done | yield 시작용 HTTP endpoint/client 사용, `Server/Play` 밖의 `.yield(...)` 사용, YD scenario 파일의 connector helper 우회, shutdown scenario의 stream connector 직접 생성 누락을 정적으로 검사한다. `logs/20260630-102243-3490701`에서 `scenario YD-E4 passed` |
...
| `Client/Program.cs` | `Client/main.ts` | client-entry | done | `full`, `YD-A1`, `YD-A2`, `YD-A3`, `YD-A4`, `YD-B1`, `YD-B2`, `YD-B3`, `YD-C1`, `YD-C2`, `YD-C3`, `YD-D2`, `YD-D3`, `YD-D4`, `YD-E1`, `YD-E2` 선택 실행이 가능하다. YD-E3는 runner가 `shutdown-wait`와 `shutdown-recovery` subcommand를 조합해 실행한다. |
| `Client/YieldDispatch.Client.csproj` | `Client/package.json`, `Client/tsconfig.json` | client-project | done | Client build 설정 추가 |
...
| `Client/Scenarios/YieldActorScenarioContext.cs` | `Client/Scenarios/yd-b1-other-actor-progress-scenario.ts` | client-support | merged | B1/B2/B3/D4에 필요한 actor context를 B1 scenario 파일에서 export한다. 별도 context 파일은 만들지 않았다. |
| `Client/Scenarios/YdA1BasicTerminatorScenario.cs` | `Client/Scenarios/yd-a1-basic-terminator-scenario.ts` | scenario | done | stream connector 요청과 marker order 검증을 포팅했고 runner에서 통과했다. |
| `Client/Scenarios/YdA2YieldTerminatorScenario.cs` | `Client/Scenarios/yd-a2-yield-terminator-scenario.ts` | scenario | done | yield marker order 검증을 포팅했고 runner에서 통과했다. |
| `Client/Scenarios/YdA3ContinuationContextScenario.cs` | `Client/Scenarios/yd-a3-continuation-context-scenario.ts` | scenario | done | request id, spot rid, correlation id, yield continuation marker order를 검증한다. |
| `Client/Scenarios/YdA4WorkerYieldScenario.cs` | `Client/Scenarios/yd-a4-worker-yield-scenario.ts` | scenario | done | worker yield와 probe interleaving 검증을 포팅했고 runner에서 통과했다. |
| `Client/Scenarios/YdB1OtherActorProgressScenario.cs` | `Client/Scenarios/yd-b1-other-actor-progress-scenario.ts` | scenario | done | 두 stream connector를 같은 actor set에 바인딩해 actor A yield와 actor B fast request를 동시에 검증한다. |
| `Client/Scenarios/YdB2SameActorReentryScenario.cs` | `Client/Scenarios/yd-b2-same-actor-reentry-scenario.ts` | scenario | done | Node stream connector 제약 때문에 두 번째 same-actor packet은 같은 connector에서 send로 전달한다. actor mailbox 재진입 금지는 Play evidence order로 검증한다. |
| `Client/Scenarios/YdB3ActorJoinYieldScenario.cs` | `Client/Scenarios/yd-b3-actor-join-yield-scenario.ts` | scenario | done | actor join yield와 actor B fast request interleaving을 검증한다. |
| `Client/Scenarios/YdC1TimerIsolationScenario.cs` | `Client/Scenarios/yd-c1-timer-isolation-scenario.ts` | scenario | done | timer yield와 other timer fast tick interleaving을 검증한다. |
...
| `Server/Delay/DelaySupport.cs` | `Server/Support/evidence-store.ts`, `Server/Support/http-server.ts` | support | merged | 공통 evidence와 HTTP readiness helper로 재분류했다. |
| `Server/Delay/YieldDispatch.Delay.csproj` | `Server/Delay/package.json`, `Server/Delay/tsconfig.json` | delay-project | done | Delay build 설정 추가 |
| `Server/Play/Program.cs` | `Server/Play/main.ts` | play-role | done | play role entrypoint 추가 |
| `Server/Play/PlayHostFactory.cs` | `Server/Play/play-host-factory.ts`, `Server/Play/Configuration/play-options.ts` | play-role | done | YD-A1/YD-E4 범위에 필요한 route, spot, actor, timer, delay channel 구성 추가. runner에서 같은 binary를 `play-a`와 `play-b`로 구동한다. |
| `Server/Play/PlaySupport.cs` | `Server/Support/evidence-store.ts`, `Server/Support/http-server.ts` | support | merged | 공통 evidence와 HTTP readiness helper로 재분류했다. |
| `Server/Play/Handlers/PlayBasicSpotHandlers.cs` | `Server/Play/Handlers/basic-spot-handlers.ts` | play-role | done | Hold/Yield/Probe spot handler 추가. Yield handler는 public `.yield<TReply>()`를 사용하고 D2 target Spot request에는 reply를 반환한다. |
...
| `Server/Play/Spots/PlaySpotTypes.cs` | `Server/Play/Spots/yield-probe-spot.ts`, `Server/Play/Spots/yield-actors.ts`, `Server/Play/Spots/yield-timer-state.ts` | play-role | merged | basic Spot, B1/B2/B3 actor entry spot/factory, B3 target Spot actor join hook, C1/C2 timer state, C3/D4 joined Spot actor handler 등록을 역할별 파일로 나눴다. |
| `Server/Play/Spots/PlaySpotRuntime.cs` | `Server/Play/Spots/yield-probe-spot.ts`, `Server/Play/Handlers/basic-spot-handlers.ts`, `Server/Play/Handlers/timer-spot-handlers.ts` | play-role | merged | Spot runtime 책임을 Spot class와 handler로 나눴고, actor join delay/evidence hook과 timer lifecycle/state 관리를 추가했다. |
| `Server/Play/Handlers/PlayActorHandlers.cs` | `Server/Play/Spots/yield-actors.ts` | play-role | done | B1/B2/B3 actor yield/fast/join-yield handler, C3 joined Spot actor yield/fast handler, D4 actor push yield handler를 추가했다. |
| `Server/Play/Handlers/PlayFailureSpotHandlers.cs` | `Server/Play/Handlers/failure-spot-handlers.ts` | play-role | done | YD-E1 timeout handler와 YD-E2 cancellation handler를 추가했다. YD-E3 shutdown은 기존 Spot yield/probe handler와 session relay 조합으로 검증한다. |
| `Server/Play/Handlers/PlayRemoteSpotHandlers.cs` | `Server/Play/Handlers/remote-spot-handlers.ts` | play-role | done | D2 remote Spot yield request/command handler를 추가했다. D3는 기존 Spot yield/probe handler와 session relay 조합으로 검증한다. |
| `Server/Play/Handlers/PlayTimerSpotHandlers.cs` | `Server/Play/Handlers/timer-spot-handlers.ts` | play-role | done | C1/C2 timer start/stop/yield/fast/next tick handler를 추가했고 C3 actor/timer 조합 검증에도 사용한다. |
| `Server/Play/YieldDispatch.Play.csproj` | `Server/Play/package.json`, `Server/Play/tsconfig.json` | play-project | done | Play build 설정 추가 |
...
| `Server/Session/Support/SessionSupport.cs` | `Server/Support/evidence-store.ts`, `Server/Support/http-server.ts` | support | merged | 공통 evidence와 HTTP readiness helper로 재분류했다. |
...
- C1/C2 timer handler는 public `spot.context.addTimer(...)`와 timer handler 안의 public `.yield<TReply>()`를 사용한다.
  timer overrun policy는 `.NET` 기준과 같은 delay-next-tick 의미의 `DelayNextTick`으로 설정했다.
- C3 joined Spot actor handler도 public Spot actor handler 등록과 public `.yield<TReply>()`만 사용한다. 동적으로
  생성되는 Spot은 Nest transient provider로 등록해 Spot instance, context, serial line이 Spot rid별로 분리되도록 했다.
- C3 actor-yield 중 timer command는 별도 stream connector로 보낸다. Node stream connector가 pending request와
...
- PASS: `timeout 420s ./run_e2e.sh full`
  ...
  - `scenario YD-E4 passed`
  - `scenario YD-A1 passed`
  - `scenario YD-A2 passed`
  - `scenario YD-A3 passed`
  - `scenario YD-A4 passed`
  - `scenario YD-B1 passed`
  - `scenario YD-B2 passed`
  - `scenario YD-B3 passed`
  - `scenario YD-C1 passed`
  - `scenario YD-C2 passed`
  - `scenario YD-C3 passed`
  - `scenario YD-D2 passed`
  - `scenario YD-D3 passed`
  - `scenario YD-D4 passed`
  - `scenario YD-E1 passed`
  - `scenario YD-E2 passed`
...
- PASS: `timeout 420s ./run_e2e.sh YD-E3`
  - 로그: `logs/20260630-015152-2270680`
  - `yield-dispatch shutdown wait result=passed`
  - `yield-dispatch shutdown recovery result=passed`
  - `scenario YD-E3 passed`
  - `yield-dispatch e2e result=passed`
