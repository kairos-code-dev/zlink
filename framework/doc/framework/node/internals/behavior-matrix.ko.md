...
[문서 목록](../../../README.ko.md) | [이전: .NET → Node.js 표면 매핑 정책](dotnet-to-node-surface-mapping.ko.md) | [다음: ZLink Framework Node.js DI Capability Exposure Policy](di-capability-exposure-policy.ko.md)
...
| client-server channel을 SPOT route 수신 대상으로 사용 | 비허용 | 외부 -> Spot route는 RouteMesh로 단일화한다 |
| route mesh channel + local SpotMesh | 허용 | RouteMesh ROUTER가 단일 SpotNode에 자동 bridge된다 |
...
| 조합 | 허용 여부 | 기대 동작 |
|------|-----------|-----------|
| `.addRouteMesh(name).connect(undefined)` + 전역 `.useDiscovery()` 있음 | 허용 | bind 없이 discovery 기반 routed client node를 만든다 |
| `.addRouteMesh(name).connect(endpoint)` 있음 | 허용 | bind 없이 manual 기반 routed client node를 만든다 |
| `routeMesh` channel + server/client 역할 둘 다 없음 | 비허용 | startup validation 오류 |
