...
{
    int lat_count = env_int("PERF_LAT_COUNT", 1000);
    ...
    std::string endpoint = endpoint_for(transport, "router_router_poll");
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        if (rc == 4 && wait_for_input(router1, 0)) {
            int id_len = router1.recv(buf16, sizeof(buf16),
    ...
    if (!wait_for_input(router2, 2000))
        return 2;
    if (router2.recv(buf16, sizeof(buf16)) < 0
        || router2.recv(buf16, sizeof(buf16)) < 0)
        return 2;
    ...
    for (int i = 0; i < lat_count; ++i) {
        ...
        if (!wait_for_input(router1, 2000))
            return 2;
        ...
        if (router1.send(rid, static_cast<size_t>(rid_len),
                         zlink::send_flag::sndmore) < 0
        ...
        if (router2.recv(rid, sizeof(rid)) < 0
            || router2.recv(recv_buf.data(), size) < 0)
            return 2;
    }

    double latency = static_cast<double>(
      std::chrono::duration_cast<std::chrono::microseconds>(
        std::chrono::steady_clock::now() - t0).count()) / (lat_count * 2);
    ...
    double throughput = static_cast<double>(msg_count) / (static_cast<double>(
      std::chrono::duration_cast<std::chrono::nanoseconds>(
        std::chrono::steady_clock::now() - t0).count()) / 1e9);

    print_result("ROUTER_ROUTER_POLL", transport, size, throughput, latency);
    return 0;
}
