...
| **E2E-CP-27** | ~~[config-4:33-34,52-80](../../common/e2e/config-4-registration-codec.ko.md): C++도 자동·attribute·수동 등록 변주를 제공해야 한다~~ | **감사 판정 취소.** [공통 framework spec §3.3](../05-framework-api.ko.md)은 C++을 runtime reflection 기반 자동 등록 원칙의 예외로 정하고 compile-time 타입과 명시 builder 호출을 요구한다. 공통 E2E의 RC-A2도 `.NET` attribute 등록으로 한정한다. A1/A2를 구현으로 표시했던 feature-map을 `not-supported`로 바로잡고, attribute를 흉내 내지만 읽히지 않던 `topic_name`을 제거했다. |
