...
| `.NET: run_sample.sh` | `run_sample.sh` | runner | done | registry, API A/B, Play A/B, Session A/B, Redis, client self-check를 실행한다. |
...
| `common: API/Play/Session 2개 이상 실행` | `run_sample.sh` | runner | done | API A/B, Play A/B, Session A/B를 실행한다. |
...
| `common: success marker PASS Bingo.Ts` | `run_sample.sh` | validation | done | runner 성공 시 출력한다. |
