...
public:
  ...
  explicit channel_runtime_manager_t (
  ...
  channel_runtime_bundle_t &get_or_create_client_bundle (
    const std::string &channel_name);
  channel_runtime_bundle_t &get_or_create_publisher_bundle (
    const std::string &channel_name);
  route_channel_runtime_t &get_route_channel (
...
private:
  const channel_snapshot_t &require_channel (
    const std::string &channel_name,
    capability_t capability) const;

  static std::pair<std::string, std::string> parse_source (
