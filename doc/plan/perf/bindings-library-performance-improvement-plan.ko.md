...
| 5 | Go | `bindings/go/perf` | `tcp/ws/wss/tls single 통과 (6.0.3 재검증, SPOT large 회귀 수정, latency 발산은 측정 아티팩트로 판정)` | `2026-05-23 LockOSThread 수정으로 ROUTER_ROUTER/DEALER_ROUTER/PUBSUB 전 transport 통과, DD·SPOT large 대폭 개선(6.6.3). 잔여 보류: SPOT_SENDSEND large, DD wss/tls 최대 size, SPOT_REQREP large, 64B FFI` | 잔여 보류 추적: `SPOT_SENDSEND` request-reply echo(size별 active-slot+1ms cap) 재설계, 암호화 DD large copy 축소 |
...
| `wss` | `PUBSUB` | `통과(64.1%)` | `통과(66.2%)` | `통과(109.1%)` | `통과(86.1%)` | `통과(80.0%)` | `통과(86.1%)` | C/Go 파일은 위 wss PAIR 행과 같다. 256B는 latency가 C 대비 77.3x, 65536B는 17.4x라 통과로 확정하지 않는다. `GOGC=off`, `GOMAXPROCS=2`, topic C 문자열 캐시 후보는 ws 제한 재측정에서 같은 latency 병목을 해결하지 못했다. auto-HWM 활성과 size별 `MsgUnit(B)` 일치를 확인했다. |
| `wss` | `DEALER_DEALER` | `통과(74.3%)` | `통과(73.5%)` | `통과(106.9%)` | `통과(92.7%)` | `통과(75.5%)` | `통과(82.2%)` | C/Go 파일은 위 wss PAIR 행과 같다. 256B는 throughput 비율은 목표권이지만 latency가 C 대비 110.1x라 통과로 확정하지 않는다. 단일 part send move/restore 후보는 public 실패 원본 보존 테스트를 깨 반영하지 않았다. auto-HWM 활성과 size별 `MsgUnit(B)` 일치를 확인했다. |
...
| `ws` | `MULTI_DEALER_DEALER` | `보류(40.4%)` | `통과(57.0%)` | `통과(67.1%)` | `보류(23.7%)` | `보류(12.1%)` | `보류(4.8%)` | C `perf_c_multi_linux_20260522_055144_codex_c_ws_multi_for_go_20260522.txt` 대비 Go 64B/256B `perf_go_multi_linux_20260522_093649_codex_go_ws_dd_small_shared_ctx_20260522.txt`, 나머지 size `perf_go_multi_linux_20260522_060003_codex_go_ws_multi_current_20260522.txt`. C처럼 단일 poll loop와 pending socket만 `POLLOUT`으로 두는 poll set 의미를 유지했다. ws/wss/tls 64B/256B client socket은 C `perf_multi_client_helpers`처럼 하나의 client context 안에서 만들도록 제한 적용했고, ws 256B는 통과권으로 올랐다. server `RecvPart`를 ws 65536B 이상으로 넓힌 후보 `perf_go_multi_linux_20260522_062510_codex_go_ws_multi_dd_recvpart_large_20260522.txt`는 65536B/131072B/262144B를 더 낮춰 반영하지 않았다. tcp에서 확인한 `RecvPart` 확대, 단일 part send move/restore 후보는 public 계약 보존 또는 large completion을 깨 반영하지 않았다. 64B와 large size는 Go 단순 one-way 최소 기준보다 낮고 public 계약을 바꾸지 않는 추가 후보가 없어 보류한다. auto-HWM 활성과 size별 `MsgUnit(B)` 일치를 확인했다. |
...
| `wss` | `MULTI_DEALER_DEALER` | `보류(39.1%)` | `통과(60.3%)` | `통과(67.9%)` | `보류(19.9%)` | `보류(2.0%)` | `보류(0.2%)` | C `perf_c_multi_linux_20260522_063910_codex_c_wss_multi_for_go_20260522.txt` 대비 Go 64B/256B `perf_go_multi_linux_20260522_093353_codex_go_wss_tls_dd_small_shared_ctx_20260522.txt`, 1024B/65536B/262144B `perf_go_multi_linux_20260522_083056_codex_go_wss_multi_dd_movemessage_selected_20260522.txt`, 131072B `perf_go_multi_linux_20260522_085801_codex_go_wss_tls_multi_dd_131_selected_movemessage_20260522.txt`. C처럼 단일 poll loop와 pending socket `POLLOUT` 의미를 유지했다. Go send builder에 명시적 ownership 이전 단계 `MoveMessage`를 추가했고, `MULTI_DEALER_DEALER` active send는 1024B 이하와 wss/tls 131072B에만 이 경로를 사용한다. `Message`의 실패 시 원본 보존 계약은 유지하며 `go test ./...`로 회귀를 확인했다. wss/tls 64B/256B client socket은 C `perf_multi_client_helpers`처럼 하나의 client context 안에서 만들도록 제한 적용해 256B를 통과권으로 올렸다. large 전체 `MoveMessage` 후보 `perf_go_multi_linux_20260522_085425_codex_go_wss_tls_multi_dd_large_all_movemessage_20260522.txt`와 131072B/262144B 선택 후보 `perf_go_multi_linux_20260522_085600_codex_go_wss_tls_multi_dd_large_selected_movemessage_20260522.txt`는 일부 size만 올리고 다른 size를 낮춰 131072B로 좁혔다. 제한 재측정 `perf_go_multi_linux_20260522_075552_codex_go_wss_multi_dd_recvpart_all_probe_20260522.txt`, `perf_go_multi_linux_20260522_075640_codex_go_wss_multi_dd_shared_ctx_probe_20260522.txt`, `perf_go_multi_linux_20260522_075923_codex_go_wss_multi_dd_client_iothreads1_probe_20260522.txt`는 large를 해결하지 못하거나 악화시켰다. `MoveMessage` 내부를 `zlink_msg_adopt`로 바꾸는 후보 `perf_go_multi_linux_20260522_092410_codex_go_wss_tls_dd_pubsub_movemessage_adopt_20260522.txt`는 수치를 올리지 못해 반영하지 않았다. 64B/65536B/131072B/262144B는 아직 낮아 보류한다. auto-HWM 활성과 size별 `MsgUnit(B)` 일치를 확인했다. |
...
| `wss` | `MULTI_SPOT_SENDSEND` | `보류(29.6%)` | `보류(28.4%)` | `보류(35.7%)` | `통과(53.1%)` | `보류(49.1%)` | `통과(51.9%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go `perf_go_multi_linux_20260524_202842.txt` 기준이다. C와 같은 size별 active slot 제한과 `SubmitNotConnected` 재시도 의미를 유지한다. 서버 echo는 public `Spot.ForwardRouted(...)`를 65536B 이상에 적용해 no-result였던 65536B를 유효 통과값으로 올렸고 262144B 통과권도 유지했다. 131072B는 단독 1회 측정 `perf_go_multi_linux_20260524_195651.txt`에서 50.1%였지만 반복 측정 median과 최신 측정이 50% 아래라 통과로 확정하지 않는다. small size는 기존 builder 경로를 유지한다. 2026-05-24 추가 후보(`ForwardRouted` small 확대, client poller `POLLIN|POLLOUT`, small active slot 32, `PERF_GO_GOMAXPROCS=8`)는 기준을 안정적으로 올리지 못했거나 악화되어 반영하지 않았다(6.6.3). auto-HWM 활성과 size별 SpotNode `MsgUnit(B)` 일치를 확인했다. |
...
| `tls` | `MULTI_DEALER_DEALER` | `보류(34.7%)` | `보류(49.9%)` | `통과(60.2%)` | `보류(4.2%)` | `보류(7.6%)` | `보류(6.2%)` | C `perf_c_multi_linux_20260522_071155_codex_c_tls_multi_for_go_20260522.txt` 대비 Go 64B/256B `perf_go_multi_linux_20260522_093353_codex_go_wss_tls_dd_small_shared_ctx_20260522.txt`, 1024B/65536B/262144B `perf_go_multi_linux_20260522_072100_codex_go_tls_multi_current_20260522.txt`, 131072B `perf_go_multi_linux_20260522_085801_codex_go_wss_tls_multi_dd_131_selected_movemessage_20260522.txt`. C처럼 단일 poll loop와 pending socket `POLLOUT` 의미를 유지했다. wss/tls 64B/256B client socket은 C `perf_multi_client_helpers`처럼 하나의 client context 안에서 만들도록 제한 적용해 256B를 기준선 직전까지 올렸다. 131072B active send는 `MoveMessage`로 C의 consumed native part 의미에 가깝게 맞춰 0.5%에서 7.6%로 올랐지만 여전히 기준보다 낮다. large 전체 `MoveMessage` 후보 `perf_go_multi_linux_20260522_085425_codex_go_wss_tls_multi_dd_large_all_movemessage_20260522.txt`와 131072B/262144B 선택 후보 `perf_go_multi_linux_20260522_085600_codex_go_wss_tls_multi_dd_large_selected_movemessage_20260522.txt`는 일부 size만 올리고 다른 size를 낮춰 131072B로 좁혔다. 제한 재측정 `perf_go_multi_linux_20260522_075512_codex_go_tls_multi_dd_large_recheck_20260522.txt`에서도 large gap이 재현됐다. `MoveMessage` 내부를 `zlink_msg_adopt`로 바꾸는 후보 `perf_go_multi_linux_20260522_092410_codex_go_wss_tls_dd_pubsub_movemessage_adopt_20260522.txt`는 수치를 올리지 못해 반영하지 않았다. wss와 같은 서버 `RecvPart` 전체 확대, IO thread 축소, 단일 part send move/restore 후보가 해결하지 못했거나 public 실패 원본 보존 계약을 깨 반영하지 않았다. 남은 개선은 고성능 consume send path 설계가 필요해 보류한다. auto-HWM 활성과 size별 `MsgUnit(B)` 일치를 확인했다. |
...
| `tls` | `MULTI_SPOT_SENDSEND` | `보류(28.3%)` | `보류(29.0%)` | `보류(31.5%)` | `보류(34.9%)` | `보류(37.7%)` | `보류(42.6%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go `perf_go_multi_linux_20260524_202842.txt` 기준이다. `SubmitNotConnected`는 C처럼 fatal이 아니라 다음 poll loop에서 재시도하는 의미로 처리한다. 서버 echo는 public `Spot.ForwardRouted(...)`를 65536B 이상에 적용해 기존 no-result였던 65536B/262144B를 유효 수치로 바꿨고, 최신 재측정에서도 전 size complete를 유지했다. 다만 fresh C 기준으로도 Go SPOT 최소 기준에는 아직 못 미친다. small size는 기존 builder 경로를 유지한다. 2026-05-24 추가 후보(`ForwardRouted` small 확대, client poller `POLLIN|POLLOUT`, small active slot 32, `PERF_GO_GOMAXPROCS=8`)는 기준을 안정적으로 올리지 못했거나 악화되어 반영하지 않았다(6.6.3). auto-HWM 활성과 size별 SpotNode `MsgUnit(B)` 일치를 확인했다. |
...
- Single `wss SPOT`: 262144B는 public `Bytes(...)` publish 선택 적용 뒤 통과권으로
  올라갔다. 다만 64B/256B는 현재 재측정에서 throughput은 높아도 latency가 커서 보류로
  남긴다.
- Go multi routed echo 보류의 consume-forward 1차 구현은 완료했다.
  ...
  다만 이 경로는 wss 65536B/262144B와 tls large 유효 수치 확보에만 효과가 있었다.
...
- **2026-05-23 근본 원인 규명 + 수정 (strace 프로파일링)**: 위 large many-client 병목의 진짜 원인을 strace로 특정했다.
  ...
  - **노이즈 cell**: `wss MULTI_SPOT 1024B` full-run 14.8%는 같은 조건 단독 재측정으로 노이즈 여부를 확인해야 한다(SPOT은 문서상 noisy cell).
...
| `tcp` | `DEALER_ROUTER` | `통과(85.2%)` | `통과(88.2%)` | `보류(62.9%)` | `보류(13.1%)` | `보류(12.0%)` | `보류(10.8%)` | C/Rust 파일은 위 행과 같다. `262144B`는 별도 재확인 `perf_rust_single_linux_20260522_125712_codex_rust_tcp_single_dealer_router_262_stop_retry_recheck_20260522.txt`에서도 complete다. active send와 stop token은 `DONT_WAIT` retry로 맞췄고, receiver burst drain으로 binary_exit는 해소했다. 1024B 이상은 routed one-way 기준보다 낮아 보류한다. |
| `tcp` | `ROUTER_ROUTER` | `통과(109.4%)` | `통과(103.5%)` | `보류(69.7%)` | `보류(13.2%)` | `보류(11.7%)` | `보류(10.7%)` | 64B/256B/65536B/131072B는 위 Rust full 파일, 1024B/262144B는 보강 `perf_rust_single_linux_20260522_130257_codex_rust_tcp_single_router_router_nonblock_recheck_20260522.txt` 기준이다. routed active send를 `DONT_WAIT` retry로 바꾸고 receiver burst drain을 적용해 제한 재측정 complete를 확보했다. 64B/256B는 C보다 높아 outlier 재검토 대상으로 남기고, 1024B 이상은 routed one-way 기준보다 낮아 보류한다. |
...
| `ws` | `DEALER_ROUTER` | `통과(91.0%)` | `통과(99.5%)` | `통과(94.9%)` | `보류(31.1%)` | `보류(23.0%)` | `보류(18.9%)` | 1024B는 Rust 보강 `perf_rust_single_linux_20260522_131936_codex_rust_ws_single_dealer_router1024_reuse_20260522.txt`와 C 보강 `perf_c_single_linux_20260522_132034_codex_c_ws_single_1024_rust_outlier_recheck_20260522.txt`로 binary_exit 없이 complete를 확보했다. 65536B 이상은 routed one-way 기준보다 낮아 보류한다. |
| `ws` | `ROUTER_ROUTER` | `통과(100.0%)` | `통과(108.8%)` | `통과(94.5%)` | `보류(30.6%)` | `보류(23.4%)` | `보류(21.7%)` | 131072B/262144B C 기준은 보강 `perf_c_single_linux_20260522_131031_codex_c_ws_single_rr_large_for_rust_recheck_20260522.txt`, 1024B C 기준은 보강 `perf_c_single_linux_20260522_132034_codex_c_ws_single_1024_rust_outlier_recheck_20260522.txt`다. 65536B 이상은 routed one-way 기준보다 낮아 보류한다. |
| `ws` | `SPOT` | `통과(149.2%)` | `통과(100.8%)` | `보류(50.8%)` | `통과(188.9%)` | `통과(179.4%)` | `통과(114.4%)` | C 기준은 `perf_c_single_linux_20260522_131048_codex_c_ws_single_spot_for_rust_20260522.txt`, 1024B 보강 `perf_c_single_linux_20260522_132034_codex_c_ws_single_1024_rust_outlier_recheck_20260522.txt`, large/outlier 보강 `perf_c_single_linux_20260522_132112_codex_c_ws_single_spot_outliers_for_rust_recheck_20260522.txt`다. 256B는 Rust 보강 `perf_rust_single_linux_20260522_131946_codex_rust_ws_single_spot256_reuse_20260522.txt`로 complete를 확보했다. 1024B는 SPOT 기준보다 낮고, 64B/65536B/131072B는 120%를 넘어 outlier 재검토 대상으로 남긴다. |
...
| `wss` | `SPOT` | `통과(154.8%)` | `보류(36.7%)` | `보류(63.3%)` | `통과(151.1%)` | `통과(161.2%)` | `통과(281.7%)` | C full 파일과 Rust 보강 `perf_rust_single_linux_20260522_141521_codex_rust_wss_single_spot_tls_path_recheck_20260522.txt` 기준이다. Rust SPOT TLS 설정은 PEM 내용이 아니라 C/C++/Go perf와 같은 인증서 파일 경로를 넘기도록 수정해 binary_exit를 해소했다. 256B/1024B는 SPOT 기준보다 낮고, 64B/65536B/131072B/262144B는 120%를 넘어 outlier 재검토 대상으로 남긴다. |
...
| `tls` | `DEALER_ROUTER` | `통과(85.8%)` | `통과(90.5%)` | `통과(87.2%)` | `보류(56.4%)` | `보류(54.0%)` | `보류(52.4%)` | Rust `perf_rust_single_linux_20260522_142918_codex_rust_tls_single_dealer_router_20260522.txt`, 1024B/131072B 보강 `perf_rust_single_linux_20260522_143047_codex_rust_tls_single_dealer_router_missing_duration1_20260522.txt` 기준이다. 65536B 이상은 routed one-way 기준보다 낮아 보류한다. |
| `tls` | `ROUTER_ROUTER` | `통과(99.4%)` | `통과(102.3%)` | `통과(87.8%)` | `보류(55.0%)` | `보류(55.1%)` | `보류(53.2%)` | Rust `perf_rust_single_linux_20260522_143047_codex_rust_tls_single_router_router_20260522.txt` 기준이다. 262144B runner report `perf_rust_single_linux_20260522_143245_codex_rust_tls_single_router_router262_recheck_20260522.txt`는 timeout이지만 direct binary 5초 측정에서 3592.6 msg/s를 확인했다. 65536B 이상은 routed one-way 기준보다 낮아 보류한다. |
| `tls` | `SPOT` | `통과(152.3%)` | `통과(87.3%)` | `보류(47.3%)` | `통과(168.1%)` | `통과(166.3%)` | `통과(160.5%)` | Rust `perf_rust_single_linux_20260522_143152_codex_rust_tls_single_spot_20260522.txt` 기준이다. 1024B는 SPOT 기준보다 낮고, 64B/65536B/131072B/262144B는 120%를 넘어 outlier 재검토 대상으로 남긴다. |
...
| Transport | Pattern | 64 | 256 | 1024 | 65536 | 131072 | 262144 | 결과 파일 / 메모 |
|-----------|---------|----|-----|------|-------|--------|--------|------------------|
| `tcp` | `MULTI_DEALER_DEALER` | `통과(77.7%)` | `통과(91.9%)` | `통과(89.3%)` | `보류(42.1%)` | `보류(30.9%)` | `보류(26.9%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Rust `perf_rust_multi_linux_20260524_201407.txt` 기준이다. Rust client가 각 socket마다 loop당 1개만 보내던 것을 1024B 이하에서는 C처럼 backpressure까지 연속 송신하고, POLLOUT event slot으로 pending socket만 재개하도록 맞췄다. 64/256/1024B는 통과로 올랐고 large는 여전히 낮아 보류한다. 전 size burst-send 후보 `perf_rust_multi_linux_20260524_201322.txt`는 large를 더 낮춰 small로 좁혔다. |
| `tcp` | `MULTI_DEALER_ROUTER` | `통과(116.6%)` | `통과(105.7%)` | `통과(112.5%)` | `보류(79.3%)` | `보류(57.3%)` | `보류(54.6%)` | C `perf_c_multi_linux_20260522_143606_codex_c_multi_tcp_no_stream_for_rust_20260522.txt`, Rust `perf_rust_multi_linux_20260522_144936_codex_rust_multi_tcp_dealer_router_20260522.txt` 기준이다. large size는 routed echo 기준보다 낮아 보류한다. |
| `tcp` | `MULTI_ROUTER_ROUTER` | `통과(84.7%)` | `통과(85.7%)` | `통과(85.7%)` | `보류(67.3%)` | `보류(53.0%)` | `보류(48.7%)` | **2026-05-24 sleep→poller 수정**: RR client와 server가 idle마다 `thread::sleep(1ms)`로 hot-loop을 throttle해 small이 ~19%에 묶여 있었다(같은 routed-echo인 DEALER_ROUTER는 signal-driven poller를 써서 106%). RR client/server를 DEALER_ROUTER와 동일한 unified poller(`perf_socket_poll(...,-1)`, POLLIN/POLLOUT 토글)로 바꿨다. fresh C baseline `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Rust `perf_rust_multi_linux_20260524_004405.txt`: small 19→**84~86% 통과**, large 65536/131072/262144=67.3/53.0/48.7%(routed large copy 영역, 70% 목표 미만이라 보류). 이전: C `perf_c_multi_linux_20260522_143606_codex_c_multi_tcp_no_stream_for_rust_20260522.txt`, Rust `perf_rust_multi_linux_20260522_145044_codex_rust_multi_tcp_router_router_20260522.txt`. |
| `tcp` | `MULTI_PUBSUB` | `보류(24.5%)` | `보류(20.1%)` | `보류(14.1%)` | `보류(49.5%)` | `통과(98.4%)` | `통과(120.4%)` | C `perf_c_multi_linux_20260522_143606_codex_c_multi_tcp_no_stream_for_rust_20260522.txt`, Rust 64B~131072B `perf_rust_multi_linux_20260522_145345_codex_rust_multi_tcp_pubsub_stop_retry_fix_20260522.txt`, 262144B `perf_rust_multi_linux_20260522_152752_codex_rust_multi_tcp_pubsub262_monitor_ready_20260522.txt` 기준이다. server stop token은 `Ok(false)`를 성공으로 보지 않도록 수정했고, client는 SUB connection-ready를 기다리도록 맞춰 262144B timeout까지 해소했다. 262144B는 C보다 높아 outlier 재검토 대상으로 남긴다. |
| `tcp` | `MULTI_SPOT` | `통과(84.7%)` | `통과(90.5%)` | `통과(85.0%)` | `보류(53.0%)` | `보류(36.5%)` | `보류(42.1%)` | **2026-05-24 multi-worker drain 추가**: copy 제거 위에 추가로, 단일 thread가 100 spot을 순차 drain하던 것을 Go recv worker처럼 scoped thread 4개로 분할(`Spot: Send`, `chunks_mut`로 disjoint 분배)했다. fresh C baseline 대비 small 64/256/1024 = **84.7/90.5/85.0% 통과**(이전 단일 thread 50/54/46%), large 65536/131072/262144 = 53.0/36.5/42.1%(이전 19/15/12%). 원래(36/37/32/11/1.5/1.0%) 대비 전 size 대폭 개선, small은 SPOT 목표(75%) 통과. large는 routed/copy 영역으로 보류. (러너의 symlink 기반 stale-guard 버그도 함께 수정: `readlink -f`로 실제 versioned .so와 비교.) — **2026-05-23 per-message copy 제거**: Rust SPOT client 수신 hot path가 매 메시지 payload를 `message_payload(...).to_vec()`로 전체 복사(262144B면 256KB alloc+memcpy/msg)해 large가 1~1.5%로 무너졌다. timestamp 디코드는 borrowed `&[u8]`로 충분하므로 `.to_vec()`를 제거했다. fresh C baseline `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Rust(빌드 후 공식 runner): 262144 1.0→11.7%, 131072 1.5→14.8%, 65536 11.0→18.7%, small 32~37→45~55%(10x large). Rust SPOT 목표(75%)에는 아직 못 미치며, 잔여 gap은 단일 thread로 100 spot을 drain하는 구조 + per-call cgo 비용이다. 이전 라운드 기록: C `perf_c_multi_linux_20260522_143606_codex_c_multi_tcp_no_stream_for_rust_20260522.txt`, Rust `perf_rust_multi_linux_20260522_145753_codex_rust_multi_tcp_spot_20260522.txt`. client inner drain loop에 active deadline check가 없어 backlog가 계속 들어오는 size에서 runner timeout이 났고, deadline check 추가 뒤 64B/131072B/262144B complete를 확보했다. runner의 fast-exit child 대기도 zombie를 종료로 처리하도록 보강했다. 전 size는 SPOT 기준보다 낮다. |
| `tcp` | `MULTI_SPOT_REQREP` | `보류(78.7%)` | `통과(88.5%)` | `보류(83.4%)` | `보류(72.2%)` | `보류(77.8%)` | `보류(66.3%)` | C `perf_c_multi_linux_20260522_143606_codex_c_multi_tcp_no_stream_for_rust_20260522.txt`, Rust `perf_rust_multi_linux_20260522_150056_codex_rust_multi_tcp_spot_reqrep_20260522.txt` 기준이다. 256B 외에는 기준보다 낮아 보류한다. |
| `tcp` | `MULTI_SPOT_SENDSEND` | `통과(93.3%)` | `통과(94.3%)` | `통과(94.7%)` | `보류(63.9%)` | `통과(75.4%)` | `보류(68.2%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Rust `perf_rust_multi_linux_20260524_200540.txt` 기준이다. client active loop를 C와 같은 size별 active slot 제한(65536B 이상 32, 131072B 이상 8)과 POLLIN poller wait로 맞췄다. 기존 18.1/68.6/85.9/60.1/2.9/6.2%에서 크게 올랐고 64/256/1024/131072B는 통과했다. 65536/262144B는 아직 SPOT 기준보다 낮다. active slot을 16으로 좁힌 후보 `perf_rust_multi_linux_20260524_200834.txt`는 65536/262144B를 더 낮춰 반영하지 않는다. |
| `tcp` | `MULTI_STREAM` | `보류(82.6%)` | `보류(79.8%)` | `보류(79.4%)` | `보류(34.7%)` | `해당 없음` | `해당 없음` | C `perf_c_multi_linux_20260522_150329_codex_c_multi_tcp_stream_clients100_for_rust_20260522.txt`, Rust `perf_rust_multi_linux_20260522_150351_codex_rust_multi_tcp_stream_clients100_20260522.txt` 기준이다. 기본 STREAM clients=10000은 thread/CPU 부담이 커서 먼저 clients=100으로 제한해 같은 조건 비교를 남겼다. 전 size가 기준보다 낮아 보류한다. |
| `ws` | `MULTI_DEALER_DEALER` | `통과(73.1%)` | `통과(94.5%)` | `통과(91.3%)` | `보류(41.8%)` | `보류(34.0%)` | `통과(84.7%)` | 64/256/1024B는 C `perf_c_multi_linux_20260522_150505_codex_c_multi_ws_no_stream_for_rust_20260522.txt` 대비 Rust `perf_rust_multi_linux_20260524_201443.txt` 기준이다. small size는 tcp와 같은 client burst-send/poller pending 수정으로 통과권에 올랐다. large는 기존 Rust `perf_rust_multi_linux_20260522_150956_codex_rust_multi_ws_dealer_dealer_20260522.txt` 기준을 유지하며 262144B 외에는 보류다. |
| `ws` | `MULTI_DEALER_ROUTER` | `통과(114.6%)` | `통과(119.0%)` | `통과(106.1%)` | `보류(58.4%)` | `보류(78.1%)` | `보류(76.0%)` | C 파일은 위 행과 같고, Rust `perf_rust_multi_linux_20260522_151019_codex_rust_multi_ws_dealer_router_20260522.txt` 기준이다. 실행 중 server `nlwp=8`, client `nlwp=7` 수준이었다. large size는 routed echo 기준보다 낮아 보류한다. |
| `ws` | `MULTI_ROUTER_ROUTER` | `통과(88.7%)` | `통과(89.4%)` | `통과(86.4%)` | `보류(55.2%)` | `보류(52.9%)` | `통과(79.4%)` | **2026-05-24 sleep→poller 수정으로 small 19→88% 통과**(fresh baseline 대비 Rust `perf_rust_multi_linux_20260524_*_ws full`, 6.7.3). 65536/131072는 routed large(70% 목표) 미만으로 보류. 이전: Rust `perf_rust_multi_linux_20260522_151114_codex_rust_multi_ws_router_router_20260522.txt`. |
| `ws` | `MULTI_PUBSUB` | `통과(95.5%)` | `통과(92.8%)` | `보류(10.9%)` | `통과(86.4%)` | `통과(103.9%)` | `통과(125.3%)` | C 파일은 위 행과 같고, Rust `perf_rust_multi_linux_20260522_152055_codex_rust_multi_ws_pubsub_ready_fix_full_20260522.txt` 기준이다. PUBSUB client가 `CLIENT_READY`를 너무 일찍 출력하던 문제는 `SocketMonitor` connection-ready 대기를 추가해 131072B/262144B timeout을 해소했다. 1024B는 낮아 보류하고, 262144B는 C보다 높아 outlier 재검토 대상으로 남긴다. |
| `ws` | `MULTI_SPOT` | `보류(53.7%)` | `보류(52.0%)` | `보류(49.3%)` | `보류(64.2%)` | `보류(32.4%)` | `보류(33.7%)` | C `perf_c_multi_linux_20260522_150505_codex_c_multi_ws_no_stream_for_rust_20260522.txt` 대비 Rust `perf_rust_multi_linux_20260524_201915.txt` 기준이다. tcp에 적용한 SPOT client 복사 제거 + 4-worker drain을 같은 transport-agnostic 경로로 재측정해 기존 39.6/38.7/30.9/14.3/10.2/1.4%보다 전반적으로 올랐지만, 아직 SPOT 기준에는 못 미친다. |
...
| `wss` | `MULTI_DEALER_DEALER` | `통과(77.9%)` | `통과(94.0%)` | `통과(98.8%)` | `보류(83.0%)` | `보류(71.8%)` | `보류(79.9%)` | 64/256/1024B는 fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Rust `perf_rust_multi_linux_20260524_201443.txt` 기준이다. small size는 client burst-send/poller pending 수정으로 통과권에 올랐다. large는 기존 Rust `perf_rust_multi_linux_20260522_153314_codex_rust_multi_wss_dealer_dealer_20260522.txt` 기준을 유지하며 보류다. |
| `wss` | `MULTI_DEALER_ROUTER` | `통과(102.2%)` | `통과(108.9%)` | `통과(107.5%)` | `통과(89.1%)` | `통과(87.9%)` | `통과(93.1%)` | C 파일은 위 행과 같고, Rust `perf_rust_multi_linux_20260522_153348_codex_rust_multi_wss_dealer_router_20260522.txt` 기준이다. 전 size가 기준선을 넘었다. |
| `wss` | `MULTI_ROUTER_ROUTER` | `통과(93.9%)` | `통과(91.9%)` | `통과(91.7%)` | `통과(79.3%)` | `통과(80.4%)` | `통과(79.3%)` | **2026-05-24 sleep→poller 수정으로 전 size 통과**(small 19→92%). fresh C baseline 대비 Rust scoped `perf_rust_multi_linux_20260524_*_rr_wss`(6.7.3 참조). 이전: Rust `perf_rust_multi_linux_20260522_153441_codex_rust_multi_wss_router_router_20260522.txt`(small 보류). |
| `wss` | `MULTI_PUBSUB` | `통과(85.8%)` | `보류(9.2%)` | `보류(6.0%)` | `보류(77.7%)` | `보류(79.5%)` | `통과(111.4%)` | C 파일은 위 행과 같고, Rust 64B/256B/1024B/131072B/262144B `perf_rust_multi_linux_20260522_153511_codex_rust_multi_wss_pubsub_ready_fix_20260522.txt`, 65536B `perf_rust_multi_linux_20260522_154002_codex_rust_multi_wss_pubsub65536_deadline_fallback_20260522.txt` 기준이다. 65536B stop token timeout은 client stop 대기 fallback으로 해소했다. 256B/1024B는 낮고 262144B는 C보다 높아 재측정 대상이다. |
| `wss` | `MULTI_SPOT` | `보류(58.4%)` | `보류(49.5%)` | `보류(58.5%)` | `통과(131.8%)` | `통과(174.0%)` | `보류(64.9%)` | C `perf_c_multi_linux_20260522_152936_codex_c_multi_wss_no_stream_for_rust_20260522.txt` 대비 Rust `perf_rust_multi_linux_20260524_201915.txt` 기준이다. SPOT client 복사 제거 + 4-worker drain 재측정으로 65536B/131072B는 통과권에 올랐다. small과 262144B는 아직 낮고, large 통과 셀은 C 기준 변동이 큰 암호화 SPOT outlier로 후속 재확인 대상이다. |
| `wss` | `MULTI_SPOT_REQREP` | `보류(78.8%)` | `보류(81.5%)` | `보류(73.7%)` | `통과(87.0%)` | `통과(93.9%)` | `통과(89.6%)` | C 파일은 위 행과 같고, Rust `perf_rust_multi_linux_20260522_154323_codex_rust_multi_wss_spot_reqrep_tls_path_20260522.txt` 기준이다. large size는 기준선을 넘었고 small size는 낮아 보류한다. |
| `wss` | `MULTI_SPOT_SENDSEND` | `보류(71.4%)` | `통과(83.2%)` | `통과(100.4%)` | `통과(94.2%)` | `통과(90.6%)` | `통과(94.3%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Rust `perf_rust_multi_linux_20260524_200645.txt` 기준이다. client active-slot + poller wait 수정으로 기존 21.8/27.0/9.2/68.4/68.0/81.6%에서 크게 올랐다. 64B만 SPOT 기준보다 약간 낮아 보류하고 나머지는 통과했다. |
...
| `tls` | `MULTI_DEALER_DEALER` | `통과(78.7%)` | `통과(95.6%)` | `통과(87.7%)` | `보류(47.8%)` | `보류(55.8%)` | `보류(49.4%)` | 64/256/1024B는 fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Rust `perf_rust_multi_linux_20260524_201443.txt` 기준이다. small size는 client burst-send/poller pending 수정으로 통과권에 올랐다. large는 기존 Rust `perf_rust_multi_linux_20260522_155213_codex_rust_multi_tls_dealer_dealer_20260522.txt` 기준을 유지하며 보류다. |
| `tls` | `MULTI_DEALER_ROUTER` | `통과(108.8%)` | `통과(109.6%)` | `통과(108.7%)` | `통과(97.6%)` | `통과(105.9%)` | `통과(110.8%)` | C 파일은 위 행과 같고, Rust `perf_rust_multi_linux_20260522_155235_codex_rust_multi_tls_dealer_router_20260522.txt` 기준이다. 실행 중 server `nlwp=7` 수준이었고 전 size가 기준선을 넘었다. |
| `tls` | `MULTI_ROUTER_ROUTER` | `통과(90.0%)` | `통과(88.7%)` | `통과(87.9%)` | `통과(79.0%)` | `통과(85.4%)` | `통과(88.4%)` | **2026-05-24 sleep→poller 수정으로 전 size 통과**(small 19→90%). fresh C baseline 대비 Rust scoped `perf_rust_multi_linux_20260524_*_rr_tls`(6.7.3 참조). 이전: Rust `perf_rust_multi_linux_20260522_155322_codex_rust_multi_tls_router_router_20260522.txt`(small 보류). |
| `tls` | `MULTI_PUBSUB` | `통과(89.6%)` | `보류(82.0%)` | `보류(5.8%)` | `통과(110.0%)` | `통과(101.8%)` | `통과(91.3%)` | C 파일은 위 행과 같고, Rust `perf_rust_multi_linux_20260522_155342_codex_rust_multi_tls_pubsub_20260522.txt` 기준이다. 실행 중 client `nlwp=7` 수준이었다. 1024B는 낮아 보류하고, large size는 기준선을 넘었다. |
| `tls` | `MULTI_SPOT` | `보류(49.5%)` | `보류(51.7%)` | `보류(57.8%)` | `통과(151.4%)` | `통과(109.3%)` | `보류(72.1%)` | C `perf_c_multi_linux_20260522_154822_codex_c_multi_tls_no_stream_for_rust_20260522.txt` 대비 Rust `perf_rust_multi_linux_20260524_201915.txt` 기준이다. SPOT client 복사 제거 + 4-worker drain 재측정으로 65536B/131072B는 통과권에 올랐고 262144B도 1.9%→72.1%까지 올랐지만, 기준선 바로 아래라 보류한다. |
| `tls` | `MULTI_SPOT_REQREP` | `보류(81.7%)` | `보류(81.0%)` | `보류(84.5%)` | `통과(85.1%)` | `통과(88.7%)` | `통과(88.5%)` | C 파일은 위 행과 같고, Rust `perf_rust_multi_linux_20260522_155531_codex_rust_multi_tls_spot_reqrep_tls_path_20260522.txt` 기준이다. 실행 중 server `nlwp=7` 수준이었고, large size는 기준선을 넘었다. |
...
- **SPOT client per-message 복사 제거 + multi-worker drain**: (1) `MULTI_SPOT` client 수신 hot path가 매 메시지 payload를 `message_payload(...).to_vec()`로 전체 복사했다(262144B면 256KB/msg) → borrowed `&[u8]` 디코드로 `.to_vec()` 제거(large 10x). (2) 그 위에, 단일 thread가 100 spot을 순차 drain하던 것을 Go recv worker처럼 scoped thread 4개로 분할했다(`Spot: Send`, `chunks_mut` disjoint, `LatencyStats::merge`로 합산). 최종 tcp 결과(fresh C baseline 대비): small 64/256/1024 = **84.7/90.5/85.0% 통과**(원래 36/37/32%), large 65536/131072/262144 = 53.0/36.5/42.1%(원래 11/1.5/1.0%). small은 SPOT 목표 75% 통과, large는 routed/copy 영역 보류. ws/wss/tls 재측정(`perf_rust_multi_linux_20260524_201915.txt`)에서는 ws 전 size가 49.3~64.2%로 개선됐지만 보류, wss 65536/131072B와 tls 65536/131072B는 통과권으로 올랐다. `PERF_MULTI_SPOT_RECV_WORKERS=8` 후보는 small 처리량을 낮췄고(`perf_rust_multi_linux_20260524_202057.txt`), server publish를 C처럼 backpressure 때 blocking retry하는 후보도 전체 size smoke에서 `ws 64B`를 14.1%로 낮춰 기각했다(`perf_rust_multi_linux_20260524_202306.txt`). 결과 `perf_rust_multi_linux_20260524_*` SPOT tcp/ws/wss/tls. (러너 stale-guard symlink 버그도 `readlink -f`로 수정.)
- **ROUTER_ROUTER sleep→poller**: RR client와 server가 idle마다 `thread::sleep(1ms)`로 hot-loop을 throttle해 small이 ~19%에 묶였다(같은 routed-echo인 `MULTI_DEALER_ROUTER`는 unified poller라 106%). RR client/server를 DEALER_ROUTER와 동일한 signal-driven poller(POLLIN/POLLOUT 토글, `wait(...,-1)`)로 바꿔 tcp small 19→**84~86% 통과**. large 65536/131072/262144=67.3/53.0/48.7%(routed large copy 영역). 결과 `perf_rust_multi_linux_20260524_004405.txt`.
- **MULTI_DEALER_DEALER small burst-send**: Rust client가 각 socket마다 active loop 1회에 메시지 1개만 보내고 POLLOUT event slot도 모두 0으로 등록해 C의 "blocked까지 연속 송신 후 pending socket만 재개" 의미와 달랐다. 1024B 이하에서만 C와 같은 burst-send/pending POLLOUT 구조를 적용했다. tcp 64/256/1024B는 77.7/91.9/89.3%(`perf_rust_multi_linux_20260524_201407.txt`), ws/wss/tls small도 전부 통과권(`perf_rust_multi_linux_20260524_201443.txt`)으로 올랐다. 전 size에 burst-send를 적용한 후보 `perf_rust_multi_linux_20260524_201322.txt`는 large를 더 낮춰 반영하지 않는다.
- **MULTI_SPOT_SENDSEND client active-slot + poller wait**: Rust sendsend client는 C와 달리 large size에서도 100개 spot을 모두 active로 돌리고, reply readiness를 poller로 기다리지 않고 progress가 없을 때 `thread::sleep(1ms)`를 썼다. C와 같은 active slot 제한(65536B 이상 32, 131072B 이상 8), POLLIN poller wait, active window 뒤 pending reply drain을 적용했다. 공식 runner complete 결과: tcp 64/256/1024/65536/131072/262144 = 93.3/94.3/94.7/63.9/75.4/68.2%(`perf_rust_multi_linux_20260524_200540.txt`), ws 전 size 81.6~101.3% 통과(`perf_rust_multi_linux_20260524_200859.txt`), wss 64B만 71.4% 보류이고 나머지 통과, tls 전 size 82.5~89.6% 통과(`perf_rust_multi_linux_20260524_200645.txt`). active slot을 16으로 좁힌 후보는 `perf_rust_multi_linux_20260524_200834.txt`에서 65536/262144B가 낮아져 반영하지 않는다.
...
- **2026-05-24 tcp full 재측정 요약**: DEALER_ROUTER 46~107%(전 size 통과), ROUTER_ROUTER small 통과/large 보류, SPOT_REQREP 62~95%(전 size 통과), STREAM 90~101% 통과, DD small 통과/large 보류, SPOT small 통과권/large 보류(copy 영역), SPOT_SENDSEND 대폭 개선/PUBSUB 일부만. PUBSUB small이 fresh baseline 대비 ~3~5%인 것은 회귀가 아니라(이전 24%는 옛 baseline 기준) 단일 thread로 100 subscriber를 drain하는 per-call 비용 + C PUBSUB(2.6M msg/s)이 매우 높기 때문이다.
- **남은 보류(Rust)**: DEALER_DEALER large, routed large 일부, SPOT large(copy 영역), SPOT_SENDSEND 일부 tcp/wss 경계값, PUBSUB small(단일 thread drain).
...
| `ws` | `SPOT` | `보류(11.8%)` | `보류(10.4%)` | `보류(11.1%)` | `통과(54.8%)` | `통과(64.6%)` | `통과(49.0%)` | C/Python 파일은 위 PAIR 행과 같다. small size는 기준보다 낮고, large size는 SPOT 최소 기준을 넘었다. |
| `wss` | `PAIR` | `보류(4.7%)` | `보류(4.4%)` | `보류(14.5%)` | `통과(89.4%)` | `통과(96.4%)` | `통과(105.1%)` | C `perf_c_single_linux_20260522_161010_codex_c_single_wss_for_python_20260522.txt`, Python `perf_python_single_linux_20260522_161851_codex_python_single_wss_spot_dontwait_20260522.txt` 기준이다. 실행 중 Python non-SPOT case는 `nlwp=5`, SPOT case는 `nlwp=9` 수준이었다. small size는 기준보다 낮고, large size는 기준선을 넘었다. |
| `wss` | `PUBSUB` | `보류(4.6%)` | `보류(4.6%)` | `보류(12.9%)` | `통과(78.4%)` | `통과(86.7%)` | `통과(86.1%)` | C/Python 파일은 위 PAIR 행과 같다. small size는 기준보다 낮고, large size는 기준선을 넘었다. |
| `wss` | `DEALER_DEALER` | `보류(4.6%)` | `보류(4.2%)` | `보류(15.0%)` | `통과(87.1%)` | `통과(101.9%)` | `통과(97.8%)` | C/Python 파일은 위 PAIR 행과 같다. small size는 기준보다 낮고, large size는 기준선을 넘었다. |
...
| `wss` | `SPOT` | `보류(12.8%)` | `보류(10.7%)` | `보류(16.0%)` | `통과(97.6%)` | `통과(89.8%)` | `통과(67.4%)` | C/Python 파일은 위 PAIR 행과 같다. 최초 Python SPOT 65536B는 blocking publish와 무기한 stop wait 때문에 장기 실행됐고, SPOT publish를 `DONT_WAIT`로 맞추고 stop wait를 bounded fallback으로 바꾼 뒤 complete를 확보했다. small size는 기준보다 낮고, large size는 기준선을 넘었다. |
| `tls` | `PAIR` | `보류(4.7%)` | `보류(4.1%)` | `보류(9.7%)` | `통과(86.6%)` | `통과(79.4%)` | `통과(85.2%)` | C `perf_c_single_linux_20260522_161932_codex_c_single_tls_for_python_20260522.txt`, Python `perf_python_single_linux_20260522_162118_codex_python_single_tls_spot_dontwait_20260522.txt` 기준이다. 실행 후 잔류 perf 프로세스는 없었다. small size는 기준보다 낮고, large size는 기준선을 넘었다. |
| `tls` | `PUBSUB` | `보류(3.9%)` | `보류(4.0%)` | `보류(7.6%)` | `통과(65.0%)` | `통과(69.8%)` | `통과(80.7%)` | C/Python 파일은 위 PAIR 행과 같다. small size는 기준보다 낮고, large size는 기준선을 넘었다. |
| `tls` | `DEALER_DEALER` | `보류(4.1%)` | `보류(4.2%)` | `보류(10.5%)` | `통과(73.1%)` | `통과(81.7%)` | `통과(89.4%)` | C/Python 파일은 위 PAIR 행과 같다. small size는 기준보다 낮고, large size는 기준선을 넘었다. |
...
| `tls` | `SPOT` | `보류(12.9%)` | `보류(11.0%)` | `보류(12.3%)` | `통과(91.5%)` | `통과(94.0%)` | `통과(84.3%)` | C/Python 파일은 위 PAIR 행과 같다. SPOT publish DONTWAIT와 bounded stop wait 적용 뒤 전체 size가 complete 됐다. small size는 기준보다 낮고, large size는 기준선을 넘었다. |

#### 6.8.2 Multi suite

| Transport | Pattern | 64 | 256 | 1024 | 65536 | 131072 | 262144 | 결과 파일 / 메모 |
|-----------|---------|----|-----|------|-------|--------|--------|------------------|
| `tcp` | `MULTI_DEALER_DEALER` | `보류(6.0%)` | `보류(9.0%)` | `보류(12.3%)` | `통과(52.2%)` | `통과(41.1%)` | `보류(22.5%)` | C `perf_c_multi_linux_20260522_162958_codex_c_multi_tcp_for_python_duration1_20260522.txt`, Python `perf_python_multi_linux_20260524_191648.txt` 기준이다. 실행 중 non-SPOT server/client는 각 `nlwp=8`/`nlwp=7` 수준이었고 `clients=100`이 thread 100개로 확장되지는 않았다. server의 metric header decode를 `to_bytes_list()` 대신 `ReceivedMessage.data` view로 바꾼 뒤 large 일부는 올랐지만, small size와 262144B는 여전히 낮다. |
| `tcp` | `MULTI_DEALER_ROUTER` | `보류(12.9%)` | `보류(14.3%)` | `보류(13.3%)` | `통과(46.5%)` | `통과(39.1%)` | `통과(38.9%)` | C 파일은 위 행과 같고, 64/256/1024/65536B는 Python `perf_python_multi_linux_20260524_194812.txt` 기준이다. 131072/262144B는 기존 Python 파일 기준이다. reply header decode를 `to_bytes_list()` 대신 `ReceivedMessage.data` view로 바꾼 뒤 65536B가 40.5%→46.5%로 올랐다. small size는 여전히 낮다. |
| `tcp` | `MULTI_ROUTER_ROUTER` | `보류(10.0%)` | `보류(10.3%)` | `보류(10.4%)` | `통과(35.5%)` | `통과(44.0%)` | `통과(44.6%)` | C 파일은 위 MULTI_DEALER_DEALER 행과 같고, Python은 `perf_python_multi_linux_20260524_191823.txt` 기준이다. client reply header decode를 `to_bytes_list()` 대신 `ReceivedMessage.data` view로 바꾼 뒤 65536B 이상은 기준선을 넘었지만, small size는 여전히 낮다. |
| `tcp` | `MULTI_PUBSUB` | `보류(5.4%)` | `보류(6.6%)` | `보류(12.1%)` | `통과(53.8%)` | `통과(90.9%)` | `통과(95.0%)` | C 파일은 위 MULTI_DEALER_DEALER 행과 같고, Python은 `perf_python_multi_linux_20260524_191738.txt` 기준이다. subscriber의 metric header decode를 `to_bytes_list()` 대신 `TopicMessage.data` view로 바꾼 뒤 large size는 더 올라갔지만, small size는 기준보다 낮다. |
...
- **(A) 바인딩 hot-path 비효율 — 최적화 대상(보류, 영구 아님)**:
  ...
  - Go/Rust/Python `MULTI_DEALER_DEALER`/`MULTI_SPOT`/`MULTI_SPOT_SENDSEND`/`MULTI_SPOT_REQREP` large: 같은 core에서
