...
The runner uses `GAMEQUEST_REDIS_ENDPOINT` when it is set. If it is not set, the
...
supplies a unique `GAMEQUEST_REDIS_KEY_PREFIX` for each execution so parallel
