...
void test_send_part_more_rejects_different_send_helper_on_same_handle ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_bind (router, "inproc://helper-interleave-dealer"));
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_connect (dealer, "inproc://helper-interleave-dealer"));
    msleep (SETTLE_TIME);
    ...
    TEST_ASSERT_EQUAL_INT (
      ZLINK_SUBMIT_OK,
      zlink_send_part (dealer, &first, static_cast<zlink_send_flags_t> (0),
                       ZLINK_PART_MORE));
    ...
    const zlink_submit_result_t wrong_rc =
      zlink_dealer_request_part (
        dealer, &wrong_family, static_cast<zlink_send_flags_t> (0),
    ...
    TEST_ASSERT_EQUAL_INT (
      ZLINK_SUBMIT_OK,
      zlink_send_part (dealer, &last, static_cast<zlink_send_flags_t> (0),
                       ZLINK_PART_FINAL));
...
void test_send_part_rid_more_rejects_interleaved_target_change ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_bind (router, "inproc://helper-interleave-rid"));
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_connect (dealer1, "inproc://helper-interleave-rid"));
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_connect (dealer2, "inproc://helper-interleave-rid"));
    msleep (SETTLE_TIME);
    ...
    TEST_ASSERT_EQUAL_INT (
      ZLINK_SUBMIT_OK,
      zlink_send_part_rid (router, &rid1, &first,
    ...
    const zlink_submit_result_t wrong_rc =
      zlink_send_part_rid (router, &rid2, &wrong_target,
                           static_cast<zlink_send_flags_t> (0),
                           ZLINK_PART_MORE);
    TEST_ASSERT_EQUAL_INT (ZLINK_SUBMIT_INVALID_ARGUMENT, wrong_rc);
    TEST_ASSERT_EQUAL_INT (EINVAL, zlink_errno ());
    ...
    TEST_ASSERT_EQUAL_INT (
      ZLINK_SUBMIT_OK,
      zlink_send_part_rid (router, &rid1, &last,
                           static_cast<zlink_send_flags_t> (0),
                           ZLINK_PART_FINAL));
