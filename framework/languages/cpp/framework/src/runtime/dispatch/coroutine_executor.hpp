...
boost::asio::awaitable<result_t<TResult>>
await_task_result (task_t<TResult> task)
{
  auto executor = co_await boost::asio::this_coro::executor;
  auto token = boost::asio::use_awaitable;
  co_return co_await boost::asio::async_initiate<
    decltype (token),
    void (result_t<TResult>)> (
    [executor, task = std::move (task)](auto handler) mutable {
      ...
      detail::observe_task_completion (
        task,
        [executor, shared_handler](const result_t<TResult> &result)
          mutable {
          auto result_copy = result;
          boost::asio::dispatch (
            executor,
            [shared_handler, result = std::move (result_copy)]()
              mutable {
...
public:
  ...
  task_t<TResult> submit (
    std::function<boost::asio::awaitable<result_t<TResult>> ()> work)
  {
    ...
    boost::asio::co_spawn (
      ...
      [work = std::move (work), completion = std::move (completion)]() mutable
        ...
        result_t<TResult> result =
          result_t<TResult>::failure (framework_error_kind_t::request_failed,
                                      "coroutine handler failed");
        try {
          result = co_await work ();
        } catch (const framework_exception_t &error) {
          result = result_t<TResult>::failure (
