...
class auto_request_handler_t
{
  public:
    ...
    using request_type = echo_auto_t;
    using reply_type = echo_auto_reply_t;

    explicit auto_request_handler_t (scenario_state_t &state) : _state (state) {}

    echo_auto_reply_t handle (const echo_auto_t &request)
    {
...
class auto_send_handler_t
{
  public:
    using dependency_types = zlink::framework::dependency_list_t<scenario_state_t>;
    using message_type = echo_auto_send_t;

    explicit auto_send_handler_t (scenario_state_t &state) : _state (state) {}

    void handle (const echo_auto_send_t &message)
    {
...
{
  public:
    echo_manual_reply_t handle (const echo_manual_t &request,
