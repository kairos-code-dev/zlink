...
export async function runMonA1(options: ClientOptions): Promise<void> {
  const request: ProfileRequest = { value: 'monitor', marker: 'mon-a1-request' };
  const reply = await postJson<ProfileReply>(options.triggerUrl, '/profile/request/disconnect', request);
