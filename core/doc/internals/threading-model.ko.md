...
data-plane 스레드 루프는 세 큐의 signaler FD를 포함한 poller와 함께 돈다.
큐가 비어 있다가 메시지가 들어오면 곧바로 스레드가 깨어난다. 유휴 tick은 100 ms다.
...
해당 Spot에 미읽은 이벤트가 `_dirty`에 있으면 `_ready`에 다시 enqueue한다.
...
data-plane 스레드가 콜백을 직접 호출하지 않는 이유는 다음과 같다:
