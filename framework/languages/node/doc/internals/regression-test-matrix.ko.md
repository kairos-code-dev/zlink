...
[Node.js 묶음](../README.ko.md) | [Behavior Matrix](./behavior-matrix.ko.md) | [Lifecycle](./lifecycle-and-failure-semantics.ko.md) | [use case validation](../../../../doc/spec/usecase-validation.ko.md)
...
| handler group mapping | `unit` | `zlinkHandlerGroup(...)` provider 등록만으로는 전역 dispatch 대상이 되지 않고, channel 의 `handlerGroups: ['...']`로 매핑한 그룹의 handler만 해당 채널에서 dispatch된다 |
...
| 같은 그룹을 여러 채널에 매핑 | `integration-single-process` | 같은 `zlinkHandlerGroup('api', ...)` provider group 을 두 채널에 `handlerGroups`로 노출해도 채널마다 dispatch namespace가 독립이다 |
...
| Spot publisher with publisher capability | `unit` | Spot publisher capability 가 있으면 `ZLinkSpotPublisherClient` 가 DI 에 등록된다 |
...
| channel client missing at call time | `unit` | `ZLinkChannelClient` 호출 시 channel client capability 가 없으면 `ZLinkConfigurationError` |
