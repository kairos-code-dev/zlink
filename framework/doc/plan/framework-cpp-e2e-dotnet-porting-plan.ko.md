...
| `DeliveryDispatch` | customer/courier role split 구현, actor-bound session partial | 기존 `DiscoveryRegistryHa` harness 대신 `.NET DeliveryDispatch` 샘플 기준 C++ E2E가 있다. registry, dispatch API, dispatch center, customer gateway, courier session, courier gateway, courier actor node 1/2, tracking, probe, client target을 실행하고, customer stream 1개와 courier stream 2개로 `SubscribeDelivery`, `BindCourierSessionReq`, `BindCourierReq`, `EnsureCourierActorReq`, `OfferDeliveryNotify`, `CourierDecisionMsg` 흐름을 검증한다. `timeout 420s framework/languages/cpp/e2e/DeliveryDispatch/run_e2e.sh` 최신 통과 증거가 있으며 successful delivery, reassignment, server evidence self-check, customer/courier gateway/session/actor-node message-flow evidence가 통과한다. 남은 차이는 `.NET`의 actor-bound session 배치와 courier actor entry spot 구조다. |
