...
fun ClientScenarioContext.runGrayFaultScenario() {
    ...
    while (index < 80 && successes < 10) {
        try {
            val reply = client.requestToChannel(Contracts.CHANNEL, Contracts.WorkRequest("b6-gray-$index"))
    ...
    val followUp = client.requestToChannel(Contracts.CHANNEL, Contracts.WorkRequest("b6-follow-up"))
        .timeout(Duration.ofSeconds(3))
        .await(Contracts.WorkReply::class.java)
