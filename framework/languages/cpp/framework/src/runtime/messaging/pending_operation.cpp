...
pending_operation_t
pending_operation_t::make_completed ()
{
...
bool
pending_operation_t::valid () const noexcept
{
  return _state != nullptr;
}

bool
pending_operation_t::completed () const noexcept
{
  return _state != nullptr && _state->completed.load ();
}

bool
pending_operation_t::cancelled () const noexcept
{
  return _state != nullptr && _state->cancelled.load ();
}

bool
pending_operation_t::cancel () noexcept
{
...
{
  return pending_operation_t (
    std::make_shared<pending_operation_state_t> ());
}

bool
complete_pending_operation (pending_operation_t &operation) noexcept
{
  return operation._state != nullptr && operation._state->try_complete ();
}

bool
fail_pending_operation (pending_operation_t &operation,
                        std::exception_ptr error) noexcept
{
  return operation._state != nullptr &&
         operation._state->try_fail (std::move (error));
}

bool
same_pending_operation (const pending_operation_t &left,
