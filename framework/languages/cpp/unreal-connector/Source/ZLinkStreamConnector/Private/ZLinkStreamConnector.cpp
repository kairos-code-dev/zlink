...
bool has_flag (std::uint8_t flags, frame_flags_t flag)
{
    return (flags & static_cast<std::uint8_t> (flag)) != 0;
}

void append_lz4_length (std::vector<std::uint8_t> &bytes, std::size_t value)
{
    while (value >= 255) {
        bytes.push_back (255);
        value -= 255;
...
std::vector<std::uint8_t> encode_lz4_payload (const std::vector<std::uint8_t> &payload)
{
...
decode_lz4_payload (const std::vector<std::uint8_t> &payload)
{
...
#ifdef ZLINK_UNREAL_STREAM_CONNECTOR_WITH_LZ4
    if (original_size > static_cast<std::uint32_t> (std::numeric_limits<int>::max ())) {
        return std::nullopt;
    }
    ...
    while (offset < payload.size ()) {
        ...
        output.insert (output.end (), payload.begin () + static_cast<std::ptrdiff_t> (offset),
                       payload.begin () + static_cast<std::ptrdiff_t> (offset + literal_size));
        offset += literal_size;
        if (offset == payload.size ()) {
            break;
        }
...
bool metadata_empty (const TMap<FString, FString> &metadata)
{
#if __has_include("CoreMinimal.h")
...
std::size_t metadata_count (const TMap<FString, FString> &metadata)
{
#if __has_include("CoreMinimal.h")
...
std::vector<std::uint8_t> encode_metadata (const TMap<FString, FString> &metadata)
{
...
TMap<FString, FString> decode_metadata (const std::vector<std::uint8_t> &bytes)
{
    TMap<FString, FString> metadata;
    if (bytes.empty ()) {
        return metadata;
...
std::vector<std::uint8_t> encode_frame (frame_kind_t kind,
                                        std::optional<std::uint64_t> request_seq,
                                        ...
                                        bool compressed)
{
    ...
    if (request_seq) {
        flags_value |= static_cast<std::uint8_t> (frame_flags_t::has_request_seq);
    }
    if (!metadata_bytes.empty ()) {
        flags_value |= static_cast<std::uint8_t> (frame_flags_t::has_metadata);
    }
    if (compressed) {
        flags_value |= static_cast<std::uint8_t> (frame_flags_t::payload_compressed);
    ...
    if (!metadata_bytes.empty ()) {
        ...
        header.insert (header.end (), metadata_bytes.begin (), metadata_bytes.end ());
    }
...
std::optional<decoded_frame_t> try_decode_frame (std::vector<std::uint8_t> &buffer)
{
    if (buffer.size () < 6) {
        return std::nullopt;
    }
    ...
    const auto frame_size = 6 + static_cast<std::size_t> (header_size) + payload_size;
    ...
    decoded.payload.assign (buffer.begin () + static_cast<std::ptrdiff_t> (payload_offset),
...
{
  public:
    void SetStateChangedCallback (std::function<void (EZLinkStreamConnectionState)> callback)
    {
        StateChanged = std::move (callback);
    }

    void SetPacketReceivedCallback (std::function<void (FZLinkStreamPacket)> callback)
    {
        PacketReceived = std::move (callback);
    }

    void SetRequestCompletedCallback (std::function<void (FZLinkStreamPacket)> callback)
    {
...
#if __has_include("CoreMinimal.h")
        SendFrame (encode_frame (frame_kind_t::send, std::nullopt, to_utf8 (PacketName.ToString ()),
    ...
    bool IsConnected () const { return LastState == EZLinkStreamConnectionState::Connected; }
    ...
    EZLinkStreamConnectionState GetLastState () const { return LastState; }
  ...
  private:
...
#if __has_include("CoreMinimal.h")
    static bool ParseTcpEndpoint (const FString &Endpoint, FString &Host, int32 &Port)
    {
    ...
    void SendFrame (const std::vector<std::uint8_t> &frame)
    {
        ...
        int32 BytesSent = 0;
        Socket->Send (frame.data (), static_cast<int32> (frame.size ()), BytesSent);
    }
    ...
    void DispatchFrame (decoded_frame_t frame)
    {
        ...
        } else if (frame.kind == frame_kind_t::response && frame.request_seq
    ...
    static FZLinkStreamPacket MakePacket (decoded_frame_t frame)
    {
        ...
        Packet.Payload.Append (frame.payload.data (), static_cast<int32> (frame.payload.size ()));
        return Packet;
    }
#endif
...
UZLinkStreamConnector::UZLinkStreamConnector () :
    _runtime (std::make_unique<UE::ZLinkStreamConnector::Private::FZLinkStreamConnectorRuntime> ())
{
    _runtime->SetStateChangedCallback (
      [this] (EZLinkStreamConnectionState state) { OnConnectionStateChanged.Broadcast (state); });
    _runtime->SetPacketReceivedCallback ([this] (FZLinkStreamPacket packet) {
        OnPacketReceived.Broadcast (packet);
        OnPacketReceivedNative.Broadcast (packet);
    });
    _runtime->SetRequestCompletedCallback ([this] (FZLinkStreamPacket packet) {
...
void UZLinkStreamConnector::Tick (float DeltaSeconds)
{
    (void) DeltaSeconds;
    _runtime->Dispatch ();
}
