...
const processCaptureStates = new WeakMap<ManagedProcess, ProcessCaptureState>();

function processCaptureState(processRef: ManagedProcess): ProcessCaptureState {
...
async function spawnMultiPair(serverScript, clientScript, args) {
  ...
  if (
    ...
    const controlConnectedLine = await waitForPrefix(
      server,
      'CONTROL_CONNECTED,',
      serverScript,
      resolveClientReadyTimeoutMs(args)
    );
