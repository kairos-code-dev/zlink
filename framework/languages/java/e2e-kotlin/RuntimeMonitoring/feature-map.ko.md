...
`logs/20260702-064114-7626` full runner에서 `Client` module binary로 통과했다. Registry, Service,
FilteredService, ThrowingService, Trigger role은 각각 `Server/Registry`, `Server/Service`,
...
monitoring evidence의 `PEER_ADMISSION_CHANGED`, Service admin evidence, registry `TOPOLOGY_CHANGED`
evidence를 함께 확인한다.
...
- `MON-A4`: service host의 public runtime socket weight를 0으로 바꿨다가 복구하고, trigger host의 client socket source에서 `PEER_ADMISSION_CHANGED`를 관찰한다. Service admin evidence와 registry topology evidence도 함께 확인한다.
