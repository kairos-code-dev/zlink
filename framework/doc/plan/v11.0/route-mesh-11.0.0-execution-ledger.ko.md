...
- C++은 exact `spot_manager_t` call builder·`spot_ref_t` surface, Location Reserve→command 47·48,
  ...
  resolver 31/31, contract headers와 M6B runtime이 통과했다.
...
- 남은 조건은 C++ 공식 Redis provider의 Pending creation intent·exact reservation fence projection,
  automatic RID active collision 재시도, typed terminal error mapping과 source-side 즉시 reservation
