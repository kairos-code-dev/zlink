...
| `zlink-framework-dotnet` | `RequestToChannel(...).Async<TReply>()` | `SendToChannel(...).Submit()` |
| `zlink-dotnet` | `Dealer.Send()` 후 `Dealer.Recv()` | `Dealer.Send().Submit()` |
...
| `REQUEST_INFLIGHTS` | `1,100` | request 측정에 사용할 inflight 목록 |
| `COMMAND_INFLIGHTS` | `1,100` | command/send 측정에 사용할 inflight 목록 |
...
- `inflight=N`: `N`개 task/slot이 동시에 request를 유지한다. reply를 받은 slot은 active phase
...
`command`도 설정된 inflight 값마다 따로 측정한다. `inflight=1`은 한 task가 순서대로
command를 보내는 경로이고, `inflight=N`은 `N`개 task/slot이 동시에 command를 보낸다.
처리량은 server가 active phase에서 받은 메시지 수를 기준으로 `KMSG/s`로 표시한다.
`1 KMSG/s`는 초당 1,000개 메시지를 뜻한다. ZLink `send`는 reply가 없으므로 client의
`Submit()` 호출 수만으로 처리량을 계산하지 않는다.
