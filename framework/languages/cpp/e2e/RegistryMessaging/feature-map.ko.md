...
| `RM-C9` | 구현 | backpressure consumer HTTP role이 느린 send handler에 다량 `ProfileMsg`를 제출하고, provider evidence와 backlog 해소 뒤 후속 request 복구를 검증한다. public send submit은 bounded-failure oracle을 노출하지 않는다. |
