...
zlink_submit_result_t zlink_dealer_request_frame_part (
  void *dealer_,
  ...
  zlink_part_flag_t part_flag_)
{
    ...
    if (zlink::part_helper_internal::validate_part_flag (part_flag_) != 0
        || reqrep::validate_request_send_flags (flags_) != 0) {
        zlink::part_helper_internal::consume_send_part (part_);
        return zlink::submit_result_internal::from_errno (errno);
    }
    if (reqrep::validate_socket_type (dealer_, ZLINK_CORE_SOCKET_DEALER) != 0) {
    ...
    zlink::part_helper_internal::send_sequence_spec_t spec;
    spec.family = zlink::part_helper_internal::send_family_dealer_request_frame;
    spec.flags = flags_;
    spec.request_like = true;
    ...
    if (first_part) {
        ...
        if (reqrep::send_request_frame (handle.socket, state.get (), NULL,
                                        &protocol_id, 1,
                                        ZLINK_SNDMORE
                                          | (flags_ & ZLINK_DONTWAIT))
              != 0
    ...
    if (reqrep::send_request_payload_part (handle.socket, state.get (), NULL,
                                           part_, flags_, part_flag_)
        != 0) {
        const int saved_errno = errno;
        zlink::part_helper_internal::abort_send_step (state);
  ...
  zlink_part_flag_t part_flag_)
{
    ...
    if (reqrep::take_dealer_reply_target (request_state, request_token_,
                                          &target)
        != 0) {
        zlink::part_helper_internal::consume_send_part (part_);
        return zlink::submit_result_internal::from_errno (errno);
    ...
    if (zlink::part_helper_internal::prepare_send_step (
          dealer_, spec, handle.socket, &state, &first_part)
        != 0) {
        reqrep::restore_dealer_reply_target (
          request_state, request_token_, target);
        zlink::part_helper_internal::consume_send_part (part_);
        return zlink::submit_result_internal::from_errno (errno);
    }

    auto send_control = [&] (const void *data_, size_t size_) -> int {
        zlink::msg_t msg;
        if (msg.init_size (size_) != 0)
            return -1;
        if (size_ > 0 && data_)
            memcpy (msg.data (), data_, size_);
        const int rc = dealer->sendpipe_to (target.pipe, &msg, ZLINK_SNDMORE);
        const int saved_errno = errno;
        (void) msg.close ();
        errno = saved_errno;
        return rc;
    ...
    if (first_part) {
        ...
        const unsigned char type = zlink::request_reply::reply_type;
        unsigned char seq_buf[8];
        zlink::request_reply::encode_u64_be (target.request_seq, seq_buf);
        if (send_control (&protocol_id, 1) != 0
            || send_control (&version, 1) != 0 || send_control (&type, 1) != 0
            || send_control (seq_buf, sizeof (seq_buf)) != 0) {
            const int saved_errno = errno;
            zlink::part_helper_internal::abort_send_step (state);
            reqrep::restore_dealer_reply_target (
              request_state, request_token_, target);
    ...
    if (dealer->sendpipe_to (
          target.pipe, reinterpret_cast<zlink::msg_t *> (part_),
