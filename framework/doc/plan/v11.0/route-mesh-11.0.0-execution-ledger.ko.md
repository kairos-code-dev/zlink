...
| `V11-CA-ONE-WAY-IFACE-NODE` | Node.js one-way·worker exact interface | Node.js contract lane, `P-DEEP` | `V11-CA-ONE-WAY-SPEC` | 완료 | One-way `Promise<void> submit`, worker `submit`, request·create 결과와 제한된 `yield`가 일치 | Canonical Node.js Server exact interface 5개에서 one-way `submit`·direct `relay`를 `Promise<void>`로 고정하고 public submit·publish result/status/detail type을 제거했다. Actor·Spot create/get-or-create의 제한된 `yield`, `RuntimeShutdown=36`과 Logical Multicast monitoring count를 반영했다. 대상 old symbol 0, code fence 짝수와 `git diff --check`를 확인했다. |
| `V11-CA-ONE-WAY-IFACE-CPP` | C++ one-way·worker exact interface | C++ contract lane, `P-DEEP` | `V11-CA-ONE-WAY-SPEC` | 완료 | One-way `task_t<void> submit`, request·worker `submit`, create 결과와 제한된 `yield`가 일치 | Canonical C++ Server exact interface 4개에서 one-way `submit`·direct `relay`를 `task_t<void>`로 고정하고 public submit·publish result/status/detail type을 제거했다. Actor·Spot create/get-or-create의 제한된 `yield`, `runtime_shutdown=36`과 Logical Multicast monitoring count를 반영했다. 대상 old symbol 0, result-bearing `submit`은 create 2건만 남았고 code fence와 `git diff --check`가 통과했다. |
| `V11-CA-ONE-WAY-PACKAGE-IFACE` | Stream Connector·HTTP Client terminator naming exact interface | package contract lanes, `P-DEEP` | `V11-CA-ONE-WAY-SPEC` | 완료 | 다섯 언어의 비동기 terminator가 .NET `Async`, Kotlin `await`, Java·Node·C++ `submit`으로 통일되고 반환 type만 다른 overload 충돌 0 | HTTP Client와 Stream Connector canonical exact interface 전 언어를 같은 naming snapshot으로 정렬했다. .NET은 `Async`·`AsyncRaw`, Kotlin wrapper는 `await`, Java·Node·C++는 `submit` 계열을 사용하며 one-way는 각각 `ValueTask`, `Unit`, `CompletionStage<Void>`, `Promise<void>`, `task_t<void>`로 결과값 없이 완료한다. Request·response와 callback result API는 유지했다. `git diff --check`가 통과했고 C++ wait-result callback인 `void submit(callback)`만 의도적으로 유지했다. |
| `V11-CA-ONE-WAY-E2E` | One-way admission·error·partial publish E2E 계약 | E2E coordinator, `P-DELIVERY` | `V11-CA-ONE-WAY-SPEC` | 완료 | immediate admission, delayed capacity, timeout·cancellation·shutdown race, target·route error, zero target와 partial multicast scenario 누락 0 | Config 13의 기존 SA-E2E-01~20·SA-REG-01~04를 유지하면서 result-free completion, immediate·delayed admission, bounded pending, timeout·cancellation·shutdown race와 late admission 차단, target·route exception, classic fanout subscriber 0과 Logical Multicast target 0 정상 완료, partial publish rollback·retry 금지와 monitoring count를 반영했다. Config 1·7과 E2E README도 같은 의미로 맞췄다. Legacy status/result 표현 0, scenario 20+4 유지와 `git diff --check`를 확인했다. |
| `V11-CA-ONE-WAY-REVIEW` | One-way 정식 흡수와 package parity 독립 review | Codex 5.6 sol xhigh review lane, `P-XHIGH` + Claude Sonnet 병렬 reviewer | `V11-CA-ONE-WAY-IFACE-DN`, `V11-CA-ONE-WAY-IFACE-JVM`, `V11-CA-ONE-WAY-IFACE-NODE`, `V11-CA-ONE-WAY-IFACE-CPP`, `V11-CA-ONE-WAY-PACKAGE-IFACE`, `V11-CA-ONE-WAY-E2E` | 대기 | 원문 항목의 spec·interface·E2E·runtime owner 추적 100%, 상충·미소유 0, 두 reviewer clean | — |
...
| `V11-M6-ONE-WAY-CPP`, `V11-M6-ONE-WAY-DN`, `V11-M6-ONE-WAY-JVM`, `V11-M6-ONE-WAY-NODE` | 네 Server runtime의 result-free one-way admission | 각 language runtime lane, `P-DEEP` | 대응 `V11-CA-ONE-WAY-IFACE-*` | 대기 | 반환 데이터 0, send-timeout 대기, single terminal·late admission 0과 exception parity contract test 통과 | 각 `<ID>/result.json` |
| `V11-M6-ONE-WAY-PACKAGES` | Stream Connector·HTTP Client naming·adapter 구현 | package implementation lanes, `P-DELIVERY` | `V11-CA-ONE-WAY-PACKAGE-IFACE` | 대기 | exact interface·source·package consumer에서 제거된 `async` terminator와 반환 type overload 충돌 0 | — |
...
| `V11-R7` | 11.0 최종 통합 review | Codex review lane, `P-DEEP` + Claude `claude-sonnet-5` 병렬 reviewer | `V11-M9-DOCS`, `V11-CA-USER-SPOT-RUNTIME-JOIN`, `V11-CA-ACTOR-CREATE-REVIEW` | 대기 | 전체 review clean 뒤 final raw·E2E·consumer·smoke 재통과 | — |
