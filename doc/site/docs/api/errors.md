...
| `NOT_ADMITTED` | `ECONNREFUSED`-class | The local peer knows the remote weight is `0`, so a new outbound is denied. The connection itself may still be alive; the refusal lifts when the peer returns to a positive weight. State propagation is best-effort, so under races the same situation may surface first as `NOT_CONNECTED` or `NOT_FOUND`. |
