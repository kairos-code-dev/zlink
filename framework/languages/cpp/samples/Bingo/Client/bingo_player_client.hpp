...
{
  public:
    static bingo_player_client_t connect (std::string actor_id, const bingo_client_options_t &options)
    {
        auto connector = stream_connector::connector_factory_t::create (
          make_immediate_connector_options (options.stream_endpoint, options.connect_timeout, options.request_timeout));
    ...
    bingo_client_call_result_t authenticate () { return request<authenticate_res_t> (authenticate_req_t{_actor_id}); }

    bingo_client_call_result_t match () { return request<match_bingo_res_t> (match_bingo_req_t{"four-player"}); }
    ...
    {
        return zlink::samples::send_client_packet<bingo_client_call_result_t> (_connector,
  ...
  private:
    ...
    void register_notifications ()
    {
        zlink::stream_connector::codecs::on<player_joined_notify_t> (
          _connector, [this] (const player_joined_notify_t &message) { _notifications.on_joined (message); });
        zlink::stream_connector::codecs::on<game_started_notify_t> (
          _connector, [this] (const game_started_notify_t &message) { _notifications.on_started (message); });
        zlink::stream_connector::codecs::on<number_drawn_notify_t> (
          _connector, [this] (const number_drawn_notify_t &message) { _notifications.on_drawn (message); });
        zlink::stream_connector::codecs::on<game_ended_notify_t> (
          _connector, [this] (const game_ended_notify_t &message) { _notifications.on_ended (message); });
    }

    template <typename TReply, typename TRequest> bingo_client_call_result_t request (const TRequest &request_message)
    {
        return zlink::samples::request_client_packet<bingo_client_call_result_t, TReply> (_connector, request_message);
    }
