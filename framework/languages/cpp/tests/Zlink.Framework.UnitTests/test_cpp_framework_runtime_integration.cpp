...
main ()
{
  ...
  runtime.offload_executor ().submit ([&] {
    offloaded.store (1);
  });
  runtime.drain ();
  ...
  zlink::framework::runtime::dispatch_projection_t projection {
    projection_offload
  };
  std::atomic<int> projected { 0 };
  projection.register_handler (
    zlink::framework::runtime::runtime_event_kind_t::channel_readable,
  ...
  projection.register_handler (
    zlink::framework::runtime::runtime_event_kind_t::timer_readable,
  ...
  const auto tcp =
    zlink::framework::transport_endpoint_t::parse ("tcp://127.0.0.1:7001");
