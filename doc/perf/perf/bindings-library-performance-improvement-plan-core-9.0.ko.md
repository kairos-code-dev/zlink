...
| 현재 pattern | Single `DEALER_ROUTER_REQREP` 진행 중 | tcp, ws, wss, tls, inproc를 완료했고 다음 transport는 ipc다. |
| paired C | .NET `DEALER_ROUTER_REQREP / inproc` 완료 | 전체 크기와 262144B 경계를 CPU pin 없이 paired 측정하고 변경 후 전체 회귀를 5회 확인했다. |
| 개선 반복 | .NET `DEALER_ROUTER_REQREP / inproc` 완료 | 262144B 처리량을 18.53K에서 45.50Kops/s로 개선했고 전체 최소 78.7%, 중앙값 91.2%를 확인했다. |
| 커밋과 푸시 | .NET reply ownership transfer 반영 중 | perf 수정, 기능 test, inproc 전체와 tcp 대표 회귀 근거를 함께 커밋하고 푸시한다. |
...
| 2 | .NET | `PAIR`, `PUBSUB`, `DEALER_DEALER`, `DEALER_ROUTER` 완료, `DEALER_ROUTER_REQREP` tcp, ws, wss, tls, inproc 완료 | 미측정 | `DEALER_ROUTER_REQREP / ipc` 전체 크기를 측정한다. |
