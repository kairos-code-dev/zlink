...
않는다**([Host Relocate와 Shutdown §451-479](../spec/28-graceful-drain-handoff.ko.md)).
...
끝낸다([Host Relocate와 Shutdown §804-805](../spec/28-graceful-drain-handoff.ko.md)).

순서에 규칙이 하나 있다 — **복원된 이전 작업이 보관해 둔 message보다 먼저 실행된다**
([Spot 모델 §92-100](../spec/11-spot-model.ko.md)). 반대로 하면 이동 전에 이미 큐에
...
([Session Actor dispatch §131-133,168-170](../spec/20-session-actor-dispatch.ko.md)).
...
([Location Store provider SPI §88-90](../spec/22-location-store-redis.ko.md)).
...
([Host Relocate와 Shutdown §522-524](../spec/28-graceful-drain-handoff.ko.md)).

이 구간에서 한 가지만은 중간 상태가 없어야 한다 — **처리를 어느 node가 받을지 전환하는
것**은 한 번에 바뀌어야 한다
([Host Relocate와 Shutdown §489-491](../spec/28-graceful-drain-handoff.ko.md)). 여기에
