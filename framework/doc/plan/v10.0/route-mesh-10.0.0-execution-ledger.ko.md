...
| S8-CPP lane (C ABI+C++ bindings→framework) | bindings 리뷰 iter-2 | 0 | 0 | 0 | iter-1 finding(F1-F11/I2/I3) 전량 수정·main 검증(라이브러리+15samples green, **no-hit ZERO**). **bindings 리뷰 iteration-2 착수**(snapshot 123파일 `c0cfcd3d`, Codex·Sonnet). 게이트=`BINDINGS REVIEW CLEAN` |
...
| S8-JVM lane (Java/Kotlin bindings→framework) | bindings 라이브러리 green | 0 | 0 | 0 | Java FFI/Panama Service 재작성 통합·main :compileJava green. 동일 pull-dispatch 표면. **다량 잔재 자기보고**(구 NativeSpotSymbols FFI 테이블·router-spot 표면·msg_gets 유지→no-hit spot_node 284 등). 잔재 정리+samples(Java/Kotlin) 변환→스냅샷→bindings 리뷰 |
