...
int main ()
{
    ...
    zlink::routing_id_t session =
      zlink::routing_id_t::from (std::string ("game-room-session"));
    ...
    auto join = [&] (zlink::service::actor_t &actor) {
        ...
        actor.join (room).message (m).timeout (std::chrono::seconds (1)).submit (
          [&done] (const zlink::actor_join_result_t &result,
    ...
    {
        std::lock_guard<std::mutex> lock (mutex);
        assert (received.size () == 2 && received[0] == "your-turn"
                && received[1] == "wait");
    }
