...
- `RL-C2`: provider 강제 종료 뒤 owner lease TTL stale entry 제거를 빠르고 결정적으로 고정하는 runner 연결은 아직 없다.
- `RL-C4`: location store 장애 중 이미 연결된 channel request는 확인됐지만, store 복구 뒤 새 client follow-up까지 고정하는 Kotlin runner는 아직 없다.
