...
| `.NET: Server/Api/Handlers/MatchBingoHandler.cs` | `Server/Api/Handlers/match_bingo_handler.hpp` | handler | done | matching API 요청을 Play room allocation으로 연결한다. |
...
| `.NET: Server/Play/Application/RoomAllocation/IBingoMatchQueue.cs` | `Server/Play/Application/RoomAllocation/bingo_match_queue.hpp` | application | done | room allocation use case가 의존하는 match queue 계약이다. |
| `.NET: Server/Play/Application/RoomAllocation/BingoRoomAllocator.cs` | `Server/Play/Application/RoomAllocation/bingo_room_allocator.hpp` | application | done | waiting room reservation과 room allocation 결과 생성을 조율한다. |
...
| `.NET: Server/Play/Infrastructure/Redis/RedisBingoMatchQueue.cs` | `Server/Play/Infrastructure/ZLink/Matchmaking/redis_bingo_match_queue.hpp` | external-adapter | done | Redis waiting room record와 actor reservation을 adapter 안에 둔다. |
...
| `.NET: Server/Play/Infrastructure/ZLink/Handlers/AllocateBingoRoomHandler.cs` | `Server/Play/Infrastructure/ZLink/Handlers/allocate_bingo_room_handler.hpp` | handler | done | API channel room allocation request를 application use case로 연결한다. |
| `.NET: Server/Play/Infrastructure/ZLink/Handlers/EnsurePlayerActorHandler.cs` | `Server/Play/Infrastructure/ZLink/Handlers/ensure_player_actor_handler.hpp` | handler | done | 인증된 player actor를 만들거나 찾는다. |
...
| `.NET: Server/Play/Infrastructure/ZLink/Spots/EntrySpot/Handlers/MatchBingoActorHandler.cs` | `Server/Play/Infrastructure/ZLink/Spots/EntrySpot/bingo_entry_spot.hpp` | spot-handler | done | actor matching 요청을 room allocation과 room join으로 연결한다. |
| `.NET: Server/Play/Infrastructure/ZLink/Spots/EntrySpot/Handlers/ObserveBingoEventsHandler.cs` | `Server/Play/Infrastructure/ZLink/Spots/EntrySpot/bingo_entry_spot.hpp` | spot-handler | done | observer actor를 observer용 local room으로 join시킨다. |
...
| `.NET: Server/Play/Infrastructure/ZLink/Spots/BingoRoomSpot/Handlers/SubmitBingoCardHandler.cs` | `Server/Play/Infrastructure/ZLink/Spots/BingoRoomSpot/bingo_room_spot.hpp` | spot-handler | done | card 제출 요청을 domain state 변경으로 연결한다. |
| `.NET: Server/Play/Infrastructure/ZLink/Spots/BingoRoomSpot/Handlers/BingoRoomDrawTimerHandler.cs` | `Server/Play/Infrastructure/ZLink/Spots/BingoRoomSpot/bingo_room_spot.hpp` | timer-handler | done | room Spot timer가 번호 추첨을 진행한다. |
| `.NET: Server/Play/Infrastructure/ZLink/Spots/BingoRoomSpot/Handlers/BingoRewardAcquiredEventHandler.cs` | `Server/Play/Infrastructure/ZLink/Spots/BingoRoomSpot/bingo_room_spot.hpp` | pubsub-handler | done | reward event 수신 후 observer actor에게 push한다. |
| `.NET: Server/Play/Infrastructure/ZLink/Spots/BingoRoomSpot/Handlers/StopObservingBingoEventsHandler.cs` | `Server/Play/Infrastructure/ZLink/Spots/BingoRoomSpot/bingo_room_spot.hpp` | spot-handler | done | observer actor를 observer용 room에서 내보낸다. |
...
없음. Bingo 샘플은 현재 inventory 기준에서 `pending` 또는 `gap` 상태를 남기지 않는다.
