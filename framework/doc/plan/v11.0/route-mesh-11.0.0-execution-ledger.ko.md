...
| `CA-D71` | publish 결과가 remote subscriber queue 또는 handler 완료까지 확인 / source-local submission 경계 | Logical Multicast의 remote 결과는 고정한 MeshNode route의 local transport queue 제출, local 결과는 일치하는 local Spot queue 제출만 집계한다. Remote Spot queue 수락, subscriber handler 시작·완료와 ACK는 기다리거나 결과에 포함하지 않는다. `SubmitAsync` 이름은 유지한다. | Spot messaging, 다섯 messaging exact interface·runtime·contract test |
