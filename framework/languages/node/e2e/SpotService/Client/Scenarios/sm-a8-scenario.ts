...
export async function runSmA8(options: ClientOptions): Promise<void> {
  ...
  const workerStartIndex = findIndex(completed.evidence, `worker-start|rid=play-a|spot=${spotRid}|marker=sm-a8-worker`);
  const stateIndex = findIndex(completed.evidence, `spot-state-request|rid=play-a|spot=${spotRid}|value=1`);
  const workerCompleteIndex = findIndex(completed.evidence, `worker-complete|rid=play-a|spot=${spotRid}|marker=sm-a8-worker`);
  ensure(
