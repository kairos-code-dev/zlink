...
| CPP-ATD-TIMER-RESUME-001 | E2E AutomaticTurnDispatch **ATD-C3B**: spot timer 핸들러가 outbound channel request(`yd::delay_channel`, delay 350ms, timeout 5s)를 await하면 재개가 오지 않는다. 단독(`run_e2e.sh ATD-C3`) 통과, 전체 스위트에서 결정적 실패 | **계측 결론**(직렬 큐 trace + 대기 20s 실험): (1) 직렬 턴 해제·재개 배선은 정상 — 형제 actor 요청이 그 사이 정상 처리된다. (2) delay 서버는 요청을 받아 `delay-completed`까지 남긴다(응답 생성됨). (3) 그럼에도 await는 **20초를 줘도 재개되지 않고**, 호출의 5초 timeout조차 발동하지 않는다 — 즉 detached thread의 blocking submit이 응답을 영영 매칭하지 못한 채 멈춰 있다. 프로세스 종료(컨텍스트 term)로 비로소 풀리며, 그때야 `resume-post`가 큐(이미 파괴됨)에 도착한다. (4) 재현 트리거: **같은 spot에서 timer가 channel 응답을 기다리는 동안 actor 요청이 동시에 디스패치**되는 조합(C3A처럼 actor가 기다리고 timer가 도는 반대 조합은 정상). **다음 단계(구체 지목)**: 호출에는 5초 timeout이 걸려 있는데 20초를 기다려도 timeout조차 발동하지 않는다. `channel_outbound_exchange.cpp`의 대기 루프는 매 반복 `submit_deadline_expired()`를 검사하므로, 그 루프 안에 있었다면 5초에 timed_out으로 빠져나와 재개됐어야 한다. 따라서 detached thread는 루프 밖 **락에 막혀** 있다 — 유력 후보는 `pump_request_progress()`가 잡는 `transport->mutex`(또는 클라이언트 `_mutex`)를 같은 spot에서 동시에 디스패치된 actor 경로가 장시간 점유하는 경합이다. **배제된 가설**: `pump_request_progress()`의 `transport->mutex`를 `try_to_lock`으로 바꿔
