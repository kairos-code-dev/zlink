...
void test_open_more_sequence_survives_bad_recv_and_bad_send_attempts ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_bind (router, "inproc://helper-more-bad-send"));
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_connect (dealer, "inproc://helper-more-bad-send"));
    msleep (SETTLE_TIME);
    ...
    TEST_ASSERT_EQUAL_INT (
      ZLINK_SUBMIT_OK,
      zlink_send_part (dealer, &final_part, static_cast<zlink_send_flags_t> (0),
                       ZLINK_PART_FINAL));
    ...
    TEST_ASSERT_EQUAL_INT (
      ZLINK_SUBMIT_OK,
      zlink_send_part (dealer, &next_msg, static_cast<zlink_send_flags_t> (0),
                       ZLINK_PART_FINAL));
    recv_string_expect_success (router, "D1", 0);
