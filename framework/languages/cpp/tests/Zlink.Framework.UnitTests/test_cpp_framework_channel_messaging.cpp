...
public:
  ...
  zlink::framework::result_t<void> dispatch_send (
...
main ()
{
  ...
  auto request_parts = envelope_codec.encode_raw_body_parts (
    request_header,
    zlink::message_t::from (std::string ("24")));
  zlink::framework::detail::channel_packet_dispatcher_t packet_dispatcher (
    local_runtime);
  ...
  const auto packet_reply_header =
    envelope_codec.decode_header (packet_reply.value ());
  const auto packet_reply_body = envelope_codec.decode_body (
    packet_reply.value ());
  if (!packet_reply_header ||
      packet_reply_header.value ().kind !=
        zlink::framework::runtime::messaging::message_kind_t::response ||
      packet_reply_header.value ().correlation_id != "corr-1" ||
      !packet_reply_body ||
      serializers.get<reply_t> ().deserialize (packet_reply_body.value ()).value !=
        124) {
    return 19;
  }
  ...
  const auto packet_error_header =
    envelope_codec.decode_header (packet_error.value ());
  if (!packet_error_header ||
      packet_error_header.value ().kind !=
        zlink::framework::runtime::messaging::message_kind_t::error ||
      packet_error_header.value ().error_code.value_or ("") !=
        "handler_not_found") {
    return 21;
  }
  ...
  if (manual_connections.size () != 2 ||
      manual_connections[0] != "tcp://127.0.0.1:7400" ||
  ...
  zlink::framework::detail::channel_receive_loop_t receive_loop (
    bundle,
    zlink::framework::detail::channel_message_pump_t (
      zlink::framework::detail::channel_packet_dispatcher_t (
        local_runtime)));
  receive_loop.enqueue_server_message (request_parts);
  if (receive_loop.pending_message_count () != 1) {
    return 25;
  }
  const auto receive_result = receive_loop.drain_server_messages (
    "local",
    provider,
    serializers,
    handlers);
  if (!receive_result ||
      receive_result.value ().dispatched != 1 ||
      receive_result.value ().replies.size () != 1 ||
      receive_loop.pending_message_count () != 0 ||
      bundle.receive_active ()) {
    return 26;
  }
  const auto loop_reply_header =
    envelope_codec.decode_header (receive_result.value ().replies[0]);
  const auto loop_reply_body =
    envelope_codec.decode_body (receive_result.value ().replies[0]);
  if (!loop_reply_header ||
      loop_reply_header.value ().kind !=
        zlink::framework::runtime::messaging::message_kind_t::response ||
      !loop_reply_body ||
      serializers.get<reply_t> ().deserialize (loop_reply_body.value ()).value !=
        124) {
    return 27;
  }
  ...
  const auto reentrant_result = receive_loop.drain_server_messages (
    "local",
    provider,
    serializers,
    handlers);
  ...
  if (route_connection_list.size () != 2 ||
      route_connection_list[0] != "tcp://route-a:7500" ||
  ...
  zlink::framework::detail::route_channel_runtime_t route_runtime (
    "game.route");
  const auto target_node =
    zlink::routing_id_t::from (std::string ("remote-node"));
  auto disconnected_send = route_runtime.submit_send (
    target_node,
    "event",
    event_t { 77 },
    serializers);
  ...
  auto route_send = route_runtime.submit_send (
    target_node,
    "event",
    event_t { 78 },
    serializers);
  if (!route_send ||
      route_runtime.outbound_packets ().size () != 1 ||
  ...
  auto route_request = route_runtime.submit_request (
    target_node,
    "request",
    request_t { 79 },
    serializers,
  ...
  const auto target_spot =
    zlink::routing_id_t::from (std::string ("remote-spot"));
  auto spot_request = route_runtime.request_to_spot_parts (
    target_node,
    target_spot,
    request_parts);
  if (!spot_request ||
      route_runtime.pending_request_count () != 2 ||
      route_runtime.outbound_packets ().back ().target_spot_rid.value () !=
        target_spot) {
    return 38;
  }
  ...
  route_runtime.stop ();
  if (route_runtime.running () ||
      route_runtime.pending_request_count () != 0) {
    return 40;
  }
  ...
  if (!managed_route.running () ||
      managed_route.list_connections ().empty ()) {
    return 45;
  }
  ...
  if (!route_receive ||
      route_receive.value ().dispatched != 1 ||
  ...
  if (!route_error_header ||
      ...
      route_error_header.value ().error_code.value_or ("") !=
  ...
  route_handlers.on_send<local_handler_t, event_t> (
    "game.route",
    "event",
  ...
  if (!route_handler_receive ||
      route_handler_receive.value ().replies.size () != 1) {
    return 48;
  }
  const auto route_reply_body = envelope_codec.decode_body (
    route_handler_receive.value ().replies[0].parts);
  if (!route_reply_body ||
      serializers.get<reply_t> ().deserialize (route_reply_body.value ()).value !=
        224 ||
      provider.get_required<local_handler_t> ().last_route_request != 24 ||
      provider.get_required<local_handler_t> ().last_route_source !=
        "source-node") {
    return 49;
  }
  route_handler_pump.enqueue (
  ...
  if (!internal_receive ||
      internal_receive.value ().replies.size () != 1) {
    return 51;
  }
  const auto internal_reply_body = envelope_codec.decode_body (
    internal_receive.value ().replies[0].parts);
  if (!internal_reply_body ||
      internal_reply_body.value ().to_string () != "88") {
    return 52;
  }

  zlink::framework::detail::route_channel_registration_t route_registration (
    "registered.route");
  route_registration.bind ("tcp://registered-bind:7600")
    ...
    .add_request_handler<local_handler_t, request_t, reply_t> (
      "request",
      &local_handler_t::handle_route_request)
    .add_send_handler<local_handler_t, event_t> (
      "event",
      &local_handler_t::handle_route_send);
  ...
  auto registered_parts = envelope_codec.encode_raw_body_parts (
    registered_header,
    zlink::message_t::from (std::string ("25")));
  zlink::framework::detail::route_receive_pump_t registered_pump {
    zlink::framework::detail::route_packet_dispatcher_t (
      "registered.route",
      provider,
      serializers,
      initialized_route.handlers,
      no_internal)
  };
  registered_pump.enqueue (
    zlink::framework::detail::route_received_packet_t {
      zlink::routing_id_t::from (std::string ("source-node")),
      80,
      registered_parts });
  auto registered_receive = registered_pump.drain ();
  if (!registered_receive ||
      registered_receive.value ().replies.size () != 1) {
    return 54;
  }
  const auto registered_reply_body = envelope_codec.decode_body (
    registered_receive.value ().replies[0].parts);
  if (!registered_reply_body ||
      serializers.get<reply_t> ().deserialize (
        registered_reply_body.value ()).value != 225) {
    return 55;
  }
  ...
  zlink::framework::detail::backend::native_route_backend_t native_backend (
    native_router);
  ...
  auto public_send_header = envelope_codec.decode_header (
    public_route.outbound_packets ()[0].parts);
  ...
  auto public_request_header = envelope_codec.decode_header (
    public_route.outbound_packets ()[1].parts);
  ...
  public_route.set_request_backend (
    [&envelope_codec, &serializers](
      ...
      if (target.to_string () != "target-node" ||
      ...
      return zlink::framework::result_t<
        ...
        envelope_codec.encode_parts (
          reply_header,
          std::type_index (typeid (reply_t)),
          &reply,
          serializers));
    });
