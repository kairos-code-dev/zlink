...
[Java 묶음](./README.ko.md) | [포팅 계획](./java-kotlin-framework-porting-plan.ko.md) | [STREAM](./spring-boot-stream.ko.md) | [STREAM 샘플](./stream-samples.ko.md) | [Samples](./sample-implementation-plan.ko.md)

# Draft -- Java/Kotlin Stream Connector

> 이 문서는 **구현 전 초안**이다.
> 현재 공개 계약이 아니며, `.NET`의 `Systems.Zlink.Stream.Connector`와 같은
> client connector를 Java/Kotlin에서 구현하기 위한 기준이다.
...
public interface ZLinkStreamConnector extends AutoCloseable {
    ...
    AutoCloseable onError(ZLinkStreamErrorHandler handler);
