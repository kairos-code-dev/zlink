...
if [[ "${IS_WINDOWS}" -eq 1 ]]; then
  BUILD_DIR="${ROOT_DIR}/bindings/dotnet/build/windows-x64"
else
  if [[ -d "${ROOT_DIR}/bindings/dotnet/build/bin" ]]; then
    BUILD_DIR="${ROOT_DIR}/bindings/dotnet/build"
  else
    BUILD_DIR="${ROOT_DIR}/bindings/dotnet/build/${PLATFORM}-${ARCH}"
  fi
fi
...
MULTI_WARMUP_SECONDS="${PERF_MULTI_WARMUP_SECONDS:-}"
MULTI_DURATION_SECONDS="${PERF_MULTI_DURATION_SECONDS:-}"
MULTI_CLIENTS="${PERF_MULTI_CLIENTS:-}"
MULTI_HWM="${PERF_MULTI_HWM:-}"
MULTI_SNDHWM="${PERF_MULTI_SNDHWM:-}"
MULTI_RCVHWM="${PERF_MULTI_RCVHWM:-}"
MULTI_SNDTIMEO_MS="${PERF_MULTI_SNDTIMEO_MS:-}"
MULTI_RCVTIMEO_MS="${PERF_MULTI_RCVTIMEO_MS:-}"
...
usage() {
  cat <<'USAGE'
Usage: bindings/dotnet/perf/run_benchmarks.sh [options]
...
Options:
  ...
  --build-dir PATH            Build directory (default: bindings/dotnet/build/<platform>-<arch>).
