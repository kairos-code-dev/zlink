  ...
  discovery_t *owner_, uint64_t now_ms_)
{
    std::vector<discovery_registered_service_snapshot_t> services;
    {
        ...
        discovery_access_t::collect_registered_services_for_heartbeat (
          owner_, now_ms_,
          discovery_access_t::bootstrap_runtime (owner_)->heartbeat_interval_ms (),
          &services);
    }
    ...
    for (size_t i = 0; i < services.size (); ++i) {
        ...
        if (discovery_protocol::send_u16 (
              ...
              < 0
            || discovery_protocol::send_u16 (dealer, owner_->auto_connect_type (),
                                             ZLINK_SNDMORE)
                 < 0
        ...
        scoped_lock_t lock (discovery_access_t::sync (owner_));
        discovery_access_t::mark_registered_service_heartbeat (
          owner_, services[i], now_ms_);
    }
}
