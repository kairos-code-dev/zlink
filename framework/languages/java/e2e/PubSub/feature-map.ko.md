...
| PS-B2 | 부분 구현 | publisher의 public drain 결과가 terminal `Drained`인지 확인하고 subscriber runtime query에서 old PUB row 제거를 기다린 뒤 같은 rid·endpoint로 재시작한다. 새 PUB row 반영 뒤 기존 subscriber가 새 event를 받는지도 확인한다. socket `ConnectionReady` event의 직접 단언만 monitoring source registry blocker로 남는다. 동적 역할 readiness는 3초다. |
