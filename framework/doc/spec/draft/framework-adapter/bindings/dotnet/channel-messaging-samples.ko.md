...
이 초안에서 channel client 수동 연결은, remote `RoutingId`[^rid] 를 받지 않는다. 이유는
...
- 반대로 outbound `DEALER(client)` 가 받는 메시지는, 먼저 보낸 request 의 reply 를
  완료하는 경로로 본다.
- 현재 초안은 `ROUTER -> DEALER` 임의 push 를, channel messaging 공용 API 에 넣지
  않는다.
...
전체 정의는 [handler-interfaces.ko.md](./handler-interfaces.ko.md) 의 section 5 를
