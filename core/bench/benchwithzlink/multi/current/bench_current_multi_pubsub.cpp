                                  ...
                                  int poll_timeout_ms)
{
    ...
    if (!setup_tls_server (pub, transport) || !setup_tls_client (sub, transport)) {
        zlink_close (sub);
        zlink_close (pub);
        return 0.0;
    }
    ...
    if (!connect_checked (sub, endpoint, transport)
                       ...
                       const std::string &lib_name)
{
    ...
    const auto emit_zero_from = [&] (size_t start_index) {
        for (size_t i = start_index; i < msg_sizes.size (); ++i) {
            print_result (
              lib_name, "MULTI_PUBSUB", transport, msg_sizes[i], 0.0, 0.0);
        }
    };
    ...
    if (endpoint.empty ()
        || !connect_clients_concurrently (
          ...
          [&] (void *sock, const std::string &ep) {
              return connect_checked (sock, ep, transport);
          },
    ...
    for (size_t s = 0; s < msg_sizes.size (); ++s) {
        ...
        const double latency = measure_pubsub_latency_us (ctx.get (),
                                                          transport,
                                                          lib_name,
                                                          current_size,
                                                          pubsub_hwm,
        ...
        print_prep_result (lib_name,
                           ...
                           prep_ready_ms);
        print_result (
          lib_name, "MULTI_PUBSUB", transport, current_size, throughput, latency);
    }
