...
class customer_session_t final : public packet_stream_session_t
{
                            ...
                            const zlink::message_t &payload) override
    {
        ...
        try {
            ...
            std::cerr << "deliverydispatch session: ensure customer delivery="
                      << request.delivery_id << "\n";
            auto ensured =
              co_await request_tracking<ensure_customer_actor_t, customer_actor_ensured_t> (
            ...
            std::cerr << "deliverydispatch session: subscribe delivery=" << request.delivery_id
                      << "\n";
            auto subscribed =
              co_await request_tracking<subscribe_customer_to_delivery_t,
  ...
  private:
    template <typename TRequest, typename TReply> task_t<TReply> request_tracking (TRequest request)
    {
        std::string last_error;
        for (int attempt = 1; attempt <= 40; ++attempt) {
            try {
                co_return co_await _channels
                  .request (sample_names_t::tracking_route_channel, request)
