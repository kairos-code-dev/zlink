...
| S8-CPP lane (C ABI+C++ bindings→framework) | bindings 라이브러리 green | 0 | 0 | 0 | **bindings Service 재작성 통합·main 컴파일 green**(`212ba9a08`, `zlink_cpp` 0 err/warn, 폐기 개념 0 hits). `mesh_node.hpp`/`dispatch.hpp`/`stream_session.hpp` 신설, route_bridge/spot_send/spot_receive/actor_operations 삭제, raw-layer drift 수정. **패러다임 발견(전 lane 공통)**: 10.0.0 C API가 콜백·fluent builder 불가(요청=operation_id 반환, 완료=pull dispatch 도착)→service=direct-method. 잔여: samples 11 변환 후 bindings 리뷰 campaign |
