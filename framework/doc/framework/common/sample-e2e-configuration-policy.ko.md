...
이 정책은 공개 framework API 계약을 새로 정의하지 않는다. 각 언어가 이미 사용하는 host의
정식 설정 시스템으로 값을 읽고, 검증된 설정을 framework builder에 적용하는 방법을 고정한다.
...
| 언어 | 설정 입력과 binding | Framework 연결 |
|------|---------------------|----------------|
| Node.js/NestJS | 설정 파일을 `ConfigModule`의 typed provider로 읽고 시작 시 검증한다. | Typed provider를 `ZLinkModule.forRootFactory(...)`에 주입한다. |
...
- Secret이 필요한 E2E는 runner가 실행별 임시 secret 파일을 만들거나 외부 secret file 경로를
  설정 파일에 기록한다.
- Runner가 만든 설정과 secret 파일은 현재 사용자만 읽을 수 있게 권한을 제한한다.
- 정상 종료와 실패 종료 모두에서 runner가 자신이 만든 임시 파일만 정리한다.
- 실패 분석을 위해 설정 파일을 보존할 때는 secret 값을 제거한 위치와 보존 경로를 출력한다.
