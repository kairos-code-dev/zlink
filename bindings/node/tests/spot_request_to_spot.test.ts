...
test('spot sendToSpot DontWait maps disconnected peer to SubmitResult.NotConnected', () => {
  ...
  try {
    ...
    assert.throws(
      () => spot.sendToSpot(textRoutingId('SPOT_MISSING_NODE'), textRoutingId('SPOT_MISSING_SPOT'))
        .message(Buffer.from('payload'))
        .flags(zlink.SendFlags.DontWait)
        .submit(),
      (error) => error instanceof zlink.SubmitError &&
...
test('spot route bridge router request handles metadata and resolves through target spot reply', async () => {
  ...
  try {
    ...
    await waitFor(
      () => targetRouter.recv(channelReceived, zlink.RecvFlags.DontWait),
      'target route router bridge request'
    );
    ...
    try {
      ...
      assert.equal(
        targetBridge.handleRouterReceived(
          'mesh',
          channelReceived.routingId,
          channelReceived.requestSeq,
          channelReceived.parts
        ),
        true
      );
    ...
    await waitFor(
      () => targetSpot.recvRouted(spotReceived, zlink.RecvFlags.DontWait),
      'target entry spot bridge request'
    );
    try {
...
test('router requestToSpot reaches peer router as SPOT-addressed request', async () => {
  ...
  try {
    ...
    const replyPromise = sourceRouter.requestToSpot(
      targetRouter.getRoutingId(),
      textRoutingId('ROUTER_SPOT_ENTRY')
    ).message(Buffer.from('router-spot-direct-ping')).timeout(2000).submit();

    const received = new zlink.Received();
    await waitFor(
      () => targetRouter.recv(received, zlink.RecvFlags.DontWait),
      'target router direct SPOT request'
    );
    try {
      assert.ok(received.routingId);
