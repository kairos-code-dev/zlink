...
| TA-B2 | implemented | 실제 actor를 destroy/recreate해 generation이 바뀐 이전 ref의 stale 동작을 검증한다. | 이전 ref의 send/request가 handler에 도달하지 않고 request가 `actorLocationStale`로 실패하며, 새 live ref의 request가 성공한다. |
