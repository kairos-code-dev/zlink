...
| `ws` | `MULTI_DEALER_DEALER` | `보류(38.3%)` | `통과(55.2%)` | `통과(67.7%)` | `통과(59.2%)` | `통과(58.7%)` | `통과(64.4%)` | 64B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_104651.txt` 대비 Go `perf_go_multi_linux_20260525_104650.txt` 기준이다. 256B/1024B는 Go `perf_go_multi_linux_20260524_222134.txt`, 65536/131072/262144B는 Go `perf_go_multi_linux_20260525_001654.txt` median 기준이다. C처럼 단일 poll loop와 pending socket만 `POLLOUT`으로 두는 poll set 의미를 유지했다. client active send는 65536B/131072B에서 public `Bytes(...)` 경로를 사용해 submit 전 native `Message` 생성을 줄였다. 262144B는 기존 경로로도 current HEAD에서 통과권이 유지된다. 64B는 최신 같은 조건 재측정에서도 Go 단순 one-way 최소 기준보다 낮다. |
