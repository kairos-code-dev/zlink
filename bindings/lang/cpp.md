...
spot.on_subscribe([](const RoutingId& src, std::string_view topic, Received msg) {
...
spot.on_message([&](const RoutingId& src, const RoutingId& spot_rid,
                    uint64_t req_seq, Received msg) {
    if (req_seq != 0) {
        // request 도착 — callback 안에서 바로 reply
        spot.reply_to_spot(src, spot_rid, req_seq, Message::from("ok"));
    } else {
...
auto timers = Timers();
timers.add(1000ms, [&](int timer_id) {
    // 1초마다 실행, callback 안에서 send/recv 가능
    spot.publish("heartbeat", Message::from("ping"));
});
