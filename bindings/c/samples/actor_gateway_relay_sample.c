...
int main (void)
{
    ...
    assert (zlink_spot_dispatch_event_handler (play_spot, actor_sample_dispatch, &capture)
            == ZLINK_HANDLER_OK);

    zlink_actor_ref_t actor;
    assert (zlink_spot_node_actor_new (gateway_node, "play-session-actor", &actor)
            == ZLINK_CONFIG_OK);
    ...
    zlink_routing_id_t play_spot_rid = actor_sample_find_spot_rid_not (play_node, NULL, 0);
    zlink_msg_t join;
    make_message (&join, "join-play");
    assert (zlink_spot_node_actor_join_spot (gateway_node, &actor, &play_node_rid, &play_spot_rid,
                                             &join, 1, actor_sample_join_reply, &capture,
                                             ZLINK_DONTWAIT, 1000)
            == ZLINK_SUBMIT_OK);
    assert (callback_signal_wait (&capture.join_signal, 2000));
    assert (capture.join_result == ZLINK_REQUEST_OK);
    ...
    assert (zlink_stream_send_bound_actor_part (stream, &session_rid, "play-session-actor", &frame,
    ...
    actor_sample_capture_reset (&capture);
    assert (zlink_spot_node_actor_leave_spot (gateway_node, &actor, &play_spot_rid,
                                              actor_sample_reply, &capture, 1000)
            == ZLINK_SUBMIT_OK);
    assert (callback_signal_wait (&capture.join_signal, 2000));
    assert (capture.join_result == ZLINK_REQUEST_OK);
    actor_sample_capture_reset (&capture);
    assert (zlink_spot_node_actor_destroy (gateway_node, &actor, actor_sample_reply, &capture, 1000)
            == ZLINK_SUBMIT_OK);
    assert (callback_signal_wait (&capture.join_signal, 2000));
    assert (capture.join_result == ZLINK_REQUEST_OK);
