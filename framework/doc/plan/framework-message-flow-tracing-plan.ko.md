...
| `Node` parity | ✅ 완료 — Node(NestJS/TS)는 자체 dispatch 런타임이라 .NET/Java처럼 두꺼운 구현. 계약(5단계 mode + phase/event/observer/control + fluent 빌더 core·NestJS 양쪽) / 트레이서(runtime/diagnostics/message-flow.ts: enabled(phase) 가드 + flowIfEnabled 옵셔널체이닝으로 off 제로-alloc, 샘플링, 구조화+node, 파일 sink, observer offload) / 에러리포터(off-게이팅+포맷통일+파일, .flow 노출, 하위호환 ctor) / host 공유 라이브셀 + setMessageFlowMode 토글. 인바운드 전 surface(channel req/send/publish, route send/req, spot subscription·actor 2경로, stream session received/dispatched + StreamSession 에러리포트 신설) + 코어 아웃바운드(channel·route 클라이언트 send/req/publish = Sent/ReplyReceived, 클라 생성 corr 공유). stream corr 와이어(flag 0x08, 메타 뒤 u8 len+bytes): framework protocol + connector 코덱 **바이트동일**(메타데이터+corr 교차검증), connector 단조 hex 클라생성/framework 세션·에러 reply echo, control 패킷 corr 거부(encode+decode 양쪽). MFLOW-001~011+010b 테스트 175 그린. Bingo.Ts Api/Play/Session 배선. **codex 4라운드 → LOOP CLEAN**. 잔여 아웃바운드(raw spot-to-spot·native actor-join·spot publish fanout)는 사용자 스코프 합의로 deferred low. |
