                                        ...
                                        const handler_group_options_state_t &handler_groups)
{
    ...
    for (const auto &spot_node_name : options.spot_nodes) {
        if (!options.spot_nodes_with_runtime_capability.contains (spot_node_name)) {
            throw framework_exception_t (framework_error_kind_t::request_protocol_error,
                                         "SPOT node '" + spot_node_name + "' must enable router or pub/sub capability");
        }
    }
    for (const auto &[spot_node_name, channel_names] : options.attached_channel_clients_by_node) {
        for (const auto &channel_name : channel_names) {
            ...
            if (options.registry_discovery_endpoints.empty ()
                && !has_manual_connections (options.attached_channel_client_manual_connections_by_node, spot_node_name,
                                            channel_name)) {
                throw framework_exception_t (framework_error_kind_t::request_protocol_error,
                                             "SPOT node '" + spot_node_name + "' attached channel client '"
                                               + channel_name + "' requires registry discovery or manual connections");
            }
        }
    }
    ...
    for (const auto &[spot_node_name, channel_names] : options.accepted_spot_route_channels_by_node) {
        ...
        for (const auto &channel_name : channel_names) {
            ...
            if (options.registry_discovery_endpoints.empty ()
                && !has_manual_connections (options.accepted_spot_route_manual_connections_by_node, spot_node_name,
                                            channel_name)) {
                throw framework_exception_t (framework_error_kind_t::request_protocol_error,
    ...
    for (const auto &channel_name : options.client_server_channels_with_server) {
        ...
        if (!handler_groups.channel_exposes_any (channel_name,
                                                 {handler_group_kind_t::request, handler_group_kind_t::send})) {
            throw framework_exception_t (framework_error_kind_t::request_protocol_error,
                                         "client/server channel '" + channel_name
