...
int spot_node_t::connect_router_channel_peer (const char *channel_name_, const char *endpoint_)
{
    ...
    if (send_data_plane_command (spot_control_protocol::cmd_connect_router_channel_peer,
        ...
        if (it != _service_attachment_state.router_channel_peers.end ()) {
            ...
            if (it->second.manual_endpoints.empty () && it->second.active_endpoints.empty ()
                && !it->second.discovery)
    ...
    {
        scoped_lock_t lock (_sync);
        _service_attachment_state.router_channel_peers[channel_name].active_endpoints.insert (
          endpoint);
