...
int run_stream_server (const std::string &pattern,
                       const std::string &transport,
                       size_t size)
{
    ...
    const std::string endpoint =
      perf_multi::make_multi_endpoint (transport, "multi-stream-server");
    ...
    if (zlink_stream_attach (g_server_socket, &on_stream_packets, 0) != 0) {
...
{
    return run_stream_server ("MULTI_STREAM", transport, size);
}
