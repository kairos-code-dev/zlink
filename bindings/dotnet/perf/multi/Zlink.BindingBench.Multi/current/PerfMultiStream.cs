...
{
    private static int RunMultiStreamServer(string transport, int size)
    {
        size = Math.Max(1, size);
        int rcvTimeoutMs = ParsePositiveEnv("PERF_MULTI_RCVTIMEO_MS", 5000);
        string endpoint = MultiEndpointFor(transport, "multi-MULTI_STREAM");
    ...
    private static int RunMultiStreamClient(string transport, int size,
        string endpoint)
    {
        size = Math.Max(1, size);
        ...
        int drainMs = ResolveMultiDrainMs("MULTI_STREAM");
        ...
        try
        {
            ...
            PrintResult("MULTI_STREAM", transport, size, throughput, latencyUs);
            return 0;
        }
        finally
        {
