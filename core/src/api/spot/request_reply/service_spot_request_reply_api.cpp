...
#include "api/core/status_internal.hpp"
...
extern "C" void zlink_spot_notify_dispatch_event (void *spot_, zlink_spot_dispatch_event_t event_)
{
    notify_spot_dispatch_info (spot_, event_, ZLINK_SPOT_DISPATCH_SUBJECT_SPOT, spot_);
}

extern "C" void zlink_spot_notify_dispatch_info (void *spot_,
                                                 zlink_spot_dispatch_event_t event_,
                                                 zlink_spot_dispatch_subject_kind_t subject_kind_,
                                                 void *subject_)
{
...
int dispatch_spot_message (spot_request_reply_state_t *state_,
                           ...
                           size_t part_count_)
{
    if (queue_spot_message (state_, source_rid_, spot_rid_, request_seq_, parts_, part_count_)
        != 0) {
...
{
    return spot_ != NULL
           && zlink::spot_dispatch_event_callback_context_t::current_handle () == spot_;
}
                                           ...
                                           size_t part_count_)
{
    ...
    return queue_spot_subscribe_message (state.get (), source_rid_, topic_, topic_len_, parts_,
                                         part_count_);
}
                                           ...
                                           zlink_recv_flags_t flags_)
{
    ...
    return recv_internal_spot_subscribe_queue (&state->recv.subscribe_queue, source_rid_out_,
