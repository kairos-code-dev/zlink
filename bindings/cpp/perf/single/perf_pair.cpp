                  ...
                  size_t size)
{
    ...
    int lat_count = env_int("PERF_LAT_COUNT", 500);
    int msg_count = resolve_msg_count(size);

    zlink::context_t ctx;
    zlink::socket_t a(ctx, static_cast<zlink::socket_type>(a_type));
    zlink::socket_t b(ctx, static_cast<zlink::socket_type>(b_type));

    std::string endpoint = endpoint_for(transport, std::string(pattern));
    ...
    print_result(pattern, transport, size, throughput, latency);
    return 0;
}

int run_pattern_pair(const std::string &transport, size_t size)
{
    return run_pair_like("PAIR", static_cast<int>(zlink::socket_type::pair), static_cast<int>(zlink::socket_type::pair), transport, size);
}
