...
| 5 | Go | `bindings/go/perf` | `tcp/ws/wss/tls single 통과 (6.0.3 재검증, SPOT large 회귀 수정, latency 발산은 측정 아티팩트로 판정)` | `2026-05-24 재측정/추가 수정으로 ROUTER_ROUTER/DEALER_ROUTER/PUBSUB 전 transport 통과, DD·SPOT large 대폭 개선(6.6.3). 잔여 보류: SPOT_SENDSEND large, DD 일부 64B/large, SPOT_REQREP large, 64B FFI` | 잔여 보류 추적: `SPOT_SENDSEND` request-reply echo(size별 active-slot+1ms cap) 재설계, 암호화 DD large copy 축소 |
