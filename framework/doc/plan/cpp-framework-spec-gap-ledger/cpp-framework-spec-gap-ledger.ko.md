...
- 기준 시점: 2026-08-03 20:17 KST. Core `core/build`와 canonical `framework/languages/cpp/build`를 다시 빌드하고, Core duplicate-routing regression, C++ runtime unit, sample contract, full serial CTest, 6개 sample process와 bounded common E2E를 trace/file log로 재검증했다. PubSub fanout missing-handler runtime gap과 Core STREAM listener stop gap은 owner-layer unittest·process file trace로 추가 확인했다. 현재 full serial CTest는 56/56, shell·PowerShell sample aggregate는 각각 6/6, packaged clean consumer는 PASS다. RuntimeMonitoring source porting은 current exact C++ interface 기준으로 A1/A2 process가 통과했지만 A3 fixture/contract 조건과 A4 physical peer removal은 아직 닫히지 않았다.
