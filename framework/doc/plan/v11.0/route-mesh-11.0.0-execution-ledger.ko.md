...
| `V11-M6-SESSION-BINDING-DN` | .NET stored route·disconnect runtime | .NET runtime lane, `P-DEEP` | `V11-CA-SESSION-BINDING-REVIEW` | 진행 | IMP-SA-1~4 .NET 항목과 contract test gap 0 | Relay의 `IZLinkActorResolver.FindWithPresenceAsync`와 hidden rebind를 제거하고 bind 때 고정한 route만 사용한다. Physical close는 exact binding snapshot 전체를 병렬 통지하고 같은 binding notification은 completion을 공유하며 callback timeout·failure 뒤에도 tombstone cleanup을 완료한다. 다른 ObjectGeneration은 explicit bind만 허용한다. `SessionActorCoordinatorTests`, `StreamSessionForcedCleanupTests`, `ActorBoundSessionRelayTests`, `ActorHandoffTests` focused 재실행은 56/56 PASS다. SpotService는 `Context.SpotId`, result-free `.Async()`, Play object stable type 등록과 Play deferred Join callback 전환을 시작해 Play build error를 86개에서 unique 16개로 줄였지만, `dotnet build framework/languages/dotnet/e2e/SpotService/Server/{Play,Session,MultiNode}/*.csproj --maxcpucount:1` 기준 Play 16개, Session 6개, MultiNode 13개가 남는다. 남은 원인은 fluent `GetOrCreate(...).Async()`와 exact `SpotRef` close, string Spot ID route, MultiNode deferred Join 전환이며 `V11-M6B-ENTRY-IDENTITY-DN`, `V11-M6-ONE-WAY-DN`, `V11-M6-DEFERRED-JOIN-DN`의 sample·E2E 전환을 함께 끝내야 한다. 현재 `sm-d5a`는 선택 logical notification만 검증하고 actual physical disconnect의 fixed all-settled·no-Store·stale token과 durable `Completed` 뒤 같은 ObjectGeneration route switch를 실행하지 않으므로 process E2E gate는 미완료다. |
| `V11-M6-SESSION-BINDING-JVM` | JVM stored route·disconnect runtime | JVM runtime lane, `P-DEEP` | `V11-CA-SESSION-BINDING-REVIEW` | 진행 | IMP-SA-1~4 Java·Kotlin 항목과 contract test gap 0 | Relay의 `ZLinkActorDirectory.find`와 per-message native bind·hidden rebind를 제거하고 stream에서 session actor runtime으로 전달하던 dead directory plumbing도 삭제했다. Stored relay는 source-local backpressure만 기다리며 stale route를 한 번 제출한 뒤 typed error로 끝낸다. Exact binding snapshot all-settled·dedupe·failure cleanup, 선택 logical notification과 same-ObjectGeneration route fence를 구현했다. `./gradlew :zlink-framework-core:test --tests systems.zlink.framework.runtime.actors.ZLinkSessionActorBindingContractTest`는 2026-07-25에 다시 통과했다. Process E2E는 아직 차단되어 있다. Java `SpotService`의 `./run_e2e.sh SM-D5`는 현재 public Spot·Actor API와 fixture가 어긋나 73개 compile error가 발생한다(`framework/languages/java/e2e/SpotService/logs/20260725-072131-2752489`). 대체 topology인 `SpotActorTransfer`는 fixture compile을 현재 API에 맞춘 뒤 `./run_e2e.sh ST-E1`에서 User Spot create가 5초 안에 완료되지 않아 실패했다(`framework/languages/java/e2e/SpotActorTransfer/log/20260725-072746-3007527`, `ActorNodeHttpServer.createSpot`의 `TimeoutException`). 따라서 Java·Kotlin process에서 Config 2 SM-D4A·D4B·D5·D5A와 Config 10 ST-E1A를 증명하려면 먼저 public object client/server registration과 deferred Join completion을 사용하는 E2E fixture 전환을 별도 범위에서 완료해야 한다. 그 전에는 이 행을 완료로 전환하지 않는다. |
