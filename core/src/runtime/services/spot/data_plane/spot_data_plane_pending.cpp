                                                           ...
                                                           uint64_t message_id_)
{
    ...
    spot_data_plane_pending_state_t::local_fanout_state_t::pending_message_map_t::iterator it =
      state_->pending.local_fanout.pending_messages.find (message_id_);
    if (it == state_->pending.local_fanout.pending_messages.end ())
        return;
    ...
    if (state_->pending.local_fanout.pending_bytes >= it->second.encoded_bytes)
        state_->pending.local_fanout.pending_bytes -= it->second.encoded_bytes;
    else
        state_->pending.local_fanout.pending_bytes = 0;
    spot_clear_msg_parts (&it->second.parts);
    state_->pending.local_fanout.pending_messages.erase (it);
}
                                                         ...
                                                         uint64_t attachment_id_)
{
    ...
    spot_data_plane_pending_state_t::local_fanout_state_t::target_map_t::iterator it =
      state_->pending.local_fanout.targets.find (attachment_id_);
    if (it == state_->pending.local_fanout.targets.end ())
        return;
    ...
    state_->pending.local_fanout.targets.erase (it);
}
                                                          ...
                                                          uint64_t message_id_)
{
    ...
    spot_data_plane_pending_state_t::remote_mesh_state_t::pending_message_map_t::iterator it =
      state_->pending.remote_mesh.pending_messages.find (message_id_);
    if (it == state_->pending.remote_mesh.pending_messages.end ())
        return;
    ...
    if (state_->pending.remote_mesh.pending_bytes >= it->second.encoded_bytes)
        state_->pending.remote_mesh.pending_bytes -= it->second.encoded_bytes;
    else
        state_->pending.remote_mesh.pending_bytes = 0;
    spot_clear_msg_parts (&it->second.parts);
    state_->pending.remote_mesh.pending_messages.erase (it);
}
                                                          ...
                                                          const std::string &endpoint_)
{
    ...
    spot_data_plane_pending_state_t::remote_mesh_state_t::target_map_t::iterator it =
      state_->pending.remote_mesh.targets.find (endpoint_);
    if (it == state_->pending.remote_mesh.targets.end ())
        return;

    if (state_->poller && it->second.sender_socket)
        (void) state_->poller->remove (it->second.sender_socket);
    if (it->second.sender_socket)
        state_->pending.remote_mesh.target_by_socket.erase (it->second.sender_socket);
    ...
    state_->pending.remote_mesh.targets.erase (it);
}
  ...
  size_t precomputed_encoded_bytes_)
{
    ...
    if (message_id == 0) {
        message_id = ++state_->pending.next_message_id;
        if (message_id == 0)
            message_id = ++state_->pending.next_message_id;

        spot_data_plane_pending_state_t::publish_pending_entry_t entry;
        entry.message_id = message_id;
        entry.topic = topic_;
        entry.encoded_bytes = precomputed_encoded_bytes_ > 0
                                ? precomputed_encoded_bytes_
                                : spot_msg_parts_encoded_bytes (parts_);
        if (!queue_has_room (state_->pending.local_fanout.pending_bytes,
                             state_->pending.local_fanout.pending_hard_limit, entry.encoded_bytes)) {
            errno = EAGAIN;
            return false;
        }
        ...
        state_->pending.local_fanout.pending_bytes += entry.encoded_bytes;
        state_->pending.local_fanout.pending_messages.emplace (message_id, std::move (entry));
    ...
    spot_data_plane_pending_state_t::local_fanout_state_t::pending_message_map_t::iterator it =
      state_->pending.local_fanout.pending_messages.find (message_id);
    if (it != state_->pending.local_fanout.pending_messages.end ())
        ++it->second.remaining_targets;
    return true;
}
  ...
  uint64_t *message_id_inout_)
{
    ...
    if (message_id == 0) {
        ...
        if (!queue_has_room (state_->pending.remote_mesh.pending_bytes,
                             state_->pending.remote_mesh.pending_hard_limit, entry.encoded_bytes)) {
            errno = EAGAIN;
            return false;
        }
        if (copy_msg_parts_to_owned (parts_, &entry.parts) != 0)
        ...
        state_->pending.remote_mesh.pending_bytes += entry.encoded_bytes;
        state_->pending.remote_mesh.pending_messages.emplace (message_id, std::move (entry));
        if (message_id_inout_)
            *message_id_inout_ = message_id;
    }
    ...
    target_->pending_message_ids.push_back (message_id);
    spot_data_plane_pending_state_t::remote_mesh_state_t::pending_message_map_t::iterator it =
      state_->pending.remote_mesh.pending_messages.find (message_id);
    if (it != state_->pending.remote_mesh.pending_messages.end ())
        ++it->second.remaining_targets;
    return true;
}
  ...
  size_t precomputed_encoded_bytes_)
{
    ...
    state_->pending.remote_mesh.broadcast_pending_message_ids.push_back (message_id);
    spot_data_plane_pending_state_t::remote_mesh_state_t::pending_message_map_t::iterator it =
      state_->pending.remote_mesh.pending_messages.find (message_id);
    if (it != state_->pending.remote_mesh.pending_messages.end ())
        ++it->second.remaining_targets;
