...
uint32_t registry_t::get (int option_) const
{
    ...
    const uint32_t value =
      zlink_registry_get (_impl->handle, static_cast<zlink_registry_option_t> (option_), &err);
...
void registry_t::set_tls_server (const std::string &cert_,
...
void registry_t::set_tls_client (const std::string &ca_cert_,
...
registry_t::service_summary (const registry_service_summary_filter_t *filter_) const
{
    ...
    size_t count = 0;
    detail::throw_if_failed<config_error_t> (static_cast<config_result_t> (
      zlink_registry_service_summary (_impl->handle, filter_ptr, nullptr, &count)));
...
std::vector<registry_topology_entry_t> registry_t::topology () const
{
    ...
    if (count > 0) {
        detail::throw_if_failed<config_error_t> (static_cast<config_result_t> (
          zlink_registry_topology (_impl->handle, nullptr, native.data (), &count)));
        native.resize (count);
    }
