...
| 현재 pattern | Single `ROUTER_ROUTER` 진행 중 | tcp를 완료했고 다음 transport는 ws다. |
| paired C | .NET `ROUTER_ROUTER / tcp` 완료 | C 전체 크기 5회 뒤 perf 의미를 바로잡고 .NET 전체 크기를 5회 측정했다. |
| 개선 반복 | .NET `ROUTER_ROUTER / tcp` 완료 | 131072B 처리량을 27.61K에서 52.67Kmsg/s로 개선하고 전체 최소 78.7%, 중앙값 93.9%를 확인했다. |
| 커밋과 푸시 | .NET ROUTER_ROUTER payload 의미 정렬 반영 중 | perf 수정, 기능 test, 측정 근거를 함께 커밋하고 푸시한다. |
...
| 2 | .NET | `PAIR`, `PUBSUB`, `DEALER_DEALER`, `DEALER_ROUTER`, `DEALER_ROUTER_REQREP` 완료, `ROUTER_ROUTER` tcp 완료 | 미측정 | `ROUTER_ROUTER / ws` 전체 크기를 측정한다. |
