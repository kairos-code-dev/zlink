...
public final class RcA4DiLifecycleScenario {
    ...
    public static void run(ScenarioContext context) {
        List<Contracts.DiLifecycleReply> replies = new ArrayList<>();
        ...
        ScenarioAssert.ensure(replies.stream().map(Contracts.DiLifecycleReply::scopedId).distinct().count() == 3,
            "RC-A4 scoped dependency was not recreated per request: " + replies);
        ScenarioAssert.ensure(replies.stream().map(Contracts.DiLifecycleReply::singletonId).distinct().count() == 1,
