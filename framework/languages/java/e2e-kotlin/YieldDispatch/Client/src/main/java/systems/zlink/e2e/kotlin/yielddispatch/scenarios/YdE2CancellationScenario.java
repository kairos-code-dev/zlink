...
public final class YdE2CancellationScenario {
    ...
    public static void run(ZLinkStreamConnector connector) {
        String requestId = "YD-E2-" + UUID.randomUUID();
        ClientStreamSupport.send(
            connector.send(new Contracts.YieldCancelMsg(requestId, 800, 100))
        ...
        ClientStreamSupport.send(
            connector.send(new Contracts.SpotProbeMsg(requestId, "cancel-probe"))
    ...
    private static void waitForMarkers(
        ...
        throw new IllegalStateException(
            "YD-E2 markers not observed in order: expected=" + expected + " actual=" + latest);
    }
