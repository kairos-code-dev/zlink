                                  ...
                                  RunPhaseFn run_phase_fn)
{
    ...
    for (size_t i = 0; i < msg_sizes.size(); ++i) {
        ...
        if (!wait_start_fn(msg_size, start_timeout_ms)
...
template<typename PrepareFn,
         typename WaitControlStartFn,
         typename RunActiveFn,
         typename FinishFn>
inline bool run_spot_client_case(size_t msg_size,
                                 ...
                                 FinishFn finish_fn)
{
    if (!prepare_fn(msg_size, phase_timeout_ms)
        || !wait_control_start_fn(phase_timeout_ms)
        || !run_active_fn()
        || !finish_fn(msg_size)) {
        return false;
    }
