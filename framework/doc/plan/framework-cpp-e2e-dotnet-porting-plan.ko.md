...
| `RegistrationCodec` | RC-A1/A3/A4/A5/A6/B1/B2/B3/B4/B5 완료, RC-A2 public gap | `.NET` 기준 inventory와 C++ client scenario/support, server configuration/handler/endpoint/support, invalid role, JSON-only peer role 분리가 있다. `timeout 420s framework/languages/cpp/e2e/RegistrationCodec/run_e2e.sh` 최신 통과 증거는 `logs/20260701-162045-82879`이며 RC-A1, RC-A3, RC-A4, RC-A5, RC-A6, RC-B1, RC-B2, RC-B3, RC-B4, RC-B5를 검증한다. 최종 `registration-codec e2e result=passed` marker는 RC-A6 invalid startup checks까지 끝난 뒤 runner가 한 번만 출력한다. RC-A2는 공통 E2E가 annotation/decorator 기반 request/send handler 등록을 요구하지만 C++ public API에는 대응 annotation/decorator 계약이 없으므로, helper macro나 테스트 전용 scan 규칙으로 대체하지 않고 feature-map gap으로 유지한다. 공개 계약 후보는 `framework/doc/framework/common/draft/handler-registration-annotations.ko.md`에 분리했다. |
...
| `YieldDispatch` | YD-A/B/C/D/E1/E3/E4/E5 완료, YD-E2 public gap | C++ e2e config와 runner가 생겼고 YD-A1~YD-A4, YD-B1~YD-B3, YD-C1~YD-C3, YD-D1~YD-D4, YD-E1, YD-E3, YD-E4 static gate와 YD-E5 report 생성은 runner 통과 증거가 있다. 최신 `ZLINK_CPP_E2E_BUILD_DIR=framework/languages/cpp/build timeout 420s framework/languages/cpp/e2e/YieldDispatch/run_e2e.sh` 통과 증거는 `logs/20260701-191329-11276`이며 `scenario YD-E5 passed`와 `yield-dispatch-report.json`을 확인한다. runner readiness와 client connect timeout은 로컬 기준 3초로 낮췄다. session actor bind는 public `actor_gateway_t`/`session_actor_manager_t`로 user Spot에 join한 actor ref를 session에 bind하고, Play user Spot actor handler가 actor yield/join/push를 처리한다. Entry Spot에는 actor admission을 위한 fast handler만 둔다. route request backend가 reply 대기 전체를 직렬화하지 않고 native ROUTER/bridge submit 구간만 짧게 보호하도록 수정해, B1/B3/C3의 교차 실행선 검증은 session-b observer 없이 같은 session-a connector에서 `.NET`과 같은 in-flight actor/spot request 경로로 통과한다. D3는 `yield-released` evidence를 확인한 뒤 probe를 보내 route bridge scheduling 차이 없이 yield turn release를 검증한다. 구현된 client scenario header는 YD-A/B/C/D2~D4/E1/E3까지 완료했고, Registry host factory/support, Delay host factory/handler/support, Play host factory/support/control/basic/timer/actor/remote/failure handler, spot type, `YieldProbeSpot` runtime, Session host factory/support, `yield_session_t` support header도 분리했다. 남은 미구현 항목은 YD-E2 public yield cancellation token contract gap이다. YD-E2는 공통 E2E가 cancellation cleanup을 요구하지만 C++ `cpp-framework-interfaces` spec이 public cancellation token/registration을 제공하지 않는다고 명시하고 현재 public `yield()` 표면도 token 인자를 받지 않으므로, 내부 pending cancel이나 handler-local timeout으로 대체하지 않는다. 공개 계약 후보는 `framework/doc/framework/common/draft/yield-cancellation.ko.md`에 분리했다. |
...
- `ZLINK_CPP_E2E_BUILD_DIR=framework/languages/cpp/build timeout 420s framework/languages/cpp/e2e/YieldDispatch/run_e2e.sh`
  - 결과: passed
  - 로그: `framework/languages/cpp/e2e/YieldDispatch/logs/20260701-191329-11276`
  - 비고: local readiness와 stream connect timeout은 3초 기준이다. YD-E3 shutdown wait도 3초 기본 wait helper를 사용하며, shutdown path의 session route spot request는 2초 안에 public `remote_error`를 올려 client 자체 request timeout과 경쟁하지 않게 한다. native route backend는 request reply 대기 전체를 잠그지 않고 ROUTER/spot bridge submit 구간만 짧게 보호하고, spot route bridge lifetime은 local `shared_ptr`로 보호한다. `play-a.evidence.log`에서 YD-C3A 순서가 `actor-yield-started`, `actor-yield-released`, `timer-fast-started`, `timer-fast-completed`, `actor-yield-resumed`, `actor-yield-completed`로 확인됐다. YD-E2는 public cancellation token contract gap으로 유지한다. READ-ONLY 재리뷰 결과는 `이슈 없음`이다.

남은 public contract gap:

- `RegistrationCodec`의 `RC-A2`: 공통 E2E는 annotation/decorator 기반 request/send handler 등록을 요구한다. C++ public API에는 대응 annotation/decorator 계약이 없으므로 helper macro나 테스트 전용 scan 규칙으로 구현하지 않는다. 공개 계약 후보는 `framework/doc/framework/common/draft/handler-registration-annotations.ko.md`에 둔다.
- `YieldDispatch`의 `YD-E2`: 공통 E2E는 yield cancellation cleanup을 요구한다. C++ public `yield()` 표면에는 cancellation token 인자가 없고 C++ framework spec은 public cancellation token/registration을 제공하지 않는다고 명시하므로, 내부 pending cancel이나 timeout으로 대체하지 않는다. 공개 계약 후보는 `framework/doc/framework/common/draft/yield-cancellation.ko.md`에 둔다.
...
- `RegistrationCodec`은 `logs/20260701-162045-82879`에서 RC-A1, RC-A3, RC-A4, RC-A5,
  RC-A6, RC-B1, RC-B2, RC-B3, RC-B4, RC-B5 marker와 최종 `registration-codec e2e result=passed`
  marker를 확인했다. RC-A2는 annotation/decorator public contract gap으로 유지하고, JSON 기본 codec
  경로를 메시지별 등록으로 우회한 흔적은 없다는 READ-ONLY 리뷰 결과 `이슈 없음`을 받았다.
  runner readiness는 local 기준 3초다.
...
- `SpotService`는 `logs/20260701-183404-20982` full sweep과 `SM-D5`/`SM-G3` focused 재확인으로
  ...
  결과는 `이슈 없음`이다.
- `YieldDispatch`는 `logs/20260701-191329-11276`에서 YD-A1~YD-E5 구현 범위 marker와 최종
  ...
  메시지별 codec 등록 미사용을 READ-ONLY 재리뷰로 재확인했고, 결과는 `이슈 없음`이다.
