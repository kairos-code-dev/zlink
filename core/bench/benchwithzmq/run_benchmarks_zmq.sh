...
usage() {
  cat <<'USAGE'
Usage: core/bench/benchwithzmq/run_benchmarks_zmq.sh [options]
...
RUN_CMD=("${PYTHON_BIN[@]}" "${ROOT_DIR}/core/bench/benchwithzmq/run_comparison_zmq.py" "${PATTERN}" --build-dir "${BUILD_DIR}" --runs "${RUNS}")
RUN_ENV=()
