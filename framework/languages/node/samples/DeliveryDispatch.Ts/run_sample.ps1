...
try {
    ...
    $ports = 1..17 | ForEach-Object { New-FreePort }
    $env:DELIVERYDISPATCH_REGISTRY_PUB = "tcp://127.0.0.1:$($ports[0])"
    $env:DELIVERYDISPATCH_REGISTRY = "tcp://127.0.0.1:$($ports[1])"
    $env:DELIVERYDISPATCH_API_HTTP = "http://127.0.0.1:$($ports[2])"
    $env:DELIVERYDISPATCH_CENTER_ROUTE = "tcp://127.0.0.1:$($ports[3])"
    $env:DELIVERYDISPATCH_COURIER_ROUTE = "tcp://127.0.0.1:$($ports[4])"
    $env:DELIVERYDISPATCH_COURIER_STREAM = "tcp://127.0.0.1:$($ports[5])"
    $env:DELIVERYDISPATCH_COURIER_ACTOR_NODE1_ROUTE = "tcp://127.0.0.1:$($ports[6])"
    $env:DELIVERYDISPATCH_COURIER_ACTOR_NODE2_ROUTE = "tcp://127.0.0.1:$($ports[7])"
    $env:DELIVERYDISPATCH_COURIER_ACTOR_NODE1_SPOT = "tcp://127.0.0.1:$($ports[8])"
    $env:DELIVERYDISPATCH_COURIER_ACTOR_NODE2_SPOT = "tcp://127.0.0.1:$($ports[9])"
    $env:DELIVERYDISPATCH_TRACKING_ROUTE = "tcp://127.0.0.1:$($ports[10])"
    $env:DELIVERYDISPATCH_STATUS_FANOUT = "tcp://127.0.0.1:$($ports[11])"
    $env:DELIVERYDISPATCH_TRACKING_SPOT_ROUTER = "tcp://127.0.0.1:$($ports[12])"
    $env:DELIVERYDISPATCH_TRACKING_SPOT = "tcp://127.0.0.1:$($ports[13])"
    $env:DELIVERYDISPATCH_SESSION_STREAM = "tcp://127.0.0.1:$($ports[14])"
    $env:DELIVERYDISPATCH_SESSION_SPOT_ROUTER = "tcp://127.0.0.1:$($ports[15])"
    $env:DELIVERYDISPATCH_SESSION_SPOT = "tcp://127.0.0.1:$($ports[16])"

    Start-Role "registry"
    Wait-Port "registry-router" $env:DELIVERYDISPATCH_REGISTRY
