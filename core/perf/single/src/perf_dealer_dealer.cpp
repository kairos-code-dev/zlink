...
struct dealer_callback_state_t
{
    dealer_callback_state_t () :
        run_id (0),
        msg_size (0),
        payload_size (0),
...
inline bool wait_for_receive_quiet (dealer_callback_state_t &state_,
...
void dealer_recv_handler (const zlink_routing_id_t *,
                          zlink_msg_t *parts_,
                          size_t part_count_,
                          void *userdata_)
{
    dealer_callback_state_t *state =
      static_cast<dealer_callback_state_t *> (userdata_);
    if (!state) {
                        ...
                        const std::string &lib_name)
{
    ...
    dealer_callback_state_t callback_state;
    single_callback_metric_queue_t callback_queue (65536);
    single_metric_worker_t<dealer_callback_state_t> metric_worker;
    ...
    if (zlink_recv_handler (s1.get (), &dealer_recv_handler, &callback_state)
        != 0) {
