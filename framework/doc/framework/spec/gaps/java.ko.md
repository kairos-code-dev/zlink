...
- [ ] **SMP-JV-01** (**절대 규칙 위반**) — TicTacToe 밖 샘플이 **수동 연결을 쓴다**(live 재검증 15곳 → 4곳)
  - 재검증 재개: GameQuest의 네 channel client는 location store 자동 발견으로 바뀌었고 금지 gate도 통과한다. 그러나 전체 runner 반복 실행에서 mission-a 재기동 직후 rehydrate 요청이 `remote error payload is invalid`로 실패해 최초 exit 0만으로 자동 재연결 안정성을 증명하지 못했다. 공개 `ZLinkLocationReadiness`는 있으나 Spring starter가 bean으로 제공하지 않으며, 샘플이 runtime 내부 타입을 가져오거나 업무 요청을 retry해서 숨길 수 없다. 선택지는 starter의 public readiness bean 제공 또는 runtime channel request의 자동 재연결 대기 계약이다. 부분 구현 커밋 `311e8304c`(2026-07-15).
