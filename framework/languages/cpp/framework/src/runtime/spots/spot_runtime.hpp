...
class spot_context_state_t
{
  public:
    bool close_now ()
    {
        ...
        for (auto iterator = node->spot_rids_by_name.begin (); iterator != node->spot_rids_by_name.end ();
             ++iterator) {
...
class spot_node_runtime_t
{
  public:
    ...
    spot_create_result_t get_or_create_spot (std::string spot_name, spot_rid_t spot_rid, zlink::message_t request);
                                                     ...
                                                     const zlink::message_t &request)
    {
        ...
        if constexpr (!has_actor_join_callback<TSpot, TActor>) {
            return result_t<actor_join_reply_t>::failure (framework_error_kind_t::handler_not_found,
            ...
            const auto committed = commit_actor_to_context<TSpot, TActor> (actor_ref, actor, *context);
            return result_t<actor_join_reply_t>::success (
    ...
    result_t<actor_ref_t> join_actor_to_entry_spot (const actor_ref_t &actor_ref,
                                                    ...
                                                    TActor &actor)
    {
        ...
        const auto committed = commit_actor_to_context<TEntrySpot, TActor> (actor_ref, actor, *context);
        return result_t<actor_ref_t>::success (committed);
    }

    template <typename TActor> result_t<void> leave_actor (const actor_ref_t &actor_ref, TActor &actor)
    {
        if (actor_ref.empty ()) {
            return result_t<void>::failure (framework_error_kind_t::actor_route_not_found, "actor ref is empty");
        }
  ...
  private:
    ...
    static constexpr bool has_actor_join_callback =
      requires (TSpot &spot, TActor &actor, const zlink::message_t &request) {
          { spot.on_actor_join (actor, request) } -> std::same_as<spot_actor_join_response_t>;
      };
    ...
    template <typename TSpot, typename TActor>
    actor_ref_t commit_actor_to_context (const actor_ref_t &actor_ref, TActor &actor, spot_context_t &context)
    {
        commit_actor_left<TActor> (actor_ref, actor);
        ...
        context_state.post_actor_joined_callbacks[std::type_index (typeid (TActor))] = [] (void *spot, void *actor) {
            if constexpr (has_post_actor_joined_callback<TSpot, TActor>) {
                static_cast<TSpot *> (spot)->on_post_actor_joined (*static_cast<TActor *> (actor));
            }
        };
        context_state.actor_left_callbacks[std::type_index (typeid (TActor))] = [] (void *spot, void *actor) {
    ...
    template <typename TActor> void notify_post_actor_joined (spot_context_state_t &state, TActor &actor)
    {
        const auto found = state.post_actor_joined_callbacks.find (std::type_index (typeid (TActor)));
