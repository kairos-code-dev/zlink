...
std::string bind_client_spot_endpoint (void *node, const std::string &transport, size_t slot_index)
{
    ...
    if (perf_set_spot_node_router_endpoint_raw (
          node, make_endpoint (transport, "perf-multi-spot-reqrep-client-router-"
...
bool wait_for_ready_settle (spot_reqrep_client_state_t *state)
{
    ...
    return idle_until (std::chrono::steady_clock::now () + std::chrono::milliseconds (settle_ms),
                       state);
}
...
bool recv_one_control_message (spot_reqrep_client_state_t *state, bool *received)
{
    ...
    if (perf_multi_spot_handshake::parse_start_command (payload.data (), payload.size (),
                                                        &started_size)) {
        state->control_started_msg_size.store (started_size, std::memory_order_release);
        state->cv.notify_all ();
    }
    return true;
}
...
bool wait_for_started_size (spot_reqrep_client_state_t *state, size_t msg_size, int timeout_ms)
{
    return perf_multi_spot_control::wait_for_started_size (state, msg_size, timeout_ms,
                                                           recv_one_control_message);
}

bool wait_for_runner_start (spot_reqrep_client_state_t *state, size_t msg_size, int timeout_ms)
{
    if (!state)
        return false;
...
bool publish_ready_count (spot_reqrep_client_state_t *state, size_t msg_size)
{
    ...
    return perf_multi_spot_control::publish_ready_count (state->control_pub, k_topic, msg_size,
                                                         state->slots.size ());
}
...
bool publish_data_endpoint (spot_reqrep_client_state_t *state)
{
    ...
    return perf_multi_spot_control::publish_data_endpoint (state->control_pub, k_topic,
                                                           state->data_endpoint);
}
                             ...
                             size_t msg_size)
{
    ...
    if (!wait_for_control_link_ready (state, settings.connect_ready_timeout_ms)) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-reqrep-client] control link ready timeout size=" << msg_size
                      << std::endl;
        return false;
    }
    if (!wait_for_ready_settle (state)) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-reqrep-client] ready settle failed size=" << msg_size
                      << " err=" << zlink_errno () << std::endl;
        return false;
    }
    if (!publish_data_endpoint (state)) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-reqrep-client] publish DATA_ENDPOINT failed size=" << msg_size
                      << " err=" << zlink_errno () << std::endl;
        return false;
    }
    if (!wait_for_control_settle (state)) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-reqrep-client] data endpoint settle failed size=" << msg_size
                      << " err=" << zlink_errno () << std::endl;
        return false;
    }
    if (!wait_for_data_link_ready (state, settings)) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-reqrep-client] data link ready timeout size=" << msg_size
                      << " err=" << zlink_errno () << std::endl;
        return false;
    }
    if (!publish_connected_once (state)) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-reqrep-client] publish CONNECTED failed size=" << msg_size
                      << " err=" << zlink_errno () << std::endl;
        return false;
    }
    if (!wait_for_control_settle (state)) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-reqrep-client] control settle failed size=" << msg_size
                      << " err=" << zlink_errno () << std::endl;
        return false;
    }
    if (!publish_ready_count (state, msg_size)) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-reqrep-client] publish READY_COUNT failed size=" << msg_size
                      << " err=" << zlink_errno () << std::endl;
        return false;
    }
                          ...
                          spot_reqrep_client_state_t *state)
{
    ...
    if (!state->control_pub || !state->control_sub
        || !perf_multi_spot_control::apply_control_options (state->control_pub, state->control_sub,
                                                            settings)) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-reqrep-client] control spot init failed err="
                      << zlink_errno () << std::endl;
        return false;
    }
    ...
    if (state->control_endpoint.empty ()) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-reqrep-client] control bind failed err=" << zlink_errno ()
                      << std::endl;
        return false;
    }
    ...
    if (zlink_spot_node_connect_peer (state->control_node, server_control_endpoint.c_str ())
          != ZLINK_CONNECT_OK
        || zlink_set_subscription (state->control_sub, k_topic) != ZLINK_CONFIG_OK) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-reqrep-client] control connect/subscribe failed err="
                      << zlink_errno () << std::endl;
        return false;
    }
                        ...
                        spot_reqrep_client_state_t *state)
{
    ...
    if (!state->data_node || !setup_tls_server (state->data_node, transport)
        || !setup_tls_client (state->data_node, transport)) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-reqrep-client] data node init failed err=" << zlink_errno ()
                      << std::endl;
        return false;
    }
    if (!apply_benchmark_context_auto_hwm_msg_unit (ctx.get (), max_msg_size))
    ...
    const std::string local_endpoint = bind_client_spot_endpoint (state->data_node, transport, 0);
    if (local_endpoint.empty ()) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-reqrep-client] data node bind failed err=" << zlink_errno ()
                      << std::endl;
        return false;
    }
    if (zlink_spot_node_connect_peer (state->data_node, endpoint.c_str ()) != ZLINK_CONNECT_OK) {
        if (bench_debug_enabled ())
    ...
    for (size_t i = 0; i < state->slots.size (); ++i) {
        ...
        if (!slot.socket) {
            if (bench_debug_enabled ())
                std::cerr << "[multi-spot-reqrep-client] spot create failed slot=" << i
                          << " err=" << zlink_errno () << std::endl;
            return false;
        }

        apply_benchmark_socket_options (slot.socket, settings.hwm, transport, ZLINK_SOCKET_DEALER,
                                        max_msg_size);
        const std::string routing_id = std::string ("SPOT-REQREP-") + std::to_string (i);
        if (zlink_set_routing_id (slot.socket, routing_id.c_str (), routing_id.size ()) != 0) {
            if (bench_debug_enabled ())
                std::cerr << "[multi-spot-reqrep-client] spot routing id init failed slot=" << i
                          << " err=" << zlink_errno () << std::endl;
            return false;
        }
    }

    perf_print_auto_hwm_snapshot (state->data_node, true, "spotnode_data_pub", transport);
    if (!state->slots.empty ()) {
        perf_print_auto_hwm_snapshot (state->slots[0].socket, true, "spotend_data_pub", transport);
    }

    state->poller = zlink_poller_new ();
    if (!state->poller) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-reqrep-client] poller create failed err=" << zlink_errno ()
                      << std::endl;
        return false;
    }
    ...
    for (size_t i = 0; i < state->slots.size (); ++i) {
        if (zlink_poller_add (state->poller, state->slots[i].socket, &state->slots[i],
            ...
            != 0) {
            if (bench_debug_enabled ())
                std::cerr << "[multi-spot-reqrep-client] poller add failed slot=" << i
                          << " err=" << zlink_errno () << std::endl;
            return false;
        }
    }
...
void on_request_reply (zlink_request_result_t result,
                       ...
                       void *userdata)
{
    spot_reqrep_client_slot_t *slot = static_cast<spot_reqrep_client_slot_t *> (userdata);
    if (!slot || !slot->owner)
        return;
    ...
    double sample_ns = 0.0;
    {
        ...
        if (now_ns >= state->active_deadline_ns.load (std::memory_order_acquire)
            || header.sent_ts_ns <= 0 || now_ns < static_cast<uint64_t> (header.sent_ts_ns)) {
            return;
        }

        sample_ns = static_cast<double> (now_ns - static_cast<uint64_t> (header.sent_ts_ns)) / 2.0;
        state->active_reply_count.fetch_add (1, std::memory_order_acq_rel);
        state->active_latency.add (sample_ns);
    }
    if (spot_trace_enabled () && slot->index == 0
        && g_client_trace_reply_logs.fetch_add (1, std::memory_order_acq_rel) < 8) {
        std::cerr << "[multi-spot-reqrep-trace] reply slot=" << slot->index << " seq=" << header.seq
                  << " sample_ms=" << (sample_ns / 1000000.0) << " rtt_ms="
                  << ((now_ns - static_cast<uint64_t> (header.sent_ts_ns)) / 1000000.0)
                  << std::endl;
    }
    if (bench_debug_enabled ()
        && g_client_debug_recv_logs.fetch_add (1, std::memory_order_acq_rel) < 8) {
        std::cerr << "[multi-spot-reqrep-client] recv reply slot=" << slot->index
...
send_result_t send_request (spot_reqrep_client_slot_t *slot,
                            ...
                            uint32_t timeout_ms)
{
    if (!slot || !slot->owner || !slot->socket || !server_node_rid || !server_spot_rid) {
        errno = EINVAL;
        return send_result_error;
    }

    spot_reqrep_client_state_t *state = slot->owner;
    const size_t payload_size = std::max (msg_size, perf_multi_metric::header_size ());
    ...
    const zlink_submit_result_t rc =
      perf_zlink_spot_request_spot_parts (slot->socket, server_node_rid, server_spot_rid, &part, 1,
                                          on_request_reply, slot, ZLINK_DONTWAIT, timeout_ms);
    ...
    switch (rc) {
        case ZLINK_SUBMIT_OK:
            if (spot_trace_enabled () && slot->index == 0
                && g_client_trace_send_logs.fetch_add (1, std::memory_order_acq_rel) < 8) {
                std::cerr << "[multi-spot-reqrep-trace] send slot=" << slot->index
                          << " seq=" << slot->next_seq << " timeout_ms=" << timeout_ms << std::endl;
            }
            if (bench_debug_enabled ()
                && g_client_debug_send_logs.fetch_add (1, std::memory_order_acq_rel) < 8) {
                std::cerr << "[multi-spot-reqrep-client] send ok slot=" << slot->index
                          << " seq=" << slot->next_seq << std::endl;
            }
            slot->send_pending.store (false, std::memory_order_release);
            ++slot->next_seq;
        ...
        case ZLINK_SUBMIT_NOT_CONNECTED:
            if (bench_debug_enabled ()
                && g_client_debug_send_logs.fetch_add (1, std::memory_order_acq_rel) < 8) {
                std::cerr << "[multi-spot-reqrep-client] send not connected slot=" << slot->index
                          << std::endl;
            }
            return send_result_not_connected;
        default:
            if (bench_debug_enabled ()) {
                std::cerr << "[multi-spot-reqrep-client] send failed slot=" << slot->index
                          << " rc=" << rc << " err=" << zlink_errno () << std::endl;
            }
            return send_result_error;
    }
}
...
int poll_timeout_until (const std::chrono::steady_clock::time_point &deadline, int max_wait_ms)
{
...
bool any_waiting_reply (const spot_reqrep_client_state_t *state, size_t active_slots)
{
    if (!state)
        return false;
    ...
    while (any_waiting_reply (state, active_slots)
        ...
        if (event_count < 0) {
            if (zlink_errno () == EINTR)
                continue;
            if (bench_debug_enabled ()) {
                std::cerr << "[multi-spot-reqrep-client] drain poller wait failed err="
                          << zlink_errno () << std::endl;
            }
            return false;
        }
...
{
    if (!client_state || !server_node_rid || !server_spot_rid || !send_progress_out
    ...
    *has_waiting_reply_out = false;
    const size_t active_slots = active_spot_slot_limit (client_state->slots.size (), msg_size);
    for (size_t i = 0; i < active_slots; ++i) {
        spot_reqrep_client_slot_t &slot = client_state->slots[i];
        const bool waiting_reply = slot.waiting_reply.load (std::memory_order_acquire);
        const bool send_pending = slot.send_pending.load (std::memory_order_acquire);
        if (waiting_reply || send_pending) {
            *has_waiting_reply_out = true;
            continue;
        }

        const send_result_t send_rc = send_request (&slot, server_node_rid, server_spot_rid, run_id,
                                                    msg_size, request_timeout_ms);
        if (send_rc == send_result_ok) {
            *send_progress_out = true;
        } else if (send_rc == send_result_blocked) {
            if (bench_debug_enabled ()
                && g_client_debug_send_logs.fetch_add (1, std::memory_order_acq_rel) < 8) {
                std::cerr << "[multi-spot-reqrep-client] send blocked slot=" << slot.index
                          << std::endl;
            }
        } else if (send_rc != send_result_not_connected) {
            if (bench_debug_enabled ()) {
                std::cerr << "[multi-spot-reqrep-client] send request fatal slot=" << slot.index
                          << " err=" << zlink_errno () << std::endl;
            }
            return false;
        }
                        ...
                        bench_latency_stats_t *latency_out)
{
    ...
    *latency_out = bench_latency_stats_t ();
    ...
    const uint32_t request_timeout_ms = static_cast<uint32_t> (bench_timeout_ms_from_env (
      "PERF_MULTI_RCVTIMEO_MS", bench_timeout_ms_from_env ("PERF_MULTI_SNDTIMEO_MS", 200)));
    const size_t active_slots = active_spot_slot_limit (state->slots.size (), msg_size);

    while (std::chrono::steady_clock::now () < deadline) {
        bool send_progress = false;
        bool has_waiting_reply = false;
        if (!try_submit_ready_requests (state, &server_node_rid, &server_spot_rid, run_id, msg_size,
                                        request_timeout_ms, &send_progress, &has_waiting_reply))
            return false;
        if (send_progress)
            continue;
        ...
        if (event_count < 0) {
            if (zlink_errno () == EINTR)
                continue;
            if (bench_debug_enabled ())
                std::cerr << "[multi-spot-reqrep-client] poller wait failed err=" << zlink_errno ()
                          << std::endl;
            return false;
        }
    }
    if (!drain_pending_replies (state, active_slots, request_timeout_ms))
        return false;

    {
        std::lock_guard<std::mutex> lock (state->mutex);
        *reply_count_out = state->active_reply_count.load (std::memory_order_acquire);
                           ...
                           uint32_t run_id)
{
    if (!apply_benchmark_context_auto_hwm_msg_unit (ctx, msg_size))
        return false;
    const int phase_timeout_ms = std::max (settings.connect_ready_timeout_ms,
    ...
    if (!wait_for_runner_start (state, msg_size, phase_timeout_ms)) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-reqrep-client] runner start timeout size=" << msg_size
                      << std::endl;
        return false;
    }
    if (!wait_for_started_size (state, msg_size, phase_timeout_ms)) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-reqrep-client] control START timeout size=" << msg_size
                      << std::endl;
        return false;
    }
    ...
    if (!run_active_window (state, settings, run_id, msg_size, &reply_count, &latency)) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-reqrep-client] active window failed size=" << msg_size
                      << " err=" << zlink_errno () << std::endl;
        return false;
    }
    if (reply_count == 0 || latency.mean_ns <= 0.0) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-reqrep-client] metrics invalid size=" << msg_size
                      << " replies=" << reply_count << " latency_mean=" << latency.mean_ns
                      << std::endl;
        return false;
    }
    ...
    print_echo_client_result_lines (k_pattern, lib_name, transport, msg_size, throughput, latency);
    return true;
}
                          ...
                          size_t fallback_size)
{
    ...
    if (!create_control_spot (ctx, transport, control_endpoint, settings, max_msg_size, &state)) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-reqrep-client] create_control_spot failed" << std::endl;
        destroy_client_state (&state);
        return 1;
    }
    perf_multi_spot_control::start_client_stdin_watcher (
      &state, [] (spot_reqrep_client_state_t *client_state, size_t start_size) {
    ...
    if (!create_spot_slots (ctx, transport, endpoint, settings, max_msg_size, &state)) {
        if (bench_debug_enabled ())
            std::cerr << "[multi-spot-reqrep-client] create_spot_slots failed" << std::endl;
        fast_exit_process (1);
    }
    ...
    for (size_t i = 0; i < msg_sizes.size (); ++i) {
        if (bench_debug_enabled ()) {
            std::cerr << "[multi-spot-reqrep-client] size begin=" << msg_sizes[i] << std::endl;
        }
        if (!run_single_size_case (&state, ctx.get (), settings, lib_name, transport, msg_sizes[i],
                                   static_cast<uint32_t> (i + 1))) {
            if (bench_debug_enabled ()) {
                std::cerr << "[multi-spot-reqrep-client] size failed=" << msg_sizes[i]
                          << " err=" << zlink_errno () << std::endl;
            }
            rc = 1;
            break;
        ...
        if (bench_debug_enabled ()) {
            std::cerr << "[multi-spot-reqrep-client] size done=" << msg_sizes[i] << std::endl;
        }
    }
