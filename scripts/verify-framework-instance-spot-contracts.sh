...
const formalFixtures = [
  {
    ...
    required: [
      ...
      'CAS winner만 factory와 initialize를 실행하고 Ready commit 뒤 envelope message를 local queue에 한 번 제출한다.',
  ...
  },
  {
    ...
    required: [
      'Instance Spot의 Missing authority는 Spot direct fluent call이 명시한 Instance intent로 cold activation할 수 있다.',
      'Spot direct send/request의 시작 method는 global Spot RID만 대상으로 받는다.',
      'Spot 전용 call builder가 Instance intent를 갖지 않으면 Missing Spot에서 target-not-found로 끝난다.',
      '생략한 경우 선택한 Mesh에 등록된 distinct Instance type이 하나일 때만 자동 선택한다.',
      'Missing Instance activation에서는 source가 owner claim을 선점하지 않고 최초 message를 포함한 activation envelope를 target에 제출하며, target의 CAS winner만 owner claim과 factory를 만든다.',
  ...
  },
  {
    ...
    required: [
      ...
      'Source는 target transport 전 owner claim이나 `Creating` authority를 만들지 않는다.',
      'Target의 CAS winner만 owner claim·factory를 만들고 Ready barrier 뒤 envelope message를 local queue에 한 번 제출한다.',
      'Spot manager의 public `Close`는 User Spot의 exact `SpotRef`를 받는다.',
    ],
  },
  {
    path: 'framework/doc/framework/spec/server/23-spot-actor.ko.md',
    required: [
      ...
      'Instance Spot은 source가 최초 message를 포함한 activation envelope를 target에 제출하고, target runtime이 local exact instance가 없을 때 자신을 owner로 reservation을 획득한다.',
  ...
  },
  {
    ...
    required: [
      ...
      'Instance Spot은 Manager create family를 제공하지 않으며 §6.1의 Spot direct cold activation으로만 Missing authority를 만든다.',
      'Spot direct call은 기본적으로 existing-only다. Instance intent가 없는 call은 Missing authority에서 target-not-found로 끝난다.',
      'Stable type을 생략했을 때 선택한 Mesh의 serving descriptor에 등록된 distinct Instance type이 하나면 자동 선택한다.',
      'Source는 전송 전에 owner claim, Missing→Pending reservation 또는 synthetic generation을 만들지 않는다.',
      'Target의 CAS winner만 factory와 initialize를 실행한다.',
...
const projections = {
  dotnet: [
    ...
    'Target runtime은 local exact instance가 없을 때만 자신을 owner로 `Creating` row와 pending capacity를 Reserve하고',
  ],
  cpp: [
    ...
    '`spot_send_call_t` 또는 `spot_request_call_t`에서 `instance_spot()`을 선택한 direct call만 missing RID의 cold activation을 시작한다.',
    '`instance_spot()`은 stable type을 생략한 marker이고, `instance_spot(stable_type)`은 type을 명시한 marker다.',
    'distinct Instance Spot type이 0개이면 `spot_route_not_found`, 둘 이상이면 `invalid_configuration`이다.',
    'class spot_manager_t {',
    'virtual spot_create_call_t create(std::string stable_type) = 0;',
    'virtual spot_create_call_t get_or_create( spot_rid_t spot_rid, std::string stable_type) = 0;',
    ...
    'enum class spot_close_reason_t { explicit_close = 0, host_shutdown = 1, relocation_out = 2 };',
    ...
    'Target runtime은 local exact instance가 없을 때만 자신을 owner로 generic Store Reserve를 수행하고',
  ],
  java: [
    ...
    'Instance Spot은 source-side reservation을 사용하지 않는다.',
    'Target runtime은 local exact instance를 확인하고, 없을 때만 자신을 owner로 `CREATING` authority와 pending capacity를 Reserve한 뒤',
  ],
  kotlin: [
    ...
    '`getOrCreate(spotRid, spotType)`은 caller가 정한 User Spot RID를 사용한다.',
    'Manager는 Instance Spot create/get-or-create를 제공하지 않는다.',
    'Spot send/request는 global SpotRid만 address로 받고 Java의 `ZLinkSpotSendCall` 또는 `ZLinkSpotRequestCall`을 반환한다.',
    ...
    'Kotlin은 address DTO, process-local handle, resolver, unbounded directory와 User Spot create/get-or-create 또는 Instance messaging의 direct terminal extension을 제공하지 않는다.',
    '`close(SpotRef)`는 Missing이면 `false`, generation 불일치는 `SpotGenerationStale`',
    '`ZLinkInstanceSpotContext.close()`를 그대로 사용한다.',
    '`ZLinkSpotCloseReason`을 사용하며 값은 `EXPLICIT_CLOSE=0`, `HOST_SHUTDOWN=1`, `RELOCATION_OUT=2`다.',
    'context: ZLinkSpotClosingContext,',
    'onClosing(systems.zlink.framework.spots.ZLinkSpotClosingContext);',
    '별도 Framework cancellation 타입을 사용하지 않는다.',
    'Source는 placement reservation을 만들지 않는다.',
    'Target Java runtime은 local exact instance가 없을 때만 자신을 owner로 Reserve하고',
  ],
  node: [
    ...
    'export declare enum ZLinkSpotCloseReason { ExplicitClose = 0, HostShutdown = 1, RelocationOut = 2 }',
    ...
    'Source는 generic Store reservation을 만들지 않는다.',
    'Target runtime은 local exact instance를 먼저 확인하고, 없을 때만 자신을 owner로 Creating row와 pending capacity를 Reserve한 뒤',
  ],
};
