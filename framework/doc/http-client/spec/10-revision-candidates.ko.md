...
| R3 | retry 백오프 + 지터 + 총 데드라인 | 고정 50ms는 장애 서버에 동시 재돌진(thundering herd) 유발. 재시도 전체를 아우르는 총 데드라인도 계약에 없음(cpp 코루틴 경로만 강제 — 6장 §6.2) | 기본 파라미터(초기/배수/상한/지터), 총 데드라인 옵션 도입 여부, 기존 50ms 계약 테스트 개정 |
...
| R6 | 응답 헤더 대소문자 무시 조회 통일 | cpp만 대소문자 구분 map(조회가 서버 표기에 의존). 나머지는 대소문자 무시와 동등(java/node 소문자 정규화, dotnet case-insensitive map) | cpp를 대소문자 무시 조회로 정렬(기존 cpp 사용자 영향) |
