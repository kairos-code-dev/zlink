...
[Node 묶음](../../../../node/README.ko.md) | [SPOT](nestjs-spot.ko.md) | [인터페이스](handler-interfaces.ko.md)

이 문서는 `Node.js` framework 의 `SpotNode` 설정 중 core route 계약과 직접
맞물리는 부분, 그리고 spot manager 표면(create / get / list / close)을 정리한다.
표면은 NestJS / TypeScript 모양이며 구현과 contract test는 이 문서를 따라야 한다.
Spot handler 작성법과 packet dispatch 규칙은
[nestjs-spot.ko.md](nestjs-spot.ko.md)를 기준으로 한다.

이 문서대로 구현하면 공통 SpotNode 동작과 Node.js 관례를 함께 만족한다.

## SpotNode 등록

NestJS는 `zlinkFramework()` fluent builder로 Spot mesh를 등록한다. builder 메서드
한 개가 node builder 메서드 한 개에 대응한다.
...
`.addSpotMesh(name)`은 Spot mesh를 등록한다. Discovery는 등록된 location store를
사용하고, 각 node는 같은 SpotNode builder 메서드 집합을 쓴다.
...
비동기 결과는 `Promise`, 메서드 이름은 camelCase로 표현한다. 생성 요청 payload는 DTO
또는 `ZLinkMessage`로 전달한다. Spot 생성·조회처럼 호출자가 중단할 수 있는 장기 작업은
Node.js 관례에 따라 `signal?: AbortSignal`을 제공한다. 일반 handler나 lifecycle
callback에는 signal을 자동으로 추가하지 않는다.
...
않는다([backend-dependency-policy](../../../../node/internals/backend-dependency-policy.ko.md)).
...
test 로 재현한다([regression-test-matrix](../../../../node/internals/regression-test-matrix.ko.md)).
