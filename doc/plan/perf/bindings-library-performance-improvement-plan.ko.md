...
| `tcp` | `MULTI_SPOT_SENDSEND` | `보류(44.4%)` | `보류(40.5%)` | `보류(45.8%)` | `보류(17.8%)` | `보류(18.8%)` | `보류(24.8%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go `perf_go_multi_linux_20260524_205329.txt` 기준이다. C++처럼 client active loop에서 waiting reply를 먼저 `RecvRouted(...DONTWAIT)`로 drain한 뒤 같은 slot에서 즉시 다음 send를 시도하도록 맞췄다. 기존 wave형 loop보다 small과 large 일부가 올라갔고 전 size complete를 유지하지만, SPOT 최소 기준에는 아직 못 미친다. |
...
| `ws` | `MULTI_SPOT_SENDSEND` | `보류(49.6%)` | `보류(42.0%)` | `보류(37.3%)` | `보류(0.2%)` | `보류(22.6%)` | `보류(0.3%)` | C `perf_c_multi_linux_20260522_055144_codex_c_ws_multi_for_go_20260522.txt` 대비 Go `perf_go_multi_linux_20260524_205329.txt` 기준이다. drain-then-submit loop로 small은 올랐지만 large collapse는 남는다. C와 같은 size별 active slot 제한을 유지하고, 서버 echo는 ws large에서 public builder 경로를 유지한다. |
...
| `wss` | `MULTI_SPOT_SENDSEND` | `보류(36.6%)` | `보류(39.7%)` | `보류(37.2%)` | `통과(54.6%)` | `보류(49.9%)` | `통과(53.2%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go `perf_go_multi_linux_20260524_205329.txt` 기준이다. client active loop를 C++처럼 drain-then-submit으로 바꿔 small과 large 일부가 올랐다. 서버 echo는 public `Spot.ForwardRouted(...)`를 65536B 이상에 적용한다. 131072B는 49.9%로 통과선 직전이지만, 새 loop 기준 active slot 16 후보 `perf_go_multi_linux_20260524_205629.txt`도 50%를 넘기지 못해 8-slot 제한을 유지한다. |
...
| `tls` | `MULTI_SPOT_SENDSEND` | `보류(39.2%)` | `보류(41.1%)` | `보류(46.2%)` | `보류(37.5%)` | `보류(42.0%)` | `보류(41.8%)` | Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go `perf_go_multi_linux_20260524_205329.txt` 기준이다. `SubmitNotConnected`는 C처럼 fatal이 아니라 다음 poll loop에서 재시도하는 의미로 처리하고, 서버 echo는 public `Spot.ForwardRouted(...)`를 65536B 이상에 적용한다. drain-then-submit loop로 전 size가 올랐지만 아직 Go SPOT 최소 기준에는 못 미친다. active slot 16 후보 `perf_go_multi_linux_20260524_205629.txt`는 131072B를 낮춰 반영하지 않는다. |
