...
const void *spot_node_access_t::native_handle (
  const service::spot_node_t &node_) noexcept
{
    return node_._impl ? node_._impl->handle : NULL;
}
...
void spot_node_t::connect_router_channel_peer (
...
void spot_node_t::attach_channel_dealer_impl (
  discovery_t &discovery_, zlink::base_socket_t &dealer_)
{
    detail::throw_if_failed<config_error_t> (
      static_cast<config_result_t> (
...
void spot_node_t::attach_channel_dealer_manual_impl (
  const std::string &channel_name_, zlink::base_socket_t &dealer_)
{
    zlink::detail::validate_bounded_c_string (
      channel_name_, 255u, "channel_name");
    if (channel_name_.empty ())
...
void spot_node_t::attach_pub_ingress_impl (zlink::base_socket_t &pub_)
{
    detail::throw_if_failed<config_error_t> (
      static_cast<config_result_t> (
...
std::optional<actor_part_t> spot_node_t::recv_actor_part (
  const actor_ref_t &actor_, recv_flags_t flags_)
{
...
actor_ref_t spot_node_t::remote_actor_ref (
  const routing_id_t &target_node_rid_, const std::string &actor_id_)
{
    ...
    std::snprintf (
      native.actor_id, sizeof (native.actor_id), "%s", actor_id_.c_str ());
    return zlink::detail::actor_model_access_t::from_native (native);
}
...
std::pair<spot_t, bool> spot_node_t::get_or_create_spot (
...
std::optional<spot_t> spot_node_t::spot_lookup (
  const routing_id_t &spot_rid_)
{
    void *handle = NULL;
