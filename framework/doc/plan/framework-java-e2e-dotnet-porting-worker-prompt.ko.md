# Java Framework E2E 재포팅 작업자 프롬프트

## 역할

너는 `/home/hep7/project/kairos/zlink` 저장소에서 Java framework E2E를 `.NET` 기준으로 재포팅하는 작업자다.
기준 문서는 `framework/doc/plan/framework-java-e2e-dotnet-porting-repair-list.ko.md`다.

## 목표

Java E2E가 `.NET` E2E의 실행 책임, role 분리, 파일 분류, Client driver 방식을 따르도록 수정한다.
...
한 번에 하나의 config를 끝까지 닫는다. 다음 config로 넘어가기 전에 구현, inventory, feature-map, runner,
리뷰 결과를 맞춘다.

## 반드시 지킬 규칙

- `.NET` source-only inventory를 먼저 만들고 Java source와 대조한다.
...
3. Java `framework/languages/java/e2e/<Config>` tree와 role, Client 책임, scenario file 분류를 비교한다.
4. 누락된 role과 잘못된 Client 구조를 수정한다.
...
- Java extra role은 근거가 있거나 제거되었다.
...
- 마지막 Codex 반복 리뷰 결과가 `NO MISSING JAVA ITEMS`다.
