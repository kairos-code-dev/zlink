...
| Actor join·leave와 lifecycle control | Framework가 lifecycle 작업을 처리하는 Spot·Actor 전용 queue |
...
`Yield`는 `SpotWide` Stage에서 Channel·Spot·Actor request 또는 CPU·I/O worker 결과를 기다릴 때만 사용할
수 있다. 이 경우 Stage 공통 turn을 반납하고 continuation은 같은 execution gate에서 새 turn으로 재개한다.
`PerActor` Stage에서는 `Yield`를 사용할 수 없다.
