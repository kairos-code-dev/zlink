...
internal static class AtdC1TimerIsolationScenario
{
    public static async Task<(string SpotRid, string RequestId)> RunAsync(IZlinkStreamConnector client)
    {
        ...
        var spot = await client.Request(new EnsureSpotReq(spotRid))
            ...
            .Async<EnsureSpotRes>();
        ScenarioAssert.That(spot.SpotRid == spotRid, "ATD-C timer spot creation mismatch.");

        var requestId = $"ATD-C1-{Guid.NewGuid():N}";
