...
Application payload는 owner의 application turn에서 직렬로 처리한다. Completion,
send-ready, location reconcile, reservation과 relocation control 같은 infrastructure
작업은 application handler가 대기 중이어도 진행한다. Transport readiness callback에서
application handler를 직접 실행하지 않는다.
