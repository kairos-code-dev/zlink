...
이 프로젝트는 framework 공통 E2E 문서의 시나리오형 테스트 구조를 Java에서 검증한다.
...
```bash
./framework/languages/java/zlink-framework-scenario-e2e/run_scenario_e2e.sh
```

client는 Spring Boot starter가 공개한 `ZLinkClient` 또는 `ZLinkRouteClient`와
`ZLinkHttpClient`만 사용한다. 검증은 샘플처럼 `ensure(...)` 구문으로 직접 작성한다.
