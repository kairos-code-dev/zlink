...
route_channel_runtime_t::route_channel_runtime_t (
...
const std::string &
route_channel_runtime_t::router_channel_id () const noexcept
{
...
void
route_channel_runtime_t::stop () noexcept
{
  _running = false;
...
bool
route_channel_runtime_t::running () const noexcept
{
  return _running;
}

bool
route_channel_runtime_t::connect (std::string endpoint)
{
  return _connections.connect (std::move (endpoint));
}

bool
route_channel_runtime_t::disconnect (const std::string &endpoint)
{
  return _connections.disconnect (endpoint);
}

std::vector<std::string>
route_channel_runtime_t::list_connections () const
{
  return _connections.list ();
}

result_t<void>
route_channel_runtime_t::submit_send_parts (
  ...
  runtime::messaging::message_parts_t parts)
{
  ...
  _outbound_packets.push_back (
    route_outbound_packet_t { target_node_rid,
                              std::nullopt,
                              std::move (parts),
                              std::nullopt });
  if (_send_backend) {
    return _send_backend (
      _outbound_packets.back ().target_node_rid,
...
route_channel_runtime_t::submit_request_parts (
  const zlink::routing_id_t &target_node_rid,
  runtime::messaging::message_parts_t parts)
{
  if (auto connected = ensure_connected (); !connected) {
...
route_channel_runtime_t::request_reply_parts (
  const zlink::routing_id_t &target_node_rid,
  runtime::messaging::message_parts_t parts,
...
result_t<std::uint64_t>
route_channel_runtime_t::request_to_spot_parts (
  const zlink::routing_id_t &target_node_rid,
  const zlink::routing_id_t &target_spot_rid,
  runtime::messaging::message_parts_t parts)
{
...
result_t<void>
route_channel_runtime_t::complete_request (std::uint64_t request_seq)
{
...
void
route_channel_runtime_t::set_send_backend (send_backend_t backend)
{
  _send_backend = std::move (backend);
}

void
route_channel_runtime_t::set_request_backend (request_backend_t backend)
{
  _request_backend = std::move (backend);
}

void
route_channel_runtime_t::attach_native_backend (
  backend::native_route_backend_t &backend)
{
  set_send_backend (
    [&backend](const zlink::routing_id_t &target_node_rid,
...
std::size_t
route_channel_runtime_t::pending_request_count () const noexcept
{
  return _pending_requests.count ();
}

result_t<void>
route_channel_runtime_t::ensure_connected () const
{
  if (!_running) {
    return result_t<void>::failure (
