...
| role split parity | 부분 구현 | Registry, DispatchApi, DispatchCenter, Tracking, Session, Probe, Client를 별도 executable로 실행한다. Session role이 고객 stream과 courier stream bind를 함께 담당하므로, 최신 .NET 샘플의 CustomerGateway, CourierSession, CourierGateway, CourierActorNode 역할은 아직 같은 의미로 분리되지 않았다. |
| shared contract parity | 부분 구현 | C++ `Shared/Contracts/messages.hpp`가 배송 생성, tracking, customer subscription, courier bind, offer notify, courier decision DTO와 typed JSON stream payload hook을 제공한다. 최신 .NET 샘플의 courier gateway/actor-node 내부 DTO와 role 흐름은 아직 완전 대응하지 않는다. |
...
route를 해석한다. C++ E2E는 courier stream bind, offer push, decision send 흐름은 public stream API로
검증하지만, 아직 별도 `CourierGateway`와 `CourierActorNode` 역할로 courier directory와 actor node
route를 분리하지 않았다.

이 gap은 샘플 public 흐름의 차이이므로 완료로 보지 않는다. 후속 작업에서는 `CourierSession`,
`CourierGateway`, `CourierActorNode` 역할 분리와 actor node route를 C++ public framework API로 포팅해야 한다.
