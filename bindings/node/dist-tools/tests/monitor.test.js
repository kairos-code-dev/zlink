...
test('spot service monitor reports remote sub delivery ready after direct peer connect', async () => {
    ...
    const monitor = clientSpot.monitorOpen(zlink.ServiceMonitorEvent.SPOT_SUB_DELIVERY_READY_CHANGED
        | zlink.ServiceMonitorEvent.ERROR);
    try {
        ...
        while (Date.now() < deadline) {
            ...
            if (event) {
                ...
                if (event.eventType === zlink.ServiceMonitorEvent.SPOT_SUB_DELIVERY_READY_CHANGED
                    && event.value > 0) {
...
test('spot service monitor does not report sub delivery ready before peer connect', async () => {
    ...
    const node = new zlink.SpotNode(ctx);
    const spot = new zlink.Spot(node);
    const monitor = spot.monitorOpen(zlink.ServiceMonitorEvent.SPOT_SUB_DELIVERY_READY_CHANGED
        | zlink.ServiceMonitorEvent.ERROR);
    try {
...
test('spot service monitor does not report remote pub delivery ready before peer connect', async () => {
    ...
    const serverSpot = new zlink.Spot(serverNode);
    const clientSpot = new zlink.Spot(clientNode);
    const monitor = serverSpot.monitorOpen(zlink.ServiceMonitorEvent.SPOT_PUB_DELIVERY_READY_CHANGED
...
test('multi spot perf helper completes a single tcp recv run', async () => {
    const { spawnMultiPair } = require(path.join(__dirname, '..', 'perf', 'multi', 'perf_multi_common.js'));
    const resultLines = await spawnMultiPair('perf_multi_spot_server.js', 'perf_multi_spot_client.js', {
        pattern: 'MULTI_SPOT',
        recv: 'recv',
        duration: 1,
        warmup: 1,
    ...
    assert.ok(resultLines.some((line) => (line.startsWith('RESULT,current,MULTI_SPOT,tcp,64,throughput,'))));
});
