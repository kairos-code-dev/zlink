...
통과권에 들어왔다. 남은 차이는 이전 1차 개선 문서에서
...
| `tcp` | `DEALER_ROUTER` | `통과(41.0%)` | `통과(41.4%)` | `통과(46.6%)` | `미달(25.8%)` | `미달(23.3%)` | `미달(22.3%)` | 64/256/1024B는 single routed 단일 payload native 수신 후보 complete 측정 `perf_node_single_linux_20260602_101414_node_single_routed_single_payload_native_probe_20260602.txt` 기준. 65536/131072/262144B는 sender worker가 runtime-owned native submit 함수를 직접 쓰고 tcp routed large HWM floor 64를 적용한 complete 측정 `perf_node_single_linux_20260604_181153_node_single_routed_tcp_large_native_sender_hwm64_20260604.txt` 기준. |
| `tcp` | `ROUTER_ROUTER` | `통과(47.0%)` | `통과(48.2%)` | `통과(48.8%)` | `미달(27.4%)` | `미달(24.0%)` | `미달(23.2%)` | 64/256/1024B는 single routed 단일 payload native 수신 후보 complete 측정 `perf_node_single_linux_20260602_101414_node_single_routed_single_payload_native_probe_20260602.txt` 기준. 65536/131072/262144B는 sender worker가 runtime-owned native submit 함수를 직접 쓰고 tcp routed large HWM floor 64를 적용한 complete 측정 `perf_node_single_linux_20260604_181153_node_single_routed_tcp_large_native_sender_hwm64_20260604.txt` 기준. |
