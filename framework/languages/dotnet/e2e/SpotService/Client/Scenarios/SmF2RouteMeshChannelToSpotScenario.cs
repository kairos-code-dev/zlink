...
internal static class SmF2RouteMeshChannelToSpotScenario
{
    public static async Task RunAsync(ZLinkHttpClient api, SpotLifecycleOrderContext context)
    {
        var state = (await api.Post("/spot/state/request")
            ...
            .Async<StateRes>()).Body;
        ZlinkStreamAssert.Ensure(state.SpotRid == context.SpotRid, "SM-F2 request reached the wrong spot.");
        ZlinkStreamAssert.Ensure(state.NodeRid == "play-a", "SM-F2 request reached the wrong node.");
        ZlinkStreamAssert.Ensure(state.Value == context.CurrentValue + 5, "SM-F2 state reply mismatch.");
        ...
        ZlinkStreamAssert.Ensure(command.SpotRid == context.SpotRid && command.Accepted, "SM-F2 command was not accepted.");
