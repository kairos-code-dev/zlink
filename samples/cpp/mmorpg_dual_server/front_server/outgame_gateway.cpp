...
void OutgameGateway::send_request (const std::string &session_key,
                                   const std::string &op,
...
int OutgameGateway::poll_responses ()
{
    ...
    while (true) {
        ...
        for (size_t i = 0; i < part_count; ++i) {
            payload_parts.emplace_back (static_cast<const char *> (zlink_msg_data (&parts[i])),
        ...
        if (payload_parts.size () >= 2) {
            ...
            if (it != pending_.end () && response_handler_) {
                response_handler_ (it->second.session_key, it->second.client_req_id, result);
                pending_.erase (it);
            }
            ++count;
        }
