...
> 이 문서는 `.NET` 가이드의 진입점이다. 언어 중립 정의는
> [공통 스펙 목차](../../../common/README.ko.md)가, `.NET` 표면의 정확한 계약은
> [.NET exact interface 목차](../../../common/spec/server/languages/dotnet/interfaces/README.ko.md)가 소유한다.
...
이 가이드의 03~17장은 **다섯 언어가 같은 정본에서 생성된다.** 예제는 `.NET` 코드만
담기며 다른 언어 코드가 섞이지 않는다. 순서는
[.NET 가이드 진입점](README.ko.md)이 제시한다.
