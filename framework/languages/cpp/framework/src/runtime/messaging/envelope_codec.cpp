...
template<typename T>
std::optional<T>
optional_json_value (const nlohmann::json &json, const char *name)
{
...
message_parts_t::message_parts_t (zlink::message_t header,
...
message_parts_t::message_parts_t (std::vector<zlink::message_t> parts)
  : _parts (std::move (parts))
{
}

const zlink::message_t &
message_parts_t::operator[] (std::size_t index) const
{
...
message_parts_t
envelope_codec_t::encode_raw_body_parts (const envelope_header_t &header,
  ...
  const serializer_registry_t &serializers) const
{
  ...
  return encode_raw_body_parts (
    header,
    serializers.serialize (body_type, body));
}

zlink::message_t
envelope_codec_t::encode_header (const envelope_header_t &header) const
{
  nlohmann::json json {
    ...
    { "correlationId", header.correlation_id.empty ()
                         ? nlohmann::json (nullptr)
                         : nlohmann::json (header.correlation_id) },
    { "deadline", header.deadline ? nlohmann::json (*header.deadline)
                                  : nlohmann::json (nullptr) },
    { "topic", header.topic ? nlohmann::json (*header.topic)
                            : nlohmann::json (nullptr) },
    { "errorCode", header.error_code ? nlohmann::json (*header.error_code)
                                     : nlohmann::json (nullptr) },
    { "errorMessage",
...
envelope_codec_t::decode_header (const zlink::message_t &message) const
{
  try {
    ...
    header.content_type = json.value<std::string> (
...
result_t<envelope_header_t>
envelope_codec_t::decode_header (const message_parts_t &parts) const
{
  if (parts.size () == 0) {
    return result_t<envelope_header_t>::failure (
      framework_error_kind_t::request_protocol_error,
...
envelope_codec_t::decode_body (const message_parts_t &parts) const
{
  if (parts.size () < 2) {
