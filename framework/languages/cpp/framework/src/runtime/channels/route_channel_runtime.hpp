...
public:
                              ...
                              const serializer_registry_t &serializers)
  {
    ...
    return submit_send_parts (
      target_node_rid,
      codec.encode_envelope_parts (header, message, serializers));
  }

  template<typename TRequest>
  result_t<std::uint64_t> submit_request (
    ...
    std::chrono::milliseconds timeout)
  {
    ...
    auto header = codec.create_envelope (
      ...
      std::move (packet_name),
      timeout);
    return submit_request_parts (
      target_node_rid,
      codec.encode_envelope_parts (header, request, serializers));
  }

  result_t<void> submit_send_parts (
    const zlink::routing_id_t &target_node_rid,
    runtime::messaging::message_parts_t parts);

  result_t<std::uint64_t> submit_request_parts (
    const zlink::routing_id_t &target_node_rid,
    runtime::messaging::message_parts_t parts);

  result_t<runtime::messaging::message_parts_t> request_reply_parts (
    const zlink::routing_id_t &target_node_rid,
    runtime::messaging::message_parts_t parts,
    std::chrono::milliseconds timeout);

  result_t<void> submit_spot_send_parts (
    const zlink::routing_id_t &target_node_rid,
    const zlink::routing_id_t &target_spot_rid,
    runtime::messaging::message_parts_t parts);

  result_t<std::uint64_t> request_to_spot_parts (
    const zlink::routing_id_t &target_node_rid,
    const zlink::routing_id_t &target_spot_rid,
    runtime::messaging::message_parts_t parts);
  ...
  const std::vector<route_outbound_packet_t> &outbound_packets () const
    noexcept;
