...
final class PerfRouterRouterPoll {
    ...
    static int run(String transport, int size) {
        ...
        try {
            String endpoint = PerfUtil.endpointFor(transport, "router-router-poll");
            ...
            for (int i = 0; i < 100; i++) {
                ...
                if (!PerfUtil.waitForInput(router1, 0)) {
                    Thread.sleep(10);
                    continue;
                }
            ...
            if (!PerfUtil.waitForInput(router2, 2000)) {
                return 2;
            }
            ...
            for (int i = 0; i < latCount; i++) {
                ...
                if (!PerfUtil.waitForInput(router1, 2000)) {
                    return 2;
                }
            ...
            PerfUtil.printResult("ROUTER_ROUTER_POLL", transport, size, thr, latUs);
            return 0;
        } catch (Exception e) {
            return 2;
        } finally {
