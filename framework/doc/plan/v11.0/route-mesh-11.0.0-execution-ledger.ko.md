...
| `V11-CA-SESSION-BINDING-E2E` | Session disconnect·relocation E2E 계약 | E2E coordinator, `P-DELIVERY` | `V11-CA-SESSION-BINDING-SPEC`, `V11-CA-SESSION-BINDING-PROTOCOL` | 진행 | all-settled failure·dedupe·no-Store, same-generation·Completed-only sequence scenario 누락 0 | Config 2 SM-D4A·D4B·D5·D5A와 Config 10 ST-E1A 계약을 갱신했다. JVM `ZLinkSessionActorBindingContractTest` 5건과 기존 route contract 2건, Kotlin projection suite, C++ `test_cpp_framework_m6b_runtime`·`test_cpp_framework_actor_gateway`가 통과했다. .NET은 Session 12/12, cleanup 13/13, bound relay 1/1, handoff 30/30 PASS로 stale fence·no-Store·all-settled contract를 검증했고 `sm-d5a` process runner를 추가했다. Node는 workspace build와 CA-D78 focused 5/5 PASS로 SM-D4A·D4B·D5·D5A를 검증했다. .NET SpotService는 선행 SpotId·terminal API gap 38개로 build가 차단됐고, 다섯 언어의 durable `Completed` 뒤 same-generation route switch를 실제 process relocation command로 증명하지 못했으므로 완료로 전환하지 않는다. |
