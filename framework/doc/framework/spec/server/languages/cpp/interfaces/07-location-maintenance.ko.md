...
## 1. Authority와 checkpoint provider capability

`location_store_t`는 아래 opaque authority CAS를 필수 capability로 제공한다. `Recreate` 또는 `Snapshot`
policy를 사용하는 host는 accepted journal을 보존하기 위해 Checkpoint Store를 별도로 등록한다. Application service code는 두 provider
operation을 직접 호출하지 않는다.
...
struct checkpoint_stored_t {
    std::string reference;
    std::chrono::system_clock::time_point expires_at;
    std::chrono::system_clock::time_point store_now;
};
struct checkpoint_found_t { std::vector<std::byte> payload; };
struct checkpoint_missing_t {};
using checkpoint_read_result_t =
  std::variant<checkpoint_found_t, checkpoint_missing_t>;
enum class checkpoint_delete_result_t { deleted = 0, missing = 1 };
struct checkpoint_renewed_t {
    std::chrono::system_clock::time_point expires_at;
    std::chrono::system_clock::time_point store_now;
};
struct checkpoint_renew_missing_t {};
using checkpoint_renew_result_t =
  std::variant<checkpoint_renewed_t, checkpoint_renew_missing_t>;

class checkpoint_store_t {
public:
    virtual ~checkpoint_store_t() = default;
    virtual task_t<checkpoint_stored_t> put_checkpoint(
      std::vector<std::byte> payload,
      std::chrono::hours retention,
      std::stop_token cancellation = {}) = 0;
    virtual task_t<checkpoint_read_result_t> get_checkpoint(
      std::string reference,
      std::stop_token cancellation = {}) = 0;
    virtual task_t<checkpoint_renew_result_t> renew_checkpoint(
      std::string reference,
      std::chrono::hours retention,
      std::stop_token cancellation = {}) = 0;
    virtual task_t<checkpoint_delete_result_t> delete_checkpoint(
      std::string reference,
      std::stop_token cancellation = {}) = 0;
};
...
checkpoint put과 renew의 retention에 24시간을 전달한다. Authority의 current checkpoint reference를 확인한
owner 또는 recovery coordinator만 renew를 호출한다. Renew 성공은 provider clock의 새 expiry와 Store time을
반환한다. Missing reference는 `checkpoint_renew_missing_t` 정상 결과이며 retention은 application option이
아니다. Framework는 logical checkpoint를 immutable 64 MiB chunk 최대 4096개와 root manifest로 내부에서
나누므로 logical state ceiling은 256 GiB다. Checkpoint Store의 opaque put/get interface는 바꾸지 않으며
...
effective bound는 checkpoint chunk 크기 때문에 줄이지 않는다. Provider는 key, authority payload와 checkpoint
payload를 해석하지 않는다.
...
Framework가 provider에 넘긴 authority와 checkpoint 입력 buffer는 asynchronous operation이 끝날 때까지
...
필요한 authority와 checkpoint provider capability만 소유한다. Authority Store를 root에 별도로 등록하는
member는 제공하지 않는다. Location Store와 Checkpoint Store 등록 member는
