...
| `RM-A3` | 부분 구현 | 공통 connection policy를 production automatic planner와 manual handshake에 연결했다. 두 Object Client 모두 RouteMesh Channel Server membership이 없을 때만 `NotRequired`이며, weight `0`을 포함해 어느 한쪽에 Server membership이 있으면 연결이 필요하다. Focused M6A test가 predicate, manual terminal, ready peer 0과 liveness probe 0을 검증한다. Public monitoring은 Location descriptor를 기준으로 `NotRequired`와 `NotConnected`를 분리하고 Node direct Object Client target은 `NotFound`로 분류한다. Automatic·Manual actual process, reconnect 억제와 public status·Node direct 조합은 runner가 아직 없어 완료로 판정하지 않는다. |
