                                                ...
                                                int events_)
{
    ...
    socket_base_t *monitor_socket = socket_->get_ctx ()->create_socket (ZLINK_CORE_SOCKET_PAIR);
    if (!monitor_socket) {
        socket_->monitor (NULL, 0, 4, ZLINK_CORE_SOCKET_PAIR);
        errno = ENOMEM;
        return NULL;
    }
...
{
    if (socket_)
        (void) socket_->monitor (NULL, 0, 4, ZLINK_CORE_SOCKET_PAIR);
