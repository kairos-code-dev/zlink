...
| `ws` | `MULTI_SPOT_SENDSEND` | `보류(46.6%)` | `보류(41.3%)` | `보류(37.6%)` | `보류(29.1%)` | `보류(22.5%)` | `보류(33.0%)` | 64/256/1024B는 C `perf_c_multi_linux_20260522_055144_codex_c_ws_multi_for_go_20260522.txt` 대비 Go `perf_go_multi_linux_20260524_220932.txt` median 기준이다. 65536/131072/262144B는 현재 core 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_041059.txt` 대비 Go `perf_go_multi_linux_20260525_041509.txt` 기준이다. drain-then-submit loop와 `Spot.ForwardRouted(...)` server echo로 complete를 유지하면서 65536B/262144B collapse를 줄였다. 131072B는 기존 no-code 재측정보다 조금 낮고 전 large가 아직 기준 아래라 보류다. `MoveMessage(...)` client send 후보는 `ws` small과 일부 large를 낮춰 적용하지 않는다. C와 같은 size별 active slot 제한을 유지한다. |
...
| `wss` | `MULTI_DEALER_DEALER` | `보류(37.4%)` | `통과(58.3%)` | `통과(71.2%)` | `보류(47.5%)` | `보류(48.2%)` | `통과(63.4%)` | 64B는 fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go `perf_go_multi_linux_20260524_231656.txt`, 256/1024B는 Go `perf_go_multi_linux_20260524_222134.txt` 기준이다. 65536B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_042502.txt` 대비 Go `perf_go_multi_linux_20260525_042512.txt` 기준이다. 131072B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_015713.txt` 대비 Go `perf_go_multi_linux_20260525_015819.txt` 기준이다. 262144B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_042131.txt` 대비 Go `perf_go_multi_linux_20260525_042206.txt` 기준이다. C처럼 단일 poll loop와 pending socket `POLLOUT` 의미를 유지했다. Go send builder에 명시적 ownership 이전 단계 `MoveMessage`를 추가했고, active send는 1024B 이하와 wss 65536/131072B, tls 131072B에서만 이 경로를 사용한다. wss 65536B는 같은 조건 no-code 재측정 `perf_go_multi_linux_20260525_042547.txt` 28.512Kmsg/s에서 31.792Kmsg/s로 올랐지만 아직 기준선 아래라 보류다. wss 262144B는 public `Bytes(...)` client send와 server `RecvPart` caller-owned 수신 경로를 함께 적용해 같은 조건 no-code 재측정 `perf_go_multi_linux_20260525_042046.txt` 1.949Kmsg/s에서 10.195Kmsg/s로 올라 통과했다. 131072B server 수신은 public `RecvPart` caller-owned 수신 경로로 확장해 no-code 재측정 9.894Kops/s(`perf_go_multi_linux_20260525_015918.txt`)에서 16.457Kops/s로 올렸다. 64B/65536B/131072B는 아직 기준선 아래다. |
...
| `tcp` | `MULTI_SPOT` | `보류(67.9%)` | `통과(77.1%)` | `통과(87.8%)` | `통과(109.7%)` | `보류(53.6%)` | `통과(104.3%)` | 64/256/1024B는 기존 파일 기준이다. 65536/131072/262144B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_022730.txt` 대비 Rust `perf_rust_multi_linux_20260525_025506.txt` 기준이다. SPOT server active publish를 `Vec` stamp 뒤 `Message::copy_from`으로 다시 복사하던 경로에서 `Message::with_size()` + `data_mut()` 직접 stamp로 바꿔 65536/262144B가 통과권으로 올라왔다. 131072B도 40.6%→53.6%로 개선됐지만 아직 SPOT 기준 아래다. **2026-05-25 선택 적용**: SPOT client recv worker가 progress 없을 때 무조건 `sleep(1ms)`로 쉬던 것을 tcp 131072B와 ws 131072/262144B에서 public `Poller` `POLLIN` wait로 바꿨다. **2026-05-24 multi-worker drain 추가**: copy 제거 위에 추가로, 단일 thread가 100 spot을 순차 drain하던 것을 Go recv worker처럼 scoped thread 4개로 분할(`Spot: Send`, `chunks_mut`로 disjoint 분배)했다. (러너의 symlink 기반 stale-guard 버그도 함께 수정: `readlink -f`로 실제 versioned .so와 비교.) — **2026-05-23 per-message copy 제거**: Rust SPOT client 수신 hot path가 매 메시지 payload를 `message_payload(...).to_vec()`로 전체 복사(262144B면 256KB alloc+memcpy/msg)해 large가 1~1.5%로 무너졌다. timestamp 디코드는 borrowed `&[u8]`로 충분하므로 `.to_vec()`를 제거했다. |
| `tcp` | `MULTI_SPOT_REQREP` | `통과(80.0%)` | `통과(88.9%)` | `통과(85.9%)` | `통과(89.4%)` | `통과(77.6%)` | `보류(72.5%)` | 64/256/1024B는 fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Rust `perf_rust_multi_linux_20260522_150056_codex_rust_multi_tcp_spot_reqrep_20260522.txt` 기준이다. 65536/131072B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_022108.txt` 대비 Rust `perf_rust_multi_linux_20260525_022246.txt` 기준이다. 262144B는 C `perf_c_multi_linux_20260525_044520.txt` 대비 Rust `perf_rust_multi_linux_20260525_044540.txt` 기준이다. server reply가 받은 single-part `Message`를 다시 복사하지 않고 그대로 reply로 넘기고, client active request를 C처럼 `DONT_WAIT`로 맞췄다. 최신 단독 재측정에서 262144B는 66.1%에서 72.5%로 올라왔지만 아직 SPOT 기준보다 낮다. |
...
| `tcp` | `MULTI_STREAM` | `통과(82.6%)` | `통과(98.3%)` | `통과(96.5%)` | `보류(34.7%)` | `해당 없음` | `해당 없음` | 64B/65536B는 C `perf_c_multi_linux_20260522_150329_codex_c_multi_tcp_stream_clients100_for_rust_20260522.txt`, Rust `perf_rust_multi_linux_20260522_150351_codex_rust_multi_tcp_stream_clients100_20260522.txt` 기준이다. 256B/1024B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_052620.txt` 대비 Rust `perf_rust_multi_linux_20260525_052625.txt` median 기준이다. 기본 STREAM clients=10000은 thread/CPU 부담이 커서 먼저 clients=100으로 제한해 같은 조건 비교를 남겼다. 64B/256B/1024B는 단순 one-way 기준을 넘고, 65536B는 아직 기준보다 낮아 보류한다. |
...
| `ws` | `MULTI_SPOT` | `보류(50.6%)` | `보류(50.4%)` | `통과(82.5%)` | `통과(115.5%)` | `보류(45.1%)` | `보류(41.7%)` | 64/256/1024B는 기존 파일 기준이다. 65536/131072/262144B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_022730.txt` 대비 Rust `perf_rust_multi_linux_20260525_025506.txt` 기준이다. SPOT server active publish를 `Message::with_size()` + `data_mut()` 직접 stamp로 바꾼 뒤 65536B가 통과권에 올랐다. 131072/262144B도 소폭 개선됐지만 아직 SPOT 기준 아래다. SPOT client 복사 제거 + 4-worker drain 위에, 131072B/262144B에서만 public `Poller` `POLLIN` wait를 사용한다. |
...
| `tls` | `MULTI_PUBSUB` | `통과(89.6%)` | `통과(82.0%)` | `보류(5.8%)` | `통과(110.0%)` | `통과(101.8%)` | `통과(91.3%)` | C 파일은 위 행과 같고, Rust `perf_rust_multi_linux_20260522_155342_codex_rust_multi_tls_pubsub_20260522.txt` 기준이다. 실행 중 client `nlwp=7` 수준이었다. 1024B는 낮아 보류하고, 나머지 size는 기준선을 넘었다. |
...
| `ws` | `MULTI_SPOT_SENDSEND` | `보류(0.2%)` | `보류(9.9%)` | `보류(9.7%)` | `보류(1.6%)` | `보류(18.5%)` | `통과(34.3%)` | C/Python 파일은 위 MULTI_DEALER_DEALER 행과 같다. active slot 제한과 pending reply drain 보강 뒤 timeout은 없고, 262144B는 SPOT 기준선을 넘었다. 나머지 size는 기준보다 낮아 send-send echo hot path 재검토가 필요하다. |
...
| `wss` | `MULTI_SPOT_SENDSEND` | `보류(0.1%)` | `보류(0.2%)` | `보류(3.3%)` | `보류(0.7%)` | `통과(38.2%)` | `통과(40.7%)` | C/Python 파일은 위 MULTI_DEALER_DEALER 행과 같고, 65536B/262144B는 제한 재측정 `perf_python_multi_linux_20260522_192645_codex_python_multi_wss_spot_sendsend_failed_recheck_20260522.txt` 기준이다. active slot 제한과 pending reply drain 보강 뒤 timeout은 없어졌고 131072B/262144B는 기준선을 넘었다. 나머지는 기준보다 낮다. |
