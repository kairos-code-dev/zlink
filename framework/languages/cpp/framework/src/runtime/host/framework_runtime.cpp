...
bool
framework_runtime_t::owns_native_context () const noexcept
{
  return static_cast<bool> (_context);
}

zlink::router_socket_t &
framework_runtime_t::channel_router ()
{
  if (!_router) {
...
framework_runtime_t::channel_dealer ()
{
  if (!_dealer) {
...
framework_runtime_t::stream_socket ()
{
  if (!_stream) {
...
framework_runtime_t::discovery ()
{
  if (!_discovery) {
    _discovery = std::make_unique<zlink::service::discovery_t> (
      *_context,
      zlink::auto_connect_type::route_mesh,
      "framework");
  }
  return *_discovery;
}

zlink::service::registry_t &
framework_runtime_t::registry ()
{
  if (!_registry) {
...
offload_executor_t &
framework_runtime_t::offload_executor () noexcept
{
  return _offload;
}
