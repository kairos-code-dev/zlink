...
int resolve_router_send_timeout_ms (void *router_)
{
    ...
    if (handle.socket->getsockopt (ZLINK_INTERNAL_OPT_SNDTIMEO, &timeout, &timeout_size) != 0)
        return -1;
    return timeout;
}
                               ...
                               void *userdata_)
{
    ...
    uint64_t request_seq = 0;
    {
        ...
        request_seq = zlink::request_reply_runtime::allocate_request_sequence (
          &state->requests.next_request_seq, state->requests.pending_sequences);
        if (request_seq == 0)
            return -1;
    ...
    if (register_spot_pending_request (state, key, timeout_ms_, handler_, userdata_) != 0) {
        return -1;
    }
    ...
    if (build_spot_request_reply_message (
          ...
          destination_class_, destination_node_rid_, destination_endpoint_rid_,
          zlink::request_reply::request_type, key.request_seq, parts_, part_count_, &combined)
        != 0) {
        erase_spot_pending_request (state, key);
        return -1;
    }
                                ...
                                void *userdata_)
{
    ...
    return start_spot_request_common (
      ...
      routed_protocol::spot_endpoint_class, destination_node_rid, destination_spot_rid, parts_,
                                  ...
                                  void *userdata_)
{
    ...
    return start_spot_request_common (spot_, routed_protocol::router_endpoint_class, std::string (),
                                      peer_rid, routed_protocol::router_endpoint_class, peer_rid,
                                      std::string (), parts_, part_count_, flags_, timeout_ms_,
                                      handler_, userdata_);
}
                                  ...
                                  void *userdata_)
{
    ...
    if (register_router_spot_pending_request (state, request_seq, key, timeout_ms_, handler_,
                                              userdata_)
        != 0) {
        return -1;
    }
    ...
    std::vector<zlink_msg_t> combined;
    if (build_spot_request_reply_message (
          routed_protocol::router_endpoint_class, std::string (), router_rid_key,
          routed_protocol::spot_endpoint_class, destination_node_rid, destination_spot_rid,
          zlink::request_reply::request_type, request_seq, parts_, part_count_, &combined)
        != 0) {
    ...
    const int rc = zlink::spot_reqrep_internal::dispatch_router_spot_delivery (
      router_, destination_node_rid, destination_spot_rid, router_spot_delivery_request, flags_,
                                                 ...
                                                 uint32_t timeout_ms_)
{
    ...
    return zlink::spot_reqrep_internal::start_spot_channel_request_bridge (
      spot_, router, parts_, part_count_, handler_, userdata_, flags_, timeout_ms_);
}
                                              ...
                                              uint32_t timeout_ms_)
{
    ...
    return zlink::submit_result_internal::from_request_submit_rc (
      start_spot_request_to_spot (spot_, dest_node_rid_, dest_spot_rid_, parts_, part_count_,
                                                ...
                                                uint32_t timeout_ms_)
{
    ...
    return zlink::submit_result_internal::from_request_submit_rc (start_spot_request_to_router (
      spot_, peer_rid_, parts_, part_count_, flags_, timeout_ms_, handler_, userdata_));
}
                                            ...
                                            size_t part_count_)
{
    ...
    if (build_spot_request_reply_message (
          routed_protocol::spot_endpoint_class, source_identity.node_rid, source_identity.spot_rid,
          routed_protocol::spot_endpoint_class, destination_node_rid, destination_spot_rid,
          zlink::request_reply::reply_type, request_seq_, parts_, part_count_, &combined)
        != 0) {
        return zlink::submit_result_internal::from_errno (errno);
    }
    const int rc = zlink::spot_reqrep_internal::dispatch_spot_routed_delivery (
                                                ...
                                                uint32_t timeout_ms_)
{
    ...
    return zlink::submit_result_internal::from_request_submit_rc (
      start_router_request_to_spot (router_, dest_node_rid_, dest_spot_rid_, parts_, part_count_,
                                    flags_, timeout_ms_, handler_, userdata_));
}
                                              ...
                                              size_t part_count_)
{
    ...
    const int rc = zlink::spot_reqrep_internal::dispatch_router_spot_delivery (
      router_, destination_node_rid, destination_spot_rid, router_spot_delivery_reply,
      ZLINK_DONTWAIT, 0, &combined);
    const int saved_errno = errno;
    zlink::request_reply::close_built_parts (&combined);
    errno = saved_errno;
                                             ...
                                             zlink_send_flags_t flags_)
{
    ...
    std::vector<zlink_msg_t> combined;
    if (build_spot_routed_message (routed_protocol::router_endpoint_class, std::string (),
    ...
    const int rc = zlink::spot_reqrep_internal::dispatch_router_spot_delivery (
      router_, destination_node_rid, destination_spot_rid, router_spot_delivery_direct, flags_,
      resolve_router_send_timeout_ms (router_), &combined);
    const int saved_errno = errno;
    zlink::request_reply::close_built_parts (&combined);
    errno = saved_errno;
