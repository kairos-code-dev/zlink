...
| Messaging | 미착수 | `contracts/messaging/`: `message.ts`, `received.ts`, `topic_message.ts`, `subscription_event.ts`, `operations.ts`, `handlers.ts`(`message_parts_envelope.ts`는 내부 지원) |
