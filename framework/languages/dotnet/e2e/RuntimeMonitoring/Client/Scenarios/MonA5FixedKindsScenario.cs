...
internal static class MonA5FixedKindsScenario
{
    public static async Task RunAsync(ClientOptions options)
    {
        ...
        ScenarioAssert.That(
            serviceEvidence.Any(line => line.Contains("monitor-spot|source=monitor.spot|kind=StatusChanged", StringComparison.Ordinal)),
            "MON-A5 spot status evidence missing.");
        ScenarioAssert.That(
            serviceEvidence.Any(line => line.Contains("monitor-spot|source=monitor.spot|kind=TimerStoppedAfterUnhandledException", StringComparison.Ordinal)
    ...
    static async Task<(string[] ServiceEvidence, string[] RegistryEvidence)> WaitForFixedKindsAsync(
        ...
        ZLinkHttpClient registry)
    {
        ...
        if (HasFixedKinds(serviceEvidence, registryEvidence))
        {
            return (serviceEvidence, registryEvidence);
        }

        throw new InvalidOperationException("MON-A5 fixed monitoring evidence was incomplete.");
    }

    static bool HasFixedKinds(string[] serviceEvidence, string[] registryEvidence) =>
        serviceEvidence.Any(line => line.Contains("monitor-socket|", StringComparison.Ordinal)
            && (line.Contains("kind=HandshakeFailed", StringComparison.Ordinal)
                || line.Contains("kind=Internal", StringComparison.Ordinal)))
        && registryEvidence.Any(line => line.Contains("monitor-registry|source=registry|kind=StatusChanged", StringComparison.Ordinal))
