...
| Transport | Pattern | 64 | 256 | 1024 | 65536 | 131072 | 262144 | 결과 파일 / 메모 |
|-----------|---------|----|-----|------|-------|--------|--------|------------------|
| `tcp` | `MULTI_DEALER_DEALER` | `보류(38.2%)` | `통과(54.6%)` | `통과(70.3%)` | `통과(81.4%)` | `보류(44.0%)` | `보류(43.7%)` | 64/256/1024/131072/262144B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_021556.txt` 대비 Go `perf_go_multi_linux_20260525_021722.txt` 기준이다. 65536B는 fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go `perf_go_multi_linux_20260525_001654.txt` median 기준이다. C처럼 단일 poll loop와 pending socket만 `POLLOUT`으로 두는 poll set 의미를 유지했다. server는 C의 single-part `zlink_recv_part` 의미와 맞춰 64B/65536B에서 public `RecvPart` caller-owned 수신 경로를 사용한다. client active send는 65536B에서 public `Bytes(...)` 경로를 사용해 submit 전 native `Message` 생성을 줄였고, 131072B/262144B는 후보 측정에서 불안정하거나 낮아 기존 경로를 유지한다. 최신 재측정에서도 64B와 131072/262144B는 기준 아래라 보류한다. |
...
| `tcp` | `MULTI_SPOT_SENDSEND` | `보류(43.8%)` | `보류(42.9%)` | `보류(47.2%)` | `보류(18.7%)` | `보류(18.9%)` | `보류(24.7%)` | 64/256/1024/131072/262144B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_021556.txt` 대비 Go `perf_go_multi_linux_20260525_021722.txt` 기준이고, 65536B는 fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go `perf_go_multi_linux_20260524_220932.txt` median 기준이다. C++처럼 client active loop에서 waiting reply를 먼저 `RecvRouted(...DONTWAIT)`로 drain한 뒤 같은 slot에서 즉시 다음 send를 시도하도록 맞췄다. client send는 `ws`가 아니고 65536B 이하일 때만 public `MoveMessage(...)`를 사용해 submit 전 복사를 줄인다. `ws`와 131072B 이상은 후보 측정에서 일부 size가 낮아져 기존 `Message(...)` 경로를 유지한다. 최신 재측정도 전 size complete지만, SPOT 최소 기준에는 아직 못 미친다. |
...
- **2026-05-23 근본 원인 규명 + 수정 (strace 프로파일링)**: 위 large many-client 병목의 진짜 원인을 strace로 특정했다.
  ...
  - **남은 보류(수정 후에도 목표 미달, 후속 개선 대상)**: `MULTI_SPOT_SENDSEND` 전 size(16~35%, per-message routed-echo FFI 비용).
    ...
    `MULTI_SPOT_REQREP` 일부 large, 64B per-message FFI cost. 이들은 LockOSThread로 부분 개선됐으나 목표 미만이라 계속 추적한다.
