# Node Bindings Core Alignment 문서

이 디렉토리는 `bindings/node`를 최신 `core` public surface에 맞춰 정렬하는
실행 문서를 담는다.
...
- [`node-bindings-core-api-alignment-plan.ko.md`](./node-bindings-core-api-alignment-plan.ko.md)
  ...
  public API 철학, 고정 결정, 단계별 구현 범위, Node 성능 계약, 검증 절차를 정의한다.
- [`node-bindings-alignment-execution-guide.ko.md`](./node-bindings-alignment-execution-guide.ko.md)
  메인 플랜을 실제 실행 순서와 완료 판정 기준으로 고정한 실행 문서다.
- [`run_node_bindings_alignment_execution.sh`](./run_node_bindings_alignment_execution.sh)
