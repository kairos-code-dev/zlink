...
- command 39 route를 `Ready` authority fence와 target-owned `ColdActivation` descriptor fence의 닫힌
  ...
  byte `2`에서 target Mesh, stable type, descriptor version, placement profile, affinity key와 deadline을
...
- 공통 schema·golden·validator·decoder fixture와 다섯 언어 exact interface를 같은 계약으로
  ...
  route/`ZLIA` placement 불일치를 negative self-test로 거부한다.
...
- C++ `GeneratedUserSpotRidRetriesActiveCollisionWithinCreateCall`은 generated RID의 same-type
  `AlreadyExists`와 다른 type Active collision을 새 RID로 재시도하고, caller RID `GetOrCreate`의
  `Existing`·`SpotTypeMismatch` 의미를 유지하는지 검증한다. 재시도 timeout은 ceil하지 않고 최초
  steady-clock deadline의 남은 millisecond를 줄여 전달한다.
...
- 남은 조건은 C++ source-side 즉시 reservation cleanup, Node native 두 process E2E, Redis endpoint가 필요한
