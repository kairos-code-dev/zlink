...
| `wss` | `MULTI_DEALER_DEALER` | `보류(37.7%)` | `통과(58.3%)` | `통과(71.2%)` | `보류(42.2%)` | `보류(44.5%)` | `보류(48.0%)` | 64B/256B/1024B/65536B는 fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go `perf_go_multi_linux_20260524_222134.txt` median 기준이고, 131072B/262144B는 제한 재측정 `perf_go_multi_linux_20260524_222545.txt` median 기준이다. C처럼 단일 poll loop와 pending socket `POLLOUT` 의미를 유지했다. Go send builder에 명시적 ownership 이전 단계 `MoveMessage`를 추가했고, active send는 1024B 이하와 wss 131072B/262144B, tls 131072B에서만 이 경로를 사용한다. 262144B를 `MoveMessage(...)`로 넓혀 0.2%/24.4% 수준이던 wss large를 48.0%까지 올렸지만 단순 one-way 기준선에는 아직 닿지 못했다. `Message`의 실패 시 원본 보존 계약은 유지하며 `go test ./...`로 회귀를 확인했다. tls 262144B까지 넓힌 후보 `perf_go_multi_linux_20260524_222040.txt`, `perf_go_multi_linux_20260524_222134.txt`는 full run에서 tls 262144B를 낮춰 wss에만 반영한다. auto-HWM 활성과 size별 `MsgUnit(B)` 일치를 확인했다. |
