...
napi_value create_member_peer_array (napi_env env, zlink_member_peer_entry_t *entries, size_t count)
{
    napi_value arr;
                                           ...
                                           size_t count)
{
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        set_uint32_property (env, obj, "spotKind", static_cast<uint32_t> (entries[i].spot_kind));
        napi_set_element (env, arr, static_cast<uint32_t> (i), obj);
    }
    return arr;
}
...
static bool
read_optional_uint32_property (napi_env env, napi_value value, const char *name, uint32_t *out)
{
    bool has_prop = false;
    napi_value prop;
    if (napi_has_named_property (env, value, name, &has_prop) == napi_ok && has_prop
...
static void read_optional_cstring_property (
  napi_env env, napi_value value, const char *name, char *buf, size_t cap)
{
    bool has_prop = false;
    napi_value prop;
    if (napi_has_named_property (env, value, name, &has_prop) == napi_ok && has_prop
...
bool build_topology_filter (napi_env env, napi_value value, zlink_registry_topology_filter_t *out)
{
    ...
    read_optional_cstring_property (env, value, "channelName", out->channel_name,
                                    sizeof (out->channel_name));
...
napi_value registry_set_heartbeat (napi_env env, napi_callback_info info)
{
    ...
    int rc = zlink_registry_set (reg, ZLINK_REGISTRY_OPT_HEARTBEAT_INTERVAL_MS, interval);
    if (rc == 0)
        rc = zlink_registry_set (reg, ZLINK_REGISTRY_OPT_HEARTBEAT_TIMEOUT_MS, timeout);
    if (rc != 0)
...
napi_value registry_set_broadcast (napi_env env, napi_callback_info info)
{
    ...
    uint32_t interval = 0;
    napi_get_value_uint32 (env, argv[1], &interval);
    int rc = zlink_registry_set (reg, ZLINK_REGISTRY_OPT_BROADCAST_INTERVAL_MS, interval);
    if (rc != 0)
...
napi_value registry_service_summary (napi_env env, napi_callback_info info)
{
    ...
    zlink_registry_service_summary_filter_t *filter_ptr =
      build_registry_service_summary_filter (env, argc >= 2 ? argv[1] : NULL, &filter) ? &filter
                                                                                       : NULL;
    ...
    for (int attempt = 0; attempt < k_snapshot_retry_limit; ++attempt) {
        ...
        rc = zlink_registry_service_summary (registry, filter_ptr, entries.data (), &count);
        if (rc != 0) {
            if (zlink_errno () == ENOBUFS && attempt + 1 < k_snapshot_retry_limit)
                continue;
            return throw_last_error (env, "registry_service_summary failed");
...
napi_value registry_topology (napi_env env, napi_callback_info info)
{
    ...
    zlink_registry_topology_filter_t *filter_ptr =
      build_topology_filter (env, argc >= 2 ? argv[1] : NULL, &filter) ? &filter : NULL;
    for (int attempt = 0; attempt < k_snapshot_retry_limit; ++attempt) {
        size_t count = 0;
...
napi_value discovery_new (napi_env env, napi_callback_info info)
{
    ...
    void *disc = zlink_discovery_new (
      ctx, static_cast<zlink_auto_connect_type_t> (auto_connect_type), channel_name.c_str ());
    if (!disc)
...
napi_value discovery_resolve_spot (napi_env env, napi_callback_info info)
{
    ...
    napi_value obj;
    napi_create_object (env, &obj);
    napi_set_named_property (env, obj, "spotRid", create_routing_id_value (env, route.spot_rid));
...
napi_value registry_set_tls_server (napi_env env, napi_callback_info info)
{
    ...
    int rc = zlink_set_tls_server (registry, cert.c_str (), key.c_str (), require_client);
    if (rc != 0)
...
napi_value registry_set_tls_client (napi_env env, napi_callback_info info)
{
    ...
    int rc = zlink_set_tls_client (registry, ca.c_str (), host.c_str (), trust_system);
    if (rc != 0)
...
napi_value discovery_set_tls_client (napi_env env, napi_callback_info info)
{
    ...
    if (argc >= 4)
        napi_get_value_int32 (env, argv[3], &trust_system);
    int rc = zlink_set_tls_client (discovery, ca.c_str (), host.c_str (), trust_system);
    if (rc != 0)
...
napi_value discovery_set_tls_server (napi_env env, napi_callback_info info)
{
    ...
    if (argc >= 4)
        napi_get_value_int32 (env, argv[3], &require_client);
    int rc = zlink_set_tls_server (discovery, cert.c_str (), key.c_str (), require_client);
    if (rc != 0)
