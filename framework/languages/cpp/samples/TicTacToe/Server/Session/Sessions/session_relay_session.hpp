...
class session_relay_session_t final
  : public zlink::framework::packet_stream_session_t
{
public:
  session_relay_session_t (
    ...
    const zlink::message_t &payload) override
  {
    ...
    auto actor = require_bound_actor (
      std::string ("relaying packet '") + std::string (header.packet_name ()) +
      "'");
    if (!actor) {
      return task_t<void> (
        zlink::framework::result_t<void>::failure (
    ...
    if (_create_match.can_handle (header)) {
      return task_t<void> (
        _create_match.handle (actor.value (), stream, header, payload));
    }
    return task_t<void> (
