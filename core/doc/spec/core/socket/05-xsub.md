...
Receive one payload part of a topic-bearing message from a raw `XSUB` socket.
...
required. `source_rid_out_` is optional and receives `NULL` for raw XSUB. On
success, the function copies the binary topic bytes into the caller's buffer
without appending a NUL and transfers ownership of the payload part to the
caller. The caller must close the part exactly once with
...
Receive every payload part of one multipart message on the same thread with
this function. `*has_more_out_` is `ZLINK_PART_MORE` when another part follows
and `ZLINK_PART_FINAL` for the last part. This function applies to raw SUB and
raw XSUB.
...
subscription. All raw subscriptions in 11.0.0 are byte-prefix filters, so it
always reports `0`.
