...
| `DD-A5` message-flow evidence | 구현 | runner가 각 role의 flow log에서 `message flow` 기록을 확인하고, `flow-customer-gateway.log`, `flow-courier-session.log`, `flow-courier-gateway.log`, `flow-delivery-courier-node-1.log`, `flow-delivery-courier-node-2.log`까지 별도로 확인한다. 최신 통과: `timeout 420s framework/languages/cpp/e2e/DeliveryDispatch/run_e2e.sh`. |
| role split parity | 부분 구현 | Registry, DispatchApi, DispatchCenter, Tracking, CustomerGateway, CourierSession, CourierGateway, CourierActorNode, Probe, Client를 별도 executable로 실행한다. Customer stream과 courier stream endpoint도 분리했다. 남은 차이는 `.NET`의 actor-bound session 배치와 courier actor entry spot 구조다. |
| shared contract parity | 부분 구현 | C++ `Shared/Contracts/messages.hpp`가 배송 생성, tracking, customer subscription, `BindCourierReq`, `EnsureCourierActorReq`, courier session bind, offer notify, courier decision DTO와 typed JSON stream payload hook을 제공한다. ActorNode 내부 entry spot용 DTO 흐름은 아직 단순화되어 있다. |
...
다만 C++ ActorNode role은 아직 `.NET`처럼 actor context의 bound session으로 직접 push하지 않고
CourierSession offer route로 전달한다.

이 gap은 샘플 public 흐름의 차이이므로 완료로 보지 않는다. 후속 작업에서는 customer/courier
actor-bound session 배치와 courier actor entry spot 구조를 C++ public framework API로 더 맞춰야 한다.
