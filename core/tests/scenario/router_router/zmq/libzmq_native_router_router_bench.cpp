...
bool configure_router (void *socket, const char *rid, int hwm)
{
    ...
    if (zmq_setsockopt (socket, ZMQ_ROUTING_ID, rid, strlen (rid)) != 0) {
        fprintf (stderr, "configure_router: ZMQ_ROUTING_ID failed: %s\n",
    ...
    if (!set_opt_int (socket, ZMQ_ROUTER_HANDOVER, one)) {
        fprintf (stderr, "configure_router: ZMQ_ROUTER_HANDOVER failed: %s\n",
                 zmq_strerror (zmq_errno ()));
        return false;
    }
    if (!set_opt_int (socket, ZMQ_ROUTER_MANDATORY, one)) {
        fprintf (stderr, "configure_router: ZMQ_ROUTER_MANDATORY failed: %s\n",
                 zmq_strerror (zmq_errno ()));
        return false;
    }
    if (!set_opt_int (socket, ZMQ_IMMEDIATE, zero)) {
        fprintf (stderr, "configure_router: ZMQ_IMMEDIATE failed: %s\n",
                 zmq_strerror (zmq_errno ()));
        return false;
    }
    if (!set_opt_int (socket, ZMQ_SNDHWM, hwm)) {
        fprintf (stderr, "configure_router: ZMQ_SNDHWM failed: %s\n",
                 zmq_strerror (zmq_errno ()));
        return false;
    }
    if (!set_opt_int (socket, ZMQ_RCVHWM, hwm)) {
        fprintf (stderr, "configure_router: ZMQ_RCVHWM failed: %s\n",
                 zmq_strerror (zmq_errno ()));
        return false;
    }
    if (!set_opt_int (socket, ZMQ_SNDTIMEO, timeout)) {
        fprintf (stderr, "configure_router: ZMQ_SNDTIMEO failed: %s\n",
                 zmq_strerror (zmq_errno ()));
        return false;
    }
    if (!set_opt_int (socket, ZMQ_RCVTIMEO, timeout)) {
        fprintf (stderr, "configure_router: ZMQ_RCVTIMEO failed: %s\n",
                 zmq_strerror (zmq_errno ()));
        return false;
    }
    if (!set_opt_int (socket, ZMQ_LINGER, linger)) {
        fprintf (stderr, "configure_router: ZMQ_LINGER failed: %s\n",
                 zmq_strerror (zmq_errno ()));
        return false;
    }
    return true;
...
int send_3 (void *socket,
            ...
            int flags)
{
    if (zmq_send (socket, target, target_len, flags | ZMQ_SNDMORE) < 0)
        return -1;
    if (zmq_send (socket, header, header_len, flags | ZMQ_SNDMORE) < 0)
        return -1;
    if (zmq_send (socket, payload, payload_len, flags) < 0)
        return -1;
    return 0;
}
...
int recv_3 (void *socket,
            ...
            int *l2)
{
    *l0 = zmq_recv (socket, &f0[0], f0.size (), flags);
    if (*l0 < 0)
        return -1;
    ...
    *l1 = zmq_recv (socket, &f1[0], f1.size (), flags);
    if (*l1 < 0)
        return -1;
    ...
    *l2 = zmq_recv (socket, &f2[0], f2.size (), flags);
    if (*l2 < 0)
        return -1;
...
{
    char ep[128];
    snprintf (ep, sizeof (ep), "inproc://rr-mon-%s-%d", name.c_str (), idx);

    const int events =
      ZMQ_EVENT_CONNECTED | ZMQ_EVENT_CONNECT_DELAYED
      | ZMQ_EVENT_CONNECT_RETRIED
                       | ZMQ_EVENT_DISCONNECTED | ZMQ_EVENT_HANDSHAKE_FAILED_NO_DETAIL
                       | ZMQ_EVENT_HANDSHAKE_FAILED_PROTOCOL
                       | ZMQ_EVENT_HANDSHAKE_FAILED_AUTH
                       | ZMQ_EVENT_HANDSHAKE_SUCCEEDED;

    if (zmq_socket_monitor (socket, ep, events) != 0)
        return false;

    void *m = zmq_socket (ctx, ZMQ_PAIR);
    if (!m)
        return false;
    if (zmq_connect (m, ep) != 0) {
        zmq_close (m);
        return false;
    }
...
void close_monitors (void * /*ctx*/, std::vector<MonitorSocket> &mons)
{
    for (size_t i = 0; i < mons.size (); ++i)
        zmq_close (mons[i].monitor);
    mons.clear ();
}
                          ...
                          const std::chrono::steady_clock::time_point &round_begin)
{
    for (;;) {
        ...
        const int n = zmq_msg_recv (&m1, monitor, ZMQ_DONTWAIT);
        if (n < 0) {
            zmq_msg_close (&m1);
            zmq_msg_close (&m2);
            break;
        ...
        uint16_t evt = 0;
        int32_t value = 0;
        if (static_cast<size_t> (n) >= sizeof (uint16_t)) {
            memcpy (&evt, zmq_msg_data (&m1), sizeof (uint16_t));
        }
        if (static_cast<size_t> (n) >= sizeof (uint16_t) + sizeof (int32_t)) {
            memcpy (&value,
                    static_cast<const char *> (zmq_msg_data (&m1))
                      + sizeof (uint16_t),
                    sizeof (int32_t));
        }

        zmq_msg_recv (&m2, monitor, ZMQ_DONTWAIT);
        zmq_msg_close (&m1);
        zmq_msg_close (&m2);
        ...
        switch (evt) {
            ...
            case ZMQ_EVENT_HANDSHAKE_SUCCEEDED:
                ...
                set_first_timestamp (events.first_connection_ready_ms, elapsed_ms);
                break;
    ...
    for (int i = 0; i < 3; ++i) {
        if (play_client)
            send_3 (play_client, kApiRid, strlen (kApiRid), hdr, strlen (hdr),
                    payload, strlen (payload), ZMQ_DONTWAIT);
        if (api_client)
...
bool run_round (const Config &cfg, Result &out)
{
    ...
    do {
        if (!ctx) {
            fail_reason = "zmq_ctx_new failed";
            break;
        }

        play_server = zmq_socket (ctx, ZMQ_ROUTER);
        play_client = zmq_socket (ctx, ZMQ_ROUTER);
        api_server = zmq_socket (ctx, ZMQ_ROUTER);
        api_client = zmq_socket (ctx, ZMQ_ROUTER);
        ...
        if (zmq_bind (play_server, play_ep.c_str ()) != 0
            || zmq_bind (api_server, api_ep.c_str ()) != 0) {
            fail_reason = "bind failed";
            break;
        }

        if (zmq_connect (play_client, api_ep.c_str ()) != 0
            || zmq_connect (api_client, play_ep.c_str ()) != 0) {
            fail_reason = "connect cross failed";
            break;
        }

        if (cfg.self_connect) {
            if (zmq_connect (play_client, play_ep.c_str ()) != 0
                || zmq_connect (api_client, api_ep.c_str ()) != 0) {
        ...
        zmq_setsockopt (play_server, ZMQ_RCVTIMEO, &rcvtimeo_inf,
                        sizeof (rcvtimeo_inf));
        zmq_setsockopt (api_server, ZMQ_RCVTIMEO, &rcvtimeo_inf,
                        sizeof (rcvtimeo_inf));

        attach_monitor (ctx, play_server, "play-server", cfg.play_port, mons);
        attach_monitor (ctx, play_client, "play-client", cfg.play_port, mons);
        attach_monitor (ctx, api_server, "api-server", cfg.api_port, mons);
        attach_monitor (ctx, api_client, "api-client", cfg.api_port, mons);
        ...
        api_thread = std::thread ([&]() {
            ...
            while (worker_running.load (std::memory_order_acquire)) {
                ...
                for (;;) {
                    l0 = l1 = l2 = 0;
                    const int d = recv_3 (api_server, f0, f1, f2, ZMQ_DONTWAIT,
                                          &l0, &l1, &l2);
                    if (d != 0)
                        break;
                    if (!worker_running.load (std::memory_order_acquire))
        ...
        play_thread = std::thread ([&]() {
            ...
            while (worker_running.load (std::memory_order_acquire)) {
                ...
                for (;;) {
                    l0 = l1 = l2 = 0;
                    const int d = recv_3 (play_server, f0, f1, f2, ZMQ_DONTWAIT,
                                          &l0, &l1, &l2);
                    if (d != 0)
                        break;
                    if (!worker_running.load (std::memory_order_acquire))
    ...
    close_monitors (ctx, mons);

    if (api_client)
        zmq_close (api_client);
    if (api_server)
        zmq_close (api_server);
    if (play_client)
        zmq_close (play_client);
    if (play_server)
        zmq_close (play_server);
    if (ctx)
        zmq_ctx_term (ctx);

    if (!ok) {
        fprintf (stderr, "[libzmq-rr] run_round fail_reason=%s\n", fail_reason);
        return false;
    }
...
int main (int argc, char **argv)
{
    ...
    if (!ok) {
        printf ("[libzmq-rr] FAIL (runner failed)\n");
        return 2;
    }
