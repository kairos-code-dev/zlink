...
| Entry Spot ID claim | Object Server descriptor의 EntrySpotId, descriptor lifecycle과 exact owner lease token | descriptor `NewClaim`에서 생성하고 exact descriptor remove·owner cleanup에서 해제 |
| Host owner lease | `(OwnerId, LeaseGeneration, ExpiresAt, StoreNow)` | Redis TTL |
| Object authority | canonical authority key, opaque payload, StoreVersion, ObjectGeneration, AuthorityOwnerGeneration, current OwnerId·LeaseGeneration, Reserved·Active placement allocation | 명시적 fenced delete까지 durable, TTL 금지 |
| Authority index | snapshot scan용 versioned key index와 active scan lease | authority row와 scan lease에 종속 |
| Creation reservation | object key, stable type, target descriptor, typed capacity bundle와 exact authority fence | Creating recovery 또는 terminal cleanup까지 durable |
| Creation operation terminal | exact source lifecycle·OperationId, reservation fence, `Created`·`Rejected` state와 opaque application reply envelope | 같은 operation의 retry retention이 끝날 때까지 durable |
| Relocation capacity reservation | current authority key·StoreVersion, source·target descriptor와 owner token, typed capacity bundle | standalone NewOwner 또는 exact abort까지 durable, TTL 금지 |
...
| Public MeshNode descriptor | `P:{zlink-location-v3}:descriptor:mesh:D` · HASH |
...
| Descriptor owner token index | `P:{zlink-location-v3}:descriptor:mesh:owner:D` · SET |
...
| Current membership | `P:{zlink-location-v3}:membership:current` · HASH |
...
`ownerLeaseGeneration`, `allocationState`, `objectKind`, `stableType`, `descriptorKey`, `descriptorLifecycleGeneration`,
`capacityBundle` field를 사용한다. Reserved allocation은 여기에
...
Entry Spot ID claim HASH는 `state`, `spotId`, `descriptorKey`, `descriptorLifecycleGeneration`, `ownerId`,
`ownerLeaseGeneration` 여섯 field만 사용한다. `state`는 case-sensitive ASCII `Claimed`이고 `spotId`는 full
Entry Spot ID의 exact UTF-8 bytes다. Generation은 선행 `0` 없는 invariant decimal이다. Optional field,
descriptor JSON 또는 provider 전용 metadata를 추가하지 않는다. 이 HASH에는 TTL을 설정하지 않으며 아래의
exact release transaction으로만 삭제한다.
...
bucket 뒤에 `user_spot` 또는 `instance_spot` token과 stable type을 차례로 encode한다. 따라서 Unicode stable type도
UTF-16 code unit 수가 아니라 UTF-8 byte 수를 사용하며, descriptor lifecycle이 바뀌면 이전 node의
capacity와 새 node의 capacity가 섞이지 않는다.

`capacityBundle`은 domain `zlink-capacity-bundle-v2`, Actor slot, Spot slot, Spot type presence를 차례로
length-prefix segment로 기록한 뒤, entry가 있으면 `(objectKind token, stableType, slot 수)`를 이어 붙인다.
Slot과 presence는 선행 `0` 없는 invariant decimal이며 presence는 `0` 또는 `1`이다. Spot type entry의
`objectKind`는 `user_spot` 또는 `instance_spot`만 허용한다. 같은 kind·stable type의 중복 entry, 음수 slot,
모든 slot이 0인 bundle은 provider I/O 전에 argument validation error로 거부한다.

Creation reservation, standalone relocation reservation과 aggregate record는 다음 exact field set을 사용한다.
Field 이름은 아래 ASCII spelling과 대소문자를 그대로 사용하며 optional field나 provider 전용 field를
추가하지 않는다.
...
Creation reservation의 `state`는 case-sensitive ASCII `Reserved`, `Committed`, `Rejected`, `Aborted` 중
하나다. Creation operation terminal의 `state`는 `Created`, `Rejected`, `Failed` 중 하나다.
Standalone relocation과 aggregate record는 `Reserved`, `Committed`, `Aborted`를 사용한다. Public prepare
결과의 `Prepared`·`AlreadyPrepared`는 operation 결과 이름이며 durable aggregate record의 준비 상태는
`Reserved`다. Creation terminal envelope는 `creation-operation-terminal-v1`의 correlation-free semantic
bytes다. Command head, request correlation과 reply route는 포함하지 않으며 Provider는 envelope를 해석하지
않는다. Terminal key는 exact source node RID bytes, source lifecycle
generation과 operation ID를 포함한다. Reservation ID는 이 operation이 소유했던 creation attempt를
감사하고 stale publication을 거부하는 fence이며, 다른 operation을 같은 terminal result에 합류시키는 key가
아니다. 128-bit ID는 hyphen 없는 32자리 lowercase hex, generation·size는 선행 `0` 없는 decimal,
SHA-256과 inventory digest는 exact 32 raw bytes로 저장한다. Authority key, descriptor key, owner ID,
StoreVersion, reference와 stable type은 계약에서 정한 UTF-8 bytes를 그대로 사용한다. `participants`는
bounded canonical participant set과 mutation을 공통 aggregate encoding으로 serialize한 bytes다.
...
`rid-byte-length`와 `source-generation`은 선행 `0` 없는 decimal, `rid-hex`는 Node RID의 exact UTF-8 bytes를
lowercase hex로 변환한 값, `operation-id-hex`는 high·low unsigned 64-bit를 각각 16자리 lowercase hex로
이어 붙인 32자리 값이다. `terminalEnvelope`는 correlation-free semantic envelope bytes의 lowercase hex
projection이며 decode한 bytes가 1,048,576 bytes를 넘으면 provider I/O 전에 거부한다.
`terminalEnvelopeSha256`은 decode한
bytes의 exact SHA-256이다. `expiresAtUnixMs`는 original operation deadline에 300,000 ms를 더한 값이며 Redis
`TIME` 기준으로 이미 만료된 입력은 mutation 전에 거부한다. Terminal key에는 `PEXPIREAT`을 적용하지만
authority와 creation reservation에는 TTL을 적용하지 않는다.
...
공통 byte fixture는 위 세 record와 Entry Spot ID claim 각각에 대해 exact Redis key bytes와 unordered HASH
`field bytes → value bytes` map을 제공해야 한다. 모든 언어 provider의 contract test는 같은 fixture를
write한 뒤 raw Redis bytes로 비교하고, fixture를 raw Redis에 넣은 뒤 public read 결과도 비교한다.
`HGETALL` 반환 순서나 언어 serializer의 field 순서에는 의존하지 않는다. Field set, state token,
`capacityBundle` bytes 또는 epoch가 다르면 호환 schema로 읽지 않고 startup을 실패한다.

Entry claim fixture는 full RID `mesh-entry-123e4567-e89b-42d3-a456-426614174000`을 사용한다. UUID component는
RFC 4122 version 4와 variant를 가진 lowercase canonical `8-4-4-4-12` 형식이다. 이 RID의 `E`는
`52704385274f30e952774ce79876f362d51f5564752cc4d687bbb60bb05d453e`이고 exact key suffix는
...
`state` value hex는 `436c61696d6564`다. Fixture는 여섯 field가 정확히 존재하고 extra field가 없음을
검증한다.
...
segment여야 한다. User·Instance Spot
capability는 `0..2^31-1`의 invariant decimal segment를 사용하며 `0`은 별도 type limit이 없다는 뜻이다.
Object Server는 Entry Spot ID presence가 `1`이고 Object Client와 object role `none`은 `0`이어야 한다.
Entry Spot ID는 별도의 UUID byte representation으로 바꾸지 않고 full Spot ID UTF-8 bytes를 digest에 넣는다.
...
각 descriptor를 제거하는 transaction은 stored descriptor의 lifecycle generation과 연결된 Entry claim의
`descriptorKey`, `descriptorLifecycleGeneration`, `ownerId`, `ownerLeaseGeneration`을 모두 exact-match한
경우에만 claim도 삭제한다. Claim이 successor lifecycle을 가리키면 descriptor cleanup과 claim delete를 모두
거부하므로 stale cleanup이 successor identity를 해제할 수 없다.
...
Actor key는 global ActorId 하나, Spot key는 global SpotId 하나로 구성한다. MeshName을 key prefix나 uniqueness
...
Missing→Reserved는 Reserve, Reserved→Active는 exact Commit, application rejection의 Reserved→Missing은
exact Reject, infrastructure failure의 Reserved→Missing은 exact Abort만 수행한다.
...
Provider는 `Reserve`, `Commit`, `Reject`, `Abort` closed operation을 제공한다. Provider는 object kind와 stable type을
...
lifecycle generation, typed capacity bundle, exact owner fence와 Framework가 encode한 opaque Creating authority payload를
...
검증하고 변경한다. `Reserve`는 bundle 전체에 대해 Actor·Spot node limit과 Spot type limit을 모두 확인한다.
Actor의 `Commit`과 `Reject`는 reservation에 결합된 exact source operation terminal만 publish한다. 다른
operation이 같은 Actor의 Creating authority를 기다리고 있으면 terminal result를 읽지 않고 authority를 다시
조회한다. Ready면 `Existing`, Missing이면 새 reservation을 경쟁한다.
User Spot 또는 Instance Spot `Reserve`는 같은 Spot ID의 Entry claim key도 같은 transaction에서 확인한다.
`Claimed` record가 있으면 identity conflict로 닫고 authority row, generation, reservation, index와 capacity를
하나도 변경하지 않는다. Framework는 canonical Spot authority key와 Entry claim key를 모두 `KEYS`로 전달하며
provider는 RID 문자열에서 어느 key도 만들지 않는다.
`active + reserved + requested`가 limit을 넘으면 아무 값을 소비하지 않고
`PlacementCapacityExhausted`를 반환한다. `Commit`은 target descriptor lifecycle과
...
User Spot aggregate는 participant별 relocation capacity fence를 만들지 않는다. Aggregate prepare가 participant
expectation과 current Active allocation을 exact-match하고, target descriptor lifecycle·owner lease와
`Actor=N, Spot=1, User Spot type=1`인 typed bundle을 검증한다. Bundle 또는 participant inventory가 다르면
conflict이며 row·capacity·aggregate record를 변경하지 않는다.
...
`PrepareAggregate`는 모든 participant expectation, target descriptor·owner lease와 typed capacity bundle을
확인하고 durable aggregate record를 만든다. 같은 transaction에서 Actor 전체, Spot 전체와 User Spot type
reserved count를 증가시키고 record state를 `Reserved`로 전이한다. Exact duplicate prepare만 idempotent하다.
`CommitAggregate`는 reserved record의 exact generation, source Active allocation match와 target descriptor
lifecycle·owner lease를 다시
확인한다. Source descriptor row·lease가 stale·missing이어도 allocation match가 유지되면 commit할 수 있다.
Target이 stale이면 participant·capacity를 바꾸지 않고 `Reserved` 상태를 유지한다. Reserved bundle 전체를
active로 전환하면서 모든 authority owner,
...
reserved bundle 전체를 정리하고 `Aborted`로 닫는다. 같은 aggregate generation의
...
`R:deleted=0`과 `R:<current field name>` 13개 field를 저장한다. Reserved full snapshot은 같은 13개 field와
위 네 `R:pendingCreation*` field를 모두 저장하므로 current row와 같은 17개 field를 복원한다. Reserved
...
RID, Entry Spot ID, lifecycle generation, normalized max message size, channel membership key, capability와
application version은 immutable하다. Existing channel weight, node placement weight, runtime state, capacity와 maintenance wave만 더
큰 revision으로 갱신한다.
...
protocol error이며 connection을 not-ready로 바꾼다. Object Server update는 descriptor에 기록된 Entry Spot
RID와 current claim의 descriptor lifecycle·owner fence도 exact-match한다. Claim이 없거나 다른 lifecycle을
가리키면 descriptor 일부를 갱신하지 않고 conflict로 끝낸다.
...
RouteMesh Channel, ClientServer Server와 node-wide placement weight는 signed decimal integer `0..10000`으로
encode한다. Provider는 범위 밖 값이나 정수가 아닌 값을 저장하지 않고 configuration error를 반환한다.
Weight update는 같은 lifecycle의 더 큰 descriptor revision과 함께 atomic하게 게시한다. Lower revision,
같은 revision의 다른 값과 narrow unsigned representation으로 truncate한 값은 거부한다.
...
보장한다. Framework는 독립적인 128-bit CSPRNG 기반 RFC 4122 UUID v4를 lowercase canonical
`8-4-4-4-12` 형식으로 encode해 `prefix-<uuid>`를 만든다. 32자리 hex suffix나 process memory byte order를
사용하지 않는다. Redis provider는 full Spot ID bytes를 opaque 값으로 취급하고 Spot ID 구조, prefix 또는 UUID를
해석하지 않는다. Claim은 exact host owner lease token과 descriptor identity를 함께 기록하고,
renew·update·release는 같은 token을 비교한다.

Active conflict는 existing descriptor mutation 0과 `RoutingIdConflict`로 즉시 끝난다. Framework는 다른
UUID로 두 번째 claim을 시도하지 않는다. Replacement lifecycle은 startup 전에 새 UUID v4 RID를 발급하지만,
그 lifecycle의 `NewClaim`도 active conflict가 발생하면 즉시 실패한다.

### 7.1 Entry Spot global identity claim

Object Server lifecycle의 Entry Spot ID는 Framework가 만든 독립적인 RFC 4122 UUID v4 component를 사용한다.
UUID component는 lowercase canonical `8-4-4-4-12` 형식이며 32자리 hex나 process memory byte order를
사용하지 않는다. Full Spot ID 구성은 Entry Spot identity 계약을 따른다. Framework는 provider 호출 전에 형식,
version, variant와 UTF-8 길이를 검증하지만 Redis provider는 full Spot ID bytes를 opaque 값으로 취급하고
prefix, marker 또는 UUID를 parse하지 않는다.

Object Server MeshNode descriptor의 `NewClaim`은 별도 public Entry claim method를 호출하지 않는다. 같은
server-side transaction에서 다음 순서로 검증하고 변경한다.

1. Exact current host owner lease와 descriptor lifecycle을 확인한다.
2. `(MeshName, MeshNode RID)` descriptor identity conflict를 확인한다.
3. 같은 Entry Spot ID의 existing `Claimed` record와 User·Instance Spot current authority row가 모두
   없는지 확인한다.
4. Public descriptor, admission metadata, descriptor index·owner index와 Entry claim HASH를 함께 생성한다.

어느 identity든 충돌하면 descriptor, claim, index와 counter mutation은 0이다. Exact duplicate `NewClaim`은
descriptor bytes, lifecycle, owner token과 Entry claim 여섯 field가 모두 같을 때만 idempotent하다.
다른 active claim과의 충돌은 `RoutingIdConflict`로 즉시 끝나며 Framework는 다른 Entry Spot ID로 두 번째
claim을 시도하지 않는다.
Framework는 descriptor key, Entry claim key와 같은 Spot ID의 canonical authority key를 모두 script
`KEYS`로 전달한다. Provider는 descriptor JSON이나 RID를 parse해 Redis key를 계산하지 않는다.

MeshNode descriptor remove는 stored descriptor의 exact owner lease와 lifecycle generation을 검증하고 연결된
Entry claim의 exact field를 같은 transaction에서 확인한 뒤 descriptor, indexes와 claim을 함께 삭제한다.
`RemoveAllByOwner`도 descriptor마다 같은 fenced release를 적용한다. 이미 같은 RID를 successor lifecycle이
claim했으면 이전 lifecycle의 remove·cleanup은 successor descriptor와 claim을 변경하지 않는다.
...
- Descriptor remove와 `RemoveAllByOwner`가 exact owner lease·lifecycle·claim field를 확인해 stale cleanup으로
  successor Entry claim을 삭제하지 않는다.
...
- Entry Spot ID의 UUID component가 RFC 4122 UUID v4 lowercase canonical 형식이고 provider가 full Spot ID를
  parse하지 않으며, schema epoch 3 raw-byte fixture의 exact claim key·여섯 HASH field·`Claimed` state를
  모든 provider가 동일하게 처리한다.
