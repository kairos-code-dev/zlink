...
[C++ 묶음](../README.ko.md) | [Runtime Architecture](../internals/runtime-architecture.ko.md) | [Application Framework](cpp-application-framework.ko.md) | [Framework 인터페이스](cpp-framework-interfaces.ko.md) | [HTTP Hosting](cpp-http-hosting.ko.md) | [HTTP Client](../../../http-client/cpp/README.ko.md)
...
기준선으로 삼는다. 하지만 public application model은 `.NET Zlink.Framework`와
`ASP.NET Core Minimal API`의 개념을 C++ 스타일로 투영한다.
...
- route mapping은 `.NET` Minimal API의 `MapGet`, `MapPost`, `MapPut`, `MapDelete`에 대응하는
...
초기 구현에서 완전 async 전환이 한 번에 어렵다면, 아래 순서로 진행한다.
...
- `http_host_service_t`에서 listener, connection, request pipeline을 분리한다.
...
- logging/monitoring/metrics event를 연결한다.
- HTTP server benchmark를 추가한다.
- p95/p99 latency와 concurrency 기준을 기록한다.
