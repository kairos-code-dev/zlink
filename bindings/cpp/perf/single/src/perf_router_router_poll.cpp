// ROUTER-ROUTER-POLL benchmark: one-way router->router with poll-assisted recv.
...
bool wait_for_input (zlink::socket_t &socket)
{
    zlink_pollitem_t item = {socket.handle (), 0, ZLINK_POLLIN, 0};
                ...
                perf::single::latency_stats_t *latency_out)
{
    ...
    std::thread receiver_thread ([&] () {
        ...
        while (true) {
            ...
            bool header_ok = false;
            int recv_rc = recv_router_header (
              receiver, payload.size (), zlink::recv_flag::none, &header, &header_ok);
            if (recv_rc > 0) {
...
void run_pattern_router_router_poll (const std::string &transport,
                                     size_t msg_size,
                                     const std::string &lib_name)
{
    if (!perf::single::transport_available (transport)) {
        std::cout << "UNSUPPORTED,ROUTER_ROUTER_POLL," << transport
                  << std::endl;
        return;
    }
...
int main (int argc, char **argv)
{
    return perf::single::run_standard_bench_main (
      argc, argv, run_pattern_router_router_poll);
}
