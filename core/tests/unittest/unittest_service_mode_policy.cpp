...
void noop_reply_handler (zlink_request_result_t, zlink_msg_t *parts_, size_t part_count_, void *)
{
    zlink_multipart_close (parts_, part_count_);
}
...
bool bind_registry_test_endpoints (void *registry_, std::string *pub_out_, std::string *router_out_)
{
    for (int i = 0; i < 64; ++i) {
        char pub_endpoint[128];
        char router_endpoint[128];
        if (!allocate_loopback_tcp_endpoint (pub_endpoint, sizeof (pub_endpoint))
            || !allocate_loopback_tcp_endpoint (router_endpoint, sizeof (router_endpoint))
            || strcmp (pub_endpoint, router_endpoint) == 0) {
            continue;
        }
        if (zlink_registry_bind (registry_, pub_endpoint, router_endpoint) == ZLINK_BIND_OK) {
            if (pub_out_)
...
{
    return zlink_test_wait_until (timeout_ms_, [=] {
        if (zlink_subscribe (subject_, NULL, parts_out_, part_count_out_, topic_out_,
...
bool wait_for_router_payload_local (void *router_, const char *expected_payload_, int timeout_ms_)
{
    return zlink_test_wait_until (timeout_ms_, [=] {
        ...
        if (zlink_router_recv (router_, &source_node_rid, &source_spot_rid, &request_seq, &parts,
            ...
            == ZLINK_RECV_OK) {
            const bool matched = part_count == 1
...
void test_spot_callback_policy ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zlink_poller_add (poller, spot, spot, ZLINK_POLLIN));
    TEST_ASSERT_SUCCESS_ERRNO (zlink_poller_remove (poller, spot));

    TEST_ASSERT_SUCCESS_ERRNO (zlink_poller_add (poller, spot, spot, ZLINK_POLLOUT));
    TEST_ASSERT_SUCCESS_ERRNO (zlink_poller_remove (poller, spot));
    ...
    char topic[64];
    size_t topic_len = sizeof (topic);
    TEST_ASSERT_EQUAL_INT (ZLINK_RECV_NO_DATA, zlink_subscribe (spot, NULL, &parts, &part_count,
...
void test_stream_send_ready_is_independent_from_recv_callback ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zlink_recv_handler (stream, &noop_socket_handler, NULL));
    TEST_ASSERT_NOT_EQUAL (ZLINK_CONFIG_OK,
                           zlink_poller_add (poller, stream, stream, ZLINK_POLLIN));
...
void test_generic_monitor_poller_accepts_non_pollin_events ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zlink_poller_add (poller, monitor, monitor, ZLINK_POLLOUT));
...
void test_spot_node_topic_surface_and_callback_modes ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zlink_set_subscription (node, "bench"));
    TEST_ASSERT_SUCCESS_ERRNO (zlink_unset_subscription (node, "bench"));
    TEST_ASSERT_EQUAL_INT (ZLINK_HANDLER_OK,
    ...
    TEST_ASSERT_NOT_EQUAL (ZLINK_SUBMIT_OK, zlink_send_rid (node, &routing_id, &routed_part, 1, 0));
    TEST_ASSERT_EQUAL_INT (ENOTSUP, zlink_errno ());
    zlink_msg_close (&routed_part);
    zlink_msg_close (&part);
    TEST_ASSERT_EQUAL_INT (ZLINK_RECV_BUSY, zlink_subscribe (node, NULL, &parts, &part_count, topic,
                                                             &topic_len, ZLINK_DONTWAIT));
    TEST_ASSERT_EQUAL_INT (EBUSY, zlink_errno ());
    TEST_ASSERT_SUCCESS_ERRNO (zlink_poller_add (poller, node, node, ZLINK_POLLIN));
    TEST_ASSERT_SUCCESS_ERRNO (zlink_poller_remove (poller, node));
    TEST_ASSERT_SUCCESS_ERRNO (zlink_poller_add (poller, node, node, ZLINK_POLLOUT));
    TEST_ASSERT_SUCCESS_ERRNO (zlink_poller_remove (poller, node));
...
void test_spot_tls_configuration_is_node_owned ()
{
    ...
    errno = 0;
    TEST_ASSERT_NOT_EQUAL (ZLINK_CONFIG_OK,
                           zlink_set_tls_server (spot, "server.crt", "server.key", 0));
    TEST_ASSERT_EQUAL_INT (ENOTSUP, zlink_errno ());

    errno = 0;
    TEST_ASSERT_NOT_EQUAL (ZLINK_CONFIG_OK, zlink_set_tls_client (spot, "ca.crt", "localhost", 0));
    TEST_ASSERT_EQUAL_INT (ENOTSUP, zlink_errno ());

    TEST_ASSERT_SUCCESS_ERRNO (zlink_set_tls_server (node, "server.crt", "server.key", 0));
    TEST_ASSERT_SUCCESS_ERRNO (zlink_set_tls_client (node, "ca.crt", "localhost", 0));
...
void test_spot_node_attach_discovery_rejects_duplicate_discovery ()
{
    ...
    errno = 0;
    TEST_ASSERT_NOT_EQUAL (ZLINK_CONFIG_OK, zlink_spot_node_attach_discovery (node, discovery));
    TEST_ASSERT_EQUAL_INT (EBUSY, zlink_errno ());
...
void test_spot_node_attach_discovery_gates_manual_peer_apis_but_allows_rid_aliases ()
{
    void *ctx = zlink_ctx_new ();
    TEST_ASSERT_NOT_NULL (ctx);
    ...
    TEST_ASSERT_EQUAL_INT (EBUSY, zlink_errno ());
    TEST_ASSERT_EQUAL (ZLINK_CONNECT_OK, zlink_spot_node_disconnect_peer_rid (node, &rid));
...
void test_spot_publish_part_final_keeps_single_message_contract ()
{
    ...
    TEST_ASSERT_EQUAL_INT (
      ZLINK_SUBMIT_OK,
      zlink_spot_publish_part (spot, "bench", &first, ZLINK_SEND_FLAGS_NONE, ZLINK_PART_FINAL));
    ...
    TEST_ASSERT_EQUAL_INT (
      ZLINK_SUBMIT_OK,
      zlink_spot_publish_part (spot, "bench", &second, ZLINK_SEND_FLAGS_NONE, ZLINK_PART_FINAL));
...
void test_discovery_protocol_accepts_socket_family_and_roles ()
{
    ...
    TEST_ASSERT_TRUE (is_valid_auto_connect_type (ZLINK_AUTO_CONNECT_ROUTE_MESH));
    TEST_ASSERT_TRUE (is_valid_auto_connect_type (ZLINK_AUTO_CONNECT_CLIENT_SERVER));
    TEST_ASSERT_TRUE (is_valid_auto_connect_type (ZLINK_AUTO_CONNECT_DEALER_MESH));
    TEST_ASSERT_TRUE (is_valid_auto_connect_type (ZLINK_AUTO_CONNECT_FANOUT));
    TEST_ASSERT_TRUE (is_valid_auto_connect_type (ZLINK_AUTO_CONNECT_SPOT_MESH));
    TEST_ASSERT_FALSE (is_valid_auto_connect_type (ZLINK_AUTO_CONNECT_INVALID));

    TEST_ASSERT_TRUE (
      auto_connect_type_allows_role (ZLINK_AUTO_CONNECT_ROUTE_MESH, service_role_router));
    TEST_ASSERT_FALSE (
      auto_connect_type_allows_role (ZLINK_AUTO_CONNECT_ROUTE_MESH, service_role_dealer));
    TEST_ASSERT_TRUE (
      auto_connect_type_allows_role (ZLINK_AUTO_CONNECT_CLIENT_SERVER, service_role_router));
    TEST_ASSERT_TRUE (
      auto_connect_type_allows_role (ZLINK_AUTO_CONNECT_CLIENT_SERVER, service_role_dealer));
    TEST_ASSERT_TRUE (
      auto_connect_type_allows_role (ZLINK_AUTO_CONNECT_DEALER_MESH, service_role_dealer));
    TEST_ASSERT_TRUE (auto_connect_type_allows_role (ZLINK_AUTO_CONNECT_FANOUT, service_role_pub));
    TEST_ASSERT_TRUE (auto_connect_type_allows_role (ZLINK_AUTO_CONNECT_FANOUT, service_role_sub));
    TEST_ASSERT_TRUE (
      auto_connect_type_allows_role (ZLINK_AUTO_CONNECT_SPOT_MESH, service_role_spot));
    TEST_ASSERT_TRUE (
      auto_connect_type_allows_role (ZLINK_AUTO_CONNECT_SPOT_MESH, service_role_router));
}
...
void test_discovery_protocol_derives_socket_roles_and_matching ()
{
    ...
    TEST_ASSERT_EQUAL_UINT16 (service_role_router,
                              derive_socket_service_role (ZLINK_CORE_SOCKET_ROUTER));
    TEST_ASSERT_EQUAL_UINT16 (service_role_dealer,
                              derive_socket_service_role (ZLINK_CORE_SOCKET_DEALER));
    TEST_ASSERT_EQUAL_UINT16 (service_role_pub, derive_socket_service_role (ZLINK_CORE_SOCKET_PUB));
    TEST_ASSERT_EQUAL_UINT16 (service_role_sub, derive_socket_service_role (ZLINK_CORE_SOCKET_SUB));
    TEST_ASSERT_EQUAL_UINT16 (service_role_invalid,
                              derive_socket_service_role (ZLINK_CORE_SOCKET_PAIR));

    TEST_ASSERT_TRUE (auto_connect_type_allows_raw_socket (ZLINK_AUTO_CONNECT_CLIENT_SERVER));
    TEST_ASSERT_FALSE (auto_connect_type_allows_raw_socket (ZLINK_AUTO_CONNECT_SPOT_MESH));
}
...
void test_discovery_protocol_applies_socket_auto_connect_policy ()
{
    ...
    TEST_ASSERT_TRUE (socket_auto_connect_target_matches (ZLINK_AUTO_CONNECT_FANOUT,
                                                          service_role_sub, service_role_pub,
                                                          local_rid, remote_rid, "", "tcp://pub"));
    TEST_ASSERT_FALSE (socket_auto_connect_target_matches (ZLINK_AUTO_CONNECT_FANOUT,
    ...
    TEST_ASSERT_TRUE (socket_auto_connect_target_matches (
      ZLINK_AUTO_CONNECT_CLIENT_SERVER, service_role_dealer, service_role_router, local_rid,
      remote_rid, "", "tcp://router"));
    TEST_ASSERT_FALSE (socket_auto_connect_target_matches (
      ZLINK_AUTO_CONNECT_CLIENT_SERVER, service_role_dealer, service_role_dealer, local_rid,
      remote_rid, "", "tcp://dealer"));
    ...
    TEST_ASSERT_TRUE (socket_auto_connect_target_matches (
      ZLINK_AUTO_CONNECT_SPOT_MESH, service_role_spot, service_role_spot, local_rid, remote_rid,
      "tcp://a", "tcp://b"));
    TEST_ASSERT_TRUE (socket_auto_connect_target_matches (
      ZLINK_AUTO_CONNECT_SPOT_MESH, service_role_spot, service_role_spot, remote_rid, local_rid,
      "tcp://b", "tcp://a"));
    TEST_ASSERT_FALSE (socket_auto_connect_target_matches (
      ZLINK_AUTO_CONNECT_SPOT_MESH, service_role_spot, service_role_router, local_rid,
      remote_rid, "tcp://a", "tcp://b"));
}
...
void test_discovery_new_accepts_socket_family ()
{
    ...
    void *discovery = zlink_discovery_new (ctx, ZLINK_AUTO_CONNECT_CLIENT_SERVER, "socket-svc");
    TEST_ASSERT_NOT_NULL (discovery);
...
void test_socket_attach_discovery_rejects_unsupported_socket_type ()
{
    ...
    TEST_ASSERT_NOT_EQUAL (ZLINK_CONFIG_OK, zlink_socket_attach_discovery (pair, discovery));
    TEST_ASSERT_EQUAL_INT (ENOTSUP, zlink_errno ());
...
void test_socket_attach_discovery_gates_manual_peer_apis ()
{
    ...
    TEST_ASSERT_NOT_EQUAL (ZLINK_CONNECT_OK, zlink_connect (dealer, "tcp://127.0.0.1:39001"));
    TEST_ASSERT_EQUAL_INT (EFSM, zlink_errno ());
    TEST_ASSERT_NOT_EQUAL (ZLINK_CONNECT_OK, zlink_disconnect (dealer, "tcp://127.0.0.1:39001"));
    TEST_ASSERT_EQUAL_INT (EFSM, zlink_errno ());
    TEST_ASSERT_NOT_EQUAL (ZLINK_CONNECT_OK, zlink_unbind (dealer, "tcp://127.0.0.1:39001"));
    TEST_ASSERT_EQUAL_INT (EFSM, zlink_errno ());
...
void test_socket_attach_discovery_fails_after_bind_without_registry ()
{
    ...
    TEST_ASSERT_NOT_EQUAL (ZLINK_CONFIG_OK, zlink_socket_attach_discovery (router, discovery));
    TEST_ASSERT_EQUAL_INT (EAGAIN, zlink_errno ());
...
void test_discovery_new_rejects_invalid_auto_connect_type ()
{
    ...
    void *discovery = zlink_discovery_new (ctx, ZLINK_AUTO_CONNECT_INVALID, "socket-svc");
...
void test_socket_attach_discovery_rejects_type_role_mismatch ()
{
    ...
    void *discovery = zlink_discovery_new (ctx, ZLINK_AUTO_CONNECT_FANOUT, "fanout-svc");
    TEST_ASSERT_NOT_NULL (discovery);
    void *dealer = zlink_socket (ctx, ZLINK_SOCKET_DEALER);
    TEST_ASSERT_NOT_NULL (dealer);

    TEST_ASSERT_NOT_EQUAL (ZLINK_CONFIG_OK, zlink_socket_attach_discovery (dealer, discovery));
    TEST_ASSERT_EQUAL_INT (ENOTSUP, zlink_errno ());
...
void test_spot_node_attach_discovery_rejects_unsupported_socket_role ()
{
    ...
    TEST_ASSERT_TRUE (
      bind_registry_test_endpoints (registry, &registry_pub_endpoint, &registry_router_endpoint));

    void *discovery = zlink_discovery_new (ctx, ZLINK_AUTO_CONNECT_CLIENT_SERVER, "svc-dealer");
    TEST_ASSERT_NOT_NULL (discovery);
    ...
    TEST_ASSERT_SUCCESS_ERRNO (register_endpoint (static_cast<zlink::discovery_t *> (discovery),
                                                  "tcp://127.0.0.1:39111", &registered_endpoint,
                                                  NULL, service_role_dealer));
...
void test_discovery_resolve_spot_rejects_invalid_arguments ()
{
    ...
    TEST_ASSERT_NOT_EQUAL (ZLINK_CONFIG_OK, zlink_discovery_resolve_spot (NULL, &spot_rid, &route));
    TEST_ASSERT_EQUAL_INT (EINVAL, zlink_errno ());

    TEST_ASSERT_NOT_EQUAL (ZLINK_CONFIG_OK, zlink_discovery_resolve_spot (discovery, NULL, &route));
    TEST_ASSERT_EQUAL_INT (EINVAL, zlink_errno ());
...
void test_socket_channel_name_metadata_roundtrip ()
{
    ...
    TEST_ASSERT_NOT_EQUAL (ZLINK_CONFIG_OK,
                           zlink_socket_get_channel_name (dealer, buf, sizeof (buf), &len));
    TEST_ASSERT_EQUAL_INT (ENOENT, zlink_errno ());

    TEST_ASSERT_EQUAL_INT (ZLINK_CONFIG_OK, zlink_socket_set_channel_name (dealer, "fixed-chan"));
...
int main (void)
{
    ...
    RUN_TEST (test_spot_node_attach_discovery_gates_manual_peer_apis_but_allows_rid_aliases);
