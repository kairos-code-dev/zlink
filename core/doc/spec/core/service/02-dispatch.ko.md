...
`TRANSFER_CONTROL`은 record의 Actor generation이 owner인 infrastructure domain에서만 반환한다. 이
control은 `zlink_mesh_operation_kind_t`로 표현하지 않고 `COMPLETION` record를 만들지 않는다.
`operation_id`는 zero이고 `operation_kind`, `terminal_result`와 `failure_errno`는 적용되지 않으며
application domain에 들어가지 않는다.
...
claim은 owner kind, generation과 domain 하나를 소유하므로 recv 함수는 MeshNode, Spot, Actor handle 또는
domain을 다시 받지 않는다. record kind가 claim owner와 domain에 맞지 않으면 Core가 반환하지 않는다.
application과 infrastructure domain은 서로 다른 claim으로 독립적으로 drain한다.
