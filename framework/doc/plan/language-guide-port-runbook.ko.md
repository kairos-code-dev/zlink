...
## 6. 선행 작업 — 이것이 이 방식의 실제 비용

**6.1 샘플에 발췌 마커 심기.** core 샘플은 애초에 문서용 자립 예제지만, framework 샘플은
실제 서비스 코드라 "가이드에 실을 크기"의 구간이 없다. 공통 샘플 7종 × 5언어에 마커를
심는 작업이 선행된다. 이 작업을 건너뛰면 결국 복붙으로 돌아간다.

챕터별로 어떤 구간이 필요한지는 공통 정본을 쓰는 lane이 정의하고, 각 언어 lane이 자기
샘플에 심는다.
...
| 산출물 | 내용 |
| --- | --- |
| 표면 매핑표 | dotnet 표면 ↔ 각 언어 표면(§9) |
| 스니펫 구간 정의 | 공통 정본 12장이 인용할 구간 목록(샘플·용도별 마커 이름) |
| 체커 확장 | `check_doc_tabs.py`에 framework 5언어 집합 추가 |
| 사이트 신설 | `framework/doc/site/` mkdocs 프로젝트와 배포 경로(§6.2) |

**Phase 1 — 공통 정본 12장 (dotnet lane 주도)**

기존 dotnet 문서를 공통으로 승격하면서 코드블록을 스니펫으로 바꾼다. 이 시점에는 dotnet
탭만 채워진다. 산문에서 .NET 고유 표현을 걷어내는 것이 이 단계의 핵심이다.

**Phase 2 — 언어별 병렬**

| lane | 하는 일 |
| --- | --- |
| **cpp** | 자기 샘플에 마커 심기 → 공통 정본에 cpp 탭 채우기 → 언어별 5장 + cpp 전용 3장 + 진입점 |
...
**Phase 0에서 채울 것** — 공통 정본 산문이 타입 이름을 언급할 때 어느 이름을 쓸지가
여기서 정해진다. **비어 있는 칸을 지어내지 않는다.**
