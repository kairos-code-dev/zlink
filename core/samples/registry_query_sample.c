...
int main (void)
{
    ...
    void *query = zlink_registry_query_client_new (ctx);
    ...
    assert (zlink_registry_query_client_connect (query, registry_router) == 0);
    ...
    while (!found) {
        ...
        count = 16;
        assert (zlink_registry_query_snapshot (query, NULL, entries, &count) == 0);
    ...
    zlink_registry_query_destroy (&query);
