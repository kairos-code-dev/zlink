...
inline long remaining_wait_ms (const std::chrono::steady_clock::time_point &deadline)
{
...
inline bool wait_for_control_readable (void *control_sub,
...
inline bool connect_peer (void *node, const std::string &endpoint)
{
    ...
    return zlink_spot_node_connect_peer (node, endpoint.c_str ()) == ZLINK_CONNECT_OK;
}

inline bool apply_control_options (void *pub, void *sub, const multi_bench_settings_t &settings)
{
    if (!pub || !sub)
        return false;
    ...
    if (zlink_set_option (pub, ZLINK_OPT_LINGER, &linger_ms, sizeof (linger_ms)) != ZLINK_CONFIG_OK
        ...
        || zlink_set_pub_option (pub, ZLINK_PUB_OPT_NODROP, &nodrop, sizeof (nodrop))
             != ZLINK_CONFIG_OK
        || zlink_set_option (sub, ZLINK_OPT_LINGER, &linger_ms, sizeof (linger_ms))
             != ZLINK_CONFIG_OK
...
{
    if (!session || !node || local_endpoint.empty () || server_endpoint.empty () || !topic
        || !*topic) {
        errno = EINVAL;
        return false;
    }
    ...
    if (zlink_spot_node_connect_peer (node, server_endpoint.c_str ()) != ZLINK_CONNECT_OK
    ...
    perf_print_auto_hwm_snapshot (session->node, true, "spotnode_control_pub", transport);
    perf_print_auto_hwm_snapshot (session->node, true, "spotnode_control_sub", transport);
    perf_print_auto_hwm_snapshot (session->pub, true, "spotend_control_pub", transport);
    perf_print_auto_hwm_snapshot (session->sub, true, "spotend_control_sub", transport);
...
inline bool initialize_client_control_session (const std::string &transport,
                                               ...
                                               std::atomic<bool> *connected_flag)
{
    if (!session || server_control_endpoint.empty () || local_endpoint.empty () || !topic
        || !*topic) {
        errno = EINVAL;
        return false;
    }

    if (!session->node || !setup_tls_server (session->node, transport)
    ...
    if (!initialize_client_session (session->node, server_control_endpoint, session->local_endpoint,
                                    topic, settings, session, connected_flag)) {
        destroy_client_session (session);
        return false;
    }
                                    ...
                                    SlotT *slot)
{
    ...
    if (!apply_benchmark_context_auto_hwm_msg_unit (ctx.get (),
    ...
    if (!apply_sub_options || !apply_sub_options (slot->handle, settings)
        || zlink_spot_node_connect_peer (slot->node, endpoint.c_str ()) != ZLINK_CONNECT_OK
    ...
    perf_print_auto_hwm_snapshot (slot->node, true, "spotnode_data_sub", transport);
    perf_print_auto_hwm_snapshot (slot->handle, true, "spotend_data_sub", transport);

    return true;
}

inline std::string
bind_data_endpoint (void *node, const std::string &transport, const std::string &token)
{
    const int bind_port = resolve_multi_int_env ("PERF_MULTI_SERVER_BIND_PORT", 0, 0);
    if (bind_port > 0) {
        return perf_bind_endpoint_once (node, make_fixed_endpoint (transport, bind_port), transport,
                                        &perf_bind_spot_node_endpoint, false);
    }
...
#endif
    (void) token;
    return perf_bind_fixed_endpoint_range (node, transport, base_port, 64,
                                           &perf_bind_spot_node_endpoint);
}

inline std::string bind_control_endpoint (void *node, const std::string &transport)
{
    const int bind_port = resolve_multi_int_env ("PERF_MULTI_SERVER_CONTROL_BIND_PORT", 0, 0);
    if (bind_port > 0) {
        return perf_bind_endpoint_once (node, make_fixed_endpoint (transport, bind_port), transport,
                                        &perf_bind_spot_node_endpoint, false);
    }
...
inline bool initialize_server_session (ctx_guard_t &ctx,
                                       ...
                                       const char *topic,
                                       const multi_bench_settings_t &settings,
                                       bool (*apply_server_options) (void *,
                                                                     const multi_bench_settings_t &,
                                                                     const std::string &,
                                                                     size_t),
                                       server_session_t *session)
{
    ...
    if (!session->node || !session->control_node || !setup_tls_server (session->node, transport)
    ...
    if (!apply_benchmark_spot_node_hwm (session->node, settings.hwm, true, true)
        || !apply_benchmark_spot_node_hwm (session->control_node, settings.hwm, true, true)) {
        destroy_server_session (session);
        return false;
    }

    session->pub = perf_create_default_spot_handle (session->node);
    session->control_pub = perf_create_default_spot_handle (session->control_node);
    session->control_sub = perf_create_default_spot_handle (session->control_node);
    if (!session->pub || !session->control_pub || !session->control_sub
        || !apply_server_options (session->pub, settings, transport, max_msg_size)
        || !apply_control_options (session->control_pub, session->control_sub, settings)
        || zlink_set_subscription (session->control_sub, topic) != ZLINK_CONFIG_OK) {
        destroy_server_session (session);
        return false;
    }
    ...
    session->control_endpoint = bind_control_endpoint (session->control_node, transport);
    if (session->control_endpoint.empty ()) {
        destroy_server_session (session);
        return false;
    }

    perf_print_auto_hwm_snapshot (session->node, true, "spotnode_data_pub", transport);
    perf_print_auto_hwm_snapshot (session->pub, true, "spotend_data_pub", transport);
    perf_print_auto_hwm_snapshot (session->control_node, true, "spotnode_control_pub", transport);
    perf_print_auto_hwm_snapshot (session->control_node, true, "spotnode_control_sub", transport);
    perf_print_auto_hwm_snapshot (session->control_pub, true, "spotend_control_pub", transport);
    perf_print_auto_hwm_snapshot (session->control_sub, true, "spotend_control_sub", transport);
                                           ...
                                           const std::atomic<int> *fatal_errno)
{
    ...
    while (std::chrono::steady_clock::now () < deadline) {
        ...
        if (perf_stop_requested ().load (std::memory_order_acquire)
            || (fatal_errno && fatal_errno->load (std::memory_order_acquire) != 0)) {
            errno = EIO;
            return false;
        }

        if (perf_socket_poll (NULL, 0, std::min<long> (remaining_wait_ms (deadline), 50)) < 0
            && zlink_errno () != EINTR)
            return false;
    }
...
inline bool ensure_connected_peers (void *node,
...
inline bool accept_reverse_connect_request (void *node,
...
inline void disconnect_peers (void *node,
                              const perf_multi_spot_handshake::peer_registry_t &registry)
{
    if (!node)
        return;
...
inline bool
publish_control_payload (void *control_pub, const char *topic, const std::string &payload)
{
    ...
    const auto deadline =
      ...
      + std::chrono::milliseconds (
        std::max (1, resolve_multi_int_env ("PERF_MULTI_CONNECT_READY_TIMEOUT_MS", 1000, 0)));
    ...
    while (std::chrono::steady_clock::now () < deadline) {
        ...
        const int rc = perf_zlink_spot_publish_parts (control_pub, topic, &part, 1,
                                     ...
                                     bool *received)
{
    ...
    const int rc =
      perf_zlink_spot_subscribe_parts (control_sub, NULL, &parts, &part_count, topic, &topic_len,
    ...
    if (control_topic_matches (topic, topic_len, expected_topic) && part_count > 0) {
        payload_out->assign (static_cast<const char *> (zlink_msg_data (&parts[0])),
...
inline bool
publish_ready_slot (void *control_pub, const char *topic, size_t msg_size, size_t slot_index)
{
    return publish_control_payload (
      control_pub, topic,
      perf_multi_spot_handshake::make_ready_slot_command (msg_size, slot_index));
}

inline bool
publish_ready_count (void *control_pub, const char *topic, size_t msg_size, size_t ready_count)
{
    return publish_control_payload (
      control_pub, topic,
      perf_multi_spot_handshake::make_ready_count_command (msg_size, ready_count));
}
...
inline bool publish_start (void *control_pub, const char *topic, size_t msg_size)
{
    return publish_control_payload (control_pub, topic,
                                    perf_multi_spot_handshake::make_start_command (msg_size));
}

inline bool
publish_data_endpoint (void *control_pub, const char *topic, const std::string &endpoint)
{
    if (endpoint.empty ()) {
        errno = EINVAL;
        return false;
    ...
    return publish_control_payload (
      control_pub, topic, perf_multi_spot_handshake::make_data_endpoint_command (endpoint));
}

inline bool wait_for_ready_units (void *control_sub,
                                  const char *topic,
    ...
    while (std::chrono::steady_clock::now () < deadline) {
        ...
        if (received && !payload.empty ()) {
            if (bench_transition_debug_enabled ()) {
                std::cerr << "[multi-spot-control] recv ts_ns=" << perf_multi_metric::now_ns ()
            ...
            if (perf_multi_spot_handshake::parse_ready_slot_command (
                ...
                perf_multi_spot_handshake::record_ready_slot (ready_state, ready_size, slot_index);
            } else if (perf_multi_spot_handshake::parse_ready_count_command (
                         payload.data (), payload.size (), &ready_size, &ready_count)) {
                perf_multi_spot_handshake::record_ready_count (ready_state, ready_size,
                                                               ready_count);
            }
        }
...
inline bool
ensure_connected (void *node, const std::string &endpoint, std::atomic<bool> *connected_flag)
{
    if (!node || endpoint.empty ()) {
        errno = EINVAL;
        return false;
    }
    if (connected_flag && connected_flag->load (std::memory_order_acquire)) {
        return true;
    }
    if (zlink_spot_node_connect_peer (node, endpoint.c_str ()) != ZLINK_CONNECT_OK)
        return false;
    if (connected_flag)
        connected_flag->store (true, std::memory_order_release);
    return true;
...
inline void
disconnect_peer (void *node, const std::string &endpoint, std::atomic<bool> *connected_flag)
{
    if (!node || endpoint.empty ())
        return;
    (void) zlink_spot_node_disconnect_peer (node, endpoint.c_str ());
...
inline bool wait_for_control_link_ready (StateT *state, int timeout_ms, const ReceiveFn &receive_fn)
{
    if (!state)
        return false;
...
template <typename StateT, typename ReceiveFn>
inline bool
wait_for_started_size (StateT *state, size_t msg_size, int timeout_ms, const ReceiveFn &receive_fn)
{
    if (!state || msg_size == 0)
        return false;
...
inline bool wait_for_phase (StateT *state, size_t msg_size, int expected_phase, int timeout_ms)
{
    if (!state || msg_size == 0)
        return false;
      ...
      [state, msg_size, expected_phase] () {
          return state->fatal.load (std::memory_order_acquire)
                 || (state->seen_msg_size.load (std::memory_order_acquire) == msg_size
                     && state->seen_phase.load (std::memory_order_acquire) == expected_phase);
      });
}

template <typename StateT> inline bool wait_for_phase_duration (StateT *state, double seconds)
{
    if (!state)
        return false;
    if (seconds <= 0.0)
...
inline void start_client_stdin_watcher (StateT *state, NotifyStartFn notify_start)
{
    ...
    std::thread stdin_watcher ([state, notify_start] () {
        ...
        while (std::getline (std::cin, line)) {
            if (bench_transition_debug_enabled ()) {
                std::cerr << "[multi-spot-client] stdin line ts_ns=" << perf_multi_metric::now_ns ()
                          << " line=" << line << std::endl;
            }
            ...
            if (perf_multi_spot_handshake::parse_start_command (line.data (), line.size (),
                                                                &start_size)) {
...
template <typename StateT, typename NotifyStartFn>
inline void
start_server_stdin_watcher (StateT *state, int connect_ready_timeout_ms, NotifyStartFn notify_start)
{
    if (!state)
        return;

    std::thread stdin_watcher ([state, connect_ready_timeout_ms, notify_start] () {
        std::string line;
        while (std::getline (std::cin, line)) {
            if (bench_transition_debug_enabled ()) {
                std::cerr << "[multi-spot-server] stdin line ts_ns=" << perf_multi_metric::now_ns ()
                          << " line=" << line << std::endl;
            }

            size_t start_size = 0;
            std::string connect_endpoint;
            if (perf_multi_handshake::parse_endpoint_command_line (line, "CONNECT_CONTROL,",
                                                                   &connect_endpoint)) {
                if (bench_transition_debug_enabled ()) {
                    std::cerr << "[multi-spot-server] reverse connect request ts_ns="
                              << perf_multi_metric::now_ns () << " endpoint=" << connect_endpoint
                              << std::endl;
                }
                if (!accept_reverse_connect_request (state->control_node, &state->control_peers,
                                                     connect_endpoint, connect_ready_timeout_ms,
                                                     &state->fatal_errno)) {
                    if (bench_debug_enabled ()) {
                        std::cerr << "[multi-spot-server] reverse connect failed"
                                  << " endpoint=" << connect_endpoint << " err=" << zlink_errno ()
                                  << std::endl;
                    }
                    const int err = zlink_errno () != 0 ? zlink_errno () : errno;
                ...
                std::cout << "CONTROL_CONNECTED," << connect_endpoint << std::endl;
                continue;
            }
                                    ...
                                    NotifyStartFn notify_start)
{
    ...
    start_server_stdin_watcher (state, connect_ready_timeout_ms, notify_start);
