...
int main(void)
{
    ...
    int linger = 0;
    CHECK(zlink_set_option(pair, ZLINK_OPT_LINGER, &linger, sizeof linger)
          == ZLINK_CONFIG_OK);

    int out_linger = -1;
    size_t out_size = sizeof out_linger;
    CHECK(zlink_get_option(pair, ZLINK_OPT_LINGER, &out_linger, &out_size)
          == ZLINK_CONFIG_OK);
    CHECK(out_size == sizeof out_linger);
    CHECK(out_linger == 0);

    CHECK(zlink_set_option(pair, ZLINK_OPT_LINGER, &linger, 1)
