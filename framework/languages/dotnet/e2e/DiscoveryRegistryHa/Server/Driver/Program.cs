...
app.MapPost("/run/{scenario}", async (string scenario) =>
{
    await RunScenarioAsync(options with { Scenario = scenario });
    return Results.Ok(new { status = "passed", scenario });
});
...
static async Task RunScenarioAsync(ClientOptions options)
{
...
static DynamicProcess StartRegistry(ClientOptions options, string name)
{
    ...
    return StartProcess(options, name, "reg-2", options.Reg2Url, arguments, options.RegistryProject);
}
    ...
    IReadOnlyList<string> peerPubEndpoints)
{
    ...
    return StartProcess(options, name, rid, httpUrl, arguments, options.EmbeddedProject);
}
