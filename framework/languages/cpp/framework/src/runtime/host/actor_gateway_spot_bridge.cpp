                                  ...
                                  const zlink::message_t &payload)
{
    ...
    if (!route || route->node_rid.empty () || route->node_rid.value () == local_spot_node_rid) {
        return runtime.join_remote_actor_to_spot_erased (
          actor_ref, std::move (spot_rid), payload, actor_gateway.actor_context (actor_ref));
    }
    ...
    runtime.record_actor_route (joined.actor,
                                        ...
                                        const zlink::message_t &payload)
{
    ...
    if (!reply) {
        return result_t<actor_join_reply_t>::failure (
          reply.error_kind (),
          reply.error () ? reply.error ()->what () : "remote entry SPOT join failed");
    }
    return result_t<actor_join_reply_t>::success (actor_join_reply_from_spot_route (reply.value ()));
}
      ...
      [&] (const spot_route_t &route,
        ...
        if (reply.value ().actor_ref_present) {
            auto updated_actor_ref = actor_ref_t (
              node_rid_t::from_string (reply.value ().actor_node_rid), reply.value ().actor_type,
              reply.value ().actor_id, reply.value ().actor_generation);
            (void) actor_gateway.update_actor_ref (updated_actor_ref);
            runtime.record_actor_route (updated_actor_ref,
                                        spot_route_t{route.node_rid, spot_rid, route.spot_name});
        }
  ...
  const std::vector<spot_node_snapshot_t> &spot_node_snapshot)
{
    ...
    for (const auto &spot_node : spot_node_snapshot) {
        ...
        actor_gateway_spot_nodes.push_back (
          actor_gateway_spot_node_binding_t{*runtime, zlink.route_client (serializers),
    ...
    auto framework_provider = services.build_provider ();
    auto &actor_gateway = framework_provider.get_required<actor_gateway_runtime_t> ();
    actor_gateway.on_join_spot ([bindings = actor_gateway_spot_nodes,
                                 actor_gateway] (const actor_ref_t &actor_ref, spot_rid_t spot_rid,
    ...
    actor_gateway.on_join_entry_spot ([bindings = actor_gateway_spot_nodes] (
        ...
        for (auto &binding : bindings) {
            if (!node_rid.empty () && node_rid.value () != binding.local_spot_node_rid) {
                auto joined = join_actor_to_entry_spot_through_route (
                  ...
                  binding.route_channel_name, actor_ref, node_rid, payload);
    ...
    for (auto &node_binding : actor_gateway_spot_nodes) {
        node_binding.runtime.on_actor_packet_relay (
          [bindings = actor_gateway_spot_nodes,
           actor_gateway] (const actor_ref_t &actor_ref, actor_context_t actor_context,
              ...
              stream_header_t header (stream_message_kind_t::request, stream_codec_t::message_pack,
