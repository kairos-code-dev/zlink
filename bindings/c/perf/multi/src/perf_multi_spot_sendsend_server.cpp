...
bool echo_routed_payload (void *spot,
                          ...
                          size_t part_count)
{
    if (!spot || !state || !source_rid || !source_spot_rid || !parts || part_count == 0) {
        errno = EINVAL;
        return false;
    }

    zlink_submit_result_t submit_rc = perf_zlink_spot_send_spot_parts (
    ...
    if (submit_rc == ZLINK_SUBMIT_BACKPRESSURED || is_peer_disconnect_submit (submit_rc)
    ...
    if (bench_debug_enabled ()) {
        std::cerr << "[multi-spot-sendsend-server] echo send failed rc=" << submit_rc
                  << " err=" << submit_errno << std::endl;
    }
    zlink_multipart_close (parts, part_count);
...
void drain_spot_routed_recv (void *spot, spot_reqrep_server_state_t *state)
{
    ...
    for (;;) {
        ...
        if (rc != ZLINK_RECV_OK) {
            ...
            if (bench_debug_enabled ()) {
                std::cerr << "[multi-spot-sendsend-server] recv failed err=" << saved_errno
                          << std::endl;
            }
        ...
        if (bench_debug_enabled ()
            ...
            std::cerr << "[multi-spot-sendsend-server] recv payload seq=" << request_seq
                      << " parts=" << part_count << std::endl;
        }

        if (!echo_routed_payload (spot, state, source_rid, source_spot_rid, parts, part_count)) {
            return;
        }
    }
}
                      ...
                      const std::vector<size_t> &msg_sizes)
{
    ...
    for (size_t i = 0; i < msg_sizes.size (); ++i) {
        ...
        if (!idle_until_runner_stop (state)) {
            if (bench_debug_enabled ())
                std::cerr << "[multi-spot-reqrep-server] stop wait failed size=" << msg_size
                          << " err=" << zlink_errno () << std::endl;
            return false;
        }
    }
...
int run_server_benchmark (const std::string &lib_name, const std::string &transport)
{
    ...
    if (!initialize_reqrep_server_session (
          ctx, transport, lib_name + std::string ("_spot_sendsend_server"), settings, &session)) {
        return 1;
    }
