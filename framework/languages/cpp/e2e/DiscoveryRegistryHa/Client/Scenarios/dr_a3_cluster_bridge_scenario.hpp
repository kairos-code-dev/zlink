...
inline void run_dr_a3_cluster_bridge_scenario ()
{
    ...
    const std::vector<dr_a3_registry_case_t> cases{
      {"reg-1", reg1_url, reg1_consumer_url},
      {"reg-2", reg2_url, reg2_consumer_url},
      {"reg-3", reg3_url, reg3_consumer_url},
    };

    for (const auto &registry_case : cases) {
        post_json<registry_peer_count_wait_req_t, operation_status_t> (
          registry_case.registry_url, "/registry/status/peers/wait", {.connected_peer_count = 2});
        ...
        const auto marker =
          "dr-a3-" + registry_case.name + "-"
          + std::to_string (std::chrono::steady_clock::now ().time_since_epoch ().count ());
        const auto reply = post_json<profile_req_t, profile_res_t> (
          registry_case.consumer_url, "/profile/request", {.value = "dr-a3", .marker = marker});
        ensure (reply.value == "profile:dr-a3", "DR-A3 " + registry_case.name + " request failed");
        ensure (reply.provider_rid == "api-a" || reply.provider_rid == "api-b",
                "DR-A3 " + registry_case.name + " routed to an unexpected provider");
        ensure (reply.marker == marker, "DR-A3 " + registry_case.name + " marker mismatch");
        ...
        const auto evidence = post_json<evidence_wait_req_t, evidence_snapshot_t> (
          evidence_url, "/evidence/wait", {.contains = marker});
                ...
                "DR-A3 " + registry_case.name + " provider evidence was not recorded");
    }

    std::cout << "scenario DR-A3 passed\n";
}
