...
class assign_agent_handler_t
{
  public:
    ...
    using dependency_types = dependency_list_t<agent_assignment_service_t>;
    ...
    task_t<assign_agent_res_t> handle (const assign_agent_req_t &request)
    {
        ...
        const auto spot_rid = spot_rid_t::from_string (
          std::string (sample_names_t::support_spot_node) + ":" + request.conversation_id);
        const auto join_request = join_conversation_req_t{request.conversation_id};
        auto joined = co_await actor.context.join_spot (spot_rid, join_request).async<join_conversation_res_t> ();
        co_return assign_agent_res_t{request.conversation_id, joined.reply.state.status, candidate->actor_id};
    }
