...
| [session-actor-dispatch-implementation-plan.ko.md](./session-actor-dispatch-implementation-plan.ko.md) | `session-gateway-usability.ko.md` 초안을 새 session actor dispatch public API, `SessionProxy`, registry metadata sample, TicTacToe sample까지 사람 개입 없이 반영하는 집중 실행 계획 |
