...
| ID | 계약 | 구현이 하는 일 |
|----|------|----------------|
| **SMP-JV-03** (미구현) | [gamequest:280](../../common/sample/event/gamequest.ko.md): "노드를 재시작해도 owner spot이 **`QuestEventStore` replay로 aggregate를 rehydrate**한다". §14(`:603`)도 rehydrate를 "노드 재시작 → replay로 aggregate 복원"으로 못박는다 | SMP-JV-19에서 domain event delta fold를, SMP-JV-13에서 Redis startup replay를, SMP-JV-12에서 실제 process restart 뒤 channel 조회 gate를 구현했다. 그러나 player owner Spot과 owner lease/re-home 책임은 아직 없다. ⇒ event-sourced 저장·복원은 동작하지만 정식 owner lifecycle은 미구현이다. 심층은 SMP-JV-11 |
