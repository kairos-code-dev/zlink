...
public final class PerfMultiStreamLen32BeServer {
    ...
    public static int runServer(String transport, int msgSize) {
        if (!PerfMultiCommon.isCoreStreamServerTransport(transport)) {
            System.out.println("UNSUPPORTED,current,STREAM_LEN32BE," + transport);
            return 0;
        }
        ...
        try (Context context = new Context();
            ...
            server.attachStreamLen32be((routingId, messages) -> {
                    ...
                    for (Message message : messages) {
                        try {
                            ...
                            PendingStreamPacket request =
                                new PendingStreamPacket(routingId, message);
                            SendStatus sendStatus = trySendPendingMessage(server,
                                request);
