...
이 문서는 Config 5 Resilience/Lifecycle 공통 시나리오 중 Node framework E2E 상태를 정리한다. 현재
추적된 Node ResilienceLifecycle E2E runner/source가 없으므로 Node 전용 stdout marker로 구현 완료를
주장하지 않는다.
...
- `RL-A1`: provider restart Node runner와 marker가 아직 없다.
...
- `RL-C1`: resource cleanup Node harness가 아직 없다.
