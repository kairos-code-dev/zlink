...
이 문서를 goal로 두고 진행할 때는 아래 순서를 빠뜨리지 않는다. 작업이 길어져 중단되거나 재개되더라도, 마지막으로 완료한 문서와 Claude 리뷰 판정이 기록되어 있어야 한다.

## Goal 진행 규칙

1. 한 번에 하나의 리포트만 진행한다.
2. 다음 리포트로 넘어가기 전에 현재 리포트의 코드 수정, 테스트, 자체 리뷰, Claude 리뷰, 문서 갱신을 모두 끝낸다.
3. Claude가 "추가 이슈 없음"이라고 명확히 판정하지 않은 리포트는 완료로 표시하지 않는다.
...
상태 값은 `대기`, `진행 중`, `수정 완료`, `테스트 완료`, `Claude 리뷰 통과`, `문서 갱신 완료`, `종결` 중 하나로 기록한다.
...
| 4 | [2026-06-13-core-src-security-review.ko.md](2026-06-13-core-src-security-review.ko.md) | 진행 중 | 2026-06-14: #1 mtrie, #3 포트 파싱, #6 message/send 가드 수정·core/C++ binding 검증 통과·Claude 추가 이슈 없음, C binding은 기존 C-BINDING-001 버전 테스트 불일치로 실패 |
...
3. Claude에 해당 리포트와 수정된 코드를 함께 주고, 문서만 보지 말고 실제 코드를 직접 대조해 리뷰하도록 요청한다.
4. Claude 리뷰에서 이슈가 남으면 코드를 다시 수정하고, 테스트를 다시 실행한 뒤 Claude 리뷰를 반복한다.
5. Claude가 해당 순서에 대해 추가 이슈가 없다고 판정하면 기능·성능 영향이 리포트의 예상과 다른지 확인한다.
...
| Claude 리뷰 | Claude가 실제 코드와 리포트를 대조했고 "추가 이슈 없음" 판정을 냈다. |
| 문서 갱신 | 해당 리포트, README, 이 실행 순서 문서의 상태를 갱신했다. |
| 재개 정보 | 마지막 테스트 명령, Claude 리뷰 요약, 남은 이슈가 문서에 남아 있다. |

체크리스트 중 하나라도 빠지면 해당 리포트는 `종결` 상태가 아니다.

## Claude 리뷰 게이트

각 실행 순서는 Claude 리뷰를 통과해야 다음 순서로 넘어갈 수 있다.

Claude에는 다음 내용을 반드시 전달한다.
...
1. Claude가 이슈를 지적하면 해당 순서는 완료된 것으로 보지 않는다.
2. 지적된 이슈를 수정하고 관련 테스트를 다시 실행한다.
3. 같은 리포트와 수정 파일을 대상으로 Claude 리뷰를 다시 요청한다.
4. Claude가 "추가 이슈 없음"이라고 명확히 판정할 때까지 반복한다.
