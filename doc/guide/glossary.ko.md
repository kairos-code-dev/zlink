...
챕터가 소유하며, 여기서는 한 줄 정의와 링크만 둔다.

## 메시징 기본

- **Context** — 프로세스의 런타임 진입점. I/O 스레드를 소유하고 모든 소켓·서비스를
  생성한다. 보통 프로세스당 하나. ([02 Core API](./02-core-api.ko.md))
...
- **Transport** — 바이트가 흐르는 하부 계층. `tcp` / `ipc` / `inproc` / `ws` /
  `wss` / `tls`. ([04 트랜스포트](./04-transports.ko.md))
- **Message** — 하나의 페이로드 프레임. 송신 시 소비된다(zero-copy 이동).
  ([09 메시지 API](./09-message-api.ko.md))
- **Multipart** — 여러 프레임으로 이뤄진 한 메시지. **전체가 함께 전달되거나
  전체가 실패**한다(원자성). ([09 메시지 API](./09-message-api.ko.md))
- **Routing ID** — 피어·스팟·액터를 식별하는 1~255바이트 바이너리 안전 값.
  ROUTER·STREAM이 이를 전제한다. ([08 Routing ID](./08-routing-id.ko.md))
...
- **Backpressure** — 수신 측 처리 속도를 넘겨 밀어 넣지 못하게 흐름을 조절하는
...
- **SPOT** — 동적으로 생성·소멸되는 논리 단위(room·stage·zone 등)로 메시지를
  라우팅하는 추상. 한 단위로 들어온 메시지를 **단일 실행 큐로 직렬 처리**한다.
...
- **Topology** — 어떤 노드가 어디에 있고 서로 어떻게 연결돼 있는지의 구성 정보.
  ([07-4 Registry](./07-4-registry.ko.md))
...
- **ZMP** — zlink의 wire protocol. 8바이트 고정 헤더(MAGIC `0x5A`, VERSION `0x01`)
  위에 프레임을 싣는다. ZMTP(ZeroMQ 프로토콜)와 호환되지 않는 별개 프로토콜.
  ([ZMP 프로토콜 레퍼런스](./zmp-protocol.ko.md))
- **VSM (Very Small Message)** — 33바이트 이하 메시지를 힙 할당 없이 메시지 객체
  안에 inline 저장하는 **메모리 최적화**. wire 형식에는 영향이 없다.
  ([설계 근거](./design-rationale.ko.md))
- **YPipe** — 락 없이 CAS 연산으로 구현한 스레드 간 FIFO 큐.
  ([설계 근거](./design-rationale.ko.md))
- **Proactor** — I/O 완료 이벤트를 핸들러로 전달하는 비동기 설계(Boost.Asio 기반).
  ([설계 근거](./design-rationale.ko.md))
- **I/O 스레드** — Context가 소유하는 백그라운드 스레드. 콜백이 여기서 실행된다.
