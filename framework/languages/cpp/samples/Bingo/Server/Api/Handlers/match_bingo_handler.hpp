...
class match_bingo_api_handler_t
{
  public:
    ...
    explicit match_bingo_api_handler_t (zlink::framework::channel_client_t &client,
    ...
    task_t<match_bingo_api_res_t> handle (const match_bingo_api_req_t &request)
    {
        auto allocated =
          co_await _client
            .request<allocate_bingo_room_res_t> (sample_names_t::play_channel, allocate_bingo_room_req_t{request.mode})
            .submit ();
