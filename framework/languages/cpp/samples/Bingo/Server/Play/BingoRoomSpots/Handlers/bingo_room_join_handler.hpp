...
class bingo_room_join_handler_t
{
  public:
    ...
    explicit bingo_room_join_handler_t (bingo_room_directory_t &rooms, zlink::framework::logger_t<> logger = {}) :
        _rooms (rooms), _logger (std::move (logger))
    {
    }

    bingo_room_join_res_t handle (bingo_room_t &, const player_actor_t &actor, const bingo_room_join_req_t &request)
    {
        ...
        _logger.info ("join bingo room", {{"room_id", request.room_id}, {"actor_id", actor_id}});
