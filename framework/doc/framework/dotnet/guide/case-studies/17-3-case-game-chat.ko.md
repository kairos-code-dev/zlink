...
```csharp
var guild = await guilds.LoadAsync(guildId, ct);
guild.RequireMember(playerId);
chatRateLimit.RequireAllowed(playerId, guildId);

var decision = await moderation.CheckAsync(playerId, text, ct);
if (!decision.Allowed)
    return new SendRejected(decision.Reason);

var message = await chatHistory.AppendGuildAsync(guildId, playerId, text, ct);
foreach (var member in guild.OnlineMemberIds)
{
    var session = await connectionRegistry.FindAsync(member, ct);
    if (session is not null)
        await gatewayDeliver.SendAsync(session.NodeId, session.SessionId, message, ct);
}
```
    ...
    : IZLinkSession
{
        ...
        CancellationToken ct)
    {
        if (dispatch.PacketName == "auth")
        {
            ...
            context.Client.Reply(new AuthPlayerOk()).Submit();
            return;
        }
...
```csharp
public sealed class SendGuildChatHandler(
    IChatHistoryStore history,
    IChatModeration moderation)
    : IZLinkSpotActorSendHandler<GuildChatSpot, PlayerActor, SendGuildChat>
{
