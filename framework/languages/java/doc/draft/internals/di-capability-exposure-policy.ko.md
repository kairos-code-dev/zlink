...
# Draft -- Java Spring DI Capability Exposure Policy

> 이 문서는 **구현 전 초안**이다.
> Spring bean으로 노출되는 public service가 어떤 capability 구성과 묶이는지 정리한다.
...
`ZLinkBoundSessionFactory`는 `.NET`의 `AddPublicClients`가 `IZLinkBoundSessionFactory`
를 framework runtime과 함께 무조건 등록하는 것과 같다(조건 없음). bound session은
actor runtime state에 저장된 현재 session rid와 binding token을 쓰며, binding이 없는
actor에서 호출하면 `ActorSessionNotBound`로 실패한다.
...
JUnit 테스트 이름은 `.NET` 테스트(`NodesAndServices`, `HandlerExposure`)를 camelCase로
옮긴 대응이다. 확인 기준(의미)은 그대로 유지하며 `.NET` 코드가 최종 기준이다.

| JUnit 테스트 (`NodesAndServicesTest` / `HandlerExposureTest`) | 확인 기준 |
|--------------------------------------------------------------|-----------|
