...
- [ ] **E2E-JV-05** (결함) — RuntimeMonitoring·PubSub·RegistryMessaging은 해결. 나머지는 readiness가 최대 **20배**(60초)이고 route settle이 **8개 runner에 없다**
...
| **E2E-JV-05** (결함) | [E2E README §2.1:157-171](../../common/e2e/README.ko.md): local readiness timeout **3초**, route settle **5초**, scenario settle **3초**. *"이 값 안에 준비되지 않는 로컬 e2e는 대기 시간을 늘려서 통과시키지 않는다 … 긴 대기는 버그를 늦게 발견하게 만들기 때문에 완료 조건으로 인정하지 않는다"* | **부분 해결:** `RuntimeMonitoring`·`PubSub`·`RegistryMessaging`은 readiness를 3초로 줄이고, 역할 readiness 뒤 client 또는 다음 topology 단계 전에 이름 있는 5초 route settle을 적용했다. 각 runner에서 기존 상한과 route settle 부재를 잡는 gate가 먼저 실패했다. MON-A1~D1, PS-A1~B2·C1, RM-A1~A6·B1~B2·C1~C9 전체가 새 기본값으로 통과했다. RegistryMessaging의 첫 병렬 실행은 포트 예약 경합에 따른 `ZlinkBindException`이었고 단독 전체 재실행은 통과했다. 나머지 runner는 여전히 재검증 당시 상태다. `AutomaticTurnDispatch`는 15초이고 일부 HTTP wait는 60초다. `SpotService`는 override 가능한 30초, `SpotActorTransfer`·`ToActorMessaging`·`ObservabilityOps`는 인라인 장기 wait를 사용한다. 나머지 8개 config에도 route settle 상수가 없으므로 이 gap은 open이다. |
