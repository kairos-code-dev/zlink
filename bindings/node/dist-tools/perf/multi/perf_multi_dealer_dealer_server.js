...
const { createPayload, latencyUsFromPayload, payloadPhase, summarizeMetrics } = require('../common/perf_metrics');
...
async function main() {
    ...
    try {
        ...
        if (stopCount > 0) {
            const resultLines = summarizeMetrics('MULTI_DEALER_DEALER', 'tcp', options.msgSize, latenciesUs, options.duration);
