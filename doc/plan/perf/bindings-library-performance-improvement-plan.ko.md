...
| `ws` | `MULTI_SPOT` | `보류(50.6%)` | `보류(50.4%)` | `통과(82.5%)` | `보류(59.3%)` | `보류(42.7%)` | `보류(40.7%)` | 64/256/65536/131072/262144B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_022730.txt` 대비 Rust `perf_rust_multi_linux_20260525_023224.txt` 기준이다. 1024B는 Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Rust `perf_rust_multi_linux_20260525_003928.txt` median 기준이다. SPOT client 복사 제거 + 4-worker drain 위에, 131072B/262144B에서만 public `Poller` `POLLIN` wait를 사용한다. 최신 같은 조건 재측정에서는 1024B만 통과권이고, 64/256/65536/131072/262144B는 아직 SPOT 기준 아래다. |
...
- **남은 보류(Rust)**: SPOT tcp 64B 및 tcp/ws large(copy 영역), SPOT_REQREP tcp 262144B, PUBSUB small/65536B(단일 thread drain).
