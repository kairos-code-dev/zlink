...
ZLINK_JAVA_EXPORT int zlink_java_router_recv (void *router,
...
{
    if (!source_node_rid_out || !source_spot_rid_out || !request_seq_out || !parts_out
        || !part_count_out) {
        errno = EFAULT;
    ...
    *part_count_out = 0;
    ...
    while (true) {
        ...
        zlink_recv_result_t rc =
          zlink_router_recv_part (router, source_node_rid_out, source_spot_rid_out, request_seq_out,
