...
*   Ensure contents of this directory are present at `core/builds/zos`
    within that extracted directory (eg, `zlink-VERSION/core/builds/zos/`; 
...
*   (Optional, but recommended) build and run the core tests with:
        ...
        cd zlink-VERSION
        core/builds/zos/maketests
        core/builds/zos/runtests

*   To remove built files, to start again (eg, rebuild with different
    compile/link flags):

        cd zlink-VERSION
        core/builds/zos/makeclean
...
as expected.  (`core/builds/zos/runtests` will automatically skip these
...
The master version of this is in `core/builds/zos/platform.hpp`.
...
*   set `CXX` to point that the full  path to the `core/builds/zos/zc++` wrapper, eg

        setenv CXX "/u/0mq/zlink-4.0.4/core/builds/zos/zc++"
...
*   On a ASCII/ISO-8859-1/UTF-8 system with `tar`, and `pax`, and
    optionally the GNU auto tools (eg, the same Linux system):
        ...
        cd libzlink-git
        ./core/autogen.sh             # Optional: to be able to run ./configure
        cd ..
