                            ...
                            const std::string& lib_name)
{
    ...
    zlink_setsockopt(server, ZLINK_ROUTING_ID, server_id.c_str(),
                     server_id.size());
    ...
    for (size_t s = 0; s < msg_sizes.size(); ++s) {
        ...
        std::vector<char> recv_buf(current_size + 256);
        char id[256];
        ...
        for (int i = 0; i < warmup_count; ++i) {
            ...
            if (zlink_send(clients[idx], buffer.data(), current_size, 0) < 0) {
                round_failed = true;
                break;
            }
        ...
        const double latency = measure_roundtrip_latency_us(lat_count, [&]() {
            if (zlink_send(clients[0], server_id.c_str(), server_id.size(),
                ...
                < 0)
                return;
            if (zlink_send(clients[0], buffer.data(), current_size, 0) < 0)
                return;
            ...
            zlink_send(server, buffer.data(), current_size, 0);
        ...
        const int received = run_multi_timed_benchmark(
          ...
          [&](size_t idx) {
              return zlink_send(clients[idx], server_id.c_str(), server_id.size(),
                                ZLINK_SNDMORE)
                         >= 0
                       && zlink_send(clients[idx], buffer.data(), current_size, 0)
                            >= 0;
          },
          [&]() {
              ...
              if (id_len <= 0)
                  return false;
              if (zlink_recv(
                    server, recv_buf.data(), recv_buf.size(), ZLINK_DONTWAIT)
                  < 0) {
