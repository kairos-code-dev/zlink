...
int recv_parts(void *sock,
               ...
               int32_t flags)
{
    const zlink_routing_id_t *routing_id_view = NULL;
    if (routing_id)
        memset(routing_id, 0, sizeof(*routing_id));
    *parts = NULL;
    *part_count = 0;
    const int rc = zlink_recv(
      sock, &routing_id_view, parts, part_count, static_cast<zlink_recv_flags_t>(flags));
    if (rc == 0 && routing_id && routing_id_view)
        *routing_id = *routing_id_view;
    return rc;
}
...
int set_socket_option(void *sock, int32_t opt, const void *data, size_t len)
{
    ...
    case k_legacy_opt_routing_id: {
        ...
        rid.size = len;
        rid.data = const_cast<uint8_t *>(static_cast<const uint8_t *>(data));
        return zlink_set_routing_id(sock, &rid);
    }
...
int get_socket_option(void *sock, int32_t opt, void *data, size_t *len)
{
    ...
    case k_legacy_opt_routing_id: {
        ...
        if (rc != 0)
            return rc;
        size_t copy_len = rid ? std::min(*len, static_cast<size_t>(rid->size)) : 0;
        if (copy_len > 0)
                                 ...
                                 void *data)
{
    ...
    if (!payload->routing_id.empty()) {
        ...
        rid.data = reinterpret_cast<uint8_t *>(payload->routing_id.data());
    }
    ...
    if (!payload->spot_routing_id.empty()) {
        ...
        spot_rid.data =
          reinterpret_cast<uint8_t *>(payload->spot_routing_id.data());
...
napi_value socket_send(napi_env env, napi_callback_info info)
{
    ...
    int rc = zlink_send(
      sock, &msg, 1, static_cast<zlink_send_flags_t>(flags));
    if (rc != 0)
...
napi_value socket_try_send_parts(napi_env env, napi_callback_info info)
{
    ...
    int rc = zlink_send(sock, parts.data(), parts.size(), ZLINK_SEND_FLAGS_DONTWAIT);
    if (rc == 0)
        rc = ZLINK_SUBMIT_OK;
    else
        rc = classify_try_send_errno();
...
napi_value socket_try_send_routing(napi_env env, napi_callback_info info)
{
    ...
    int rc = zlink_send_rid(
      sock, &routing_id, &msg, 1, ZLINK_SEND_FLAGS_DONTWAIT);
    if (rc == 0)
        rc = ZLINK_SUBMIT_OK;
    else
        rc = classify_try_send_errno();
    ...
    for (;;) {
        ...
        if (rc == 0) {
          napi_value out = create_subscribed_value(
            env, routing_id ? *routing_id : zlink_routing_id_t{0, NULL},
            topic.data(), topic_len, parts, part_count);
          close_recv_parts(parts, part_count);
          return out;
        }
    ...
    for (;;) {
        ...
        if (rc == 0)
            return create_subscription_event_value(
              env, routing_id ? *routing_id : zlink_routing_id_t{0, NULL},
              subscribed, topic.data(), topic_len);
        if (zlink_errno() != EMSGSIZE)
...
napi_value socket_stream_send(napi_env env, napi_callback_info info)
{
    ...
    memcpy(rid_storage, rid_data, rid_len);
...
napi_value handle_set_routing_id(napi_env env, napi_callback_info info)
{
    ...
    if (!parse_routing_id(env, argv[1], &routing_id))
        return NULL;
    int rc = zlink_set_routing_id(handle, &routing_id);
    if (rc != 0)
...
napi_value dealer_request(napi_env env, napi_callback_info info)
{
    ...
    int rc = zlink_dealer_request(
      ...
      state, static_cast<zlink_send_flags_t>(flags),
...
napi_value router_recv_message(napi_env env, napi_callback_info info)
{
    ...
    if (peer_rid)
        rid_copy = *peer_rid;
    zlink_routing_id_t spot_rid_copy;
    memset(&spot_rid_copy, 0, sizeof(spot_rid_copy));
    if (spot_rid)
        spot_rid_copy = *spot_rid;
    napi_value out = create_router_recv_message_value(
      env, rid_copy, spot_rid ? &spot_rid_copy : NULL, request_seq, parts, part_count);
    close_recv_parts(parts, part_count);
    return out;
}
