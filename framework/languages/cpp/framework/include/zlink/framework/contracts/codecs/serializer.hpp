...
{
public:
  explicit payload_view_t (const zlink::message_t &message) noexcept
    : _message (&message)
  {
  }
...
{
public:
  using serialize_any_fn_t =
    std::function<zlink::message_t (const void *)>;
  using deserialize_any_fn_t =
    std::function<void (const zlink::message_t &, void *)>;
                              ...
                              typename serializer_t<T>::deserialize_fn_t deserialize)
  {
    return add_erased (
      ...
      [deserialize = std::move (deserialize)](const zlink::message_t &message,
                                             void *out) {
  ...
  template<typename T>
  serializer_t<T> get () const
  {
    return serializer_t<T> (
      [this](const T &value) {
        return serialize (std::type_index (typeid (T)), &value);
      },
