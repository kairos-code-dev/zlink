...
| Transport | Pattern | 64 | 256 | 1024 | 65536 | 131072 | 262144 | 결과 파일 / 메모 |
|-----------|---------|----|-----|------|-------|--------|--------|------------------|
| `tcp` | `MULTI_DEALER_DEALER` | `보류(66.4%)` | `보류(68.2%)` | `보류(70.6%)` | `보류(38.6%)` | `보류(32.3%)` | `보류(26.2%)` | C `perf_c_multi_linux_20260522_143606_codex_c_multi_tcp_no_stream_for_rust_20260522.txt`, Rust `perf_rust_multi_linux_20260522_144824_codex_rust_multi_tcp_dealer_dealer_20260522.txt` 기준이다. clients=100, server/client IO thread 기본 4개 조건에서 complete였고, 실행 중 관측한 Rust server/client 프로세스는 각 `nlwp=7` 수준이었다. C 대비 전 size가 낮아 보류한다. |
...
| `ws` | `MULTI_DEALER_DEALER` | `보류(28.9%)` | `보류(47.8%)` | `보류(69.4%)` | `보류(41.8%)` | `보류(34.0%)` | `통과(84.7%)` | C `perf_c_multi_linux_20260522_150505_codex_c_multi_ws_no_stream_for_rust_20260522.txt`, Rust `perf_rust_multi_linux_20260522_150956_codex_rust_multi_ws_dealer_dealer_20260522.txt` 기준이다. 실행 중 client `nlwp=7` 수준으로 thread 폭증은 아니며, 262144B 외에는 기준보다 낮아 보류한다. |
...
| `wss` | `MULTI_DEALER_DEALER` | `보류(40.3%)` | `보류(80.9%)` | `통과(85.9%)` | `보류(83.0%)` | `보류(71.8%)` | `보류(79.9%)` | C `perf_c_multi_linux_20260522_152936_codex_c_multi_wss_no_stream_for_rust_20260522.txt`, Rust `perf_rust_multi_linux_20260522_153314_codex_rust_multi_wss_dealer_dealer_20260522.txt` 기준이다. 1024B 외에는 기준보다 낮아 보류한다. |
...
| `tls` | `MULTI_DEALER_DEALER` | `보류(35.9%)` | `보류(43.5%)` | `보류(78.1%)` | `보류(47.8%)` | `보류(55.8%)` | `보류(49.4%)` | C `perf_c_multi_linux_20260522_154822_codex_c_multi_tls_no_stream_for_rust_20260522.txt`, Rust `perf_rust_multi_linux_20260522_155213_codex_rust_multi_tls_dealer_dealer_20260522.txt` 기준이다. 실행 중 client `nlwp=7` 수준으로 thread 폭증은 아니며, 전 size가 기준보다 낮아 보류한다. |
...
- **2026-05-24 tcp full 재측정 요약**: DEALER_ROUTER 46~107%(전 size 통과), ROUTER_ROUTER small 통과/large 보류, SPOT_REQREP 62~95%(전 size 통과), STREAM 90~101% 통과, DD small 70~80% 통과/large 보류, SPOT small 통과권/large 보류(copy 영역), SPOT_SENDSEND·PUBSUB는 일부만. PUBSUB small이 fresh baseline 대비 ~3~5%인 것은 회귀가 아니라(이전 24%는 옛 baseline 기준) 단일 thread로 100 subscriber를 drain하는 per-call 비용 + C PUBSUB(2.6M msg/s)이 매우 높기 때문이다.
- **남은 보류(Rust)**: SPOT large(copy 영역, 12~19%), SPOT_SENDSEND 64B/large, PUBSUB small(단일 thread drain). ws/wss/tls는 같은 transport-agnostic 수정이 적용되므로 재측정으로 확인 중.
