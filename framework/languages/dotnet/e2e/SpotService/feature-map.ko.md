...
| SM-G5 | 구현·process 통과 | `sm-g5` actual-process에서 startup/runtime signed 범위, Actor·User Spot 80개씩의 `20:60` 배치, 기존 Actor owner 유지, weight `0` 신규 placement 제외와 typed capacity-first fallback을 확인했다. Snapshot 뒤 경쟁 reservation이 첫 target capacity를 소진하는 production test는 low-weight node 재선택, losing reservation 반환과 최종 factory exactly-once를 확인한다. Relocation test는 weight `0` target의 새 reservation 거부와 변경 전에 얻은 fence의 commit 유지를 확인한다. `WeightContractTests`는 64-bit 합계와 최소 selection cycle을 포함해 9/9다. 증거: `logs/20260729-162042-2859820/`. |
