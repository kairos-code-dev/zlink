...
public:
  ...
  virtual result_t<void> dispatch_send (
    const route_received_packet_t &received) const = 0;

  virtual result_t<zlink::message_t> dispatch_request (
    const route_received_packet_t &received,
    const runtime::messaging::envelope_header_t &header) const = 0;
};

class no_route_internal_packet_dispatcher_t final
  : public route_internal_packet_dispatcher_t
{
public:
  ...
  result_t<void> dispatch_send (
    const route_received_packet_t &received) const override;
  result_t<zlink::message_t> dispatch_request (
    const route_received_packet_t &received,
    const runtime::messaging::envelope_header_t &header) const override;
};

class composite_route_internal_packet_dispatcher_t final
  : public route_internal_packet_dispatcher_t
{
public:
...
private:
  const route_internal_packet_dispatcher_t *resolve_send (
    std::string_view packet_name) const;
  const route_internal_packet_dispatcher_t *resolve_request (
    std::string_view packet_name) const;
