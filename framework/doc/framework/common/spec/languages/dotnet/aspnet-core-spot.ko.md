...
이 계약은 이를 `IZLinkSpotOutbound`의 callback scheduler로 두지 않는다. 대신
...
topology 계약에서는 framework 기본 표면을 `targetRid + spotRid` direct
...
topology 계약에서 `SpotNode`는 channel 이름을 직접 소유하지 않는다.
...
[^per-spot-scope]: per-spot scope는 Spot 인스턴스마다 별도로 만들어지는 DI 컨테이너 scope로, 해당 Spot의 lifecycle 동안 등록된 서비스를 함께 유지한다.
