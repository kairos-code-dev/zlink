...
bound_session_send_call_t bound_session_t::disconnect ()
{
                                                        ...
                                                        const zlink::message_t &payload)
{
    ...
    if (!sink) {
        return bound_session_send_call_t (send_call_t (result_t<void>::failure (
          framework_error_kind_t::actor_session_not_bound, "actor bound session has no send sink")));
    }
    return bound_session_send_call_t (send_call_t (
      std::move (packet_name),
...
actor_yield_join_call_t actor_context_t::join_entry_spot_payload (node_rid_t spot_node_rid,
                                                              const zlink::message_t &request)
{
    return actor_yield_join_call_t (
      [context = *this, spot_node_rid = std::move (spot_node_rid), request] () mutable {
          ...
          auto joined = dispatcher (*context._actor_ref, spot_node_rid, effective_request);
          if (!joined) {
              const auto *error = joined.error ();
              return result_t<actor_join_result_t>::failure (
                joined.error_kind (),
...
relay_call_t session_actor_t::relay (const zlink::message_t &payload)
{
    ...
    detail::actor_gateway_state_t::relay_dispatcher_t dispatcher;
    {
        ...
        if (found != _state->actors_by_id.end () && found->second.disconnected) {
            return relay_call_t (result_t<void>::failure (framework_error_kind_t::disconnected,
...
relay_call_t session_actor_t::notify_disconnected ()
{
    ...
    if (dispatcher) {
        return relay_call_t (dispatcher (_ref));
    }
    return relay_call_t (result_t<void>::success ());
}
...
request_call_t<session_actor_t> session_actor_manager_t::bind_or_get (actor_ref_t actor_ref)
{
    ...
    found->second.disconnected = false;
    return request_call_t<session_actor_t> (
      result_t<session_actor_t>::success (session_actor_t (_state, found->second.ref)));
}
...
{
    detail::actor_gateway_runtime_t (_state).bind_session_stream (std::move (actor_id),
                                                                  std::move (stream), codec);
}
                                          ...
                                          bool replace_existing)
{
    detail::actor_gateway_runtime_t (_state).bind_session_route (
      ...
      std::move (target_node_rid), codec, replace_existing);
}
...
actor_context_t actor_gateway_runtime_t::actor_context (const actor_ref_t &actor_ref) const
{
    return actor_context_t (_state, actor_ref);
}
      ...
      [state = _state, actor_id, route_client = std::move (route_client),
       ...
       target_node_rid = std::move (target_node_rid)] (std::string packet_name,
          ...
          return task_t<void> (
            route_client
              .send_to_node (
                route_channel_name, target_node_rid,
