...
- [x] **TH-CP-01** (구현 완료) — C++ 언어별 connector 계약 문서에 `expect_none`·`wait_for_sequence`와 별도 `assertions` 유틸을 공통 §10.2 및 .NET 결정과 같은 의미로 고정했다(`227bef031`). 문서 부재 게이트를 먼저 실패시킨 뒤(`678ec4eed`) `test_cpp_framework_target_contract`에서 TH-CP-01이 사라지고 `test_cpp_stream_connector`가 통과했다. 구현과 집중 계약 테스트는 `8e355273d`에 있다. POSD/DDD 재검토에서는 수신 큐 지식을 connector builder 안에 유지하고 범용 단언을 별도 이름 공간에 두었으며, status·REST 지식을 추가하지 않았음을 확인했다.
