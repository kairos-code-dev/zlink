...
int main ()
{
    ...
    auto api_a = zlink::http_client::client_t::create (topology.api_a_http_url)
                   ...
                   .build ();
    auto api_b = zlink::http_client::client_t::create (topology.api_b_http_url)
                   .timeout (std::chrono::milliseconds (5000))
                   .build ();
    ...
    auto duplicate = api_b.post ("/orders/start").body (success_req).fetch<start_order_res_t> ();
    ...
    auto pending_ok = api_a.post ("/self-check/idempotency/pending")
                        .body (pending_mapping_req_t{
                          pending_req.idempotency_key, "order-pending-0001", "api-a"})
                        .fetch<ok_res_t> ();
    ...
    auto resumed = api_b.post ("/self-check/workflow/continue")
            ...
            "resumed reservation");
    ensure (resumed.state.payment_id == "payment-" + inventory_reserved.order_id,
            "resumed payment");
    ...
    auto inventory_failed = wait_for_status (api_a, inventory_started.order_id, order_status_t::failed);
    ensure (inventory_failed.reason.find ("inventory") != std::string::npos, "inventory failure");
    ...
    auto payment_failed = wait_for_status (api_b, payment_started.order_id, order_status_t::failed);
    ensure (!payment_failed.reservation_id.empty (), "payment failure reservation");
    ensure (payment_failed.reason.find ("payment") != std::string::npos, "payment failure");
    ...
    auto assertion = api_a.post ("/self-check/assert")
                       .body (server_assertion_req_t{success.order_id,
                                                     pending.order_id,
                                                     concurrent_a.order_id,
                                                     inventory_reserved.order_id,
                                                     inventory_started.order_id,
                                                     payment_started.order_id,
                                                     scale.order_id})
