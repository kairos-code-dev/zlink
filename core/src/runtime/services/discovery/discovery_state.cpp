...
int discovery_local_state_t::get_route_value_max_size (void *optval_, size_t *optvallen_) const
{
    if (!optvallen_) {
        errno = EINVAL;
        return -1;
...
int discovery_local_state_t::get_spot_owner_sync_enabled (void *optval_, size_t *optvallen_) const
{
    if (!optvallen_) {
        errno = EINVAL;
        return -1;
...
int discovery_local_state_t::get_actor_route_sync_enabled (void *optval_, size_t *optvallen_) const
{
    if (!optvallen_) {
        errno = EINVAL;
        return -1;
...
void discovery_local_state_t::snapshot_registration (int64_t *value_out_) const
{
    if (value_out_)
        *value_out_ = _value;
}
...
int discovery_t::set_option (int option_, const void *optval_, size_t optvallen_)
{
...
{
    if (option_ != ZLINK_OPT_ROUTE_VALUE_MAX_SIZE && option_ != ZLINK_OPT_DISCOVERY_SPOT_OWNER_SYNC
...
void discovery_t::snapshot_member_peers (std::vector<zlink_member_peer_entry_t> *out_) const
{
    if (!out_)
        return;
    out_->clear ();
        ...
        for (std::map<registered_service_key_t, registered_service_t>::const_iterator it =
            ...
            if (it->second.channel_name == _channel_name) {
                local_members.insert (
                  discovery_member_key_t (it->second.service_role, it->second.endpoint));
            }
        }
        _service_state.snapshot_member_peers (
          static_cast<zlink_auto_connect_type_t> (_auto_connect_type), local_members, out_);
    }
}

int discovery_t::member_peers (zlink_member_peer_entry_t *entries_, size_t *count_) const
{
    service_public_api_scope_t admission (_public_api);
    if (!admission.acquired ())
        return -1;
...
void discovery_t::upsert_service_summary (const zlink_registry_topology_entry_t &entry_)
{
    if (entry_.routing_id.size == 0 || entry_.channel_name[0] == '\0')
        return;

    const topology_key_t key = make_summary_key (entry_.service_kind, entry_.service_role,
                                                 entry_.routing_id, entry_.channel_name);

    {
        scoped_lock_t lock (_sync);
        store_summary_entry_locked (key, entry_, true, entry_.state == ZLINK_TOPOLOGY_STATE_STOPPED,
...
discovery_t::topology_key_t discovery_t::make_summary_key (uint16_t service_kind_,
...
void discovery_t::store_summary_entry_locked (const topology_key_t &key_,
...
void discovery_t::mark_spot_owner_summaries_dirty_locked (bool stopped_, uint64_t now_ms_)
{
...
int discovery_t::destroy ()
{
    ...
    _uplink_runtime->take_shutdown_state (this, &report_dealers, &control_dealers);
    ...
    for (size_t i = 0; i < bootstrap_dealers.size (); ++i) {
        ...
        if (!bootstrap_dealers[i].first.empty ()) {
            zlink_disconnect (bootstrap_dealers[i].second, bootstrap_dealers[i].first.c_str ());
        }
    ...
    for (size_t i = 0; i < report_dealers.size (); ++i) {
        ...
        if (!report_dealers[i].first.empty ())
            zlink_disconnect (report_dealers[i].second, report_dealers[i].first.c_str ());
    ...
    for (size_t i = 0; i < control_dealers.size (); ++i) {
        ...
        if (!control_dealers[i].first.empty ())
            zlink_disconnect (control_dealers[i].second, control_dealers[i].first.c_str ());
...
int discovery_t::ensure_sub_socket ()
{
    ...
    _bootstrap_runtime->apply_socket_options (static_cast<socket_base_t *> (sub));
    if (!_bootstrap_runtime->ensure_socket_routing_id (static_cast<socket_base_t *> (sub))) {
