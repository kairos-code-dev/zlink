...
의미는 [소켓 옵션](../../12-socket-options.ko.md)을 참고하세요.
...
[TLS 보안](../../05-tls-security.ko.md)을 참고하세요.
...
| `Zlink.CreateTimer(ISpot spot)` | 해당 SpotNode 공유 | SPOT 디스패치 루프 안에서 도는 타이머. 같은 노드의 타이머들이 스케줄러를 공유해 오버헤드가 준다(개념: [02-core-api §타이머](../../02-core-api.ko.md)) |
...
다음: [레퍼런스 — 에러 · 코덱 · C API 대응표 · API 레퍼런스 · 샘플 →](./05-reference.ko.md)
