[스펙 목차](../../../README.ko.md)

[C++ 묶음](./README.ko.md) | [STREAM 샘플](./stream-samples.ko.md) | [STREAM open items](./stream-open-items.ko.md)

# Draft -- ZLink Framework C++ STREAM

> 이 문서는 **구현 전 초안**이다.
> 현재 공개 계약이 아니며, `C++` runtime에서 `STREAM`을 어떤 표면으로 올릴지
> 정리한다.

## 1. 방향

`C++`에서도 `STREAM`은 packet handler와 raw handler 두 축으로 설명한다.
다만 다른 언어와 달리 host/runtime이 poll loop를 더 직접 소유한다는 점을 같이
적어야 한다.

## 2. Handler

```cpp
class stream_packet_handler_t {
public:
    virtual ~stream_packet_handler_t() = default;
    virtual void handle(
      const message_t &header,
      const message_t &body,
      const stream_context_t &context) = 0;
};

class stream_raw_handler_t {
public:
    virtual ~stream_raw_handler_t() = default;
    virtual void handle(
      const message_t &payload,
      const stream_context_t &context) = 0;
};
```

recv loop는 raw `stream_socket`을 직접 꺼내 쓰는 방식보다, host가 dispatch를 맡고
application은 handler를 등록하는 모델을 기본으로 본다.
