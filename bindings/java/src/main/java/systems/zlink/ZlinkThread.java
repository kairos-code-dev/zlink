...
public final class ZlinkThread implements AutoCloseable {
    ...
    private static final ConcurrentHashMap<Long, ZlinkThread> THREADS =
    ...
    public ZlinkThread(Runnable task) {
    ...
    private static MethodHandle callbackHandle() {
        try {
            return MethodHandles.lookup().findStatic(ZlinkThread.class,
    ...
    private static void handleEntry(MemorySegment arg) {
        ...
        ZlinkThread thread = THREADS.get(token);
        if (thread == null) {
            return;
        }
