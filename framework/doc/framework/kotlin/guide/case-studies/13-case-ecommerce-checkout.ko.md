...
@Service
public final class OrderGrpcService {
    private final PaymentGrpcClient payments;
    private final OrderRepository orders;

    public PlaceOrderReply placeOrder(PlaceOrderRequest req) {
        return orders.find(req.orderId()).map(Order::toReply).orElseGet(() -> createOrder(req));
    }

    private PlaceOrderReply createOrder(PlaceOrderRequest req) {
        ChargeReply charge = payments.charge(
            new ChargeRequest(req.orderId(), req.accountId(), req.amountMinor(), req.orderId()),
            Duration.ofSeconds(2));
        try (OrderTransaction tx = orders.beginTransaction()) {
            orders.insertOrder(req.orderId(), charge.receiptId(), tx);
            orders.insertOutbox("OrderPlaced", req.orderId(), tx);
            tx.commit();
        }
        return new PlaceOrderReply(req.orderId(), charge.receiptId());
    }
}
```
...
```java
outbox.drain(row -> kafkaProducer.send("order.events", row.toMessage()));

kafkaConsumer.consume("order.events", event -> {
    if (!inventory.tryReserve(event.orderId(), event.sku(), event.qty())) {
        kafkaProducer.send("order.events", new StockReserveFailed(event.orderId()));
    }
});
```
...
class CheckoutZLinkConfig {
    ...
    fun checkoutFramework(): ZLinkFrameworkConfigurer =
        ZLinkFrameworkConfigurer { options ->
            ...
            options.addHandlersFromPackageOf(CheckoutApplication::class.java)
        }
}
```

```kotlin
@ZLinkHandlerGroup("orders")
class PlaceOrderHandler(
    private val services: ZLinkClient,
    private val orders: OrderRepository,
) : ZLinkSuspendingRequestHandler<PlaceOrder, OrderPlaced> {
    override suspend fun handle(req: PlaceOrder, context: ZLinkRequestContext): OrderPlaced {
        ...
        return OrderPlaced(req.orderId, charge.receiptId)
    }
}
```
...
- 실행 코드: [Kotlin ShoppingMall 샘플](../../../../../languages/java/samples/kotlin/ShoppingMall)
