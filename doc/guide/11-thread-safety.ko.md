...
이 가이드의 나머지에서는 `send`와 `connect`를 함께 쓸 때 무슨 일이
일어나는지, 다른 스레드가 아직 전송 중인데 `close`를 부르면 어떻게 동작하는지,
콜백에서 무엇을 주의해야 하는지 설명합니다.
...
**요약:** `send`는 어느 스레드에서든 자유롭게 호출하세요. 연결 추가,
...
- `zlink_send()` — 원시 소켓
- `zlink_publish()` — SPOT
- 콜백 안에서 `send` / `publish`를 불러도 안전합니다

**순서:**
- 한 스레드 안에서는 `send`를 호출한 순서대로 메시지가 전달됩니다.
- 여러 스레드에서 동시에 보내면 각 메시지는 온전하게 전달되지만
  스레드 사이의 순서는 보장되지 않습니다 (먼저 도착한 스레드가 먼저 전송).
...
메시지를 보내는 동안 다른 스레드가 엔드포인트를 더 연결할 수 있습니다:
...
다른 스레드가 아직 쓰고 있는 핸들을 닫아도 크래시하지 않습니다 —
...
핸들은 스레드 안전하지만 메시지 객체는 그렇지 않습니다. 각 `zlink_msg_t`는
한 번에 한 스레드에서만 써야 합니다.

대개는 자연스럽게 지켜집니다 — 스레드마다 스택이나 힙에 메시지를 만들면
됩니다:
...
넘어갑니다. 반환하기 전에 각 part를 `zlink_msg_close()`하고, 다른 스레드에서
건드리면 안 됩니다.
...
**콜백에서 할 수 있는 것:**
- 같은 핸들로 `send` / `publish` 호출 — 권장하는 request-reply 패턴입니다.
...
| 실수 | 이유 | 해결 |
|---|---|---|
| 같은 `zlink_msg_t` 공유 | 메시지 객체는 스레드 안전하지 않음 | 스레드별 별도 메시지 생성 |
| 콜백 경로에서 무거운 작업 | 내부 워커 또는 I/O 경로 정체 | 큐로 오프로드 |
