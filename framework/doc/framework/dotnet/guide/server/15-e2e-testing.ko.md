...
[문서 목록](../../../../README.ko.md) | [이전: 샘플 고르기](14-samples.ko.md) | [다음: 설정 — 옵션 목록 · 기본값 · 변경 시점](16-options.ko.md)
...
> [Stream Connector 공개 계약](../../../common/spec/stream-connector/languages/dotnet/03-stream-connector.ko.md)이
...
[Stream Connector 가이드](../stream-connector/INDEX.ko.md)가 다룬다.

두 라이브러리의 전체 사용법은 각자의 가이드가 소유한다.

- [HTTP Client 가이드](../http-client/README.ko.md) — 요청 구성, 본문, 인증·TLS, 재시도,
  오류 처리까지 13장
- [Stream Connector 가이드](../stream-connector/INDEX.ko.md) — 실행 환경별 통합(Unity,
...
- **연결은 되지만 push가 도착하지 않는다** → 엔진 통합처럼 수동 펌프가 필요한 환경에서
  `Dispatch`를 실행하지 않은 경우다([Stream Connector 가이드](../stream-connector/INDEX.ko.md)).

## 8. 관련 문서

- 어떤 샘플을 먼저 볼지: [14-samples](14-samples.ko.md)
- 서버 쪽 STREAM 등록과 session: [09-stream](09-stream.ko.md)
- HTTP client 전체 사용법: [HTTP Client 가이드](../http-client/README.ko.md)
- 엔진 통합과 수동 펌프: [Stream Connector 가이드](../stream-connector/INDEX.ko.md)
- connector 정식 계약:
  [Stream Connector 공개 계약](../../../common/spec/stream-connector/languages/dotnet/03-stream-connector.ko.md)
