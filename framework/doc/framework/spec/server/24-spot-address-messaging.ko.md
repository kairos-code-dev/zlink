...
이 문서는 ZLink Framework 11.0.0에서 global Spot identity를 생성·조회하고 직접 호출하는 공개 계약을
...
있지만 Actor create·join·leave·transfer와 Logical Multicast subscription은 사용할 수 없다.

## 3. Create와 GetOrCreate

Spot manager는 User·Instance Spot의 명시적인 create operation을 제공한다. `Create`는 Framework가 global
RID를 생성하고 required stable type과 kind를 받는다. `GetOrCreate`는 caller가 지정한 global RID, stable type과
kind를 받는다. 두 operation은 target node나 endpoint를 받지 않는 single-use fluent call이다.
...
같은 caller RID의 kind 또는 stable type이 다르면 `SpotTypeMismatch`다. `GetOrCreate`는 같은 kind·type의 Ready 또는 Creating attempt에
...
## 4. Instance Spot creation intent와 reactivation

Instance Spot의 최초 create는 global SpotRid, stable type, kind와 최초 MeshName을 durable authority에 기록한다.
일반 send·request는 type이나 Mesh를 입력으로 받지 않으며 Missing RID의 최초 creation intent를 만들지 않는다.

이미 creation intent가 있는 Instance Spot에 Ready owner가 없으면 Framework는 저장된 type과 Mesh를 사용해
reactivation을 수행할 수 있다. Reactivation은 새 logical identity를 만들지 않으며 current authority와 exact
generation을 복구한다. 일반 message가 다른 type·Mesh를 제공하거나 target node를 선택할 수 없다. Concurrent
reactivation은 authority attempt 하나에 합류한다.

## 5. Existing-owner resolve와 direct call

Spot direct send/request는 global Spot RID와 typed payload만 받는다. Framework는 positive route cache 또는
...
One-way call은 local outbound admission까지만 기다린다. Reactivation이 필요해도 application handler 실행은
기다리지 않는다. Request는 original correlation을 terminal-once로 완료한다. Target queue admission 뒤의
failure를 current owner를 다시 찾아 hidden retry하지 않는다.

## 6. Route cache와 stale-route forwarding

`RouteCacheMaxAge`의 기본값은 15초이고 `TransferForwardingWindow`의 기본값은 30초다. 둘 다 0이면 각각
cache와 forwarding을 끈다. 두 값이 양수이면 cache max age가 forwarding window보다 최소 5초 작아야 한다.
Runtime 변경은 새 cache entry와 새 transfer에만 적용한다.

Transfer commit 뒤 source는 committed source→target mapping만 사용해 stale physical route를 relay한다. Relay
...
User Spot aggregate transfer는 Spot과 member Actor의 mapping을 같은 aggregate commit에서 설치한다. 개별
...
default는 제공하지 않는다. Snapshot은 stable state contract와 typed adapter를 요구한다.

Source seal, durable capture, target reservation·prepare, authority commit, restore와 admission 순서는
[23 Spot과 Actor membership](23-spot-actor.ko.md)이 정한다. Commit 전 failure는 source를 유지하고 commit 뒤에는
target recovery만 계속한다. Original send·request를 maintenance target에 자동 재제출하지 않는다.

## 9. 실패와 관측

- Object role Mesh 후보가 없거나 여러 개인 create는 §3의 typed error로 끝난다.
- Ready authority가 없으면 not-found, exact generation이나 owner fence가 다르면 typed stale error다.
- `Closing`, moving 또는 draining owner는 신규 admission을 거부한다.
...
reactivation·close·maintenance operation kind, forwarding hop·drop과 stale 분류를 구분한다. Spot RID는 metric
label로 사용하지 않는다.
...
- Spot direct가 Spot RID만 받고 type, MeshName과 owner route를 요구하지 않는다.
- Missing Instance Spot의 일반 message가 creation intent를 만들지 않는다.
- Reactivation이 durable intent의 stable type과 최초 Mesh를 사용한다.
...
- C++, .NET, JVM과 Node.js가 create 경쟁, logical messaging, reactivation, close와 stale forwarding에서 같은
  terminal 결과를 제공한다.
