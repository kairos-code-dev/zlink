...
timer := NewSpotTimer(spot)
timer.Start(1*time.Second, 0)  // 1초, 무한 반복

// dispatch event handler — 단일 context, 동기화 불필요
spot.OnDispatchEvent(func(s *Spot, event SpotDispatchEvent) {
    switch event {
    case SpotDispatchEventSubscribeReadable:
        sub := s.SubscribeRecv()
        // pub/sub 메시지 처리
    case SpotDispatchEventRoutedReadable:
        msg := s.SpotRecv()
        if msg.RequestSeq != 0 {
            s.ReplyToSpot(msg.SourceRid, msg.SpotRid,
                          msg.RequestSeq, NewMessage([]byte("ok")))
        }
    case SpotDispatchEventTimerReadable:
        fireCount := timer.Recv()
        _ = fireCount
        s.Publish("heartbeat", NewMessage([]byte("ping")))
    }
})
