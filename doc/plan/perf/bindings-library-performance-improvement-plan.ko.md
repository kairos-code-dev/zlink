...
| `ws` | `MULTI_DEALER_DEALER` | `보류(38.4%)` | `통과(55.2%)` | `통과(67.7%)` | `통과(59.2%)` | `통과(58.7%)` | `통과(64.4%)` | 64B는 fresh C `perf_c_multi_linux_20260525_122320.txt` 대비 current Go `perf_go_multi_linux_20260525_122359.txt` 기준이다. 256B/1024B는 Go `perf_go_multi_linux_20260524_222134.txt`, 65536/131072/262144B는 Go `perf_go_multi_linux_20260525_001654.txt` median 기준이다. C처럼 단일 poll loop와 pending socket만 `POLLOUT`으로 두는 poll set 의미를 유지했다. client active send는 65536B/131072B에서 public `Bytes(...)` 경로를 사용해 submit 전 native `Message` 생성을 줄였다. 262144B는 기존 경로로도 current HEAD에서 통과권이 유지된다. 64B는 최신 같은 조건 재측정에서도 Go 단순 one-way 최소 기준보다 낮다. |
...
- **2026-05-23 근본 원인 규명 + 수정 (strace 프로파일링)**: 위 large many-client 병목의 진짜 원인을 strace로 특정했다.
  ...
  - **2026-05-25 `ws MULTI_DEALER_DEALER 64B` `Bytes(...)` 후보 기각**: wss/tls 64B와
    ...
    경로를 유지한다.
  - **2026-05-25 `ws/wss/tls MULTI_DEALER_DEALER 64B` 최신 재측정 및 latency sampling 후보 기각**:
    ...
    보류를 해소하지 못했다. 따라서 DD 64B는 public API 변경 없는 server-side latency
    sampling이나 `Bytes(...)` client send로는 해결되지 않는 small FFI/cgo 경계 비용으로
    남기고 코드는 반영하지 않는다.
