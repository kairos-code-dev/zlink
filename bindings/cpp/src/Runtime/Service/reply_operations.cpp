...
void submit_raw_reply (detail::spot_operation_state_t &state_)
{
    ...
    const int rc = detail::submit_message_parts (
      state_.parts, [&] (zlink_msg_t *part_out_, zlink_part_flag_t part_flag_, bool) {
          if (state_.kind == detail::spot_operation_kind_t::raw_reply) {
              return zlink_router_reply_part (state_.raw_socket,
          ...
          return zlink_router_reply_spot_part (
            ...
            zlink::detail::routing_id_native (*state_.second_rid), state_.request_seq, part_out_,
            part_flag_);
      });
    if (rc != 0) {
        throw last_error ();
...
void reply_submit_operation_t::submit () &&
{
    ...
    if (state.kind == detail::spot_operation_kind_t::received_reply) {
        if (!state.received
            || !zlink::detail::received_access_t::has_reply_context (*state.received))
            throw submit_error_t (submit_result_t::invalid_argument, EINVAL);
