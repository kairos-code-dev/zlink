...
internal static class AtdE2CancellationScenario
{
    public static async Task RunAsync(IZlinkStreamConnector client)
    {
        ...
        var spot = await client.Request(new EnsureSpotReq(spotRid))
            ...
            .Async<EnsureSpotRes>();
        ScenarioAssert.That(spot.SpotRid == spotRid, "ATD-E2 spot creation mismatch.");

        var requestId = $"ATD-E2-{Guid.NewGuid():N}";
