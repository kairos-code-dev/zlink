...
inline bool is_stop_payload (const unsigned char *data_,
                             size_t size_,
                             const char *stop_token_)
{
    return data_ && stop_token_ && *stop_token_
           && size_ == std::strlen (stop_token_)
...
inline send_result_t try_send (queued_message_t &queued_, void *send_socket_)
{
    ...
    const int rc = zlink_send_part_rid (
      send_socket_, &queued_.routing_id, &queued_.msg, ZLINK_DONTWAIT,
      ZLINK_PART_FINAL);
    if (rc == 0)
...
inline bool enqueue (session_t *session_,
                     const zlink_routing_id_t *rid_,
                     zlink_msg_t *msg_part_)
{
                                ...
                                const zlink_msg_t *body_part_)
{
    ...
    const size_t header_size =
      zlink_msg_size (const_cast<zlink_msg_t *> (header_part_));
    const size_t body_size =
      zlink_msg_size (const_cast<zlink_msg_t *> (body_part_));
    const size_t total_size = 6 + header_size + body_size;
    if (zlink_msg_init_size (packet_out_, total_size) != 0)
        return false;

    unsigned char *dst =
      static_cast<unsigned char *> (zlink_msg_data (packet_out_));
    ...
    if (header_size > 0) {
        std::memcpy (
          dst + 6,
          zlink_msg_data (const_cast<zlink_msg_t *> (header_part_)),
          header_size);
    }
    if (body_size > 0) {
        std::memcpy (
          dst + 6 + header_size,
          zlink_msg_data (const_cast<zlink_msg_t *> (body_part_)),
          body_size);
    }
    return true;
}

inline send_result_t try_send_packet_now (void *stream_socket_,
                                          const zlink_routing_id_t *rid_,
                                          zlink_msg_t *packet_)
{
    ...
    const int rc = zlink_send_part_rid (
      stream_socket_, rid_, packet_, ZLINK_DONTWAIT, ZLINK_PART_FINAL);
    if (rc == 0)
        return send_result_sent;
    ...
    if (debug_enabled ()) {
        std::cerr << "[multi-stream-server] try_send_packet_now failed err="
                  << err << " rid_size="
                  << static_cast<unsigned> (rid_->size) << " packet_size="
                  << zlink_msg_size (packet_) << " rid_hex=";
        for (unsigned int i = 0; i < static_cast<unsigned int> (rid_->size);
             ++i) {
...
inline bool handle_packet_message (session_t *session_,
                                   ...
                                   const char *stop_token_)
{
    if (!session_ || !rid_ || !header_part_ || !body_part_
        || !session_->send_socket) {
        return false;
    }
    ...
    const send_result_t send_rc =
      try_send_packet_now (stream_socket_, rid_, &packet);
...
inline void drain_pending (session_t *session_)
{
    ...
    while (true) {
        ...
        if (rc == send_result_sent) {
            ...
            if (pending_before > 0) {
                session_->pending_count.fetch_sub (
...
inline void stream_packet_handler_callback (void *stream_,
                                            ...
                                            void *userdata_)
{
    packet_handler_context_t *ctx =
      static_cast<packet_handler_context_t *> (userdata_);
    if (!stream_ || !ctx || !ctx->session || !rid_ || !header_part_
        || !body_part_) {
        if (header_part_)
    ...
    if (!handle_packet_message (
          ctx->session, stream_, rid_, header_part_, body_part_,
          ctx->stop_token)) {
        stop_requested ().store (true, std::memory_order_release);
    }
                                  ...
                                  void *loop_tick_ctx_)
{
    ...
    while (!stop_requested ().load (std::memory_order_acquire) && rc == 0) {
        ...
        if (pending_size (session_) == 0) {
            ...
            if (idle_rc < 0 && zlink_errno () != EINTR
                && zlink_errno () != EAGAIN) {
                if (debug_enabled ()) {
