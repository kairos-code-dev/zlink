...
| `RuntimeMonitoring` | 구현 범위 통과, MON-A4 failover partial | `.NET` 기준 이름인 `RuntimeMonitoring`으로 전환했고 기존 C++ `Monitoring` PubSub 보조 wrapper는 삭제했다. C++ 전용 registry, service, filtered service, throwing service, trigger, client target과 `.NET` 기준 inventory가 있다. `timeout 420s framework/languages/cpp/e2e/RuntimeMonitoring/run_e2e.sh` 최신 통과 증거는 `logs/20260630-173613-574152`이며 MON-A1, MON-A2, MON-A3, MON-A4 drain/registry topology slice, MON-A5, MON-B1, MON-B2, MON-C1, MON-D1을 검증한다. MON-A4 provider 교체/failover socket transition은 아직 부분 구현으로 남긴다. |
| `YieldDispatch` | Track A/B/C/D/E1/E3/E4 진행 중 | C++ e2e config와 runner가 생겼고 YD-A1~YD-A4, YD-B1~YD-B3, YD-C1~YD-C3, YD-D1~YD-D4, YD-E1, YD-E3, YD-E4 static gate는 runner 통과 증거가 있다. 구현된 client scenario header는 YD-A/B/C/D2~D4/E1/E3까지 완료했고, Registry host factory/support, Delay host factory/handler/support, Play host factory/support/control/basic/timer/actor/remote/failure handler, spot type, `YieldProbeSpot` runtime, Session host factory/support, `yield_session_t` support header도 분리했다. A3 metadata 보존, C3 같은 stream session 증거, E2 cancellation token, E5 report aggregation은 gap/partial이므로 config 완료 판정은 보류한다. |
