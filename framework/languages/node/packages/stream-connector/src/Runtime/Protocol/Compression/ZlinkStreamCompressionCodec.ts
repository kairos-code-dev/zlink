...
function pickleUncompressed(payload: Uint8Array): Uint8Array {
  ...
  // wire contract without adding a native compression dependency.
...
function readLittleEndian(source: Uint8Array, offset: number, size: number): number {
  ...
  if (size === 4) {
    return readUInt32LE(source, offset);
  }
...
function decodeLz4Block(source: Uint8Array, resultLength: number): Uint8Array {
  ...
  while (sourceOffset < source.length) {
    ...
    const literalLength = readLength(source, token >>> 4, () => sourceOffset++);
    ...
    const matchLength = readLength(source, token & 0x0f, () => sourceOffset++) + 4;
...
function readLength(source: Uint8Array, nibble: number, nextOffset: () => number): number {
  let length = nibble;
  if (length !== 15) {
    return length;
  }
