...
const { attachProcessCapture, reservePort, spawnMultiPair, stopServer, waitForLine } = require('./perf_multi_common');
    ...
    for (const patternName of patternNames) {
        ...
        for (const msgSize of options.msgSizes) {
            ...
            if (patternName === 'MULTI_STREAM') {
                const lines = await spawnSharedStreamPair({ ...options, pattern: patternName, msgSize, clients: options.clients });
                patternRows.push({
                    pattern: patternName,
                    msgSize,
                    metrics: primaryMetricsFromResultLines(patternName, msgSize, lines)
