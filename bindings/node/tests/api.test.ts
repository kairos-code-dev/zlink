...
test('context options, shutdown, and tls facades follow the aligned surface', () => {
  ...
  assert.equal((message as { value?: unknown }).value, undefined);
  assert.equal((message as { packetName?: unknown }).packetName, undefined);
  assert.equal((message as { withPacketName?: unknown }).withPacketName, undefined);
