...
class message_t
{
  public:
    ...
    /**
     * @brief Mark this message as REQUEST with the given correlation ID.
     * @param correlation_id_ Request-reply matching identifier.
     * @return 0 on success, -1 on failure.
     */
    int set_request (uint64_t correlation_id_)
    {
        if (!_valid)
            return -1;
        return zlink_msg_set_request (&_msg, correlation_id_);
    }

    /**
     * @brief Mark this message as REPLY with the given correlation ID.
     * @param correlation_id_ Must match the original request's ID.
     * @return 0 on success, -1 on failure.
     */
    int set_reply (uint64_t correlation_id_)
    {
        if (!_valid)
            return -1;
        return zlink_msg_set_reply (&_msg, correlation_id_);
    }

    /**
     * @brief Retrieve request-reply information from the message.
     * @param type_out_ Message type (DATA=0, REQUEST=1, REPLY=2). May be NULL.
     * @param correlation_id_out_ Correlation ID. May be NULL.
     * @return 0 on success, -1 on failure.
     */
    int get_request_info (uint8_t *type_out_,
                          uint64_t *correlation_id_out_) const
    {
        if (!_valid)
            return -1;
        return zlink_msg_get_request_info (&_msg, type_out_,
                                           correlation_id_out_);
    }
    ...
    /**
     * @brief Set a metadata key-value pair on this message.
     * @param key_ User key (>= 0x0100).
     * @param value_ Value bytes (NULL for zero-length).
     * @param value_size_ Value size in bytes.
     * @return 0 on success, -1 on failure.
     */
    int set_metadata (uint16_t key_, const void *value_, size_t value_size_)
    {
        if (!_valid)
            return -1;
        return zlink_msg_set_metadata (&_msg, key_, value_, value_size_);
    }

    /**
     * @brief Retrieve a metadata value by key.
     * @param key_ Metadata key.
     * @param size_out_ Output value size. May be NULL.
     * @return Pointer to value bytes, or NULL if absent.
     */
    const void *get_metadata (uint16_t key_, size_t *size_out_ = NULL) const
    {
        if (!_valid)
            return NULL;
        return zlink_msg_get_metadata (&_msg, key_, size_out_);
    }
