internal static class MultiPerfPatternRegistry
{
    private static readonly PerfPatternRegistry Registry = new(
        new IPerfPattern[]
        {
            ...
            new MultiPerfPattern("SPOT_REQREP",
                static options => PerfMultiSpotReqRep.RunServer(options),
                static options => PerfMultiSpotReqRep.RunClient(options)),
            new MultiPerfPattern("SPOT_SENDSEND",
                static options => PerfMultiSpotReqRep.RunSendSendServer(options),
                static options => PerfMultiSpotReqRep.RunSendSendClient(options)),
