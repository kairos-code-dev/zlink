...
bool handle_recv_parts(spot_reqrep_client_slot_t *slot,
                       zlink_msg_t *parts,
                       size_t part_count)
{
    ...
    if (now_ns < state->active_deadline_ns.load(std::memory_order_acquire)
        ...
        if (bench_debug_enabled()
            && g_client_debug_recv_logs.fetch_add(1, std::memory_order_acq_rel)
                 < 8) {
            std::cerr << "[multi-spot-sendsend-client] recv reply slot="
    ...
    switch (rc) {
        case ZLINK_SUBMIT_OK:
            if (bench_debug_enabled()
                && g_client_debug_send_logs.fetch_add(1, std::memory_order_acq_rel)
                     < 8) {
                std::cerr << "[multi-spot-sendsend-client] send ok slot="
        ...
        case ZLINK_SUBMIT_NOT_CONNECTED:
            if (bench_debug_enabled()
                && g_client_debug_send_logs.fetch_add(1, std::memory_order_acq_rel)
                     < 8) {
                std::cerr << "[multi-spot-sendsend-client] send not connected slot="
                          << slot->index << std::endl;
            }
            return send_result_not_connected;
        default:
            if (bench_debug_enabled()) {
                std::cerr << "[multi-spot-sendsend-client] send failed slot="
                          << slot->index << " rc=" << rc
                       ...
                       bench_latency_stats_t *latency_out)
{
    ...
    *latency_out = bench_latency_stats_t();
    ...
    for (size_t i = 0; i < state->slots.size(); ++i) {
        state->slots[i].waiting_reply.store(false, std::memory_order_release);
        state->slots[i].send_pending.store(false, std::memory_order_release);
    ...
    while (std::chrono::steady_clock::now() < deadline) {
        ...
        for (size_t i = 0; i < state->slots.size(); ++i) {
            ...
            if (send_rc == send_result_ok) {
                ...
                if (bench_debug_enabled()) {
                    std::cerr
                      << "[multi-spot-sendsend-client] send request fatal slot="
