...
| `tcp` | `MULTI_SPOT_REQREP` | `통과(80.0%)` | `통과(88.9%)` | `통과(85.9%)` | `통과(88.3%)` | `보류(67.2%)` | `보류(67.6%)` | 64/256/1024B는 fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Rust `perf_rust_multi_linux_20260522_150056_codex_rust_multi_tcp_spot_reqrep_20260522.txt` 기준이다. 65536/131072/262144B는 fresh C 대비 Rust 반복 측정 `perf_rust_multi_linux_20260524_211538.txt` median 기준이다. server reply가 받은 single-part `Message`를 다시 복사하지 않고 그대로 reply로 넘기고, client active request를 C처럼 `DONT_WAIT`로 맞춰 65536B는 통과로 올렸다. 131072/262144B는 아직 기준보다 낮다. |
| `tcp` | `MULTI_SPOT_SENDSEND` | `통과(93.3%)` | `통과(94.3%)` | `통과(94.7%)` | `보류(61.1%)` | `통과(75.4%)` | `보류(67.1%)` | 65536B/262144B는 fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Rust 반복 측정 `perf_rust_multi_linux_20260524_210213.txt` median 기준이다. 나머지는 Rust `perf_rust_multi_linux_20260524_200540.txt` 기준이다. client active loop를 C와 같은 size별 active slot 제한(65536B 이상 32, 131072B 이상 8)과 POLLIN poller wait로 맞췄다. 기존 18.1/68.6/85.9/60.1/2.9/6.2%에서 크게 올랐고 64/256/1024/131072B는 통과했다. 65536/262144B는 반복 측정에서도 SPOT 기준보다 낮다. active slot을 16으로 좁힌 후보 `perf_rust_multi_linux_20260524_200834.txt`는 65536/262144B를 더 낮춰 반영하지 않는다. |
...
- **MULTI_SPOT_REQREP server single-part reply + client DONT_WAIT**: Rust server는 받은 payload를 `Message::copy_from(...)`으로 다시 복사한 뒤 reply했다. `Received::single_part()`로 받은 owned `Message`를 그대로 reply에 넘기도록 바꿔 C의 받은 part 재전송 의미와 맞췄고, client active request도 C처럼 `DONT_WAIT`로 제출하게 했다. tcp repeat3 median은 65536B 88.3%로 통과, 131072/262144B 67.2/67.6%로 보류(`perf_rust_multi_linux_20260524_211538.txt`)다. active slot 16 후보(`perf_rust_multi_linux_20260524_211217.txt`)와 callback latency `mpsc`를 `Mutex` 기록으로 바꾸는 후보(`perf_rust_multi_linux_20260524_211329.txt`)는 131072B 또는 262144B를 낮춰 반영하지 않는다.
- **MULTI_SPOT_SENDSEND client active-slot + poller wait**: Rust sendsend client는 C와 달리 large size에서도 100개 spot을 모두 active로 돌리고, reply readiness를 poller로 기다리지 않고 progress가 없을 때 `thread::sleep(1ms)`를 썼다. C와 같은 active slot 제한(65536B 이상 32, 131072B 이상 8), POLLIN poller wait, active window 뒤 pending reply drain을 적용했다. 공식 runner complete 결과: tcp 64/256/1024/131072B는 통과, 65536/262144B는 반복 측정 median 61.1/67.1%로 보류(`perf_rust_multi_linux_20260524_210213.txt`), ws 전 size 81.6~101.3% 통과(`perf_rust_multi_linux_20260524_200859.txt`), wss도 64B 반복 측정 median 77.6%(`perf_rust_multi_linux_20260524_210036.txt`)로 올라 전 size 통과, tls 전 size 82.5~89.6% 통과(`perf_rust_multi_linux_20260524_200645.txt`). active slot을 16으로 좁힌 후보는 `perf_rust_multi_linux_20260524_200834.txt`에서 65536/262144B가 낮아져 반영하지 않는다.
...
- **2026-05-24 tcp full 재측정 요약**: DEALER_ROUTER 46~107%(전 size 통과), ROUTER_ROUTER small 통과/large 보류, SPOT_REQREP 64/256/1024/65536B 통과 및 131072/262144B 보류, STREAM 90~101% 통과, DD small 통과/large 보류, SPOT small 통과권/large 보류(copy 영역), SPOT_SENDSEND 대폭 개선/PUBSUB 일부만. PUBSUB small이 fresh baseline 대비 ~3~5%인 것은 회귀가 아니라(이전 24%는 옛 baseline 기준) 단일 thread로 100 subscriber를 drain하는 per-call 비용 + C PUBSUB(2.6M msg/s)이 매우 높기 때문이다.
- **남은 보류(Rust)**: DEALER_DEALER large, routed large 일부, SPOT large(copy 영역), SPOT_REQREP tcp 131072/262144B, SPOT_SENDSEND tcp 65536/262144B, PUBSUB small(단일 thread drain).
