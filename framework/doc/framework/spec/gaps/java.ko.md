...
| ID | 계약 | 구현이 하는 일 |
|----|------|----------------|
| **SMP-JV-03** (미구현) | [gamequest:280](../../common/sample/event/gamequest.ko.md): "노드를 재시작해도 owner spot이 **`QuestEventStore` replay로 aggregate를 rehydrate**한다". §14(`:603`)도 rehydrate를 "노드 재시작 → replay로 aggregate 복원"으로 못박는다 | SMP-JV-19에서 domain event delta fold를, SMP-JV-13에서 Redis startup replay를 구현했다. 그러나 rehydrate 완료 gate는 아직 실제 재시작 뒤 channel 조회가 아니라 `markRehydrated` 호출 횟수를 세며, player owner Spot 책임도 없다. ⇒ 저장·복원 기반은 생겼지만 정식 owner lifecycle은 아직 구현되지 않았다. 심층은 SMP-JV-11·12 |
