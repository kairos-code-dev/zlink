...
| SM-F3 | 전환 필요 | 같은 MeshNode에 ChannelName `ChannelEchoReq`, RID direct request와 Spot direct `StateReq`를 섞어 제출하고 각각의 handler evidence가 정확히 한 번 남는지 검증한다. |
...
| SM-F5 | 전환 필요 | ChannelName request와 Spot direct request를 처리한 뒤 target Spot을 닫고, 해당 Spot 경로만 실패하며 MeshNode peer와 ChannelName handler는 계속 ready인지 확인한다. |
...
| route client target spot | `SM-F1`, `SM-F2`, `SM-F3`, `SM-F4`, `SM-F5` | 혼합 | `SM-F1`·`SM-F2`·`SM-F4`는 기존 증거가 있다. `SM-F3`은 ChannelName·RID direct·Spot direct 혼재와 handler evidence를, `SM-F5`는 Spot close 뒤 Spot 경로만 실패하고 MeshNode peer·ChannelName handler가 유지되는지를 10.0.0 표면으로 다시 검증해야 한다. |
