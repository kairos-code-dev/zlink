...
{
  public:
    using invoker_t = std::function<task_t<zlink::message_t> (service_provider_t &,
                                                              serializer_registry_t &,
                                                              const zlink::message_t &,
                                                              const framework::route_handler_context_t &)>;

    template <typename TOwner, typename TMessage>
    route_handler_registry_t &on_send (std::string router_channel_id,
                                       std::string packet_name,
                                       void (TOwner::*method) (const TMessage &,
                                                               const framework::route_handler_context_t &))
    {
        const auto packet =
          packet_name.empty () ? framework::detail::message_name<TMessage> () : std::move (packet_name);
        return add_handler (
          route_handler_descriptor_t{runtime::messaging::message_kind_t::command, std::move (router_channel_id), packet,
        ...
        return add_handler (
          ...
          [method] (service_provider_t &services, serializer_registry_t &serializers, const zlink::message_t &message,
              ...
              try {
                  ...
                  co_return current_exception_to_message_result ("routed handler threw an exception");
              }
          });
    }

    template <typename TOwner, typename TRequest, typename TReply>
    route_handler_registry_t &on_request (std::string router_channel_id,
                                          std::string packet_name,
                                          TReply (TOwner::*method) (const TRequest &,
                                                                    const framework::route_handler_context_t &))
    {
        const auto packet =
          packet_name.empty () ? framework::detail::message_name<TRequest> () : std::move (packet_name);
        return add_handler (
          route_handler_descriptor_t{runtime::messaging::message_kind_t::request, std::move (router_channel_id), packet,
