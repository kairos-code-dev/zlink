[한국어](05-stream-session.ko.md) | English

[Specification index](../../README.md) · [Core index](../README.md) · [Service overview](README.md) · [MeshNode](01-mesh-node.md) · [Actor](04-actor.md) · [Dispatch](02-dispatch.md) · [raw STREAM](../socket/08-stream.md)
...
When Actor transfer prepare begins, every active binding becomes a transfer
participant. The service records a barrier after the last old-epoch message in
the same session FIFO and assigns participant-local sequences. Messages
admitted after the barrier remain in a bounded pending queue and are not
delivered to the source Actor mailbox. Target prepare's private readiness
negotiation reserves the maximum transfer allowance of each queue without
requiring a participant list or pending-message batch from the caller.

The service admits a pending message only within reserved allowance. When the
allowance is exhausted, `zlink_stream_session_send_to_actor()` and
`zlink_stream_session_request_to_actor()` return
`ZLINK_SUBMIT_BACKPRESSURED` with `EAGAIN`, and raw-session ingress admission
stops until allowance is released. A message that was not admitted is not part
of the transfer. Status `pending_message_count` and `pending_byte_count` include
only admitted pending traffic.

After target readiness, source Core automatically transfers pending messages
in session FIFO order. Starting target commit makes the service seal the last
pending sequence as the terminal high-water and separates later traffic into a
new-epoch forwarding queue. The target stages the contiguous range through the
terminal high-water and acknowledges it; target commit cannot succeed before
that acknowledgment. Activate exposes the participant FIFO and new-epoch
traffic after the frozen Actor backlog. No global order is guaranteed across
different session participants.

A session disconnect becomes a terminal participant state only after admitted
messages and the terminal high-water have been processed in order. Disconnect
alone does not complete the high-water. If transport disconnects before order
can be proven, transfer prepare fails. Peer loss and retransmission after
prepare follow the [Actor transfer contract](04-actor.md#6-transfer-fence).

After commit, the service keeps pending bindings and messages in target-Actor
staging and does not start application dispatch before activation. Activate
exposes the new-epoch FIFO. On authority abort, target staging is discarded
before the source binding, pending FIFO, and new-epoch forwarding traffic are
restored in original order. A stale transfer ID, Actor generation, membership
epoch, or binding generation returns `ESTALE`.
...
The [errno map](../04-errno-map.md) defines the exact mapping between result enums and errno values.
