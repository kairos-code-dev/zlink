...
| `V11-M6C-DN` | .NET maintenance·monitoring·ASP.NET | .NET lane, `P-DEEP` | `V11-R5B` | 진행 | CAS·lease·Task race·terminal observation·ASP.NET shutdown contract 통과 | `IZLinkFrameworkRuntime`의 host-wide state·snapshot·bounded observer·`RetireAsync`·`ShutdownAsync`와 ASP.NET hosting stop 연결을 유지한다. Retire preflight blocker는 admission seal 전에 `Serving`을 유지하고, Shutdown과 Retire는 first effective intent의 shared operation에 합류하며 waiter cancellation은 shared operation을 취소하지 않는다. Target .NET 계약의 `IZLinkAuthorityStore`, `IZLinkRelocationStore`, Actor·Spot relocation adapter와 policy, 분리된 `AddRelocationStore`를 추가했다. User·Instance Spot과 Actor participant를 함께 담는 deterministic root는 application state, accepted queue sequence·payload와 logical timer cursor·payload를 보존한다. Root를 24시간 retention으로 먼저 저장하고 CRC32C·immutable read를 검증한 뒤 single authority CAS 또는 aggregate prepare·commit으로 publish한다. CAS conflict와 prepare reject는 orphan을 정리하고, commit outcome exception은 authority reference를 읽어 reconcile하며 published root missing·checksum·inventory mismatch는 rollback하지 않는 data-loss로 분류한다. Managed Spot·Actor outbound frame은 object generation이나 상수 `1`을 authority fence로 쓰지 않고 Location row에서 관찰한 `AuthorityOwnerGeneration`만 사용하며 inbound는 local actual generation과 일치할 때만 dispatch한다. In-memory와 Redis Location row는 owner claim generation을 이 field로 materialize한다. 이번 slice에서 `IZLinkLocationStore`가 `IZLinkAuthorityStore`를 직접 상속하도록 연결하고 in-memory provider에 exact read·CAS·snapshot scan·reservation·aggregate state를 추가했다. 공식 Redis package에는 Location과 별도 options·connection lifecycle을 가진 `ZLinkRedisRelocationStore`를 추가했다. Relocation payload는 SHA-256 reference와 CRC32C를 사용하고 Redis `TIME` 기준 retention으로 저장·renew·read·delete한다. `ZLinkRedisLocationStore`는 같은 namespace에서 authority Preserve/Delete CAS, 1분 snapshot scan, reservation과 bounded aggregate prepare·commit·abort를 server-side script로 실행한다. 새 Redis integration 4/4, relocation contract 10/10, stateful authority regression 10/10, 문서 regression을 제외한 unit runtime 687/687과 Framework·Redis provider build warning·error 0이 통과했다. Redis 기존 회귀 8건 중 `IReadOnlySet<string>` codec 원인 6건을 수정했고 34/36이 통과했다. 남은 2건은 target fixture의 새 descriptor·authority HASH와 이전 test codec 비교가 충돌하는 contract migration gap이다. `ZLinkAuthorityMutation.Put`에 target owner token이 없어 opaque payload를 해석하지 않는 provider가 `NewOwner`·`NewObject` metadata를 만들 수 없는 public contract hole도 확인했다. 따라서 Redis single CAS는 Preserve/Delete만 구현하고 owner 전환은 `TargetOwner`가 있는 aggregate·reservation 경로만 허용한다. Exact owner lease generation·capacity schema, fixture row format과 실제 maintenance owner queue·timer·adapter capture/restore·target replay 연결은 계속한다. 아직 `M6-RUNTIME` 또는 `V11-R5C` 시작 증거로 사용하지 않으며 Sample·E2E·Core·bindings 변경·실행은 0이다. |
| `V11-M6C-JVM` | JVM maintenance·monitoring·Spring | JVM lane, `P-DEEP` | `V11-R5B` | 진행 | Java·Kotlin lifecycle·coroutine·Spring metadata와 shutdown contract 통과 | 목표 Java 계약의 host-wide `retire`·`shutdown`, runtime state·termination result·snapshot·capacity-bounded observer를 기존 bounded drain/teardown owner에 연결했다. Retire는 admission seal 전에 Actor target availability를 preflight하며 blocker에서는 Serving을 유지하고 재시도를 허용한다. Shutdown이 Retire preflight 중 호출되면 effective intent를 Shutdown으로 바꾸고 shared operation을 계속한다. ready-first relocation scheduler는 기본 64 unit과 256 MiB를 함께 제한하고 oversized unit을 단독 실행한다. Location Store와 별도로 등록하는 `ZLinkRelocationStore`, immutable root 결과 계약, Actor·Spot relocation adapter, Instance Spot lifecycle과 close reason public surface를 추가했다. Spring `SmartLifecycle` 종료는 deprecated drain facade 대신 같은 Framework runtime의 30초 bounded `shutdown()`을 호출한다. 최신 raw-only bindings에서 제거된 Core actor-transfer token 생성 bridge를 testkit에서도 제거했다. User·Instance Spot timer registry는 relocation turn에서 logical registration, schedule cursor, next execution과 pending tick을 canonical binary envelope로 freeze한다. Precommit abort는 기존 timer handle을 유지해 resume하고 target restore는 새 native timer를 만든 뒤 pending tick을 먼저 dispatch한다. 정식 Java 계약의 `ZLinkAuthorityStore`와 read·CAS·scan, object reservation·commit·abort, aggregate prepare·commit·abort 결과·mutation type을 public surface에 추가했다. 같은 Location provider가 authority capability를 구현하면 기존 등록 경로가 이를 runtime service로 노출하며, 별도 internal authority port나 기본 fake Store는 만들지 않았다. `ZLinkAsyncSerialQueue`는 closure tail 대신 순번과 직렬화 payload를 가진 ingress record를 보관한다. 현재 turn 경계에서 seal하면 기존 application record를 capture하고 이후 ingress를 hold하며 infrastructure work는 계속 실행한다. Abort는 기존 infrastructure work를 먼저 유지한 뒤 captured→held 순서로 admission을 복원하고, commit은 held record만 relay 대상으로 반환한다. 이번 slice에서 실제 User Spot route와 local Actor session ingress가 transport identity, request sequence, metadata, STREAM header와 message parts를 canonical accepted-journal record로 queue에 함께 저장한다. Commit은 실행하지 않은 source handler의 native resource를 해제하고 held record만 relay 대상으로 반환한다. 공식 Redis extension에는 Location과 별도 options·connection lifecycle을 가진 `ZLinkRedisRelocationStore`를 추가했다. Payload는 SHA-256 content reference와 CRC32C를 사용하고 Redis `TIME` 기준 retention으로 immutable put·read·renew·delete한다. `ZLinkRedisLocationStore`는 authority capability를 직접 구현하여 exact read, mutation마다 revision을 확인하는 snapshot page, `PRESERVE`·delete CAS와 object reserve·commit·abort를 server-side script로 처리한다. Runtime의 publication coordinator는 immutable root put·read·checksum 검증 뒤 authority CAS를 수행하고 conflict orphan을 삭제한다. CAS 응답이 불명확하면 exact read로 reconcile하며 published root missing·checksum mismatch는 source rollback 없이 data-loss로 종료한다. 두 Store가 등록된 runtime만 이 coordinator를 사용한다. Accepted-journal·queue·authority focused contract, JVM core regression 416/416, Spring starter regression 33/33, Kotlin compile과 임시 local Redis에서 provider 18개 test 중 cross-language 환경 의존 2개 skip·실패 0이 통과했다. Redis authority fixture도 현재 공통 authority HASH 계약에 맞췄다. Java exact contract에는 `NEW_OWNER`·`NEW_OBJECT` mutation이 새 owner token을 전달하지 못하고 aggregate가 검증할 exact owner lease claim/read와 adapter registration mapping이 없는 공백이 있다. 따라서 opaque payload를 해석하는 우회를 만들지 않았으며 Redis single-key CAS는 `PRESERVE`·delete만 허용한다. Aggregate prepare·commit·abort, User Spot·member Actor adapter capture/restore, timer·journal root 조합, target replay-before-commit과 `ZLinkLocationStore` authority 상속은 이 계약 공백을 정리한 뒤 연결해야 한다. 현재 drain owner도 기존 network Actor handoff와 recreatable Spot release를 사용하므로 아직 `M6-RUNTIME` 또는 `V11-R5C` 시작 증거가 아니다. Spring construction/start 분리도 남아 있다. Sample·E2E·Core·bindings 변경·실행은 0이다. |
...
| `V11-R5C` | Maintenance runtime slice 독립 review | Codex review lane, `P-DEEP` + Claude `claude-sonnet-5` 병렬 reviewer | `V11-M6C-CPP`, `V11-M6C-DN`, `V11-M6C-JVM`, `V11-M6C-NODE` | 대기 | lifecycle·authority·handover·recovery·observability·hosting과 실행 격리의 I1·I2·I3 review clean | C++ candidate는 host first-intent barrier, exact target preflight, User Spot aggregate CAS, STREAM generation fence와 terminal-once observation까지 확장했고 focused 100회 반복·전체 6/6·full compile 증거를 `V11-M6C-CPP`에 기록했다. JVM candidate는 actual Spot·Actor accepted-journal ingress, 공식 Redis Relocation Store, authority `PRESERVE`·delete와 reservation, immutable payload→CAS publication·reconcile까지 연결했다. JVM의 target owner mutation·exact owner lease·adapter registration 공백과 aggregate·target replay 연결이 남았으므로 review candidate로 승격하지 않는다. 네 언어 M6C candidate와 독립 reviewer가 모두 준비되기 전에는 이 row를 시작하거나 clean으로 판정하지 않는다. |
