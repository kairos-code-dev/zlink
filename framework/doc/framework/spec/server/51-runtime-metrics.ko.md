...
| `zlink.transfer.started` | counter | `{transfer}` | `mesh_name`, `object_kind`, `policy` | Actor·Instance Spot transfer 시작 누계 |
...
`recreate|snapshot`, transfer `outcome`은 `completed|aborted|recovered|failed|shutdown`, `transport`는
