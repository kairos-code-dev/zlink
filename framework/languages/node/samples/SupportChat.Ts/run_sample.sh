...
cleanup() {
  ...
  if [[ "${SUPPORTCHAT_TS_KEEP_RUN_DIR:-}" != "1" ]]; then
    rm -rf "${RUN_DIR}"
  else
    echo "runDir=${RUN_DIR}"
  fi
...
export SUPPORTCHAT_SESSION_ENDPOINT="${SUPPORTCHAT_SESSION_ENDPOINT:-tcp://127.0.0.1:${PORTS[2]}}"
export SUPPORTCHAT_SUPPORT_ENDPOINT="${SUPPORTCHAT_SUPPORT_ENDPOINT:-tcp://127.0.0.1:${PORTS[3]}}"
export SUPPORTCHAT_NOTIFICATION_ENDPOINT="${SUPPORTCHAT_NOTIFICATION_ENDPOINT:-tcp://127.0.0.1:${PORTS[4]}}"
export SUPPORTCHAT_API_ENDPOINT="${SUPPORTCHAT_API_ENDPOINT:-tcp://127.0.0.1:${PORTS[5]}}"
...
with open(sys.argv[1], "w", encoding="utf-8") as output:
    json.dump({
        "sample": {
            "registryPubEndpoint": "${SUPPORTCHAT_REGISTRY_PUB_ENDPOINT}",
            "registryRouterEndpoint": "${SUPPORTCHAT_REGISTRY_ROUTER_ENDPOINT}",
            "sessionEndpoint": "${SUPPORTCHAT_SESSION_ENDPOINT}",
            "supportEndpoint": "${SUPPORTCHAT_SUPPORT_ENDPOINT}",
            "notificationEndpoint": "${SUPPORTCHAT_NOTIFICATION_ENDPOINT}",
            "apiEndpoint": "${SUPPORTCHAT_API_ENDPOINT}"
        }
    }, output, indent=2)
PY
...
wait_port registry-pub "${SUPPORTCHAT_REGISTRY_PUB_ENDPOINT}"
