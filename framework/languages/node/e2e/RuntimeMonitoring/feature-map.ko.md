...
|---|---|---|---|
| MON-A1 | 10.0.0 전환 대상 | `svc-b` 종료·재기동에서 topology node 수와 service summary 수가 감소한 뒤 복구된다. | Snapshot 하나의 MeshNode·peer·channel·multicast·claim·location·drain field, sequence와 불변성을 검증한다. |
...
기존 runner 통과는 표의 현재 증거만 보존한다. 남은 gap을 구현한 뒤 public operation
result, typed event와 후속 snapshot을 같이 단언하는 canonical scenario를 재실행한다.
