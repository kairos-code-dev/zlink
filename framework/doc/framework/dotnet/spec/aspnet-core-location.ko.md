...
| 서비스 | 표면 |
|--------|------|
| `IZLinkPeerLocationResolver` | `ListPeersAsync(ZLinkPeerLocationFilter)` |
| `IZLinkSpotLocationResolver` | `ResolveSpotAddressAsync(RoutingId spotRid)` → `ZLinkSpotAddress?` |
| `IZLinkActorLocationResolver` | `ResolveActorSpotAddressAsync(string actorType, string actorId)` → `ZLinkSpotAddress?` |
| `IZLinkRouteLocationResolver` | `ResolveRouteAsync(ZLinkRouteLocationKey)` |
| `IZLinkLocationRuntimeQuery` | `GetStatusAsync`, `ListPeersAsync`, `ListSpotsAsync`, `ListActorsAsync`, `ListRoutesAsync`, `ListTopologyAsync`, `ListServiceSummariesAsync` |

`ZLinkSpotAddress` 는 `MeshName + NodeRid + SpotRid` 값 객체다. 호출자가 보관하고 전송
실패 시 재resolve 한다.
