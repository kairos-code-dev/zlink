...
void spot_ctrl_debugf (const char *fmt_, ...)
{
    ...
    zlink::debug_vfprintf_with_file ("ZLINK_SPOT_CTRL_DEBUG", "[spot-ctrl] ",
                                     zlink::spot_debug::ctrl_log_path (), fmt_, args);
    va_end (args);
}
}

int zlink::spot_data_plane_message_io::send_ascii_frame (socket_base_t *socket_,
...
int zlink::spot_data_plane_message_io::send_control_snapshot (socket_base_t *socket_,
                                                              const char *topic_,
                                                              const std::string &target_endpoint_,
                                                              const std::string &source_node_id_,
                                                              const std::set<std::string> &filters_)
{
    if (!socket_ || !topic_ || target_endpoint_.empty () || source_node_id_.empty ()) {
        errno = EINVAL;
        return -1;
    }
    ...
    if (send_ascii_frame (socket_, version, has_filters ? ZLINK_SNDMORE : 0) != 0)
        return -1;
    ...
    while (it != filters_.end ()) {
        ...
        ++next;
        if (send_ascii_frame (socket_, *it, next != filters_.end () ? ZLINK_SNDMORE : 0) != 0) {
            return -1;
        }
        it = next;
    }

    spot_ctrl_debugf ("send snapshot target=%s source=%s filters=%zu", target_endpoint_.c_str (),
                      source_node_id_.c_str (), static_cast<size_t> (filters_.size ()));
    return 0;
}

int zlink::spot_data_plane_message_io::send_snapshot_to_target (socket_base_t *socket_,
                                                                spot_node_t *node_,
                                                                const std::string &target_endpoint_)
{
    ...
    return send_control_snapshot (
      ...
      spot_control_protocol::node_id_string (spot_node_access_t::runtime (node_)->node_id),
      filters);
}
  ...
  const std::map<std::string, std::string> &peer_ctrl_endpoints_)
{
    ...
    for (std::map<std::string, std::string>::const_iterator it = peer_ctrl_endpoints_.begin ();
         it != peer_ctrl_endpoints_.end (); ++it) {
        if (it->first.empty ())
            continue;
        if (send_control_snapshot (socket_, spot_control_protocol::ctrl_snapshot_topic, it->first,
                                   source_node_id, filters)
            != 0) {
            return -1;
        }
    }
    ...
    &outbound_ready_filters_)
{
    ...
    const std::map<std::string, std::map<std::string, std::set<std::string>>>::const_iterator
    ...
    for (std::map<std::string, std::set<std::string>>::const_iterator it =
        ...
        if (it->first.empty ())
            continue;
        if (send_control_snapshot (socket_, spot_control_protocol::ctrl_ready_ack_topic,
                                   target_endpoint_, it->first, it->second)
            != 0) {
            return -1;
        }
    }
...
bool zlink::spot_data_plane_message_io::parse_ready_ack_arg (const std::string &arg_,
...
int zlink::spot_data_plane_message_io::recv_remaining_frame_strings (socket_base_t *socket_,
                                                                     std::vector<std::string> *out_)
{
    ...
    while (true) {
        ...
        out_->push_back (std::string (static_cast<const char *> (frame.data ()), frame.size ()));
...
int zlink::spot_data_plane_message_io::recv_remaining_frames_to_vector (
  socket_base_t *socket_, std::vector<zlink_msg_t> *out_, size_t *wire_bytes_out_)
{
...
int zlink::spot_data_plane_message_io::recv_remaining_frames_to_parts (
  socket_base_t *socket_, spot_owned_msg_parts_t *parts_out_, size_t *wire_bytes_out_)
{
