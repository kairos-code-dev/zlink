...
### flaky 시나리오 목록과 판정 규칙

전체 실행이 9개에서 8개로 줄고 OBS-B4가 실패해 회귀를 의심했다. Baseline과 대조하니
아니다.

```
OBS-B4  변경 있음 : 6회 중 3 pass 3 fail
OBS-B4  baseline  : 4회 중 2 pass 2 fail
```

실패 문구는 `ZLink framework runtime is not accepting operations.`인데 gate 수정 전후가
같은 비율이므로 이 수정과 무관하다.

이 세션에서 baseline 대조로 회귀가 아님을 확인한 시나리오가 셋이다.
...
세 번 모두 단발 또는 2회 실행이면 회귀로 오판했을 상황이었다. 이 스위트들에서 회귀를
판정하려면 변경 전후를 **같은 횟수로** 돌려 비율을 비교해야 한다. 특히 aggregate 실행의
통과 개수는 flaky 시나리오 때문에 실행마다 흔들리므로 그 숫자 하나로 판정하면 안 된다.
