...
| **IMP-JV-04** | 결함 | [24 §4.1](../server/24-spot-address-messaging.ko.md)·[05 §13](../05-framework-api.ko.md#13-오류-kind): `SpotRouteNotFound`/`RouteNotConnected`/`RequestTargetNotFound`를 구분한다 | `ZLinkChannelSpotCalls.java:228-236` — 전부 `ZLinkConfigurationException`(kind=`REQUEST_FAILED`, retriable=false). ⇒ **"spot이 사라졌다"와 "mesh가 아직 수렴 중이다"를 구분할 수 없다.** retriable 기반 재시도 정책이 영영 안 돈다 |
...
| **IMP-JV-06** | 결함 | [05 §1.1](../05-framework-api.ko.md#11-public-contract와-runtime-implementation의-경계): exact interface는 실제 public contract를 정확히 투영한다 | **해결:** 값을 전달하지 않던 `metadata(k,v)`를 세 channel call interface와 11개 구현에서 제거했다. 공개 표면 집중 테스트와 core 전체 테스트 통과. 구현 커밋 `3db218ee0`(2026-07-15). |
