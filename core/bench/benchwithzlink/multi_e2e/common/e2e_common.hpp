#ifndef BENCH_WITH_ZLINK_MULTI_E2E_COMMON_HPP
#define BENCH_WITH_ZLINK_MULTI_E2E_COMMON_HPP
...
namespace bench_with_zlink_multi_e2e {
...
inline pattern_t parse_pattern(const std::string &raw)
{
    ...
    if (p == "MULTI_GATEWAY" || p == "GATEWAY")
        return pattern_gateway;
    if (p == "MULTI_SPOT" || p == "SPOT")
        return pattern_spot;
...
{
    return pattern == pattern_pubsub || pattern == pattern_gateway
           || pattern == pattern_spot;
}

inline void e2e_print_result(const std::string &lib_name,
                             pattern_t pattern,
