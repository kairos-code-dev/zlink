...
serverOptions.addRouteMesh("orders")
    ...
    .server()
    .setWeight(100)
    .addRequestHandler(CheckoutHandler); // 이 node를 checkout 요청 처리 후보로 등록한다.
```
...
`SpotMoving=33`, `RelocationDataLost=34`, `SpotIdConflict=35`,
`RuntimeShutdown=36`, `RelocationDisabled=37`,
`RelocationTargetUnavailable=38`, `RelocationFailed=39`를 반환한다.
...
`ZLinkLocationAutoConnectType`, `ZLinkLocationRole`과 `ZLinkPeerLocation` 계열은 [RouteMesh](../../../../01-glossary.ko.md#routemesh) peer만 표현한다.
...
[deadline](../../../../01-glossary.ko.md#deadline)은 admission을 닫은 채 recovery handoff와 bounded teardown을 수행하고
