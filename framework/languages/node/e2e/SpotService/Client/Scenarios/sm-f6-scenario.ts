...
export async function runSmF6(options: ClientOptions): Promise<void> {
  ...
  const expected = [
    `spot-state-request|rid=${SpotServiceNames.multiSpotNodeB}|spot=${targetSpotId}|value=7`,
    `spot-state-command|rid=${SpotServiceNames.multiSpotNodeB}|spot=${targetSpotId}|marker=sm-f6-send-${marker}`,
    `spot-actor-joined|rid=${SpotServiceNames.multiSpotNodeB}|spot=${targetSpotId}|actor=${actorId}`
  ];
  const evidence = await postJson<string[]>(options.multiBUrl, '/evidence/wait', {
    ...
    evidence.some((line) => line.includes(`spot-actor-joined|rid=${SpotServiceNames.multiSpotNodeB}|spot=${targetSpotId}|actor=${actorId}`)),
