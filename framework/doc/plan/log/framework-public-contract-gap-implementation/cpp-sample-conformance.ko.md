...
| CPP-ATD-TIMER-RESUME-001 | E2E AutomaticTurnDispatch **ATD-C3B**: spot timer 핸들러가 outbound channel request를 await하면 client가 3초 안에 `timer-await-resumed` 증거를 못 본다. 단독(`run_e2e.sh ATD-C3`)은 통과, 전체 스위트에서는 결정적 실패 | **계측으로 정정된 사실**(직렬 큐에 trace를 넣어 확인): 재개는 *유실*이 아니라 **지연**이다. 순서가 `timer-await-released` → 형제 actor 요청 정상 처리 → (클라이언트 3초 대기 실패로 러너가 서버 종료 시작, spot 직렬 큐 3개가 cancel+destroy) → 그 **뒤에야** `resume-post`가 도착한다(파괴된 큐에 post → closed=1 → inline fallback). 즉 직렬 턴 해제·재개 배선은 정상이고, **timer 핸들러가 await한 outbound channel 응답이 늦게 관측되는 것**이 원인이다. delay 서버는 요청을 받아 `delay-completed`까지 남겼으므로 응답 자체는 생성됐다. 다음 단계: 같은 호출을 actor 핸들러에서 하면(ATD-C3A) 즉시 재개되므로, timer 경로에서 detached thread의 blocking submit이 응답을 관측하는 데 걸리는 시간을 route/channel client 펌프 관점에서 계측(관련: CPP-CONN-PUMP-001 — cpp connector는 유지보수/수신 펌프가 호출 구동식) | framework(열림) |
