...
export async function runRmA4(options: ClientOptions): Promise<void> {
  ...
  try {
    ...
    const first = await postJson<ProfileRes>(providerV1.httpUrl, '/profile/request', { value: 'rm-a4-v1' });
    ...
    for (let i = 0; i < 20; i += 1) {
      const reply = await postJson<ProfileRes>(providerV2.httpUrl, '/profile/request', { value: `${marker}-${i}` });
