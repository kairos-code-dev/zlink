...
> - 제안은 perf 목적 범위 안에 있어야 하며, PERF 정책과 `core/perf`의 측정 의미를 그대로 따라야 한다.
...
- SPOT 경로의 책임을 `perf_spot_client.cpp` 한 파일에 계속 쌓지 않는다.
- callback/recv 모드가 같은 metrics 수집 골격을 각자 복제하지 않게 한다.
- `single/common/perf_single_common.hpp`의 넓은 공용 facade를 더 좁은 책임 단위로 쪼갤 수 있는지 판단한다.
- 측정 의미, 결과 포맷, smoke 검증 흐름은 `core/perf`와 똑같이 유지한다.

## 2. 현재 구조 요약

- `bindings/cpp/perf/multi/src/perf_spot_client.cpp`는 1007라인이고, control-plane 준비부터 callback/recv active 실행, drain 처리, 결과 출력, resource probe, worker stop까지 한꺼번에 떠안는다.
- 같은 파일 안에서 `run_callback_active()`와 `run_recv_active()`가 서로 다른 동기화 모델을 쓰고 종료/수집 경로도 따로 간다.
- `callback_client_state_t`는 phase wait, thread metrics registry, active start/fatal 상태를 한데 들고 있다.
...
- `single/common/perf_single_common.hpp`는 382라인이고, queue probe, callback receiver, subscribe callback receiver, phase runner 선언을 한 헤더에 모아 둔다.
- `multi/common/perf_common.hpp`는 664라인으로 result 출력과 resource probe helper를 같이 담지만, 이쪽은 이미 공용 helper 성격이 강해서 이번 후속의 1차 타깃은 아니다.
- runner orchestration은 `run_binding_single.sh`(808라인), `run_binding_multi.sh`(1012라인) 쪽에 따로 있다. 크기는 크지만 측정 의미 자체를 바꾸는 구조 문제로 보기는 어렵다.
...
### 3.1 `perf_spot_client.cpp`가 너무 많은 책임을 떠안는다

이 파일은 SPOT control-plane, callback path, recv path, metrics aggregation, recv worker 수명, result emission을 한꺼번에 다룬다.
특히 `run_callback_active()`와 `run_recv_active()`는 서로 다른 phase 대기 모델을 쓰고, `drain_recv()`는 데이터-plane 처리까지 직접 한다.
과거에 지적된 정책 이슈는 이미 정리됐지만 deep module 관점에서 책임 경계는 여전히 넓다.
...
### 3.2 callback/recv metrics helper family가 같은 골격을 두 번 들고 있다
...
모드별 차이는 실제 수집 소스와 출력 시점뿐이고, registry/collection skeleton은 거의 같다. 이건 지금도 grep 구조로 확인된다.
...
이 헤더는 queue probe, callback receiver, subscribe callback receiver, phase runner 선언을 한곳에 묶는다.
`callback_receiver_t`와 `subscribe_callback_receiver_t`는 서로 비슷한 event queue / worker / result wait 구조를 공유하면서도 별도 타입으로 남아 있다.
single perf 공용부는 이미 많이 정리됐지만, 여전히 "공용 도구"와 "모드별 receiver state machine"이 같은 파일에 섞여 있다.
...
가장 먼저 할 일이다. SPOT client가 control-plane, recv worker lifecycle, result emission을 한꺼번에 쥐는 구조를 줄여야 한다.

### P1. callback/recv metrics helper 골격 통합

중복은 작아 보여도 패턴이 같으니, registry/epoch/snapshot 흐름을 한 번만 정의하는 쪽이 더 깊은 모듈이 된다.

### P2. `single/common/perf_single_common.hpp`의 공용 facade 축소 여부 결정

이 파일은 이미 maintenance-heavy 해졌다. 완전한 재설계까지는 아니라도, receiver state machine과 queue/report 도구를 가를 경계가 명확한지 먼저 판정해야 한다.
...
- `perf_spot_client.cpp`가 어디까지 책임지는지 함수/필드 단위로 다시 쪼갠다.
- `single/common/perf_single_common.hpp`가 실제로 어떤 책임들을 묶는지 재분류한다.

완료 기준:

- 이미 끝난 settle/env 정리는 계획에서 완전히 빠진다.
...
- callback-active 전용 상태와 recv-active 전용 상태의 소유 경계를 다시 가른다.
- `phase_mutex` / `_phase_mutex` / `_recv_metrics_mutex` / `_recv_stop` / `_recv_fatal`의 소유 이유를 한 번에 설명할 수 있는 수준으로 정리한다.
- `run_callback_active()`와 `run_recv_active()`가 한 클래스 안에서 서로 다른 state machine을 공유하는 부분을 줄인다.
...
- queue probe / result formatting / receiver state machine이 같은 헤더에 있을 이유를 다시 따진다.
...
- 분리하지 않기로 한 부분은 왜 유지보수 영역인지 설명할 수 있어야 한다.
...
지금 기준으로 `bindings/cpp/perf`는 더 이상 settle/env 류의 policy 위반 후속 과제를 갖고 있지 않다.
남은 가치는 `perf_spot_client.cpp`의 책임 분해, callback/recv helper 중복 축소, `single/common/perf_single_common.hpp`의 경계 정리처럼 유지보수성 중심의 구조 개선에 그친다.
즉, 이 디렉토리가 완전한 maintenance-only는 아니지만 follow-up 리팩토링의 범위는 이미 매우 좁아졌다.
