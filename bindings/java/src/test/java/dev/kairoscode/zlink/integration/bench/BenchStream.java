...
final class BenchStream {
    ...
    private static int runMode(String transport, int size, boolean len32beDispatch,
        ...
        int msgCount = BenchUtil.parseEnv("BENCH_MSG_COUNT", 10000);
        int ioTimeoutMs = BenchUtil.parseEnv("BENCH_STREAM_TIMEOUT_MS", 5000);
        int drainTimeoutMs = BenchUtil.parseEnv("BENCH_STREAM_DRAIN_TIMEOUT_MS",
          Math.max(ioTimeoutMs * 6, 30000));
        int ioThreads = Math.max(1, BenchUtil.parseEnv("BENCH_IO_THREADS", 4));
        ...
        try {
            ...
            String endpoint = BenchUtil.endpointFor(transport, endpointName);
            ...
            BenchUtil.printResult(patternName, transport, size, thr, latUs);
            return 0;
        } catch (Exception ex) {
            return 2;
        } finally {
    ...
    private static int resolveInflightLimit() {
        return Math.max(1, BenchUtil.parseEnv("BENCH_STREAM_INFLIGHT", 10));
    }
