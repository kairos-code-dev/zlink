...
| 현재 pattern | Single `DEALER_ROUTER_REQREP` 진행 중 | tcp와 ws를 완료했고 다음 transport는 wss다. |
| paired C | .NET `DEALER_ROUTER_REQREP / ws` 완료 | 전체 크기를 CPU pin 없이 C와 .NET 순서로 각각 5회 측정했다. |
| 개선 반복 | .NET `DEALER_ROUTER_REQREP / ws` 완료 | 최소 70.9%, 크기 중앙값 80.2%, 평균 latency 최대 약 1.40배로 추가 개선 없이 통과했다. |
| 커밋과 푸시 | .NET `DEALER_ROUTER_REQREP / ws` 문서 반영 중 | source 변경은 없으며 측정 근거만 별도 커밋한다. |
...
| 2 | .NET | `PAIR`, `PUBSUB`, `DEALER_DEALER`, `DEALER_ROUTER` 완료, `DEALER_ROUTER_REQREP` tcp, ws 완료 | 미측정 | `DEALER_ROUTER_REQREP / wss` 전체 크기를 측정한다. |
