...
ScenarioRunner::ScenarioRunner (const std::vector<ClientConfig> &configs) : configs_ (configs)
{
}
...
void ScenarioRunner::create_client (const ClientConfig &cfg)
{
    ...
    if (!client->connect ()) {
        ...
        ConsoleDisplay::print_info (cfg.name + " connected to " + cfg.server_label + " (" + cfg.host
...
std::vector<std::string> ScenarioRunner::drain_events (const std::string &client_name, int wait_ms)
{
...
int ScenarioRunner::run ()
{
    ...
    if ((alice_has_api1 && bob_has_api2) || (alice_has_api2 && bob_has_api1)) {
        passed++;
        ConsoleDisplay::print_info (
          "[check 11] Load balanced: requests served by different API servers");
        ...
        passed++;
        ConsoleDisplay::print_info (
          "[check 11] Both got API responses (same server -- LB may need more requests)");
    } else {
    ...
    if (bob_cfg) {
        ...
        if (!carol_got_event) {
            ...
            ConsoleDisplay::print_info (
              "[check 15] Carol correctly received no Alice events after transfer");
        } else {
            ConsoleDisplay::print_info (
              "[check 15] Carol unexpectedly received Alice event -> FAIL");
        }
    } else {
