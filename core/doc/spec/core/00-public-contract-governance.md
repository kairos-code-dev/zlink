...
This document defines the sources, documentation responsibilities, and change procedure for the ZLink Core 10.0.0 public contract. Its audience is developers who design, implement, and review the public Core C ABI. It answers: “How are the formal specification, public headers, tests, and packages kept consistent?”

## 1. Contract sources

The formal specifications under `core/doc/spec/` define Core 10.0.0 public behavior. `core/include/zlink.h` and the domain headers it includes are the C ABI expression of the same contract. The implementation, contract tests, bindings, and installed packages must agree with both expressions.
...
Formal specifications, guides, and internals describe only the current Core 10.0.0 contract and structure. Navigation from a formal document points only to formal indexes and public-contract owners.

## 3. Change procedure

A public-contract change follows this order:

1. Record the exact 10.0.0 contract in the Korean and English formal specifications.
...
The Korean and English documents have the same heading order, C signatures, enum and struct fields and numeric values, defaults, results, ownership, thread-safety rules, and link targets. A public contract present in only one language is not a valid Core 10.0.0 contract.
