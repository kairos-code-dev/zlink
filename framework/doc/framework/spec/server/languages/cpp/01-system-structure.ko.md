...
| 축 | 내용 |
|---|---|
| **application host** | 부트스트랩, hosted service, module, lifecycle |
| **DI 컨테이너** | lifetime, scope, 생성자 주입 |
| **configuration** | 계층적 설정 모델 |
| **logging** | level, backend, 비동기 큐, 회전 파일 |
| **HTTP hosting** | 내장 서버, route, middleware |
| **zlink messaging** | channel, SPOT, STREAM, actor |
| **handler model** | 등록, dispatch, filter |
| **observability** | metric, flow, health |
...
| 타깃 | CMake | 내용 |
|---|---|---|
| `zlink_framework` | **`zlink::framework`** (STATIC) | framework core. **C++20 필요**(`cxx_std_20`) |
...
**scope 경계가 곧 실행 문맥 경계다.** spot activation scope에서 resolve한 서비스는 그 spot의
...
**startup validation의 항목은 공통 스펙이 소유한다** — [channel-messaging §4](../../11-channel-messaging.ko.md),
[spot-messaging §8](../../20-spot-messaging.ko.md), [stream-session §7.2](../../30-stream-session.ko.md),
