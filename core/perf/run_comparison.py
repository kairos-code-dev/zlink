...
STREAM_SINGLE_VARIANT_PATTERNS = (
    "STREAM",
    "STREAM_CALLBACK",
    "STREAM_LEN32BE",
)
...
STREAM_SERVER_BINARY_BY_PATTERN = {
    "STREAM": "comp_current_multi_stream_server",
    "STREAM_CALLBACK": "comp_current_multi_stream_callback_server",
    "STREAM_LEN32BE": "comp_current_multi_stream_len32be_server",
...
STREAM_SINGLE_TO_MULTI_PATTERN = {
    "STREAM": "MULTI_STREAM",
...
def is_stream_pattern(pattern_name):
    pattern = (pattern_name or "").strip().upper()
    return (
        pattern in STREAM_SINGLE_VARIANT_PATTERNS
        or pattern in STREAM_VARIANT_PATTERNS
    )
...
def resolve_required_binaries(current_bin, pattern_name):
    pattern = (pattern_name or "").strip().upper()
    if pattern in STREAM_SINGLE_VARIANT_PATTERNS:
        server_binary = resolve_stream_server_binary(pattern)
        required = [STREAM_SHARED_CLIENT_BINARY]
        if server_binary:
            required.insert(0, server_binary)
        return required
    if pattern.startswith("MULTI_"):
...
def run_multi_sizes_test_stream_shared(server_binary_name, lib_name, transport, sizes, pattern_name):
    ...
    for _ in range(1):
        ...
        inflight_value = env_pair_value(env, "PERF_MULTI_INFLIGHT", "BENCH_MULTI_INFLIGHT")
        if not inflight_value:
            inflight_value = "1"
            set_env_pair(env, "PERF_MULTI_INFLIGHT", "BENCH_MULTI_INFLIGHT", inflight_value)
        ...
        try:
            inflight_int = max(1, int(inflight_value))
        except ValueError:
            inflight_int = 1
        try:
        ...
        try:
            ...
            sampled = run_command_with_metrics(
                client_cmd + ["--endpoint", endpoint], env, timeout_sec
            )
...
def run_multi_sizes_test(_binary_name, lib_name, transport, sizes, pattern_name):
    if pattern_name in STREAM_VARIANT_PATTERNS:
        ...
        return run_multi_sizes_test_stream_shared(
            server_binary, lib_name, transport, sizes, pattern_name
        )
...
def run_single_stream_test_shared_client(lib_name, transport, size, pattern_name):
    pattern = (pattern_name or "").strip().upper()
    server_pattern = STREAM_SINGLE_TO_MULTI_PATTERN.get(pattern, pattern)
    server_binary = resolve_stream_server_binary(pattern)
    if not server_binary:
        return []

    server_binary_path = os.path.join(BUILD_DIR, server_binary + EXE_SUFFIX)
    shared_client_path = os.path.join(BUILD_DIR, STREAM_SHARED_CLIENT_BINARY + EXE_SUFFIX)
    if not os.path.exists(server_binary_path) or not os.path.exists(shared_client_path):
        return []

    env = get_env_for_lib(lib_name)
    set_env_pair(env, "PERF_MULTI_PATTERN", "BENCH_MULTI_PATTERN", server_pattern)
    set_env_pair(env, "PERF_MSG_SIZES", "BENCH_MSG_SIZES", str(size))
    set_env_pair(env, "PERF_MULTI_STREAM_MSG_SIZES", "BENCH_MULTI_STREAM_MSG_SIZES", str(size))
    set_env_pair(env, "PERF_MULTI_CLIENTS", "BENCH_MULTI_CLIENTS", "1")
    set_env_pair(env, "PERF_MULTI_CONNECT_CONCURRENCY", "BENCH_MULTI_CONNECT_CONCURRENCY", "1")
    ...
    ws_family = transport in ("tls", "ws", "wss")
    default_msg_count = 5000 if ws_family else (200000 if size <= 1024 else 20000)
    default_warmup_count = 100 if ws_family else 1000
    default_lat_count = 100 if ws_family else 500
    msg_count = max(
        1,
        parse_env_int(
            "PERF_MSG_COUNT",
            parse_env_int("BENCH_MSG_COUNT", default_msg_count),
        ),
    )
    warmup_count = max(
        0,
        parse_env_int(
            "PERF_WARMUP_COUNT",
            parse_env_int("BENCH_WARMUP_COUNT", default_warmup_count),
        ),
    )
    lat_count = max(
        1,
        parse_env_int(
            "PERF_LAT_COUNT",
            parse_env_int("BENCH_LAT_COUNT", default_lat_count),
        ),
    )

    timeout_sec = max(120, 30 + int((msg_count / 50000.0) * 15))
    server_cmd = build_bench_cmd(server_binary_path, [lib_name, transport])
    client_args = [
        "--transport",
        transport,
    ...
    client_cmd = build_bench_cmd(shared_client_path, client_args)
    ...
    try:
        ...
        while not endpoint and time.monotonic() <= ready_deadline:
            ...
            if stream_name == "stdout":
                ...
                if token:
                    ...
                    if (
                        token_lib == lib_name
                        and token_pattern == server_pattern
                        and token_transport == transport
                    ):
                        early_status = token_status
                        break
        ...
        if early_status == "unsupported":
            stop_server()
            drain_server_output()
            return [{"metric": "_unsupported_transport_", "value": 1.0}]
        if early_status == "skip":
            stop_server()
            drain_server_output()
            return []
        if not endpoint:
        ...
        combined_stdout = "".join(server_stdout_lines) + client_stdout
        combined_stderr = "".join(server_stderr_lines) + client_stderr
        if "protocol not supported" in combined_stderr.lower():
            return [{"metric": "_unsupported_transport_", "value": 1.0}]

        parsed = []
        expected_sizes = {size}
        for line in combined_stdout.splitlines():
            parsed_line, _warning = parse_result_line(line, transport, expected_sizes)
            if not parsed_line:
                continue
            _line_transport, _line_size, metric, value = parsed_line
            if metric not in ("throughput", "bandwidth", "latency", "cpu_pct", "mem_mb"):
                continue
            parsed.append({"metric": metric, "value": value})

        token_status, _token_reason = detect_special_status(
            combined_stdout, lib_name, server_pattern, transport
        )
        if token_status == "unsupported":
            return [{"metric": "_unsupported_transport_", "value": 1.0}]
        if token_status == "skip":
            return []
...
def run_single_test(binary_name, lib_name, transport, size, pattern_name=""):
    if pattern_name in STREAM_SINGLE_VARIANT_PATTERNS:
        return run_single_stream_test_shared_client(
            lib_name, transport, size, pattern_name
        )
...
def main():
    ...
    comparisons = [
        ...
        ("comp_current_multi_stream_server", "MULTI_STREAM"),
        ("comp_current_multi_stream_callback_server", "MULTI_STREAM_CALLBACK"),
        ("comp_current_multi_stream_len32be_server", "MULTI_STREAM_LEN32BE"),
