...
One-way send·publish·reply가 operation family의 admission boundary에서 수락되는 결과를 실제 다중 process
환경에서 검증한다. Submit 완료는 원격 handler 실행 완료가 아니다. 일반 send family는 queue가 바로
...
[Spot messaging](../../spec/server/20-spot-messaging.ko.md)이 소유한다.

## 1. 목적과 범위

이 config는 다음 경계를 함께 검증한다.

- Public one-way call은 언어별 비동기 `submit` 하나로 local outbound admission 결과를 반환한다.
...
- Logical Multicast는 일반 send-ready retry와 executor 대기 queue를 사용하지 않는다. Bounded executor의
  `worker slot` 하나를 direct handoff로 얻으면 target snapshot을 한 번 확정하고 각 snapshot target에 최대 한
  번만 admission을 시도한다. Snapshot 처리가 시작되면 operation이 commit된 것으로 본다.
- `Submitted`는 family admission boundary가 operation을 수락했다는 뜻이며 원격 handler나 subscriber가
  실행됐다는 뜻이 아니다.
...
있지만, connector submit 결과를 Config 13의 성공 증거로 사용하지 않는다.
...
4. Pending capacity가 1인 상태에서 다음 검증 operation을 시작해 즉시 `Backpressured`인지 확인하거나,
   gate를 열어 기존 pending operation이 writable wakeup 뒤 한 번만 `Submitted`가 되는지 확인한다.
...
| evidence | 증가 시점 |
|---|---|
| `publicInvocationCount` | application이 public `submit` terminator를 호출할 때 증가한다 |
...
이미 가득 찬 상태에서 새 operation을 거부하면
유효한 call도 먼저 non-blocking transport 또는 local mailbox admission을 한 번 시도한다. 그 결과가
`EAGAIN` 또는 mailbox-full이고 waiter 공간도 없을 때 `Backpressured`이므로 `transportAttemptCount = 1`이다.
...
실패도 snapshot pass와 commit 없이 `Backpressured`로 끝난다. Commit 뒤에는 snapshot pass 하나를 끝까지
처리하고 최종 `PublishResult`로 완료한다.
...
대신하지 않는다. Fixture는
snapshot 내용과 pause·release 순서만 기록하며 member 선택, admission 결과나 반환 detail을 바꾸지 않는다.
...
| `deadlineAt`, `terminalAt`, `terminalStatus` | deadline과 terminal winner를 비교한다 |
...
| `snapshotPassCount`, `targetAttemptCount`, `commitAt`, `publishDetail` | Logical Multicast의 snapshot 단일 처리, member별 단일 admission과 commit 이후 결과를 확인한다 |
...
public result, 역할 server evidence와 사용한 gate 기록을 함께 대조한다. Timestamp는 순서 확인에만 쓰며
...
- **목적:** Logical Multicast 이외의 ready queue가 one-way call을 즉시 `Submitted`로 완료하며 Framework
...
- **기대 결과:** 각 call은 public invocation 1회, transport attempt 1회, commit 1회, scheduler enqueue 0회,
  terminal 1회의 `Submitted`다.
- **공통 evidence:** Public result, operation terminal marker, scheduler enqueue delta, target handler sequence를
...
- **절차:** 검증 operation이 pending임을 확인한 뒤 deadline 전에 gate를 한 번 연다. 같은 operation을 다시
  submit하지 않는다.
- **기대 결과:** Writable wakeup 뒤 검증 operation이 `Submitted`가 된다. Public invocation과 commit은 각각
...
- **공통 evidence:** Gate transition, pending marker, send-ready·capacity signal count, retry attempt count,
  poll·timer retry count, terminal status와 handler sequence를 시간 순서로 남긴다.

### SA-E2E-03 — pending admission capacity 초과
...
- **절차:** 두 번째 pending 후보를 시작하고 endpoint 응답과 operation evidence를 즉시 확인한다. Gate는 결과를
  확인할 때까지 열지 않는다.
- **기대 결과:** 두 번째 후보는 non-blocking transport 또는 local mailbox admission을 한 번 시도해
  `EAGAIN` 또는 mailbox-full을 확인한다. Waiter를 추가하지 않고 `Backpressured`로 한 번 완료되며 첫 pending
  operation의 상태는 바뀌지 않는다.
- **공통 evidence:** Pending waiter count 1, 두 operation의 서로 다른 ID, 두 번째 terminal time과
  `Backpressured`, public invocation 1회, transport attempt 1회, commit 0회를 남긴다.

### SA-E2E-04 — deadline 만료와 late admission 차단

- **목적:** Queue가 deadline까지 비지 않으면 `TimedOut`으로 끝나고 이후 writable event가 완료된 operation을
  다시 제출하지 않는지 확인한다.
...
- **절차:** 다음 하위 case를 실행한다.
  - `SA-E2E-04.a`: Gate를 닫은 채 deadline terminal을 기다린다. `TimedOut` 확인 뒤 gate를 열고 정상
...
- **기대 결과:** 만료 operation은 `TimedOut` 한 번이며 late admission과 handler delivery는 0이다. 새 operation만
  `Submitted`다. `0`과 `-1`은 늦어도 host startup에서 거부되고 기본값으로 바뀌지 않는다. `INT_MAX`는
...
- **목적:** Resolve할 대상이 없는 상태와 알려진 target에 ready route가 없는 상태를 다른 result로 반환하는지
  확인한다.
...
- **기대 결과:** 대상 부재는 `TargetNotFound`, 알려진 target의 미연결은 `RouteNotConnected`다. 둘 다 terminal
...
- **기대 결과:** 일반 send와 commit 전 multicast는 cancelled awaitable이며 late admission과 두 번째 completion이
  ...
  않고 snapshot pass 하나의 최종 `PublishResult`를 보존한다. Java·Kotlin의 commit 후 `cancel(false)`는
  `false`이며 shared stage가 Framework runtime의 최종
  `PublishResult`로 한 번 완료된다. .NET과 Node.js caller도 commit 뒤 최종 결과를 받는다. Kotlin coroutine이
  commit 뒤 취소되면 caller의 `await`는 cancellation으로 끝나지만 shared JVM stage와 underlying runtime
  operation은 최종 결과를 보존한다. 이를 coroutine 성공 결과로 기록하지 않는다. 세 terminal signal이
...
- **목적:** Node direct가 local target과 remote target에서 같은 admission result 계약을 사용하는지 확인한다.
...
- **기대 결과:** Route 위치와 관계없이 각각 `Submitted`, `Submitted`, `TimedOut`이며 원격 경로에만 application
  retry나 더 긴 timeout을 적용하지 않는다.
...
- **목적:** ChannelName send가 process-local index가 선택한 MeshNode의 ROUTER send timeout과 결과를 사용하는지
  확인한다.
...
- **공통 evidence:** ChannelName, selected MeshNode ID, deadline source, terminal status와 target별 handler count를
  기록한다.
...
- **기대 결과:** 첫 call은 `Submitted`, 둘째는 client DEALER deadline의 `TimedOut`이다. RouteMesh deadline이나
  server retry를 사용하지 않는다.
...
- **기대 결과:** Remote capacity drop이 하나 이상인 `SA-E2E-13.a`는 `Backpressured`이고 admitted와 dropped
  ...
  `SA-E2E-13.c`는 transport attempt·snapshot pass·commit 없이 `Backpressured`다. Snapshot count가 모두 0인
  `SA-E2E-13.d`는 유효한 local index entry를 유지하며 target admission attempt 없이 한 번의 snapshot pass로
  `TargetNotFound`다. Local Spot만 capacity drop된 `SA-E2E-13.e`는 `Submitted`이며 local dropped detail만
  증가한다. Commit된 operation은 snapshot pass를 정확히 한 번 끝내며 각 snapshot member에는 admission을
  최대 한 번 시도한다. Commit 뒤 signal은 결과를 cancelled 또는 `Shutdown`으로 바꾸지 않는다.
- **공통 evidence:** Case마다 public invocation, executor direct handoff, transport attempt, commit count,
  ...
  remote·local snapshot/admitted/dropped/unreachable detail도
...
- **절차:** Fanout message 하나를 submit하고 publisher의 local admission result를 확인한다. 이후 subscriber를
  시작해도 같은 message를 다시 publish하지 않는다.
- **기대 결과:** Subscriber 0이어도 local PUB queue가 수락하면 `Submitted`다. Late subscriber delivery와 replay는
  0이다.
- **공통 evidence:** Publisher terminal status, subscriber snapshot 0, transport attempt와 commit 각각 1회,
...
- **절차:** 다음 하위 case를 실행한다.
  ...
  - `SA-E2E-15.b`: Session Actor relay의 local·remote 경로에서 같은 순서로 `Submitted`, `Backpressured`,
    `TimedOut`을 만들고 각 언어의 async `SubmitResult`를 확인한다.
- **기대 결과:** 두 family의 local·remote 경로는 같은 deadline source와 async `SubmitResult` 계약을 사용한다.
...
- **기대 결과:** 수락된 send는 `Submitted`, 닫힌 case는 `TimedOut`이다. Peer wire sequence는 수락 순서와 같고
...
- **공통 evidence:** Session ID, socket deadline, operation sequence·terminal, peer wire capture sequence를 남긴다.
  Connector send result는 증거로 사용하지 않는다.
...
- **절차:** 다음 하위 case를 실행한다.
  ...
  - `SA-E2E-17.b`: 서로 다른 request token으로 pending capacity 초과의 `Backpressured`, gate가 닫힌
    `TimedOut`, cancellation 지원 언어의 cancelled awaitable을 각각 만든다. 각 terminal 뒤 같은 token을 다시
    사용하고 gate를 연다.
...
- **기대 결과:** 유효한 첫 terminator invocation은 transport attempt 전에 token을 claim하고 소비한다. 첫 정상
  reply만 `Submitted`이며 duplicate와 token 없는 send packet reply는 invalid-state exceptional completion이다.
  `Backpressured`, `TimedOut`, cancellation도 token을 소비하며 이후 같은 token의 invocation은 exceptional
...
- **Topology·사전 조건:** Receiver gate와 connection gate로 pending operation을 `TimedOut` 또는 `Shutdown`까지
  유지한다.
...
- **기대 결과:** Submit은 `Submitted`지만 handler completion은 gate-open 뒤에만 기록된다. Submit result에 remote
  handler 반환값이나 실행 성공을 넣지 않는다.
- **공통 evidence:** Terminal timestamp가 handler release·completion보다 앞서는지, operation과 handler terminal
  count가 각각 1인지 기록한다.

## 6. 회귀 시나리오

### SA-REG-01 — 제거된 public TrySubmit 잔여 검사

- **목적:** 다섯 언어의 public declaration·guide·sample·consumer에 제거한 동기 `TrySubmit` 계열과
  `SubmitResult`를 버리는 session Actor relay가 남지 않는지 확인한다.
...
- **절차:** 언어별 exported declaration과 source allowlist를 검사하고 compile-negative fixture에서 제거된 호출이
  compile되지 않는지 확인한다.
- **기대 결과:** Public `TrySubmit`, `trySubmit`, `try_submit`과 결과를 버리는 one-way `void` wrapper가 0개다.
  Session Actor relay도 모든 언어에서 async `SubmitResult`를 반환한다.
...
### SA-REG-03 — Kotlin admission 결과 보존

- **목적:** Kotlin convenience wrapper가 Java runtime의 admission 결과를 `Unit`으로 버리지 않는지 확인한다.
- **Topology·사전 조건:** Kotlin compile fixture와 JVM Config 13 역할 server를 사용한다. Receiver gate로
  `Submitted`, `Backpressured`, `TimedOut` case를 만든다.
- **절차:** `SA-REG-03.a`는 coroutine에서 call의 `submit().await()` 결과를 변수에 받아 status별 branch를
  실행한다. 결과를 버리는 동일 이름 extension과 `Unit` 반환 wrapper가 compile surface에 없는지 확인한다.
  `SA-REG-03.b`는 Logical Multicast commit marker 뒤 coroutine을 취소하고 shared JVM stage가 끝날 때까지 runtime
  evidence만 별도로 수집한다.
- **기대 결과:** 세 status가 Kotlin caller까지 유지되고 cancellation은 pending admission cleanup과 연결된다.
  Commit 뒤 취소된 coroutine은 cancellation 상태를 유지하며 성공값을 반환하지 않는다. Shared JVM stage와
  underlying runtime operation은 취소 상태로 바뀌지 않고 최종 `PublishResult`로 한 번 완료된다.
...
- **공통 evidence:** Dispose 횟수, terminal status·count, resource counter baseline·final, process exit code와
...
| Kotlin | `framework/languages/java/e2e-kotlin/SubmitAdmission/feature-map.ko.md` | JVM runtime runner를 재사용하면 feature map에 해당 경로를 명시 | Coroutine result·cancellation compile fixture |
...
- Logical Multicast는 direct handoff 실패 시 snapshot pass와 commit이 0이고, direct handoff에 성공한
  ...
  commit 뒤에는 최종 `PublishResult`로 한 번만 완료된다.
