...
const { MeshPump, collectActorPayloads, destroyMeshActor, frame, isActorJoinRequest, leaveActorFromSpot, reservePort, tcpEndpoint } = require('./sample_support');
async function main() {
    ...
    try {
        ...
        const joinOperation = node.joinActorSpot(actor, nodeRid, spot.routingId, spot.status().lifecycleGeneration, Buffer.from('enter-room'), 2000);
