...
inline bool provider_evidence_prefix_exists (const std::string &prefix)
{
...
inline void wait_provider_evidence_prefix (const std::string &prefix)
{
    const auto deadline = std::chrono::steady_clock::now () + std::chrono::seconds (15);
    while (std::chrono::steady_clock::now () < deadline) {
        if (provider_evidence_prefix_exists (prefix)) {
            return;
        }
...
inline void run_rl_c1_client_host_lifecycle_probe (zlink::framework::channel_client_t &channels)
{
    ...
    wait_provider_evidence_prefix ("rl-c1-");
    wait_provider_evidence_prefix ("rl-c1-after-cleanup");
