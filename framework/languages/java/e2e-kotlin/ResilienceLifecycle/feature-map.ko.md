...
Redis location store, public location runtime query를 사용하고, Provider role은 `ZLinkChannelRuntimeOptions`와
...
옮겼다. Shared message type도 Kotlin source다. registry role은 Redis location store 전환 뒤 제거했다.
`logs/20260704-042125-24694` runner에서 `RL-A1`, `RL-A2`, `RL-A3`, `RL-A5`, `RL-B1`, `RL-B3`,
`RL-B2`, `RL-B4`, `RL-B5`, `RL-B6`, `RL-C1`, `RL-C2`, `RL-C3`, `RL-C4`, `RL-D1`, `RL-D2`, `RL-D3`, `RL-D4`, `RL-D5` marker와 각 client mode의
`resilience-lifecycle kotlin e2e result=passed`를 확인했다. `RL-A4`는
`logs/20260707-215053-3533357` runner에서 별도로 실행해 같은 routing id provider를 drain, 종료,
green endpoint 기동, green traffic 검증, 원래 endpoint 복구 순서로 확인했다. `RL-D2`는
`logs/20260707-235045-4053936` focused runner에서 observer 예외가 runtime error event로 보고되고
후속 request가 정상 처리되는지 다시 확인했다.
...
- `RL-A2`: provider-a를 같은 routing id의 다른 endpoint로 재기동하고, 같은 client 프로세스가 location peer row의 endpoint 갱신과 follow-up request 성공을 확인한다.
...
- `RL-B3`: provider 정상 종료 뒤 location peer row에서 빠지고 같은 client의 후속 request가 남은 provider로만 가는지 확인한다.
...
- `RL-C2`: runner가 provider-b를 `SIGKILL`해 owner lease가 갱신되지 않는 stale row를 만들고 consumer를 새 discovery host로 재시작한다. public location runtime query에서 provider-b가 live topology에서 빠지는지, 이후 request가 provider-a로만 가는지, provider-b 재시작 뒤 다시 traffic을 받는지 확인한다.
...
- `RL-D2`: provider dispatch-error observer가 예외를 던지는 mode에서도 missing-handler 오류 뒤 후속 request가 정상 처리되는지 확인한다. observer 예외는 Java/Kotlin public monitoring event handler가 받는 `RuntimeError` evidence로도 확인한다.
...
- `RL-D4`: 미등록 request handler가 public HTTP 실패로 노출되고, provider dispatch-error evidence에 handler missing과 reply error marker가 남으며, 이후 request가 정상 동작하는지 확인한다. 공통 문서의 code round-trip 검증은 server-side dispatch evidence 경로로 닫는다.
