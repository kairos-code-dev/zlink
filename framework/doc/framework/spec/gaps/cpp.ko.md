...
- [x] **IMP-CP-02** (결함) — 31
  - 근거: 수정 전 집중 계약 게이트가 binding token 회귀 부재를 검출했다(`cdbbf1b3b`). scoped session manager가 `sessionId + bindingToken`을 만들고 stream host가 disconnect 때 같은 token만 정리하도록 바꿨다(`aebe65c26`). 이전 session 정리 뒤 새 session push가 유지되고 현재 session 정리 뒤에는 `actor_session_not_bound`가 반환되는 단위 회귀, module-hosted·sample-parity 시험과 SpotService SM-D1·D2·D5·D8 실제 runner가 통과했다. POSD 재검토에서는 lifecycle 순서와 codec 선택을 application에서 제거하고 하나의 내부 binding context로 모았다.
- [ ] **IMP-CP-03** (결함) — 24 §3
  - 정식 C++ interface는 공통 계약과 같은 `spot_handle_t` 기반 `send_to_spot`·`request_to_spot`으로 확정했다. Source의 `spot_context_t::send_to/request_to(node rid, spot rid, ...)`를 제거하고 handle 기반 outbound로 교체하는 구현과 contract test가 남아 있다. handle registry·location event/주기 재조회·stale 1회 refresh는 이미 구현돼 있으므로 별도 DI 우회는 만들지 않는다.
...
- [ ] **IMP-CP-11** (미구현) — 31
- [ ] **IMP-CP-41 / §12.27** (미구현) — `actor_location_t`과 Redis codec에 현재
  `spot_generation`이 없다. 같은 Spot RID의 재사용 뒤 stale membership을 거부하도록 record, lifecycle,
  in-memory·Redis round-trip과 stale 판정을 함께 구현해야 한다.
...
- [x] **§12.21** (해소) — request·actor join·worker의 `async()`는 turn을 유지하고 `yield()`만
  turn을 반납한다. `call.hpp`, `worker.hpp`와 `task.hpp`의 serial turn scheduler가 두 완료 의미를
  분리한다.
- [ ] **§12.22** (결함+미구현) — HTTP client가 framework 계약 밖에 있다
- [ ] **§12.23** (미구현) — CPU/I/O worker 분리와 `async`·`yield`는 구현됐지만 callback이 exact
  `std::stop_token`을 받지 않아 timeout·shutdown cancellation을 실행 중인 작업에 전달하지 못한다.
- [ ] **§12.24** (결함) — `spot_runtime.hpp:805-867`이 source `commit_actor_left`를 먼저 실행하고
  target callback과 route를 공개한다. location CAS를 먼저 commit한 뒤 source leave와 target
  membership·`OnJoinedActor`를 순서대로 실행하고 TD-E2·E3로 검증해야 한다.
- [ ] **§12.27** (미구현) — Actor location의 Spot generation이 없다
- [ ] **IMP-CP-42 / §12.28** (미구현) — `stream_node_options_builder_t`에 exact
  `enable_actor_dispatch(mesh_name)`이 없고 registration도 relay용 MeshName을 보존하지 않는다. startup
  검증과 mesh별 session Actor dispatch 격리를 함께 구현해야 한다.
- [ ] **IMP-CP-43 / §12.29** (미구현) — exact `actor_transfer_store_t`와 공식 Redis
  prepare·commit·abort·takeover 구현이 없다. process-local coordinator 상태를 durable participant set,
  active transfer index와 recovery lease 원자 전이로 연결해야 한다.
- [ ] **IMP-CP-44 / §12.30** (미구현) — C++ STREAM TLS builder가
  `require_client_certificate`를 받지 않고 Core TLS option에도 전달하지 않는다. Node options builder에
  bool을 추가하고 계약 밖 low-level `stream_builder_t::set_tls_server`는 제거한 뒤 기본 false와 인증서
  없는 client 거부를 실제 TLS 연결로 검증해야 한다.
- [ ] **IMP-CP-45 / §12.31** (결함) — Actor transfer counter와 duration을 label 없이 성공 경로에서만
  기록한다. `mesh_name`과 닫힌 terminal outcome을 activation·실패마다 정확히 한 번 기록해야 한다.
- [ ] **IMP-CP-46 / §12.32** (결함) — envelope body를 content-type 검증 없이 raw message로 넘기고
  handler 타입 serializer로 decode한다. 알 수 없는 non-JSON 값은 handler 호출 전에
  `payload_decode_failed`로 종료해야 한다.
- [ ] **IMP-CP-47 / §12.33** (미구현) — `add_route_mesh(channel_name)`이
  `route_mesh_channel_builder_t`를 반환하고 client-server channel·SpotMesh 등록이 분리돼 있다. exact
  `mesh_node_builder_t add_route_mesh(mesh_name)`과 통합 MeshNode 구성으로 source·package·sample·E2E를
  옮겨야 한다.
- [ ] **IMP-CP-48 / §12.34** (결함) — `actor_ref_t`가 공통 세 필드 외에 `actor_type`을 공개하고
  `actor_ref_snapshot_t::to_actor_ref(actor_type)` 호출자가 snapshot에 없는 값을 다시 주입해야 한다.
  Actor type은 runtime registry가 소유하도록 옮기고 참조와 snapshot은 `node_rid`·`actor_id`·`generation`
  세 값만 보존해야 한다.
- [ ] **IMP-CP-49 / Stream Connector §9** (결함) — 정식 `error_code_t`는 공통 오류 표의 닫힌 16개
  값만 선언하지만 source enum에는 계약 밖 `unsupported_codec`·`closed`·`canceled`가 남아 있다. 지원하지
  않는 codec은 공통 `configuration_error`, 연결 종료는 `disconnected`, 호출 취소는 호출 표면의 cancellation
  결과로 매핑하고 세 값을 public enum에서 제거해야 한다.
...
| **IMP-CP-02** | 결함 | [31](../server/31-session-actor-dispatch.ko.md): unbind는 **같은 `sessionId + bindingToken`인 entry만** 제거한다. **이전 stream의 늦은 정리가 새 binding token을 지우면 안 된다** | **해결.** stream host가 scoped `session_actor_manager_t`에 현재 stream·codec을 연결하고 manager가 actor별 token을 발급한다. disconnect는 그 session context가 보유한 token만 조건부 제거하므로 이전 stream의 늦은 정리가 새 sink를 지우지 못한다. |
| **IMP-CP-03** | 결함 | [24 §3](../server/24-spot-address-messaging.ko.md): snapshot 갱신 트리거는 셋(**location event · 주기 재조회 · stale 1회**). **spot rid와 node rid를 나란히 받는 전송 overload는 없다** | handle registry와 세 갱신 트리거는 구현돼 있다. 그러나 source의 spot-context outbound는 아직 `request_to(node_rid, spot_rid, …)`를 받는다. 정식 C++ interface에 확정한 handle 기반 `send_to_spot`·`request_to_spot`으로 교체하고 기존 overload를 제거해야 한다. |
...
- [x] **IMP-CP-19** (미구현) — **connector에 자동 재연결이 없다.** 수립된 연결이 끊기면 `Disconnected`에 계속 머문다
...
- [x] **IMP-CP-20** (결함) — connector `connect()`가 **현재 상태를 무시**한다(`Closed`에서도 다시 연결되고, `Connected`면 소켓이 하나 더 열린다)
  - 근거: 상태별 connect 정책을 한곳에 두어 `Connected` 재호출은 성공 no-op, `Connecting`·`Reconnecting`은 거부, `Closed`에서는 다시 연결하지 않도록 고쳤다(`0c0ed5b79`). sync·callback connect 상태 회귀와 connector 전체 테스트가 통과했다.
...
| **IMP-CP-13** | [50 §2](../server/50-runtime-monitoring.ko.md): 주기적으로 상태를 읽고 **직전 상태와 비교해 바뀐 때만** event를 합성한다 | `monitoring_runtime.cpp:277-308` — 직전 스냅샷 상태가 **아예 없고** 매 tick `status_changed` + `topology_changed` + `service_summary_changed`를 **무조건** 발행한다. ⇒ 유휴 클러스터에서도 **초당 3개**가 계속 발행되고, `TopologyChanged`에 등록한 앱 handler(재dial·캐시 재구축)가 **1초마다 같은 데이터로** 실행된다 |
...
| **IMP-CP-19** | [32 §6](../stream-connector/32-stream-connector.ko.md): **자동 reconnect는 기본으로 켜져 있다.** `Reconnecting` = 자동 재연결 진행 중 | `connector_runtime.cpp:756-828` — `options.reconnect`를 읽는 **유일한 곳**이 `connect()` 안의 재시도 루프다. 수립된 연결이 끊기면(`zlink_stream_calls.cpp:854-872`, `:1439-1448`) `disconnected`로 바꾸고 pending을 실패시킬 뿐 **다시 dial하지 않는다.** ⇒ 서버가 재시작하면 .NET/Java/TS는 250ms 만에 다시 연결하지만 **Unreal/Godot 클라이언트만 연결이 복구되지 않는다.** C++의 `Reconnecting`은 **"첫 연결을 재시도 중"이라는 뜻일 뿐**이다 |
| **IMP-CP-20** | [32 §6](../stream-connector/32-stream-connector.ko.md): `Closed` → **오류로 실패**. `Connected` → 즉시 성공 반환. `Connecting` → 진행 중인 시도를 기다린다 | `connector_runtime.cpp:745-751` — `state->state`를 **읽지 않고** 무조건 dial 루프로 들어간다. ⇒ `close()` 뒤 `connect()`가 **닫힌 connector에 다시 연결하고**(스펙 금지), `Connected` 상태의 `connect()`는 **소켓을 하나 더 열어** 이전 소켓의 read pump가 유지된 채로 누수시키고 서버엔 **중복 세션**이 보인다 |
...
| **IMP-CP-22** | [32 §6](../stream-connector/32-stream-connector.ko.md): 켜져 있으면 주기마다 control ping을 보내고, timeout 동안 inbound frame이 없으면 끊긴 것으로 처리한다 | `zlink_stream_calls.cpp:1437-1454` — heartbeat 검사와 발송이 **`dispatch()`에서만** 불린다. ⇒ `Immediate` 모드 앱은 `dispatch()`를 안 부르므로 **ping을 하나도 보내지 않고 응답하지 않는 peer를 감지하지 못한다.** half-open TCP에서 `Connected`로 남고 pending은 30초 request timeout에서야 실패한다 |
...
- [x] **IMP-CP-29** (결함) — `unhandled_dispatch_options_t` 5개 필드가 **검증만 되고 읽히지 않는다**
  - 근거: §13.4 D6와 공통 framework API·channel messaging 계약을 다시 대조해 action과 log level은 호출자가 정하는 option이 아니라 framework가 고정하는 정책임을 확인했다. 수정 전 target gate는 validation-only 공개 enum·options·member를 2건으로 검출해 실패했다(`b40f78a6b`). 다섯 필드와 enum, no-op validation 및 거짓 양성 시험을 제거했다(`0cc3a81b2`). target·header·module-hosted와 channel messaging·message-flow 시험이 모두 통과했다.
  - POSD/DDD: 다섯 동작을 runtime까지 전파하는 안과 거짓 설정 표면을 제거하는 안을 비교했다. 전자는 공통 고정 정책을 호출자에게 누출하고 모든 dispatch 경로에 조건을 퍼뜨리므로 기각했다. 제거로 validation-only 얕은 모듈과 pass-through 상태를 없앴으며 domain·wire 경계 변화는 없다.
...
- [x] **IMP-CP-35** (결함) — framework runtime에 **owner-lease join이 아예 없다.** store에 떠넘겼다
  - 근거: 수정 전 target-contract gate가 Redis 내부 lease 필터, framework live-row join 부재, auto-connect의 raw store 우회를 각각 검출했다. Redis와 in-memory store의 read는 만료 owner row도 raw row로 반환하도록 바꾸고, framework 내부 `live_location_reader_t`가 store 시간 기준 lease snapshot과 monotonic 경과 시간을 한 곳에서 결합해 peer·spot·actor·route read를 필터링한다. resolver 회귀 테스트가 raw row 2개와 live row 1개를 분리해 확인했고 Config 6 SF-C1·C2·D2 및 전체 runner가 통과했다.
...
| ID | 계약 | 구현이 하는 일 |
|----|------|----------------|
| **IMP-CP-34** | [04](../04-async-execution-policy.ko.md): 한 spot의 두 callback은 동시에 실행되지 않는다. [21 §close](../server/21-mesh-node.ko.md) | `callback_depth`·`callback_thread`·`close_requested`는 **`callback_mutex`**가 지킨다(`spot_runtime.hpp:253-261`). 그런데 `close_erased()`(`spot_runtime.cpp:994-1005`)는 **`node->mutex`만 잡고** 그 둘을 읽고 쓴다. 결과가 두 갈래다 — ① **callback 실행 중 close가 실행된다**: 낡은 `callback_depth == 0`을 보고 `close_now()`로 직행해 다른 스레드가 handler를 실행 중인 spot의 컨텍스트를 해제한다. **한 spot에서 두 callback이 동시에 실행되는 것** — 직렬 실행 규칙이 막아야 하는 상태다. ② **close가 조용히 유실된다**: `leave_callback`이 먼저 `close_requested == false`를 읽고 나가면, 뒤늦게 A가 낡은 `callback_depth == 1`을 보고 `close_requested = true`를 쓴다. **`close_spot()`은 성공을 반환했지만 방이 종료되지 않는다** |
...
| **IMP-CP-33** | — | `contracts/dispatch/execution.hpp:74,96,254-258`이 전부. 관련 `include_message_sizes`는 사용되지만 이 옵션만 적용 경로가 없다 |
| **IMP-CP-35** | [40 §1·§5.1](../server/40-location-runtime.ko.md): **store는 저장만 하고, owner lease join·generation guard는 framework runtime의 책임**이다. 성공 결과에 stale row가 섞이면 안 된다 | **해결.** Redis와 in-memory store는 raw row만 반환한다. framework의 read-only `live_location_reader_t`가 owner lease snapshot을 polling interval 동안 공유하고, store 기준 남은 lease 시간을 monotonic 시각으로 환산해 모든 내부 peer·spot·actor·route read에 같은 live-owner join을 적용한다. auto-connect, Spot mesh, resolver, runtime query, actor directory와 actor client가 이 경계를 사용한다. |
...
- [x] **IMP-CP-39** (결함) — **`actor_gateway_t`** — C++ 카탈로그가 **스스로 "공개 계약이 아니다"라고 이름까지 적어 명시한 타입**인데, 샘플·e2e **18개 파일**이 DI로 주입받아 쓴다
  - 근거: 수정 전 집중 계약 게이트가 public header와 DI 등록의 `actor_gateway_t`를 검출했다(`cdbbf1b3b`). public 타입·DI factory·application 의존성을 제거하고 session actor 표면만 남겼다(`aebe65c26`). 집중 계약 게이트, 관련 target 15개, 단위 시험 3개와 Bingo·TicTacToe·DeliveryDispatch·SupportChat·GameQuest 실제 runner가 통과했다.
- [x] **IMP-CP-40** (결함) — **`bind_session_route(...)`** — 스펙이 **이름을 짚어 금지한 세 값**(route client·route channel 이름·target node rid)을 application handler에 넘긴다
  - 근거: 수정 전 집중 계약 게이트가 application의 수동 route 바인딩을 검출했다(`cdbbf1b3b`). SpotService handler의 route client·channel·target node 분기와 요청용 제어 필드를 제거하고 framework registrar가 원격 bound-session route를 만들게 했다(`aebe65c26`). SM-D2 원격 actor push가 통과했고(`logs/20260716-103251-3493210`), SM-D1·D5·D8도 통과했다. POSD/DDD 재검토에서는 transport 위치 결정을 runtime 내부로 되돌리고 session/application의 도메인 책임만 남겼다.

| ID | 계약 | 구현이 하는 일 |
|----|------|----------------|
| **IMP-CP-39** | [C++ 카탈로그 §16.24](../server/languages/cpp/02-framework-interfaces.ko.md): `actor_gateway_t`는 `src/runtime/`의 **runtime 내부 타입**이며 "**application은 이 타입을 직접 다루지 않는다**"고 **목록에 이름까지 넣어** 명시한다. 스펙이 정한 bind 표면은 `session_actor_manager_t::bind(actor_ref_t)` **인자 1개**다(`:886`, 스펙 31의 `IZLinkSessionActors.BindAsync(...)`와 대응) | **해결.** public header와 service provider에서 `actor_gateway_t`를 제거했다. application session은 `session_actor_manager_t`로 actor ref만 바인드하고 stream·codec·unbind는 stream host와 내부 runtime이 처리한다. |
| **IMP-CP-40** | [C++ 카탈로그 §1516-1518](../server/languages/cpp/02-framework-interfaces.ko.md): "ActorGateway session relay는 `session_actor_manager_t`·`session_actor_t`·`actor_context_t`·`bound_session_t`가 담당한다. **이 표면은 route mesh channel을 직접 보여 주지 않는다.**" [31 §375-377](../server/31-session-actor-dispatch.ko.md): "resolver가 반환하는 target node 식별값(`RoutingId`류)은 **사용자가 일반 handler에서 직접 다루는 값이 아니다.** transport 위치값은 resolver 구현체와 framework routed transport **내부에만** 머물러야 한다" | **해결.** application handler의 `bind_session_route(...)` 호출과 route 좌표 분기를 제거했다. remote actor의 bound-session route 등록은 `actor_gateway_spot_bridge`와 내부 route dispatcher가 담당한다. |
...
- [x] **SMP-CP-02** (결함) — Bingo에서 **2번째 참가자가 `GameStartedNotify`를 받지 못한다**
...
- [x] **SMP-CP-24** (결함) — **SupportChat Application 레이어가 serving 경로에서 dead code**다(위조 self-check에서만 쓰인다)
  - 근거: 수정 전 집중 gate `7c8f498d4`가 serving runtime의 `_available_agent` 중복 상태와 `agent_assignment_service_t` 미사용을 5건으로 검출했다. `d1dc8b6ce`에서 .NET 정본 결정과 같이 Application service가 conversation별 reservation·capacity·fairness·재등록 load 복원·close 해제를 소유하고 serving handler가 이 use case를 호출하도록 바꿨다. 집중 gate와 sample parity 52건, 실제 SupportChat runner가 통과했다. POSD/DDD 재검토에서는 optional 상태를 남긴 얕은 wrapper 대안을 배제하고 배정 지식을 Application 한 곳에 모았다.
...
- [x] **SMP-CP-40** (**wire 파손**) — **TicTacToe `GameState`의 nullable 5개를 C++가 sentinel로 뭉갠다.** 양방향으로 깨진다
  - 근거: 수정 전 집중 wire gate가 `null` 역직렬화 예외, 빈 문자열·`-1` 방출, 추가 `draw` 필드를 모두 검출했다. 다섯 필드를 `std::optional`로 바꾸고 공통 nullable JSON 경계에서 읽고 쓰며 `draw`를 제거했다(`b6526de45`). parity·spot runtime·ActorGateway gate와 전체 runner(`PASS TicTacToe.Cpp`)가 통과했다.
- [x] **SMP-CP-41** (결함) — **SupportChat `JoinConversationReq`가 C++에선 빈 구조체**다. 문서가 요구한 참가자 신원을 **보낼 수단이 없다**
  - 근거: 수정 전 sample-parity gate가 Support 서버의 participant identity join payload 부재로 실패했다. DTO에 `participantId`·`role`·`displayName`을 추가하고, client의 빈 값 요청은 유지하되 Support 서버가 conversation actor join에서 실제 profile을 채우며 owner Spot이 세 값과 actor profile의 일치를 검증하도록 수정했다(`7ec26537b`). parity와 SupportChat 전체 runner가 통과했다.
...
- [x] **SMP-CP-43** (결함) — **ShoppingMall이 금액을 `double`로 쓴다.** 문서·`.NET`은 `decimal`이다
  - 근거: .NET의 `decimal`·`decimal?`와 JSON number 결정을 따라 C++ 금액을 검증·정규화하는 `decimal_t` 값 객체로 바꾸고 `OrderState.Amount`는 nullable로 만들었다(`2701fe3f9`). 수정 전 집중 gate가 `double` 필드, `null` decode 예외, 기본 `0.0` 방출을 검출했고, 수정 뒤 decimal 왕복·전체 parity·관련 타깃 빌드와 ShoppingMall runner(`PASS ShoppingMall.Cpp`)가 통과했다.
...
- [x] **SMP-CP-46** (미구현) — **TicTacToe `TicTacToeGameJoinRes`가 C++에 없다**
  - 근거: 수정 전 sample-parity gate가 내부 room-join 응답 타입과 해당 typed decode 부재를 두 단언으로 검출해 실패했다. `tictactoe_game_join_res_t`를 외부 `join_game_res_t`와 분리하고 Domain admission은 내부 타입을, entry Spot 경계는 외부 타입 매핑을 소유하도록 수정했다(`ecd7c9fc5`). parity·spot runtime·ActorGateway 시험과 TicTacToe 전체 runner가 통과했다.
...
- [x] **SMP-CP-59** (결함) — **TicTacToe entry spot이 disconnect 시 milestone observer를 정리하지 않는다.** 종료한 세션에 계속 push한다
...
- [x] **E2E-CP-02** (결함) — **Config 9 Track A(P0 전부)가 이름만 그렇다.** session gateway 역할도 stream connector도 없다
  - 근거: 수정 전 집중 target-contract gate가 session 역할, 두 gateway runner 배치, connector bind/push가 모두 없어서 실패했다(`c0cf60600`). 실제 `Session` 역할이 `packet_stream_session_t`에서 actor directory 조회, session bind·disconnect와 역할 evidence를 소유하게 하고, runner가 `session-a`·`session-b`를 별도 설정과 endpoint로 시작하도록 구성했다(`3d2afc496`). TA-A1은 bind 전후 push와 동일 gateway bind 1건, A2는 두 gateway의 bind 부재, A3는 no-bind send/request 뒤 `session-b` bind와 push, A4는 connector disconnect 뒤 actor 생존과 명시적 destroy를 검증한다. 네 target 빌드, 집중 gate의 E2E-CP-02 no-hit, 전체 Config 9 runner와 reverse start-order Track A 실행(`logs/20260716-084532-3140237`)이 통과했다. POSD/DDD 재검토에서는 caller가 session 상태를 흉내 내는 안을 버리고 실제 session 역할에 bind 지식과 evidence를 모았으며, connector 설정은 client support가 흡수해 시나리오에는 사용자 흐름만 남겼다. 새 public API나 wire 의미 변경은 없다. B2/B3의 row 위조와 실제 owner·route lifecycle gap은 E2E-CP-59에 유지한다.
...
- [x] **E2E-CP-08** (결함) — **§2.6 설정 정책을 `ToActorMessaging` 빼고 전 config가 위반**하고, feature-map에 기록한 곳이 0개다
  - 근거: 수정 전 환경 접근 게이트가 C++ sample/E2E의 `getenv`·`load_env`와 runner의 `os.environ`·`ZLINK_*` 설정 전달을 검출했다. 11개 config를 role별 JSON + `load_json()` + `bind_required<T>()`로 통일하고 standalone client도 한 번만 읽는 typed 설정 경계로 옮겼다(`284532847`, `e7cc4c436`, `750794354`, `293efed3b`, `fda18ef28`, `9928969b2`, `8caf88da8`, `9a832e80a`, `68df13ea1`, `a064621d9`, `32c7a4b3d`). 정적 게이트가 0건이고 여섯 sample runner와 SpotService 전체 sweep이 통과했다.
  - 재검증: 기존 게이트가 sample별 prefix 환경 기본값을 놓쳐 Bingo·GameQuest·ShoppingMall·TicTacToe runner에 port·endpoint·Redis prefix·run directory·startup sleep 우회 36개가 남아 있었다. prefix 네 종류까지 실패 게이트를 넓힌 뒤 환경 fallback과 sleep을 제거하고, runner가 실행별 값을 생성해 기존 role별 `0600` JSON에만 기록하도록 정리했다(`29a69675f`). layout의 설정 위반은 0건이며 네 sample 전체 runner가 다시 통과했다.
  - 추가 재검증: 전체 정책을 다시 대조한 실패 게이트가 RuntimeMonitoring client의 로그 경로·여러 topology CLI와 통합 runner의 `E2E_START_ORDER` 환경 전달을 검출했다(`03807f03c`). client 입력을 mode `0600`의 단일 typed JSON으로 모으고 start-order는 자식 runner의 `--start-order` option으로 전달했다(`920199511`). 모든 `--config` runner의 0600 생성과 종료 정리를 일반 회귀로 고정했다(`4f6260dff`). layout·target gate, `MON-B2`, 통합 runner의 `TA-A1` forward·reverse·고정 seed shuffle이 통과했다.
  - 추가 재검증 2: 다섯 E2E가 `configure_locations()`의 반환값을 복사해 polling·heartbeat·lease 설정을 수정한 뒤 버리는 거짓 설정을 실제 OBS-C1 lease 정체로 재현했다. target gate가 PubSub·ObservabilityOps·ResilienceLifecycle·RuntimeMonitoring·SpotService를 5건으로 검출했다(`4335ad7ad`). 모두 기존 options 객체를 참조하도록 고쳤고(`1622f4208`, `920e51a0e`), target gate와 `PS-A1`·`RL-A1`·`MON-A1`·`SM-A1`·`OBS-C1 drain`이 통과했다.
  - POSD/DDD: handler와 shell에 퍼진 설정 지식은 role별 typed 설정으로 모으고, 여섯 sample runner의 shell→Python 환경 전달은 명시적 인자로 바꿨다. 재검증에서는 네 runner의 환경 fallback 분기와 startup sleep을 제거해 실행 설정 경로를 role별 JSON 하나로 줄였다. RuntimeMonitoring client도 개별 CLI 여덟 개 대신 설정 파일 경계 하나만 알도록 바꿨다. F 시나리오의 location 준비 판단은 `ensure_f_spot()` 한 곳으로 합쳐 호출부의 lifecycle 지식을 줄였으며, 새 public API나 domain 경계 변경은 없다.
...
- [x] **E2E-CP-10** (결함) — **§2.5 시나리오 파일 분리 위반** — 4개 config가 단일 `main.cpp`
  - 근거: 수정 전 집중 계약 테스트가 ToActorMessaging 7개, SpotActorTransfer 20개, DiscoveryRegistryHa 10개, ObservabilityOps 13개 시나리오 파일의 부재로 실패했다(`dde9521ff`). 각 ID의 실제 client 흐름과 단언을 `Client/Scenarios/`의 독립 파일로 옮기고 공통 설정·evidence 대기는 `Client/Support/`에 모았으며(`e63efd7ae`, `90989c89c`; 타 범위 staged 변경은 `9cd831d14`로 순변경 0 복구), 갱신된 공통 E2E의 시나리오 ID와 §2.5 계약을 다시 대조했다. 네 client target 빌드와 `test_cpp_framework_layout_contract`가 통과했고, 앞선 각 config 전체 runner도 모두 통과했다. POSD/DDD 재검토에서는 `main.cpp`를 설정·dispatch에 한정하고 시나리오 간 pass-through map과 transport 세부 중복을 제거했으며 새 public API나 wire 변경은 없다.
...
- [x] **E2E-CP-14** (미구현) — **§3.1 "route mesh 없음 × 분리 배치" 조합이 아예 만들어지지 않는다**
  - 근거: 수정 전 집중 계약 게이트가 `SM-D2`의 route mesh 비활성화와 Play·Session의 조건부 등록 부재로 실패했다(`21b5120e5`). `342b047e6`에서 같은 `SM-D2` client와 분리된 Play·Session 역할을 유지한 채 route mesh를 제거하고, 전달받은 전체 actor snapshot을 직접 bind하며 remote session 등록·actor relay를 SpotMesh로 처리했다. location actor row가 아직 보이지 않는 첫 요청도 재시도 없이 actor owner node의 내부 entry 주소로 전달한다. actor gateway unit, target gate의 E2E-CP-14 단언, shell 문법 검사가 통과했고 forward·reverse·`shuffle:20260709` 실제 runner가 모두 통과했다(`logs/20260716-111123-3627029`, `logs/20260716-111548-3634712`, `logs/20260716-111550-3635089`). POSD/DDD 재검토에서는 별도 no-route 역할 복제를 피하고 기존 역할의 배치 옵션만 바꾸었으며, 중복 actor 생성과 두 fallback 분기의 node-entry 결정 지식을 제거했다. 새 public API와 wire 변경은 없다.
...
- [x] **E2E-CP-17** (결함) — **`SM-F5`가 자기 계약의 정반대를 단언한다.** Spot을 닫지 않고 유지 상태를 확인한다
  - 근거: 수정 전 target-contract 검증이 target Spot close, 닫힌 경로 실패, close 이후 일반 channel request 재검증의 부재를 모두 검출했다. actor가 없는 전용 user Spot을 public create 표면으로 만든 뒤 소유 node의 public close 표면으로 닫고, 해당 Spot 경로만 실패하며 같은 channel의 일반 request는 계속 성공하는지 client와 server evidence에서 확인했다. target-contract 검증과 `./run_e2e.sh SM-F5`가 통과했다.
...
- [x] **E2E-CP-19** (결함) — **`SM-F4`(P0)가 request 절반만 본다.** send drop·failure counter·flow 분류가 없다
  - 근거: 수정 전 target-contract gate가 source Spot의 request/send, failure counter 증가, 두 message-flow 분류의 부재를 각각 검출했다. source Spot이 닫힌 원격 target RoutingId로 `DirectSpotReq`와 `DirectSpotMsg`를 보내도록 바꾸고, request의 `request_target_not_found`를 `reply_error`로 보고하며 route bridge의 one-way delivery 거절을 `drop`으로 보고하도록 연결했다(`4dacf9b6e`). client는 두 node의 observer evidence 개수가 2 이상 증가하는지와 정확한 packet·action을 확인하고 정상 route도 다시 검증한다. channel unit, target-contract의 E2E-CP-19 단언, `./run_e2e.sh SM-F4`, `SM-F5`가 통과했다(남은 target-contract 실패 3건은 E2E-CP-56·15·45).
  - POSD/DDD: 처음 검토한 E2E 전용 retained-handle helper는 얕은 우회라 제거하고 기존 source Spot의 공개 `request_to`·`send_to` 경로를 사용했다. 오류 변환은 request 결과 경계와 native route bridge adapter에 두고, E2E·domain에는 transport frame이나 내부 counter 타입을 노출하지 않았다. core relay 문자열을 복제하지 않고 adapter가 이미 받는 typed envelope만 해석하도록 줄였으며 새 public API와 wire 변경은 없다.
...
- [x] **E2E-CP-21** (결함) — **"수렴 직후 첫 요청"이 10초 retry 루프에 가려져 관측 불가**
  - 근거: 수정 전 집중 gate가 Config 2의 이중 request retry, Config 1의 adapter retry와 route settle sleep, Config 5의 복구 request 예외 삼키기를 검출해 실패했다(`6797c9e77`). `0bad52f79`에서 SM-C5는 공개 location 조회 뒤 의미 request를 한 번만 보내고, RegistryMessaging은 모든 adapter retry와 settle sleep을 제거했으며, RL-B4는 복구 직후 이름 있는 첫 request를 한 번 보내 성공을 단언하도록 바꿨다. target gate에서 E2E-CP-21이 사라졌고 관련 target 빌드와 shell 문법 검사가 통과했다. SpotService 정방향·역방향·shuffle(`logs/20260716-112710-3659925`, `logs/20260716-112840-3664696`, `logs/20260716-112845-3665001`), RegistryMessaging RM-A1 세 순서와 RM-A6·RM-C8(`logs/20260716-112856-3665528`, `logs/20260716-112901-3666226`, `logs/20260716-112905-3667083`, `logs/20260716-113002-3669072`, `logs/20260716-113242-3673109`), RL-B4(`logs/20260716-113242-3673110`)가 통과했다. POSD/DDD 재검토에서는 readiness 관측과 의미 요청을 분리하고 오류 분류는 framework 경계에 유지했으며 새 public API나 wire 변경은 없다.
- [x] **E2E-CP-22** (결함) — **start-order 축이 11개 config 중 9개에서 no-op**이다. 같은 실행을 3번 반복한다
...
- [x] **E2E-CP-24** (결함) — **`RM-B2`(P0)가 문서가 금지한 "종료한 endpoint 반복 timeout"을 무시한다.** scale-in 중 트래픽이 0
...
- [x] **E2E-CP-29** ~~(미구현) — **`RC-B4`(P0)의 JSON fallback 규칙이 검증되지 않는다.** 미지원 타입을 보내지 않는다~~
  - 근거: 재검증 결과 B4의 `json_roundtrip_req_t`는 codec extension에 별도 등록되지 않은 일반 JSON DTO이고, 공통 문서가 fallback 대상으로 든 POCO에 해당한다. B4는 이 타입을 Protobuf·MessagePack과 섞어 보낸 뒤 reply 값과 `application/json`을 이미 단언한다. feature-map과 inventory에 기본 JSON 경로임을 명시했으며 `./run_e2e.sh RC-B4`가 통과했다.
...
- [x] **E2E-CP-33** (결함) — **`RL-D5` soak가 순차 burst**이고, **`RL-D4` wire 호환이 검증되지 않는다**
  - 근거: 수정 전 target-contract gate가 RL-D4 raw camelCase error header 단언 부재와 RL-D5의 구현 오표기·client/runner PASS 경로를 모두 검출했다. RL-D4는 실제 missing-handler E2E와 함께 runtime unit에서 raw `Error=5`, `errorCode`/`errorMessage`, `status` 부재, 정상 `Response=2`를 검증한다. 선택 P2인 RL-D5는 공통 규칙대로 `deferred`로 바로잡고 순차 burst PASS 경로를 제거했다. messaging unit과 target-contract의 E2E-CP-33 조건, `./run_e2e.sh RL-D4`가 통과했다.
- [x] **E2E-CP-34** (결함) — **`MON-A2`(P0)에 trigger가 없고 원리적으로 실패할 수 없다**(IMP-CP-13이 매 tick 발행)
  - 근거: 수정 전 svc-b node payload와 안정 구간 무중복을 요구한 client gate가 기존 count-only evidence를 거부해 실제 runner가 실패했다. runner의 svc-a→svc-b 시작 순서를 실제 peer-row 추가 trigger로 사용하고, svc-a가 받은 public topology payload의 `svc-b` rid와 service summary를 단언한다. 이어지는 500ms 동안 동일 event 수가 늘지 않아 diff 없는 매-tick 발행도 실패시킨다. client/service 빌드와 `./run_e2e.sh MON-A2`가 통과했다.
- [x] **E2E-CP-35** (결함) — **`MON-D1`·`MON-A4`가 전이가 아니라 카운터를 센다.** `MON-A1`은 `RoutingId`를 아예 기록하지 않는다
  - 근거: 수정 전 target-contract gate가 RID별 endpoint payload, A4의 세 socket 전이·이전/새 endpoint 단언, D1의 두 down/up 검증 부재를 모두 검출했다. 공통 문서가 `RoutingId`를 "있으면" 요구하지만 정식 C++ `socket_event_payload_t`에는 해당 필드가 없으므로 공개 계약은 늘리지 않았다. native client monitor event를 기존 공개 monitoring 경로로 전달하고 location evidence에 ready RID·endpoint를 기록했다. runner는 같은 `svc-a` RID를 다른 endpoint로 교체해 `Disconnected`→`Connected`→`ConnectionReady`와 drain/restore 뒤 각각 새 `PeerAdmissionChanged`를 단언하며, `svc-b`를 두 번 SIGKILL·재시작해 각 down/up payload 순서를 검증한다. monitoring unit, target-contract gate, `./run_e2e.sh MON-A4`, `./run_e2e.sh MON-D1`, 전체 `./run_e2e.sh all`이 통과했다.
  - 재검증: 실제 `MON-A1`과 `all`이 restore 뒤 `TopologyChanged` 개수 증가를 기다리다 실패해 기존 완료 근거와 구현의 불일치를 확인했다(`logs/20260715-220422-1234163`). 실패 게이트를 추가하고 A1의 drain/restore를 각각 새 `PeerAdmissionChanged`로 판정하도록 바로잡았다(`2e8dfcd22`, `a7ea63502`). A4의 같은 판정을 공용 evidence helper로 합쳐 중복 polling 구현도 제거했다. target gate와 실제 `MON-A1`(`logs/20260715-221110-1258562`)·`MON-A4`(`logs/20260715-221155-1261348`)가 통과했다.
...
- [x] **E2E-CP-37** (결함) — **장애가 `docker pause`뿐이다.** 문서가 요구한 stop/restart를 한 번도 안 해 **IMP-CP-06의 발동 조건이 생기지 않는다**
  - 근거: 수정 전 target-contract gate가 pause/unpause와 stop/restart 경계 부재를 검출했고, 첫 stop/restart 실행은 Docker의 임의 host port 변경 때문에 복구하지 못했다. Config 6 runner가 고정 loopback port로 Redis를 만들고 client가 `docker stop -t 0`·`docker restart`와 readiness 확인을 수행하도록 바꿨다. 이 과정에서 빈 store 첫 성공 조회를 즉시 disconnect diff로 적용하는 IMP-CP-06도 재현되어, auto-connect loop가 local row를 먼저 다시 등록하고 heartbeat 1회 뒤에 diff를 적용하며 정식 polling interval을 사용하도록 고쳤다. target-contract와 SF-B1·B2·D1·D2·D3 단독 실행 및 전체 `./run_e2e.sh all`이 통과했다.
- [x] **E2E-CP-38** (결함) — **`SF-B2`가 읽는 곳이 0인 knob을 세팅하고 `SF-B1`과 같은 걸 단언한다.** `store_failure_grace`가 미구현이어도 통과
  - 근거: 수정 전 target-contract gate가 grace 소비, replacement provider, 복구 전·후 단언 부재 3건으로 실패했다. auto-connect loop가 store 장애 시작 시각과 마지막 desired set을 내부에 유지하고 `store_failure_grace` 안에서만 미완료 target을 다시 등록하도록 고쳤다. SF-B2 runner는 Redis process가 정지된 상태를 확인한 뒤 `api-b`를 다른 channel endpoint에서 재기동한다. client는 grace를 넘긴 장애 구간의 모든 응답이 기존 `api-a`에서만 오는지, store 복구 뒤 peer row가 새 endpoint를 가리키고 `api-b`가 다시 요청을 처리하는지 확인한다. 단독 SF-B2와 전체 `./run_e2e.sh all`이 통과했다.
- [x] **E2E-CP-39** (결함) — **`SF-C1`·`C2`·`D2`가 framework가 아니라 Redis 확장의 lease 필터를 증명한다.** IMP-CP-35가 안 보인다
  - 근거: failure gate가 Redis `owner_is_live()` 제거와 framework live-row reader 배선을 요구해 수정 전 3건으로 실패했다. store read의 lease 필터를 제거한 상태에서 SF-C1·C2·D2가 통과했고, 전체 `./run_e2e.sh all`도 통과했다. 따라서 stale row 제외와 auto-connect diff는 Redis 확장 정책이 아니라 framework의 owner lease join으로 증명된다.
- [x] **E2E-CP-40** (결함) — **`SF-D1`·`SF-D2`(P0)가 장애·복구 구간에 트래픽을 하나도 안 흘린다.** 복구 순서·grace·disconnect marker 전부 미단언
  - 근거: 수정 전 target-contract gate가 지속 traffic, socket transition evidence, survivor 전이 부정 단언 부재 3건으로 실패했다. consumer가 정식 socket monitoring event를 process-local evidence로 기록하고 `/query/connections`에서 제공한다. D1은 두 provider 연결을 traffic으로 준비한 뒤 장애 전부터 복구 뒤까지 모든 request 성공과 두 endpoint의 Connected/Disconnected count 불변을 확인한다. D2는 장애 전부터 허용 실패를 포함한 traffic을 흘려 최대 성공 간격을 제한하고, 사용 가능한 `api-a` count 불변과 종료한 `api-b` Disconnected 증가를 확인한다. 단독 D1·D2와 전체 runner가 통과했다.
- [x] **E2E-CP-41** (결함) — **consumer의 30초 retry 루프가 SF 전반의 "요청 성공" 단언을 세탁한다**
...
- [x] **E2E-CP-45** (결함) — **`SF-E1`이 store가 아니라 앱 데코레이터에 지연을 넣어** 자기 `sleep`을 잰다
  ...
  - 재검증: 설정 정책 전환 뒤 target gate가 제거된 `ZLINK_CPP_SF_REDIS_PROXY_ADMIN_URL` 환경 변수를 요구해 거짓 실패했다. `redisProxyAdminUrl`을 typed client JSON에서 읽고 환경 변수를 사용하지 않는 조건으로 바로잡았다(`1427363ca`). target-contract gate 전체가 통과했고 SF-E1(`logs/20260715-215405-1194690`)에서 baseline p99 102.91ms, 지연된 store 조회 2406.43ms, 동시 status 조회 0.79ms, application request p99 51.79ms로 823.27ms budget 안에 남았다.
...
- [x] **E2E-CP-56** (결함) — **Config 10의 topology가 문서와 다르다.** actor 노드 3개, session gateway·transfer controller **0개**
  - 근거: §13.4 D5 확정에 따라 제거된 원격 actor 생성·join API와 별도 transfer controller를 만들지 않는다. runner는 actor-a/b와 session-a/b 네 process를 시작하고, client는 최초 소유 node에서 actor 생성·join을 시작하며 연속 이동도 매 시점의 현재 소유 node에 요청한다. session gateway endpoint는 환경변수가 아니라 `nodeAStream`·`nodeBStream` typed client JSON으로 전달한다. stale 환경변수를 요구하던 gate를 바로잡은 뒤(`65664f836`) target-contract에서 E2E-CP-56이 사라졌고 `ST-A1`이 통과했다(`logs/20260715-215227-1188856`). 전체 스무 시나리오 검증은 `logs/20260715-115825-3241768`에 있다.
...
- [x] **E2E-CP-58** (결함) — **`TA-B1`의 error kind를 e2e caller가 스스로 만들어 던진다.** framework 분류를 **지워도 통과**한다. send를 존재 확인 수단으로 쓴다(문서가 이름 짚어 금지)
  - 근거: 수정 전 target gate `2d7416436`이 caller의 directory-miss 오류 위조와 send 실패 단언을 검출했다. `44b28e5b5`에서 실제 actor를 생성·destroy한 뒤 concrete candidate ref로 호출해 request의 public `actor_route_not_found`를 framework에서 받고, send는 로컬 접수 성공만 판정하도록 바꿨다. target gate와 `TA-B1`, Config 9 전체 runner가 통과했다.
- [x] **E2E-CP-59** (결함) — **`TA-B2`·`TA-B3`가 앱 코드로 location row를 위조한다.** actor 노드가 1개뿐이라 owner 교체가 **구성 불가능**하고, B3는 route를 **끊지도 복구하지도 않는다**
  - 근거: 수정 전 집중 target gate가 caller의 location row 위조, 단일 actor owner, B2의 실제 교체 부재, B3의 실제 route 단절·복구 부재를 검출해 실패했다(`1d0000968`). `abb6d7e98`에서 Redis row 직접 쓰기를 제거하고 actor-a에서 저장한 ref를 actor 제거 뒤 actor-b의 같은 id 재생성과 대조해 `actor_location_stale` 및 새 owner 성공을 확인했다. B3는 기존 공개 `router_connections()` handle로 두 RID endpoint를 실제 제거·재연결하고 같은 ref의 `route_not_connected`·복구 성공·단절 중 handler 미도달을 확인한다. RID 지정 연결이 handle 목록에 없어서 `disconnect()`가 동작하지 않던 거짓 계약도 같은 커밋에서 실제 `connect_peer_rid`·`disconnect_peer_rid`에 연결했다. module-hosted 단위 시험, E2E-CP-59 집중 gate no-hit, B2+B3(`logs/20260716-091842-3270255`), Config 9 전체(`logs/20260716-091927-3274099`), forward/reverse B3(`logs/20260716-092253-3285804`, `logs/20260716-091723-3263087`)가 통과했다. POSD/DDD 재검토에서는 transport frame을 조작하는 프록시 안을 버리고 기존 endpoint handle에 RID 지식을 모았으며, actor ref 저장소는 stale-ref 관측만 소유하고 location lifecycle과 오류 분류는 framework에 유지했다. 새 public API나 wire 변경은 없다.
- [x] **E2E-CP-60** (미구현) — **Track B의 negative 역할서버 evidence**가 세 시나리오 전부 미단언
  - 근거: `44b28e5b5`에서 client가 B1 send/request, B2 stale, B3 route-disconnected의 actor handler evidence 부재를 직접 검사하고 B1은 location row 삭제·미재생성까지 bounded observation으로 확인하도록 추가했다. 준비 row가 caller polling에 보이기 전 호출하던 경합도 location 관측으로 제거했다. target gate, `TA-B1`, `TA-B2`+`TA-B3`, Config 9 전체 runner가 모두 통과했다. POSD/DDD 재검토에서는 caller의 선판정을 제거해 오류 분류를 framework에 모으고 present/missing polling을 단일 관측 함수로 합쳤다.
- [x] **E2E-CP-61** (결함) — **`OBS-C1`(P0)이 문서가 금지한 row 삭제를 통과로 받아들이고**, create-rejection은 **무조건 PASS를 찍는다**
  - 근거: 수정 전 target gate가 row 삭제 허용, lease·drain metric·기존 traffic 부재, 조건부 create 판정과 evidence 부재를 5건으로 검출했다(`94af5da46`). runtime이 시작 시 `zlink.drain.state{state=serving}`을 발행하고 evidence가 실제 peer row·owner lease를 읽도록 연결했다. runner는 typed draining row를 terminal 전까지 요구하고 play-a의 기존 route connection으로 요청 8건을 모두 성공시킨 뒤 lease 갱신 시각 증가, `serving→draining`, 명시적 create 거절과 최종 `drained`를 확인한다(`1622f4208`). `./run_e2e.sh drain`이 `logs/20260716-051542-2415012`에서 통과했다.
  - POSD/DDD: shell이 row 삭제나 transport 실패를 성공 의미로 재해석하는 안과 runtime의 기존 상태를 evidence로 투영하는 안을 비교했다. 전자는 lifecycle 지식을 runner에 복제하므로 기각하고, route·lease·metric의 원래 소유 경계를 그대로 관찰했다. 새 public API나 domain·wire 변경은 없다.
- [x] **E2E-CP-62** (결함) — **`OBS-A1`~`A4`가 판별력이 없다.** `OBS-A2`는 **C++ 전용 `outcome=` 토큰을 고정**해 IMP-CP-18을 못박는다
  - 근거: 수정 전 집중 gate가 A1의 순서, A2의 같은 flow 성공·실패, A3의 off-node 전후 동일성, A4의 N subscriber 관계가 없어서 실패했다(`620a23314`). client 공통 flow 판독기에 동일 flow와 시간순 단언을 모으고 A1~A4가 실제 session·route·spot·두 subscriber 로그를 관계로 비교하도록 고쳤으며 `outcome=` 대신 공통 `phase=error`를 사용한다(`f89bcc835`). flow `logs/20260716-092832-3306098`, fanout `logs/20260716-094302-3355921`, offnode `logs/20260716-094745-3369410`과 CP-62 집중 gate가 통과했다. POSD/DDD 재검토에서는 로그 형식 지식을 공통 판독기에 한정하고 시나리오에는 업무 흐름 관계만 남겼으며 새 public API와 wire 변경은 없다.
- [x] **E2E-CP-63** (결함) — **`OBS-B2`·`B3`·`B4`·`C3`가 항진명제이거나 절반만 돈다**
  - 근거: 최초 집중 gate가 B2의 정확한 transfer 계기, B3의 lease·label, B4의 traffic·무적재, C3의 재생 구분이 없어 실패했다(`620a23314`). room 부하와 transfer 1회, fanout 1:2와 Redis 외부 지연, metrics-off 메시징, append-only workflow event replay를 구현했다(`a8d65d048`). 추가 반증에서 C3가 drain-natural을 실행하지 않아 gate가 실패했고(`b1d29d5be`), runtime이 user spot의 public close까지 기다린 뒤 두 policy별 room counter를 기록하도록 고쳤다(`d4a84a3e3`). B4도 app collector의 빈 결과에 기대지 않도록 reader 없는 10,000회 계측 뒤 내부 저장 구조 불변을 단위 테스트로 고정했다(`bc3aeacb6`, `de5a8f99a`). metrics `logs/20260716-094229-3353674`, fanout `logs/20260716-094302-3355921`, policy `logs/20260716-100036-3398019`, offnode `logs/20260716-094745-3369410`, `test_cpp_framework_monitoring`과 CP-63 집중 gate가 통과했다. POSD/DDD 재검토에서는 자연 종료 대기를 runtime에, event replay를 workflow 저장소에, 수치 판정을 client에 두었고 우회 API나 공유 wire 변경은 없다.
...
| **SMP-CP-11** | [shoppingmall:964-965,981-982](../../common/sample/event/shoppingmall.ko.md): 같은 idempotency key를 **두 CommerceApi에 동시에** 보내고, 서로 다른 owner가 **동시에** 처리한다 | `ShoppingMall/Client/main.cpp:104-110` — API-A 호출을 **끝내고** API-B를 부른다. scale-out도 나중에 별도로 실행한다(`:186-190`). GameQuest도 Alice→Bob 순차다(`gamequest_client_scenario.hpp:35-123`) |
...
| **SMP-CP-15** | [shoppingmall:833](../../common/sample/event/shoppingmall.ko.md) §9.3: `O->>O: ContinueOrderWorkflowReq 예약 (기다리지 않음, fire-and-forget)` — **예약은 owner spot 안에서** 한다 | `Server/OrderWorkflow/main.cpp:62-71` — `start()`가 `run_workflow(..., max_steps=1)`을 실행하고 **아무것도 예약하지 않고 끝난다.** 트리 전체에서 `ContinueOrderWorkflowMsg`를 만드는 **유일한 곳이 `Server/CommerceApi/main.cpp:234-245`(`schedule_continue`)**이고, HTTP 응답 직후에 불린다(`:74`). ⇒ **`CommerceApi`가 응답과 send 사이에 종료되면 주문이 `Created`에서 진행되지 않는다** — §9.5 복구가 다루려는 바로 그 실패다. 샘플의 대표 주장("saga 인프라가 순차 코드로 접힌다", §9.2)이 **edge에서 구동되고 있다** |
...
| **SMP-CP-36** | [gamequest:604-605,608](../../common/sample/event/gamequest.ko.md): "**연결 끊고 binding 해제** → 다른 노드로 재접속 → **조회로 복원** → 이후 notify가 새 노드로", "2 노드에서 PlayerA/B가 **다른 owner에서** 동시 처리" | `GameQuest/Client/gamequest_client_scenario.hpp:230-235` — alice의 `api_a` connector를 **닫지도 disconnect하지도 않고** 두 번째 connector를 열어 재-join한다. 원래 세션의 **binding이 유지된다.** unbind도, 재접속 후 `GetQuestProgressReq` **복원 조회**도, notify가 **이전 노드에 전달되지 않았다는 negative**도 없다 — 새 노드의 positive push만 본다(`:238-253`). scale-out의 **"다른 owner"**도 어디서도 단언되지 않는다(owner 신원을 응답·push 어디서도 안 읽는다). rehydrate 준비는 **500ms 고정 sleep**이다(`:264`) — reactivation을 관측하지 않고 대기 시간만 지난다 |
...
| **SMP-CP-53** | [bingo:1053-1055](../../common/sample/bingo/README.ko.md): "이 정리는 **observer 구독 수명만** 끝내며, game room의 player cleanup이나 winner 판정 상태를 **바꾸지 않는다**" | `Spots/BingoRoomSpot/Handlers/stop_observing_bingo_events_handler.hpp:9-19` — `observers`에서 지우고 **무조건** `_context.leave_actor(...)`를 부른 뒤 `{true, node_rid}`를 돌려준다. **`_is_observer`도, 호출자가 등록된 observer인지도, `request.room_id`도 확인하지 않는다.** 그런데 이 handler는 `bingo_room_spot_t`에 등록돼 있어(`bingo_room_spot.hpp:42`) **game room에도 등록된다.** ⇒ **진행 중인 방의 player가 이 packet을 보내면 방에서 제거되고 `stopped = true`를 받는다.** `.NET`은 네 조건을 다 본다(`BingoRoom.cs:249`) |
...
| **SMP-CP-40** | [tictactoe:716-726](../../common/sample/tictactoe/README.ko.md): `GameState`의 `Winner?`·`XActorId?`·`OActorId?`·`LastMoveActorId?`·`LastMoveCell: int?` **다섯이 nullable** | **해결.** 다섯 필드는 `std::optional`이며 JSON의 값 없음과 `null`을 비어 있는 optional로 읽고, 비어 있는 값은 `null`로 쓴다. 계약에 없는 `draw` 필드와 `-1` sentinel도 제거했고 게임 종료는 `Status == "Draw"`로 표현한다. |
| **SMP-CP-41** | [supportchat:475-481](../../common/sample/supportchat/README.ko.md): `JoinConversationReq` = `ParticipantId`·`Role`·`DisplayName`. client는 비워 보내지만 **Support 서버가 conversation-actor join 때 채워 보낸다** | **해결.** client hop은 세 값을 비워 둘 수 있고, Support 서버가 conversation actor join을 만들 때 actor profile의 참가자 ID·역할·표시 이름을 채운다. owner Spot은 payload와 profile이 다르면 admission을 거절한다. |
| **SMP-CP-42** | [supportchat:397-402](../../common/sample/supportchat/README.ko.md): `ActorRefSnapshot`은 framework가 주는 wire 모델이며 "**샘플이 정의하지 않는다**". `NodeRid: bytes` | **해결.** `EnsureSupportUserActorRes`와 `EnsureAgentConversationRes`가 framework의 전체 `actor_ref_snapshot_t`를 사용한다. 사설 DTO와 사설 actor-ref 복원 함수는 제거했고, Support 경계를 나가는 snapshot만 외부에서 도달 가능한 node routing id로 투영한다. |
| **SMP-CP-43** | [shoppingmall:737,762,794-797,805](../../common/sample/event/shoppingmall.ko.md): `Amount: decimal`, `OrderState.Amount: decimal?` | **해결.** 계약·이벤트·aggregate의 금액은 `decimal_t`가 소유하며 유효성, 정규화, .NET decimal 정밀도 범위와 JSON number 변환을 한 경계에서 처리한다. `OrderState.Amount`는 `std::optional<decimal_t>`로 `null`을 보존한다. |
...
| **SMP-CP-45** | [gamequest §11.2:503-509](../../common/sample/event/gamequest.ko.md): entry→owner는 **flat one-way `GameplayMsg`** = `{EventId, PlayerId, Type, Payload: bytes, OccurredAtUnixMs}` | C++ `messages.hpp:222-226` = `gameplay_msg_t{ gameplay_event_envelope_t event; }` — **8필드 envelope를 감싼 wrapper**(`:209-219`). `Payload: bytes`가 없고, `Type`→`event_type`, `OccurredAtUnixMs`→`created_at_unix_ms`이며 **문서에 없는 필드가 4개** 더 추가된다. `.NET`엔 **`GameplayMsg` 자체가 없고** 같은 hop을 **`ApplyGameplayEventReq`(request!)**로 보낸다(`Server/Configuration/SampleConfiguration.cs:25`). C++는 one-way `send`다(`Server/GameApi/main.cpp:469`, 수신 `Server/QuestMission/main.cpp:264,282`). ⇒ **같은 hop인데 packet 이름도 호출 방식(one-way vs request)도 다르다** |
| **SMP-CP-46** | [tictactoe:672-674](../../common/sample/tictactoe/README.ko.md): `TicTacToeGameJoinRes` | **해결.** 내부 room admission은 `tictactoe_game_join_res_t`로 응답하고, entry Spot handler가 이를 외부 `join_game_res_t`로 변환한다. 내부 actor join과 client 응답의 이름이 더 이상 섞이지 않는다. |
...
| **E2E-CP-07** | [config-10:78-92](../../common/e2e/config-10-spot-actor-transfer.ko.md): ST-A1은 **정확히** `admission → location_committed → leave → joined → success_reply` 순서를 요구한다 | `SpotActorTransfer/Client/main.cpp:355` — `wait_evidence`로 **순서 없는 포함 여부**만 본다(`:183`). **순서 helper는 있는데(`:203`) 쓰지 않는다.** `location_committed` 마커는 요구조차 안 한다. ST-A2도 요구된 negative side effect 6개 중 **1개만** 본다(`:378`) |
| **E2E-CP-08** | [Sample/E2E 설정 정책 §2·§8](../../common/sample-e2e-configuration-policy.ko.md): framework host에는 **설정 파일 경로만** 넘긴다. **server와 client 앱 코드가 직접 쓸 수 있는 환경 변수는 0개다.** | 11개 config의 framework host가 role별 JSON을 `load_json()`으로 읽고 `bind_required<T>()`로 변환한다. standalone client는 CLI 또는 client JSON을 시작 경계에서 typed 설정으로 바꾸며, runner가 만든 설정은 mode `0600`이고 종료 경로에서 삭제한다. C++ `samples/**`·`e2e/**`의 직접 환경 접근, 설정 전달용 `ZLINK_*`, sample별 prefix 환경 기본값은 회귀 게이트 기준 0건이다. |
...
| **E2E-CP-14** | [§3.1](../../common/e2e/README.ko.md): Config 2의 P0 시나리오를 **route mesh 없음 × Session/Play 분리 배치**로 실행한다 | 구현 완료(`342b047e6`). `SM-D2`가 기존 client 시나리오와 분리된 `session-a`·`play-a`·`play-b` 역할을 그대로 사용하면서 각 역할의 route mesh 등록을 끈다. Stream 인증은 Play가 반환한 전체 actor snapshot을 직접 bind하고, framework의 bound-session 등록과 actor request/push는 SpotMesh로 전달된다. forward·reverse·고정 seed shuffle 실행이 모두 통과했다. |
...
| **E2E-CP-17** | [config-2:588-593](../../common/e2e/config-2-spot-service.ko.md): "public Spot manager로 **target user Spot을 닫는다.** 이후 **닫힌 Spot 경로의 실패를 확인**하고 같은 channel로 일반 channel request를 다시 보낸다" | `Client/Scenarios/sm_f5_scenario.hpp:16-56` — **spot을 닫는 코드가 없다.** 대신 `/spot/direct`를 보내고 **성공하지 않으면 throw한다**(`:49-53`, `value != "route-survived-f5:reply"`). runner도 `route-survived-f5`가 도착했음을 단언한다(`run_e2e.sh:2404-2406`). ⇒ **시나리오가 자기 계약의 정반대("Spot이 계속 유지된다")를 증명한다.** 덤으로 `play_b` client가 `play_b_http_endpoint`가 아니라 `play_http_endpoint`로 만들어져(`:34-36`) 인자가 사용되지 않는다 |
| **E2E-CP-18** | [config-2:473](../../common/e2e/config-2-spot-service.ko.md): "검증: error reply + **message-flow error evidence**(`surface=spot_route`, `reason=no_handler`, `action=reply_error`/`drop`)". **그 evidence가 곧 이 시나리오다** | `Client/Scenarios/sm_e1_scenario.hpp:69,84-87,100-104,117` — 서버 HTTP helper가 돌려준 bool(`request_failed`·`failed`·`sent`)만 본다. `sent`는 one-way가 **제출됐다**는 뜻일 뿐 drop을 말하지 않는다. runner 블록(`run_e2e.sh:3390-3427`)도 앱 marker만 보고 **`-flow.log` grep이 없다**. SM-C1(`:2144-2147`)·SM-C2(`:2202-2205`)는 폐기된 `reason=handler_missing`을 검사해 정식 `no_handler` 값과 다르고, E1은 flow 로그 검사 자체가 없다. |
| **E2E-CP-19** | [config-2:574-579](../../common/e2e/config-2-spot-service.ko.md): `SM-F4`(P0)는 넷을 요구한다 — ① request → error reply, ② **`send`(command) → reply 없이 drop되고 failure counter가 오른다**, ③ **message-flow error 분류가 남는다**, ④ 같은 channel의 다른 routing은 무영향 | 구현 완료(`4dacf9b6e`). `Client/Scenarios/sm_f4_scenario.hpp:52-145`가 source·target Spot을 만들고 target을 닫은 뒤 source Spot에서 request/send를 수행한다. 두 node의 `SpotRouteDispatchFailure` 개수가 2 이상 늘고 `DirectSpotReq/reply_error`·`DirectSpotMsg/drop`인지 확인한 다음 정상 route를 재검증한다. `framework/src/runtime/spots/spot_runtime.cpp:1571-1583`과 `framework/src/runtime/backend/native_route_backend.cpp:392-432`가 request error와 one-way bridge 거절을 같은 dispatch reporter에 연결하며, runner도 두 flow 분류를 직접 검사한다(`run_e2e.sh:2485-2489`). |
| **E2E-CP-20** | [config-2:73](../../common/e2e/config-2-spot-service.ko.md): `SM-A1`은 "생성된 user spot의 location row가 `IZLinkLocationRuntimeQuery.ListSpotLocationsAsync(filter)`로 **조회된다**(spot lifecycle의 자동 row 등록 확인)"를 요구한다 | `Client/Scenarios/sm_a1_scenario.hpp:46-82`는 reply 필드만 본다. `run_e2e.sh:1367-1381`은 앱 marker만 본다. **`SpotService/` 트리 전체에서 location runtime query 표면(`list_spot_locations` 등) grep 0건**이다(`Server/Shared/Support/location_store.hpp:26`의 store *등록*만 있다). ⇒ 하필 **[IMP-CP-10](#라운드-3-2026-07-14--근거-없는-표면--조용한-no-op--경합--redis-store)이 "spot topology 조회는 항상 빈 페이지를 반환한다"고 지목한 그 표면**이고, 그걸 잡아야 할 e2e가 없다 |
| **E2E-CP-21** | [§3.1:527-529](../../common/e2e/README.ko.md): "location 발견·dial 수렴 직후 settle 지연 없이 즉시 첫 요청을 보낸다. **재시도나 sleep으로 가리지 않는다** — 첫 요청이 바로 성공하거나 fail-fast로 분류되는 것 자체가 검증 대상이다" | **해결.** Config 2는 location row 관측과 의미 request를 분리하고 request를 한 번만 보낸다. Config 1은 adapter retry와 route settle sleep 없이 client 시나리오를 실행한다. Config 5의 RL-B4는 restore 뒤 첫 request의 오류를 삼키지 않고 즉시 결과를 단언한다. |
| **E2E-CP-22** | [§3.1 기동 순서 축:493,504-506](../../common/e2e/README.ko.md): config 러너가 기동 순서를 **인자로 받고**, 역방향 1회 + 고정 seed shuffle 1회를 최소로 실행한다. Config **1·2·9**가 이 축의 대상이다 | `run_e2e_all.sh:29-32,121`이 `forward`/`reverse`/`shuffle:20260709` **3변형을 11개 config 전부에 `E2E_START_ORDER`로 export**한다. 그런데 **그 변수를 읽는 러너는 `SpotService`·`ToActorMessaging` 둘뿐**이다. ⇒ **나머지 9개 config는 완전히 동일한 실행을 3번 반복**한다 — 커버리지는 0인데 게이트 시간만 3배다. 특히 **문서가 이 축의 대상으로 지목한 Config 1(`RegistryMessaging`)이 변수를 읽지 않는다** |
| **E2E-CP-23** | [config-1:121-123](../../common/e2e/config-1-location-messaging.ko.md): `RM-A4`(P0)는 rid `api-a`가 endpoint p2로 바뀔 때까지 기다려 **`ListPeerLocations`가 유효한 row 하나(endpoint=p2)만** 반환함을 단언하고, **consumer 재시작 없이** 재요청해 **stale p1으로 반복 timeout하지 않음**을 본다 | `run_e2e.sh:405-406` — v1을 종료하고 **새 프로세스**를 띄운다. `rm_a4_same_rid_failover_scenario.hpp:25-31`은 그 **새 프로세스의 HTTP endpoint에 직접** 20번 요청한다. resolve를 하는 channel client가 **v1 종료 뒤에 생성**되므로 **peer handover도, 피해야 할 stale endpoint도 존재하지 않는다.** `/locations/peers` 호출 **0건**. 유일한 단언 `instance_id == "api-a-v2"`(`:29`)는 v2가 유일한 사용 가능 대상이라 **자명하게 참**이다. `.NET`은 둘 다 한다(`RmA4SameRidFailoverScenario.cs:39,69-80` — row 1개·endpoint v2, `:56-66` — `v1Count == 0 && v2Count == 20`) |
| **E2E-CP-24** | [config-1:158](../../common/e2e/config-1-location-messaging.ko.md): `RM-B2`(P0) — "consumer가 종료한 endpoint로 timeout을 **반복하지 않음**" + "**지속 request 중** scale-in이 나도 완료된 요청은 정상 reply 또는 정해진 public error로 끝난다" | `rm_b2_scale_in_scenario.hpp:52-64` — settle 루프가 요청을 `catch (...) { settled = 0; sleep 200ms; }`로 감싸 **40 × 1.5초** 재시도한다. **문서가 금지한 바로 그 동작을 단언 대상이 아니라 무시 대상으로 만든다.** 게다가 client는 `wait_for_file`에서 대기하고(`:30-31`) 그동안 러너가 api-b를 종료하고 5초를 기다린다(`run_e2e.sh:458-461`) ⇒ **scale-in 순간에 흐르는 트래픽이 0**이다 |
...
| **E2E-CP-27** | ~~[config-4:33-34,52-80](../../common/e2e/config-4-registration-codec.ko.md): C++도 자동·attribute·수동 등록 변주를 제공해야 한다~~ | **감사 판정 취소.** [공통 framework spec §8](../05-framework-api.ko.md#8-handler-등록과-dispatch)은 C++을 runtime reflection 기반 자동 등록 원칙의 예외로 정하고 compile-time 타입과 명시 builder 호출을 요구한다. 공통 E2E의 RC-A2도 `.NET` attribute 등록으로 한정한다. A1/A2를 구현으로 표시했던 feature-map을 `not-supported`로 바로잡고, attribute를 흉내 내지만 읽히지 않던 `topic_name`을 제거했다. |
| **E2E-CP-28** | [config-4:156-167](../../common/e2e/config-4-registration-codec.ko.md): `RC-B5` — explicit non-JSON content-type에 대응 codec이 없으면 정확히 `PayloadDecodeFailed`로 끝나고 정상 JSON 트래픽은 유지되어야 한다 | mismatched peer 자체는 **진짜다**(`Server/JsonOnlyPeer/main.cpp:17` + `server_host.hpp:149-152`가 protobuf/msgpack serializer를 건너뛴다). 그러나 `Server/Endpoints/operational_endpoints.hpp:296-304`는 **"성공하지 않았다"만 단언한다.** 5초 timeout이나 dial 실패도 통과할 수 있으며 error kind, json-only peer의 `payload_decode_failed` marker와 content-type을 확인하지 않는다. client 파일은 server가 계산한 bool만 읽는다(`codec_mismatch_scenario.hpp:13-16`). feature-map(`:22`)도 정확한 public error와 정상 JSON 격리를 완료 조건으로 갱신해야 한다 |
| **E2E-CP-29** | ~~[config-4:152](../../common/e2e/config-4-registration-codec.ko.md): `RC-B4`의 JSON fallback 규칙이 검증되지 않는다~~ | **감사 판정 취소.** 감사 행은 custom serializer가 등록된 `custom_roundtrip_req_t`만 봤지만, B4는 별도 serializer가 없는 일반 DTO `json_roundtrip_req_t`도 함께 보낸다. 이는 공통 문서가 든 POCO 경로이며 client가 reply 값과 `application/json`을 단언한다. |
...
| **E2E-CP-33** | [config-5:244-262](../../common/e2e/config-5-resilience-lifecycle.ko.md): `RL-D4`는 error-reply wire 직렬화를, 선택 P2인 `RL-D5`는 지속 부하 harness가 있을 때 soak를 검증한다 | **해결.** RL-D4의 실제 missing-handler dispatch는 public E2E가 확인하고, runtime unit gate가 같은 channel reply의 raw kind와 camelCase error fields를 검증한다. RL-D5는 harness가 없으면 미구현으로 두라는 공통 규칙에 따라 `deferred`로 표시하고 순차 burst를 canonical PASS 경로에서 제거했다. |
| **E2E-CP-34** | [config-7:73-74](../../common/e2e/config-7-monitoring.ko.md): `MON-A2`는 svc-b 추가로 peer location을 바꾸고 실제 payload를 검증한다 | **해결.** runner의 svc-a→svc-b 시작 순서가 실제 Redis peer row 추가를 만들고, client는 svc-a의 topology payload에서 `svc-a,svc-b` node 목록과 service summary를 확인한다. 이후 500ms 안정 구간의 topology/summary event 총수가 증가하지 않아야 하므로 diff 없는 매-tick 발행은 실패한다. |
| **E2E-CP-35** | [config-7:64,93-94,149-150](../../common/e2e/config-7-monitoring.ko.md): `MON-A1`은 payload에 `RemoteAddr`와 **있으면 `RoutingId`**를 포함한다. `MON-A4`는 **같은 rid·다른 endpoint로 provider 교체 → socket `Disconnected`/`Connected`/`ConnectionReady`** + drain/restore → `PeerAdmissionChanged`. `MON-D1`은 crash+restart를 **여러 번** 흔들며 **각 down/up 전이**를 관측한다 | **해결.** 정식 C++ socket payload에 없는 조건부 `RoutingId` 필드는 추가하지 않았다. native client socket monitor를 공개 event 발행 경로에 연결하고, location payload evidence가 ready RID·endpoint 대응을 보존하도록 바꿨다. A4는 같은 RID의 실제 endpoint 교체와 세 socket event, drain/restore의 새 admission event를 순서대로 확인한다. D1은 두 번의 SIGKILL·lease 제거·재시작마다 지속 observer의 실제 route payload가 down/up으로 바뀌는지 확인한 뒤 정상 request로 monitoring 지속을 검증한다. |
...
| **E2E-CP-37** | [config-6:61](../../common/e2e/config-6-store-failure-recovery.ko.md): "장애 시나리오는 harness가 Redis process를 **정지(stop)**했다가 **재기동(restart)**한다" | **해결.** runner가 재시작 뒤에도 유지되는 고정 loopback host port로 Redis를 만들고, B1·B2·D1·D2·D3가 `docker stop -t 0` 뒤 같은 container를 restart해 빈 store 복구 조건을 실제로 만든다. restart readiness 뒤 runtime status와 messaging을 검증한다. 이 harness가 드러낸 빈 store 즉시 disconnect 결함은 local row 재등록 → heartbeat 1회 유예 → diff 순서로 고쳤고, auto-connect tick은 정식 polling interval을 사용한다. |
| **E2E-CP-38** | [config-6:116](../../common/e2e/config-6-store-failure-recovery.ko.md): `SF-B2` — "**새 outbound connect는 중단된다** — 장애 중 재시작한 provider가 있어도 store 복구 전에는 연결 대상에 추가되지 않음" | **해결.** runtime은 store 장애 시작 시각과 마지막 desired set을 보존하고 `store_failure_grace` 안에서만 미완료 target 등록을 재시도한다. runner는 Redis가 정지된 동안 `api-b`를 새 channel endpoint에서 재기동한다. client는 grace 초과 구간에 새 provider가 요청을 처리하지 않는지 확인하고, store 복구 뒤 새 endpoint row와 실제 `api-b` 응답을 확인한다. |
| **E2E-CP-39** | [40 §1·§5.1](../server/40-location-runtime.ko.md): **store는 저장만 하고, owner lease join은 framework runtime의 책임**이다([IMP-CP-35](#상세_1)) | **해결.** Redis store의 row loader는 lease key를 조회하지 않고 raw row를 반환한다. SF-C1·C2·D2는 framework `live_location_reader_t`가 owner lease snapshot을 join한 결과로 stale peer를 제외하고 auto-connect diff를 적용하는 구성을 실제 Redis에서 검증한다. |
| **E2E-CP-40** | [config-6:157,164-168](../../common/e2e/config-6-store-failure-recovery.ko.md): `SF-D1`(P0)은 "각 provider evidence에 **불필요한 disconnect/reconnect marker가 없음**" + "request는 **전 구간에서** 성공"을 요구하고, `SF-D2`(P0)는 **재등록 → heartbeat 1회 유예 → diff** 순서와 "live link를 끊지 않음"을 요구한다 | **해결.** consumer가 client channel의 정식 socket event를 기록한다. D1은 outage 전부터 비동기 traffic을 시작해 전 구간 성공을 확인하고 두 provider endpoint의 connect/disconnect count가 변하지 않는지 단언한다. D2는 장애·복구 전 구간의 최대 성공 간격을 제한하며 survivor `api-a`의 count 불변과 종료한 `api-b`의 disconnect 증가를 각각 확인한다. |
| **E2E-CP-41** | [config-6:106,157](../../common/e2e/config-6-store-failure-recovery.ko.md): "**기존 연결이 유지된다**"(fail-static), "request는 **전 구간에서** 성공한다" | **해결.** `/profile/request` 한 번은 5초 제한의 framework request 한 번만 실행한다. handler 내부 deadline·backoff·오류 삼키기를 제거했으므로 연결 오류나 timeout은 해당 probe의 실패로 즉시 드러나며, D1·D2 traffic window가 실제 연결 연속성을 검증한다. |
| **E2E-CP-42** | [config-6:93-94](../../common/e2e/config-6-store-failure-recovery.ko.md): `SF-A2` — "provider 하나를 **추가로 띄웠다가** 정상 종료한다", "추가 후 **polling interval 몇 tick 안에 새 provider가 routing 대상이 된다**", "watch를 지원하는 배포와 결과 의미가 같다" | **해결.** runner는 client가 초기 `api-c` 부재를 확인한 뒤 별도 provider를 시작한다. client는 polling interval 기반 제한 안에 peer row가 나타나고 `api-c`가 실제 profile request를 처리하는지 확인한다. 이후 `api-c`를 정상 종료하고 같은 제한 안에 row가 제거되며 후속 request가 제거된 provider로 가지 않는지 단언한다. `watch_enabled=false` 확인은 배포 mode 확인으로만 남고, correctness는 add/remove routing 전이로 검증한다. |
...
| **E2E-CP-52** | config-10 각 시나리오의 절차·검증 | **`ST-D2`**(P1, `:251-254` — cleanup retry를 **지연시켰다가 실행**): `Client/main.cpp:654-675`가 평범한 transfer + `sleep 2s` + 재조회일 뿐 — **지연도 트리거도 주입도 없다.** source-cleanup 경로가 아예 없는 framework에서도 통과한다. 그걸 구현할 수 있는 유일한 서버 표면(`transfer_gate_store_t` + `/transfer-gates/{actorId}/release`, `Server/ActorNode/main.cpp:180-189,698-713`)은 **호출부가 0건**이다. **`ST-B2`**(P0, `:143-146` — `commit_ack`와 `source_cleanup` **사이에서** source를 중단한 뒤 종료한다): gate를 설정하지 않고 transfer를 끝낸 뒤 `shutdown_node()`를 부른다(`:466-492`) — **`ST-C2`와 같은 형태**이고 target generation도 확인하지 않는다. **`ST-C1`**(P0, `:196` — target이 **pending admission timeout cleanup evidence를 남긴다**): 서버에 `pending_admission` **0건**, negative만 단언한다(`:548-586`). **`ST-C3`**(P1, `:225`): `require_no_contains(..., "packet_handler|after-joined-failure")`(`:1003-1005`)인데 **그 marker를 실은 packet을 아무도 보내지 않는다** — 어떤 동작에서도 존재할 수 없다. **`ST-F5`**(P1, `:358-366`): `mapping_evicted`를 단언하지 않고(client 0건) entry snapshot도 읽지 않으며 `sleep 3300ms` 뒤 stale probe로 **추론**한다(`:816-844`) |
...
| **E2E-CP-56** | [§13.4 D5](../90-implementation-gap.ko.md): actor 생성·join은 그 actor를 소유하는 node에서 시작하고, 별도 원격 controller API는 만들지 않는다 | **해결.** actor-a/b가 자기 actor의 생성·join과 기존 handoff를 소유하고 session-a/b가 connector session 역할을 맡는다. client는 각 node의 HTTP 제어 endpoint와 session stream endpoint를 typed JSON으로 받으며, controller·환경변수 우회는 없다. |
...
| **E2E-CP-65** | [05 §13](../05-framework-api.ko.md#13-오류-kind)은 현재 mesh member snapshot에 없는 target을 `RequestTargetNotFound`, 알려졌지만 pipe가 준비되지 않은 target을 `RouteNotConnected`로 구분한다 | `RegistryMessaging`의 RM-C2 runner와 feature-map은 없는 RID를 `RouteNotConnected`로 완료 처리했다. RM-C2를 전환 대상으로 되돌리고 missing RID는 `RequestTargetNotFound`, known-but-disconnected RID는 `RouteNotConnected`라는 대조를 추가해야 한다. |
...
grace 미구현·reconcile 100ms 하드코딩)과 [IMP-CP-35](#상세_1)(framework가 owner lease를 join하지
...
(b) Redis를 SIGSTOP하면 즉석 probe가 실패하고 해제하면 성공한다, (c) 30초 retry가 결국 사용 가능한
...
> 현재 구현은 [§12.21](../90-implementation-gap.ko.md#1221-c-yield-terminator-부재)을 해소했다. Config 8도
...
`expect_none`·`wait_for_sequence`뿐이다. 조건·실패 종류·시간 초과를 검사하는 범용 단언은 connector
제품 API가 아니라 각 sample·E2E의 `Client/Support`가 소유한다. 아래 TH-CP-01·02는 이전 목표를
구현하고 지역 관측 helper를 제거한 이력이며, 범용 단언의 현재 배치가 완료됐다는 뜻은 아니다.

- [x] **TH-CP-01** (과거 구현) — `expect_none`·`wait_for_sequence`를 구현하고, 당시 목표였던 별도 `assertions` 유틸도 함께 구현했다. 관측 builder는 현재 계약에도 남지만 범용 단언은 제품 API의 현재 목표가 아니다. 구현과 집중 계약 테스트는 `8e355273d`에 있다.
...
- [ ] **TH-CP-03** (계약 전환) — `assertions`를 connector 제품 export에서 제거하고, 이를 사용하는 sample·E2E의 범용 단언을 각 `Client/Support`로 옮긴다. `expect_none`·`wait_for_sequence`와 그 수신 큐·timeout 정책은 connector에 유지한다.
