...
bool actor_route_state_t::is_disconnected (zlink::spot_node_t *source_node_,
                                           const zlink_routing_id_t &target_rid_) const
{
    ...
    if (source_node_->node_routing_id (&source_rid) == 0
        ...
        return false;
    return disconnected.count (std::make_pair (source_node_, routing_id_key (target_rid_))) != 0;
}

bool actor_route_state_t::active_matches (const actor_handle_t *actor_) const
{
    if (!actor_)
        return false;
    std::map<std::string, zlink_actor_route_t>::const_iterator it = active.find (actor_->actor_id);
    return it != active.end () && same_routing_id (it->second.actor.node_rid, actor_->node_rid)
...
bool actor_route_state_t::find_active (const char *actor_id_, zlink_actor_route_t *route_out_) const
{
    std::map<std::string, zlink_actor_route_t>::const_iterator it = active.find (actor_id_);
    if (it == active.end ())
        return false;
    if (route_out_)
...
void actor_route_state_t::remove_matching_active (actor_handle_t *actor_)
{
    if (!actor_)
        return;
    std::map<std::string, zlink_actor_route_t>::iterator it = active.find (actor_->actor_id);
    if (it == active.end ())
        return;
...
void actor_route_state_t::erase_disconnected_for_node (zlink::spot_node_t *node_)
{
...
void actor_route_state_t::mark_disconnected (zlink::spot_node_t *node_,
                                             const zlink_routing_id_t &target_node_rid_)
{
    disconnected.insert (std::make_pair (node_, routing_id_key (target_node_rid_)));
}
