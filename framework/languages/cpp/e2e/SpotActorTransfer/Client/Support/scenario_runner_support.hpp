...
inline void scenario_runner_t::in_flight_request_correlation ()
{
    ...
    auto join_task = std::async (
      std::launch::async, [&] { return join_actor (_nodes.a, actor_id, {"ST-F6", spot_rid}); });
...
inline straggler_setup_t scenario_runner_t::transfer_for_straggler (const std::string &scenario,
                                                                    int state_version)
{
...
inline void scenario_runner_t::local_location_commit_timing ()
{
    ...
    auto join_task = std::async (
      std::launch::async, [&] { return join_actor (_nodes.a, actor_id, {"ST-D1", spot_rid}); });
    const auto waiting =
      wait_evidence (_nodes.a, {
    ...
    wait_evidence (_nodes.a, {"ST-D1|" + actor_id + "|success_reply|" + spot_rid});
}
