...
void test_spot_node_discovery_direct_and_child_interop ()
{
    ...
    void *registry = create_started_registry_with_port_seed (ctx, &registry_seed, registry_pub,
    ...
    void *pub_discovery =
      zlink_discovery_new (ctx, ZLINK_AUTO_CONNECT_SPOT_MESH, "spot-discovery-interop");
    void *sub_discovery =
      zlink_discovery_new (ctx, ZLINK_AUTO_CONNECT_SPOT_MESH, "spot-discovery-interop");
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zlink_set_routing_id (pub_node, "z-spot-discovery-pub", 20));
    TEST_ASSERT_SUCCESS_ERRNO (zlink_set_routing_id (sub_node, "a-spot-discovery-sub", 20));

    char pub_endpoint[MAX_SOCKET_STRING];
    char sub_endpoint[MAX_SOCKET_STRING];
    int port_seed = 33100;
    TEST_ASSERT_SUCCESS_ERRNO (
      bind_spot_node_with_port_seed (pub_node, "tcp://127.0.0.1:", &port_seed, pub_endpoint));
    TEST_ASSERT_SUCCESS_ERRNO (
      bind_spot_node_with_port_seed (sub_node, "tcp://127.0.0.1:", &port_seed, sub_endpoint));

    TEST_ASSERT_SUCCESS_ERRNO (zlink_spot_node_attach_discovery (pub_node, pub_discovery));
    TEST_ASSERT_SUCCESS_ERRNO (zlink_spot_node_attach_discovery (sub_node, sub_discovery));

    TEST_ASSERT_SUCCESS_ERRNO (zlink_set_subscription (sub, "interop:node"));

    TEST_ASSERT_TRUE (wait_for_spot_node_ready_state (pub_node, ZLINK_SPOT_ROLE_PUB,
                                                      ZLINK_MONITOR_STATE_READY, 1, 10000));
    TEST_ASSERT_TRUE (wait_for_spot_node_connected_peers (pub_node, 1, 10000));
    TEST_ASSERT_TRUE (wait_for_spot_node_connected_peers (sub_node, 1, 10000));

    bool received = false;
    for (int attempt = 0; attempt < 40 && !received; ++attempt) {
        TEST_ASSERT_SUCCESS_ERRNO (
          publish_text (&zlink_publish, pub, "interop:node", "node-hop", 0));
        received = wait_for_spot_recv_message (sub, "interop:node", "node-hop", 8, 250);
    }
    TEST_ASSERT_TRUE (received);

    TEST_ASSERT_SUCCESS_ERRNO (zlink_unset_subscription (sub, "interop:node"));
