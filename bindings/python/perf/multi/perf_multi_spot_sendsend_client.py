...
def _active_slot_limit(total_slots, msg_size):
    override = os.environ.get("PERF_MULTI_SPOT_SENDSEND_ACTIVE_SLOTS")
    if override:
        try:
            return max(1, min(total_slots, int(override)))
        except ValueError:
            pass
    if msg_size >= 262144:
        return min(total_slots, 2)
    if msg_size >= 131072:
        return min(total_slots, 4)
    if msg_size >= 65536:
