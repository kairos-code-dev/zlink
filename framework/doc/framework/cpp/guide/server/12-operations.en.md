---
title: "12. Operations — Runtime Metrics · Graceful Drain · Readiness · C++"
---
...
View in another language — [C#/.NET](../../../dotnet/guide/server/12-operations.en.md) · **C++** · [Java](../../../java/guide/server/12-operations.en.md) · [Kotlin](../../../kotlin/guide/server/12-operations.en.md) · [Node/TypeScript](../../../node/guide/server/12-operations.en.md)
<!-- language-switch:end -->
...
Spots' `on_closing` with the `HostShutdown` reason, and once that callback finishes, cleans up
...
`RollingUpdate` requires a `target_application_version` greater than the source's, and uses
...
A MeshNode registered with `add_route_mesh` is operated through two DI singletons.
