...
service_provider_t::service_provider_t (
  std::shared_ptr<detail::service_registry_t> registry,
...
service_provider_t::service_provider_t (service_provider_t &&) noexcept =
  default;

service_provider_t &service_provider_t::operator= (
  service_provider_t &&) noexcept = default;

service_scope_t
service_provider_t::create_scope (service_scope_kind_t kind)
{
  if (is_closed ()) {
    throw framework_exception_t (framework_error_kind_t::shutdown,
                                 "service provider is closed");
  }
  return service_scope_t (service_provider_t (
    _registry, std::make_shared<detail::service_scope_state_t> (true, kind)));
}

void
service_provider_t::close () noexcept
{
  if (_scope) {
    _scope->closed = true;
...
bool
service_provider_t::is_closed () const noexcept
{
  return !_scope || _scope->closed;
}

std::shared_ptr<void>
service_provider_t::resolve (std::type_index type)
{
  if (is_closed ()) {
    throw framework_exception_t (framework_error_kind_t::shutdown,
                                 "service provider is closed");
...
service_scope_t::service_scope_t (service_provider_t provider)
...
service_scope_t &service_scope_t::operator= (service_scope_t &&) noexcept =
  default;

void
service_scope_t::close () noexcept
{
  _provider.close ();
}

service_scope_kind_t
service_scope_t::kind () const noexcept
{
...
service_collection_t::service_collection_t (service_collection_t &&) noexcept =
  default;

service_collection_t &service_collection_t::operator= (
  service_collection_t &&) noexcept = default;
...
{
  _registry->add (type,
                  detail::service_descriptor_t {
                    lifetime, std::move (factory), nullptr });
  return *this;
}

service_provider_t
service_collection_t::build_provider () const
{
