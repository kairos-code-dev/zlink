...
- [ ] **E2E-JV-03** (결함) — Client 주 경로 wrapper 11 : raw 0. 서버 위임·보조 경로 raw 5
...
| **E2E-JV-03** (결함) | [E2E README:43-44](../../common/e2e/README.ko.md): client는 **언어별 HTTP client wrapper**를 사용하고 raw `HttpClient`로 app endpoint를 호출하지 않는다 | **부분 해결:** Client 주 경로는 wrapper 11 : raw 0이지만, 서버 위임 코드와 보조 launcher를 포함한 전체 app에는 raw 5곳이 남아 있다. 아래 `E2E-JV-03 진행 기록`에서 검증과 잔여 경로를 추적한다. |
...
- timeout과 수명은 역할별 기존 계약을 유지했다. 대표적으로 RuntimeMonitoring은 5분,
  SpotActorTransfer는 14초 POST·5초 GET, SpotService는 2분, ResilienceLifecycle은 10분
  scenario POST·300ms health probe를 유지한다. ToActorMessaging은 30초 route-failure 결과를
...
- `ResilienceLifecycle`은 문서에 없던 `ResilienceProcessManager` raw health probe도 red gate가
  ...
  역할 프로세스를 Client가 기동하고 서버 `/scenario/<mode>`가 검증을 대신하는 문제는
  E2E-JV-05·06에 계속 남긴다.
- 전체 app을 다시 검색하면 raw 경로가 5곳 남는다. `RegistrationCodec/Client/Support/Evidence.java`,
  `RegistryMessaging/Client/Support/DynamicClusterLauncher.java`,
...
- POSD/DDD 재리뷰에서는 JDK transport 타입 누출, Shared의 server/client 책임 혼합, endpoint·상태·수명
  ...
  Shared server helper에서 Client로 옮겼다. 의미 있는 잔여 위험 신호는 위 raw 5곳과 E2E-JV-06에서
  계속 추적한다.
