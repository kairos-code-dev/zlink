...
int main (void)
{
    ...
    rc = zlink_router_recv (router, &source_node_rid, &source_spot_rid,
                            &request_seq, &request_parts, &request_count, 0);
    assert (rc == 0);
    ...
    assert (zlink_msg_size (&request_parts[0]) == strlen (k_dealer_router_request));
    assert (memcmp (zlink_msg_data (&request_parts[0]), k_dealer_router_request,
                    strlen (k_dealer_router_request))
            == 0);
    zlink_multipart_close (request_parts, request_count);
    ...
    rc = zlink_recv (dealer, &echo_rid, &reply_parts, &reply_count, 0);
    assert (rc == 0);
    assert (reply_count == 1);
    assert (zlink_msg_size (&reply_parts[0]) == strlen (k_dealer_router_reply));
    assert (memcmp (zlink_msg_data (&reply_parts[0]), k_dealer_router_reply,
                    strlen (k_dealer_router_reply))
            == 0);
    printf (
      "[dealer-router/callback] recv-only path send: \"%s\" → recv: \"%.*s\"\n",
      k_dealer_router_request, (int) zlink_msg_size (&reply_parts[0]),
      (const char *) zlink_msg_data (&reply_parts[0]));
