...
[스펙 목차](../README.ko.md) | [이전: Session Actor Dispatch](../server/31-session-actor-dispatch.ko.md) | [다음: Location Runtime](../server/40-location-runtime.ko.md)
...
서버 session callback이 받는 것과 같은 packet(header + payload)을 client에서도 동일하게
주고받게 한다.

connector는 도메인을 포함하지 않는다. 사용자가 그 위에 채팅·게임·장비 제어·알림 같은 자기
protocol을 구성한다.
...
| 상태 | 의미 |
|---|---|
| `Created` | 생성됨. 아직 연결하지 않음 |
| `Connecting` | 초기 연결 진행 중 |
| `Connected` | 연결됨 |
| `Reconnecting` | 자동 재연결 진행 중 |
| `Disconnected` | 연결이 끊김 |
| `Closed` | 닫힘. **같은 connector 객체를 다시 연결하지 않는다** |
...
| `Connecting` | 진행 중인 연결 시도를 기다린다 |
| `Connected` | 성공으로 즉시 반환한다 |
| `Reconnecting` | 자동 reconnect 결과를 기다린다 |
| `Closed` | 오류로 실패한다 |
