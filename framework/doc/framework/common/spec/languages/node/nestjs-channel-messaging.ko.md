...
[문서 목록](../../../../../README.ko.md) | [이전: ZLink Framework Node.js Interface Catalog](handler-interfaces.ko.md) | [다음: ZLink Framework NestJS SPOT Integration](nestjs-spot.ko.md)
<!-- framework-adapter-nav:end -->

[스펙 목차](../../../../node/README.ko.md)

[Node.js 묶음](../../../../node/README.ko.md) | [인터페이스](handler-interfaces.ko.md) | [SPOT](nestjs-spot.ko.md) | [STREAM](nestjs-stream.ko.md) | [Registry](nestjs-registry.ko.md)
...
- handler[^handler]를 등록하면 NestJS provider 컨테이너와 자연스럽게 연동되어야 한다.
...
[정본 샘플](../../../../node/README.ko.md) 를 참고한다.
...
이 계약에서 수동 연결은 remote `RoutingId`[^rid]를 받지 않는다. 이유는 다음과 같다.
...
- 인자 없는 `enableClient()`와 `enableSubscriber()`는 등록된 location store를 사용한다.
- 특정 channel 만 수동으로 바꾸고 싶을 때는, 그 channel builder 에서
  `.enableClient(endpoint)` 또는 `.enableSubscriber(endpoint)` 를 호출한다.
- 이때 명시한 역할만 수동으로 분류되고, 나머지는 location store discovery를 쓴다.
...
맞춘다. handler 메서드 시그니처는 Node.js 관례에 따라
`handle(request, context)`로 표현하고 `Promise`로 완료를 알린다. 일반 handler에는
`AbortSignal`을 전달하지 않는다.
...
- `ZLinkRequestContext`에서 packet 이름과 content type을 읽는다.
- 일반 handler에는 `AbortSignal`을 전달하지 않는다. request를 보내는 caller가 대기를
  중단할 수 있는 경우에만 request call에 signal을 넘긴다.
...
- channel runtime 은 host startup 단계(`onApplicationBootstrap`)에서 등록된 역할
...
로그와 counter 는 생략하지 않는다. observer callback 실패는 runtime error sink 로 분리하며 원래 reply
또는 drop 결과를 바꾸지 않는다.
