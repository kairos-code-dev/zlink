...
public class CheckoutZLinkConfig implements ZLinkFrameworkConfigurer {
    ...
    public void configure(ZLinkFrameworkOptions options) {
        ...
        options.addHandlersFromPackageOf(CheckoutApplication.class);
    }
}
```
...
public final class PlaceOrderHandler implements ZLinkRequestHandler<PlaceOrder, OrderPlaced> {
    private final ZLinkClient services;
    private final OrderRepository orders;

    @Override
    public OrderPlaced handle(PlaceOrder req, ZLinkRequestContext context) {
...
|----|-------------------|-------|
| 계약 | `.proto` + 코드 생성(CI 단계) | record DTO(공유 DTO 패키지), proto 파이프라인 없음 |
...
- 실행 코드: [Java ShoppingMall 샘플](../../../../../languages/java/samples/java/ShoppingMall)
