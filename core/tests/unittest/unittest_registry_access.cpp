...
void test_registry_status_snapshot_tracks_configured_state ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zlink_registry_status (registry, &status));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zlink_registry_add_peer (registry, "ws://peer-a"));
    TEST_ASSERT_SUCCESS_ERRNO (zlink_registry_add_peer (registry, "wss://peer-b"));

    errno = 0;
    TEST_ASSERT_NOT_EQUAL (ZLINK_CONFIG_OK, zlink_registry_add_peer (registry, "inproc://peer-c"));
