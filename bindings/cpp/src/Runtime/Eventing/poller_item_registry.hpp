  ...
  const void *socket_handle_) noexcept
{
    ...
    if (cached != socket_item_indexes_.end ()) {
        ...
        if (index < items_.size ()
            && items_[index]->source_kind == poll_source_kind_t::socket
...
inline int
find_fd_item (const std::vector<std::unique_ptr<poller_item_t> > &items_,
              int fd_) noexcept
{
    for (size_t i = 0; i < items_.size (); ++i) {
        if (items_[i]->source_kind == poll_source_kind_t::fd
            && items_[i]->fd == fd_)
            return static_cast<int> (i);
    }
    return -1;
...
inline int
find_timer_item (const std::vector<std::unique_ptr<poller_item_t> > &items_,
...
inline void rebuild_socket_item_indexes (
  const std::vector<std::unique_ptr<poller_item_t> > &items_,
