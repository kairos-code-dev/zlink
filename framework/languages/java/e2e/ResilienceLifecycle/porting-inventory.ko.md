...
경로를 유지하며, 완료되지 않은 scenario는 `feature-map.ko.md`에 gap으로 남긴다.
...
| `run_e2e.sh` | `run_e2e.sh` | runner | done | `installDist`, Redis location endpoint/key prefix 전달, Client suite 실행, marker 검증을 담당한다. provider/consumer lifecycle은 Client support가 수행한다. `logs/20260704-031956-18420`에서 통과했다. |
...
| `Client/Support/TopologyEntryResult.cs` | `Server/Consumer/src/main/java/systems/zlink/e2e/resiliencelifecycle/consumer/scenarios/ConsumerScenario.java` | support | done | Java는 public `monitoringLocationRuntimeQuery().listPeersAsync(...)` result를 Consumer role에서 검사한다. |
...
| `Client/Scenarios/RlC4RegistryOutageScenario.cs` | `Client/src/main/java/systems/zlink/e2e/resiliencelifecycle/client/Scenarios/RlC4RegistryOutageScenario.java` | scenario | not-needed | registry outage는 location store 전환 뒤 Config 5의 실행 그래프에서 제거했다. Redis store outage는 Config 6 store failure recovery가 검증한다. |
...
| `Server/Registry/Infrastructure/FaultState.cs` | 없음 | server-role | not-needed | Redis store outage는 Config 6 store failure recovery가 검증한다. |
...
- `RL-C4`: registry outage scenario는 location store 전환 뒤 Config 5 실행 그래프에서 제거했다. Redis
  store outage는 Config 6 store failure recovery가 검증한다.
