...
- [x] **TH-KT-02** (리팩토링) — DeliveryDispatch의 지역 `StatusWaits`와 독립 `waitFor` 목록을 Kotlin `waitForSequence` wrapper로 교체하고 재배정 흐름의 `PickedUp` 단언을 복구했다. 추가 재검토에서는 Bingo·TicTacToe·SpotService·AutomaticTurnDispatch에 남아 있던 AUTO dispatch의 `receivedCount(...) == 0` 여섯 곳을 동작 전에 등록하는 `expectNone(...).within(...)`으로 교체했다. Kotlin DeliveryDispatch·Bingo·TicTacToe 전체 self-check, SpotService `SM-D6`, AutomaticTurnDispatch `ATD-D4`, runner 재도입 방지 검사 통과. 구현 커밋 `22484d93e`, 추가 재검토 커밋 `9b5a8527e`(2026-07-15).
