...
// Topology: dealer(connect) -> router(bind)
...
bool run_phase (zlink::socket_t &dealer,
                zlink::socket_t &router,
                ...
                perf::single::latency_stats_t *latency_out)
{
    ...
    auto do_one = [&] () -> bool {
        ...
        if (!perf_single_metric::stamp_payload (payload.data (),
                                                ...
                                                seq++,
                                                sent_ts)
            || !send_single_payload_once (dealer, payload.data (), payload_size)) {
            return false;
        }
        if (queue_probe)
        ...
        bool header_ok = false;
        const int recv_rc = recv_header_router (
          router, payload_size, zlink::recv_flag::none, &header, &header_ok);
        if (recv_rc <= 0)
            return false;
        if (queue_probe)
        ...
        return drain_router_queue (router,
                                   payload_size,
                                   run_id,
                                   phase,
                                   msg_size,
                                ...
                                const std::string &lib_name)
{
    ...
    (void) dealer.sock ().set (zlink::socket_options::routing_id,
                               std::string ("CLIENT"));
    ...
    (void) router.sock ().set (zlink::socket_options::rcvtimeo, recv_timeout);
    (void) dealer.sock ().set (zlink::socket_options::rcvtimeo, recv_timeout);
