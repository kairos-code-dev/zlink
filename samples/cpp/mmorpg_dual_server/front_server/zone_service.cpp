...
ZoneService::ZoneService (StreamIngress &ingress) : ingress_ (ingress)
{
}
...
void ZoneService::on_client_message (const std::string &session_key, const std::string &raw)
{
    ...
    if (msg.args.empty ()) {
        ingress_.send_to_client (session_key, build_response (msg.req_id, "ERR", "empty command"));
        return;
    }
...
void ZoneService::handle_enter (const std::string &session_key,
                                ...
                                const std::vector<std::string> &args)
{
    ...
    if (!session) {
        ingress_.send_to_client (session_key, build_response (req_id, "ERR", "no session"));
        return;
    }
    ...
    std::printf ("[zone-service] ENTER: %s -> zone(%d,%d) player=%s\n", session_key.c_str (),
                 zone_x_, zone_y_, player_name.c_str ());
    std::fflush (stdout);

    // Send response to client
    std::string body =
      "zone(" + std::to_string (zone_x_) + "," + std::to_string (zone_y_) + ") entered";
...
void ZoneService::handle_move (const std::string &session_key,
                               const std::string &req_id,
                               const std::vector<std::string> &args)
{
    // args: [MOVE, x, y]
    if (args.size () < 3) {
        ingress_.send_to_client (session_key, build_response (req_id, "ERR", "MOVE requires x y"));
        return;
    }
    ...
    if (session->zone_x < 0) {
        ingress_.send_to_client (session_key, build_response (req_id, "ERR", "not in a zone"));
        return;
    }
    ...
    if (is_near_boundary (pos) && spot_handler_) {
        std::string spot_payload = "PLAYER|" + session->name + "|" + std::to_string (new_x) + "|"
                                   + std::to_string (new_y) + "|100|0";
        spot_handler_ (zone_x_, zone_y_, spot_payload);
    }
}
...
void ZoneService::handle_leave (const std::string &session_key, const std::string &req_id)
{
    ...
    std::printf ("[zone-service] LEAVE: %s from zone(%d,%d) player=%s\n", session_key.c_str (),
                 old_zx, old_zy, player_name.c_str ());
    std::fflush (stdout);
...
void ZoneService::handle_outgame (const std::string &session_key,
                                  const std::string &req_id,
                                  const std::vector<std::string> &args)
{
...
void ZoneService::on_outgame_response (const std::string &session_key,
                                       const std::string &req_id,
