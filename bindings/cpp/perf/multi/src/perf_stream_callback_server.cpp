...
inline send_status_t try_send_stream_message (pending_stream_message_t *message)
{
    ...
    if (rc < 0) {
        ...
        if (err == EAGAIN || err == EINTR || err == ENOTCONN
...
bool perf_stream_callback_server (const std::string &transport, size_t)
{
    ...
    const std::string endpoint = perf::multi::bind_and_resolve_endpoint (
      server.sock (), transport, "cpp_multi_stream_callback",
...
int main (int argc, char **argv)
{
    ...
    return perf_stream_callback_server (transport, size) ? 0 : 1;
}
