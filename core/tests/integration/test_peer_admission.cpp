...
bool bind_registry_test_endpoints_local (
...
int compare_routing_id_local (const zlink_routing_id_t *lhs_, const zlink_routing_id_t *rhs_)
{
    ...
    const int cmp = shared > 0 ? memcmp (lhs_->data, rhs_->data, shared) : 0;
    if (cmp != 0)
        return cmp;
...
bool routing_id_equals_local (const zlink_routing_id_t *lhs_, const zlink_routing_id_t *rhs_)
{
...
zlink_submit_result_t send_text_local (void *socket_, const char *text_, zlink_send_flags_t flags_)
{
    zlink_msg_t part;
...
{
    return zlink_set_router_option (router_, ZLINK_ROUTER_OPT_WEIGHT, &weight_, sizeof (weight_));
}
...
zlink_submit_result_t dealer_request_text_local (void *dealer_, const char *text_)
{
    zlink_msg_t part;
    memset (&part, 0, sizeof (part));
    init_text_part_local (&part, text_);
    return zlink_dealer_request (dealer_, &part, 1, &noop_reply_handler, NULL, ZLINK_DONTWAIT,
                                 1000);
}

zlink_submit_result_t
router_request_text_local (void *router_, const zlink_routing_id_t *rid_, const char *text_)
{
    zlink_msg_t part;
    memset (&part, 0, sizeof (part));
    init_text_part_local (&part, text_);
    return zlink_router_request (router_, rid_, &part, 1, &noop_reply_handler, NULL, ZLINK_DONTWAIT,
                                 1000);
}
                      ...
                      int timeout_ms_)
{
    ...
    const zlink_recv_result_t rc = zlink_recv (socket_, &source_rid, &parts, &part_count, 0);
    if (rc != ZLINK_RECV_OK)
        return false;
    ...
    TEST_ASSERT_EQUAL_MEMORY (expected_, zlink_msg_data (&parts[0]), strlen (expected_));
    if (source_rid_out_)
...
{
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_set_option (router_, ZLINK_OPT_RCVTIMEO, &timeout_ms_, sizeof (timeout_ms_)));
    ...
    zlink_msg_t *parts = NULL;
    size_t part_count = 0;
    const zlink_recv_result_t rc = zlink_router_recv (router_, &source_rid, &source_spot_rid,
...
bool recv_router_any_local (void *router_, int timeout_ms_)
{
    ...
    const zlink_recv_result_t rc =
      zlink_router_recv (router_, &source_rid, &source_spot_rid, &request_seq, &parts, &part_count,
...
bool recv_buffer_text_local (void *socket_, const char *expected_, int timeout_ms_)
{
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_set_option (socket_, ZLINK_OPT_RCVTIMEO, &timeout_ms_, sizeof (timeout_ms_)));
                                   ...
                                   int timeout_ms_)
{
    return zlink_test_wait_until (timeout_ms_, [=] {
        const zlink_submit_result_t rc = send_text_local (sender_, text_, ZLINK_DONTWAIT);
                                     ...
                                     int timeout_ms_)
{
    return zlink_test_wait_until (timeout_ms_, [=] {
        ...
        if (rc == ZLINK_SUBMIT_OK && recv_router_text_local (receiver_, text_, NULL, 1000))
            return true;
        return false;
    });
}
...
{
    return zlink_test_wait_until (timeout_ms_, [=] {
        const int route_rc = zlink_send (sender_, target_rid_->data, target_rid_->size,
                                         ZLINK_SNDMORE | ZLINK_DONTWAIT);
        if (route_rc >= 0) {
            const int payload_rc = zlink_send (sender_, text_, strlen (text_), ZLINK_DONTWAIT);
            if (payload_rc >= 0 && recv_buffer_text_local (receiver_, expected_source_rid_, 1000)
...
bool wait_for_socket_peer_weight_event_local (void *monitor_,
    ...
    return zlink_test_wait_until (timeout_ms_, [=] {
        ...
        if (recv_monitor_event_from_socket (monitor_, &event, ZLINK_DONTWAIT) == 0) {
            ...
            if (event.event == ZLINK_EVENT_PEER_WEIGHT_CHANGED
                && event.value == static_cast<uint64_t> (expected_weight_) && routing_id_ok) {
                return true;
            }
            return false;
        }
...
{
    return zlink_test_wait_until (timeout_ms_, [=] {
        if (send_text_rid_local (socket_, rid_, text_, ZLINK_DONTWAIT) == expected_) {
            return true;
        }
        return false;
    });
...
void send_weight_sample_local (void *sender_, int count_)
{
    for (int i = 0; i < count_; ++i) {
        ...
        for (int attempt = 0; attempt < 100 && !sent; ++attempt) {
            sent = send_text_local (sender_, "m", ZLINK_DONTWAIT) == ZLINK_SUBMIT_OK;
            if (!sent)
                msleep (1);
        }
...
void test_dealer_router_weighted_ratio_100_50 ()
{
    ...
    for (int i = 0; i < 10 && (!saw_a || !saw_b); ++i) {
        TEST_ASSERT_EQUAL_INT (ZLINK_SUBMIT_OK, send_text_local (dealer, "warmup", ZLINK_DONTWAIT));
        msleep (5);
    ...
    TEST_ASSERT_TRUE (wait_for_socket_peer_weight_event_local (dealer_monitor, &rid_b, 50, 5000));
...
void test_dealer_dealer_weighted_ratio_100_50 ()
{
    ...
    int weight_b = 50;
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_set_dealer_option (dealer_b, ZLINK_DEALER_OPT_WEIGHT, &weight_b, sizeof (weight_b)));
    TEST_ASSERT_TRUE (wait_for_socket_peer_weight_event_local (sender_monitor, &rid_b, 50, 5000));
...
void test_router_weight_blocks_dealer_outbound_and_emits_monitor_event ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (set_router_weight_local (router, 0));
    TEST_ASSERT_TRUE (
      wait_for_socket_peer_weight_event_local (dealer_monitor, &router_rid, 0, 5000));

    TEST_ASSERT_TRUE (
      wait_for_submit_result_local (dealer, "blocked", ZLINK_SUBMIT_NOT_ADMITTED, 4000));
    TEST_ASSERT_TRUE (wait_for_dealer_request_result_local (dealer, "request-blocked",
                                                            ZLINK_SUBMIT_NOT_ADMITTED, 4000));

    TEST_ASSERT_SUCCESS_ERRNO (set_router_weight_local (router, 100));
    TEST_ASSERT_TRUE (
      wait_for_socket_peer_weight_event_local (dealer_monitor, &router_rid, 100, 5000));
    msleep (SETTLE_TIME);
...
void test_router_weight_blocks_router_outbound_and_emits_monitor_event ()
{
    ...
    memset (&rid_b, 0, sizeof (rid_b));
    TEST_ASSERT_SUCCESS_ERRNO (zlink_get_routing_id (router_b, &rid_b));
    TEST_ASSERT_TRUE (wait_for_router_envelope_delivery_local (router_a, &rid_b, router_b,
                                                               "router-a", "warmup", 5000));

    TEST_ASSERT_SUCCESS_ERRNO (set_router_weight_local (router_b, 0));
    TEST_ASSERT_TRUE (wait_for_socket_peer_weight_event_local (router_a_monitor, &rid_b, 0, 5000));

    TEST_ASSERT_TRUE (wait_for_router_submit_result_local (router_a, &rid_b, "blocked",
                                                           ZLINK_SUBMIT_NOT_ADMITTED, 4000));
    TEST_ASSERT_TRUE (wait_for_router_request_result_local (router_a, &rid_b, "request-blocked",
                                                            ZLINK_SUBMIT_NOT_ADMITTED, 4000));

    TEST_ASSERT_SUCCESS_ERRNO (set_router_weight_local (router_b, 100));
    TEST_ASSERT_TRUE (
      wait_for_socket_peer_weight_event_local (router_a_monitor, &rid_b, 100, 5000));
    msleep (SETTLE_TIME);
    TEST_ASSERT_TRUE (wait_for_router_envelope_delivery_local (router_a, &rid_b, router_b,
                                                               "router-a", "after-serving", 5000));
...
void test_discovery_member_peers_reports_weight ()
{
    ...
    TEST_ASSERT_TRUE (bind_registry_test_endpoints_local (
      registry, registry_pub, sizeof (registry_pub), registry_router, sizeof (registry_router)));

    void *router_discovery =
      zlink_discovery_new (ctx, ZLINK_AUTO_CONNECT_CLIENT_SERVER, "peer-weight-socket");
    ...
    TEST_ASSERT_TRUE (
      bind_socket_test_endpoint_local (router, 50160, router_endpoint, sizeof (router_endpoint)));
    TEST_ASSERT_TRUE (
      bind_socket_test_endpoint_local (dealer, 50320, dealer_endpoint, sizeof (dealer_endpoint)));

    TEST_ASSERT_TRUE (
      wait_for_discovery_peer_weight_local (dealer_discovery, router_endpoint, 100, 15000));

    TEST_ASSERT_SUCCESS_ERRNO (set_router_weight_local (router, 0));
    TEST_ASSERT_TRUE (
      wait_for_discovery_peer_weight_local (dealer_discovery, router_endpoint, 0, 15000));
    TEST_ASSERT_SUCCESS_ERRNO (set_router_weight_local (router, 100));
    TEST_ASSERT_TRUE (
      wait_for_discovery_peer_weight_local (dealer_discovery, router_endpoint, 100, 15000));
