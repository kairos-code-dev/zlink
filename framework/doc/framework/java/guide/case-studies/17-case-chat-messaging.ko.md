...
```java
ZLinkStreamNodeBuilder stream = options.addStreamNode("chat");
...
public final class UserActor implements ZLinkActor {
    ...
    public void pushChat(ChatMessage chat) { context.boundSession().send(chat).submit(); }
}
```

```java
@Component
public final class ChatSession implements ZLinkSession {
    private final ZLinkSessionContext context;
    private final ZLinkActorManager actors;
    private ZLinkSessionActor user;

    public ChatSession(ZLinkSessionContext context, ZLinkActorManager actors) {
...
- 실행 코드: [Java SupportChat 샘플](../../../../../languages/java/samples/java/SupportChat)
