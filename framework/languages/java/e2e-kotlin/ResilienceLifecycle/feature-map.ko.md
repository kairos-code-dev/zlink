...
- `RL-D2` (전환 필요): provider dispatch-error observer가 실패해도 후속 request가 정상 처리되는지는 확인한다. 현재 `RuntimeErrorEvidenceHandler`는 application evidence handler이며 public runtime error sink가 아니다. 공통 `zlink.runtime_error`/`observer_failed`/`message_flow_observer` event를 받을 공개 표면을 먼저 확정하고 구현해야 한다.
