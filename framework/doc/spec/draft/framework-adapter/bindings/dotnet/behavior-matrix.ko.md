...
> 이 문서는 **구현 전 초안**이다.
> 즉 아직 공개 계약[^public-contract]이 아니며, `.NET` framework 구현에서
> capability[^capability] 조합별로 어떤 동작을 기대하는지, 그리고 startup
> validation[^startup-validation]에서 무엇을 거부할지를 한곳에 정리한다.
...
| `SessionProxy.Send(...)`가 오래된 binding이나 이미 닫힌 stream에 도착 | 허용된 실패 | 해당 push만 실패하고, route loop와 host shutdown은 계속 진행한다 |
...
| `IZLinkSessionProxy` 사용 + actor-session binding 없음 | 비허용 | 현재 actor에 묶인 session이 없으면 명확한 오류 |
