...
{
    const int warmup = env_int ("BENCH_WARMUP_COUNT", 1000);
    const int lat_count = env_int ("BENCH_LAT_COUNT", 500);
    const int msg_count = env_int ("BENCH_MSG_COUNT", 10000);
    const int inflight_limit = std::max (1, env_int ("BENCH_STREAM_INFLIGHT", 10));
    const int io_timeout_ms = env_int ("BENCH_STREAM_TIMEOUT_MS", 5000);
    ...
    const std::string endpoint =
      endpoint_for (transport_,
                    len32be_dispatch_ ? "stream-len32be" : "stream");
    ...
    const size_t send_size =
      len32be_dispatch_ ? payload.size () : frame.size ();
    ...
    if (rc == 0) {
        ...
        const int drain_timeout_ms =
          env_int ("BENCH_STREAM_DRAIN_TIMEOUT_MS",
                   std::max (io_timeout_ms * 6, 30000));
        if (sent > 0) {
...
int run_stream (const std::string &transport,
                size_t size,
                bool len32be_dispatch)
{
    return run_stream_mode (
      transport, size, len32be_dispatch,
      len32be_dispatch ? "STREAM_LEN32BE" : "STREAM");
}

int run_pattern_stream (const std::string &transport, size_t size)
{
    return run_stream (transport, size, false);
}

int run_pattern_stream_len32be (const std::string &transport, size_t size)
{
    return run_stream (transport, size, true);
}
