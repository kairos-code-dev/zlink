...
std::pair<std::string, std::uint16_t> parse_http_host_port (const std::string &endpoint)
{
...
bool can_connect_tcp (const std::string &host, std::uint16_t port)
{
...
void wait_for_http_listener (const std::string &endpoint, int &api_exit_code)
{
...
int main ()
{
    ...
    run_client_e2e_stream_server (server, topology.stream_endpoint, sample_names_t::x_actor_id);
