...
public final class Env {
    ...
    public static void configure(String[] args) {
        ...
        try (Reader reader = Files.newBufferedReader(Path.of(args[1]))) {
            ...
            throw new IllegalStateException("Could not load AutomaticTurnDispatch config.", error);
        }
        values = loaded;
    }
    ...
    private static Properties properties() {
        if (values == null) {
            throw new IllegalStateException("AutomaticTurnDispatch configuration was not loaded.");
        }
        return values;
    }
}
