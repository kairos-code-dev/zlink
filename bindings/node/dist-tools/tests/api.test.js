...
test('registry, discovery, and query client expose canonical service discovery flow', async () => {
    ...
    try {
        ...
        const topologyEntry = await waitFor(5000, () => (registry.topology().find((entry) => entry.channelName === 'service-found') ?? null));
        ...
        const queryEntry = await waitFor(5000, () => (query.topology({ channelName: 'service-found' }).find((entry) => (entry.channelName === 'service-found')) ?? null));
