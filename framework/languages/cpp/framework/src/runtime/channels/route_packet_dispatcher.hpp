...
class route_packet_dispatcher_t
{
  public:
    ...
    result_t<std::optional<route_dispatch_reply_t>> dispatch (const route_received_packet_t &received) const;

  private:
    result_t<std::optional<route_dispatch_reply_t>>
    dispatch_send (const route_received_packet_t &received, const runtime::messaging::envelope_header_t &header) const;

    result_t<std::optional<route_dispatch_reply_t>>
    dispatch_request (const route_received_packet_t &received,
                      const runtime::messaging::envelope_header_t &header) const;

    result_t<std::optional<route_dispatch_reply_t>> reply_error (const route_received_packet_t &received,
