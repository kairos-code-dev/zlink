...
| **H. 샘플 wire 계약** | nullable, enum 표현, 필드 drift, 계약에 없는 메시지, **교차 언어 호환** | 샘플 + 교차 언어 대조 |
...
> H(wire 계약)는 **한 언어만 고치면 오히려 깨진다** — 교차 언어로 함께 움직인다.
...
- **wire 계약(묶음 H)은 한 언어만 고치면 오히려 깨진다.** 다섯 언어를 함께 움직인다.
...
| ID | 계약 | 구현이 하는 일 |
|----|------|----------------|
| **SMP-JV-03** (미구현) | [gamequest:280](../../common/sample/event/gamequest.ko.md): "노드를 재시작해도 owner spot이 **`QuestEventStore` replay로 aggregate를 rehydrate**한다". §14(`:603`)도 rehydrate를 "노드 재시작 → replay로 aggregate 복원"으로 못박는다 | **event store가 JVM 프로세스 메모리다.** `QuestStore.java:21`의 `List<StoredQuestEvent> events`가 stream 본체이고, "replay"인 `rebuildProjection`(`:97-115`)은 그 `List`를 훑는다. Redis에는 `appendQuestEvents`(`:44,85`)로 **쓰기만** 하고, 읽는 API `RedisSampleStore.readQuestEvents()`(`:67-72`)를 부르는 곳은 **GameApi의 self-check evidence 빌더 하나뿐**(`GameQuestStore.java:130`)이다 — **owner(`QuestMission`)는 Redis stream을 한 번도 읽지 않는다.** 게다가 `StoredQuestEvent`가 `currentCount`·`status` **절대값 스냅샷**을 실어서 fold(`:110-115`)가 마지막 행 덮어쓰기로 퇴화한다. rehydrate 게이트는 `markRehydrated` **호출 횟수 카운터**(`:134-136`)를 `>= 2`로 보는 것이 전부인데(`GameQuestStore.java:173-176`), 그 카운터를 `/self-check/owner`(`Program.java:95`)**와 모든 gameplay 메시지**(`GameplayMsgHandler.java:22`)가 함께 올린다. ⇒ 노드를 재시작하면 상태가 0에서 시작하고, Redis에 그대로 있는 stream을 **아무도 읽지 않는다.** 심층은 SMP-JV-12·13·19 |
...
| **SMP-JV-05** (결함) | [tictactoe §4:46,54-56](../../common/sample/tictactoe/README.ko.md)·[§9:250-251](../../common/sample/tictactoe/README.ko.md): "TicTacToe의 payload codec은 JSON이다 … **MessagePack이나 Protobuf로 바꾸지 않는다**" | `Server/.../play/PlayServer.java:25`·`server/api/ApiServer.java:17`·`Client/.../TicTacToeClientScenario.java:256`이 전부 `ZLinkMessagePackCodec.defaultCodec()`을 등록한다. ⇒ **JSON 경로의 정본 예제가 JSON을 안 쓴다.** Bingo가 Protobuf를 맡아 codec 축을 나눠 둔 설계(`README.ko.md`의 Bingo/TicTacToe 역할 분리)가 무너진다. SMP-JV-26과 같은 항목이다 |
| **SMP-JV-06** (결함) | [공통 샘플 §메시지 이름 원칙:56-58,68](../../common/sample/README.ko.md): `Msg`는 **응답 없는 단방향 send**, `Event`는 **publish 호출에만** 쓴다. [bingo:36,808,991,1026](../../common/sample/bingo/README.ko.md)은 room reward fanout 메시지를 **`BingoRewardAcquiredEvent`**로, 그 handler를 `BingoRewardAcquiredEventHandler`(`:280,426`)로 고정한다 | `BingoRoomSpot.java:239-255` — `context.outbound().publish(SampleNames.WinnerTopic, BingoMessages.bingoWinnerMsg(...))`. **publish 메시지 이름이 `BingoWinnerMsg`**(`bingo_messages.proto:165`)이고 구독 handler도 `BingoWinnerMsgHandler`다. 정작 계약이 정한 **`BingoRewardAcquiredEvent`는 `bingo_messages.proto:174`에 선언만 되어 있고 참조가 0건**이다. ⇒ publish에 send 접미어를 달았고, 맞는 이름은 wire에 나가지 않는다. C++(SMP-CP-06)은 반대로 hpp가 `Msg`, proto가 `Event`여서 **같은 샘플 안에서 이름이 둘**이다 |
| **SMP-JV-07** (결함) | [deliverydispatch §5:191-199](../../common/sample/deliverydispatch/README.ko.md)의 프로세스 표는 `Dispatch`·`CourierSession`·`CourierSpotNode1/2`·`Tracking`·`CustomerGateway`·`Client`뿐이다. `:243-245`는 *"courier별 session route는 **별도 gateway나 registry가 아니라** 해당 courier actor가 기억한다"* | `Server/CourierGateway/`가 문서에 없는 **7번째 프로세스**로 존재하고 runner가 빌드·기동한다(`run_sample.sh:158,186`). 그런데 그 프로세스의 유일한 handler인 `BindCourierHandler`(`BindCourierReq`)에 **보내는 쪽이 트리 전체에 0건**이고, handler group `"courier-gateway"`는 등록된 어떤 channel·mesh 이름과도 일치하지 않는다. ⇒ **계약이 이름까지 짚어 금지한 역할이 죽은 채로 프로세스 하나를 차지한다.** (빈 `Server/Registry/` 디렉터리도 남아 있다 — `standalone.settings.gradle.kts:24-32`에 없다.) **그리고 relay를 건너뛰는 자리는 bind다.** [§7.2:311](../../common/sample/deliverydispatch/README.ko.md)은 CourierSession이 *"actor bind relay 때 actor 위치와 session route를 채워 actor node가 현재 session을 알게"* 한다고 규정하고, `:448-450`의 시퀀스는 `CourierSession ->> CourierActor: BindAsync and relay BindCourierSessionReq` → `CourierActor -->> CourierSession: BindCourierSessionRes` → client다. Java `CourierSession.java:65-66`은 그 packet을 가로채 `handleBindCourierSessionReq`(`:80-92`)로 보내고, actor를 bind한 뒤 **`context.client().reply(...)`로 응답을 자기가 만들어 곧장 client에 돌려준다**(`:87-91`). actor로 relay하지 않으므로 **등록돼 있는 `BindCourierSessionActorHandler`(`CourierSpotNode/spots/handlers/`)가 한 번도 호출되지 않는다** — courier actor는 자기 session route를 relay로 받는 계약 경로를 **영영 타지 않는다.** 같은 샘플의 `CustomerSession.java:59`는 relay를 **제대로 한다** — 두 session이 비대칭이다. status push(`Tracking/handlers/DeliveryStatusChangedHandler.java:34-40` → `sendToActor`)와 offer(`DispatchWorker.java:99-114` → entry spot `OfferDeliveryHandler.java:28-32` → `sendToActor`)는 정본 relay를 탄다. 그 밖에 relay를 우회하는 코드는 **역시 죽어 있는** `CustomerStatusPushHandler.java:11-27`뿐이다(프로세스 전역 `CustomerActorDirectory` HashMap(`:31-45`)에서 곧장 push하며 entry spot도 actor도 거치지 않는다 — handler group `"customer-route"`가 등록된 mesh 이름 `delivery-customers`와 다르다) |
| **SMP-JV-08** (결함) | [공통 샘플:196-197](../../common/sample/README.ko.md): "Endpoint, Redis, routing id, timeout과 로그 경로를 환경 변수나 JVM system property로 전달하지 않으며, server와 client 애플리케이션 코드에서 **직접 사용할 수 있는 환경 변수는 0개다**" | Bingo **6곳**, DeliveryDispatch **9곳**(ShoppingMall·SupportChat·TicTacToe·GameQuest는 0). `Bingo/Server/Configuration/SampleTopology.java:110,117`과 `DeliveryDispatch/Server/Configuration/SampleTopology.java:54,58`이 **토폴로지 전체**를 `System.getProperty("zlink.samples.<sample>.*")`로 읽고, `CourierSpotNodeApplication.java:36`은 **자기가 node1인지 node2인지**까지 system property로 고른다. 로그 경로는 `System.getenv("BINGO_LOG_DIR")`·`"DELIVERYDISPATCH_LOG_DIR"`이다(`PlayServerApplication.java:55`, `CourierSpotNodeApplication.java:41` 등). ⇒ 계약이 **환경 변수와 JVM system property 두 축을 모두 이름으로 짚어** 금지했는데 둘 다 그대로 쓴다 |
| **SMP-JV-09** (**실패할 수 없는 단언**) | [공통 샘플 §Client self-check 기준:358](../../common/sample/README.ko.md): "**자기 자신에게 보내면 안 되는 join notify는 받지 않았음을 확인한다**" | 그 단언의 구현이 `receivedCount(...) == 0` **네 곳**이다 — `BingoClientScenario.java:31,64`, `TicTacToeClientScenario.java:78,102`. 그런데 두 client 모두 connector를 **`ZLinkStreamDispatchMode.AUTO`**로 만든다(`Bingo/Client/.../Program.java:35`, `TicTacToeClientScenario.java:244`). AUTO에서는 `ZLinkStreamReceiveDispatcher.java:152-153`이 callback을 **곧장 실행하고 dispatch 큐에 넣지 않아** `receivedCounts`를 **아예 건드리지 않는다.** 게다가 `:136-139`는 **등록된 handler가 없는 메시지를 세지도 않고 버린다.** ⇒ Java에서 `receivedCount`는 **항상 0**이고, 릴리즈 게이트의 네 단언이 **구조적으로 실패할 수 없다.** `.NET`은 같은 단언을 쓰지만(`BingoClientScenario.cs:35,76`) 기준선엔 독립 unread-history가 있어 의미가 있다(§12.1). **약한 게이트는 더 있다.** [bingo §10:574-577](../../common/sample/bingo/README.ko.md)의 **7단계**는 card 제출 **response state**에 두 player card가 모두 9칸인지 보라는데 `BingoClientScenario.java:70,102`는 `status == Running` 하나만 보고(9칸 검사는 **종료 push**(`:134`)에만 있다), **8단계**의 "양쪽 push의 `DrawSeq`·`Number`·**state**가 같은지"에서 `:111-113`은 **state를 비교하지 않는다.** [deliverydispatch §10:681-685](../../common/sample/deliverydispatch/README.ko.md)의 상태 **순서** 요구는 `DeliveryDispatchClientScenario.java:55-61,109-115`가 status별 wait future 4개를 미리 만들고 `CompletableFuture.allOf`로 한꺼번에 기다려(`:79-82,134-137`) **`Delivered`가 `Assigned`보다 먼저 와도 전부 통과한다** — 순서를 단언하는 코드가 한 줄도 없다(`.NET` SMP-DN-08·C++ SMP-CP-37과 같은 결함) |
...
| **E2E-JV-02** (결함) | [config-2:558](../../common/e2e/config-2-spot-service.ko.md)에 `SM-F3`("한 channel에 일반 packet과 spot route packet 혼재")가 있다. [E2E README §2.8:435-447](../../common/e2e/README.ko.md)은 feature-map이 **config 문서의 모든 시나리오 ID를 행으로** 두라고 한다 | `SpotService/run_e2e.sh`의 `scenario_modes()`(`:612-659`)에 **`SM-F3` 분기가 없다** — `run_e2e.sh SM-F3`는 *"is not mapped to an implemented client mode"*로 죽는다. 단언 자체는 `Shared/.../ClientScenario.java:303-316`에 있어 `route-mesh` 모드(SM-F1/F2/F4·default-batch)에 얹혀 돌 뿐, **자기 ID로 선택·격리 실행할 수 없다.** 반대로 runner(`:58,646`)와 `feature-map.ko.md:155`는 **공통 문서에 존재하지 않는 `SM-Q9`**(e2e 문서 트리 grep **0건**)를 정식 시나리오로 실행·기록한다. ⇒ 계약 ID 하나는 실행 불가, 계약에 없는 ID 하나는 실행·통과로 집계된다 |
| **E2E-JV-03** (결함) | [E2E README:43-44](../../common/e2e/README.ko.md): "client는 **언어별 HTTP client wrapper**를 사용한다 … raw `HttpClient`로 e2e app endpoint를 직접 호출하지 않는다" | `ZLinkHttpClient`를 쓰는 config는 **`RegistryMessaging`과 `RegistrationCodec` 둘뿐**이다. 나머지는 전부 raw `java.net.http.HttpClient`다 — `PubSub/Client/Support/{ScenarioContext,PublisherClient,Evidence}.java`, `SpotService/Client/Program.java`, `RuntimeMonitoring/Client/Support/TriggerScenarioClient.java`, `ResilienceLifecycle/Client/Support/ConsumerScenarioClient.java:16`, `AutomaticTurnDispatch/Client/Support/AutomaticTurnDispatchScenarioSupport.java`, `SpotActorTransfer/Client/Program.java`, `ToActorMessaging/Shared/JsonHttp.java`, `StoreFailure/Shared/HttpSupport.java`. wrapper를 쓴다는 두 config조차 support 코드(`RegistrationCodec/Client/Support/Evidence.java`, `RegistryMessaging/Client/Support/DynamicClusterLauncher.java`)에서 raw로 새어 나간다. ⇒ **framework HTTP client 계약(timeout·redirect·retry·proxy·flow 전파)이 e2e에서 거의 실행되지 않는다** — IMP-JV-18·19가 e2e를 통과한 이유다. (체크리스트의 "21개 중 19개"라는 수는 재현하지 못했다. config 단위 실측은 wrapper 2 : raw 9다) |
...
| **E2E-JV-05** (결함) | [E2E README §2.1:157-171](../../common/e2e/README.ko.md): local readiness timeout **3초**, route settle **5초**, scenario settle **3초**. *"이 값 안에 준비되지 않는 로컬 e2e는 대기 시간을 늘려서 통과시키지 않는다 … 긴 대기는 버그를 늦게 발견하게 만들기 때문에 완료 조건으로 인정하지 않는다"* | `RuntimeMonitoring/run_e2e.sh:23-24` — `LOCAL_READINESS_ATTEMPTS=600` × `LOCAL_READINESS_POLL_SECONDS=0.1` = **60초**, 계약의 **20배**다. `PubSub/run_e2e.sh:23-25`와 `RegistryMessaging`은 20초(≈6.7배), `AutomaticTurnDispatch/run_e2e.sh:26-27`은 15초(5배)다. `SpotService/run_e2e.sh:95-97`은 기본값이 **30초**인 데다 그 기본값 자체가 **환경변수로 덮어쓸 수 있게** 선언돼 있고(`${LOCAL_READINESS_TIMEOUT_SECONDS:-30}`), `SpotActorTransfer`·`ToActorMessaging`·`ObservabilityOps`는 readiness 상수를 **선언조차 하지 않는다**(`ObservabilityOps/run_e2e.sh:112`의 인라인 `seq 1 600` = **60초**). 그리고 **route settle 상수가 11개 runner 전부에 0건**이다 — settle 축을 이름 붙인 곳은 `StoreFailure/run_e2e.sh:36`의 `SCENARIO_SETTLE_SECONDS=3` 하나뿐이다. ⇒ 느린 수렴·자동 연결 회귀가 **긴 readiness 대기에 그대로 덮인다** |
| **E2E-JV-06** (결함) | [E2E README:236](../../common/e2e/README.ko.md): `Client/Scenarios/<ScenarioId><Name>Scenario.*`는 **시나리오 ID 하나마다 파일 하나**다. [§2.5:324-329](../../common/e2e/README.ko.md)는 *"driver의 `/run` endpoint 하나를 호출하고 '나머지는 server 쪽에서 알아서 검증'하게 만들면 안 된다"*, `:58-60`은 시나리오 파일이 위임 endpoint 하나만 부르고 끝나면 안 된다고 적는다 | `StoreFailure/Client/scenarios/`의 시나리오 파일 **11개가 정확히 12줄**이고 본문이 한 줄이다(`SfD1ShortOutageRecoveryScenario.java:9-11` → `context.runStoreFailureShortOutageTraffic()`). 검증은 전부 `Client/support/ClientContext.java` **532줄** 안에 있다. `ResilienceLifecycle`·`RuntimeMonitoring`은 더 나쁘다 — 12줄 시나리오(`RlA1ProviderRestartScenario.java:9-11`)가 `Support/ConsumerScenarioClient.java:19-23`으로 **driver 서버의 `POST /scenario/<mode>`** 를 부르고, 실제 검증 본문은 **서버 쪽** `Server/Consumer/.../ConsumerScenario.java`(**959줄**)와 `Server/Trigger/.../Program.java`(**663줄**)에 있다. **가장 나쁜 건 `SpotService`다** — 51개 시나리오 파일이 전부 `gateway.runMode("<mode>")` 한 줄(`SmA1Scenario.java:9-11`)이고, 그것이 `Server/Gateway`의 **`POST /scenario/<mode>`**(`Client/Support/GatewayScenarioClient.java:19-22`)를 부른다. 요청·대기·단언 **2011줄**은 `Shared/.../ClientScenario.java`에 있고 **Gateway 서버 프로세스 안의 spot에서 실행된다**(`Shared/.../ClientDriverSpot.java:52-56`). ⇒ 계약이 이름까지 찍어 금지한 **driver 위임**이고, config-2는 client가 검증을 **한 줄도** 하지 않는다 |
| **E2E-JV-08** (미구현) | [E2E README §2.8:435-447](../../common/e2e/README.ko.md): 언어별 e2e에는 **config별 `feature-map.ko.md`를 둔다.** [§2.7:355](../../common/e2e/README.ko.md)은 `run_e2e.*`가 build·기동·client 실행을 책임진다고 규정한다 | `SpotActorTransfer/`(config-10)에 **`feature-map.ko.md`가 없다** — `run_e2e_all.sh:8-19`가 매번 돌리는 config인데 구현/gap 표가 없어 ST-* 시나리오의 상태를 문서로 확인할 수 없다. **`YieldDispatch/`는 갭이 아니다 — 리포에 없다.** `git ls-files framework/languages/java/e2e/YieldDispatch` = **0건**이다. 워킹 트리에 5,246개 파일(`build/`·`logs/`·`.gradle/`·`hs_err_pid747923.log` 크래시 덤프)이 쌓여 있지만 **전부 추적되지 않은 로컬 잔해**이고, 클론한 사람에겐 이 디렉터리가 **존재하지 않는다.** `run_e2e_all.sh`의 config 목록에도 없다. ⇒ **작업 대상이 아니다.** 다만 이 기계의 `.gitignore`가 e2e 빌드 산출물을 못 걸러 낸다는 신호이므로 지우기만 하면 된다. (`yield` 시나리오 자체의 부재는 SMP-JV-04와 [§12.21](../90-implementation-gap.ko.md)이 소유한다) |
...
| **E2E-JV-21** (미구현) | [E2E README:487-497,546-547](../../common/e2e/README.ko.md): Config 2·9의 P0는 **route mesh 없음 × session/spot 분리 배치** 조합을 실행한다 | Config 2의 Play는 `SpotService/Server/Play/.../Program.java:97-106`에서 route mesh를 **조건 없이** 등록하고 endpoint 3개를 직접 연결한다. `SpotService/run_e2e.sh:615-654`의 topology 목록에도 route-mesh를 제거하는 변형이 없다. Config 9는 이미 E2E-JV-16처럼 session gateway 자체가 없다. ⇒ 두 config 모두 요구 조합을 만들 수 없다 |
| **E2E-JV-22** (**가짜 통과**) | [config-1 RM-C2:174-182](../../common/e2e/config-1-location-messaging.ko.md)는 미존재 rid가 **public error**로 실패해야 한다. [RM-C4:194-202](../../common/e2e/config-1-location-messaging.ko.md)는 첫 실패가 **timeout**이고 느린 handler가 결국 완료됐는지까지 본다 | `RmC2TargetedRouteScenario.java:17-20`은 앱 endpoint가 돌려준 `failed` boolean만 보고 error kind를 버린다. `RmC4TimeoutIsolationScenario.java:13-25`도 `failed` 하나만 보고 1.1초 sleep 뒤 정상 reply 두 개만 검사하며, timeout kind와 느린 handler 완료 evidence는 보지 않는다. ⇒ route 없음이 임의 예외여도, slow 요청이 즉시 임의 오류여도 통과한다 |
| **E2E-JV-23** (미구현) | [config-1 RM-C8:228-238](../../common/e2e/config-1-location-messaging.ko.md): `MaxMessageSize` 근접 payload는 왕복하고 **상한 초과 payload는 public error로 거부**된 뒤 정상 request가 동작해야 한다 | `RmC8PayloadRoundTripScenario.java:19-35`는 1 B·4 KiB·256 KiB·1 MiB 네 payload의 성공 왕복과 follow-up만 검사한다. 초과 payload도 실패 단언도 없다. `feature-map.ko.md:42-44`도 이 후반부를 나중으로 미뤘는데 행과 실행은 `RM-C8`을 구현됨으로 취급한다 |
| **E2E-JV-24** (미구현) | [E2E README:514-519](../../common/e2e/README.ko.md): 표면을 넘은 actor ref는 node rid가 비어 있지 않고 **`generation > 0`**인지 어서션한다 | Config 2의 client-visible `ActorAuthRes`는 `SpotService/Shared/.../Contracts.java:199-205`에서 generation 필드 자체를 버리고, `ClientScenario.java:616-626,664-668`은 actor id·node rid만 단언한다. Java E2E production source 전체에 generation 양수 단언은 0건이다. ⇒ generation 직렬화가 0으로 퇴행해도 이 게이트는 잡지 못한다 |
