...
- [ ] **E2E-JV-05** (결함) — 6개 runner 해결. 나머지는 readiness가 최대 **20배**(60초)이고 route settle이 **5개 runner에 없다**
...
| **E2E-JV-05** (결함) | [E2E README §2.1:157-171](../../common/e2e/README.ko.md): local readiness timeout **3초**, route settle **5초**, scenario settle **3초**. *"이 값 안에 준비되지 않는 로컬 e2e는 대기 시간을 늘려서 통과시키지 않는다 … 긴 대기는 버그를 늦게 발견하게 만들기 때문에 완료 조건으로 인정하지 않는다"* | **부분 해결:** `RuntimeMonitoring`·`PubSub`·`RegistryMessaging`·`AutomaticTurnDispatch`·`RegistrationCodec`·`StoreFailure`은 readiness를 3초로 제한하고 이름 있는 5초 route settle을 적용했다. ATD는 60회 readiness client 재시도를 제거해 settle 직후 한 번의 요청만 허용하며 restart 뒤에도 같은 경로를 쓴다. 각 runner에서 기존 상한이나 route settle 부재를 잡는 gate가 먼저 실패했다. MON-A1~D1, PS-A1~B2·C1, RM-A1~A6·B1~B2·C1~C9, ATD-A1~E5, RC-A1~A6·B1~B5가 새 기본값으로 통과했다. StoreFailure 전체 실행은 SF-A1~D1까지 통과한 뒤 SF-D2 traffic stall로 한 번 실패했지만 SF-D2 단독 재실행과 뒤의 SF-D3·E1은 통과했다. RegistryMessaging의 첫 병렬 실행도 포트 예약 경합에 따른 `ZlinkBindException` 뒤 단독 전체 재실행이 통과했다. 나머지 runner는 여전히 재검증 당시 상태다. `SpotService`는 override 가능한 30초, `SpotActorTransfer`·`ToActorMessaging`·`ObservabilityOps`는 인라인 장기 wait를 사용한다. ResilienceLifecycle은 Client가 역할 process를 기동하는 E2E-JV-06 구조를 먼저 고쳐야 runner가 route settle을 소유할 수 있다. 나머지 5개 config에 route settle 상수가 없으므로 이 gap은 open이다. |
