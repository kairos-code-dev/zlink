...
> POSD 리뷰 등급: **B** — 직전 리팩토링으로 native API 제거/shadow path 정리는 완료됐고,
> 초기 진단 시점의 주요 문제는 single/multi 간 중복과 callback/recv 이중 구현이었다
...
단계 0-7 완료. `wait_for_spot_ready_settle()`와 `PERF_MULTI_SPOT_READY_SETTLE_MS` 경로가 제거되었고, direct native API grep도 0건으로 유지된다. 빌드 single 6+multi 11타겟 통과, 스모크 결과는 [perf_linux_callback_20260406_165859.txt](/home/hep7/project/kairos/zlink/bindings/cpp/perf/results/multi/report/perf_linux_callback_20260406_165859.txt) 에 저장되었다.
