...
inline bool enqueue_packet_message(session_t *session,
                                   const zlink_routing_id_t *rid,
                                   zlink_msg_t *packet)
{
                               ...
                               const zlink_msg_t *body_part)
{
    ...
    const size_t header_size = zlink_msg_size(
      const_cast<zlink_msg_t *> (header_part));
    ...
    if (header_size > 0)
        std::memcpy (dst + 6, zlink_msg_data (
                                   const_cast<zlink_msg_t *> (header_part)),
                      header_size);
    if (body_size > 0)
        std::memcpy (dst + 6 + header_size,
                                 ...
                                 const char *stop_token)
{
    ...
    size_t previous_msg_size =
      session->auto_hwm_msg_size.load (std::memory_order_acquire);
    if (body_payload_size > 0 && previous_msg_size != body_payload_size
        && session->auto_hwm_msg_size.compare_exchange_strong (
          previous_msg_size,
          body_payload_size,
                                          ...
                                          void *userdata)
{
    packet_handler_context_t *ctx =
      static_cast<packet_handler_context_t *> (userdata);
    ...
    if (!handle_packet_message (
          ctx->session, stream_, rid, header_part, body_part, ctx->stop_token)) {
        perf_stop_requested ().store (true, std::memory_order_release);
    }
