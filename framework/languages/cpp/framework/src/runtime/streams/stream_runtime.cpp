...
class stream_write_call_state_t
{
  public:
    ...
    void metadata (std::string key, std::string value) { _metadata[std::move (key)] = std::move (value); }
    ...
    result_t<void> submit ()
    {
        ...
        const auto packet_name = _packet_name.empty () ? std::string (_header->packet_name ()) : _packet_name;
        const auto header = stream_header_t (_header->kind (), _header->codec (), flags, _header->request_seq (),
...
stream_write_call_t::stream_write_call_t (stream_header_t header, zlink::message_t payload, submit_fn_t submit) :
    _state (
      std::make_shared<detail::stream_write_call_state_t> (std::move (header), std::move (payload), std::move (submit)))
{
}
...
stream_error_t::stream_error_t (stream_session_error_t error, int native_code, std::string message) :
...
stream_metadata_t::stream_metadata_t (std::map<std::string, std::string> values) : _values (std::move (values))
{
}
...
stream_write_call_t stream_t::write_packet (const stream_header_t &header, const zlink::message_t &payload)
{
    auto state = _state;
    return stream_write_call_t (
      header, payload, [state] (const stream_header_t &submitted_header, const zlink::message_t &submitted_payload) {
          if (state->closed) {
              return result_t<void>::failure (framework_error_kind_t::disconnected, "STREAM session is disconnected");
          }
...
stream_write_call_t stream_t::reply_packet (const stream_header_t &request_header, const zlink::message_t &payload)
{
    if (!request_header.request_seq ()) {
        return stream_write_call_t (result_t<void>::failure (framework_error_kind_t::request_protocol_error,
                                                             "STREAM reply requires request sequence"));
    }
    stream_header_t reply_header (stream_message_kind_t::response, request_header.codec (),
                                  stream_header_flags_t::has_request_seq, request_header.request_seq (),
                                  std::string (request_header.packet_name ()), {});
    return write_packet (reply_header, payload);
}

stream_builder_t::stream_builder_t () : _state (std::make_shared<detail::stream_builder_state_t> (""))
{
}

stream_builder_t::stream_builder_t (std::shared_ptr<detail::stream_builder_state_t> state) : _state (std::move (state))
{
}
...
zlink_builder_t &zlink_builder_t::stream (std::string stream_name, std::function<void (stream_builder_t &)> configure)
{
...
stream_runtime_t::stream_runtime_t (std::shared_ptr<stream_runtime_state_t> state) : _state (std::move (state))
{
}
...
result_t<void> stream_runtime_t::validate_header (const stream_header_t &header) const
{
    ...
    constexpr auto known_flags = static_cast<std::uint8_t> (stream_header_flags_t::has_request_seq)
    ...
    if (auto valid_name = validate_name (header.packet_name (), header.kind () == stream_message_kind_t::control);
        !valid_name) {
        return valid_name;
    }
    ...
    if ((header.kind () == stream_message_kind_t::request || header.kind () == stream_message_kind_t::response)
        && !has_request_seq) {
        return result_t<void>::failure (framework_error_kind_t::request_protocol_error,
    ...
    if (header.kind () == stream_message_kind_t::control) {
        if (header.flags () != stream_header_flags_t::none || header.codec () != stream_codec_t::raw || has_request_seq
            || has_metadata) {
            return result_t<void>::failure (framework_error_kind_t::request_protocol_error,
...
result_t<std::vector<std::uint8_t>> stream_runtime_t::encode_header (const stream_header_t &header) const
{
    if (auto valid = validate_header (header); !valid) {
        return result_t<std::vector<std::uint8_t>>::failure (valid.error_kind (), valid.error ()->what ());
    }
    ...
    if (!header.metadata ().empty ()) {
        if (header.metadata ().values ().size () > std::numeric_limits<std::uint8_t>::max ()) {
            return result_t<std::vector<std::uint8_t>>::failure (framework_error_kind_t::request_protocol_error,
...
result_t<stream_header_t> stream_runtime_t::decode_header (const std::vector<std::uint8_t> &bytes) const
{
    if (bytes.size () < 4) {
    ...
    stream_header_t header (kind, codec, flags, request_seq, std::move (name), std::move (metadata));
    if (auto valid = validate_header (header); !valid) {
        return result_t<stream_header_t>::failure (framework_error_kind_t::payload_decode_failed,
                                                   valid.error ()->what ());
    }
...
result_t<void> stream_runtime_t::dispatch_disconnected (packet_stream_session_t &session, stream_t &stream) const
{
    stream._state->closed = true;
    return dispatch_serial (stream, "disconnected", [&] { return session.on_disconnected (stream); });
}

result_t<void>
stream_runtime_t::dispatch_error (packet_stream_session_t &session, stream_t &stream, const stream_error_t &error) const
{
