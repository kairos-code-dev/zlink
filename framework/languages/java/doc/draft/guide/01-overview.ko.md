...
# Draft -- ZLink Framework for Java/Kotlin Overview

> 이 문서는 **구현 전 초안**이다.
> Spring Boot 개발자가 ZLink Framework를 읽고 바로 따라 쓸 수 있도록 설명하는
> guide 초안이다. 정식 기준은 `.NET` 코드와 Java draft 계약 문서다.
...
이 guide는 구현 전 초안이다. 따라서 실제 구현이 끝난 뒤 public API와 테스트가
확정되면 정식 guide/spec로 나눠 승격한다. 그 전까지는
[표면 매핑 정책](../internals/dotnet-to-java-surface-mapping.ko.md)과
[인터페이스 카탈로그](../handler-interfaces.ko.md)를 함께 읽는다.
