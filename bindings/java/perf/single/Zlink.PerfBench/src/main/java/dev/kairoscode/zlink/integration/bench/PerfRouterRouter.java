...
final class PerfRouterRouter {
    ...
    static int runInternal(String transport, int size, boolean usePoll) {
        int latCount = PerfUtil.parseEnv("PERF_LAT_COUNT", 1000);
        ...
        try {
            String endpoint = PerfUtil.endpointFor(transport, "router-router");
            ...
            for (int i = 0; i < 100; i++) {
                ...
                if (usePoll && !PerfUtil.waitForInput(router1, 0)) {
                    Thread.sleep(10);
                    continue;
                }
            ...
            if (usePoll && !PerfUtil.waitForInput(router2, 2000)) {
                return 2;
            }
            ...
            for (int i = 0; i < latCount; i++) {
                ...
                if (usePoll && !PerfUtil.waitForInput(router1, 2000)) {
                    return 2;
                }
            ...
            PerfUtil.printResult(usePoll ? "ROUTER_ROUTER_POLL" : "ROUTER_ROUTER", transport, size, thr, latUs);
            return 0;
        } catch (Exception e) {
            return 2;
        } finally {
