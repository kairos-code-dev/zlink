...
- .NET focused Stateful 16/16와 Redis authority 9/9, JVM core 470과 actual peer M6B 19/19, Node M6B
  35/35·M6C 32/32, C++ M6B binary가 통과했다. C++·JVM·Node post-fix review는 범위 내 추가 P0/P1 없이
  승인됐다. .NET fixed public API snapshot은 generator 출력으로 갱신했다.
- C++은 exact `spot_manager_t` call builder·`spot_ref_t` surface, Location Reserve→command 47·48,
  ...
  resolver 32/32, contract headers와 M6B runtime이 통과했다.
...
- 남은 조건은 C++ typed terminal error mapping과 source-side 즉시 reservation
  cleanup, Node native 두 process E2E, Redis endpoint가 필요한
