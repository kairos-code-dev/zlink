...
main ()
{
  ...
  auto first = runtime.dispatch_fire_count (
    timer,
    3,
    [&callback_count](const zlink::framework::timer_tick_t &tick) {
      ++callback_count;
      if (tick.name != "stage-tick") {
        throw std::runtime_error ("unexpected timer name");
      }
    });
  if (!first || callback_count != 1 ||
      first.value ().delivery_index != 1 ||
      first.value ().scheduled_index != 3 ||
      first.value ().skipped_ticks != 2 ||
      first.value ().period != 16ms ||
      first.value ().scheduled_elapsed != 48ms ||
      first.value ().started_elapsed != 48ms ||
  ...
  auto catch_up = context.add_timer<tick_handler_t> (
    "catch-up",
    10ms,
    { .overrun_policy = timer_overrun_policy_t::catch_up_bounded,
  ...
  if (!delay_tick || delay_tick.value ().scheduled_index != 1 ||
      delay_tick.value ().skipped_ticks != 0 ||
  ...
  auto stop_timer = context.add_timer<failing_tick_handler_t> (
    "stop-on-error",
    10ms,
    { .stop_on_unhandled_exception = true });
  ...
  if (failure ||
      failure.error_kind () != framework_error_kind_t::request_failed ||
      failures.size () != 1 || !failures[0].stopped ||
      failures[0].timer_name != "stop-on-error" ||
      !stop_timer.is_disposed ()) {
    return 5;
  }
  ...
  auto continue_timer = context.add_timer<failing_tick_handler_t> (
    "continue-on-error",
    10ms,
    { .stop_on_unhandled_exception = false });
  ...
  if (continue_failure ||
      continue_failure.error_kind () != framework_error_kind_t::request_failed ||
      runtime.failure_events (continue_timer).size () != 1 ||
      continue_timer.is_disposed ()) {
    return 6;
  }
  ...
  auto running_result = runtime.dispatch_fire_count (
    running_timer,
    1,
    [&runtime, &running_timer](const zlink::framework::timer_tick_t &) {
      const auto nested = runtime.dispatch_fire_count (running_timer, 1);
      if (nested ||
          nested.error_kind () != framework_error_kind_t::request_rejected) {
  ...
  runtime.cancel_all ();
  if (!timer.is_disposed () || !catch_up.is_disposed () ||
      !delay_next.is_disposed () || !continue_timer.is_disposed () ||
      !running_timer.is_disposed ()) {
    return 10;
  }

  auto closed_result = runtime.dispatch_fire_count (timer, 1);
  if (closed_result ||
      closed_result.error_kind () != framework_error_kind_t::closed) {
    return 11;
  }
