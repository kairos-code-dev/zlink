...
- **single message-socket native result fast path 적용**: perf single helper의 `send_nonblocking(...)`와
  ...
  절대 처리량 개선이 재현됐으므로 message socket send에 반영하고, PUBSUB publish에는
  적용하지 않는다. 같은 fast path를 routed send에도 적용한 기본 측정
...
- **MULTI_STREAM bytes callback 후보 기각**: public `on_packet` 의미는 유지하고 perf 전용
  private callback이 native header/body를 Python `bytes`로만 전달하면 `Message.from_bytes(...)`
