...
inline void record_ready_slot(ready_state_t *state,
                              size_t msg_size,
                              size_t slot_index)
{
    if (!state || msg_size == 0)
...
inline void record_ready_count(ready_state_t *state,
                               size_t msg_size,
                               size_t ready_count)
{
...
{
    return perf_multi_handshake::make_size_count_command(
      "READY,", msg_size, slot_index);
}

inline std::string make_ready_count_command(size_t msg_size, size_t ready_count)
{
    return perf_multi_handshake::make_size_count_command(
      "READY_COUNT,", msg_size, ready_count);
}
...
{
    return perf_multi_handshake::parse_size_count_command_bytes(
      data, size, "READY,", msg_size_out, slot_index_out);
}
...
{
    return perf_multi_handshake::parse_size_count_command_bytes(
      data, size, "READY_COUNT,", msg_size_out, ready_count_out);
}

inline bool parse_start_command(const void *data,
                                size_t size,
                                size_t *msg_size_out)
{
    if (!data)
