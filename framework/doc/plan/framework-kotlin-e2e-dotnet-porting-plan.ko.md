...
- `SpotService`: 이전 full runner와 focused `actor-session` run에서 `ActorPush`가
  `SPOT_ACTOR/HANDLER_MISSING/DROP`으로 처리되는 실패를 확인했고, 원인은 public
  `boundSession().send(...)` runtime이 stream session rid로 보내야 할 frame을 actor relay path로 다시
  보내던 문제였다. `ZLinkBoundSessionRuntime`의 send 경로를 수정한 뒤
  `logs/20260630-121648-3951366` focused `actor-session` runner에서 B/D actor-session marker와 focused
  통과 marker를 확인했고, `logs/20260630-121736-3954627` full runner에서 구현 mode와 최종
  `spot-service kotlin e2e result=passed` marker를 확인했다. `BoundSessionTest` fake backend 회귀
  테스트도 통과했다. 추가 이슈 없음.
- `PubSub`: 공통 README와 `.NET feature-map`을 대조해 Pub/Sub fanout은 subscriber 역할 server의
  bounded `/evidence/wait`를 성공 기준으로 사용할 수 있음을 확인했다. Kotlin client wait loop와
  subscriber operational endpoint를 `/evidence/wait` 기반으로 맞춘 뒤 `logs/20260630-123126-4001589`
  runner에서 `PS-A1`, `PS-A2`, `PS-A3`, `PS-A4`, `PS-B1`, `PS-B2`, `PS-C1` marker와 각 client mode의
  `pub-sub kotlin e2e result=passed`를 확인했다. `feature-map.ko.md`와 `porting-inventory.ko.md`도
  더 이상 HTTP polling gap을 과장하지 않는다. 추가 이슈 없음.
- `RegistrationCodec`: 공통 config 문서, `.NET feature-map`, `.NET` 파일 목록과 현재 Kotlin
  `feature-map.ko.md`, `porting-inventory.ko.md`, role/project tree를 대조했다. `Shared`, `Client`,
  `Server/Main`, `Server/CodecRequester`, `Server/JsonOnlyPeer`, `Server/InvalidDuplicate` role project와
  RC-A/B scenario file 대응이 현재 tree와 일치한다. `logs/20260630-123407-4013823` runner에서
  `RC-A1`, `RC-A2`, `RC-A3`, `RC-A4`, `RC-A5`, `RC-A6`, `RC-B1`, `RC-B2`, `RC-B3`, `RC-B4`,
  `RC-B5` marker와 `registration-codec kotlin e2e result=passed`를 확인했다. 추가 이슈 없음.
- `DiscoveryRegistryHa`: 공통 config 문서, `.NET feature-map`, `.NET` 파일 목록과 현재 Kotlin
  `feature-map.ko.md`, `porting-inventory.ko.md`, role/project tree를 대조했다. `Client`,
  `Server/Registry`, `Server/Provider`, `Server/Consumer`, `Server/Probe`, `Server/Embedded` role project와
  DR-A/B/C/D scenario file 대응이 현재 tree와 일치한다. `logs/20260630-123723-4022513` runner에서
  `DR-A1`, `DR-A2`, `DR-A3`, `DR-A4`, `DR-B1`, `DR-B2`, `DR-B3`, `DR-C1`, `DR-C2`, `DR-C3`,
  `DR-D1`, `DR-D2`, `DR-D3`, `DR-D4` marker와 각 client mode의
  `discovery-registry-ha kotlin e2e result=passed`를 확인했다. `consumer-DR-C3-flow.log`에는 같은 consumer
  label의 `SENT`/`REPLY_RECEIVED` 흐름이 남아 있고, `DR-D4`는 client mode provider 집합 비교 marker가
  통과했다. 추가 이슈 없음.
- `ResilienceLifecycle`: 공통 config 문서, `.NET feature-map`, `.NET` 파일 목록과 현재 Kotlin
  `feature-map.ko.md`, `porting-inventory.ko.md`, role/project tree를 대조했다. `Shared`, `Client`,
  `Server/Registry`, `Server/Provider` role project와 구현된 RL-A/B/C/D scenario file 대응이 현재 tree와
  일치한다. `logs/20260630-125201-4081089` runner에서 `RL-A1`, `RL-A2`, `RL-A3`, `RL-A5`,
  `RL-B1`, `RL-B3`, `RL-B4`, `RL-B5`, `RL-B6`, `RL-C1`, `RL-C3`, `RL-D1`, `RL-D3`, `RL-D5`
  marker와 각 client mode의 `resilience-lifecycle kotlin e2e result=passed`를 확인했다. `Server/Consumer`
  role과 `RL-A4`, `RL-B2`, `RL-C2`, `RL-C4`, `RL-D2`, `RL-D4`는 public contract/harness gap으로 남아 있고,
  현재 feature-map과 inventory가 이를 완료로 과장하지 않는다. 추가 이슈 없음.
- `RuntimeMonitoring`: 공통 config 문서, `.NET feature-map`, `.NET` 파일 목록과 현재 Kotlin
  `feature-map.ko.md`, `porting-inventory.ko.md`, role/project tree를 대조했다. `Shared`, `Client`,
  `Server/Registry`, `Server/Service`, `Server/FailoverService`, `Server/FilteredService`,
  `Server/ThrowingService`, `Server/Trigger` role project와 MON-A/B/C/D scenario file 대응이 현재 tree와
  일치한다. `logs/20260630-125539-4092618` runner에서 `MON-A1`, `MON-A2`, `MON-A3`, `MON-A4`,
  `MON-A5`, `MON-B1`, `MON-B2`, `MON-C1`, `MON-D1` marker와
  `runtime-monitoring kotlin e2e result=passed`를 확인했다. inventory 첫 문단의 `MON-A4` 누락 표기도
  runner와 scenario 표에 맞춰 고쳤다. 추가 이슈 없음.
