...
final class PerfUtil {
    ...
    private static double bandwidthMbps(String pattern, double throughput,
                                        int size) {
        boolean isEcho = "MULTI_DEALER_ROUTER".equalsIgnoreCase(pattern)
          || "MULTI_ROUTER_ROUTER".equalsIgnoreCase(pattern)
          || "MULTI_STREAM".equalsIgnoreCase(pattern)
    ...
    static int parseEnv(String name, int def) {
        ...
        if ((v == null || v.isEmpty()) && name.startsWith("PERF_")) {
            v = System.getenv("PERF_" + name.substring("PERF_".length()));
        } else if ((v == null || v.isEmpty()) && name.startsWith("PERF_")) {
            v = System.getenv("PERF_" + name.substring("PERF_".length()));
        }
        if (v == null || v.isEmpty()) {
            return def;
    ...
    static int resolveMsgCount(int size) {
        String v = System.getenv("PERF_MSG_COUNT");
        if (v != null && !v.isEmpty()) {
            try {
                int p = Integer.parseInt(v);
                if (p > 0) {
    ...
    private static synchronized void registerIpcEndpoint(String endpoint) {
        ...
        if (!ipcCleanupHooked) {
            Runtime.getRuntime().addShutdownHook(new Thread(PerfUtil::cleanupIpcPaths));
