...
가이드의 코드 예제는 시간이 지나면 실제 API와 어긋나기 쉽다(드리프트). 이 문서는
...
- 이 샘플들은 CI에서 빌드·스모크 실행된다(`run_samples.sh` 등).
...
- 이러면 가이드 코드가 **샘플에서 자동 생성**되어 드리프트가 구조적으로 불가능해진다.
- 도입하려면 추출 스크립트와 CI 단계가 필요하므로 후속 작업으로 둔다.

## 3. 검증 (회귀)

- 샘플은 CI에서 빌드·실행된다 — 가이드가 참조하는 샘플이 깨지면 CI가 잡는다.
