...
int main ()
{
    ...
    zlink::async_result_t<std::vector<zlink::message_t>> reply_future = requester.request_channel (channel_name)
    ...
    responder.join ();
    std::printf ("[spot/request/async] request: \"spot-ping\" -> reply: \"%s\"\n", reply.c_str ());
    return 0;
}
