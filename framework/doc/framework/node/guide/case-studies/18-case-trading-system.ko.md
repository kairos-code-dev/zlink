[문서 목록](../../README.ko.md) | [이전: 케이스 — 게임 채팅](17-3-case-game-chat.ko.md) | [다음: ZLink Framework Node Interface Catalog (spec)](../../spec/handler-interfaces.ko.md)
...
```text
// 매칭 핫루프는 JVM/네이티브 전용 코어에서 Disruptor·Aeron 으로 돈다.
// Node/NestJS 는 이 latency tier 를 노리지 않으므로, 이 코어는 전용 인프라로 남긴다.
disruptor.handleEventsWith(new MatchingHandler());   // 심볼당 단일 컨슈머
aeronPublication.offer(encodeSbe(tick));             // UDP 위 zero-copy 시세
fixSession.send(new NewOrderSingle(...));            // 외부 venue 연동
```
...
```ts
@zlinkSpotRequestHandler({ spot: () => SymbolBookSpot, packetName: 'SubmitOrder' })
export class SubmitOrderHandler
  implements ZLinkSpotRequestHandler<SymbolBookSpot, SubmitOrder, OrderAck> {
  async handle(
    spot: SymbolBookSpot,
    order: SubmitOrder,
...
| 주문 라우팅/리스크 | 내부 버스 | `requestToChannel`/`sendToChannel` channel |
