...
**C++ framework는 binding의 public generic timer를 사용하고 만료 event를 owner Spot mailbox에 제출한다.**
...
host 종료와 caller cancellation을 합친 `std::stop_token`을 전달한다. `submit()`은 현재 claim과 gate를
유지한 채 결과를 기다린다. `yield()`는 `SpotWide`
