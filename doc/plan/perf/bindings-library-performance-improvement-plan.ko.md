...
| `wss` | `MULTI_SPOT_SENDSEND` | `보류(27.9%)` | `보류(28.9%)` | `보류(34.3%)` | `통과(52.2%)` | `통과(50.1%)` | `통과(52.5%)` | C `perf_c_multi_linux_20260522_063910_codex_c_wss_multi_for_go_20260522.txt` 대비 Go `perf_go_multi_linux_20260522_122832_codex_go_spot_sendsend_forward_routed_selected_20260522.txt`, 131072B는 fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go `perf_go_multi_linux_20260524_195651.txt` 기준이다. C와 같은 size별 active slot 제한과 `SubmitNotConnected` 재시도 의미를 유지한다. 서버 echo는 새 public `Spot.ForwardRouted(...)`를 65536B 이상에 적용해 no-result였던 65536B를 유효 통과값으로 올렸고, 131072B도 48.5% 보류에서 50.1% 통과권으로 올렸으며, 262144B 통과권도 유지했다. small size는 기존 builder 경로를 유지한다. 이전 `Bytes(...)`/`MoveMessage(...)`/`RecvRoutedPart(...)` 후보들은 다른 size의 completion 또는 public 원본 보존 계약을 깨 반영하지 않았다. auto-HWM 활성과 size별 SpotNode `MsgUnit(B)` 일치를 확인했다. |
...
| `tls` | `MULTI_SPOT_SENDSEND` | `보류(26.5%)` | `보류(27.8%)` | `보류(28.7%)` | `보류(33.5%)` | `보류(34.9%)` | `보류(40.6%)` | C `perf_c_multi_linux_20260522_071155_codex_c_tls_multi_for_go_20260522.txt` 대비 Go `perf_go_multi_linux_20260522_122832_codex_go_spot_sendsend_forward_routed_selected_20260522.txt`. `SubmitNotConnected`는 C처럼 fatal이 아니라 다음 poll loop에서 재시도하는 의미로 처리한다. 서버 echo는 public `Spot.ForwardRouted(...)`를 65536B 이상에만 적용해 기존 no-result였던 65536B/262144B를 유효 수치로 바꿨지만, Go SPOT 최소 기준에는 아직 못 미친다. small size는 기존 builder 경로를 유지한다. 남은 개선은 consume-forward primitive만으로 해결되지 않는 encrypted routed echo submit 비용을 더 좁히는 쪽이다. auto-HWM 활성과 size별 SpotNode `MsgUnit(B)` 일치를 확인했다. |
...
- Go multi routed echo 보류의 consume-forward 1차 구현은 완료했다.
  ...
  다만 이 경로는 wss 65536B/262144B와 tls large 유효 수치 확보에만 효과가 있었고,
  2026-05-24에 `wss 131072B`도 forward 적용 범위에 포함해 48.5%→50.1% 통과권으로 올렸다.
  small size는 아직 보류다.
...
| `tcp` | `MULTI_SPOT_SENDSEND` | `보류(18.1%)` | `보류(68.6%)` | `통과(85.9%)` | `보류(60.1%)` | `보류(2.9%)` | `보류(6.2%)` | C `perf_c_multi_linux_20260522_143606_codex_c_multi_tcp_no_stream_for_rust_20260522.txt`, Rust `perf_rust_multi_linux_20260522_150220_codex_rust_multi_tcp_spot_sendsend_20260522.txt` 기준이다. 1024B 외에는 기준보다 낮고, large size는 특히 낮아 send-send echo hot path 재검토가 필요하다. |
...
| `ws` | `MULTI_SPOT_SENDSEND` | `보류(13.4%)` | `보류(12.8%)` | `보류(0.3%)` | `보류(43.1%)` | `보류(3.2%)` | `보류(6.5%)` | C 파일은 위 행과 같고, Rust `perf_rust_multi_linux_20260522_152400_codex_rust_multi_ws_spot_sendsend_20260522.txt` 기준이다. 전 size가 기준보다 낮아 send-send echo hot path 재검토가 필요하다. |
...
| `wss` | `MULTI_SPOT_SENDSEND` | `보류(21.8%)` | `보류(27.0%)` | `보류(9.2%)` | `보류(68.4%)` | `보류(68.0%)` | `보류(81.6%)` | C 파일은 위 행과 같고, Rust `perf_rust_multi_linux_20260522_154419_codex_rust_multi_wss_spot_sendsend_tls_path_20260522.txt` 기준이다. 전 size가 기준보다 낮아 send-send echo hot path 재검토가 필요하다. |
...
| `tls` | `MULTI_SPOT_SENDSEND` | `보류(9.7%)` | `보류(0.3%)` | `보류(27.0%)` | `보류(58.4%)` | `보류(65.3%)` | `보류(71.5%)` | C 파일은 위 행과 같고, Rust `perf_rust_multi_linux_20260522_155621_codex_rust_multi_tls_spot_sendsend_tls_path_20260522.txt` 기준이다. 실행 중 server/client는 각 `nlwp=14`로, SPOT 내부 worker가 양쪽 node에 붙는 고정 구조다. 전 size가 기준보다 낮아 send-send echo hot path 재검토가 필요하다. |
