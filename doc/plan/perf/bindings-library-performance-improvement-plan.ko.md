...
| 5 | Go | `bindings/go/perf` | `tcp/ws/wss/tls single 통과 (6.0.3 재검증, SPOT large 회귀 수정, latency 발산은 측정 아티팩트로 판정)` | `2026-05-25 추가 수정으로 ROUTER_ROUTER/DEALER_ROUTER/PUBSUB 전 transport 통과, DD·SPOT_REQREP 일부 large 통과, SPOT large 대폭 개선(6.6.3). 잔여 보류: SPOT_SENDSEND large, DD 일부 64B/large, SPOT_REQREP tcp 131072/262144B, 64B FFI` | 잔여 보류 추적: `SPOT_SENDSEND` request-reply echo(size별 active-slot+1ms cap) 재설계, 암호화 DD large copy 축소 |
...
| `wss` | `MULTI_DEALER_DEALER` | `보류(37.4%)` | `통과(58.3%)` | `통과(71.2%)` | `보류(42.5%)` | `보류(48.2%)` | `보류(47.5%)` | 64B는 fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go `perf_go_multi_linux_20260524_231656.txt`, 256/1024B는 Go `perf_go_multi_linux_20260524_222134.txt`, 65536B는 Go `perf_go_multi_linux_20260525_001654.txt` median 기준이다. 131072/262144B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_015713.txt` 대비 Go `perf_go_multi_linux_20260525_015819.txt` 기준이다. C처럼 단일 poll loop와 pending socket `POLLOUT` 의미를 유지했다. Go send builder에 명시적 ownership 이전 단계 `MoveMessage`를 추가했고, active send는 1024B 이하와 wss 131072B/262144B, tls 131072B에서만 이 경로를 사용한다. 131072B server 수신은 public `RecvPart` caller-owned 수신 경로로 확장해 no-code 재측정 9.894Kops/s(`perf_go_multi_linux_20260525_015918.txt`)에서 16.457Kops/s로 올렸다. 262144B까지 확대한 후보는 tls 262144B를 낮춰 반영하지 않는다(`perf_go_multi_linux_20260525_015620.txt`). 암호화 large는 아직 기준선 아래다. |
...
| `wss` | `MULTI_SPOT_SENDSEND` | `보류(39.5%)` | `보류(41.6%)` | `보류(37.5%)` | `통과(53.3%)` | `통과(50.4%)` | `통과(53.6%)` | 64/256/1024/65536/262144B는 fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go `perf_go_multi_linux_20260524_220932.txt` median 기준이다. 131072B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_020201.txt` 8.046Kops/s 대비 Go `perf_go_multi_linux_20260525_020157.txt` 4.057Kops/s 기준이다. client active loop를 C++처럼 drain-then-submit으로 바꿨고, 65536B 이하 client send는 public `MoveMessage(...)`를 사용한다. 서버 echo는 public `Spot.ForwardRouted(...)`를 65536B 이상에 적용한다. 131072B는 최신 같은 조건 재측정에서 통과선에 올라왔지만 small size는 아직 낮다. |
...
| `tls` | `MULTI_DEALER_DEALER` | `보류(38.1%)` | `통과(57.7%)` | `통과(68.1%)` | `보류(42.9%)` | `보류(46.0%)` | `보류(47.2%)` | 64B는 fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Go `perf_go_multi_linux_20260524_231656.txt`, 256/1024B는 Go `perf_go_multi_linux_20260524_222134.txt`, 65536B는 Go `perf_go_multi_linux_20260525_001654.txt` median 기준이다. 131072/262144B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_015713.txt` 대비 Go `perf_go_multi_linux_20260525_015819.txt` 기준이다. C처럼 단일 poll loop와 pending socket `POLLOUT` 의미를 유지했다. active send는 1024B 이하와 tls 131072B에서만 `MoveMessage(...)`를 사용한다. 131072B server 수신은 public `RecvPart` caller-owned 수신 경로로 확장해 no-code 재측정 17.177Kops/s(`perf_go_multi_linux_20260525_015918.txt`)에서 24.207Kops/s로 올렸다. 262144B까지 확대한 후보는 tls 262144B를 낮춰 반영하지 않는다(`perf_go_multi_linux_20260525_015620.txt`). 64B/65536B/131072B/262144B는 아직 단순 one-way 기준보다 낮다. |
...
- **2026-05-23 근본 원인 규명 + 수정 (strace 프로파일링)**: 위 large many-client 병목의 진짜 원인을 strace로 특정했다.
  ...
  - **`wss MULTI_SPOT 1024B` 보류 재확인**: full-run 14.8%는 노이즈 가능성이 있었지만, 같은 조건 제한 재측정에서도 45.3%(`perf_c_multi_linux_20260525_020305.txt`, `perf_go_multi_linux_20260525_020305.txt`)로 Go SPOT 기준 아래다. 이 셀은 노이즈 통과가 아니라 실제 보류로 둔다.
...
| `tcp` | `MULTI_SPOT` | `통과(80.7%)` | `통과(84.0%)` | `통과(87.8%)` | `보류(57.4%)` | `보류(45.0%)` | `보류(41.9%)` | 64~65536B는 Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Rust `perf_rust_multi_linux_20260525_003928.txt` median 기준이다. 131072/262144B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_015047.txt` 대비 Rust `perf_rust_multi_linux_20260525_015047.txt` median 기준이다. **2026-05-25 선택 적용**: SPOT client recv worker가 progress 없을 때 무조건 `sleep(1ms)`로 쉬던 것을 tcp 131072B와 ws 131072/262144B에서 public `Poller` `POLLIN` wait로 바꿨다. 전 size 적용 후보 `perf_rust_multi_linux_20260525_003555.txt`는 small과 ws 65536B를 낮춰 반영 범위를 좁혔다. tcp small은 여전히 통과권이고, large는 개선됐지만 같은 조건 재측정에서도 아직 SPOT 기준 아래다. **2026-05-24 multi-worker drain 추가**: copy 제거 위에 추가로, 단일 thread가 100 spot을 순차 drain하던 것을 Go recv worker처럼 scoped thread 4개로 분할(`Spot: Send`, `chunks_mut`로 disjoint 분배)했다. (러너의 symlink 기반 stale-guard 버그도 함께 수정: `readlink -f`로 실제 versioned .so와 비교.) — **2026-05-23 per-message copy 제거**: Rust SPOT client 수신 hot path가 매 메시지 payload를 `message_payload(...).to_vec()`로 전체 복사(262144B면 256KB alloc+memcpy/msg)해 large가 1~1.5%로 무너졌다. timestamp 디코드는 borrowed `&[u8]`로 충분하므로 `.to_vec()`를 제거했다. |
...
| `ws` | `MULTI_SPOT` | `보류(53.7%)` | `보류(52.9%)` | `통과(82.5%)` | `통과(76.3%)` | `보류(39.4%)` | `보류(37.3%)` | 64~65536B는 Fresh C `perf_c_multi_linux_20260523_111534_goal_c_multi_603_baseline.txt` 대비 Rust `perf_rust_multi_linux_20260525_003928.txt` median 기준이다. 131072/262144B는 같은 조건 제한 재측정 C `perf_c_multi_linux_20260525_015047.txt` 대비 Rust `perf_rust_multi_linux_20260525_015047.txt` median 기준이다. SPOT client 복사 제거 + 4-worker drain 위에, 131072B/262144B에서만 public `Poller` `POLLIN` wait를 사용한다. 1024B와 65536B는 통과권으로 올라갔지만, 131072B/262144B는 같은 조건 재측정에서도 아직 SPOT 기준 아래다. 64B/256B는 보류로 남는다. |
...
- **MULTI_PUBSUB event-slot drain 후보 기각**: C client처럼 poll event가 발생한 subscriber만 drain하도록 바꾸는 후보를 시험했다. 공식 runner `perf_rust_multi_linux_20260525_005023.txt`는 complete였지만, fresh C baseline 대비 tcp 65536B가 38.7%(기존 49.5%), ws 256B가 45.5%(기존 92.8%), ws 65536B가 63.2%(기존 86.4%)로 내려갔다. Rust binding에서는 wake 뒤 전체 subscriber를 훑어 backlog를 한 번에 비우는 기존 방식이 더 낫다.
...
- **single routed `router_recv_part` fast path 후보 기각**: C single routed receiver는 `zlink_router_recv_part`로 단일 part를 직접 받지만, Rust single routed perf는 public `RouterSocket::recv(&mut Received, ...)`를 거친다. Rust binding에 `RouterPart`/`recv_part_with_flags` 공개 후보를 얇게 추가하고 `DEALER_ROUTER`/`ROUTER_ROUTER` receiver hot path만 part 단위 수신으로 바꿔 시험했다. `cargo test --manifest-path bindings/rust/Cargo.toml --no-run`과 `cargo test --manifest-path bindings/rust/perf/single/Cargo.toml --no-run`은 통과했지만, 공식 runner `PERF_FAIL_FAST=1 bindings/rust/perf/run_benchmarks.sh --transports tcp --pattern DEALER_ROUTER,ROUTER_ROUTER --msg-sizes 65536,131072,262144 --duration 1 --runs 3`가 `DEALER_ROUTER tcp 65536B`에서 `binary_exit` partial(`perf_rust_single_linux_20260525_014415.txt`)로 끝났다. 공개 API를 늘려도 안정 실행을 확보하지 못하므로 반영하지 않는다.
...
- **남은 보류(Rust)**: SPOT tcp/ws large(copy 영역), SPOT_REQREP tcp 131072/262144B, SPOT_SENDSEND tcp 262144B, PUBSUB small(단일 thread drain).
