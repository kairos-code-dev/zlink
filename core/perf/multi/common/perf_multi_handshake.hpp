...
inline bool parse_size_command_line(const std::string &line,
                                    const char *prefix,
                                    size_t *value_out)
{
...
inline bool parse_endpoint_command_line(const std::string &line,
                                        const char *prefix,
                                          ...
                                          size_t *count_out)
{
    ...
    const unsigned long long parsed_size =
      std::strtoull(line.c_str() + prefix_len, &end, 10);
    if (!end || static_cast<size_t>(end - line.c_str()) != comma
        || parsed_size == 0) {
        return false;
    }
...
inline bool parse_size_count_command_bytes(const void *data,
                                           size_t size,
                                           const char *prefix,
                                           size_t *size_out,
                                           size_t *count_out)
{
    if (!data)
        return false;
    return parse_size_count_command_line(
...
inline bool wait_for_start(start_signal_state_t *state,
                           size_t msg_size,
                           int timeout_ms)
{
    if (!state || msg_size == 0) {
    ...
    const bool signaled = state->cv.wait_for(
      ...
      [state, msg_size]() {
          return state->stopped
                 || state->pending_sizes.find(msg_size)
                      != state->pending_sizes.end();
      });
...
inline std::string make_size_count_command(const char *prefix,
                                           size_t size_value,
                                           size_t count_value)
{
