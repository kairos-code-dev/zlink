...
- 표의 `yield` 행은 언어별 시그니처만 비교한다. 실제 호출은 `SpotWide` User Spot 또는 Instance Spot의
  shared turn에서만 유효하다. Entry Spot, `PerActor` User Spot, Actor의 독립 실행 lane과 단독 client에서는
  사용할 수 없다. 허용되지 않은 문맥에서는 HTTP operation을 제출하거나 turn을 반환하지 않고
