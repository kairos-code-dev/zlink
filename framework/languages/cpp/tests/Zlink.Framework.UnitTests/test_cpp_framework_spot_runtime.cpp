...
int main ()
{
    ...
    const auto join_dispatch = context.handlers ().invoke_actor_join (
      "join", stage_spot, actor, spot_provider, spot_serializers, zlink::message_t::from (std::string ("41")));
    if (!join_dispatch || spot_serializers.get<move_reply_t> ().deserialize (join_dispatch.value ()).value != 42
    ...
    const auto joined_dispatch = context.handlers ().invoke_post_actor_joined (
      ...
      zlink::framework::spot_actor_change_result_t (zlink::framework::spot_actor_change_kind_t::join_spot));
    if (!joined_dispatch || stage_spot.joined_count != 1 || actor.joined_value != 141
        || stage_spot.last_change_kind != zlink::framework::spot_actor_change_kind_t::join_spot) {
        return 25;
    }

    const auto left_dispatch = context.handlers ().invoke_actor_left (
      stage_spot, actor, spot_provider, spot_serializers,
      zlink::framework::spot_actor_change_result_t (zlink::framework::spot_actor_change_kind_t::join_entry_spot));
    if (!left_dispatch || stage_spot.left_count != 1 || actor.moved_value != 156
        || stage_spot.last_change_kind != zlink::framework::spot_actor_change_kind_t::join_entry_spot) {
        return 26;
    }
    ...
    try {
        ...
        invalid_actor_change_failed = error.kind () == framework_error_kind_t::request_protocol_error;
    }
