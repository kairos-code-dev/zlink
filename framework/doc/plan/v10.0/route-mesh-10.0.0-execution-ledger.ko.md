...
| S1-15A | remote `NODROP` ingress 계약 보강 | 수신 snapshot, peer lifecycle별·MeshNode 전체 bounded staging, FIFO, infrastructure 우선 처리, drain seal과 overflow 시 peer 한정 failure를 정식 spec에 명시 | 진행 중 | 2026-07-18 silent drop gap과 1차 수정의 공유 ROUTER 정지 문제를 확인. 독립 리뷰에서 exact-once 범위, FIFO 추월, 전체 staging 상한, drain 경쟁과 allocation failure 분류 문제를 추가 확인해 `core/doc/spec/core/service/01-mesh-node.*`, `03-spot.*`, `core/doc/spec/core/07-monitoring.*` 및 S3 재리뷰를 다시 개방 |
...
| S3-11 | S1-15A 변경분 독립 재리뷰 | 한영 정식 spec이 기존 NODROP 설계와 일치하고 구현 가능한지 Codex와 Claude Fable이 독립 검토 | 진행 중 | 1차 리뷰에서 FIFO 추월, drain seal 부재, MeshNode 전체 상한 부재, peer generation 누락, frame allocation 정리와 exact-once 문구 모순을 확인. 수정과 test 뒤 새 snapshot으로 재리뷰 |
...
| S4-12A | remote multicast ingress staging 구현 | 공유 ROUTER read를 막지 않는 peer lifecycle별·MeshNode 전체 bounded staging, fixed snapshot, FIFO, infrastructure 우선 처리, 공정한 재시도, overflow peer 종료와 shutdown 회귀 통과 | 수정 중 | 2026-07-18 기존 remote ingress의 silent drop과 1차 wait-loop의 설계 위반을 확인. bounded staging 구현 뒤 독립 리뷰에서 FIFO·drain·전체 예산·peer generation·RAII 결함을 확인해 S1-15A 정식 계약에 맞춰 다시 수정 중 |
...
| S8-07 | Logical Multicast와 NoDrop 연결 | 기본 true와 명시적 false 회귀 통과 | 완료 | binding `ISpot.SetNoDrop/GetNoDrop` 공개(Core spec 03-spot §6 근거, publisher SetNoDrop idiom 미러) + wrapper `ApplySpotPublisherPolicy`가 CreateSpot/GetOrCreateSpot/EntrySpot에 explicit-false 적용. 테스트가 잡은 결함 수정: `ZLinkSpotPublisherConfig.NoDrop` C# 기본 false → exact interface 기본 true로 정정. `SpotPublisherNoDropTests` 2/2(기본 true·명시적 false, native getter 검증), UnitTests 679/685(기존 추적 6건 외 회귀 0). binding 표면 추가이므로 DN lane bindings focused 재리뷰 대상에 등록 |
