...
copy_parts (const runtime::messaging::message_parts_t &parts)
{
...
auto
append_remaining_parts (TOperation operation,
...
result_t<void>
submit_failure (const char *message)
{
...
result_t<void>
native_route_backend_t::submit_send (
  ...
  const runtime::messaging::message_parts_t &parts)
{
  ...
  try {
    auto submit = std::move (_router->send (target_node_rid)).message (
      copied[0]);
...
native_route_backend_t::submit_request (
  const zlink::routing_id_t &target_node_rid,
  const runtime::messaging::message_parts_t &parts,
  std::chrono::milliseconds timeout)
{
  ...
  try {
    auto submit = std::move (_router->request (target_node_rid)).message (
      copied[0]);
    submit = append_remaining_parts (std::move (submit), copied);
