...
| **SMP-JV-07** (결함) | [deliverydispatch §5:191-199](../../common/sample/deliverydispatch/README.ko.md)의 프로세스 표는 `Dispatch`·`CourierSession`·`CourierSpotNode1/2`·`Tracking`·`CustomerGateway`·`Client`뿐이다. `:243-245`는 *"courier별 session route는 **별도 gateway나 registry가 아니라** 해당 courier actor가 기억한다"* | `Server/CourierGateway/`가 문서에 없는 **7번째 프로세스**로 존재하고 runner가 빌드·기동한다(`run_sample.sh:158,186`). 그런데 그 프로세스의 유일한 handler인 `BindCourierHandler`(`BindCourierReq`)에 **보내는 쪽이 트리 전체에 0건**이고, handler group `"courier-gateway"`는 등록된 어떤 channel·mesh 이름과도 일치하지 않는다. ⇒ **계약이 이름까지 짚어 금지한 역할이 죽은 채로 프로세스 하나를 차지한다.** (빈 `Server/Registry/` 디렉터리도 남아 있다 — `standalone.settings.gradle.kts:24-32`에 없다.) **다만 체크리스트의 "actor relay를 건너뛴다"는 살아 있는 경로에서 확인되지 않았다** — status push는 `Tracking/handlers/DeliveryStatusChangedHandler.java:34-40`이 `sendToActor(DeliveryStatusUpdatedMsg)`로 정본 relay를 타고, offer도 `DispatchWorker.java:99-114` → entry spot `OfferDeliveryHandler.java:28-32` → `sendToActor` → `OfferDeliveryActorHandler`로 간다. relay를 우회하는 코드는 **역시 죽어 있는** `CustomerStatusPushHandler.java:11-27`뿐이다(프로세스 전역 `CustomerActorDirectory` HashMap(`:31-45`)에서 곧장 push하며 entry spot도 actor도 거치지 않는다 — handler group `"customer-route"`가 등록된 mesh 이름 `delivery-customers`와 다르다) |
