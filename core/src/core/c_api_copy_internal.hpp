...
inline void copy_routing_id_from_bytes (const void *data_,
                                        size_t size_,
                                        zlink_routing_id_t *out_)
{
    ...
    const size_t copy_size =
      size_ > sizeof (out_->data) ? sizeof (out_->data) : size_;
    out_->size = static_cast<uint8_t> (copy_size);
    if (copy_size > 0 && data_)
        memcpy (out_->data, data_, copy_size);
}

inline void copy_routing_id_from_msg (const zlink_msg_t &msg_,
                                      zlink_routing_id_t *out_)
{
...
inline void copy_fixed_c_string_from_bytes (char *dst_,
                                            size_t dst_size_,
                                            const void *src_,
                                            size_t src_size_)
{
    ...
    const size_t copy_size =
      src_size_ >= dst_size_ ? dst_size_ - 1 : src_size_;
    if (copy_size > 0 && src_)
        memcpy (dst_, src_, copy_size);
    dst_[copy_size] = '\0';
}

inline void copy_fixed_c_string_from_cstr (char *dst_,
                                           size_t dst_size_,
                                           const char *src_)
{
