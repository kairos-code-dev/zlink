...
public final class ZLinkHttpClientBuilder {
    ...
    public ZLinkHttpClientBuilder maxResponseBodySize(long bytes) {
        if (bytes <= 0) {
            throw new ZLinkFrameworkException("HTTP client max response body size must be greater than zero");
        }
    ...
    public ZLinkHttpClientBuilder followRedirects(int maxRedirects) {
        if (maxRedirects <= 0) {
            throw new ZLinkFrameworkException("HTTP client follow_redirects must be greater than zero");
        }
    ...
    public ZLinkHttpClientBuilder retry(int attempts) {
        if (attempts <= 0) {
            throw new ZLinkFrameworkException("HTTP client retry attempts must be greater than zero");
        }
    ...
    public ZLinkHttpClientBuilder proxy(String url) {
        ...
        if (!url.startsWith("http://")) {
            throw new ZLinkFrameworkException("HTTP client proxy url must start with http://");
        }
        this.proxy = url;
        return this;
    }
    ...
    public ZLinkHttpClient build() {
        ...
        if (!lower.startsWith("http://") && !lower.startsWith("https://")) {
            throw new ZLinkFrameworkException("HTTP client base_url must start with http:// or https://");
        }
