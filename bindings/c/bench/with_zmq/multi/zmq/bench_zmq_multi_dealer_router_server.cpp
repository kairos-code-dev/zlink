...
inline bool relay_router_once (void *server)
{
    ...
    size_t part_count = 0;
    const int rc = ::zlink_std_compat_recv (server, &source_rid, &parts, &part_count, 0);
    if (rc < 0) {
    ...
    const int send_rc = ::zlink_std_compat_send_rid (server, &source_rid, parts, part_count, 0);
    if (send_rc >= 0)
        return true;
